"""
Slick MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Slick-specific extra)

Usage:
    slick mcp serve
    slick mcp serve --verbose
...
logger = logging.getLogger("slick.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SLICK_HOME."""
    try:
        from slick_constants import get_slick_home
        return get_slick_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SLICK_HOME", Path.home() / ".slick")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from slick_constants import get_slick_home
        directory_file = get_slick_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SLICK_HOME", Path.home() / ".slick")
...
class EventBridge:
    ...
    This is the Slick equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from slick_constants import get_slick_home
            db_file = get_slick_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SLICK_HOME", Path.home() / ".slick")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Slick MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "slick",
        instructions=(
            "Slick Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Slick MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
