"""CLI entry point for the slick-agent ACP adapter.

Loads environment variables from ``~/.slick/.env``, configures logging
...
try:
    ...
    # shadowing Slick's own modules — ``slick acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from SLICK_HOME (default ``~/.slick``)."""
    from slick_cli.env_loader import load_slick_dotenv

    slick_home = get_slick_home()
    loaded = load_slick_dotenv(slick_home=slick_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", slick_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="slick-acp",
        description="Run Slick Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Slick version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Slick provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.slick/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import SlickACPAgent  # noqa: F401

    print("Slick ACP check OK")


def _run_setup() -> None:
    from slick_cli.main import main as slick_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "slick", "model"]
        slick_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting slick-agent ACP adapter")
    ...
    import acp
    from .server import SlickACPAgent
    ...
    if os.environ.get("SLICK_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
