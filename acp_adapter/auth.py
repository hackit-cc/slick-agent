"""ACP auth helpers — detect and advertise Slick authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "slick-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Slick runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Slick can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Slick.
    ...
    not have Slick provider credentials configured yet, so Slick always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Slick using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Slick provider",
            description=(
                "Open Slick' interactive model/provider setup in a terminal. "
                "Use this when Slick has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
