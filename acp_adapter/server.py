"""ACP agent server — exposes Slick Agent via the Agent Client Protocol."""
...
from tools.approval import (
    reset_slick_interactive_context,
    set_slick_interactive_context,
)
...
try:
    from slick_cli import __version__ as SLICK_VERSION
except Exception:
    SLICK_VERSION = "0.0.0"
...
# already uses (`slick_cli/moa_cmd.py`). This bounds each provider's row, not
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Slick running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Slick cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Slick/OpenAI-compatible user content payload."""
...
class SlickACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Slick AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Slick version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Slick maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Slick inventory is also used by ``slick model``, the TUI,
        ...
        try:
            ...
            from slick_cli.models import normalize_provider, provider_label
            ...
            def choice_provider(model_id: str) -> str:
                ...
                if parts[:1] == ["custom"] and len(parts) > 1:
                    from slick_cli.models import _configured_custom_provider_ids
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from slick_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Slick estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_slick_session_id: str,
        previous_slick_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.slick.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_slick_session_id,
                previous_slick_session_id=previous_slick_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_slick_session_id: Optional[str] = None,
        previous_slick_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Slick changes it.

        When the internal Slick head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_slick_session_id`` so the
        attached ``_meta.slick.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # slick_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_slick_session_id or session_id,
            previous_slick_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["slick-acp"],
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        """
        try:
            from slick_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from slick_cli.mcp_startup import join_mcp_discovery
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="slick-agent", version=SLICK_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Slick' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Slick setup/model selection out-of-band.
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.slick`` (ACP's extensibility
        ...
        * ``containsCompactionSummary: true`` — a merged-tail message: real
        ...
        if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
            ...
            kind = "standalone"
        if kind == "standalone":
            return {"slick": {"compactionSummary": True}}
        if kind == "merged":
            return {"slick": {"containsCompactionSummary": True}}
        return None
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Slick
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Slick on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Slick' local kawaii waiting/status
        ...
        # tools.approval (set_slick_interactive_context) rather than
        # os.environ["SLICK_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue SLICK_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind SLICK_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # Without it the prompt advertises the global Slick workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.slick/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global SLICK_CRON_SESSION (#37968).
            ...
            interactive_token = set_slick_interactive_context(True)
            ...
            previous_session_id = os.environ.get("SLICK_SESSION_ID")
            ...
            try:
                ...
                if interactive_token is not None:
                    reset_slick_interactive_context(interactive_token)
                # Restore SLICK_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("SLICK_SESSION_ID", None)
                else:
                    os.environ["SLICK_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Slick DB session id before the turn so we
            ...
            pre_turn_slick_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (SLICK_SESSION_KEY in
        ...
        # _meta.slick.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_slick_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_slick_id
            and pre_turn_slick_id
            and post_turn_slick_id != pre_turn_slick_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_slick_session_id=post_turn_slick_id,
                    previous_slick_session_id=pre_turn_slick_id,
                )
            except Exception:
                logger.debug(
        ...
        # Slick' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the Slick install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Slick Agent v{SLICK_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Slick has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
