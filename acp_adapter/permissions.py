"""ACP permission bridging for Slick dangerous-command approvals."""
...
# Maps ACP permission option ids to Slick approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True,
    ...
    """Return ACP options that match Slick approval semantics."""
    ...
    # Smart DENY override — the editor must not offer a scope Slick
    ...
    if not once_only:
        options.append(PermissionOption(
            ...
            # hint while keeping Slick semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_slick(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Slick approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_SLICK.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Slick-compatible approval callback that bridges to ACP.
