...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/SophosTechne/slick-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    slick_meta = (fm.get("metadata") or {}).get("slick") or {}
    tags = slick_meta.get("tags") or []
    related = slick_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `slick skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Slick Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Slick ships with a large built-in skill library copied into `~/.slick/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Slick also syncs bundled skills on `slick update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.slick/skills/` tree, it is still shipped with Slick; restore it with `slick skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with slick-agent — install via slick skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with slick-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "slick skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "slick skills install official/blockchain/solana",
        "slick skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "slick skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
