#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Slick docs site.
...
LLM reads to learn what Slick does.

Both publish at:
  https://slick-agent.sophostechne.com/docs/llms.txt
  https://slick-agent.sophostechne.com/docs/llms-full.txt
...
SITE_BASE = "https://slick-agent.sophostechne.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Slick as a Python Library", None),
        ("guides/use-mcp-with-slick", "Use MCP with Slick", None),
        ("guides/use-voice-mode-with-slick", "Use Voice Mode with Slick", None),
        ("guides/use-soul-with-slick", "Use SOUL.md with Slick", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Slick"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Slick Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Sophos Techne. A terminal-native "
        ...
        "backends. Works with Sophos Portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/SophosTechne/"
        "slick-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/SophosTechne/slick-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Slick Agent — Full Documentation\n",
        (
            "This file is the entire Slick Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Slick, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://slick-agent.sophostechne.com/docs\n",
        "Short index: https://slick-agent.sophostechne.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
