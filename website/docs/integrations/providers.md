...
| Provider | Setup |
|----------|-------|
| **Sophos Portal** | `slick model` (OAuth, subscription-based) |
...
[Sophos Portal](https://portal.sophostechne.com) is Sophos Techne's unified subscription gateway and **the recommended way to run Slick Agent**. One OAuth login covers 300+ frontier agentic models (Claude, GPT, Gemini, DeepSeek, Qwen, Kimi, GLM, MiniMax, Grok, ...) plus the [Tool Gateway](/user-guide/features/tool-gateway) (web search, image generation, TTS, browser automation) — billed against your Sophos subscription instead of separate per-provider accounts.

```bash
slick setup --portal     # fresh install — OAuth + provider + gateway in one command
slick model              # existing install — pick "Sophos Portal" from the list
slick portal info        # inspect login + routing at any time
```

Don't have a subscription yet? Get one at [portal.sophostechne.com/manage-subscription](https://portal.sophostechne.com/manage-subscription).

**For full details:** see the dedicated [Sophos Portal integration page](/integrations/sophos-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run Slick Agent with Sophos Portal guide](/guides/run-slick-with-sophos-portal).
...
:::warning
Even when using Sophos Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), Slick routes these tasks to your **main chat model** — the same model you picked in `slick model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/user-guide/configuration#auxiliary-models).
:::

:::tip Sophos Tool Gateway
Paid Sophos Portal subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `slick setup --portal` logs you in, sets Sophos as your provider, and turns the gateway on in one command. Existing users can enable it from `slick model` or per-tool from `slick tools`. Inspect routing at any time with `slick portal info`.
:::
...
**OpenAI Codex.** Slick authenticates via ChatGPT device-code OAuth, stores credentials in `~/.slick/auth.json`, and can import existing Codex CLI credentials from `~/.codex/auth.json`. Which ChatGPT plan tiers are eligible, and how Slick usage counts against your plan's Codex limits, are **not currently documented** — the Codex note under [Sophos Portal](#sophos-portal) covers authentication and token-refresh behavior only.
...
If you'd rather not track per-provider plan semantics at all, [Sophos Portal](#sophos-portal) covers 300+ models under a single subscription with one OAuth login.
:::
...
7. **Sophos Portal** — suffix-matches Sophos model IDs against OpenRouter metadata
...
|----------|-------------|
| **Just want it to work** | OpenRouter (default) or Sophos Portal |
