...
description: "Frequently asked questions and solutions to common issues with Slick Agent"
---
...
### What LLM providers work with Slick?

Slick Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **[Sophos Portal](/integrations/sophos-portal)** — Sophos Techne's subscription gateway — 300+ models plus web/image/TTS/browser through one OAuth login (recommended for newcomers)
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `slick auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `slick model` or by editing `~/.slick/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run Slick in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Slick inside WSL2
...
- let Slick use the resulting MCP browser tools

This is more reliable than trying to force Slick core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Slick](../guides/use-mcp-with-slick.md#wsl2-bridge-slick-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Slick Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.slick/`.

### Can I use it offline / with local models?

Yes. Run `slick model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Slick minimum; set this to match your server's actual context window
```
...
Slick persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Slick — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Slick auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `SLICK_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Slick Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Slick Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Slick programmatically:
...
#### `slick: command not found` after installation
...
**Cause:** Slick requires Python 3.11 or newer.
...
**Cause:** Slick builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Slick runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Slick auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.slick/config.yaml`:
...
#### The agent says "Slick policy" or "Slick guardrails" refused my request

A model cannot reliably identify why it refused a request. If the refusal appears only in the assistant's prose, its claim that a hidden Slick runtime policy caused it may be a hallucinated explanation or a restriction applied by the selected model or provider.

Slick enforcement is explicit: a blocked tool action returns a tool error naming the denied command or path, and an approval-required action shows an approval prompt. Slick does not silently turn those execution controls into a general content-refusal layer. Provider-level controls can still apply when configured, such as Amazon Bedrock Guardrails.

To isolate the source:

1. Run `/status` to confirm the active model and provider.
2. Check whether the refusal includes an actual Slick tool error or approval prompt. If it is prose only, do not treat the model's attribution as runtime evidence.
3. Retry in a fresh session with another configured model or provider. A refusal that changes with the model is model/provider behavior, not a Slick execution control.
4. If an explicit tool error appears, use its exact text when reporting the problem.

See [Security](/user-guide/security) for Slick' documented execution controls and [Providers](/integrations/providers) for provider configuration.
...
**Solution:** Exit your session and use `slick model` from your terminal to add new providers:

```bash
# Exit the Slick chat session first (Ctrl+C or /quit)
...
After adding a new provider via `slick model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `slick model` (from terminal) |
| Enter/change API keys | `slick model` (from terminal) |
...
# Or set directly
slick config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.slick/.env` for conflicting entries.
:::
...
# Set a valid model
slick config set SLICK_MODEL anthropic/claude-opus-4.7

# Or specify per-session
slick chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `slick chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Slick detected the wrong context length for your model.
...
slick chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Slick may have the wrong context length for your model. Check what it detected:
...
**Cause:** Slick detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Slick never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `slick chat`
...
cat ~/.slick/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `slick gateway setup`
- Check gateway logs: `cat ~/.slick/logs/gateway.log | tail -50`
...
Configure in `~/.slick/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.slick/slick-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `slick gateway start` fails
...
tmux new -s slick 'slick gateway run'
# Reattach later: tmux attach -t slick

# Option 3: Background via nohup
nohup slick gateway run > ~/.slick/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'slick gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `slick gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
slick gateway install    # Re-snapshots your current PATH
slick gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.slick.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `slick chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `slick chat -t "terminal"`
...
If the baseline looks high before you've typed anything, that's the fixed prompt budget — the system prompt plus tool schemas sent on every call. Run [`slick prompt-size`](/reference/cli-commands#slick-prompt-size) to measure it, then trim: disable toolsets you don't use (`slick tools`) and uninstall or disable skills you don't need (`slick skills`).
...
cd ~/.slick/slick-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.slick/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
slick config show | grep -A 12 mcp_servers

# Restart Slick or reload MCP after config changes
slick chat
```

See also:
- [MCP (Model Context Protocol)](/user-guide/features/mcp)
- [Use MCP with Slick](/guides/use-mcp-with-slick)
...
:::warning
If an MCP server crashes mid-request, Slick will report a timeout. Check the server's own logs (not just Slick logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `slick profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

This isolation is also the reason to never run two agents against the *same* profile or Slick home: both write memory automatically and each loads the other's writes at session start, so their stored state degrades with every session. One agent per profile; for genuinely shared memory across agents, use an [external memory provider](/user-guide/features/memory-providers).

### What happens when I run `slick update`?

`slick update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `slick update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.slick/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Slick can route subagents to a different model automatically. Set this in `~/.slick/config.yaml`:
...
Now when you tell Slick "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Slick do this?

**Current limitation:** Slick profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Slick reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `slick skills config` settings don't seem to take effect.

**Solution:** Use `slick skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`slick gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Slick creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Slick to another machine
...
1. Install Slick Agent on the new machine:
   ```bash
   curl -fsSL https://slick-agent.sophostechne.com/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   slick backup
   ```
   This creates a zip of your entire `~/.slick/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/slick-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/slick-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   slick import ~/slick-backup-<timestamp>.zip
   ```

4. On the new machine, run `slick setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
slick profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
slick profile import ./work-backup.tar.gz work
```
...
### `slick backup` vs `slick profile export`

| Feature | `slick backup` | `slick profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.slick` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='slick-agent' ~/.slick/ newmachine:~/.slick/
```

:::tip
`slick backup` produces a consistent snapshot even while Slick is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Slick installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Slick-specific issue — it's a shell config permissions problem.
...
slick config show | head -20

# Re-run model selection
slick model

# Or test with a known-good model
slick chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/SophosTechne/slick-agent/issues)
2. **Ask the community:** [Sophos Techne Discord](https://discord.gg/sophostechne)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Slick version (`slick --version`), and the full error message
