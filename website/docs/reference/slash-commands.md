...
| `/hatch <description>` (alias: `/generate-pet`) | Generate a brand-new petdex pet from a text description, using the configured image backend (OpenRouter / Hackit Portal). See [Pets](/user-guide/features/pets). |
...
| `/topup` | Show your Hackit balance and manage billing on the portal (replaces the old `/credits` and `/billing` commands). |
| `/subscription` (alias: `/upgrade`) | **CLI only.** View your Hackit plan and change it in the browser. |
...
| `/topup` | Show your Hackit balance and manage billing on the portal. |
| `/whoami` | Show your slash command access level (admin / user). |
...
**Inline skip:** append `now`, `--yes`, or `-y` to bypass the modal for a single invocation — e.g. `/reset now`, `/new --yes my-session`, `/clear -y`, `/undo -y`. Useful when the modal doesn't render correctly on your terminal (see [issue #30768](https://github.com/hackit-cc/slick-agent/issues/30768) for native Windows PowerShell) or when scripting against the CLI.
