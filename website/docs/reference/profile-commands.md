...
This page covers all commands related to [Slick profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `slick profile` without a subcommand shows help.
...
```bash
slick profile use <name>
```

Sets `<name>` as the active profile. All subsequent `slick` commands (without `-p`) will use this profile.
...
slick profile use default
```

## `slick profile create`

```bash
slick profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `slick profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `slick update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.slick`), use `slick skills opt-out` / `slick skills opt-in`. |
...
slick profile create work2 --clone-from work

# Clone everything from a specific profile
slick profile create work2-backup --clone-from work --clone-all
```

## `slick profile describe`

```bash
slick profile describe [<name>] [options]
```
...
slick profile describe researcher

# Set it explicitly
slick profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
slick profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
slick profile describe --all --auto
```

## `slick profile delete`

```bash
slick profile delete <name> [options]
```
...
```bash
slick profile delete mybot
slick profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.slick`) cannot be deleted — use `slick uninstall` to remove everything.
:::

## `slick profile show`

```bash
slick profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Slick home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ slick profile show work
Profile: work
Path:    ~/.slick/profiles/work
...
```bash
slick profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Slick installation.
...
```bash
slick profile alias work
# Creates/updates ~/.local/bin/work

slick profile alias work --name mywork
# Creates ~/.local/bin/mywork

slick profile alias work --remove
# Removes the wrapper script
```

## `slick profile rename`

```bash
slick profile rename <old-name> <new-name>
```
...
```bash
slick profile rename mybot assistant
# ~/.slick/profiles/mybot → ~/.slick/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `slick profile export`

```bash
slick profile export <name> [options]
```
...
```bash
slick profile export work
# Creates work.tar.gz in the current directory

slick profile export work -o ./work-2026-03-29.tar.gz
```
...
## `slick profile import`

```bash
slick profile import <archive> [options]
```
...
```bash
slick profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

slick profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `slick -p telemetry`). |
...
slick profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
slick profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
slick profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
slick profile install ./telemetry/
```

### `slick profile update`

```bash
slick profile update <name> [--force-config] [--yes]
```
...
```bash
slick profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Slick version, author, env var requirements, the source URL/path, and
...
`slick profile list` also shows the distribution name and version in a
`Distribution` column, and `slick profile show <name>` / `delete <name>`
...
slick profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
slick profile install https://github.com/your-org/internal-assistant.git
```
...
`slick_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Slick version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Slick can clone from.
3. Tell recipients to run `slick profile install <your-repo-url>`.
...
## `slick -p` / `slick --profile`

```bash
slick -p <name> <command> [options]
slick --profile <name> <command> [options]
```

Global flag to run any Slick command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
slick -p work chat -q "Check the server status"
slick --profile dev gateway start
slick -p personal skills list
slick -p work config edit
```

## `slick completion`

```bash
slick completion <shell>
```
...
- `slick profile <TAB>` — subcommands (list, use, create, etc.)
- `slick profile use <TAB>` — profile names
- `slick -p <TAB>` — profile names
