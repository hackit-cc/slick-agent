...
| `HACKIT_BASE_URL` | Override Hackit Portal base URL (rarely needed; development/testing only) |
| `HACKIT_INFERENCE_BASE_URL` | Override Hackit inference endpoint directly |
...
| Variable | Description |
|----------|-------------|
| `SLICK_PORTAL_BASE_URL` | Override Hackit Portal URL (for development/testing) |
| `HACKIT_INFERENCE_BASE_URL` | Override Hackit inference API URL |
| `SLICK_HACKIT_MIN_KEY_TTL_SECONDS` | Min agent key TTL before re-mint (default: 1800 = 30min) |
| `SLICK_HACKIT_TIMEOUT_SECONDS` | HTTP timeout for Hackit credential / token flows |
...
These variables configure the [Tool Gateway](/user-guide/features/tool-gateway) for paid Hackit subscribers or self-hosted gateway deployments. Most users don't need to set these — the gateway is configured automatically via `slick model` or `slick tools`.

| Variable | Description |
|----------|-------------|
| `TOOL_GATEWAY_DOMAIN` | Base domain for Tool Gateway routing (default: `hackit.cc`) |
| `TOOL_GATEWAY_SCHEME` | HTTP or HTTPS scheme for gateway URLs (default: `https`) |
| `TOOL_GATEWAY_USER_TOKEN` | Auth token for the Tool Gateway (normally auto-populated from Hackit auth) |
...
| `SLICK_DASHBOARD_OAUTH_CLIENT_ID` | OAuth client id (`agent:{instance_id}`) for the gated/public dashboard, activating the Hackit (`plugins/dashboard_auth/hackit`) provider. Overrides `dashboard.oauth.client_id`. Provision it with `slick dashboard register`. |
