...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `slick send`."
---

# Pipe Script Output to Messaging Platforms

`slick send` is a small, scriptable CLI that pushes a message to any
messaging platform Slick is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | slick send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `slick
...
slick send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
slick send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
slick send --list

# Filter by platform
slick send --list telegram
```
...
Any platform Slick ships adapters for works as a target:
...
`slick send` resolves the message body in this order:

1. **Positional argument** — `slick send --to telegram "hi"`
2. **`--file PATH`** — `slick send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | slick send --to telegram`

When stdin is a TTY (no pipe), Slick does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  slick send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `slick send` reuses your Slick config, the same script works on
any host where Slick is installed — no need to export bot tokens into
...
`slick send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  slick send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | slick send \
...
```bash
./train.py --epochs 200 && \
  slick send --to telegram "training done" || \
  slick send --to telegram "training failed (exit $?)"
```
...
slick send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(slick send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `slick send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `slick send` calls
the platform's REST endpoint directly using credentials from
`~/.slick/.env` and `~/.slick/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `slick gateway start` and retry.
...
# Machine-readable
slick send --list --json
```

The listing is built from `~/.slick/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`slick gateway
...
| Approach | Multi-platform | Reuses Slick creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `slick send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`slick send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `slick send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `slick send` shares with cron delivery.
