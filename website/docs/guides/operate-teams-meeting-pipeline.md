...
```bash
slick teams-pipeline token-health
slick teams-pipeline token-health --force-refresh
```
...
```bash
slick teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
slick teams-pipeline maintain-subscriptions
slick teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Slick cron (recommended if you already run the Slick gateway)

Slick ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.slick/scripts/`. First create the script:

```bash
mkdir -p ~/.slick/scripts
cat > ~/.slick/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec slick teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.slick/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
slick cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/slick-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Slick Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=slick
EnvironmentFile=/etc/slick/env
ExecStart=/usr/local/bin/slick teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/slick-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Slick Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now slick-teams-pipeline-maintain.timer
systemctl list-timers slick-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/slick teams-pipeline maintain-subscriptions >> /var/log/slick/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.slick/.env` at the top of a wrapper script that crontab calls.
...
```bash
slick teams-pipeline subscriptions   # should show expirationDateTime advanced
slick teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
slick teams-pipeline list
slick teams-pipeline list --status failed
slick teams-pipeline show <job-id>
```

### Replay a stored job

```bash
slick teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
slick teams-pipeline fetch --meeting-id <meeting-id>
slick teams-pipeline fetch --join-web-url "<join-url>"
slick teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `slick teams-pipeline maintain-subscriptions --dry-run`
- inspect `slick teams-pipeline list --status failed`
...
- run `slick teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `slick teams-pipeline run`
...
- [ ] `slick teams-pipeline validate` returns an OK snapshot
- [ ] `slick teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Slick cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
