...
slick cron create "0 2 * * *" \
  "You are a project manager triaging the Hackit/slick-agent GitHub repo.

1. Run: gh issue list --repo Hackit/slick-agent --state open --json number,title,labels,author,createdAt --limit 30
...
slick cron create "0 9 * * 1" \
  "Scan the Hackit/slick-agent repo for documentation drift.

1. Run: gh pr list --repo Hackit/slick-agent --state merged --json number,title,files,mergedAt --limit 30
