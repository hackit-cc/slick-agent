...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Slick Agent — no API key required"
---

# MiniMax OAuth

Slick Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Slick automatically refreshes your session.
...
# → Slick opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.slick/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.slick/.env
```
...
```bash
slick auth add minimax-oauth --no-browser
```

Slick will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Slick implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Slick generates a PKCE verifier / challenge pair and a random state value.
...
4. Slick polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.slick/auth.json` under the `minimax-oauth` key.
...
```bash
slick config set model.default MiniMax-M2.7
slick config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.slick/config.yaml` will contain entries similar to:
...
```bash
slick --provider minimax-oauth    # canonical
slick --provider minimax-portal   # alias
slick --provider minimax-global   # alias
slick --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `slick setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
slick --provider minimax-oauth
```
...
Slick refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Slick marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Slick marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `slick auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Slick raises a timeout error.

**Fix:** re-run `slick auth add minimax-oauth` (or `slick model`). The flow starts fresh.

### State mismatch (possible CSRF)

Slick detected that the `state` value returned by the authorization server does not match what it sent.
...
If `slick` cannot open a browser window, use `--no-browser`:

```bash
slick auth add minimax-oauth --no-browser
```

Slick prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `slick model` and select MiniMax (OAuth), or run `slick auth add minimax-oauth`.
...
```bash
slick auth logout minimax-oauth
```
...
- [slick doctor](../reference/cli-commands.md)
