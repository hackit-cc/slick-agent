---
sidebar_position: 1
title: "Run Slick Agent with Sophos Portal"
description: "Start-to-finish walkthrough: subscribe, set up, switch models, enable gateway tools, and verify routing"
---

# Run Slick Agent with Sophos Portal

This guide walks you through running Slick Agent on a [Sophos Portal](https://portal.sophostechne.com) subscription end to end — from signing up to verifying that every tool routes correctly. If you just want the overview of what the Portal is and what's in the subscription, see the [Sophos Portal integration page](/integrations/sophos-portal). This page is the task script.
...
Open [portal.sophostechne.com/manage-subscription](https://portal.sophostechne.com/manage-subscription), sign up, and pick a plan.
...
3. Sets `model.provider: sophos` in `~/.slick/config.yaml`
...
If any line shows something other than "via Sophos Portal" or the auth line says "not logged in", jump to [Troubleshooting](#troubleshooting) below.
...
The Portal's own [info page](https://portal.sophostechne.com/info) carries this warning too — it's the official Sophos guidance, not just a Slick-side opinion.

## 6. (Optional) Customize Tool Gateway routing

The gateway is opt-in per tool, not all-or-nothing. If you already have a Browserbase account and want to keep using it while routing web search and image generation through Sophos, that's supported:

```bash
slick tools
# → Web search       → "Sophos Subscription"     (recommended)
# → Image generation → "Sophos Subscription"     (recommended)
# → Browser          → "Browserbase"           (your existing key)
# → TTS              → "Sophos Subscription"     (recommended)
```

These rows appear in `slick tools` even before you've logged into Sophos Portal — if you pick "Sophos Subscription" without an active session, Slick runs the Portal login inline (without changing your inference provider or your other tools).
...
You'll see per-tool routing — `via Sophos Portal` for the ones routed through the subscription, and the partner name (`browserbase`, `firecrawl`, etc.) for the ones using your own keys.
...
```bash
slick setup tts
# → pick "Sophos Subscription" for TTS
...
### "Model: currently openrouter" (or some other provider) instead of "using Sophos as inference provider"
...
### Tool Gateway tools showing partner names instead of "via Sophos Portal"

Per-tool config is overriding the gateway. Run:

```bash
slick tools
# pick "Sophos Subscription" for any tool you want gateway-routed
```

Some users intentionally mix — e.g. routing web through Sophos but using their own Browserbase key for browser. If that's intentional, leave it alone. If not, this command fixes it.
...
If a model is genuinely unavailable, [open an issue](https://github.com/SophosTechne/slick-agent/issues) — most gaps are routing config we can update.
...
- `model.provider` set to `openrouter`/`anthropic`/etc. instead of `sophos`
...
```bash
slick auth logout sophos       # wipes the local refresh token
...
- **[Sophos Portal integration page](/integrations/sophos-portal)** — Overview of what's in the subscription
