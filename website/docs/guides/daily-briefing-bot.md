...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `slick setup --portal`. See [Sophos Portal](/integrations/sophos-portal).
:::
...
2. **Slick spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.slick/cron/output/` and you can read them anytime.
:::
...
Slick will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Slick Agent
```
...
Before creating cron jobs, ensure Slick has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Slick what you want:
...
Slick will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Slick to delegate each topic to a sub-agent:
...
Slick will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
