...
description: "Diagnose and fix common Slick cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`slick gateway` for foreground, or `slick gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `slick cron tick`.
...
```bash
date
slick cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.slick/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.slick/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.slick/.env` |
...
| `local` | Write access to `~/.slick/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `slick cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (Slick Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `slick skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `slick skills list`.
...
2. `~/.slick/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `slick cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Slick config directory). Verify:
```bash
ls ~/.slick/scripts/your-script.py   # Must exist
slick cron edit <job_id> --script ~/.slick/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `slick skills install <skill-name>`.
...
Jobs are stored in `~/.slick/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.slick/cron/jobs.json
chmod 600 ~/.slick/cron/jobs.json   # Your user should own it
```
...
```bash
slick cron list                    # Show all jobs, states, next_run times
slick cron run <job_id>            # Schedule for next tick (for testing)
slick cron edit <job_id>           # Fix configuration issues
slick logs                         # View recent Slick logs
slick skills list                  # Verify installed skills
```
...
1. Run the job with `slick cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.slick/logs/agent.log` for scheduler messages and `~/.slick/logs/errors.log` for warnings
3. Open an issue at [github.com/SophosTechne/slick-agent](https://github.com/SophosTechne/slick-agent) with:
