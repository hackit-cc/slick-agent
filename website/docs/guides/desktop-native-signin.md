...
```
Desktop app                Gateway (/auth/native/*)          Hackit Portal (IDP)
...
desktop app* and an OAuth client *to the upstream identity provider* (Hackit
...
dashboard-auth subsystem. OAuth providers (e.g. the bundled **Hackit** provider)
...
- [Run Slick with Hackit Portal](./run-slick-with-hackit-portal.md)
