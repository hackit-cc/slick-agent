...
description: "How the Slick Desktop app signs in to a gated gateway using your system browser and PKCE — no embedded webview, no session cookies"
---

# Desktop Native Sign-In (RFC 8252)

When the Slick Desktop app connects to a **gated gateway** (a hosted or
...
For Slick specifically, native sign-in means:
...
```
Desktop app                Gateway (/auth/native/*)          Sophos Portal (IDP)
...
desktop app* and an OAuth client *to the upstream identity provider* (Sophos
...
dashboard-auth subsystem. OAuth providers (e.g. the bundled **Sophos** provider)
...
- [Run Slick with Sophos Portal](./run-slick-with-sophos-portal.md)
