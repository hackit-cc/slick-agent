---
sidebar_position: 7
title: "Use SOUL.md with Slick"
description: "How to use SOUL.md to shape Slick Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Slick

`SOUL.md` is the **primary identity** for your Slick instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Slick to feel like the same assistant every time you talk to it — or if you want to replace the Slick persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Slick should be
- what Slick should avoid stylistically
- how Slick should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Slick is and how Slick speaks
...
Slick now uses only the global SOUL file for the current instance:

```text
~/.slick/SOUL.md
```

If you run Slick with a custom home directory, it becomes:
...
Slick automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Slick does not overwrite it
- if the file exists but is empty, Slick adds nothing from it to the prompt

## How Slick uses it

When Slick starts a session, it reads `SOUL.md` from `SLICK_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Slick feels.
...
Slick already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Slick should sound.

# Avoid
What Slick should not do.

# Defaults
How Slick should behave when ambiguity appears.
```
...
Then restart Slick or start a new session.
...
### I edited SOUL.md but Slick still sounds the same

Check:
- you edited `~/.slick/SOUL.md` or `$SLICK_HOME/SOUL.md`
...
### Slick is ignoring parts of my SOUL.md
