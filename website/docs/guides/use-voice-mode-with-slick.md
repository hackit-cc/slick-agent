---
sidebar_position: 8
title: "Use Voice Mode with Slick"
description: "A practical guide to setting up and using Slick voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Slick
...
:::tip
[Hackit Portal](/integrations/hackit-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Slick sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Slick.
...
## Step 1: make sure normal Slick works first
...
Slick supports both local and cloud speech stacks.
...
### If you use `slick setup`

If you choose NeuTTS in the setup wizard, Slick checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Slick transcribes and responds
...
- asking Slick to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Slick loop.

## Tuning CLI behavior

### Silence threshold

If Slick starts/stops too aggressively, tune:
...
Slick stays a normal chat bot, but can speak replies.
...
- you want Slick to function like a portable research or ops assistant
...
Slick joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Slick detects speech boundaries
- transcripts are posted in the associated text channel
- Slick responds in text and audio
...
1. get text Slick working
2. run `slick setup tts` to enable voice support
