...
Slick Agent uses a SQLite database (`~/.slick/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `slick_state.py`


## Architecture Overview

```
~/.slick/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `slick_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `slick_state.py` for the exact SQL.
...
Multiple slick processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.slick/state.db
...
Default path: `~/.slick/state.db`

This is derived from `slick_constants.get_slick_home()` which resolves to
`~/.slick/` by default, or the value of `SLICK_HOME` environment variable.
