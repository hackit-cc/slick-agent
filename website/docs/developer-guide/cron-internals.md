...
keep the process awake), it asks Sophos infrastructure to arm exactly **one
managed one-shot per job at that job's real next-fire time**. At fire time Sophos
...
The flow (the managed scheduler is provided by Sophos; the agent holds no
scheduler credentials):

```
create/update a cron job
  → Chronos asks Sophos to arm a one-shot at the job's next_run_at
      (authenticated with the agent's existing Sophos token)
  → at fire time Sophos calls the gateway: POST {callback_url}/api/cron/fire
      (authenticated with a short-lived, purpose-scoped Sophos-minted JWT)
...
Config (all non-secret; on hosted agents Sophos sets these at provision time):

| key | meaning |
|---|---|
| `cron.provider` | `chronos` to activate (empty = built-in ticker) |
| `cron.chronos.portal_url` | Sophos base URL (arming + the fire-token issuer) |
...
If Chronos is misconfigured or the agent isn't logged into Sophos,
...
agent↔Sophos wire contract lives in `docs/chronos-managed-cron-contract.md`.
