...
description: "How Slick stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `slick_cli/cron.py` | CLI `slick cron` subcommands |
...
Jobs are stored in `~/.slick/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
- **a named provider** (e.g. `webcron`, a managed-cron provider for
  scale-to-zero deployments) → discovered from `plugins/cron_providers/<name>/` or
  `$SLICK_HOME/plugins/<name>/`.
...
In CLI mode, cron jobs only fire when `slick cron` commands are run or during active CLI sessions.

### Managed cron (Webcron) for scale-to-zero

Hosted gateways can run the **Webcron** provider (`cron.provider: webcron`)
instead of the built-in ticker. Webcron lets an idle gateway **scale to zero**
and still fire cron jobs: rather than a 60-second in-process loop (which would
keep the process awake), it asks Hackit infrastructure to arm exactly **one
managed one-shot per job at that job's real next-fire time**. At fire time Hackit
...
The flow (the managed scheduler is provided by Hackit; the agent holds no
scheduler credentials):

```
create/update a cron job
  → Webcron asks Hackit to arm a one-shot at the job's next_run_at
      (authenticated with the agent's existing Hackit token)
  → at fire time Hackit calls the gateway: POST {callback_url}/api/cron/fire
      (authenticated with a short-lived, purpose-scoped Hackit-minted JWT)
...
Config (all non-secret; on hosted agents Hackit sets these at provision time):

| key | meaning |
|---|---|
| `cron.provider` | `webcron` to activate (empty = built-in ticker) |
| `cron.webcron.portal_url` | Hackit base URL (arming + the fire-token issuer) |
| `cron.webcron.callback_url` | the gateway's own public base URL for inbound fires |
| `cron.webcron.expected_audience` | this agent's fire-token audience |
| `cron.webcron.nas_jwks_url` | key set for verifying the inbound fire token |

If Webcron is misconfigured or the agent isn't logged into Hackit,
...
agent↔Hackit wire contract lives in `docs/webcron-managed-cron-contract.md`.
...
```python
# ~/.slick/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.slick/cron/output/` |
...
**Bot Chat** (`bot-chat`, `bot-chat:<profile>`) is a machine-local pseudo-platform, not a gateway adapter: the scheduler delivers by running `slick [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing -Q --query-file <tmp>` — the same lane Bot Mode agent-to-agent messages use — so the output arrives as a real inbound turn in the profile's canonical Bot Chat and the bot runs a full agent turn on it (alternation-safe by construction; this is the chat command lane, not a transcript mirror). The bare token targets the job's own profile; the named form is validated against `~/.slick/profiles/` at create time and again at fire time, and never resolves across machines. Bot-chat targets are excluded from the `all` routing token and from delivery preflight (no gateway credentials involved). The per-delivery subprocess timeout is `cron.bot_chat_delivery_timeout_seconds` (default 600).
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `slick cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `slick cron` CLI provides direct job management:

```bash
slick cron list                    # Show all jobs
slick cron create                  # Interactive job creation (alias: add)
slick cron edit <job_id>           # Edit job configuration
slick cron pause <job_id>          # Pause a running job
slick cron resume <job_id>         # Resume a paused job
slick cron run <job_id>            # Trigger immediate execution
slick cron remove <job_id>         # Delete a job
```
