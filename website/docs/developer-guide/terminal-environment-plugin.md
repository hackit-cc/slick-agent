...
Slick runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.slick/plugins/`, registering a backend the user selects
...
| `slick setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `slick status` / `slick doctor` | `doctor_checks()` |
...
```python title="~/.slick/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.slick"    # where synced cache files land, or None
...
```yaml title="~/.slick/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
slick plugins enable acmebox
slick config set terminal.backend acmebox
```
