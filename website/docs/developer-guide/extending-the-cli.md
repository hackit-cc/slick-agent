...
description: "Build wrapper CLIs that extend the Slick TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

Slick exposes protected extension hooks on `SlickCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the 1000+ line `run()` method. This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends Slick."""
...
Called after Slick registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
