...
The ACP adapter wraps Slick' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
slick acp / slick-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.slick/.env
  -> configure stderr logging
  -> construct SlickACPAgent
...
- `allow_once` -> Slick `once`
- `allow_always` -> Slick `always`
- reject options -> Slick `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Slick tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["slick-acp"])
...
Instead it reuses Slick' runtime resolver:

- `acp_adapter/auth.py`
- `slick_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Slick provider/credentials. It also always advertises a terminal setup auth method (`slick-setup`, args `--setup`) so first-run ACP clients can open Slick' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.slick/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `slick-acp` toolset definition
- `slick_cli/main.py` — `slick acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `slick-acp` script
