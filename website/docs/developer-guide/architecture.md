...
sidebar_position: 1
title: "Architecture"
description: "Slick Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Slick Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ slick_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # SlickCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── slick_state.py           # SQLite session/state database with FTS5
├── slick_constants.py       # SLICK_HOME, profile-aware paths
...
│
├── slick_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `slick` subcommands (large file)
...
│   ├── skills_config.py      # slick skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # slick tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # slick gateway start/stop
│
...
```text
User input → SlickCLI.process_input()
...
Three discovery sources: `~/.slick/plugins/` (user), `.slick/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `slick plugins` or `config.yaml`.
...
Exposes Slick as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`slick -p <name>`) gets its own SLICK_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
