...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Slick Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Slick tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Slick core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Slick Plugin](/developer-guide/plugins)
...
2. **`toolsets.py`** — add tool name to `_SLICK_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `slick_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `slick_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `slick chat -q "Use the weather tool for London"`
