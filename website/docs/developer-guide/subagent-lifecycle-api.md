...
Plugins can launch and supervise fresh Slick child sessions without importing
...
active agent turn fails closed with `No active Slick parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Slick can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Slick's existing unsafe-tool block remains enforced.
