...
description: "How to build a memory provider plugin for Slick Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Slick Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `slick plugins`.
:::

## Installation Layouts

Slick discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Slick. Closed to new providers — see [CONTRIBUTING](https://github.com/SophosTechne/slick-agent/blob/main/CONTRIBUTING.md). |
| User | `$SLICK_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.slick/plugins/<name>/` | Opt-in via `SLICK_ENABLE_PROJECT_PLUGINS=1`. |
| Package | `slick_agent.memory_providers` entry point | `pip install`, nothing to copy. |
...
Slick, in `$SLICK_HOME/plugins/<name>/` when installed by a user, or in
`./.slick/plugins/<name>/` for a project-local one:
...
`slick_agent.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."slick_agent.memory_providers"]
...
package layout. No copy under `$SLICK_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Slick reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `slick <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          slick_home (str): Active SLICK_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `slick memory setup` | **Yes** |
| `save_config(values, slick_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
The gate binds to every compaction authority, not just the Slick
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Slick
...
`get_config_schema()` returns a list of field descriptors used by `slick memory setup`:
...
the `messages` parameter; Slick will continue calling them with the legacy
signature.
...
All storage paths **must** use the `slick_home` kwarg from `initialize()`, not hardcoded `~/.slick`:

```python
# CORRECT — profile-scoped
from slick_constants import get_slick_home
data_dir = get_slick_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.slick/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `slick my-provider status`, `slick my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `slick <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `slick --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: slick my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the slick my-provider argparse tree.
