...
title: "Desktop Plugin SDK (@slick/plugin-sdk)"
description: "Extend the native Slick Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Slick Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `SlickPlugin`.
It imports one module — `@slick/plugin-sdk` — and gets everything: the app's
...
`$SLICK_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Slick. This page is the **native
desktop app** (`slick desktop`) SDK — the `@slick/plugin-sdk` module and
`$SLICK_HOME/desktop-plugins/`. The **web dashboard** (`slick dashboard`) has
its own, unrelated plugin system on `window.__SLICK_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Slick Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$SLICK_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$SLICK_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `SlickPlugin` contract, appear in **Settings → Plugins**,
...
Create `$SLICK_HOME/desktop-plugins/hello/plugin.js` (that's `~/.slick/...`
by default, or `~/.slick/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.slick/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@slick/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Slick' }),
      jsx('div', {
...
The only importable specifiers are `@slick/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `SlickPlugin`:
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `slick.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@slick/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@slick/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@slick/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@slick/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@slick/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@slick/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@slick/plugin-sdk'
...
`window.slick.send('get-price eth')` (or a declarative
`<button data-slick-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@slick/plugin-sdk'
...
for requests; `targetProfile` is the backend Slick profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Slick
...
ctx.os.notify({
  ...
  // Body click → focus Slick + navigate. Same vocabulary as OS deep links:
  activate: 'slick://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'slick://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `slick://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `slick://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@slick/plugin-sdk'
...
```javascript
import { queryClient } from '@slick/plugin-sdk'
...
a package into `~/.slick/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.slick/plugins` is not reachable as a filesystem —
...
the `slick://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular Slick plugin and declare it in a
`manifest.json`:
...
slick-agent codebase directly (`slick_state`, `slick_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.slick/`)
...
(`slick.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `SlickPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@slick/plugin-sdk` alias.
2. It's still lint-fenced to `@slick/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@slick/plugin-sdk`, `react`,
...
| Plugin contract | `SlickPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `SlickOpenTarget`, `Contribution` |
...
### Agents: the `slick-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`slick-desktop-plugins`** skill — it carries the same contract as this page in
...
`$SLICK_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `slick logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@slick/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.slick/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.slick/logs/errors.log` for
