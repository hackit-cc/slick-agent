...
| Variable | Meaning |
|----------|---------|
| `SLICK_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `SLICK_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `SLICK_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Slick itself — they're private to these helpers. The variables Slick *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `slick --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_slick_root)" || { echo "htui: not in a Slick checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$SLICK_MAIN_CHECKOUT/.venv/bin/python" -m slick_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `SLICK_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_slick_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `SLICK_TUI_DIR` are mutually exclusive
`SLICK_TUI_DIR` points Slick at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `slick --tui --dev` exits with an error. Run `unset SLICK_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_slick_root)" || { echo "hgui: not in a Slick checkout" >&2; return 1; }
  deps="${SLICK_GUI_DEPS_CHECKOUT:-$SLICK_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _slick_link_deps "$desktop" "$deps/apps/desktop"
    _slick_link_deps "$root" "$deps"
  else
  ...
  trap '_slick_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    SLICK_DESKTOP_SLICK_ROOT="$root" \
    SLICK_DESKTOP_PYTHON="$SLICK_MAIN_CHECKOUT/.venv/bin/python" \
    SLICK_DESKTOP_IGNORE_EXISTING=1 \
    SLICK_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `SLICK_DESKTOP_SLICK_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `slick`. |
| `SLICK_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `SLICK_DESKTOP_IGNORE_EXISTING` | Ignores any `slick` on `PATH` so it can't shadow the worktree. |
| `SLICK_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Slick checkout.
_slick_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/slick_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_slick_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'slick_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `slick --tui --dev` and the `SLICK_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/SophosTechne/slick-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `SLICK_*` variable Slick reads
