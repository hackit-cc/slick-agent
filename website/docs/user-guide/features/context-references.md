...
Type `@` followed by a reference to inject content directly into your message. Slick expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Slick env: `$SLICK_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$SLICK_HOME/skills/.hub/`
