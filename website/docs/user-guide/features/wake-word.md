...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Slick' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Slick")

The wake word turns Slick into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Slick listens in the background for a spoken
trigger phrase. Say it, and Slick starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Slick backend (for example a
...
Slick supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey slick"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey slick"** — a model for it ships with Slick, so
...
```bash
cd ~/.slick/slick-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `slick` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.slick/config.yaml`,
...
wake_word:
  ...
  phrase: "hey slick"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_slick         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey slick" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey slick" utterances are missed.
...
default) to let Slick choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Slick installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Slick surfaces, and `surface` picks which one
...
| `cli` | Only the classic `slick` CLI. |
| `tui` | Only `slick --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Slick surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Slick does not silently fail over to another open surface.
...
"Hey Slick" works out of the box — the bundled openWakeWord model
(`model: hey_slick`) is the default. To wake on something else, the easiest
...
command (`slick -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.slick/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("slick" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Slick" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey slick"
  porcupine:
    keyword: ~/.slick/wakewords/hey_slick.ppn
```

Set your access key in `~/.slick/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `slick tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `slick-agent[wake]`).
...
but the phrase never fires. Slick detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Slick
backend (it may appear as your terminal, `python`, or Slick), then toggle the
...
```bash
slick config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
slick config set wake_word.input_device null
```
