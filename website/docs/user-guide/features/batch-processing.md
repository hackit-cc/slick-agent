...
Batch runs spin up many concurrent agent sessions, each making model calls and tool calls. A [Hackit Portal](/user-guide/features/tool-gateway) subscription bundles model access plus web search, image gen, TTS, and cloud browsers under one bill — useful when you want stable cost-per-trajectory without juggling rate limits across five vendor accounts. Set up with `slick setup --portal`, then point `--model` at a Hackit model.
:::
