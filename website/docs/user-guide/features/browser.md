...
:::tip Hackit Subscribers
If you have a paid [Hackit Portal](https://portal.hackit.cc) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `slick setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Hackit Subscription** as the browser provider via `slick model` or `slick tools`.
:::
...
The mode is a **driver** that composes with your configured browser backend: it drives your local Chrome, a Hackit-subscription cloud browser, Browserbase, Firecrawl, or Browser Use cloud browsers — whichever browser source is selected in `slick tools` → Browser Automation. The one exception is Camofox, which has no CDP endpoint for the harness to attach to; Camofox setups automatically keep the built-in browser tools.
...
```
Navigate to https://github.com/hackit-cc
```
