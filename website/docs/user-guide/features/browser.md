...
Slick Agent includes a full browser automation toolset with multiple backend options:
...
:::tip Sophos Subscribers
If you have a paid [Sophos Portal](https://portal.sophostechne.com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `slick setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Sophos Subscription** as the browser provider via `slick model` or `slick tools`.
:::
...
The `.env` keys above supply **credentials only**. The active cloud browser is chosen by the `browser.cloud_provider` selection written by `slick tools` → Browser Automation (`browserbase`, `browser-use`, `camofox`, or `sophos` for the Sophos Subscription). Once a selection exists, adding or removing a key does not switch providers — and a selected provider with a missing key errors with guidance to run `slick tools` instead of silently rerouting. Never-configured setups still autodetect from available credentials.
:::
...
**This is the default browser mode**: when `browser.backend` is unset and the `browser-use` CLI is runnable (installed, or available through `uvx`), the agent gets the single `browser_exec` tool. If the CLI can't run, Slick falls back to the built-in browser tools automatically.

The mode is a **driver** that composes with your configured browser backend: it drives your local Chrome, a Sophos-subscription cloud browser, Browserbase, Firecrawl, or Browser Use cloud browsers — whichever browser source is selected in `slick tools` → Browser Automation. The one exception is Camofox, which has no CDP endpoint for the harness to attach to; Camofox setups automatically keep the built-in browser tools.
...
When a cloud provider is configured, Slick auto-spawns a **local Chromium sidecar**
...
mode, so you need it installed (`slick setup tools → Browser Automation`
...
When enabled, Slick copies your default browser's **active** profile — the one
...
`~/.slick/browser-profile/<browser>/`, then drives that snapshot with its
...
When you turn the toggle back off, Slick deletes the snapshot store
(`~/.slick/browser-profile/`) on the next browser use, so the copied
...
an exclusive (deny-all) lock, so Slick cannot copy them while the browser is
...
Set `browser.real_profile_autoclose: true` to let Slick **offer to close the
browser for you** when it's holding the profile. Even with this on, Slick never
closes it automatically — when the profile is locked it always stops and the
agent asks you first; only on your approval does it run `slick browser
...
(e.g. a background/tray instance relaunched), Slick stays blocked and tells you
...
Or configure via `slick tools` → Browser Automation → Camofox.

Camofox is selected like any other browser backend: pick **Camofox** in `slick tools` → Browser Automation, which writes `browser.cloud_provider: camofox` to `config.yaml`. `CAMOFOX_URL` is only the server address — setting it no longer selects the backend by itself once a browser selection exists (never-configured setups still autodetect it).

#### Persistent browser sessions

By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.slick/config.yaml`:
...
Then fully restart Slick so the new config is picked up.

:::warning Nested path matters
Slick reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — Slick ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, Slick silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active Slick profile, so different Slick profiles get different browser profiles (profile isolation).

##### What Slick does not do
- It does not force persistence on the Camofox server. Slick only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), Slick cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start Slick and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted Slick after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

Slick derives the stable `userId` from the profile-scoped directory `~/.slick/browser_auth/camofox/` (or the equivalent under `$SLICK_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding Slick profile's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agent), configure Slick to operate inside that same identity instead of spawning its own isolated profile.

Three knobs control the behavior:

| Setting | Env var | Effect |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Camofox `userId` Slick uses when creating tabs. Setting this opts the session into "externally managed" mode. |
...
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | When true, Slick calls `GET /tabs?userId=<user_id>` on first use and reuses an existing tab before creating a new one. |
...
- Slick skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/profile survive.
- Slick does **not** call `DELETE /sessions/<user_id>` — that endpoint wipes all user data, so it would nuke the external app's session if it fired.

**How tab adoption works (when `adopt_existing_tab: true`):**

1. On the first browser tool call after a process start, Slick issues `GET /tabs?userId=<user_id>` (5-second timeout).
2. If any tab in the response has `listItemId == session_key`, Slick adopts the most recently created one in that group.
3. Otherwise, Slick adopts the most recently created tab for the user (any `listItemId`).
4. If no tabs exist or the request fails, Slick falls back to creating a new tab on the next operation.

Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser tool call will surface a Camofox error — Slick does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want Slick to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, Slick generates a per-task `session_key` (`task_<id>`) — Slick will share cookies and the profile with the external app, but will open its own tab alongside instead of reusing one.

**Concurrency note:** the external app and Slick can drive the same Camofox `userId` simultaneously, but Camofox does not coordinate per-tab focus between clients. Coordinate ownership at the application layer (e.g. the external app pauses while Slick runs).

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Slick automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.
...
Slick drives Lightpanda through `agent-browser` over CDP, the same way it drives local Chrome.

**Automatic Chrome fallback.** Lightpanda doesn't yet cover everything Chrome does, so the integration is non-disruptive: Lightpanda handles the actions it supports, and Slick transparently retries on Chrome for anything it doesn't. The supported set covers the core agent workflow — navigate, snapshot, click, type, scroll, back, press, and eval. Screenshots also fall back to Chrome because Lightpanda has no graphical renderer; `browser_vision` is pre-routed straight to Chrome for the same reason.

### Local Chromium-family browser via CDP (`/browser connect`)

Instead of a cloud provider, you can attach Slick browser tools to your own running Chrome, Brave, Chromium, or Edge instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start Slick from the terminal (`slick` or `slick chat`) and issue `/browser connect` there.
:::
...
brave-browser \
  ...
  --user-data-dir=$HOME/.slick/chrome-debug \
  --no-first-run \
...
Then launch the Slick CLI and run `/browser connect`.
...
If Slick runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects Slick itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let Slick talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through Slick MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Slick can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
# Extra Chromium launch flags (comma- or newline-separated). Slick auto-injects
...
# Slick doesn't add automatically; setting it disables the auto-injection.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `slick config set toolsets '["slick-cli", "browser"]'`.
:::
...
```
Navigate to https://github.com/SophosTechne
```
...
You can also run `slick config set browser.snapshot_threshold 30000`. The setting applies to both explicit `browser_snapshot` calls and the automatic snapshot returned after navigation, including the Camofox backend (minimum 1000). Restart the current Slick session after changing it so the browser config cache reloads.
...
```
Type "slick agent" into the search field @e3
```
...
Screenshots are stored in `~/.slick/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.slick/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Slick automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
...
- **Snapshot size** — large pages are truncated at `browser.snapshot_threshold` (default 15,000 characters, matching `web_extract`; no LLM summarization); the complete snapshot is saved to `~/.slick/cache/web/` and the output points at it for `read_file` paging
