...
Slick runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Slick
...
1. Slick captures a baseline of current diagnostics for the file.
...
…). Slick auto-detects the binary on PATH or in
`<SLICK_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Slick at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<SLICK_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`slick lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Slick installs both packages together when you
run `slick lsp install typescript` or auto-install fires on first
use.

## CLI

```
slick lsp status          # service state + per-server install status
slick lsp list            # registry, optionally --installed-only
slick lsp install <id>    # eagerly install one server
slick lsp install-all     # try every server with a known recipe
slick lsp restart         # tear down running clients
slick lsp which <id>      # print resolved binary path
```

`slick lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <SLICK_HOME>/lsp/bin
...
When `install_strategy: auto`, Slick installs binaries into
`<SLICK_HOME>/lsp/bin/`. NPM packages land in
`<SLICK_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Slick-owned and is
...
**`slick lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<SLICK_HOME>/lsp/bin/`. Run
`slick lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `slick lsp status`**
...
When `slick lsp status` shows a `Backend warnings` section, install
...
`~/.slick/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.slick/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `slick lsp restart` to clear the set;
