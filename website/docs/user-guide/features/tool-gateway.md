---
title: "Hackit Tool Gateway"
description: "One subscription, every tool. Web search, image generation, TTS, and cloud browsers — all routed through Hackit Portal with no extra API keys."
...
The Tool Gateway is included with every paid [Hackit Portal](https://portal.hackit.cc) subscription. It routes Slick' tool calls — web search, image generation, text-to-speech, and cloud browser automation — through infrastructure Hackit already runs, so you don't have to sign up with Firecrawl, FAL, OpenAI, Browser Use, or anyone else just to make your agent useful.

<div style={{display: 'flex', gap: '1rem', flexWrap: 'wrap', margin: '1.5rem 0'}}>
  <a href="https://portal.hackit.cc/manage-subscription" style={{background: 'var(--ifm-color-primary)', color: 'white', padding: '0.75rem 1.5rem', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold'}}>Start or manage subscription →</a>
</div>
...
All four are pay-as-you-use billed against your Hackit subscription. Use any combination — run the gateway for web and images while keeping your own ElevenLabs key for TTS, or route everything through Hackit.
...
- **One bill.** Pay Hackit; we handle the rest.
- **One signup.** No Firecrawl, FAL, Browser Use, or OpenAI audio accounts to manage.
- **One key.** Your Hackit Portal OAuth covers every tool.
...
```bash
slick setup --portal     # Fresh install: Hackit OAuth + set Hackit as provider + turn on the Tool Gateway in one go
```

```bash
slick model              # Switch your inference provider to Hackit Portal — Slick then offers to turn on the gateway for all tools
```

```bash
slick tools              # Enable the gateway per-tool — pick "Hackit Subscription" for any tool you want
```
...
**You don't have to log in first.** With `slick tools`, the Hackit-managed backends (Web search, Image, Video, TTS, Browser) are always listed, even if you've never signed into Hackit Portal. Select one and Slick runs the Portal login right there if you aren't already authenticated — no need to run `slick model` beforehand. If your Hackit OAuth is already active, selecting the backend enables it immediately with no extra prompt. This path only logs you in and turns on the one tool you picked — it does **not** switch your inference provider, and it does **not** prompt you to enable the gateway for every other tool.
...
```
◆ Hackit Tool Gateway
  Hackit Portal     ✓ managed tools available
  Web tools       ✓ active via Hackit subscription
  Image gen       ✓ active via Hackit subscription
  TTS             ✓ active via Hackit subscription
  Browser         ○ active via Browser Use key
```

Tools marked "active via Hackit subscription" are going through the gateway. Anything else is using your own keys.

## Eligibility

The Tool Gateway is a **paid-subscription** feature. Free-tier Hackit accounts can use Portal for inference but don't include managed tools — [upgrade your plan](https://portal.hackit.cc/manage-subscription) to unlock the gateway.
...
Picking a Hackit model (`slick model`) offers a per-tool checklist of gateway backends. Its behavior respects your existing setup:
...
- Declines stick: if you submit the checklist with a tool unchecked, it won't be pre-checked on future Hackit model swaps (stored in `tool_gateway_declined_tools` in `config.yaml`; checking it later clears the decline).

## Mix and match

The gateway is per-tool. Turn it on for just what you want:

- **All tools through Hackit** — easiest; one subscription, done.
- **Gateway for web + images, bring your own TTS** — keep your ElevenLabs voice, let Hackit handle the rest.
...
Select the tool, pick **Hackit Subscription** as the provider (or any direct provider you prefer). No config editing required. If you aren't logged into Hackit Portal yet, picking **Hackit Subscription** kicks off the Portal login inline — you don't need to authenticate through `slick model` first.
...
Each tool category has a single provider-selection key, written by the `slick tools` picker (or the desktop GUI). Picking the **Hackit Subscription** row stores the value `hackit`, which routes that category through the managed Tool Gateway. Picking a BYOK row stores the vendor name (`fal`, `openai`, `firecrawl`, `browser-use`, ...), which goes direct with your own credentials:

```yaml
web:
  backend: hackit          # web search/extract via the Tool Gateway

image_gen:
  provider: hackit         # image generation via the Tool Gateway

tts:
  provider: hackit         # TTS via the Tool Gateway

stt:
  provider: hackit         # speech-to-text via the Tool Gateway

browser:
  cloud_provider: hackit   # cloud browser via the Tool Gateway
```

The runtime **always uses the stored selection** — credential presence never selects or reroutes a category. A `FAL_KEY` sitting in `.env` is ignored while `image_gen.provider: hackit`; conversely, `image_gen.provider: fal` with no `FAL_KEY` set produces a clear error instead of silently falling back to the gateway:
...
Yes — the [Hackit Portal dashboard](https://portal.hackit.cc) breaks usage down by tool so you can see what's driving your bill.

### Is Modal (serverless terminal) included?

Modal is available as an **optional add-on** through the Hackit subscription, not part of the default Tool Gateway bundle. Configure it via `slick setup terminal` or directly in `config.yaml` when you want a remote sandbox for shell execution.

### Do I need to delete my existing API keys when I enable the gateway?

No — keep them in `.env`. While a tool's selection is **Hackit Subscription**, direct keys for that tool are simply ignored. Pick the direct provider again in `slick tools` and your keys become the source again. The gateway isn't a lock-in.
