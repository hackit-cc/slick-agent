...
Slick Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Slick integration. It
...
**Fresh installs already have the driver.** The Slick installer
...
- **`slick tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
slick computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `slick computer-use
status` to verify the install.

Already have cua-driver? Slick reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `slick update`, and the first
`computer_use` call of a session, Slick checks the local version and
...
selected with `SLICK_CUA_DRIVER_CMD` stays
under your control, so Slick reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Slick autodetection is a
planned cua-driver follow-up, so currently point Slick at that directory or
...
interface. The built-in toolset provides Slick actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `slick computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Slick host identity. |
...
```
slick -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.slick/config.yaml`.

## Permission modes and logged-in browser profiles

Slick maps its existing approval UX onto cua-driver's immutable runtime
...
| Slick session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Slick approvals; Cua stops at its protected boundary | Refuses unless `computer_use.grant_existing_profile: true` (one-time config opt-in) |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Slick risk acceptance; no runtime Cua prompts | Refuses unless `computer_use.grant_existing_profile: true`; YOLO does not substitute for this grant |
...
Slick then launches the cua-driver runtime with the trusted-launcher grant
...
computer_use:
  ...
  capability_manifest: ~/.slick/cua-manifest.yaml
```
...
for the format). Slick launches a private runtime with
...
identity instead of resetting with every Slick build), and Slick verifies
...
resetting or closing the Slick session, cancellation cleanup, or process exit
closes that transport session. Slick also stops private runtimes that it
launched for bounded, unrestricted, or existing-profile access. One Slick
...
embedded service under the Slick host identity.
...
## `slick computer-use doctor` — your first triage stop

`slick computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ slick computer-use doctor
...
Slick run declares a public cua-driver **session name** (something like
`slick-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Slick disables it automatically where it is a known failure
...
Slick keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Slick-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Slick
autodetection is a planned cua-driver follow-up, so currently point Slick at
...
These are **platform deep dives, not duplicates of the Slick skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Slick
...
image capture also saves a bounded, shareable copy under Slick' image cache
...
Slick applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.slick/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Slick applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Slick agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Slick agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Slick warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Slick from inside the RDP / console
...
```
SLICK_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
SLICK_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Slick disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Slick sets
...
When it's on, `slick computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Slick at a binary you built from source instead
of the published release. Slick resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`SLICK_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Slick testing (see notes).
...
### Option B — point Slick straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`SLICK_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
SLICK_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
SLICK_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Slick is using your build

- `slick computer-use status` prints the resolved binary path and
  version.
- `slick computer-use doctor` confirms the binary is reachable and
...
- **Slick spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Slick YOLO, Slick instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Slick
  change needed either way.
- **Local builds skip the version check.** Slick warns when the
...
**First action when anything's off: run `slick computer-use doctor`.**
...
Run `slick computer-use install` to fetch the cua-driver binary, or
run `slick tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `slick computer-use doctor` will
...
- **Slick-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Slick `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Slick autodetection is a
  planned follow-up; currently point Slick at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Slick among them)
