...
description: "Customize Slick Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Slick Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `SLICK_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Slick is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Slick now seeds a default `SOUL.md` automatically in:

```text
~/.slick/SOUL.md
```

More precisely, it uses the current instance's `SLICK_HOME`, so if you run Slick with a custom home directory, it will use:
...
- Slick creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Slick loads `SOUL.md` only from `SLICK_HOME`
- Slick does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Slick falls back to a built-in default identity
...
If Slick loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `SLICK_HOME`, the personality belongs to the Slick instance itself.

That also makes it easier to teach users:
- "Edit `~/.slick/SOUL.md` to change Slick' default personality."
...
- how Slick should handle uncertainty, disagreement, or ambiguity
...
## What Slick injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Slick falls back to a built-in default identity ("You are Slick Agent, an intelligent AI assistant created by Sophos Techne..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Slick ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Slick, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Slick its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.slick/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.slick/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-slick)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Slick speaks
- `display.skin` and `/skin` affect how Slick looks in the terminal
