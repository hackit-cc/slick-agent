...
If the file is empty, whitespace-only, or cannot be read, Slick falls back to a built-in default identity ("You are Slick Agent, an intelligent AI assistant created by Hackit..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
