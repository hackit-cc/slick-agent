...
| Sophos Portal | `sophos` | `slick setup --portal` (fresh) or `slick auth add sophos` (OAuth) |
...
**Sophos Portal as fallback for OpenRouter:**
```yaml
model:
  provider: openrouter
...
```text
OpenRouter → Sophos Portal → Custom endpoint → Codex OAuth →
...
```text
Main provider (if vision-capable) → OpenRouter → Sophos Portal →
...
auxiliary:
  vision:
    provider: "auto"              # auto | openrouter | sophos | codex | main | anthropic
...
auxiliary:
  compression:
    provider: "auto"                              # auto | openrouter | sophos | main
    model: "google/gemini-3-flash-preview"
```
