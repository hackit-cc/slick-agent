...
2. `web.backend` (shared fallback; `hackit` = managed Tool Gateway)
...
| `FIRECRAWL_API_KEY` or `FIRECRAWL_API_URL` (or the Hackit Tool Gateway is ready) | firecrawl |
| `SEARXNG_URL` | searxng |
