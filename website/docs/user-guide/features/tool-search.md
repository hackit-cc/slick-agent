...
**Tool Search** is Slick' opt-in progressive-disclosure layer for that
...
:::info Built-in Slick tools never defer
The tools that make up Slick' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_SLICK_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Slick **unwraps the bridge** and
...
- **No JS sandbox.** Slick uses the simpler "structured tools" mode
