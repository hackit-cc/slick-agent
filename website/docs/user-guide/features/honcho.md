...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Slick's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Slick instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
slick memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `slick` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Slick syncs them back at session init.
...
The `slick honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `slick memory setup honcho` (or run `slick memory setup` and pick it from the list); the `slick honcho` subcommand then appears on the next invocation.

```bash
slick memory setup honcho    # Configure Honcho directly (works before activation)
slick honcho status          # Connection status, config, and key settings
slick honcho setup           # Redirects to `slick memory setup` (post-activation alias)
slick honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
slick honcho peer            # Show or update peer names + dialectic reasoning level
slick honcho mode            # Show or set recall mode (hybrid/context/tools)
slick honcho tokens          # Show or set token budget for context and dialectic
slick honcho identity        # Seed or show the AI peer's Honcho identity
slick honcho sync            # Sync Honcho config to all existing profiles
slick honcho peers           # Show peer identities across all profiles
slick honcho sessions        # List known Honcho session mappings
slick honcho map             # Map current directory to a Honcho session name
slick honcho enable          # Enable Honcho for the active profile
slick honcho disable         # Disable Honcho for the active profile
slick honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `slick honcho`

If you previously used the standalone `slick honcho setup`:
...
No re-login or re-setup needed. Run `slick memory setup` and select "honcho" — the wizard detects your existing config.
