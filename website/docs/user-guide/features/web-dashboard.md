...
:::tip
Hosted-mode auth uses Hackit Portal OAuth; if you also want the dashboard to talk to a real backend, `slick setup --portal` wires up the model and tool gateway too. See [Hackit Portal](/integrations/hackit-portal).
:::
...
**Catalog:** browse the Hackit-approved MCP servers (the bundled `optional-mcps/`
...
- **Hackit Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `slick portal`.
...
![System admin page — host stats and Hackit Portal status](/img/dashboard/admin-system-top.png)
...
| `GET /api/mcp/catalog` | Browse the Hackit-approved MCP catalog |
...
| `GET /api/portal` | Hackit Portal auth + Tool Gateway routing (read-only) |
...
- **[OAuth (Hackit Portal)](#default-provider-hackit)** — for hosted deployments and any dashboard reachable over the public internet, and the recommended path for a [remote Slick Desktop connection](#connecting-slick-desktop-to-a-remote-backend). Every login is verified against your Hackit account, so this is the provider suitable for internet-facing use.
- **[Self-hosted OIDC](#self-hosted-oidc-provider)** — for bringing your own identity provider via standard OpenID Connect (Keycloak, Auth0, Okta, Google, GitHub via an OIDC bridge, etc.). No Hackit Portal involved; suitable for public-internet exposure when fronted by a conformant OIDC server.
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Hackit plugin, no custom plugin), `slick dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.
...
### Default provider: Hackit

The bundled `plugins/dashboard_auth/hackit` plugin is **always installed** and auto-loaded. It auto-registers a `DashboardAuthProvider` named `hackit` when a client ID is configured.

Because every login is verified against Hackit Portal and protected by your Hackit account, **the Hackit provider is the one suitable for exposing a dashboard to the public internet.**

#### Registering a dashboard

To use the Hackit provider you need an OAuth client ID (shape `agent:{id}`). There are two ways to get one:

- **CLI — `slick dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Hackit login (run `slick setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `SLICK_DASHBOARD_OAUTH_CLIENT_ID` into `~/.slick/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).
...
Bundled providers reported these issues:
  • hackit: SLICK_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `slick dashboard register` (Hackit Portal) or install a
...
#### Worked example: Hackit

From a logged-in Slick install to a Hackit-gated dashboard in three steps.

**1. Log in and register the dashboard.** `slick dashboard register` uses your existing Hackit login to provision an OAuth client and writes `SLICK_DASHBOARD_OAUTH_CLIENT_ID` into `~/.slick/.env` for you:

```bash
slick setup            # if you're not already logged into Hackit Portal
slick dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes SLICK_DASHBOARD_OAUTH_CLIENT_ID to ~/.slick/.env
```

**2. Run the dashboard on a reachable address.** A non-loopback bind engages the OAuth gate, and the `client_id` just written activates the `hackit` provider:

```bash
slick dashboard --host 0.0.0.0 --port 9119 --no-open
```

**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click **Sign in with Hackit** → authenticate at the Portal → land back on the authenticated dashboard. Verify the gate from any machine:
...
`GET /api/auth/me` then returns the verified session (`provider: hackit`). For an internet-facing host, register with `--redirect-uri https://slick.example.com/auth/callback` and set `SLICK_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
The username/password provider is intended for self-hosted / on-prem / homelab dashboards on a **trusted network**, or reachable only over a **VPN**. It protects a single shared credential with no external identity provider, MFA, or per-user accounts behind it, so it is **not suitable for exposing a dashboard directly to the public internet**. For an internet-facing dashboard, use the [Hackit provider](#default-provider-hackit) (or your own [self-hosted OIDC](#self-hosted-oidc-provider) / [custom OAuth](#custom-providers) provider) instead.
:::

#### Configuration

Like the Hackit provider, it reads from `config.yaml` (canonical) with environment variables winning when set non-empty. It activates only when `username` plus either `password_hash` (preferred) or `password` are configured — otherwise it's a no-op, so OAuth users and loopback operators are unaffected.
...
`GET /api/auth/me` then returns the verified session (`provider: basic`). Keep this behind a VPN — see the warning above; for a public host use the [Hackit](#default-provider-hackit) or [self-hosted OIDC](#self-hosted-oidc-provider) provider instead.
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** — no per-IDP code, no Hackit Portal involved. It's verified against and works with any conformant OIDC server:

> **Authentik · Keycloak · Zitadel · Authelia · Auth0 · Okta · Google · …**

Like the Hackit provider, it auto-loads and only registers itself once it's configured, so it's a no-op for loopback dashboards.
...
The provider implements the [Hackit Portal OAuth contract v1](https://github.com/hackit-cc/hackit-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):

1. User hits `/` without a session cookie → gate redirects to `/login`.
2. Login page shows a "Continue with Hackit" button → `/auth/login?provider=hackit`.
3. Server stashes PKCE state in a short-lived cookie, redirects user to `https://portal.hackit.cc/oauth/authorize?…`.
...
The sidebar widget shows `Logged in as <user_id…> via hackit` with a logout icon. Clicking it POSTs `/auth/logout`, which clears all dashboard-auth cookies and redirects back to `/login`.
...
To plug a non-Hackit OAuth provider (e.g. Google, GitHub, custom OIDC), create a plugin that registers a `DashboardAuthProvider`:
...
You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Hackit Portal)** (register it with [`slick dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Hackit*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.

The recipe below uses the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Default provider: Hackit](#default-provider-hackit).
...
| **Hackit Blue** (`hackit-blue`) | Hackit-branded blue accents with airy spacing |
