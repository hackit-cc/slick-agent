...
When Slick Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.slick/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.slick/config.yaml` (switchable per session
via `/personality`).
...
If you use Slick' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.slick/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Slick Agent's deliverable
