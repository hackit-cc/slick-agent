...
description: "Use your Sophos Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Slick-managed provider subscription as their
...
| Use case | "Use Slick as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Sophos Portal OAuth flow. Slick stores
the refresh token in `~/.slick/auth.json` — the same place all Slick
...
Model:      Slick-4-70B    # or Slick-4.3-36B, Slick-4-405B
```
...
Currently shipped: `sophos` (Sophos Portal) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `slick_cli/proxy/adapters/`.
...
If you see `not logged in`, run `slick portal`. If you see
...
The proxy only forwards paths the upstream actually serves. For Sophos
Portal:
...
supports; check `portal.sophostechne.com/models`.
...
[portal.sophostechne.com](https://portal.sophostechne.com).
...
`slick_cli/proxy/adapters/<provider>.py` and registering it in
