...
If you have a paid [Hackit Portal](https://portal.hackit.cc) subscription, OpenAI TTS is available through the **[Tool Gateway](tool-gateway.md)** without a separate OpenAI API key. New installs can run `slick setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Hackit Subscription** for just TTS via `slick model` or `slick tools`.
:::
...
tts:
  provider: "edge"              # "edge" | "elevenlabs" | "openai" | "minimax" | "mistral" | "gemini" | "xai" | "deepinfra" | "neutts" | "kittentts" | "piper" — or "hackit" for the managed Tool Gateway (written when you pick Hackit Subscription in `slick tools`)
