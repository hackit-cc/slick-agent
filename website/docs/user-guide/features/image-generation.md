...
If you have a paid [Hackit Portal](https://portal.hackit.cc) subscription, you can use image generation through the **[Tool Gateway](tool-gateway.md)** without a FAL API key. Your model selection persists across both paths. New installs can run `slick setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Hackit Subscription** as the image-gen backend via `slick tools`.
...
Navigate to **🎨 Image Generation**, pick your backend (Hackit Subscription or FAL.ai), then the picker shows all supported models in a column-aligned table — arrow keys to navigate, Enter to select:
...
```yaml
image_gen:
  provider: fal                 # `hackit` if you picked Hackit Subscription
  model: fal-ai/flux-2/klein/9b
  max_parallel_requests: 4      # concurrent images in one tool-call batch
```

`image_gen.provider` is the single selection key: `hackit` routes through the managed Tool Gateway; a vendor name (`fal`, `openai`, `xai`, `krea`, ...) goes direct with your own key. The runtime always follows this stored selection — a `FAL_KEY` in `.env` is ignored while `provider: hackit`, and `provider: fal` without `FAL_KEY` errors with `image_gen is configured to use fal (set via slick tools), but FAL_KEY is not set. Run 'slick tools' to change it.` rather than silently rerouting. Change providers via `slick tools`, not by adding/removing keys. (The old `use_gateway` boolean is legacy — still read as `hackit` when `true`, but never written anymore.)
...
Hackit Portal proxies the chat-completions protocol only, so its picker offers
...
3. **Submission** — `_submit_fal_request()` routes via direct FAL credentials or the managed Hackit gateway, according to the stored `image_gen.provider` selection.
...
- **Requires credentials** for the active backend (FAL `FAL_KEY` / Hackit Subscription, `OPENAI_API_KEY`, xAI OAuth, `KREA_API_KEY`)
