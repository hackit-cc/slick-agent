...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [Sophos Portal](/integrations/sophos-portal) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
For providers that support both API keys and OAuth (Anthropic, Sophos, Codex), the add flow asks which type:
...
| OAuth tokens (auth.json) | Codex device code, Sophos device code | Yes |
