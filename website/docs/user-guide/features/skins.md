...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Slick CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Slick CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin ruby           # switch to a built-in skin
/skin mytheme        # switch to a custom skin from ~/.slick/skins/mytheme.yaml
```

Or set the default skin in `~/.slick/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Slick — gold and kawaii | `Slick Agent` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar hero banner. Clean and inviting. |
| `ruby` | Crimson and bronze | `Ruby Agent` | Deep crimson borders with bronze accents. Aggressive spinner verbs ("forging", "marching", "tempering steel"). Custom sword-and-shield ASCII art banner. |
| `mono` | Monochrome — clean grayscale | `Slick Agent` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Slick Agent` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Slick Agent` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Slick Agent` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
| `mint` | Deep blue and seafoam | `Mint Agent` | Deep blue to seafoam gradient. Ocean-themed spinners ("charting currents", "sounding the depth"). Wave ASCII art banner. |
| `refactor` | Austere grayscale with persistence | `Refactor Agent` | Light grays with stark contrast. Refactor-themed spinners ("pushing upstream", "resetting the branch", "enduring the loop"). Boulder-and-hill ASCII art banner. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Slick Agent` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Slick Agent! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! 😎` |
| `response_label` | Label on the response box header | ` 😎 Slick ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default SLICK_AGENT banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default hero art) | `""` |

## Custom skins

Create YAML files under `~/.slick/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.slick/skins/mytheme.yaml
...
## Slick Mod — Visual Skin Editor

[Hackit Mod](https://github.com/cocktailpeanut/hackit-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Hackit Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/hackit-mod/master/sophos.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Slick skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.slick/skins/`
- Activates a skin by updating `~/.slick/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/hackit-mod.git
cd slick-mod/app
npm install
npm start
```
...
6. Click **Save** to write the skin YAML to `~/.slick/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Slick Mod respects the `SLICK_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `slick_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.slick/skins/` take precedence over built-in skins with the same name.
