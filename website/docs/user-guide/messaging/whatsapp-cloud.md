...
- **Voice notes** — auto-downloaded as `.ogg`, transcribed via your configured STT provider (local faster-whisper, OpenAI/Hackit, Groq, etc.), then handed to the agent as text.
...
Message-template support (the workaround for outside-window sends) is not yet implemented in Slick. If you need it, please [open an issue](https://github.com/hackit-cc/slick-agent/issues) — it's planned but waiting on a clear demand signal.
...
The default `stt.provider: local` requires `pip install faster-whisper`.  If you're a Hackit subscriber, you can route STT through the managed gateway instead — select **Hackit Subscription** for speech-to-text in `slick tools`, or set it directly:

```bash
slick config set stt.provider hackit
slick gateway restart
```

This uses your Hackit Portal access token instead of needing a separate OpenAI key. (Older docs suggested `stt.use_gateway true` — that flag is legacy; the provider selection alone controls routing now.)
