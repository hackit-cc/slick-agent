...
Connect Slick to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Slick — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Slick runs it in a
...
slick photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Slick Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.slick/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.slick/.env`
...
(device token, dashboard project id) lives in `~/.slick/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Slick
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Slick replies with a pairing code. Approve it with:

```bash
slick pairing approve photon <CODE>
```

Use `slick pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.slick/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.slick/.env`):
...
By default Slick responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Slick` and
`@Slick agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Slick will reply.
...
assigned iMessage line Slick uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run slick photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `slick photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `slick photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `slick photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Slick sends images, voice
  ...
  media.
- **Native polls are supported.** Slick sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Slick sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `slick send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<slick-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Slick is set
...
| `PHOTON_MENTION_PATTERNS` | Slick wake words  | JSON list / comma / newline regex patterns for group mentions |
