...
1. Resolves a fresh Sophos Portal access token from your existing login
   (`~/.slick/auth.json`) — this proves which Sophos org (tenant) you own. If
   `gateway.idp.token_url` is configured, your own IdP is used instead (the
   air-gapped / self-hosted-IdP path, no Sophos Portal involved): with
...
| `gateway.idp.token_url` | `config.yaml` | When set, enrollment/provisioning authenticates against your own IdP instead of Sophos Portal: OAuth2 client-credentials when `gateway.idp.client_id`/`client_secret` are also set; otherwise an ambient token endpoint (plain GET returning the token, raw or `{"access_token": …}`). |
...
token. Re-login with `slick auth add sophos` (or `slick setup`) and retry.
