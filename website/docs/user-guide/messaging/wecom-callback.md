...
Connect Slick to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Slick supports two WeCom integration modes:
...
> Run `slick gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Slick decrypts the message, queues it for the agent
...
(Use `slick gateway start` only after `slick gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Slick and the token the
...
in `~/.slick/.env` values around `=` will also break signature checks. After
fixing, restart `slick gateway run`.
...
Check `slick gateway run` logs for the bound host/port. If the adapter bound to
