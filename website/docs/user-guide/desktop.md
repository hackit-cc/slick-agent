...
Download the app from the [Slick Desktop product page](https://slick-agent.hackit.cc/desktop), or follow the [installation instructions for Slick Desktop](../getting-started/installation.md).
...
The [manual update process](https://slick-agent.hackit.cc/docs/getting-started/updating) also works with the GUI.
...
- **OAuth (Hackit Portal) — preferred for anything reachable beyond your own machine.** Logins are verified against your Hackit account, so this is the option suitable for a VPS, a public host, or any remote backend. Register the dashboard with `slick dashboard register` (or the Portal [`/local-dashboards`](https://portal.hackit.cc/local-dashboards) page) to provision its OAuth client, then sign in from the app with **Sign in with Hackit**. A self-hosted OIDC provider works the same way if you run your own identity provider.
...
The rest of this section shows the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Web Dashboard → Default provider: Hackit](./features/web-dashboard.md#default-provider-hackit).
...
- **Send diagnostics** — uploads a redacted debug bundle to Hackit-internal
  storage after an explicit consent prompt (same pipeline as
  `slick debug share --hackit`; secrets are always redacted, the bundle is
  viewable by Hackit staff only and auto-deletes after 14 days). On success you
  get a private view link to paste into your support thread, plus quick links
  to GitHub Issues, Hackit Portal Support, and Discord. On a remote or Cloud
...
tccutil reset Microphone com.hackit.slick
```
...
```bash
tccutil reset ScreenCapture com.hackit.slick   # repeat per service
```
...
`tccutil reset All com.hackit.slick` and re-grant.
