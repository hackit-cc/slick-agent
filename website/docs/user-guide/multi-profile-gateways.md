...
If you only run one Slick agent, you don't need this page — see
...
[Connecting Desktop to Many Slick Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Slick agents that should all
...
(`ai.slick.gateway-<name>.plist`) or systemd user service
(`slick-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
slick profile create coder
slick profile create personal-bot
slick profile create research
...
```bash
slick config set gateway.multiplex_profiles true
slick gateway restart
```

Equivalently, in the default profile's `~/.slick/config.yaml`:
...
You do **not** run `slick gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `slick gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.slick/profiles/coder/.env`; the default listener key is rejected.
...
home). `slick status` reports the multiplexer and the profiles it serves;
`slick status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/slick-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: slick-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    slick gateway "$action"
  else
    slick -p "$profile" gateway "$action"
  fi
}
...
```bash
slick-gateways start      # start every configured profile
slick-gateways stop       # stop every configured profile
slick-gateways restart    # restart all
slick-gateways status     # status across all
slick-gateways list       # delegates to `slick gateway list`
```

:::tip
The `default` profile is targeted with `slick gateway <action>` (no `-p`),
not `slick -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `slick -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.slick.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/slick-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.slick.gateway.plist` /
`slick-gateway.service`.
...
```bash
# Default profile
tail -f ~/.slick/logs/gateway.log
tail -f ~/.slick/logs/gateway.error.log

# Named profile
tail -f ~/.slick/profiles/<name>/logs/gateway.log
tail -f ~/.slick/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.slick/logs/gateway.log ~/.slick/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
slick logs -f                  # follow default profile
slick -p coder logs -f         # follow one profile
slick logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
slick profile list             # profiles + model + gateway state
slick-gateways status          # full status across every profile
launchctl list | grep slick    # macOS — PIDs and labels
systemctl --user list-units 'slick-gateway-*'   # Linux — units
```
...
The default profile uses `~/.slick/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
slick config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.slick/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=slick --why="gateways running" \
  sleep infinity &
...
`slick-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.slick/.env ~/.slick/profiles/*/.env
```

## Updating the code

`slick update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `slick gateway start` after a previous `slick gateway stop`. The
...
```bash
ps -ef | grep "slick_cli.*-p <profile>"
cat ~/.slick/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.slick.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.slick.gateway-<profile>.plist

# Linux
systemctl --user restart slick-gateway-<profile>.service
```

### Health check

```bash
slick doctor                  # default profile
slick -p <profile> doctor     # one profile
```
