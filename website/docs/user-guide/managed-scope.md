...
`~/.slick/config.yaml`, `~/.slick/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/slick`:

```text
/etc/slick/
├── config.yaml     # managed config layer (wins over ~/.slick/config.yaml)
└── .env            # managed env layer (wins over ~/.slick/.env + shell)
```
...
The location can be relocated with the `SLICK_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `SLICK_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Slick.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export SLICK_MANAGED_DIR=/opt/org/slick-policy
```

:::warning
A user who can set `SLICK_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `slick doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/slick/config.yaml` (managed) | `/etc/slick/.env` (managed) |
| 2 | `~/.slick/config.yaml` (user) | `~/.slick/.env` (user) |
...
```bash
slick config        # shows a header naming the managed source + the pinned keys
slick doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Slick refuses and names the source:

```bash
$ slick config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/slick/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `slick config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/slick

# Pin some config values for every user on this machine
sudo tee /etc/slick/config.yaml >/dev/null <<'YAML'
model:
  provider: sophos
security:
  redact_secrets: true
...
sudo tee /etc/slick/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/slick
sudo chmod 0644 /etc/slick/config.yaml /etc/slick/.env
```

Changes take effect on the next Slick start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`slick doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Slick as `root`), managed scope is advisory.
