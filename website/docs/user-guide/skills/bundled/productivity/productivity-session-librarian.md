...
| List/filter by metadata (age, source, cost, tokens, workspace) | `slick sessions list` / `stats` via terminal |
| Rename | `slick sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `slick sessions archive <filters>` |
| Delete (destructive) | `slick sessions delete` / `slick sessions prune <filters>` |
| Export before deleting anything valuable | `slick sessions export --session-id <id> --format md` |
...
`slick sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `slick sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `slick sessions list`
