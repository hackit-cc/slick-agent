...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _slick_env="${SLICK_HOME:-$HOME/.slick}/.env"; [ -f "$_slick_env" ] && grep -q "^GITHUB_TOKEN=" "$_slick_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_slick_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${SLICK_HOME:-$HOME/.slick}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Slick Agent\",
    \"comments\": [
...
```bash
source "${SLICK_HOME:-$HOME/.slick}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Slick Agent. Code looks clean — good test coverage, no security concerns."
