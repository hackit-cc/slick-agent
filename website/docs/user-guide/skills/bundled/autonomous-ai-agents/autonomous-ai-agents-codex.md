...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`slick-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-slick-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Slick terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Slick itself, `model.provider: openai-codex` uses Slick-managed Codex
OAuth from `~/.slick/auth.json` after `slick auth add openai-codex`. For the
...
## Slick Gateway Caveat

When invoking the Codex CLI from a Slick gateway/service context (for example,
