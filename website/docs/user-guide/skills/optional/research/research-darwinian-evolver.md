...
| | |
|---|---|
| Source | Optional — install with `slick skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Slick Agent |
| License | MIT |
...
import upstream classes into Slick itself.
...
```bash
mkdir -p ~/.slick/cache/darwinian-evolver && cd ~/.slick/cache/darwinian-evolver
...
```bash
cd ~/.slick/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.slick/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.slick/skills/research/darwinian-evolver
DE_DIR=~/.slick/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Slick core.
   Custom driver scripts under `~/.slick/skills/...` are user-side and fine.
