---
title: "Honcho — Configure and troubleshoot Honcho memory for Slick"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Slick"
---
...
Configure and troubleshoot Honcho memory for Slick.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `slick skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Slick

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Slick profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/hackit#running-honcho-locally-with-hackit

### Verify

```bash
slick honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Slick profile's AI peer
...
Honcho models conversations as interactions between **peers**. Slick creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Slick instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Slick run |
| `global` | Single session across all directories |

Manual override: `slick honcho map my-project-name`
...
Each Slick profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block slick.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `slick.coder` host block in `honcho.json`
...
```bash
slick honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Slick when Honcho memory is active.
...
Config file: `$SLICK_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `slick honcho setup`. Ensure `memory.provider: honcho` is in `~/.slick/config.yaml`.

### Memory not persisting across sessions
Check `slick honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `slick profile create <name> --clone`. For existing profiles: `slick honcho sync`.
...
| Command | Description |
|---------|-------------|
| `slick honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `slick honcho status` | Show resolved config, connection test, peer info for active profile |
| `slick honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `slick honcho disable` | Disable Honcho for the active profile |
| `slick honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `slick honcho peers` | Show peer identities across all profiles |
| `slick honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `slick honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `slick honcho sessions` | List known directory-to-session-name mappings |
| `slick honcho map <name>` | Map current working directory to a Honcho session name |
| `slick honcho identity` | Seed AI peer identity or show both peer representations |
| `slick honcho sync` | Create host blocks for all Slick profiles that don't have one yet |
| `slick honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Slick + Honcho |
| `slick memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `slick memory status` | Show active memory provider and config |
| `slick memory off` | Disable external memory provider |
