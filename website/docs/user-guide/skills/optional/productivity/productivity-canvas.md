...
| | |
|---|---|
| Source | Optional — install with `slick skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Slick Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${SLICK_HOME:-~/.slick}/.env`:
...
```bash
CANVAS="python $SLICK_HOME/skills/productivity/canvas/scripts/canvas_api.py"
