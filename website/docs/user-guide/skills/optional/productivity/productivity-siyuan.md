...
| | |
|---|---|
| Source | Optional — install with `slick skills install official/productivity/siyuan` |
...
3. Store it in `${SLICK_HOME:-~/.slick}/.env`:
   ```
...
```yaml
# In ~/.slick/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
