...
| | |
|---|---|
| Source | Optional — install with `slick skills install official/productivity/shopify` |
...
4. Save to `${SLICK_HOME:-~/.slick}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Slick","productType":"Apparel","tags":["test"]}}'
```
