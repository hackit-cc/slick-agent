...
| | |
|---|---|
| Source | Optional — install with `slick skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SLICK_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SLICK_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${SLICK_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client slick
```
...
```bash
PUBLISH="${SLICK_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client slick
```
...
```bash
DRIVE="${SLICK_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `slick`)    |
