...
| | |
|---|---|
| Source | Optional — install with `slick skills install official/mcp/fastmcp` |
...
| Related skills | [`slick-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-slick-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Slick or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Slick. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.slick/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.slick/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Slick integration, either:

- configure the server in `~/.slick/config.yaml` using the `native-mcp` skill, or
...
### Slick cannot see the deployed server

The server-building part may be correct while the Slick config is not. Load the `native-mcp` skill and configure the server in `~/.slick/config.yaml`, then restart Slick.
