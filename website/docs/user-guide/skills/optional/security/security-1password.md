...
| | |
|---|---|
| Source | Optional — install with `slick skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Slick Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Slick terminal calls (desktop app flow only)
...
### Service Account (recommended for Slick)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${SLICK_HOME:-~/.slick}/.env` (the skill will prompt for this on first load).
...
## Slick Execution Pattern (desktop app flow)

Slick terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/slick-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/slick-op.sock"
