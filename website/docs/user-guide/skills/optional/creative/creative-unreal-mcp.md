...
| | |
|---|---|
| Source | Optional — install with `slick skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Slick MCP catalog. The
...
Two halves, in this order: the editor side must be up before Slick connects.
...
### One-time, Slick side
    ...
    slick mcp install unreal-engine
...
`~/.slick/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Slick — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Slick connects
...
2. Start the Slick session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Slick
they appear as:
...
then happens at `slick mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Slick runs batched calls
...
- **Start order matters.** Editor + server up first, then the Slick
...
- **Timeouts on long operations.** Slick' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.slick/config.yaml` for
