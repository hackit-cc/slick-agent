...
title: "Git Worktrees"
description: "Run multiple Slick agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Slick Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Slick so each session has a clean, isolated working directory.

## Why Use Worktrees with Slick?

Slick treats the **current working directory** as the project root:

- CLI: the directory where you run `slick` or `slick chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.slick/config.yaml`
...
Slick creates `.worktrees/my-experiment/` inside the repo (branch
`slick/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `slick-<id>`
...
git worktree add ../repo-feature feature/slick-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/slick-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Slick there:

```bash
cd ../repo-feature

# Start Slick in the worktree
slick
```
...
git worktree add ../repo-experiment-a feature/slick-a
git worktree add ../repo-experiment-b feature/slick-b
```
...
- Works on its own branch (`feature/slick-a` vs `feature/slick-b`).
...
- Slick checkpoint data under `~/.slick/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Slick experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/slick-checkpoints-docs`, `feature/slick-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Slick from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `slick -w` (Automatic Worktree Mode)

Slick has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `slick/slick-<hash>`).
...
```bash
slick -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `slick -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Slick session its own clean checkout.
