...
description: "The self-improving AI agent built by Hackit. A built-in learning loop that creates skills from experience, improves them during use, and remembers across sessions."
...
The self-improving AI agent built by [Hackit](https://hackit.cc). The only agent with a built-in learning loop — it creates skills from experience, improves them during use, nudges itself to persist knowledge, and builds a deepening model of who you are across sessions.
...
After installing, run `slick setup --portal` — one OAuth covers a model plus all four Tool Gateway tools (web search, image generation, TTS, browser). See [Hackit Portal](/integrations/hackit-portal).
:::
...
- **Built by model trainers** — Created by [Hackit](https://hackit.cc), the lab behind Slick, Nomos, and Psyche. Works with [Hackit Portal](https://portal.hackit.cc), [OpenRouter](https://openrouter.ai), OpenAI, or any endpoint
...
- **Full web control** — Search, extract, browse, vision, image generation, TTS — one subscription via [Hackit Portal](/integrations/hackit-portal) bundles all of them
- **MCP support** — Connect to any MCP server for extended tool capabilities
- **Research-ready** — Batch processing, trajectory export, RL training with Atropos. Built by [Hackit](https://hackit.cc) — the lab behind Slick, Nomos, and Psyche models
