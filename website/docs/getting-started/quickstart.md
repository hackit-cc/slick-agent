...
To easily install the command-line and desktop applications, [download the Slick Desktop installer](https://slick-agent.hackit.cc/) from our website and run it.
...
:::tip Easiest path: Hackit Portal
...
That logs you in, sets Hackit as your provider, and turns on the Tool Gateway in one command.
:::

:::info Setup modes
On a fresh install, `slick setup` offers three modes:

- **Quick Setup (Hackit Portal)** — free OAuth login, no API keys; sets up a model plus the Tool Gateway tools. The recommended fast path.
...
|----------|-----------|---------------|
| **Hackit Portal** | Subscription-based, zero-config | OAuth login via `slick model` |
