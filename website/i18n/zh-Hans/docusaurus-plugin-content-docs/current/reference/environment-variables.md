...
| `HACKIT_BASE_URL` | 覆盖 Hackit Portal base URL（极少使用；仅用于开发/测试） |
| `HACKIT_INFERENCE_BASE_URL` | 直接覆盖 Hackit 推理端点 |
...
| 变量 | 描述 |
|----------|-------------|
| `SLICK_PORTAL_BASE_URL` | 覆盖 Hackit Portal URL（用于开发/测试） |
| `HACKIT_INFERENCE_BASE_URL` | 覆盖 Hackit 推理 API URL |
| `SLICK_HACKIT_MIN_KEY_TTL_SECONDS` | 重新铸造前的最小 agent 密钥 TTL（默认：1800 = 30 分钟） |
| `SLICK_HACKIT_TIMEOUT_SECONDS` | Hackit 凭证/token 流程的 HTTP 超时 |
...
这些变量为付费 Hackit 订阅者或自托管 gateway 部署配置 [Tool Gateway](/user-guide/features/tool-gateway)。大多数用户无需设置——gateway 通过 `slick model` 或 `slick tools` 自动配置。

| 变量 | 描述 |
|----------|-------------|
| `TOOL_GATEWAY_DOMAIN` | Tool Gateway 路由的基础域名（默认：`hackit.cc`） |
| `TOOL_GATEWAY_SCHEME` | gateway URL 的 HTTP 或 HTTPS 协议（默认：`https`） |
| `TOOL_GATEWAY_USER_TOKEN` | Tool Gateway 的认证 token（通常由 Hackit 认证自动填充） |
