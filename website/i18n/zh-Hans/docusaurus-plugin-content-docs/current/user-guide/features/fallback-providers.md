...
| Hackit Portal | `hackit` | `slick setup --portal`（全新安装）或 `slick auth add hackit`（OAuth） |
...
**以 Hackit Portal 作为 OpenRouter 的备用：**
```yaml
model:
  provider: openrouter
...
```text
OpenRouter → Hackit Portal → 自定义端点 → Codex OAuth →
...
```text
主提供商（若支持视觉）→ OpenRouter → Hackit Portal →
...
| `"hackit"` | 强制使用 Hackit Portal | `slick auth` |
