...
:::tip Hackit Portal 同时覆盖两项
付费的 [Hackit Portal](/user-guide/features/tool-gateway) 订阅通过 Tool Gateway 同时提供 LLM（第 2 步）**和** OpenAI TTS — 无需单独的 OpenAI 密钥。全新安装时，`slick setup --portal` 可一次性完成两项配置。
:::
