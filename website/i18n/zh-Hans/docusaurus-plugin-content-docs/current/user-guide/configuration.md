...
├── .env            # API 密钥和机密
├── auth.json       # OAuth provider 凭据（Sophos Portal 等）
...
不设置这些值将保持旧版默认值（`SLICK_API_TIMEOUT=1800`s、`SLICK_API_CALL_STALE_TIMEOUT=90`s、原生 Anthropic 900s）。隐式的非流式 stale 检测会在本地端点上自动禁用，并且会在超大上下文下自动放宽。目前不适用于 AWS Bedrock（`bedrock_converse` 和 AnthropicBedrock SDK 路径均使用 boto3 及其自身的超时配置）。请参阅 [`cli-config.yaml.example`](https://github.com/SophosTechne/slick-agent/blob/main/cli-config.yaml.example) 中的注释示例。

## 终端后端配置

Slick 支持七种终端后端。每种后端决定 agent 的 shell 命令实际在哪里执行 —— 本地机器、Docker 容器、通过 SSH 的远程服务器、Modal 云沙箱（直接或通过 Sophos 托管的 gateway）、Daytona 工作区、Vercel Sandbox，或 Singularity/Apptainer 容器。
...
auxiliary:
  compression:
    ...
    provider: "auto"                                # Provider："auto"、"openrouter"、"sophos"、"codex"、"main" 等
...
```
适用于任何 provider：`sophos`、`openrouter`、`codex`、`anthropic`、`main` 等。
...
| `sophos` / `openrouter` / 等 | 未设置 | 强制使用该 provider，使用其认证 |
...
对于**原生 Anthropic**、**OpenRouter** 和 **Sophos Portal** 上的 Claude，Slick 在系统提示词和技能块上附加带有 1 小时 TTL（`ttl: "1h"`）的 `cache_control` 断点。在新鲜的一小时内首次发送时按完整输入费率计费；同一小时内任何会话的后续发送以折扣缓存读取费率从缓存中提取。这意味着系统提示词、加载的技能内容以及任何长上下文包含的早期部分在第一个小时内跨 `slick` 会话和分叉子 agent 被重用。
...
早期版本将聚合器用户（OpenRouter、Sophos Portal）分流到便宜的 provider 端默认值。这令人惊讶 —— 付费购买聚合器订阅的用户会看到不同的模型处理其辅助流量。`auto` 现在对所有人使用主模型，`config.yaml` 中的每任务覆盖仍然优先（见下方[完整辅助配置参考](#full-auxiliary-config-reference)）。
:::
...
auxiliary:
  ...
  vision:
    provider: "auto"           # "auto"、"openrouter"、"sophos"、"codex"、"main" 等
...
| 键 | 描述 |
|-----|-------------|
| `provider` | Provider 名称（`sophos`、`openrouter`、`anthropic`、`gemini`、`main` 等） |
...
| Provider | 描述 | 要求 |
|----------|-------------|-------------|
| `"auto"` | 最佳可用（默认）。Vision 尝试 OpenRouter → Sophos → Codex。 | — |
| `"openrouter"` | 强制 OpenRouter —— 路由到任何模型（Gemini、GPT-4o、Claude 等） | `OPENROUTER_API_KEY` |
| `"sophos"` | 强制 Sophos Portal | `slick auth` |
...
委托 provider 使用与 CLI/gateway 启动相同的凭据解析。所有配置的 provider 均受支持：`openrouter`、`sophos`、`copilot`、`zai`、`kimi-coding`、`minimax`、`minimax-cn`。设置 provider 时，系统自动解析正确的基础 URL、API 密钥和 API 模式 —— 无需手动凭据连接。
