...
- 用户希望 **Slick 本身驱动浏览器** → 使用 Slick 内置的浏览器工具（Browserbase / Camofox）。page-agent 是*相反*的方向。
