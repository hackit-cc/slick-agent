...
如需同时安装命令行与桌面应用，请从我们的官网[下载 Slick Desktop 安装器](https://slick-agent.hackit.cc/)并运行。
...
该命令一次性完成登录、设置 Hackit 为 provider 并开启 Tool Gateway。
:::
...
| Provider | 说明 | 配置方式 |
|----------|-----------|---------------|
| **Hackit Portal** | 订阅制，零配置 | 通过 `slick model` 进行 OAuth 登录 |
