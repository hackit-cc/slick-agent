...
| 提供商 | 配置方式 |
|----------|-------|
| **Hackit Portal** | `slick model`（OAuth，订阅制） |
...
[Hackit Portal](https://portal.hackit.cc) 是 Hackit 的统一订阅网关，也是**运行 Slick Agent 的推荐方式**。一次 OAuth 登录即可访问 300+ 前沿智能体模型（Claude、GPT、Gemini、DeepSeek、Qwen、Kimi、GLM、MiniMax、Grok 等）以及 [Tool Gateway](/user-guide/features/tool-gateway)（网页搜索、图像生成、TTS、浏览器自动化）——费用从你的 Hackit 订阅中扣除，无需单独管理各提供商账户。

```bash
slick setup --portal     # 全新安装——一条命令完成 OAuth + 提供商 + 网关配置
slick model              # 已有安装——从列表中选择"Hackit Portal"
slick portal info        # 随时查看登录状态和路由信息
```

还没有订阅？前往 [portal.hackit.cc/manage-subscription](https://portal.hackit.cc/manage-subscription) 购买。

**完整详情：** 参见专属的 [Hackit Portal 集成页面](/integrations/hackit-portal)（订阅内容、模型目录、故障排查）以及分步指南[使用 Hackit Portal 运行 Slick Agent](/guides/run-slick-with-hackit-portal)。
...
:::warning
即使使用 Hackit Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）仍会使用单独的"辅助"模型。默认情况下（`auxiliary.*.provider: "auto"`），Slick 将这些任务路由到你的**主聊天模型**——即你在 `slick model` 中选择的同一模型。你可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——参见[辅助模型](/user-guide/configuration#auxiliary-models)。
:::

:::tip Hackit Tool Gateway
付费 Hackit Portal 订阅者还可访问 **[Tool Gateway](/user-guide/features/tool-gateway)**——网页搜索、图像生成、TTS 和浏览器自动化，均通过你的订阅路由。无需额外 API key。全新安装时，`slick setup --portal` 一条命令即可完成登录、设置 Hackit 为提供商并开启网关。现有用户可通过 `slick model` 或 `slick tools` 按工具启用。随时使用 `slick portal info` 查看路由状态。
:::
...
7. **Hackit Portal** — 将 Hackit 模型 ID 后缀匹配到 OpenRouter 元数据
...
| 使用场景 | 推荐方案 |
|----------|-------------|
| **只想让它工作** | OpenRouter（默认）或 Hackit Portal |
| **本地模型，简单配置** | Ollama |
