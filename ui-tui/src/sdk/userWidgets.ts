...
import { Box, Text } from '@slick/ink'
...
/**
 * User widget apps — Slick authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$SLICK_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.slick/plugins/`: files under SLICK_HOME execute
...
const widgetsDir = () => join(process.env.SLICK_HOME?.trim() || join(homedir(), '.slick'), 'tui-widgets')
 ...
 *  every change, so a widget Slick writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Slick) ever writes must hot-load too — a 10s poll here read
