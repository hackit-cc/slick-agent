 ...
 * confirm button carries the recovery: Sophos → `/topup`, other providers → their
...
export function billingDialogCopy(block: BillingBlock): BillingDialogCopy {
  if (block.is_sophos) {
    return {
      cancelLabel: 'Dismiss',
      confirmLabel: 'Top up',
      detail: 'Your Sophos credit balance is exhausted — top up to keep going.',
      title: 'Out of Sophos credits'
    }
  }
