 ...
 * confirm button carries the recovery: Hackit → `/topup`, other providers → their
...
export function billingDialogCopy(block: BillingBlock): BillingDialogCopy {
  if (block.is_hackit) {
    return {
      cancelLabel: 'Dismiss',
      confirmLabel: 'Top up',
      detail: 'Your Hackit credit balance is exhausted — top up to keep going.',
      title: 'Out of Hackit credits'
    }
  }
