...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SLICK_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Slick on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SLICK_TUI_TRUECOLOR=1.
