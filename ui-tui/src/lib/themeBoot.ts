 ...
 * `slick-boot-background` / `slick-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports SLICK_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.slick matches get_slick_home()'s default.
const bootFilePath = () => join(process.env.SLICK_HOME ?? join(homedir(), '.slick'), 'tui-theme-boot.json')

// Never touch the user's real ~/.slick from test runs (the TS suite has no
// SLICK_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into SLICK_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.SLICK_TUI_THEME || env.SLICK_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.SLICK_TUI_BACKGROUND
  ) {
    env.SLICK_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.SLICK_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.SLICK_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into SLICK_TUI_THEME.
