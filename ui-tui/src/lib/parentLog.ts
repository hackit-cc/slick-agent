...
const logDir = join(process.env.SLICK_HOME?.trim() || join(homedir(), '.slick'), 'logs')
...
// a real ~/.slick (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('slick-tui: parent lifecycle log unavailable\n')
    }
  }
}
