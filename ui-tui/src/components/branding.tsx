import { Box, Text, useStdout } from '@slick/ink'
import { useEffect, useState } from 'react'
import unicodeSpinners from 'unicode-animations'

import { artWidth, caduceus, SMILY_WIDTH, logo, LOGO_WIDTH } from '../banner.js'
...
const TAG_FULL = 'Sophos Techne · Messenger of the Digital Gods'
const TAG_MID = 'Messenger of the Digital Gods'
const TAG_TINY = 'Sophos Techne'
const HIDE_BELOW = 34
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  const leftW = Math.min((artWidth(heroLines) || SMILY_WIDTH) + 4, Math.floor(cols * 0.4))
  ...
  // slick_cli/banner.py) and the "connected" label on the collapse toggle.
  ...
  const heroColumn = wide ? (
    <Box flexDirection="column" width="100%">
      ...
      <Text color={t.color.accent}>
        ...
        <Text color={t.color.muted}> · Sophos Techne</Text>
      </Text>
    ...
    <Box flexDirection="column" width="100%">
      ...
      {typeof info.update_behind === 'number' && info.update_behind > 0 && (
        <Text bold color={t.color.warn}>
          ...
          <Text bold color={t.color.warn}>
            {info.update_command || 'slick update'}
          </Text>
          <Text bold={false} color={t.color.warn} dimColor>
            {' '}
            to update
