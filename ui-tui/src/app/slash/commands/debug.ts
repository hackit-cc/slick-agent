...
import { terminalBackgroundHex } from '@slick/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $SLICK_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see SLICK_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['SLICK_TUI_BACKGROUND', process.env.SLICK_TUI_BACKGROUND ?? '(unset)'],
            ['SLICK_TUI_THEME', process.env.SLICK_TUI_THEME ?? '(unset)'],
