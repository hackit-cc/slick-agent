import type { RunExternalProcess } from '@slick/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`slick ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching slick: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`slick ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
