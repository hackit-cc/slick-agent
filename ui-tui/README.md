...
React + Ink terminal UI for Slick. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SLICK_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `slick setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage Hackit remote spending — buy credits, auto-reload, limits
...
- Input history is stored in `~/.slick/.slick_history` or under `SLICK_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage Hackit remote spending — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — Hackit credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `slick setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/slick-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external slick setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage Hackit remote spending
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.slick/tui-parent.log
    ...
    types/
      slick-ink.d.ts            type declarations for @slick/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent SlickCLI subprocess for slash commands
```
