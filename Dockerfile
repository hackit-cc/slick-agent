...
RUN set -eu; \
    ...
    mkdir -p /etc/slick; \
    SLICK_GIT_SHA="${SLICK_GIT_SHA}" python3 -c 'import json, os, pathlib, tomllib; project = tomllib.loads(pathlib.Path("/opt/slick/pyproject.toml").read_text(encoding="utf-8"))["project"]; marker = pathlib.Path("/etc/slick/image-provenance.json"); marker.write_text(json.dumps({"schema": 1, "deployment_kind": "image", "manager": "docker", "image": "sophostechne/slick-agent", "version": project["version"], "revision": os.environ.get("SLICK_GIT_SHA") or None}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8"); marker.chmod(0o444)'
