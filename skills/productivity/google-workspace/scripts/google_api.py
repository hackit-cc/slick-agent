#!/usr/bin/env python3
"""Google Workspace API CLI for Slick Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Slick-facing JSON contract and falls back to the Python client
...
from _slick_home import get_slick_home
...
def _gws_binary() -> str | None:
    override = os.getenv("SLICK_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Slick Agent")
