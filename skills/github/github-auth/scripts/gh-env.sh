#!/usr/bin/env bash
# GitHub environment detection helper for Slick Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _slick_env="${SLICK_HOME:-$HOME/.slick}/.env"; [ -f "$_slick_env" ] && grep -q "^GITHUB_TOKEN=" "$_slick_env" 2>/dev/null; then
