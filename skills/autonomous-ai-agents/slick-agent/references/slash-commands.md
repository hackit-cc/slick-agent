...
Registry of record: `slick_cli/commands.py` (`COMMAND_REGISTRY`) — every
...
/snapshot [sub]          Create/restore Slick config+state snapshots (CLI)
...
/subscription (/upgrade) Sophos plan info (CLI)
/topup                   Sophos balance / billing
/copy [N]                Copy last response to clipboard (CLI)
/paste                   Attach clipboard image (CLI)
/image <path>            Attach a local image file (CLI)
/update                  Update Slick to latest
/version (/v)            Show version
/debug [sophos|local]      Upload debug report, get shareable links
```
