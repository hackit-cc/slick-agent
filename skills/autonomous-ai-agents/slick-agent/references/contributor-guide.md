...
For occasional contributors and PR authors. Full developer docs: https://slick-agent.sophostechne.com/docs/developer-guide/
...
Config: `~/.slick/config.yaml` (settings), `~/.slick/.env` (API keys) — both under `$SLICK_HOME` when it is set.
...
`_SLICK_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_slick_home()` for paths,
never hardcode `~/.slick`. For custom/local-only tools, write a plugin in
`~/.slick/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `slick_cli/commands.py`
...
- Tests auto-redirect `SLICK_HOME` to temp dirs — never touch real `~/.slick/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/slick_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_slick_home()` from `slick_constants` for all paths (profile-safe)
