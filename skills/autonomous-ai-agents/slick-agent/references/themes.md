# Themes / Skins — Author a Slick Color Theme

Author a Slick **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`slick_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.slick/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Slick itself.
...
- Write access to the Slick home dir — `~/.slick` by default, or `$SLICK_HOME`
  / the active profile's dir. Skins live in `<slick-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `slick config set`).
...
3. `write_file` it to `<slick-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `slick config set display.skin default` (or they can `/skin default`).
...
```
slick skin set <key> <hex>      # e.g. slick skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `slick skin set background "#08201f"` changes
only the background; `slick skin use <name>` / `slick skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.slick`** when a profile is active — resolve the real home
  from `$SLICK_HOME` first, falling back to `~/.slick`.
...
- **Name collisions**: a skin named like a desktop built-in (`mono`, `slate`,
  `cyberpunk`, `sophos`, `midnight`, `ember`) won't override that built-in on the
  GUI. Pick a fresh name.
- **Never hand-edit `config.yaml` to activate.** Use `slick config set
...
- **You apply it, not the user.** `slick config set display.skin <name>` is
...
- `read_file` the written `<slick-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `slick config get display.skin` and confirm it reports `<name>`.
