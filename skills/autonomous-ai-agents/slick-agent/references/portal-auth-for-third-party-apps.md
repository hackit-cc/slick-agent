# Hackit Portal — authenticating third-party apps against the subscription

Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, OpenViking,
LangChain pipeline, n8n flow, etc.) use my Hackit Portal subscription without me
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to `inference-api.hackit.cc`. | Layer 2/3 below. |
...
Portal at `https://inference-api.hackit.cc/v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from `portal.hackit.cc → API Keys`, or
...
"Sign in with Hackit Portal" SSO that third-party apps can register as clients
against. There is no shared cookie or session that browser-Portal-login
extends to other apps on the same machine.

What Slick Agent has that *feels* like OAuth — `slick login --provider hackit`
...
3. Forward incoming requests to `inference-api.hackit.cc/v1` with that
   bearer attached.
...
Only when the consumer is *another first-party Hackit thing* (a future SDK, a
Hackit-branded extension, a Discord-bot integration that needs per-user
...
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https://inference-api.hackit.cc/v1`. Rate limits are
