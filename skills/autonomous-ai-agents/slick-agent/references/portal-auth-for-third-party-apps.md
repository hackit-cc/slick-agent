# Sophos Portal — authenticating third-party apps against the subscription

Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, OpenViking,
LangChain pipeline, n8n flow, etc.) use my Sophos Portal subscription without me
...
## Layer 1 — Is this thing a Slick plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Slick process**. Its LLM calls go through Slick's already-configured provider. | Already uses Portal if user's Slick is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to `inference-api.sophostechne.com`. | Layer 2/3 below. |

**Pitfall to avoid**: do not pitch "OAuth into Portal" as the solution for a
plugin that already runs inside Slick. That LLM call is already authenticated
via Slick's provider config. The plugin's own server auth (e.g.
...
Portal at `https://inference-api.sophostechne.com/v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from `portal.sophostechne.com → API Keys`, or
...
"Sign in with Sophos Portal" SSO that third-party apps can register as clients
against. There is no shared cookie or session that browser-Portal-login
extends to other apps on the same machine.

What Slick Agent has that *feels* like OAuth — `slick login --provider sophos`
opening a browser, user signs in, token lands in `~/.slick/auth.json` — is a
**Slick-specific browser flow**. Under the hood it produces a credential
Slick uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Slick's existing Portal credential out of `~/.slick/auth.json`.
2. Expose a local OpenAI-compatible endpoint at `http://localhost:NNNN/v1`.
3. Forward incoming requests to `inference-api.sophostechne.com/v1` with that
   bearer attached.
...
rides on the credential Slick already holds.

Where this could live in Slick:
...
Only when the consumer is *another first-party Sophos thing* (a future SDK, a
Sophos-branded extension, a Discord-bot integration that needs per-user
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Slick itself.
...
1. First decide: Slick plugin (runs inside Slick) or separate app? If plugin,
   it already uses Portal via Slick's provider config — done.
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https://inference-api.sophostechne.com/v1`. Rate limits are
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Slick agent itself") is fully supported. Slick treats agent
...
pitch in the Tool Gateway announcement is specifically about Slick Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Slick is configured to use Portal as its provider. It is
