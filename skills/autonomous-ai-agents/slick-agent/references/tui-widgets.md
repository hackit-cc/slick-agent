...
Author widget apps for the Slick TUI (`slick --tui`): glanceable ambient
...
- The TUI must be in use (`slick --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.slick/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.slick/logs/tui_gateway_crash.log` if a widget never appears.
