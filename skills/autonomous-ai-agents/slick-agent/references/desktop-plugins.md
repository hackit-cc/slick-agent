...
Write plugins for the Slick desktop app: statusbar items, layout panes,
...
system (`~/.slick/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$SLICK_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$SLICK_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Slick desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$SLICK_HOME/desktop-plugins/` (usually
  `~/.slick/desktop-plugins/`).

## How to Run

1. Create `$SLICK_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.slick/...` by default, or `~/.slick/profiles/<profile>/...` under a
...
The ONLY import surface is `@slick/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Slick
  ...
  (`slick://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.slick/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@slick/plugin-sdk`, `react`, and
