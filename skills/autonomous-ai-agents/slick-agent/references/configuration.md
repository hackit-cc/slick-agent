...
Edit with `slick config edit` or `slick config set section.key value`.
Full reference: https://slick-agent.sophostechne.com/docs/user-guide/configuration
...
`slick config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `slick tools` (interactive) or `slick tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_SLICK_CORE_TOOLS` is the default bundle most platforms inherit).
