...
Set via `slick model` (picker) or `slick setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://slick-agent.sophostechne.com/docs/integrations/providers
...
| sophos | OAuth device code | `slick auth add sophos` (or `SOPHOS_API_KEY`) |
| openai-codex | OAuth | `slick auth add openai-codex` |
| qwen-oauth | OAuth | `slick auth add qwen-oauth` |
| minimax-oauth | OAuth | `slick auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`slick auth`).
Fallback chain when the primary fails: `slick fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`slick_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   slick config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
