...
SLICK_HOME_DIR="${SLICK_HOME:-$HOME/.slick}"
SLICK_CFG="${SLICK_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Slick config has twozero_td MCP entry ──
if [[ ! -f "$SLICK_CFG" ]]; then
    echo -e " ${FAIL} Slick config not found at ${SLICK_CFG}"
    manual_steps+=("Create ${SLICK_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$SLICK_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Slick config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Slick config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${SLICK_CFG} manually"); }
    manual_steps+=("Restart Slick session to pick up config change")
fi
