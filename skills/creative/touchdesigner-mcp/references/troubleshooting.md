...
`$SLICK_HOME/config.yaml` (defaults to `~/.slick/config.yaml` when `SLICK_HOME` is unset)
...
Restart the Slick session for changes to take effect. The MCP connection is
