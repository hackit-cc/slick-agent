...
Ledger path resolution (first wins):
  --ledger PATH
  $SLICK_CITATION_LEDGER
  $SLICK_HOME/cache/citations/ledger.json
"""
...
from _slick_home import get_slick_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("SLICK_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_slick_home() / "cache" / "citations" / "ledger.json"
