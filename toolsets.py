...
_SLICK_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (SLICK_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `slick tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (SLICK_KANBAN_TASK env "
    ...
    # Coding posture (base Slick — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in slick_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Slick toolsets (CLI + messaging platforms)
    #
    ...
    # the `slick send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "slick-cli": {
        ...
        "tools": _SLICK_CORE_TOOLS,
        "includes": []
    },

    "slick-cron": {
        # Mirrors slick-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `slick tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as slick-cli; gated by `slick tools`",
        "tools": _SLICK_CORE_TOOLS,
        "includes": []
    },
    ...
    "slick-email": {
        "description": "Email bot toolset - interact with Slick via email (IMAP/SMTP)",
        "tools": _SLICK_CORE_TOOLS,
        "includes": []
    },
    ...
    "slick-sms": {
        "description": "SMS bot toolset - interact with Slick via SMS (Twilio)",
        "tools": _SLICK_CORE_TOOLS,
        "includes": []
    },
    ...
    "slick-webhook": {
        ...
        "tools": _SLICK_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "slick-gateway": {
        ...
        "tools": [],
        "includes": ["slick-telegram", "slick-discord", "slick-whatsapp", "slick-slack", "slick-signal", "slick-bluebubbles", "slick-homeassistant", "slick-email", "slick-sms", "slick-mattermost", "slick-matrix", "slick-dingtalk", "slick-feishu", "slick-wecom", "slick-wecom-callback", "slick-weixin", "slick-qqbot", "slick-webhook", "slick-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``slick-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_SLICK_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``slick-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (slick-<name>).
        # Gives them _SLICK_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("slick-"):
            platform_name = name[len("slick-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_SLICK_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
