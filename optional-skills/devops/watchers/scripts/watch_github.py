...
Usage (via cron with --no-agent):
    ...
    slick cron create slick-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$SLICK_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name slick-issues --repo SophosTechne/slick-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Slick .env file
(``${SLICK_HOME:-~/.slick}/.env``) to avoid the 60 req/hr
