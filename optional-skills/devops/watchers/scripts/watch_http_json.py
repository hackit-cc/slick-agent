...
Usage (via cron with --no-agent):
    ...
    slick cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$SLICK_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Slick-Watcher/1.0"})
