...
### 1. Start order: editor first, Slick second

Slick probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Slick session.
...
restart the editor, restart the Slick session.
...
server fails to bind (Output Log) or Slick' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.slick/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Slick

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Slick' connection
lives in `~/.slick/config.yaml` via `slick mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Slick — just don't
mistake it for the Slick setup step.
...
explicitly warns against overlapping calls. Slick executes same-turn tool
...
### 9. Timeouts: Slick gives up before Unreal does

Slick' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Slick runs elsewhere (SSH backend,
