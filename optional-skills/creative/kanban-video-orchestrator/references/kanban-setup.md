...
the actual `setup.sh` that creates the project workspace, configures Slick
...
> [SophosTechne/kanban-video-pipeline](https://github.com/SophosTechne/kanban-video-pipeline).
...
2. **Create profiles** — `slick profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.slick/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `slick kanban create` assigned to the director
...
```bash
slick profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.slick/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.slick/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Slick
...
Each profile gets a `SOUL.md` at `~/.slick/profiles/<name>/SOUL.md` that
...
```bash
slick kanban create "Direct production of <video title>" \
    --assignee director \
...
the Slick `.env` (`${SLICK_HOME:-$HOME/.slick}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _slick_env="${SLICK_HOME:-$HOME/.slick}/.env"
    if grep -q "^${var}=" "$_slick_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_slick_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_slick_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY slick ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY slick OPENROUTER_API_KEY || exit 1
# ...
```
