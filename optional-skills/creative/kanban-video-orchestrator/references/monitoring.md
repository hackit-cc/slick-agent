...
slick kanban watch --tenant <project-slug>

# Snapshot of the board
slick kanban list --tenant <project-slug>
slick kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
slick kanban stats --tenant <project-slug>

# Visual dashboard (browser)
slick dashboard

# Inspect a specific task (includes comments + events)
slick kanban show <task-id>

# Follow a single task's event stream
slick kanban tail <task-id>
```

Verify available subcommands with `slick kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `slick kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `slick kanban block <id>` then `slick kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `slick kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `slick kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `slick profile list` (profile exists?), `slick status` (gateway/dispatcher up?), and verify tenant. |
...
slick kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
slick kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(slick kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
slick kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
slick kanban block <task-id>
slick kanban archive <task-id>

# Diagnose what happened
slick kanban show <task-id>      # task body, comments, recent events
slick kanban tail <task-id>      # follow the live event stream
slick kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `slick kanban create` for the director
...
while true; do
    clear
    slick kanban list --tenant <slug>
    echo "---"
    slick kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `slick kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `slick profile list` and start
