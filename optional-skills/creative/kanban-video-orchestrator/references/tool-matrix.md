...
Maps each role archetype to the Slick skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public slick-agent
...
## Slick skills relevant to video production

### Visual / rendering skills (`slick-agent/skills/creative/`)
...
### Design / pre-production skills (`slick-agent/skills/creative/`)
...
### Audio / media skills (`slick-agent/skills/creative/` + `skills/media/`)
...
These are **not** Slick skills but external CLIs / APIs that profiles invoke.
...
## Built-in Slick tools for media review

These are native Slick tools — not invoked via terminal but through their own
...
key is present in `${SLICK_HOME:-~/.slick}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Slick profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${SLICK_HOME:-~/.slick}/.env` → environment variable.
...
When a new Slick-public video skill ships:
