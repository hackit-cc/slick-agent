...
## 5. GODMODE FAST — Slick 4 405B (Zero Refusal)

**Model:** `sophostechne/slick-4-405b`
**Strategy:** Slick models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Slick Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.slick/config.yaml`:
