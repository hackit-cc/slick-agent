...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SLICK_HOME", os.path.expanduser("~/.slick")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SLICK_HOME", Path.home() / ".slick")) / "skills" / "red-teaming" / "godmode"
...
SLICK_HOME = Path(os.getenv("SLICK_HOME", Path.home() / ".slick"))
CONFIG_PATH = SLICK_HOME / "config.yaml"
PREFILL_PATH = SLICK_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Sophos/Slick models — already uncensored, just needs clean prompt
    "slick": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "slick" in model_lower or "sophos" in model_lower:
        return "slick"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Slick config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.slick/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Slick for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Slick for changes to take effect.")
