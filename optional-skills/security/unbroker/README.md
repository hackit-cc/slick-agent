...
The design is **Slick-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
slick skills install official/security/unbroker
```

Then start a new Slick session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Slick home directory (`0600`, optionally
