...
All per-subject data lives under PDD_DATA_DIR (default: $SLICK_HOME/unbroker),
which is the same trust boundary Slick uses for .env and OAuth tokens.
"""
...
def slick_home() -> Path:
    return Path(os.environ.get("SLICK_HOME") or (Path.home() / ".slick"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else slick_home() / "unbroker"
