...
Usage:
  ...
  <server_name> matches the files in $SLICK_HOME/mcp-tokens/<server>.json etc.
...
def _slick_home():
    # Prefer Slick' own resolver (profile-safe); fall back to env then ~/.slick.
    try:
        from slick_constants import get_slick_home
        return str(get_slick_home())
    except Exception:
        return os.environ.get("SLICK_HOME") or os.path.expanduser("~/.slick")


def _tokens_dir():
    return os.path.join(_slick_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "slick-diag", "version": "1.0"}},
        },
        headers={
