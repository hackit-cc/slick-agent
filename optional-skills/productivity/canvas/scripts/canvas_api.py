#!/usr/bin/env python3
"""Canvas LMS API CLI for Slick Agent.
...
def _check_config():
    ...
    if missing:
        slick_env = os.path.join(
            os.environ.get("SLICK_HOME", os.path.expanduser("~/.slick")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {slick_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Slick Agent"
    )
