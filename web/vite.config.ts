...
const BACKEND = process.env.SLICK_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `slick dashboard` server injects a one-shot
 * session token into `index.html` (see `slick_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__SLICK_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function slickDevToken(): Plugin {
  const TOKEN_RE = /window\.__SLICK_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__SLICK_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "slick:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[slick] Could not find session token in ${BACKEND} — ` +
              `is \`slick dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__SLICK_SESSION_TOKEN__="${match[1]}";` +
              `window.__SLICK_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[slick] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`slick dashboard\` or set SLICK_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@slick/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
    // When @sophostechne/ui is symlinked via `file:../../design-language`,
      ...
      output: {
        codeSplitting: {
          ...
          groups: [
            ...
            {
              name: "ui",
              test: /node_modules[\\/]@sophostechne[\\/]ui([\\/]|$)/,
            },
            {
              name: "vendor",
              test: /node_modules[\\/]/,
  ...
  server: {
    proxy: {
      ...
      // Same host as `slick dashboard` must serve these; Vite has no
