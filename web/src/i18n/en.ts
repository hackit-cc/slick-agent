...
export const en: Translations = {
  ...
  status: {
    ...
    updateSlickConfirmMessage:
      "This runs slick update and restarts the gateway when it finishes. Active sessions keep their prompt cache until then.",
    updateSlickConfirmNow: "Update now",
    updateSlickConfirmTitle: "Update Slick?",
    updatingSlick: "Updating Slick…",
  ...
  pluginsPage: {
    ...
    headline:
      "Discover, install, enable, and update Slick plugins (`slick plugins` parity).",
    ...
    removeConfirm: "Remove this plugin from ~/.slick/plugins/?",
    removeHint: "Only user-installed plugins under ~/.slick/plugins can be removed.",
  ...
  skills: {
    ...
    filters: "Filters",
    noSkills: "No skills found. Skills are loaded from ~/.slick/skills/",
  ...
  env: {
    ...
    customHint: "Arbitrary environment variables stored in your .env that Slick doesn't recognise. Use these to inject env vars for skills, MCP servers, or your own tooling.",
  ...
  achievements: {
    hero: {
      ...
      title: "Slick Achievements",
      subtitle:
        "Collectible Slick badges earned from real session history. Known unfinished achievements are shown as Discovered; Secret achievements stay hidden until the first matching behavior appears.",
      scan_subtitle:
        "Scanning Slick session history. First scan can take 5–10 seconds on large histories.",
    },
    actions: {
      rescan: "Rescan",
    },
    stats: {
      ...
      highest_tier_hint: "Copper → Silver → Gold → Diamond → Root",
      latest: "Latest",
      latest_hint_empty: "run Slick more",
      none_yet: "None yet",
    },
    state: {
      unlocked: "Unlocked",
    ...
    guide: {
      ...
      secret_body:
        "Secrets hide their exact trigger. Once Slick sees a related signal, the card becomes Discovered and shows its requirement.",
      scan_status_header: "Scan status",
      scan_status_body:
        "Slick is scanning local history once, then cards will appear automatically. Nothing is stuck if this takes a few seconds.",
    ...
    share: {
      ...
      tweet_text: "Just unlocked {tier_part}\"{name}\" in Slick Agent 😎",
    },
  },
