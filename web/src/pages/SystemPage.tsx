...
import { Badge } from "@sophostechne/ui/ui/components/badge";
import { Button } from "@sophostechne/ui/ui/components/button";
import { Spinner } from "@sophostechne/ui/ui/components/spinner";
import { H2 } from "@sophostechne/ui/ui/components/typography/h2";
import { Card, CardContent } from "@sophostechne/ui/ui/components/card";
import { Checkbox } from "@sophostechne/ui/ui/components/checkbox";
import { Input } from "@sophostechne/ui/ui/components/input";
import { Label } from "@sophostechne/ui/ui/components/label";
import { Select, SelectOption } from "@sophostechne/ui/ui/components/select";
import { Toast } from "@sophostechne/ui/ui/components/toast";
import { useToast } from "@sophostechne/ui/hooks/use-toast";
import { useConfirmDelete } from "@sophostechne/ui/hooks/use-confirm-delete";
import { ConfirmDialog } from "@sophostechne/ui/ui/components/confirm-dialog";
import { useModalBehavior } from "@/hooks/useModalBehavior";
import { DeleteConfirmDialog } from "@/components/DeleteConfirmDialog";
import { SlickConsoleModal } from "@/components/SlickConsoleModal";
...
export default function SystemPage() {
  ...
  // spawned `slick import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_slick === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkSlickUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_slick],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_slick === false) {
      showToast(
        "Slick updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updateSlick();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "slick-update");
  ...
  const canUpdateSlick = status?.can_update_slick !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateSlick && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Slick?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'slick update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'slick update' (${updateInfo?.update_command ?? "slick update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Slick</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.slick_version}</span>
                  {canUpdateSlick &&
                    updateInfo &&
      ...
      <section className="flex flex-col gap-3">
        <H2 variant="sm" className="flex items-center gap-2 text-muted-foreground">
          <Globe className="h-4 w-4" /> Sophos Portal
        </H2>
        <Card>
          <CardContent className="flex flex-col gap-3 py-4">
            <div className="flex items-center gap-3">
              ...
              <a
                href={portal?.subscription_url || "https://portal.sophostechne.com/manage-subscription"}
                target="_blank"
                rel="noreferrer"
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">slick portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$SLICK_HOME/backups/slick-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Slick backup?"
              description={`This will overwrite your current Slick configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Slick team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
