...
interface SlickConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function SlickConsoleModal({ open, onClose }: SlickConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("slick> ");
  const inputPromptRef = useRef("slick> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "slick> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Slick Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="slick-console-title"
    >
      <div
        className={cn(
          themedBody,
