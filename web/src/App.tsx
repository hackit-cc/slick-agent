...
import { SelectionSwitcher } from "@sophostechne/ui/ui/components/selection-switcher";
import { Spinner } from "@sophostechne/ui/ui/components/spinner";
import { Typography } from "@sophostechne/ui/ui/components/typography/index";
import { ConfirmDialog } from "@sophostechne/ui/ui/components/confirm-dialog";
import { cn } from "@/lib/utils";
import { SidebarFooter } from "@/components/SidebarFooter";
import { SidebarStatusStrip, gatewayLine } from "@/components/SidebarStatusStrip";
import { useBelowBreakpoint } from "@sophostechne/ui/hooks/use-below-breakpoint";
...
const SIDEBAR_COLLAPSED_KEY = "slick-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="slick-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="slick-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateSlick = status?.can_update_slick === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'slick update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "slick update";
    return (
      t.status.updateSlickConfirmMessage ??
      `This will run 'slick update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateSlickConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateSlick) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateSlick,
      runningLabel: t.status.updatingSlick,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Slick gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateSlickConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateSlickConfirmTitle ?? `${t.status.updateSlick}?`}
    />
    </>
  );
}
