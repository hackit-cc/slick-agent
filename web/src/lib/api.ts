import { buildSlickWebSocketUrl } from "@slick/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/slick/). The Python backend
// injects ``window.__SLICK_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__SLICK_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const SLICK_BASE_PATH = readBasePath();
const BASE = SLICK_BASE_PATH;
...
declare global {
  interface Window {
    __SLICK_SESSION_TOKEN__?: string;
    __SLICK_BASE_PATH__?: string;
    ...
    __SLICK_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Slick-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__SLICK_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``slick update``, ``slick gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__SLICK_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__SLICK_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__SLICK_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``slick_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__SLICK_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Slick-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``slick_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__SLICK_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateSlick: () =>
    fetchJSON<ActionResponse>("/api/slick/update", { method: "POST" }),
  checkSlickUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/slick/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * Sophos Portal contract V1 (the access token has no email/name claims —
...
export interface SkillHubSource {
  ...
  /** slick-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
  ...
  /** Phase 7: registered ``DashboardAuthProvider`` names (e.g. ``["sophos"]``).
   ...
   * updates are handled by the outer launcher instead of ``slick update``. */
  can_update_slick?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the SLICK_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the SLICK_HOME volume — no staleness dimension, so no
 * sampled_at. */
