/**
 * Slick Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__SLICK_PLUGIN_SDK__`` and
 * ``window.__SLICK_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__SLICK_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@sophostechne/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``SlickPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@slick/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @sophostechne/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__SLICK_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__SLICK_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__SLICK_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface SlickPluginSDK {
  ...
  /**
   * Shared UI primitives (Sophos DS / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __SLICK_PLUGIN_SDK__?: SlickPluginSDK;
    __SLICK_PLUGINS__?: PluginRegistry;
  }
}
