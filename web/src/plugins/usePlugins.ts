...
import { api, SLICK_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "slick:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${SLICK_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${SLICK_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?slick_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-slick-plugin", manifest.name);
