...
(local, remote URL, SSH, Slick Cloud, docker) must be able to find every
...
def _(rid, params: dict) -> dict:
    ...
    ``slick -p <profile> chat -c "Bot Chat"`` transport local DMs use and
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "slick" else profile
        if resolved not in known:
            return _err(rid, 4092, f"no profile '{profile}' on this gateway")

        fd, tmp = tempfile.mkstemp(prefix="slick-relay-dm-", suffix=".txt", text=True)
        try:
