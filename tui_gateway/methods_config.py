...
@method("diagnostics.share_sophos")
def _(rid, params: dict) -> dict:
    """Upload a redacted debug bundle to Sophos-internal diagnostics storage.

    Desktop's "Send Diagnostics" action (error card / diagnostics UI). Same
    collection + force-redaction pipeline as ``slick debug share --sophos``
    (collect_share_bundle → build_sophos_bundle → share_to_sophos); redaction is
    ...
    try:
        ...
        blob = build_sophos_bundle(bundle, redact=True)
