    ...
    try:
        with _profile_db(params) as db:
            ...
            with pdb.connect_closing() as conn:
                ...
                # zero Slick sessions still surface. The desktop's native scan
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from slick_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_slick_home), "display": display_slick_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _slick_home / "config.yaml"
        try:
        ...
        if (
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Slick provider is configured.",
                },
            )
...
@method("diagnostics.share_hackit")
def _(rid, params: dict) -> dict:
    """Upload a redacted debug bundle to Hackit-internal diagnostics storage.

    Desktop's "Send Diagnostics" action (error card / diagnostics UI). Same
    collection + force-redaction pipeline as ``slick debug share --hackit``
    (collect_share_bundle → build_hackit_bundle → share_to_hackit); redaction is
    ...
    try:
        ...
        blob = build_hackit_bundle(bundle, redact=True)
