...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind SLICK_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    with _profile_db(params) as db:
        ...
        try:
            ...
            # custom `SLICK_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        _enable_gateway_prompts()
        home_token = (
            set_slick_home_override(str(profile_home)) if profile_home is not None else None
        )
        secret_token = (
        ...
        with _session_resume_lock:
            ...
            try:
                init_home_token = (
                    set_slick_home_override(str(profile_home))
                    if profile_home is not None
                    else None
                )
                init_secret_token = (
                ...
                try:
                    ...
                    if init_home_token is not None:
                        reset_slick_home_override(init_home_token)
                ...
                if sid in _sessions:
                    ...
                    # Remember the profile home so each turn re-binds SLICK_HOME (the
        ...
        # atexit.register(_drain_token_queue_at_exit) (slick_state.py), which only
...
def _(rid, params: dict) -> dict:
    ...
    # ``slick --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``slick gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    # Sophos credits block — agent-independent (a portal fetch), so it shows even
    # with zero API calls or on a resumed session. The TUI /usage panel renders
    # these lines regardless of `calls`. Fail-open: [] when not logged into Sophos
    # or on any portal hiccup.
    try:
        from agent.account_usage import sophos_credits_lines

        credits = sophos_credits_lines()
        if credits:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from slick_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from slick_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from slick_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from slick_cli.pets import set_pet_scale
...
def _(rid, params: dict) -> dict:
    ...
    True only when a reference-capable image backend (Sophos Portal / OpenRouter /
...
def _(rid, params: dict) -> dict:
    ...
    from slick_cli.sophos_billing import BillingError, post_subscription_preview
...
def _(rid, params: dict) -> dict:
    ...
    billing:manage.
    """
    from slick_cli.sophos_billing import BillingError, put_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    """
    from slick_cli.sophos_billing import BillingError, delete_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    from slick_cli.sophos_billing import BillingError, post_subscription_upgrade
...
def _(rid, params: dict) -> dict:
    ...
    """
    from slick_cli.sophos_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from slick_cli.sophos_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from slick_cli.sophos_billing import BillingError
        ...
        granted = step_up_sophos_billing_scope(
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Slick profile home
    # (~/.slick/sessions/saved/) rather than the project/workspace CWD, and
    # include the system prompt so the export matches the dashboard save.
    saved_dir = get_slick_home() / "sessions" / "saved"
    try:
    ...
    path = saved_dir / f"slick_conversation_{timestamp}.json"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if parent_home:
            ...
            branch_owns_db = True
        home_token = (
            set_slick_home_override(parent_home) if parent_home else None
        )
