...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.slick/.env`` into the gateway process via
    ``slick_cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from slick_cli.config import reload_env
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m slick_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "slick_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m slick_cli.main` (can drive the agent) →
            # needs provider credentials. Tier-1 secrets still stripped (#29157).
            env=slick_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from slick_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind SLICK_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_slick_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("SLICK_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("SLICK_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_slick_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off SLICK_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from slick_constants import set_slick_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from slick_constants import reset_slick_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``slick journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from slick_cli.skills_hub import do_install
        ...
        if action == "browse":
            from slick_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from slick_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `slick mcp` offers, so
    ...
    try:
        ...
        from slick_cli import mcp_catalog
...
# (slick_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes SLICK_HOME via set_slick_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses slick_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from slick_cli.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from slick_cli.config import load_config, save_config, save_env_value
        from slick_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from slick_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.slickDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``slick mcp login`` uses
    ...
    try:
        ...
        slick_home = str(get_slick_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(slick_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``slick plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.slick/plugins/`` (non-interactive).
    ...
    plugins live under each profile's SLICK_HOME, so a client can list or
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Slick plugin.
