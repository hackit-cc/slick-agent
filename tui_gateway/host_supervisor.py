...
def _default_registry_path() -> Path:
    return get_slick_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_slick_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_slick_home = expected_slick_home if expected_slick_home is not None else str(get_slick_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["SLICK_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("slick_home") or "")
        if got_home and got_home != self.expected_slick_home:
            raise RuntimeError(f"compute host SLICK_HOME mismatch: {got_home} != {self.expected_slick_home}")
