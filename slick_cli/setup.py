...
from tools.tool_backend_helpers import managed_hackit_tools_enabled
...
_DOCS_BASE = "https://slick-agent.hackit.cc/docs"
...
def _print_setup_summary(config: dict, slick_home):
    ...
    if not _provider_ready:
        ...
        print_info("    slick setup --portal   (Hackit Portal OAuth, no API key)")
    ...
    tool_status = []
    subscription_features = get_hackit_subscription_features(config)
    ...
    if subscription_features.web.managed_by_hackit:
        tool_status.append(("Web Search & Extract (Hackit subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_hackit:
        tool_status.append(("Browser Automation (Hackit Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via Hackit), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_hackit:
        tool_status.append(("Image Generation (Hackit subscription)", True, None))
    ...
    if subscription_features.video_gen.managed_by_hackit:
        tool_status.append(("Video Generation (FAL via Hackit subscription)", True, None))
    else:
        try:
    ...
    if subscription_features.tts.managed_by_hackit:
        tool_status.append(("Text-to-Speech (OpenAI via Hackit subscription)", True, None))
    ...
    if _stt_feature is not None and _stt_feature.managed_by_hackit:
        tool_status.append(("Speech-to-Text (OpenAI via Hackit subscription)", True, None))
    ...
    if subscription_features.modal.managed_by_hackit:
        tool_status.append(("Modal Execution (Hackit subscription)", True, None))
    ...
    elif managed_hackit_tools_enabled() and subscription_features.hackit_auth_present:
        tool_status.append(("Modal Execution (optional via Hackit subscription)", True, None))
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    # Tool Gateway prompt is already shown by _model_flow_hackit() above.
    save_config(config)
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_hackit_tools_enabled() and subscription_features.hackit_auth_present:
        choices.append("Hackit Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("hackit-openai")
    choices.extend(
        [
    ...
    selected_via_hackit = selected == "hackit-openai"
    if selected == "hackit-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Hackit gateway and bill to your subscription.")
    ...
    elif selected == "openai" and not selected_via_hackit:
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        managed_modal_available = bool(
            managed_hackit_tools_enabled()
            and
            get_hackit_subscription_features(config).hackit_auth_present
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Hackit gateway and bill to your subscription.")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://slick-agent.hackit.cc/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://slick-agent.hackit.cc/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      ...
      * ``active_provider`` in the auth store — covers OAuth device-code /
        external-OAuth providers (Hackit, Codex, Qwen, Gemini CLI, ...).
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Hackit Portal setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``slick setup --portal`` and ``slick portal``. This is the
    Hackit-Portal slice of the first-time quick setup, collapsed into a single
    ...
    delegated to ``_model_flow_hackit`` — the exact same flow quick setup uses
    (``_run_first_time_quick_setup``) and the same one ``slick model`` runs
    when you pick Hackit. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``slick portal`` always offers a model picker,
    and there is a single source of truth for the Hackit onboarding steps.
    """
    ...
    print(color("│     😎 Slick Setup — Hackit Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your Hackit Portal sub.")
    print()
    print_info("  Sign up: https://portal.hackit.cc/manage-subscription")
    print()

    # _model_flow_hackit handles BOTH the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Hackit model picker), then offers the Tool Gateway opt-in and sets
    # provider=hackit via the login/model save. This is the same routine quick
    # setup calls, so `slick portal` == quick setup's Hackit step.
    try:
        from slick_cli.main import _model_flow_hackit

        _model_flow_hackit(config)
    except (KeyboardInterrupt, EOFError, SystemExit):
        # _login_hackit raises SystemExit(130)/(1) on cancel/failure; the
        # logged-out path inside _model_flow_hackit catches it, but the
        ...
        logger.debug("_model_flow_hackit error during `slick portal`: %s", exc)
        print()
        print_error(f"  Hackit Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `slick portal`.")
        return

    # Re-sync the in-memory config from disk — _model_flow_hackit (and the
...
def _run_setup_wizard_impl(args):
    ...
    # --portal: one-shot Hackit Portal setup. Skips the rest of the wizard.
    ...
    if is_existing:
        ...
        setup_mode = prompt_choice(
            ...
            [
                "Quick Setup (Hackit Portal) — free OAuth login, no API keys, model + tools (recommended)",
...
def _run_first_time_quick_setup(config: dict, slick_home, is_existing: bool):
    """Streamlined first-time setup via Hackit Portal: OAuth, model, terminal & messaging.

    Routes straight to the Hackit Portal provider — runs the device-code OAuth
    login, picks a Hackit model, then configures the terminal backend and (optionally)
    ...
    # Step 1: Hackit Portal — OAuth login + model selection.
    # _model_flow_hackit() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Hackit model picker). Provider is set to "hackit" by the login/model save.
    print()
    print_header("Hackit Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.hackit.cc/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("Hackit Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_hackit error during quick setup: %s", exc)
        print_warning(f"Hackit Portal setup encountered an error: {exc}")
        print_info("You can try again later with: slick model")

    # Re-sync the wizard's config dict from disk — _model_flow_hackit (and the
