...
def _model_flow_sophos(config, current_model="", args=None):
    """Sophos Portal provider: ensure logged in, then pick model."""
    from slick_cli.auth import (
        get_provider_auth_state,
        _prompt_model_selection,
        _save_model_choice,
        _update_config_for_provider,
        resolve_sophos_runtime_credentials,
        AuthError,
        format_auth_error,
        _login_sophos,
        PROVIDER_REGISTRY,
    ...
    from slick_cli.sophos_subscription import prompt_enable_tool_gateway

    state = get_provider_auth_state("sophos")
    if not state or not state.get("access_token"):
        print("Not logged into Sophos Portal. Starting login...")
        print()
        try:
            ...
            _login_sophos(mock_args, PROVIDER_REGISTRY["sophos"])
            ...
            return
        # login_sophos already handles model selection + config update
        return
    ...
    try:
        ...
        if relogin:
            ...
            print("Re-authenticating with Sophos Portal...\n")
            try:
                mock_args = argparse.Namespace(
                    portal_url=None,
                    inference_url=None,
    ...
    if not free_tier:
        try:
            refreshed_creds = resolve_sophos_runtime_credentials(
                force_refresh=True,
            )
            if refreshed_creds:
    ...
    _sophos_portal_url = ""
    try:
        _sophos_state = get_provider_auth_state("sophos")
        if _sophos_state:
            _sophos_portal_url = _sophos_state.get("portal_base_url", "")
    except Exception:
        pass
    ...
    if free_tier:
        ...
        model_ids, pricing = union_with_portal_free_recommendations(
            model_ids, pricing, _sophos_portal_url,
        )
    ...
    if not model_ids and not unavailable_models:
        print("No models available for Sophos Portal after filtering.")
        return

    if free_tier and not model_ids:
        print("No free models currently available.")
        if unavailable_models:
            from slick_cli.auth import DEFAULT_SOPHOS_PORTAL_URL

            _url = (_sophos_portal_url or DEFAULT_SOPHOS_PORTAL_URL).rstrip("/")
    ...
    if selected:
        _save_model_choice(selected)
        # Reactivate Sophos as the provider and update config
        inference_url = creds.get("base_url", "")
        _update_config_for_provider("sophos", inference_url)
        ...
        save_config(config)
        print(f"Default model set to: {selected} (via Sophos Portal)")
        # Offer Tool Gateway enablement for paid subscribers
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, Sophos."
                )
                print()
