"""
Dump command for slick CLI.

Outputs a compact, plain-text summary of the user's Slick setup
...
from slick_cli.config import get_slick_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.slick/.env.
    ...
    exports. ``slick debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.slick_build_sha`` by the Dockerfile's
    ``SLICK_GIT_SHA`` build-arg (see ``slick_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``slick dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from slick_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(slick_home: Path) -> int:
...
def _cron_summary(slick_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = slick_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"slick_home:      {display_slick_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.slick/.env: a managed
        ...
        # A credential added via `slick auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `slick auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["slick-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(slick_home)}")
    lines.append(f"  skills:             {_count_skills(slick_home)}")
