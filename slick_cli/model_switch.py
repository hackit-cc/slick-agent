...
- ``slick_cli.providers``        -- canonical provider identity + overlays
- ``slick_cli.model_normalize``  -- per-provider name formatting
...
from slick_cli.model_normalize import (
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Slick wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Slick.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Slick versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``slick model`` wizard — not a
    ...
    while ``slick model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog Slick itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Slick itself discovered (``models_discovered: true``
...
_SLICK_MODEL_WARNING = (
    "Sophos Techne Slick 3 & 4 models are NOT agentic and are not designed "
    "for use with Slick Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Sophos Techne Slick 3 / Slick 4 chat families.
# The previous substring check (`"slick" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``slick-brain:qwen3-14b-ctx16k`` that just
# happen to carry "slick" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   SophosTechne/Slick-3-Llama-3.1-70B, slick-4-405b, openrouter/slick3:70b
# Negative examples it must NOT match:
#   slick-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_SOPHOS_SLICK_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])slick[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_sophos_slick_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Sophos Slick 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_SOPHOS_SLICK_NON_AGENTIC_RE.search(model_name))


def _check_slick_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Sophos Slick 3/4 chat model."""
    if is_sophos_slick_non_agentic(model_name):
        return _SLICK_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``slick config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from slick_cli.model_switch import
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "sophos")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "sophos")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Sophos, and falls back to models.dev for the
    rest.
    ...
    if config_context_length is not None and (
        ...
        try:
            from slick_cli.route_identity import should_clear_context_pin
...
async def resolve_display_context_length_async(
    ...
    ``/v1/models``, Copilot, Sophos, Codex, GMI, Ollama, models.dev and
    ...
    ``slick_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'slick model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'slick doctor' — config issues detected:"
        ...
        from slick_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from slick_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
        ...
        if not new_model:
            if pdef.base_url:
                from slick_cli.runtime_provider import _auto_detect_local_model
    ...
    # =================================================================
        ...
        try:
            from slick_cli.config import load_config
            from slick_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from slick_cli.providers import resolve_user_provider as _ruser
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from slick_cli.models import should_use_ollama_native_catalog
    ...
    # --- Sophos Portal dual-wire override ---
    ...
    if target_provider in {"sophos", "sophos-portal", "sophostechne"}:
        from slick_cli.providers import sophos_api_mode

        api_mode = sophos_api_mode(new_model)
    ...
    # slick_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    # kimi) the same way.
    from slick_cli.models import opencode_provider_family as _oc_family_fn
    if _oc_family_fn(target_provider) is not None and isinstance(base_url, str):
        from slick_cli.models import normalize_opencode_base_url
    ...
    slick_warn = _check_slick_model_warning(new_model)
    if slick_warn:
        warnings.append(slick_warn)
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Slick provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from slick_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from slick_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from slick_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from slick_cli.providers import SLICK_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from slick_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: Slick-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for slick_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(slick_id)
        if (
            _alias_target
            and _alias_target != slick_id
        ...
        try:
            ...
            pass
        if _canonical != slick_id:
            continue
        if slick_id.lower() in seen:
            continue
        if slick_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(slick_id)
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from slick_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(slick_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(slick_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        slick_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(slick_id)
            seen.add(slick_id.lower())

    # --- Section 2: Slick-only providers (SLICK_OVERLAYS) ---
    _mdev_to_slick = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in SLICK_OVERLAYS.items():
        if pid.lower() in seen:
            continue
        slick_slug = _mdev_to_slick.get(pid, pid)
        if slick_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or slick_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or slick_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                if _credential_pool_is_usable(slick_slug):
                    has_creds = True
            except Exception:
                pass
        if has_creds:
            slugs.append(slick_slug)
            seen.add(pid.lower())
            seen.add(slick_slug.lower())
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None,
    *,
    force_fresh_sophos_tier: bool = False,
    ...
    Uses the curated model lists from slick_cli/models.py (OPENROUTER_MODELS,
    ...
    ``force_fresh_sophos_tier`` bypasses the short Sophos tier cache for explicit
    ...
    from slick_cli.models import (
        ...
        clear_provider_models_cache, get_curated_sophos_model_ids,
    )
    ...
    from slick_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # Compared against slick_id / mdev_id (section 1), pid / slick_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from slick_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by slick provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "sophos" pulls from the remote model-catalog manifest published at
    # https://slick-agent.sophostechne.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Slick release. Falls back to the in-repo
    # _PROVIDER_MODELS["sophos"] snapshot when the manifest is unreachable.
    curated["sophos"] = get_curated_sophos_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from slick_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from slick_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Slick-mapped providers ---
    from slick_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from slick_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for slick_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip slick_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose slick_id matches the canonical profile name so
        ...
        # same slick_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if slick_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        # section 2 (SLICK_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Slick may not route yet.
        ...
        if not has_creds:
            try:
                from slick_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(slick_id)
                )
        ...
        # /model picker sees the SAME list `slick model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(slick_id)
        if not model_ids:
            model_ids = curated.get(slick_id, [])
            if slick_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(slick_id, model_ids)
    ...
    # --- 2. Check Slick-only providers (sophos, openai-codex, copilot, opencode-go) ---
    from slick_cli.providers import SLICK_OVERLAYS
    from slick_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Slick provider ID.
    # SLICK_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Slick IDs ("copilot").
    _mdev_to_slick = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in SLICK_OVERLAYS.items():
        ...
        # Resolve Slick slug — e.g. "github-copilot" → "copilot"
        slick_slug = _mdev_to_slick.get(pid, pid)
        if slick_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or slick_slug.lower() in _excluded:
            continue
        ...
        if getattr(overlay, "keyless", False):
            ...
            has_creds = _has_aws_sdk_creds_for_listing(slick_slug)
        elif overlay.auth_type == "vertex":
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or slick_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(slick_slug):
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(slick_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", slick_slug, exc)
        ...
        # configured.
        if not has_creds and slick_slug == "anthropic":
            try:
                ...
                if (slick_creds and slick_creds.get("accessToken")) or \
        ...
        if slick_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(slick_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(slick_slug)
                model_ids = _ids if _ids else (curated.get(slick_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(slick_slug, []) or curated.get(pid, [])
        elif slick_slug == "sophos":
            # Sophos serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `slick model` picker deliberately shows ONLY the curated agentic
            ...
            # order. Mirror that exactly (see _model_flow_sophos in main.py) so
            ...
            model_ids = curated.get("sophos", [])
            try:
                from slick_cli.models import (
                    get_pricing_for_provider as _sophos_pricing,
                    check_sophos_free_tier as _sophos_free,
                    union_with_portal_free_recommendations as _union_free,
                    union_with_portal_paid_recommendations as _union_paid,
                )
                from slick_cli.auth import get_provider_auth_state as _sophos_state

                _pricing = _sophos_pricing("sophos") or {}
                _portal = ""
                try:
                    _st = _sophos_state("sophos") or {}
                    _portal = _st.get("portal_base_url", "") or ""
                except Exception:
                    _portal = ""
                if _sophos_free(force_fresh=force_fresh_sophos_tier):
            ...
            if not model_ids:
                model_ids = curated.get(slick_slug, []) or curated.get(pid, [])
                if slick_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(slick_slug, model_ids)
        total = len(model_ids)
        if slick_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": slick_slug,
            "name": get_label(slick_slug),
            "is_current": slick_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "slick",
        ...
        seen_slugs.add(slick_slug.lower())
        _record_builtin_endpoint(slick_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or SLICK_OVERLAYS (keeps /model in sync
    # with `slick model`).
    try:
        from slick_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Slick writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Slick's own writer uses (mirrors section-4 grouping).
            ...
            # ``slick model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``slick model``. Pin with
            ...
            if _probe_live:
                ...
                try:
                    from slick_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Slick's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, slick_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``slick model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from slick_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`slick_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from slick_cli.models import fetch_openrouter_models
