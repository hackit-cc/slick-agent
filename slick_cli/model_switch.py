...
_SLICK_MODEL_WARNING = (
    "Hackit Slick 3 & 4 models are NOT agentic and are not designed "
    "for use with Slick Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Hackit Slick 3 / Slick 4 chat families.
...
#   Hackit/Slick-3-Llama-3.1-70B, slick-4-405b, openrouter/slick3:70b
# Negative examples it must NOT match:
#   slick-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_HACKIT_SLICK_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])slick[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_hackit_slick_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Hackit Slick 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_HACKIT_SLICK_NON_AGENTIC_RE.search(model_name))


def _check_slick_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Hackit Slick 3/4 chat model."""
    if is_hackit_slick_non_agentic(model_name):
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "hackit")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "hackit")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Hackit, and falls back to models.dev for the
    rest.
...
async def resolve_display_context_length_async(
    ...
    ``/v1/models``, Copilot, Hackit, Codex, GMI, Ollama, models.dev and
...
def switch_model(
    ...
    # --- Hackit Portal dual-wire override ---
    ...
    if target_provider in {"hackit", "hackit-portal", "hackit"}:
        from slick_cli.providers import hackit_api_mode

        api_mode = hackit_api_mode(new_model)
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None,
    *,
    force_fresh_hackit_tier: bool = False,
    ...
    ``force_fresh_hackit_tier`` bypasses the short Hackit tier cache for explicit
    ...
    from slick_cli.models import (
        ...
        clear_provider_models_cache, get_curated_hackit_model_ids,
    )
    ...
    # "hackit" pulls from the remote model-catalog manifest published at
    # https://slick-agent.hackit.cc/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Slick release. Falls back to the in-repo
    # _PROVIDER_MODELS["hackit"] snapshot when the manifest is unreachable.
    curated["hackit"] = get_curated_hackit_model_ids()
    ...
    # --- 2. Check Slick-only providers (hackit, openai-codex, copilot, opencode-go) ---
    ...
    for pid, overlay in SLICK_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        ...
        elif slick_slug == "hackit":
            # Hackit serves a large live /v1/models catalog (vendor-prefixed
            ...
            # order. Mirror that exactly (see _model_flow_hackit in main.py) so
            ...
            model_ids = curated.get("hackit", [])
            try:
                from slick_cli.models import (
                    get_pricing_for_provider as _hackit_pricing,
                    check_hackit_free_tier as _hackit_free,
                    union_with_portal_free_recommendations as _union_free,
                    union_with_portal_paid_recommendations as _union_paid,
                )
                from slick_cli.auth import get_provider_auth_state as _hackit_state

                _pricing = _hackit_pricing("hackit") or {}
                _portal = ""
                try:
                    _st = _hackit_state("hackit") or {}
                    _portal = _st.get("portal_base_url", "") or ""
                except Exception:
                    _portal = ""
                if _hackit_free(force_fresh=force_fresh_hackit_tier):
