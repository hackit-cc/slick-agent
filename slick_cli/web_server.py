...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # SPA's StatusPage can show "OAuth gate ON via Sophos Techne" or
        ...
        # Sophos bootstrap-session validity for the NAS health sweep. A hosted
        # agent whose Sophos auth dies terminally (invalid_grant / quarantine)
        ...
        sophos_session_valid = "unknown"
        try:
            from slick_cli.auth import get_sophos_session_validity
            sophos_session_valid = get_sophos_session_validity()
        except Exception:
            sophos_session_valid = "unknown"
        ...
        status = {
            ...
            "sophos_session_valid": sophos_session_valid,
        }
        ...
        # sophos_session_valid above. Deliberately NOT folded into
...
# ---------------------------------------------------------------------------
# Portal endpoint — Sophos Portal auth + Tool Gateway routing status (read-only).
# ---------------------------------------------------------------------------
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from slick_cli.auth import get_sophos_auth_status_local

        # Read-only dashboard endpoint: refresh-free snapshot so polling
        # never performs an OAuth refresh or burns a refresh token.
        auth = get_sophos_auth_status_local() or {}
    except Exception:
        auth = {}

    features = []
    try:
        from slick_cli.sophos_subscription import get_sophos_subscription_features

        feats = get_sophos_subscription_features(cfg)
        if feats is not None:
            for feat in feats.items():
                if getattr(feat, "managed_by_sophos", False):
                    state = "via Sophos Portal"
    ...
    return {
        ...
        "subscription_url": "https://portal.sophostechne.com/manage-subscription",
        "features": features,
    }
...
def get_recommended_default_model(provider: str = ""):
    ...
    Sophos this honors the user's free/paid tier: free users get a free model,
    ...
    where free_tier is True/False for Sophos and None otherwise. `model` may be
    ...
    if slug == "sophos":
        try:
            from slick_cli.models import (
                get_curated_sophos_model_ids,
                get_pricing_for_provider,
                check_sophos_free_tier,
                partition_sophos_models_by_tier,
            ...
            model_ids = get_curated_sophos_model_ids()
            pricing = get_pricing_for_provider("sophos") or {}
            free_tier = check_sophos_free_tier(force_fresh=True)

            portal_url = ""
            try:
                state = get_provider_auth_state("sophos") or {}
            ...
            if free_tier:
                ...
                model_ids, _unavailable = partition_sophos_models_by_tier(
            ...
            model = pick_silent_default_model(model_ids, provider="sophos")
            return {"provider": "sophos", "model": model, "free_tier": bool(free_tier)}
        except Exception:
            _log.exception("GET /api/model/recommended-default (sophos) failed")
            return {"provider": "sophos", "model": "", "free_tier": None}

    # Non-Sophos: preferred silent default when the provider's curated list
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # When switching the main provider to Sophos, mirror the CLI's
        # post-model-selection behaviour (slick_cli/main.py
        # prompt_enable_tool_gateway / tools_config apply_sophos_managed_defaults):
        # auto-route any *unconfigured* tools through the Sophos Tool Gateway.
        # This is purely additive — apply_sophos_managed_defaults skips every
        ...
        if provider.strip().lower() == "sophos":
            try:
                from slick_cli.sophos_subscription import apply_sophos_managed_defaults
                from slick_cli.tools_config import _get_platform_tools

                enabled = _get_platform_tools(
                    cfg, "cli", include_default_mcp_servers=False
                )
                changed = apply_sophos_managed_defaults(
                    cfg,
                ...
                # Portal lookup hiccups / non-subscriber / non-sophos gating
                # must never block saving the model assignment.
                _log.debug("apply_sophos_managed_defaults skipped", exc_info=True)
        ...
        # a now-unpaid provider (e.g. sophos with $0 balance) keeps paying 402s
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "email": {
        ...
        "docs_url": "https://slick-agent.sophostechne.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "google_chat": {
        ...
        "docs_url": "https://slick-agent.sophostechne.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://slick-agent.sophostechne.com/docs/user-guide/messaging/weixin/",
    ...
    "teams": {
        ...
        "docs_url": "https://slick-agent.sophostechne.com/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        ...
        "docs_url": "https://slick-agent.sophostechne.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Slick from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://slick-agent.sophostechne.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Slick over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://slick-agent.sophostechne.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Slick through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://slick-agent.sophostechne.com/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://slick-agent.sophostechne.com/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Slick over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://slick-agent.sophostechne.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
    ...
    "webhook": {
        ...
        "docs_url": "https://slick-agent.sophostechne.com/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://slick-agent.sophostechne.com/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Slick via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://slick-agent.sophostechne.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.slick-agent.sophostechne.com"
...
# Anthropic, device-code for Sophos/Codex) still runs in the CLI for now;
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Sophos Portal",
        "flow": "device_code",
        "cli_command": "slick auth add sophos",
        "docs_url": "https://portal.sophostechne.com",
        "status_fn": None,  # dispatched via auth.get_sophos_auth_status
    },
    {
        "id": "openai-codex",
    ...
    },
    {
        ...
        # as Sophos's device-code flow; the PKCE bit is a security
    ...
    },
    {
        ...
        "docs_url": "https://slick-agent.sophostechne.com/docs/guides/xai-grok-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        ...
        if provider_id == "sophos":
            # Read-only accounts-tab card: refresh-free snapshot so listing
            # providers never performs an OAuth refresh.
            raw = hauth.get_sophos_auth_status_local()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "sophos_portal",
                "source_label": raw.get("portal_base_url") or "Sophos Portal",
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            try:
                from slick_cli.auth import clear_provider_auth, invalidate_sophos_auth_status_cache
                cleared = clear_provider_auth(provider_id)
                if provider_id == "sophos":
                    invalidate_sophos_auth_status_cache()
...
#
#   Device code (Sophos, OpenAI Codex):
#     1. POST /api/providers/oauth/{sophos|openai-codex}/start
...
async def _start_device_code_flow(
    ...
    """Initiate a device-code flow (Sophos, OpenAI Codex, MiniMax, or xAI).
    ...
    if provider_id == "sophos":
        ...
        import httpx
        pconfig = PROVIDER_REGISTRY["sophos"]
        portal_base_url = (
            os.getenv("SLICK_PORTAL_BASE_URL")
            or os.getenv("SOPHOS_PORTAL_BASE_URL")
            or pconfig.portal_base_url
        ).rstrip("/")
        client_id = pconfig.client_id
        scope = pconfig.scope

        def _do_sophos_device_request():
            with httpx.Client(
        ...
        device_data, effective_scope = await asyncio.get_running_loop().run_in_executor(
            None, _do_sophos_device_request
        )
        sid, sess = _new_oauth_session("sophos", "device_code", profile=profile)
        ...
        threading.Thread(
            target=_sophos_poller, args=(sid,), daemon=True, name=f"oauth-poll-{sid[:6]}"
        ).start()
        return {
            "session_id": sid,
            "flow": "device_code",
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to Sophos's device-code
...
def _sophos_poller(session_id: str) -> None:
    """Background poller that drives a Sophos device-code flow to completion."""
    from slick_cli.auth import (
        _poll_for_token,
        refresh_sophos_oauth_from_state,
    )
    from datetime import datetime, timezone
    import httpx
    with _oauth_sessions_lock:
    ...
    try:
        ...
        # Same post-processing as _sophos_device_code_login (validate/refresh JWT)
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            full_state = refresh_sophos_oauth_from_state(
                auth_state,
                timeout_seconds=15.0,
                force_refresh=False,
            )
            from slick_cli.auth import persist_sophos_credentials
            persist_sophos_credentials(full_state)
        with _oauth_sessions_lock:
            sess["status"] = "approved"
        _log.info("oauth/device: sophos login completed (session=%s)", session_id)
    except Exception as e:
        _log.warning("sophos device-code poll failed (session=%s): %s", session_id, e)
        with _oauth_sessions_lock:
            sess["status"] = "error"
...
def _minimax_poller(session_id: str) -> None:
    ...
    Mirrors `_sophos_poller` but calls the MiniMax-specific token endpoint,
    which uses a PKCE-style ``code_verifier`` + ``user_code`` rather than
    the ``device_code`` field used by Sophos. On success, builds the same
...
async def poll_oauth_session(
    ...
    Shared by the device-code flows (Sophos, OpenAI Codex, MiniMax, xAI).
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    the managed "Sophos Subscription" image row instead carries the legacy
...
_BUILTIN_DASHBOARD_THEMES = [
    ...
    {"name": "sophos-blue",     "label": "Sophos Blue",           "description": "Light mode — vivid Sophos-blue accents on cream canvas"},
...
def start_server(
    ...
    try:
        from slick_cli.sophos_auth_keepalive import start_sophos_auth_keepalive

        start_sophos_auth_keepalive()
    except Exception as exc:
        _log.debug("Sophos auth keepalive did not start: %s", exc)
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            try:
                from plugins.dashboard_auth import sophos as _sophos_plugin

                if _sophos_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • sophos: {_sophos_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `slick dashboard register` (Sophos Portal) or "
