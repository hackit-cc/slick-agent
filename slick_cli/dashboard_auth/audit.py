...
from ``slick_constants`` or other slick_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$SLICK_HOME/logs/dashboard-auth.log``.

    Uses ``slick_constants.get_slick_home()`` (a leaf module — no import
    ...
    return get_slick_home() / "logs" / "dashboard-auth.log"
