...
* ``slick_cli.web_server.auth_middleware`` — loopback / ``--insecure``
  mode, gates on the ephemeral ``_SESSION_TOKEN``.
* ``slick_cli.dashboard_auth.middleware.gated_auth_middleware`` —
...
liveness probe (``hackit-account-service`` ``fly-provider.ts``
...
PUBLIC_API_PATHS: frozenset[str] = frozenset({
    ...
    # Webcron managed-cron fire webhook (NAS -> agent). NOT cookie-gated: it
