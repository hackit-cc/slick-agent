"""Shared curses-based UI components for Slick CLI.

Used by `slick tools` and `slick skills` for interactive checklists.
...
_MENU_NAVIGATION_HANDLER: ContextVar[MenuNavigationHandler | None] = ContextVar(
    "slick_menu_navigation_handler", default=None
)
_NUMBERED_BACK_ENABLED: ContextVar[bool] = ContextVar(
    "slick_numbered_back_enabled", default=False
)
...
def _read_numbered_input(prompt_text: str) -> str | _NumberedNavigation:
    ...
    from slick_cli.pt_input_extras import install_modify_other_keys_aliases
