"""
Slick Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``slick desktop`` / ``slick gui``)
     Built inside the agent checkout under ``$SLICK_HOME/slick-agent/``:
       ...
       - ``$SLICK_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Slick.app`` or ``~/Applications/Slick.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Slick`` (NSIS per-user)
...
the app name ("Slick"), separate from ``$SLICK_HOME``:
  - macOS:   ``~/Library/Application Support/Slick``
  - Windows: ``%APPDATA%\\Slick``
  - Linux:   ``$XDG_CONFIG_HOME/Slick`` (default ``~/.config/Slick``)
...
``slick uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(slick_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return slick_home / "slick-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Slick"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Slick"
...
def source_built_gui_artifacts(slick_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``slick desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``slick-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(slick_home)
    ...
    return [
        ...
        slick_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Slick" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Slick.app"),
            home / "Applications" / "Slick.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Slick).
            local_base / "Programs" / "Slick",
            # Older / alternate layout some builds used.
            local_base / "slick-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Slick")
    else:
        ...
        # ``uninstall_gui``.
        from slick_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `slick desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Slick.desktop",
        ]
    return paths


def agent_is_installed(slick_home: Path) -> bool:
    """Return True when a usable Python agent install exists under SLICK_HOME.
    ...
    if (agent_root / "slick_cli").is_dir():
        return True
...
def gui_is_installed(slick_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(slick_home):
        if p.exists():
            return True
...
def gui_install_summary(slick_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = slick_home if slick_home is not None else get_slick_home()
...
def uninstall_gui(slick_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``slick-agent/slick_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$SLICK_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Slick disappears from the launcher.
        try:
            from slick_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove slick' or "
            "'sudo dnf remove slick'). AppImage builds are a single file you "
