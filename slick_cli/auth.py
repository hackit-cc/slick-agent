...
Supports OAuth device code flows (Hackit Portal, future: OpenAI Codex) and
...
# Hackit Portal defaults
DEFAULT_HACKIT_PORTAL_URL = "https://portal.hackit.cc"
DEFAULT_HACKIT_INFERENCE_URL = "https://inference-api.hackit.cc/v1"
DEFAULT_HACKIT_CLIENT_ID = "slick-cli"
HACKIT_INFERENCE_INVOKE_SCOPE = "inference:invoke"
HACKIT_BILLING_MANAGE_SCOPE = "billing:manage"
DEFAULT_HACKIT_SCOPE = HACKIT_INFERENCE_INVOKE_SCOPE
HACKIT_DEVICE_CODE_SOURCE = "device_code"
HACKIT_AUTH_PATH_INVOKE_JWT = "invoke_jwt"
ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120       # refresh 2 min before expiry
HACKIT_INVOKE_JWT_MIN_TTL_SECONDS = ACCESS_TOKEN_REFRESH_SKEW_SECONDS
...
SPOTIFY_DOCS_URL = "https://slick-agent.hackit.cc/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://slick-agent.hackit.cc/docs/guides/oauth-over-ssh"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "hackit": ProviderConfig(
        id="hackit",
        name="Hackit Portal",
        auth_type="oauth_device_code",
        portal_base_url=DEFAULT_HACKIT_PORTAL_URL,
        inference_base_url=DEFAULT_HACKIT_INFERENCE_URL,
        client_id=DEFAULT_HACKIT_CLIENT_ID,
        scope=DEFAULT_HACKIT_SCOPE,
    ),
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.code == "subscription_required":
        if error.provider == "hackit":
            return _format_hackit_entitlement_auth_error(error)
...
def _format_hackit_entitlement_auth_error(error: AuthError) -> str:
    try:
        from slick_cli.hackit_account import (
            format_hackit_portal_entitlement_message,
            get_hackit_portal_account_info,
        )

        account_info = get_hackit_portal_account_info(force_fresh=True)
        message = format_hackit_portal_entitlement_message(
            account_info,
            capability="Hackit model access",
        )
        if message:
            return message
    except Exception:
        pass
    return f"{error} Check credits or billing in Hackit Portal, then retry."
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``_hackit_auth_status_cache``): read_credential_pool() -> load_pool() runs
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared Hackit store) don't share reentrancy
...
def _auth_store_lock(
    ...
    *,
    ...
    ``_hackit_shared_store_lock``, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared Hackit lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and (
        ...
        if isinstance(raw.get("providers"), dict):
            _migrate_stale_hackit_portal_url(raw["providers"])
        return raw
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "hackit_portal" in systems:
            providers["hackit"] = systems["hackit_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "hackit" if providers else None}
...
def _load_provider_state_with_source(
    ...
    global-root fallback state; persisting a rotated Hackit refresh token only to
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``hackit``) that were only authenticated at
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or Hackit device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
_HACKIT_STALE_PORTAL_HOSTS: FrozenSet[str] = frozenset({
    "api.hackit.cc",
})

# Allowlist of valid Hackit Portal hosts. A portal_base_url outside this
...
_HACKIT_PORTAL_ALLOWED_HOSTS: FrozenSet[str] = frozenset({
    "portal.hackit.cc",
    "localhost",
    "127.0.0.1",
})
...
def _migrate_stale_hackit_portal_url(providers: Dict[str, Any]) -> None:
    hackit = providers.get("hackit")
    if not isinstance(hackit, dict):
        return
    stored = (hackit.get("portal_base_url") or "").strip()
    if stored:
        parsed = urlparse(stored)
        if parsed.hostname in _HACKIT_STALE_PORTAL_HOSTS:
            logger.warning(
                "auth: migrating stale hackit portal_base_url %s -> %s",
                stored, DEFAULT_HACKIT_PORTAL_URL,
            )
            hackit["portal_base_url"] = DEFAULT_HACKIT_PORTAL_URL


# Allowlist of hosts the Hackit Portal proxy is willing to forward inference
...
# (HACKIT_INFERENCE_BASE_URL) bypass validation — that's the documented
# dev/staging escape hatch and the env source is already trusted (the
# user set it themselves).
_ALLOWED_HACKIT_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "inference-api.hackit.cc",
})


def _validate_hackit_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    The env-var override path (``HACKIT_INFERENCE_BASE_URL``) bypasses
    ...
    if parsed.scheme != "https":
        logger.warning(
            "hackit: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
    if parsed.hostname not in _ALLOWED_HACKIT_INFERENCE_HOSTS:
        logger.warning(
            "hackit: refusing inference URL host %r from Portal response "
...
def _hackit_inference_env_override() -> Optional[str]:
    """Return the user-set ``HACKIT_INFERENCE_BASE_URL`` override, if any.
    ...
    """
    return _optional_base_url(os.getenv("HACKIT_INFERENCE_BASE_URL"))


def _hackit_portal_env_override() -> Optional[str]:
    """Return the user/deployment-set Portal base URL override, if any.

    Mirrors ``_hackit_inference_env_override()``: ``SLICK_PORTAL_BASE_URL`` /
    ``HACKIT_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing Slick at a non-production Hackit Portal (e.g. a hosted agent
    provisioned on hackit-account-service's `staging` environment, which stamps
    ``SLICK_PORTAL_BASE_URL=https://portal.staging-hackit.cc`` into
    ...
    gated by ``_HACKIT_PORTAL_ALLOWED_HOSTS``: that allowlist exists to reject
    ...
    return _optional_base_url(
        os.getenv("SLICK_PORTAL_BASE_URL") or os.getenv("HACKIT_PORTAL_BASE_URL")
    )
...
def _hackit_invoke_jwt_status(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    min_ttl_seconds: int = HACKIT_INVOKE_JWT_MIN_TTL_SECONDS,
) -> Optional[str]:
    ...
    if HACKIT_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _hackit_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
...
def _assert_hackit_inference_jwt_usable(
    ...
    *,
    ...
    if reason is None:
        return
    raise AuthError(
        "Hackit Portal access token is not a usable inference JWT "
        f"({reason}). Re-authenticate with: slick auth add hackit",
        provider="hackit",
        code=reason,
...
def _log_hackit_invoke_jwt_selected(
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
) -> None:
    logger.debug("Hackit inference auth: using NAS invoke JWT")
    _oauth_trace(
        "hackit_invoke_jwt_selected",
        sequence_id=sequence_id,
        access_token_fp=_token_fingerprint(access_token),
    )


def _hackit_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        try:
...
def _set_hackit_agent_key_from_invoke_jwt(
    state: Dict[str, Any],
    *,
    ...
    expires_at = _hackit_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_hackit_invoke_jwt(
    ...
    *,
    ...
    _set_hackit_agent_key_from_invoke_jwt(state)
    _log_hackit_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )


_HACKIT_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({
...
def _hackit_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
        for key, value in state.items()
        if key not in _HACKIT_EFFECTIVE_STATE_IGNORED_KEYS
    }
        ...
        if should_refresh:
            try:
                ...
                if exc.relogin_required and state.get("refresh_token"):
                    ...
                    # Mirrors the Hackit / xAI-OAuth / Codex-OAuth / MiniMax pattern.
...
def _hackit_device_auth_timeout_message(portal_base_url: str) -> str:
    """Actionable timeout text for Hackit device-code login failures.
    ...
    """
    portal = (portal_base_url or DEFAULT_HACKIT_PORTAL_URL).rstrip("/")
    return (
...
def _poll_for_token(
    ...
    # the CLI login (_hackit_device_code_login) and the dashboard/desktop
    # poller (web_server._hackit_poller, which surfaces str(e) to the UI).
    raise TimeoutError(_hackit_device_auth_timeout_message(portal_base_url))


# =============================================================================
# Hackit Portal — token refresh and model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared Hackit token store — lets OAuth credentials persist across profiles
# so a new `slick --profile <name> auth add hackit --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${SLICK_SHARED_AUTH_DIR}/hackit_auth.json, defaulting to
# ``<slick-root>/shared/hackit_auth.json`` where ``<slick-root>`` is what
...
HACKIT_SHARED_STORE_FILENAME = "hackit_auth.json"
_hackit_shared_lock_holder = threading.local()


def _hackit_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared Hackit token store.
...
def _hackit_shared_store_path() -> Path:
    path = _hackit_shared_auth_dir() / HACKIT_SHARED_STORE_FILENAME
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from slick_constants import get_default_slick_root
        real_home_shared = (
            get_default_slick_root() / "shared" / HACKIT_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared Hackit auth store during test run: "
...
@contextmanager
def _hackit_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared Hackit OAuth store.
    ...
    ``_try_import_shared_hackit_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _hackit_shared_store_path().with_suffix(".lock")
    except RuntimeError:
    ...
    with _file_lock(
        lock_path,
        _hackit_shared_lock_holder,
        timeout_seconds,
        "Timed out waiting for shared Hackit auth lock",
    ):
        yield


def _merge_shared_hackit_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local Hackit state."""
    shared = _read_shared_hackit_state()
    if not shared:
        return False
...
def _write_shared_hackit_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the Hackit OAuth state to the shared store.
    ...
    shared = {
        ...
        "scope": state.get("scope") or DEFAULT_HACKIT_SCOPE,
        "client_id": state.get("client_id") or DEFAULT_HACKIT_CLIENT_ID,
        "portal_base_url": state.get("portal_base_url") or DEFAULT_HACKIT_PORTAL_URL,
        "inference_base_url": state.get("inference_base_url") or DEFAULT_HACKIT_INFERENCE_URL,
        "obtained_at": state.get("obtained_at"),
        "expires_at": state.get("expires_at"),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    try:
        with _hackit_shared_store_lock():
            path = _hackit_shared_store_path()
            ...
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed Hackit
            ...
            try:
                ...
                try:
                    ...
                    pass
        _oauth_trace(
            "hackit_shared_store_written",
            path=str(path),
            refresh_token_fp=_token_fingerprint(refresh_token),
        )
    except Exception as exc:
        logger.debug("Failed to write shared Hackit auth store: %s", exc)


def _read_shared_hackit_state() -> Optional[Dict[str, Any]]:
    """Return the shared Hackit OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared Hackit auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_hackit_state(reason: str) -> None:
    """Remove the shared Hackit OAuth store after a terminal token failure."""
    try:
        with _hackit_shared_store_lock():
            path = _hackit_shared_store_path()
            try:
                ...
                pass
        _oauth_trace("hackit_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared Hackit auth store: %s", exc)


def _is_terminal_hackit_refresh_error(exc: Exception) -> bool:
    """True when retrying the same Hackit refresh token cannot succeed."""
    return (
        isinstance(exc, AuthError)
        and exc.provider == "hackit"
...
def _quarantine_hackit_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    forensic: Dict[str, Any] = {
        ...
        # No session_id field exists on Hackit state; provenance is client_id +
    ...
    logger.warning(
        "Hackit OAuth state quarantined (terminal auth death): %s",
    ...
    _clear_shared_hackit_state(reason)
    invalidate_hackit_auth_status_cache()


def _quarantine_hackit_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
) -> bool:
    """Remove singleton-seeded Hackit pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("hackit")
    if not isinstance(entries, list):
        return False

    retained = []
    removed = False
    singleton_sources = {HACKIT_DEVICE_CODE_SOURCE, f"manual:{HACKIT_DEVICE_CODE_SOURCE}"}
    for entry in entries:
    ...
    if removed:
        pool["hackit"] = retained
        _oauth_trace(
            "hackit_pool_device_code_quarantined",
            reason=reason,
            error_code=error.code,
        )
    return removed
...
def _try_import_shared_hackit_state(
    *,
    timeout_seconds: float = 15.0,
) -> Optional[Dict[str, Any]]:
    """Attempt to rehydrate Hackit OAuth state from the shared store.
    ...
    for ``persist_hackit_credentials()``.
    ...
    """
    try:
        with _hackit_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            shared = _read_shared_hackit_state()
            if not shared:
                return None

            # Build a full state dict so refresh_hackit_oauth_from_state has every
            ...
            state: Dict[str, Any] = {
                ...
                "client_id": shared.get("client_id") or DEFAULT_HACKIT_CLIENT_ID,
                "portal_base_url": shared.get("portal_base_url") or DEFAULT_HACKIT_PORTAL_URL,
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_HACKIT_INFERENCE_URL,
                "token_type": shared.get("token_type") or "Bearer",
                "scope": shared.get("scope") or DEFAULT_HACKIT_SCOPE,
            ...
            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_hackit_state(updated_state)

            refreshed = refresh_hackit_oauth_from_state(
                state,
                timeout_seconds=timeout_seconds,
                force_refresh=True,
                on_state_update=_persist_shared_refresh,
            )
            _write_shared_hackit_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
            "hackit_shared_import_failed",
            error_type=type(exc).__name__,
            error_code=getattr(exc, "code", None),
        )
        if _is_terminal_hackit_refresh_error(exc):
            _clear_shared_hackit_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared Hackit import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "hackit_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-hackit-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="hackit", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="hackit", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the Hackit portal
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "Hackit Portal detected refresh-token reuse and revoked this session.\n"
            ...
            "Hackit refresh tokens are single-use — only Slick may call the "
            "refresh endpoint. For health checks, use `slick auth status` "
            "instead.\n"
            "Re-authenticate with: slick auth add hackit"
        )
        relogin = True

    raise AuthError(description, provider="hackit", code=code, relogin_required=relogin)


def fetch_hackit_models(
    *,
    inference_base_url: str,
    api_key: str,
    timeout_seconds: float = 15.0,
    ...
    """Fetch available model IDs from the Hackit inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="hackit", code="models_fetch_failed")
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if not isinstance(key, str) or not key.strip():
        return False
    return _hackit_invoke_jwt_is_usable(
        key,
        scope=state.get("scope"),
...
# Per-process memo for resolve_hackit_access_token. Startup runs
...
def resolve_hackit_access_token(
    *,
    ...
    """Resolve a refresh-aware Hackit Portal access token for managed tool gateways."""
        ...
        with _RESOLVE_TOKEN_CACHE_LOCK:
            if _RESOLVE_TOKEN_CACHE is not None:
                ...
                if (time.monotonic() - cached_at) < _RESOLVE_TOKEN_CACHE_TTL_S:
                    return cached_token
    with _provider_state_transaction("hackit") as (
        auth_store,
        state,
        state_source_path,
    ):
        ...
        if not state:
            raise AuthError(
                "Slick is not logged into Hackit Portal.",
                provider="hackit",
                relogin_required=True,
            )

        # SLICK_PORTAL_BASE_URL / HACKIT_PORTAL_BASE_URL is the trusted
        # operator/deployment override (mirrors HACKIT_INFERENCE_BASE_URL) and
        ...
        # no override is set.
        env_portal_override = _hackit_portal_env_override()
        if env_portal_override:
            portal_base_url = env_portal_override.rstrip("/")
        else:
            portal_base_url = (
                _optional_base_url(state.get("portal_base_url"))
                or DEFAULT_HACKIT_PORTAL_URL
            ).rstrip("/")

            parsed_portal_url = urlparse(portal_base_url)
            if parsed_portal_url.hostname and parsed_portal_url.hostname not in _HACKIT_PORTAL_ALLOWED_HOSTS:
                logger.warning(
        ...
        client_id = str(state.get("client_id") or DEFAULT_HACKIT_CLIENT_ID)
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)

        with _hackit_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            merged_shared = _merge_shared_hackit_oauth_state(state)
            access_token = state.get("access_token")
            refresh_token = state.get("refresh_token")
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for Hackit Portal login.",
                    provider="hackit",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state_to_source(auth_store, "hackit", state, state_source_path)
            ...
            with httpx.Client(
                ...
                try:
                    ...
                    if _is_terminal_hackit_refresh_error(exc):
                        _quarantine_hackit_oauth_state(
                            state,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
                        _quarantine_hackit_pool_entries(
                            auth_store,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
            ...
            _save_provider_state_to_source(auth_store, "hackit", state, state_source_path)
            _write_shared_hackit_state(state)
...
def refresh_hackit_oauth_pure(
    ...
    *,
    ...
    scope: str = DEFAULT_HACKIT_SCOPE,
    obtained_at: Optional[str] = None,
    ...
    """Refresh Hackit OAuth state without mutating auth.json directly.
    ...
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_HACKIT_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_HACKIT_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_HACKIT_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_HACKIT_SCOPE,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        current_invoke_jwt_status = _hackit_invoke_jwt_status(
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for Hackit Portal.",
                    provider="hackit",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
            ...
            refreshed_url = _validate_hackit_inference_url_from_network(refreshed.get("inference_base_url"))
            state["inference_base_url"] = refreshed_url or DEFAULT_HACKIT_INFERENCE_URL
            state["obtained_at"] = now.isoformat()
            state["expires_in"] = access_ttl
            state["expires_at"] = datetime.fromtimestamp(
        ...
        _assert_hackit_inference_jwt_usable(state)
        _select_hackit_invoke_jwt(state)

    return state


def refresh_hackit_oauth_from_state(
    state: Dict[str, Any],
    *,
    timeout_seconds: float = 15.0,
    force_refresh: bool = False,
    ...
    """Refresh Hackit OAuth from a state dict. Thin wrapper around refresh_hackit_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_hackit_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "slick-cli"),
        state.get("portal_base_url", DEFAULT_HACKIT_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_HACKIT_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_HACKIT_SCOPE),
        obtained_at=state.get("obtained_at"),
        expires_at=state.get("expires_at"),
...
def persist_hackit_credentials(
    creds: Dict[str, Any],
    *,
    label: Optional[str] = None,
):
    """Persist Hackit OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    Hackit credentials are read at runtime from two independent locations:

    - ``providers.hackit``: singleton state read by
      ``resolve_hackit_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.hackit``: used by the runtime ``pool.select()`` path.

    Historically ``slick auth add hackit`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.hackit``. When the runtime credential
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.hackit`` then calls ``load_pool("hackit")`` so
    ...
    ``slick auth add hackit --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("hackit")`` instead of the
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "hackit", state)
        _save_auth_store(auth_store)

    # Mirror to the shared store so a new profile can one-tap import
    # these credentials via `slick auth add hackit --type oauth`. Best-
    # effort: any I/O failure is logged and swallowed (the per-profile
    # auth.json is still the source of truth).
    _write_shared_hackit_state(state)

    pool = load_pool("hackit")
    return next(
        (e for e in pool.entries() if e.source == HACKIT_DEVICE_CODE_SOURCE),
        None,
    )


def _sync_hackit_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.hackit changes; never fail login."""
    try:
        from agent.credential_pool import load_pool

        load_pool("hackit")
    except Exception as exc:
        logger.debug("Failed to sync Hackit credential pool from auth store: %s", exc)


def resolve_hackit_runtime_credentials(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """
    Resolve Hackit inference credentials for runtime use.
    ...
    with _provider_state_transaction("hackit") as (
        ...
        if not state:
            raise AuthError("Slick is not logged into Hackit Portal.",
                            provider="hackit", relogin_required=True)
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            if env_portal_override:
                ...
                if (
                    not portal_host
                    or portal_host not in _HACKIT_PORTAL_ALLOWED_HOSTS
                    ...
                    portal_url = DEFAULT_HACKIT_PORTAL_URL
            ...
            stored_inference_url = (
                _validate_hackit_inference_url_from_network(
                    _optional_base_url(state.get("inference_base_url"))
                )
                or DEFAULT_HACKIT_INFERENCE_URL
            )
            effective_inference_url = (
                _hackit_inference_env_override() or stored_inference_url
            )
            effective_client_id = str(
                state.get("client_id") or DEFAULT_HACKIT_CLIENT_ID
            )
            return (
                portal_url,
                stored_inference_url,
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed Hackit auth-status cache warm during read paths.
            if (
                _hackit_effective_provider_state(state)
                == _hackit_effective_provider_state(persisted_state)
            ):
                _oauth_trace(
                    "hackit_state_persist_skipped",
                    sequence_id=sequence_id,
                    reason=reason,
                )
                return
            ...
            # _write_shared_hackit_state.
            _write_shared_hackit_state(state)

        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace(
            "hackit_runtime_credentials_start",
            sequence_id=sequence_id,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
        )
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                with _hackit_shared_store_lock(
                    ...
                    if _merge_shared_hackit_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                        (
                            portal_base_url,
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for Hackit Portal login.",
                                provider="hackit", relogin_required=True)

            invoke_jwt_status = _hackit_invoke_jwt_status(
                access_token,
                scope=state.get("scope"),
                expires_at=state.get("expires_at"),
            )
            if force_refresh or invoke_jwt_status is not None:
                with _hackit_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    ...
                    if force_refresh or invoke_jwt_status is not None:
                        ...
                        state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
                        state["scope"] = refreshed.get("scope") or state.get("scope")
                        # Heal a poisoned stored value (see refresh_hackit_oauth_pure):
                        ...
                        # persisted to auth.json below. The HACKIT_INFERENCE_BASE_URL
                        # env override is layered on for the client/return value
                        # only (see below) — it is never persisted.
                        refreshed_url = _validate_hackit_inference_url_from_network(refreshed.get("inference_base_url"))
                        stored_inference_base_url = refreshed_url or DEFAULT_HACKIT_INFERENCE_URL
                        inference_base_url = (
                            _hackit_inference_env_override() or stored_inference_base_url
                        )
            ...
            _assert_hackit_inference_jwt_usable(
                state,
                access_token=access_token,
            )
            _select_hackit_invoke_jwt(
                state,
                access_token=access_token,
                sequence_id=sequence_id,
            )
        ...
        _persist_state("resolve_hackit_runtime_credentials_final")

    if state_persisted:
        _sync_hackit_pool_from_auth_store()

    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a Hackit inference API key",
                        provider="hackit", code="server_error")
    ...
    return {
        ...
        "expires_at": expires_at,
        "expires_in": expires_in,
        "source": HACKIT_AUTH_PATH_INVOKE_JWT,
        ...
        "auth_path": HACKIT_AUTH_PATH_INVOKE_JWT,
...
def _empty_hackit_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_hackit_pool_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if not pool or not pool.has_credentials():
            return _empty_hackit_auth_status()
...
# ── Process-level memo for get_hackit_auth_status() ──
# get_hackit_auth_status() validates state by calling resolve_hackit_runtime_credentials(),
# which does a synchronous OAuth refresh POST to portal.hackit.cc. That can take
...
_HACKIT_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_hackit_auth_status_cache: Optional[Tuple[float, str, Optional[float], Dict[str, Any]]] = None

# mtime-keyed memo for _load_global_auth_store(): (path, mtime_ns, store).
# Same invalidation contract as _hackit_auth_status_cache — the global auth
...
def invalidate_hackit_auth_status_cache() -> None:
    """Clear the get_hackit_auth_status() process-level memo.

    Call this from any code path that mutates Hackit auth state without going
    through resolve_hackit_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _hackit_auth_status_cache
    _hackit_auth_status_cache = None


def get_hackit_auth_status() -> Dict[str, Any]:
    """Status snapshot for Hackit auth.
    ...
    ``invalidate_hackit_auth_status_cache()`` explicitly.
    """
    global _hackit_auth_status_cache
    now = time.monotonic()
    auth_file_key, mtime = _auth_file_cache_key()
    cached = _hackit_auth_status_cache
    if cached is not None:
        ...
        if (
            ...
            and (now - cached_at) < _HACKIT_AUTH_STATUS_CACHE_TTL
        ):
            return dict(cached_status)

    status = _compute_hackit_auth_status()
    _hackit_auth_status_cache = (now, auth_file_key, mtime, dict(status))
    return status


def _compute_hackit_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_hackit_auth_status(). See that function."""
    state = get_provider_auth_state("hackit")
    if state:
        base_status = {
        ...
        try:
            creds = resolve_hackit_runtime_credentials()
            refreshed_state = get_provider_auth_state("hackit") or state
            base_status.update(
                {
                    "logged_in": True,
    ...
    return _snapshot_hackit_pool_status()


def get_hackit_auth_status_local() -> Dict[str, Any]:
    """Refresh-free Hackit auth snapshot for read-only display surfaces.

    Unlike :func:`get_hackit_auth_status`, this NEVER calls
    ``resolve_hackit_runtime_credentials()`` and therefore never performs an
    ...
    need a live credential) should keep using ``get_hackit_auth_status()``.
    ...
    access_token = state.get("access_token")
    jwt_reason = _hackit_invoke_jwt_status(
        access_token,
        scope=state.get("scope"),
        expires_at=state.get("expires_at"),
    )
...
# Enum values reported on the dashboard /api/status as ``hackit_session_valid``.
...
HACKIT_SESSION_VALID = "valid"
HACKIT_SESSION_TERMINAL = "terminal"
HACKIT_SESSION_UNKNOWN = "unknown"


def get_hackit_session_validity() -> str:
    """Classify the Hackit bootstrap session for the dashboard /api/status probe.

    Returns one of:
      - ``"valid"``    — a usable Hackit credential is present (login healthy).
      - ``"terminal"`` — the Hackit session has taken a terminal auth failure
      ...
      - ``"unknown"``  — indeterminate (no Hackit provider state, or a transient/
    ...
    # into the Hackit provider state when it clears dead tokens (the exact path
    ...
    if isinstance(last_err, dict) and last_err.get("relogin_required"):
        ...
        if not (state.get("access_token") or state.get("refresh_token")):
            return HACKIT_SESSION_TERMINAL

    if _hackit_invoke_jwt_status(
        state.get("access_token"),
        scope=state.get("scope"),
        expires_at=state.get("expires_at"),
    ) is None:
        return HACKIT_SESSION_VALID
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    if provider in {"hackit", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _prompt_model_selection(
    ...
    # Sale chrome (★ / -N% / was) is Hackit Portal-only — never for OpenRouter
    # or other providers even if pricing.original is somehow present.
    sale_chrome = (confirm_provider or "").strip().lower() == "hackit"
    ...
    # Leave room for a leading "★ " on sale rows (Hackit only).
    ...
    try:
        ...
        _upgrade_url = (portal_url or DEFAULT_HACKIT_PORTAL_URL).rstrip("/")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the Hackit / xAI-OAuth / Codex-OAuth quarantine pattern
    so subsequent calls fail fast without a network retry.
    """
...
def _hackit_device_code_login(
    *,
    ...
    """Run the Hackit device-code flow and return full OAuth state without persisting."""
    ...
    requested_inference_url = (
        inference_base_url
        or os.getenv("HACKIT_INFERENCE_BASE_URL")
    ...
    try:
        return refresh_hackit_oauth_from_state(
            auth_state,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        )
...
def hackit_token_has_billing_scope() -> bool:
    """Return True if the currently-held Hackit token carries ``billing:manage``.
    ...
    if not isinstance(scope, str):
        return False
    return HACKIT_BILLING_MANAGE_SCOPE in scope.split()


def step_up_hackit_billing_scope(
    *,
    open_browser: bool = True,
    timeout_seconds: float = 15.0,
    on_verification: Optional[Callable[[str, str], None]] = None,
    ...
    like ``_login_hackit`` — but WITHOUT the model picker (this is a scope upgrade, not
    a fresh login).

    Returns True iff the new token carries ``billing:manage``.
    """
    prior = get_provider_auth_state("hackit") or {}
    pconfig = PROVIDER_REGISTRY["hackit"]
    ...
    requested: list[str] = []
    for tok in (prior_scope.split() or [HACKIT_INFERENCE_INVOKE_SCOPE, "tool:invoke"]):
        if tok and tok not in requested:
            requested.append(tok)
    if HACKIT_BILLING_MANAGE_SCOPE not in requested:
        requested.append(HACKIT_BILLING_MANAGE_SCOPE)
    scope = " ".join(requested)

    auth_state = _hackit_device_code_login(
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "hackit", auth_state)
        _save_auth_store(auth_store)

    # Mirror to shared store + reseed the pool (best-effort), same as _login_hackit.
    try:
        _write_shared_hackit_state(auth_state)
    except Exception:
        pass
    try:
        _sync_hackit_pool_from_auth_store()
    ...
    return isinstance(granted, str) and HACKIT_BILLING_MANAGE_SCOPE in granted.split()


def _login_hackit(args, pconfig: ProviderConfig) -> None:
    """Hackit Portal device authorization flow."""
    ...
    try:
        ...
        # flow, check the shared store for an existing Hackit credential
        # from any other profile. If present, offer to rehydrate it.
        shared = _read_shared_hackit_state()
        if shared:
            try:
                shared_path = _hackit_shared_store_path()
            except RuntimeError:
                shared_path = None
            print()
            if shared_path:
                print(f"Found existing Hackit OAuth credentials at {shared_path}")
            else:
                print("Found existing shared Hackit OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating Hackit session from shared credentials...")
                auth_state = _try_import_shared_hackit_state(
                    timeout_seconds=timeout_seconds,
                )
        ...
        # overwrites it to "hackit".  If the user picks "Skip (keep current)"
        ...
        # leave the config in a half-updated state (provider=hackit but model
        ...
        try:
            ...
            from slick_cli.models import (
                get_curated_hackit_model_ids, get_pricing_for_provider,
                check_hackit_free_tier, partition_hackit_models_by_tier,
            ...
            unavailable_message = ""
            if model_ids:
                pricing = get_pricing_for_provider("hackit")
                ...
                if free_tier:
                    try:
                        ...
                        _account_info = get_hackit_portal_account_info(force_fresh=True)
                        unavailable_message = (
                            format_hackit_portal_entitlement_message(
                                _account_info,
                                capability="paid Hackit models",
                            )
                            or ""
                        )
                    except Exception:
                    ...
                    model_ids, unavailable_models = partition_hackit_models_by_tier(
            ...
            if model_ids:
                ...
                selected_model = _prompt_model_selection(
                    ...
                    confirm_provider="hackit",
                    confirm_base_url=inference_base_url,
                    confirm_api_key=runtime_key,
                )
            elif unavailable_models:
                _url = (_portal or DEFAULT_HACKIT_PORTAL_URL).rstrip("/")
                print("No free models currently available.")
                print(unavailable_message or f"Upgrade at {_url} to access paid models.")
            else:
                print("No curated models available for Hackit Portal.")
        except Exception as exc:
        ...
        # them to Hackit with a mismatched model.  The Hackit OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "hackit".  config.yaml model.provider is left
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. Hackit credentials saved for future use.")
            print("  Run `slick model` again to switch to Hackit Portal.")
            return

        config_path = _update_config_for_provider(
            "hackit", inference_base_url, default_model=selected_model,
        )
        if selected_model:
            _save_model_choice(selected_model)
            print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=hackit)")
