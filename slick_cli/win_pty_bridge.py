"""Windows ConPTY bridge for the `slick dashboard` chat tab.

Drop-in counterpart to ``slick_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``slick_cli.web_server`` consumes: ``spawn``, ``read``,
