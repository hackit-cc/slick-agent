"""Helpers for Hackit subscription managed-tool capabilities."""
...
from slick_cli.config import get_env_value, load_config
from slick_cli.hackit_account import (
    HackitPortalAccountInfo,
...
# Maps a tools_config provider's ``managed_hackit_feature`` to the tool-pool
# coverage category (slick_cli.hackit_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `slick tools` picker scope its entitlement gate to the selected backend, so a
# free-tool-pool user is allowed image gen but denied video gen at select time —
# consistent with the per-category feature gates in get_hackit_subscription_features.
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    an in-memory section dict: ``"hackit"`` for the managed selection (stored
    ``hackit`` value or legacy ``use_gateway: true``), a vendor name for BYOK
    picks, or ``None`` when no selection is stored. Keeping this in lockstep
    with the runtime resolver is what stops ``slick status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
class HackitFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_hackit: bool
...
@dataclass(frozen=True)
class HackitSubscriptionFeatures:
    subscribed: bool
    hackit_auth_present: bool
    provider_is_hackit: bool
    features: Dict[str, HackitFeatureState]
    account_info: Optional[HackitPortalAccountInfo] = None

    @property
    def web(self) -> HackitFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> HackitFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> HackitFeatureState:
        return self.features["tts"]

    @property
    def stt(self) -> HackitFeatureState:
        return self.features["stt"]

    @property
    def browser(self) -> HackitFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> HackitFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> HackitFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[HackitFeatureState]:
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Slick-managed Node dirs (Windows installer / POSIX $SLICK_HOME/node)
        ...
        managed_path = with_slick_node_path().get("PATH", "")
        if managed_path:
...
def _local_stt_backend_available() -> bool:
    ...
    ``apply_hackit_managed_defaults`` from flipping a working local setup
    to the managed gateway.
    """
    if get_env_value("SLICK_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_hackit_subscription_features(
    ...
    *,
    ...
    provider_is_hackit = str(model_cfg.get("provider") or "").strip().lower() == "hackit"

    try:
        if force_fresh:
            account_info = get_hackit_portal_account_info(force_fresh=True)
        else:
            account_info = get_hackit_portal_account_info()
    except Exception:
        account_info = None
    ...
    hackit_auth_present = bool(account_info and account_info.logged_in)

    def _entitled_for(category: str) -> bool:
        return bool(account_info and account_info.tool_gateway_entitled_for(category))
    subscribed = provider_is_hackit or hackit_auth_present
    ...
    # requires `pip install faster-whisper`. For Hackit subscribers we'd
    # rather route through the managed OpenAI audio gateway — see
    # apply_hackit_managed_defaults below.
    ...
    # "hackit" (stored value or legacy use_gateway: true) = managed gateway;
    ...
    web_use_gateway = web_selected == "hackit"
    tts_use_gateway = tts_selected == "hackit"
    stt_use_gateway = stt_selected == "hackit"
    browser_use_gateway = browser_selected == "hackit"
    image_use_gateway = image_selected == "hackit"
    video_use_gateway = video_selected == "hackit"

    # The "hackit" selection is serviced by a concrete vendor implementation —
    # normalize the current-provider labels so downstream vendor checks hold.
    if web_backend == "hackit" or web_use_gateway:
        web_backend = "firecrawl"
    if tts_provider == "hackit" or tts_use_gateway:
        tts_provider = "openai"
    if stt_provider == "hackit" or stt_use_gateway:
        stt_provider = "openai"
    if browser_provider == "hackit" or browser_use_gateway:
    ...
    # Keyless Tavily is opt-in: selecting it in `slick tools` / setup writes
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("SLICK_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("SLICK_LOCAL_STT_COMMAND"))
    ...
    features = {
        "web": HackitFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_hackit=web_managed,
        ...
        "image_gen": HackitFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_hackit=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if (image_selected not in (None, "hackit") or (image_selected is None and direct_fal)) else ("Hackit Subscription" if (image_managed or image_use_gateway) else ""),
            explicit_configured=image_selected is not None or direct_fal,
        ),
        "video_gen": HackitFeatureState(
            key="video_gen",
            label="Video generation",
            included_by_default=False,
            available=video_available,
            active=video_active,
            managed_by_hackit=video_managed,
            direct_override=video_active and not video_managed,
            toolset_enabled=video_tool_enabled,
            current_provider="FAL" if (video_selected not in (None, "hackit") or (video_selected is None and direct_fal_video)) else ("Hackit Subscription" if (video_managed or video_use_gateway) else ""),
            explicit_configured=video_selected is not None or direct_fal_video,
        ),
        "tts": HackitFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_hackit=tts_managed,
        ...
        "stt": HackitFeatureState(
            key="stt",
            label="Speech-to-text",
            included_by_default=True,
            available=stt_available,
            active=stt_active,
            managed_by_hackit=stt_managed,
        ...
        "browser": HackitFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_hackit=browser_managed,
        ...
        "modal": HackitFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_hackit=modal_managed,
    ...
    return HackitSubscriptionFeatures(
        subscribed=subscribed,
        hackit_auth_present=hackit_auth_present,
        provider_is_hackit=provider_is_hackit,
        features=features,
...
def apply_hackit_managed_defaults(
    ...
    *,
    ...
    if not (
        ...
        return set()
    if not features.provider_is_hackit:
        return set()
    ...
    if "tts" in selected_toolsets and not features.tts.explicit_configured and not (
        ...
        tts_cfg["provider"] = "hackit"
        tts_cfg.pop("use_gateway", None)
        changed.add("tts")

    # STT: same pattern as TTS. The DEFAULT_CONFIG seed is "local"
    # (requires `pip install faster-whisper`); for Hackit subscribers we
    ...
    if "browser" in selected_toolsets and not features.browser.explicit_configured and not (
        ...
        browser_cfg["cloud_provider"] = "hackit"
...
# ``get_hackit_subscription_features`` reads (web uses "backend", browser
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    - unconfigured: tools with no direct credentials and no explicit
      non-hackit selection (easy switch, safe to pre-check)
    - has_direct: tools where the user has their own API keys
    - explicit_configured: tools with an explicit non-hackit selection stored
    ...
    All lists are empty when the user is not a paid Hackit subscriber or
    is not using Hackit as their provider.
    """
    ...
    # Quick provider check without the heavy get_hackit_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "hackit":
        return [], [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_hackit which fires implicitly when
    ...
    for key in _ALL_GATEWAY_KEYS:
        ...
        elif selected is not None and selected != "hackit":
            # An explicit non-hackit selection (e.g. a keyless local backend
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    # already_managed, just for a non-hackit vendor.
    ...
    try:
        from slick_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    try:
        account_info = get_hackit_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
    ...
    source_label = "free tool pool" if pool_only else "Hackit subscription"
    ...
    # pre-checked checklist re-fired on every Hackit model swap.
    ...
    if pool_only:
        title = "Your free Hackit tool pool — pick the tools to enable:"
    else:
        title = (
            "Your Hackit subscription includes the Tool Gateway — "
    ...
    # so the next Hackit model swap doesn't pre-check it again. Cancel paths
...
# ---------------------------------------------------------------------------
# Inline Hackit Portal login for the Tool Gateway picker (`slick tools`)
# ---------------------------------------------------------------------------


def ensure_hackit_portal_access(
    *,
    capability: str = "the Hackit Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the Hackit Tool Gateway, logging in if
    needed.

    Used by ``slick tools`` when a user selects a Hackit-managed Tool Gateway
    backend (e.g. "Firecrawl (Hackit Portal)").  Unlike ``slick model``'s Hackit
    login, this:
    ...
    It only performs the Hackit Portal device-code OAuth (when the user isn't
    ...
    try:
        info = get_hackit_portal_account_info(force_fresh=True)
    except Exception:
        info = None
    if _entitled(info):
        return True
    ...
    if info is None or not info.logged_in:
        if not _run_hackit_portal_login_only(capability=capability):
            return False
        try:
            info = get_hackit_portal_account_info(force_fresh=True)
        except Exception:
...
def _run_hackit_portal_login_only(*, capability: str) -> bool:
    """Run the Hackit Portal device-code OAuth and persist credentials only.
    ...
    try:
        from slick_cli.auth import (
            _auth_store_lock,
            _load_auth_store,
            _hackit_device_code_login,
            _read_shared_hackit_state,
            _save_auth_store,
            _save_provider_state,
            _sync_hackit_pool_from_auth_store,
            _try_import_shared_hackit_state,
            _write_shared_hackit_state,
        )
    except Exception as exc:  # pragma: no cover - defensive
        print(f"  Could not start Hackit Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a Hackit Portal login.")
    try:
        proceed = input("  Log in to Hackit Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped Hackit Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to Hackit.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing Hackit OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                auth_state = _try_import_shared_hackit_state(timeout_seconds=15.0)
        ...
        _sync_hackit_pool_from_auth_store()
        print("  Hackit Portal login successful.")
        return True
    except KeyboardInterrupt:
        print("\n  Login cancelled.")
        return False
    except SystemExit:
        # _hackit_device_code_login raises SystemExit on subscription_required;
        # it already printed billing guidance.
        return False
    except Exception as exc:
        print(f"  Hackit Portal login failed: {exc}")
        return False
