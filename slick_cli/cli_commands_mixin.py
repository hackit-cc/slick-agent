...
class CLICommandsMixin:
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        - ``/debug sophos``   → upload to Sophos-internal storage (private, staff-only)
        - ``/debug local``  → render the report to stdout, no upload

        ``sophos`` and ``local`` are mutually exclusive; if both are given,
        ...
        sophos = "sophos" in words and not local
        ...
        args = SimpleNamespace(
            lines=200, expire=7, local=local, sophos=sophos, yes=True
        )
