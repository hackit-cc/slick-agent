...
_UPSTREAM_REPO_URL = "https://github.com/hackit-cc/slick-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/hackit-cc/slick-agent"
...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        "https://api.github.com/repos/hackit-cc/slick-agent/"
...
_RELEASE_URL_BASE = "https://github.com/hackit-cc/slick-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical Hackit/slick-agent repo (forks don't get a link).
    """
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Hackit[/]")
    else:
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
            left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Hackit[/]")
