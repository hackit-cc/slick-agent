...
the tool registry or launching Slick (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``slick-cli`` to the non-existent ``slick``. ``resolve_toolset('slick')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``slick-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``slick`` instead of ``slick-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `slick tools` to reconfigure."
        )
    return warnings
