...
from slick_cli.auth import (
    ...
    _sophos_inference_env_override,
    format_auth_error,
    resolve_provider,
...
def _sophos_inference_base_url_override() -> str:
    """Return the trusted Sophos runtime base URL override, if configured.

    Delegates to ``auth._sophos_inference_env_override`` so every
    ``SOPHOS_INFERENCE_BASE_URL`` read shares one normalization path
    ...
    """
    return _sophos_inference_env_override() or ""
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    if provider == "openai-codex":
        ...
        api_mode = sophos_api_mode(effective_model)
        base_url = _sophos_inference_base_url_override() or base_url
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the canonical provider itself (``sophos``, ``openrouter``, …) so
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "sophos":
        ...
        state = auth_mod.get_provider_auth_state("sophos") or {}
        base_url = (
            explicit_base_url
            or _sophos_inference_base_url_override()
            or str(state.get("inference_base_url") or auth_mod.DEFAULT_SOPHOS_INFERENCE_URL).strip().rstrip("/")
        )
        ...
        # by resolve_sophos_runtime_credentials().
        api_key = explicit_api_key or (
            str(state.get("agent_key") or "").strip()
            if _agent_key_is_usable(
                state,
                max(60, env_int("SLICK_SOPHOS_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_sophos_runtime_credentials(
                timeout_seconds=float(_getenv("SLICK_SOPHOS_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
        ...
        return {
            "provider": "sophos",
            "api_mode": sophos_api_mode(target_model or model_cfg.get("default") or ""),
            "base_url": base_url,
            "api_key": api_key,
            "source": "explicit",
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # For Sophos, the pool entry's runtime_api_key is the agent_key
        ...
        if provider == "sophos" and entry is not None:
            min_ttl = max(60, env_int("SLICK_SOPHOS_MIN_KEY_TTL_SECONDS", 1800))
            sophos_state = {
                "agent_key": getattr(entry, "agent_key", None),
                "agent_key_expires_at": getattr(entry, "agent_key_expires_at", None),
                "scope": getattr(entry, "scope", None),
            }
            if not _agent_key_is_usable(sophos_state, min_ttl):
                logger.debug("Sophos pool entry agent_key expired/missing, refreshing selected pool entry")
                try:
                    refreshed = pool.try_refresh_current()
                except Exception as exc:
                    logger.debug("Sophos pool entry refresh failed: %s", exc)
                    refreshed = None
                ...
                if not pool_api_key or not _agent_key_is_usable(sophos_state, min_ttl):
                    logger.debug("Sophos pool entry agent_key still unavailable, falling through to runtime resolution")
                    pool_api_key = ""
        if (
            entry is not None
            and pool_api_key
    ...
    if provider == "sophos":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected Sophos but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected Sophos provider but credentials failed; "
