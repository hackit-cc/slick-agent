...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or slick -c.",
    ...
    "/skin changes the CLI theme — try ruby, mono, slate, mint, or charizard.",
    ...
    "Ctrl+Z suspends Slick to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "slick -c resumes your most recent CLI session. slick -c \"project name\" resumes by title.",
    "slick -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "slick -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "slick chat -t web,terminal enables only specific toolsets for a focused session.",
    "slick chat -s github-pr-workflow preloads a skill at launch.",
    "slick chat -q \"query\" runs a single non-interactive query and exits.",
    "slick chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "slick chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "slick --yolo bypasses all dangerous command approval prompts for the entire session.",
    "slick chat --source telegram tags the session for filtering in slick sessions list.",
    "slick -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "slick doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "slick dump outputs a compact setup summary — great for bug reports.",
    "slick config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "slick config edit opens config.yaml in your default editor.",
    "slick config check scans for missing or stale configuration options.",
    "slick sessions browse opens an interactive session picker with search.",
    "slick sessions stats shows session counts by platform and database size.",
    "slick sessions prune --older-than 30 cleans up old sessions.",
    "slick skills search react --source skills-sh searches the skills.sh public directory.",
    "slick skills check scans installed hub skills for upstream updates.",
    "slick skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "slick skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "slick mcp add github --command npx adds MCP servers from the command line.",
    "slick mcp serve runs Slick itself as an MCP server for other agents.",
    "slick auth add lets you add multiple API keys for credential pool rotation.",
    "slick completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "slick logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "slick backup creates a zip backup of your entire Slick home directory.",
    "slick profile create coder creates an isolated profile that becomes its own command.",
    "slick profile create work --clone copies your current config and keys to a new profile.",
    "slick update syncs new bundled skills to ALL profiles automatically.",
    "slick gateway install sets up Slick as a system service (systemd/launchd).",
    "slick memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "slick webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: slick tools disables unused tools, slick skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "slick models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as slick -w).",
    ...
    "SLICK_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Slick tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'slick profile create coder' creates the 'coder' command.",
    "slick profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Slick prints a resume command with session ID and stats.",
    "slick sessions export backup.jsonl exports all sessions for backup or analysis.",
    "slick -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "slick skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .slick/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: slick cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Slick runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "slick gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Slick via a local macOS server.",
    ...
    "Context files (.slick.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Slick loads project context from .slick.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "slick mcp opens an interactive picker of Hackit-approved MCPs you can install in one keystroke.",
    "slick mcp catalog lists Hackit-approved MCP servers shipped with the repo.",
    "slick mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — slick tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.slick/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "slick chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "slick plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "slick acp runs Slick as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "SLICK_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "slick auth add hackit or slick auth add openai-codex sets up OAuth-based providers.",
    ...
    "slick status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.slick/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Slick your own.",
    ...
    "Before a gateway session reset, Slick auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.slick/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Slick state via SLICK_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "SLICK_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SLICK_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.slick/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'slick profile create ls' would be rejected.",
    "slick profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "slick claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "SLICK_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via slick tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.slick/.env.",
    "Skills from trusted repos (Hackit) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.slick/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Slick auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Slick config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Slick is working.',
    ...
    '/reload reloads ~/.slick/.env into the running session — pick up new API keys without restarting.',
    ...
    "SLICK_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.slick/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.slick/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'slick curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "slick curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'slick curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'slick auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Hackit subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'SLICK_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "SLICK_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'slick dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'slick dashboard embeds the full Slick TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.slick/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.slick/dashboard-plugins/ — no npm build required.',
    ...
    'SLICK_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'SLICK_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'SLICK_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'SLICK_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.slick/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.slick/skills/ so drop-in skills appear without restarting the session.',
    ...
    'slick -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'slick chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'slick chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'slick chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "slick chat --source tool tags programmatic chats so they don't clutter slick sessions list.",
    'slick dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'slick sessions rename <ID> "new title" renames any past session; slick sessions delete <ID> removes one.',
    'slick import restores a full Slick backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'slick fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'slick pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'slick setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'slick status --deep runs the full health sweep across every component; plain slick status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'SLICK_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'SLICK_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.slick/plugins/ — trust-gated by design.',
    "SLICK_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'SLICK_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'SLICK_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'SLICK_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'SLICK_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'SLICK_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'SLICK_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'SLICK_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'SLICK_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'SLICK_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'SLICK_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Slick block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from slick sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.slick/dashboard-plugins/.',
]
...
# any project or none — Slick is not a coding-only agent.
# ---------------------------------------------------------------------------
