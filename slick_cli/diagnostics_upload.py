"""Client for uploading ``slick debug share`` bundles to Sophos-internal S3.

This is the opt-in (``--sophos``) destination for ``slick debug share``.
Unlike the public paste.rs path, bundles uploaded here go to a Sophos-owned
S3 bucket via a short-lived signed URL minted by the Sophos account service
(NAS).  The bucket auto-expires objects after 14 days, and the contents are
only viewable by Sophos staff (and allowlisted Discord mods) through a
...
# Base URL of the Sophos account service that mints the signed upload URL.
...
NAS_BASE = os.environ.get(
    "SLICK_DIAGNOSTICS_BASE_URL", "https://portal.sophostechne.com"
)
...
_USER_AGENT = "slick-agent/debug-share"
...
def share_to_sophos(report_bundle: bytes) -> dict:
    """Orchestrate the full Sophos-S3 upload of a gzipped *report_bundle*.
