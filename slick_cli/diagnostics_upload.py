"""Client for uploading ``slick debug share`` bundles to Hackit-internal S3.

This is the opt-in (``--hackit``) destination for ``slick debug share``.
Unlike the public paste.rs path, bundles uploaded here go to a Hackit-owned
S3 bucket via a short-lived signed URL minted by the Hackit account service
(NAS).  The bucket auto-expires objects after 14 days, and the contents are
only viewable by Hackit staff (and allowlisted Discord mods) through a
...
# Base URL of the Hackit account service that mints the signed upload URL.
...
NAS_BASE = os.environ.get(
    "SLICK_DIAGNOSTICS_BASE_URL", "https://portal.hackit.cc"
)
...
def share_to_hackit(report_bundle: bytes) -> dict:
    """Orchestrate the full Hackit-S3 upload of a gzipped *report_bundle*.
