"""Normalized Sophos Portal account entitlement helpers."""
...
SophosAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]

# Free tool-pool coverage categories. Kept byte-for-byte aligned with the
# Portal's TOOL_COVERAGE_CATEGORIES (sophos-account-service
...
_account_info_cache: tuple[str, float, "SophosPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class SophosPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class SophosPaidServiceAccessInfo:
...
@dataclass(frozen=True)
class SophosPortalAccountInfo:
    logged_in: bool
    source: SophosAccountInfoSource
    fresh: bool
    ...
    sophos_client: Optional[str] = None
    ...
    subscription: Optional[SophosPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[SophosPaidServiceAccessInfo] = None
    tool_access: Optional[SophosToolAccessInfo] = None
...
def sophos_portal_billing_url(account_info: Optional[SophosPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized Sophos account snapshot."""
    try:
    ...
    if not isinstance(base, str) or not base.strip():
        base = DEFAULT_SOPHOS_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def sophos_portal_topup_url(account_info: Optional[SophosPortalAccountInfo] = None) -> str:
    ...
    """
    base_billing = sophos_portal_billing_url(account_info)  # {base}/billing
...
def format_sophos_portal_entitlement_message(
    account_info: Optional[SophosPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
    coverage_category: Optional[str] = None,
) -> Optional[str]:
    """Return user-facing guidance for a missing Sophos tool-gateway entitlement.
    ...
    """
    billing_url = sophos_portal_billing_url(account_info)
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current Sophos Portal "
    ...
    if account_info is None:
        return (
            f"Slick could not verify your Sophos Portal entitlement, so {capability} "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Sophos inference credentials are configured, but Slick cannot verify "
                f"your Sophos Portal paid access for {capability}. Log in with "
                f"`slick model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Sophos Portal to use {capability}: run `slick model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Slick could not verify your Sophos Portal paid access, so {capability} "
            f"is unavailable."
        )
    ...
    if reason == "account_missing":
        return (
            f"Slick could not find a Sophos Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `slick model` to "
            f"authenticate again; if the problem persists, contact Sophos support."
        )
    ...
    return (
        f"Your Sophos Portal account does not currently have paid service access, "
        f"so {capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def _no_paid_access_message(
    account_info: SophosPortalAccountInfo,
    capability: str,
    billing_url: str,
) -> str:
    access = account_info.paid_service_access_info
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return (
            f"Your Sophos Portal access is paused because you've exceeded the"
    ...
    if has_active_subscription and active_subscription_is_paid:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Sophos Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current Sophos Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Sophos Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your Sophos Portal account has no usable paid credits{credit_detail}, so "
    ...
    *,
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        ...
        if pool_info is not None:
            return pool_info
        return SophosPortalAccountInfo(
            logged_in=False,
            source="none",
            fresh=False,
            portal_base_url=portal_base_url,
...
def _fresh_account_info(
    *,
    ...
    try:
        from slick_cli.auth import get_provider_auth_state, resolve_sophos_access_token

        access_token = resolve_sophos_access_token()
        ...
        payload = _fetch_sophos_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[SophosPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque Sophos keys."""
    try:
        entry = _select_sophos_pool_entry()
        if entry is None:
            return None
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_sophos_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _select_sophos_pool_entry() -> Optional[Any]:
...
def _fetch_sophos_account_info(
    access_token: str,
    portal_base_url: Optional[str] = None,
) -> dict[str, Any]:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    access_info = SophosPaidServiceAccessInfo(
        allowed=paid_access,
    ...
    return SophosPortalAccountInfo(
        ...
        sophos_client=_coerce_str(claims.get("sophos_client")),
        portal_base_url=portal_base_url,
...
def _tool_access_from_value(value: Any) -> Optional[SophosToolAccessInfo]:
    """Parse a Portal ``tool_access`` object (from the JWT claim or the account
    API) into :class:`SophosToolAccessInfo`. Fails closed: a non-object value
    ...
    return SophosToolAccessInfo(enabled=enabled, coverage=coverage)


def _subscription_from_payload(value: Any) -> Optional[SophosPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return SophosPortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[SophosPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return SophosPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
