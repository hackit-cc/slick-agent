"""slick memory setup|status — configure memory provider plugins.
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``slick memory setup`` — if the update-time refresh only reinstalled
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_slick_home() / "hindsight" / "config.json"
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as slick tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``slick update`` heals the active memory provider
    ...
    # data volume (SLICK_LAZY_INSTALL_TARGET). install_specs handles the
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'slick memory setup' to see available providers.\n")
        return
    ...
    if hasattr(provider, "post_setup"):
        ...
        provider.post_setup(slick_home, config)
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.slick/plugins/ and try again.\n")
        return
...
def _write_env_vars(
    env_writes: dict,
    slick_home: str | os.PathLike[str] | None = None,
) -> None:
    """Persist memory-provider env vars through the canonical ``.env`` writer.

    Delegates to ``slick_cli.config.save_env_value`` so every key flows
    ...
    ``SLICK_HOME`` / etc.), CR/LF stripping on the value, and the atomic
    ...
    ``.env`` -> ``os.environ`` chain on the next Slick startup, and the
    ...
    ``slick_home`` may be supplied by plugin ``post_setup`` hooks that
    already received an explicit home directory (e.g. a non-default
    profile). It is applied through the context-local Slick home override
    ...
    token = set_slick_home_override(slick_home) if slick_home is not None else None
    try:
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if provider:
            ...
            if provider.is_available():
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        print(line)
                print(
                    "  Note: systemd/gateway services do not inherit ~/.slick/.env —"
                )
                print(
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.slick/plugins/")
