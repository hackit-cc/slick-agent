"""Bounded product contract for the first Slick shared-metrics slice."""
...
SCHEMA_KEY = "slick.metrics.schema_version"
SCHEMA_VERSION = "slick.metrics.event.v2"
MODEL_CALL_SCOPE = "slick.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "slick.task_run"
TOOL_CALL_SCOPE = "slick.tool_call"
CLIENT_ACTIVE_MARK = "slick.client.active"
TOOL_APPROVAL_MARK = "slick.tool_approval"
SKILL_LIFECYCLE_MARK = "slick.skill.lifecycle"
SKILL_LOAD_MARK = "slick.skill.load"
SUBSCRIBER_NAME = "slick.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "slick.client.active"
LEGACY_MODEL_CALL_METRIC = "slick.model_call.count"
MODEL_ROUTE_METRIC = "slick.model_route.count"
TASK_STARTED_METRIC = "slick.task_run.started"
TASK_FINISHED_METRIC = "slick.task_run.finished"
TOOL_CALL_METRIC = "slick.tool_call.count"
TOOL_APPROVAL_METRIC = "slick.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "slick.skill.lifecycle.count"
SKILL_LOAD_METRIC = "slick.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Slick installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(slick_version or "").strip()
    ...
    return {
        ...
        "slick_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("slick_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Slick logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("slick.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from slick_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Slick terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Slick registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "slick-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Slick tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Slick."""
