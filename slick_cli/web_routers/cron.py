...
async def cron_fire_webhook(request: Request):
    """Webcron managed-cron fire webhook (NAS -> agent) — gateway forwarder.

    Authenticated by a short-lived NAS-minted JWT (verified by the pluggable
    Webcron fire-verifier), NOT the dashboard session cookie — so this path is
    ...
    api_server disabled) → 503, so NAS retries per the Webcron contract
    ...
    if forwarded is None:
        ...
        #   and the gateway's Webcron provider reconciles + re-arms every
        #   job on its next startup (plugins/cron_providers/webcron
        ...
        if await _run_cron_dashboard_io(_gateway_intentionally_stopped, profile):
            _log.info(
                ...
                "Webcron reconcile on next gateway start",
                profile, job_id,
            )
            return JSONResponse(
                {
...
async def instantiate_blueprint(body: AutomationBlueprintInstantiate, profile: str = "default"):
    ...
    try:
        ...
        # a Webcron reconcile does file I/O plus NAS network calls.
