    ...
    def _read():
        with _profile_scope(profile):
            ...
            if cat:
                ...
                # re-probing per row.
                features = get_hackit_subscription_features(config, force_fresh=True)
                for prov in _visible_providers(cat, config, force_fresh=True):
                    ...
                    row = {
                        ...
                        "requires_hackit_auth": bool(prov.get("requires_hackit_auth")),
                        "is_active": is_active,
                        # Honest server-side readiness. The GUI's old client-side
                        # heuristic showed "Ready" for every zero-env-var row —
                        # including logged-out Hackit Subscription rows and never-run
...
async def select_toolset_provider(
    ...
    Managed Hackit rows (``managed_hackit_feature``) additionally report the
    Portal entitlement state: the CLI flow gates these selections on
    ``ensure_hackit_portal_access`` (inline login), but the GUI has no inline
    ...
    ``managed_by_hackit``). The response now carries an additive
    ``needs_hackit_auth: true`` + ``feature`` so the client can drive the
    existing Hackit Portal OAuth flow (``POST /api/providers/oauth/hackit/start``)
    and refetch.
    """
    from slick_cli.tools_config import (
        TOOL_CATEGORIES,
    ...
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            # Entitlement check for managed Hackit rows — mirrors the gate the CLI
            # applies via ensure_hackit_portal_access at selection time. This hits
            ...
            managed_feature = (row or {}).get("managed_hackit_feature")
            if managed_feature:
                features = get_hackit_subscription_features(config, force_fresh=True)
                ...
                if not entitled:
                    response["needs_hackit_auth"] = True
