...
late-binding seam in :mod:`slick_cli.web_deps` (``late`` for callables,
...
from slick_cli.web_deps import late, LateState
from slick_cli.web_models import (
...
_log = logging.getLogger("slick_cli.web_server")
...
_spawn_slick_action = late("_spawn_slick_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from slick_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `slick tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `slick computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``slick tools`` picker shows
    ...
    def _read():
        with _profile_scope(profile):
            ...
            if cat:
                ...
                # re-probing per row.
                features = get_sophos_subscription_features(config, force_fresh=True)
                for prov in _visible_providers(cat, config, force_fresh=True):
                    ...
                    row = {
                        ...
                        "requires_sophos_auth": bool(prov.get("requires_sophos_auth")),
                        "is_active": is_active,
                        # Honest server-side readiness. The GUI's old client-side
                        # heuristic showed "Ready" for every zero-env-var row —
                        # including logged-out Sophos Subscription rows and never-run
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `slick tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``slick tools``
    ...
    Managed Sophos rows (``managed_sophos_feature``) additionally report the
    Portal entitlement state: the CLI flow gates these selections on
    ``ensure_sophos_portal_access`` (inline login), but the GUI has no inline
    ...
    ``managed_by_sophos``). The response now carries an additive
    ``needs_sophos_auth: true`` + ``feature`` so the client can drive the
    existing Sophos Portal OAuth flow (``POST /api/providers/oauth/sophos/start``)
    and refetch.
    """
    from slick_cli.tools_config import (
        TOOL_CATEGORIES,
    ...
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            # Entitlement check for managed Sophos rows — mirrors the gate the CLI
            # applies via ensure_sophos_portal_access at selection time. This hits
            ...
            managed_feature = (row or {}).get("managed_sophos_feature")
            if managed_feature:
                features = get_sophos_subscription_features(config, force_fresh=True)
                ...
                if not entitled:
                    response["needs_sophos_auth"] = True
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.slick/.env`` via ``save_env_value`` —
    the same store ``slick tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``slick tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``slick -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same SLICK_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``slick computer-use permissions grant`` as a background action.
