"""Profile distributions — shareable, packaged Slick profiles via git.

A distribution is a Slick profile published as a git repository (or
...
* ``slick profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``slick skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``slick profile``, not a parallel tree):

    slick profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "slick-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            slick_requires=str(data.get("slick_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.slick_requires:
            out["slick_requires"] = self.slick_requires
        if self.author:
...
def check_slick_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Slick {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``slick profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Slick profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Slick distribution."
        )

    # Version check up-front so we fail fast
    check_slick_requires(manifest.slick_requires, slick_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.slick).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="slick_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `slick profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`slick profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="slick_dist_update_") as tmp:
        plan = plan_install(
