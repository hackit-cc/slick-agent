"""``slick debug`` debug tools for Slick Agent.

Currently supports:
    slick debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.slick/logs/*.log`` are not leaked into
                          the public paste service. Pass ``--no-redact``
                          to disable.
                          Pass ``--sophos`` to upload instead to Sophos-internal
                          storage (AWS S3) via a signed URL minted by the
                          Sophos account service: the bundle is private
                          (viewable only by Sophos staff / allowlisted mods via
...
_REDACTION_BANNER = (
    "[slick debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.slick/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``slick debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``slick debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `slick debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "slick-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``slick debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.slick/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``slick debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "slick-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----SlickDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from slick_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_slick_home() / "logs" / filename) if filename else None


# Logs written by a client process rather than by this backend. When the
# desktop app talks to a remote/docker/SSH backend, `slick debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Slick Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``slick dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``slick dump`` is run
        internally.
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by both the paste.rs and Sophos-S3 paths)
# ---------------------------------------------------------------------------

# Bundle format identifier embedded in the Sophos-S3 JSON envelope. The
# discord-support viewer keys off this string to parse the bundle.
_SOPHOS_BUNDLE_FORMAT = "slick-debug-share/1"
...
def collect_share_bundle(
    ...
    Sophos-S3 path (``--sophos``).  Centralising it guarantees the Sophos bundle is
    built from the *same* force-redacted snapshots as the public paste path —
    redaction is the safety boundary, so the Sophos path must never see raw
    logs.
...
def build_sophos_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip-compress a :func:`collect_share_bundle` mapping into the Sophos envelope.

    The JSON shape is what the discord-support viewer (Repo 3) parses::

        {"format": "slick-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``slick debug share`` (CLI) and the
    ...
    # shared collector so the paste.rs and Sophos-S3 paths build identical,
    ...
    if redact:
        logger.info(
            "slick debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    sophos = getattr(args, "sophos", False)
    ...
    if sophos:
        _run_debug_share_sophos(args, log_lines=log_lines, redact=redact)
        return
    ...
    try:
        ...
        print("\nRun `slick debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  slick debug delete <url>")

    print("\nShare these links with the Slick team for support.")


_SOPHOS_PRIVACY_NOTICE = """\
⚠️  --sophos: This uploads your debug bundle to Sophos-INTERNAL storage (AWS S3),
    NOT a public paste service. The following is included:
  • System info (OS, Python/Slick version, provider, which API keys are
  ...
  • The bundle is viewable only by Sophos staff (and allowlisted Discord mods)
...
def _run_debug_share_sophos(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``slick debug share --sophos``: upload the bundle to Sophos-S3.

    Collects the same force-redacted bundle as the paste path, gzips it into
    the Sophos envelope, requests a signed URL from NAS, uploads, and prints the
    private viewer link. On any failure falls back to a clear error that
    suggests ``--local``.
    """
    from slick_cli.diagnostics_upload import share_to_sophos

    print(_SOPHOS_PRIVACY_NOTICE)
    if not _confirm_upload(args):
        return
    if not redact:
        print(
    ...
    if redact:
        logger.info(
            "slick debug share --sophos: applied force-mode redaction before upload"
        )
    blob = build_sophos_bundle(bundle, redact=redact)

    print("Uploading to Sophos diagnostics storage...")
    try:
        res = share_to_sophos(blob)
    except Exception as exc:
        print(
            f"\nSophos upload failed: {exc}\n"
            "\nThe Sophos diagnostics service may be unavailable or not yet "
            "provisioned.\n"
            "Run `slick debug share --local` to print the report instead, "
            "or `slick debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)

    view_url = res.get("viewUrl") or res.get("view_url")
    print("\nDebug bundle uploaded to Sophos (private):")
    if view_url:
    ...
    print(
        "\nShare this private link with the Sophos team — only Sophos staff "
        "(via Google login) can open it."
    )
    print(
        "\nPick up the discussion in:\n"
        "  GitHub Issues        https://github.com/SophosTechne/slick-agent/issues\n"
        "  Sophos Portal Support  https://portal.sophostechne.com/help\n"
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: slick debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'slick debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``slick debug`` call.
    ...
    # best-effort — any failure is swallowed so ``slick debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: slick debug <command>")
        print()
        print("Commands:")
        ...
        print("  --sophos       Upload to Sophos-internal storage (private, staff-only,")
