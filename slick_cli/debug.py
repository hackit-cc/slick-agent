...
Currently supports:
    slick debug share    Upload debug report (system info + logs) to a
                          ...
                          to disable.
                          Pass ``--hackit`` to upload instead to Hackit-internal
                          storage (AWS S3) via a signed URL minted by the
                          Hackit account service: the bundle is private
                          (viewable only by Hackit staff / allowlisted mods via
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by both the paste.rs and Hackit-S3 paths)
# ---------------------------------------------------------------------------

# Bundle format identifier embedded in the Hackit-S3 JSON envelope. The
# discord-support viewer keys off this string to parse the bundle.
_HACKIT_BUNDLE_FORMAT = "slick-debug-share/1"
...
def collect_share_bundle(
    ...
    Hackit-S3 path (``--hackit``).  Centralising it guarantees the Hackit bundle is
    built from the *same* force-redacted snapshots as the public paste path —
    redaction is the safety boundary, so the Hackit path must never see raw
    logs.
...
def build_hackit_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip-compress a :func:`collect_share_bundle` mapping into the Hackit envelope.
...
def build_debug_share(
    *,
    ...
    # shared collector so the paste.rs and Hackit-S3 paths build identical,
...
def run_debug_share(args):
    ...
    hackit = getattr(args, "hackit", False)
    ...
    if hackit:
        _run_debug_share_hackit(args, log_lines=log_lines, redact=redact)
        return
...
_HACKIT_PRIVACY_NOTICE = """\
⚠️  --hackit: This uploads your debug bundle to Hackit-INTERNAL storage (AWS S3),
  ...
  • The bundle is viewable only by Hackit staff (and allowlisted Discord mods)
...
def _run_debug_share_hackit(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``slick debug share --hackit``: upload the bundle to Hackit-S3.

    Collects the same force-redacted bundle as the paste path, gzips it into
    the Hackit envelope, requests a signed URL from NAS, uploads, and prints the
    private viewer link. On any failure falls back to a clear error that
    suggests ``--local``.
    """
    from slick_cli.diagnostics_upload import share_to_hackit

    print(_HACKIT_PRIVACY_NOTICE)
    if not _confirm_upload(args):
        return
    if not redact:
        print(
    ...
    if redact:
        logger.info(
            "slick debug share --hackit: applied force-mode redaction before upload"
        )
    blob = build_hackit_bundle(bundle, redact=redact)

    print("Uploading to Hackit diagnostics storage...")
    try:
        res = share_to_hackit(blob)
    except Exception as exc:
        print(
            f"\nHackit upload failed: {exc}\n"
            "\nThe Hackit diagnostics service may be unavailable or not yet "
            "provisioned.\n"
            "Run `slick debug share --local` to print the report instead, "
    ...
    print("\nDebug bundle uploaded to Hackit (private):")
    if view_url:
    ...
    print(
        "\nShare this private link with the Hackit team — only Hackit staff "
        "(via Google login) can open it."
    )
    print(
        "\nPick up the discussion in:\n"
        "  GitHub Issues        https://github.com/hackit-cc/slick-agent/issues\n"
        "  Hackit Portal Support  https://portal.hackit.cc/help\n"
...
def run_debug(args):
    ...
    if subcmd == "share":
        ...
        print("  --hackit       Upload to Hackit-internal storage (private, staff-only,")
