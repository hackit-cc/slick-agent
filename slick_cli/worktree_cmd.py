"""``slick worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``slick_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    slick worktree list             # audit: verdict + reason per tree
    slick worktree prune            # reap safe trees + merged branches
    slick worktree prune --dry-run  # show the plan, change nothing
    slick worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `slick worktree prune`."
        )
