"""Slash command definitions and autocomplete for the Slick CLI.
...
from slick_constants import INDICATOR_STYLES
...
# _hackit_auth_status_cache). Falls back to a fresh load when the file cannot
# be stat'ed.
...
class CommandDef:
    ...
    # Names a key in ``slick_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``slick_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Slick config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Slick works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Slick' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Slick is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.slick/skills/ for newly installed or removed skills",
    ...
    CommandDef("subscription", "View your Hackit plan and change it in the browser", "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", "Show your Hackit balance and manage billing on the portal", "Info"),
    ...
    CommandDef("update", "Update Slick Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Slick Agent version", "Info", aliases=("v",),
               busy_policy="dispatch", execute="version"),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info",
               args_hint="[hackit|local]"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`slick_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/slick`` subcommand mapping, and
    ...
    """
    try:
        from slick_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
# Telegram allows up to 100 BotCommands. Slick ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Slick
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``slick skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``slick skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``slick skills list`` and
...
# unrelated command landed. These claim their slots right after /slick,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /slick <command>).
...
# native slot, the alias spelling stays reachable via /slick reset).
...
# ``/slick <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/slick" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /slick topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /slick moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /slick debug on Slack.
#   - egress: Docker-only proxy status; reachable as /slick egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /slick init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /slick version on
...
#   - diff: git working-tree diff; reached via /slick diff on Slack so it
...
#     /slick update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /slick heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /slick refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /slick pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /slick whoami on Slack.
...
#     /slick platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_SLICK_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "review", "pause", "whoami", "platform"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/slick <verb>`` subcommand.
    ...
    can still reach them via ``/slick <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/slick`` is always reserved as the first entry so the
    legacy ``/slick <subcommand>`` form keeps working for anything that
    ...
    # Reserve /slick as the catch-all top-level command.
    entries.append(("slick", "Talk to Slick or run a subcommand", "[subcommand] [args]"))
    seen.add("slick")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_SLICK_ONLY:
            # Intentionally Slack-via-/slick only (see _SLACK_VIA_SLICK_ONLY).
            return
    ...
    # everything except /slick, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://slick-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /slick handler.

    Maps both canonical names and aliases so /slick bg do stuff works
    the same as /slick background do stuff.

    Plugin-registered slash commands are included so ``/slick <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from slick_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via slick_cli.personality."""
        try:
            ...
            # freshness-correct (same pattern as load_env / _hackit_auth_status_cache).
