"""Normalized Hackit Portal account entitlement helpers."""
...
HackitAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]

# Free tool-pool coverage categories. Kept byte-for-byte aligned with the
# Portal's TOOL_COVERAGE_CATEGORIES (hackit-account-service
...
_account_info_cache: tuple[str, float, "HackitPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class HackitPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class HackitPaidServiceAccessInfo:
...
@dataclass(frozen=True)
class HackitPortalAccountInfo:
    logged_in: bool
    source: HackitAccountInfoSource
    fresh: bool
    ...
    hackit_client: Optional[str] = None
    ...
    subscription: Optional[HackitPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[HackitPaidServiceAccessInfo] = None
    tool_access: Optional[HackitToolAccessInfo] = None
...
def hackit_portal_billing_url(account_info: Optional[HackitPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized Hackit account snapshot."""
    try:
    ...
    if not isinstance(base, str) or not base.strip():
        base = DEFAULT_HACKIT_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def hackit_portal_topup_url(account_info: Optional[HackitPortalAccountInfo] = None) -> str:
    ...
    """
    base_billing = hackit_portal_billing_url(account_info)  # {base}/billing
...
def format_hackit_portal_entitlement_message(
    account_info: Optional[HackitPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
    coverage_category: Optional[str] = None,
) -> Optional[str]:
    """Return user-facing guidance for a missing Hackit tool-gateway entitlement.
    ...
    """
    billing_url = hackit_portal_billing_url(account_info)
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current Hackit Portal "
    ...
    if account_info is None:
        return (
            f"Slick could not verify your Hackit Portal entitlement, so {capability} "
            f"is unavailable. Run `slick model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Hackit inference credentials are configured, but Slick cannot verify "
                f"your Hackit Portal paid access for {capability}. Log in with "
                f"`slick model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Hackit Portal to use {capability}: run `slick model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Slick could not verify your Hackit Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `slick model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Slick could not find a Hackit Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `slick model` to "
            f"authenticate again; if the problem persists, contact Hackit support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `slick model` to refresh Slick."
        return message

    return (
        f"Your Hackit Portal account does not currently have paid service access, "
        f"so {capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def _no_paid_access_message(
    account_info: HackitPortalAccountInfo,
    capability: str,
    billing_url: str,
) -> str:
    access = account_info.paid_service_access_info
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return (
            f"Your Hackit Portal access is paused because you've exceeded the"
            f" per-member spend cap set by your organisation.{cap_detail}"
            f"{credit_detail} Ask your organisation admin to raise the"
            f" member spend cap at {billing_url}, then run `slick model`"
            f" to refresh."
        )
    ...
    if has_active_subscription and active_subscription_is_paid:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Hackit Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current Hackit Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Hackit Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your Hackit Portal account has no usable paid credits{credit_detail}, so "
    ...
    *,
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        ...
        if pool_info is not None:
            return pool_info
        return HackitPortalAccountInfo(
            logged_in=False,
            source="none",
            fresh=False,
            portal_base_url=portal_base_url,
...
def _fresh_account_info(
    *,
    ...
    try:
        from slick_cli.auth import get_provider_auth_state, resolve_hackit_access_token

        access_token = resolve_hackit_access_token()
        ...
        payload = _fetch_hackit_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[HackitPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque Hackit keys."""
    try:
        entry = _select_hackit_pool_entry()
        if entry is None:
            return None
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_hackit_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _select_hackit_pool_entry() -> Optional[Any]:
...
def _fetch_hackit_account_info(
    access_token: str,
    portal_base_url: Optional[str] = None,
) -> dict[str, Any]:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from slick_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = HackitPaidServiceAccessInfo(
        allowed=paid_access,
    ...
    return HackitPortalAccountInfo(
        ...
        hackit_client=_coerce_str(claims.get("hackit_client")),
        portal_base_url=portal_base_url,
...
def _tool_access_from_value(value: Any) -> Optional[HackitToolAccessInfo]:
    """Parse a Portal ``tool_access`` object (from the JWT claim or the account
    API) into :class:`HackitToolAccessInfo`. Fails closed: a non-object value
    ...
    return HackitToolAccessInfo(enabled=enabled, coverage=coverage)


def _subscription_from_payload(value: Any) -> Optional[HackitPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return HackitPortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[HackitPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return HackitPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
