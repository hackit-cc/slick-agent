...
on Japanese locales, etc.).  Slick's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``slick_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Slick is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SLICK_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SLICK_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Slick-managed Git install directories so
    ...
    # already-running shells don't see the change — which means slick
    ...
    # install without Slick-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Slick's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Slick's
    ``slick config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Slick (User env var in Windows Settings, or
    export in a PowerShell profile) and Slick picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Slick-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\slick\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``slick`` from right after install) retain their old PATH.

    Any subprocess Slick spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Slick-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "slick", "git", "cmd"),
        os.path.join(local_appdata, "slick", "git", "bin"),
        os.path.join(local_appdata, "slick", "git", "usr", "bin"),
        # Slick venv Scripts directory — host of the slick.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes slick, but harmless to include.
        os.path.join(local_appdata, "slick", "slick-agent", "venv", "Scripts"),
