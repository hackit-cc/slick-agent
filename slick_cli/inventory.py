...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (Hackit /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for Hackit, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``slick model`` CLI picker. Adds network calls
      (pricing fetch + Hackit tier check); only set for interactive pickers.
    ...
    - ``force_fresh_hackit_tier``: bypass the short Hackit free-tier cache when
      selecting Portal-recommended Hackit models and applying tier gating. Keep
    ...
    rows = list_authenticated_providers(
        ...
        force_fresh_hackit_tier=force_fresh_hackit_tier,
    ...
    try:
        from slick_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
    ...
    if pricing:
        _apply_pricing(rows, force_fresh_hackit_tier=force_fresh_hackit_tier)
    if capabilities:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``slick_cli/moa_cmd.py``.
...
def _reasoning_catalog_reader(slug: str):
    ...
    try:
        from slick_cli.models import (
            hackit_model_reasoning_capabilities,
            openrouter_model_reasoning_capabilities,
            warm_hackit_reasoning_caps_async,
    ...
    if slug == "hackit":
        warm_hackit_reasoning_caps_async()
        return hackit_model_reasoning_capabilities
    if slug == "openrouter":
        warm_openrouter_reasoning_caps_async()
...
def _apply_featured(rows: list[dict]) -> None:
    ...
    Aggregator providers (hackit, openrouter) serve dozens of models across many
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`slick_cli.providers.custom_provider_aliases` — lets the frontend
    do a membership check instead.
    """
    from slick_cli.providers import custom_provider_aliases
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from slick_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `slick model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _anthropic_oauth_credentials_present() -> bool:
    ...
    model.provider / API-key env vars: Slick' own Anthropic device flow
    ...
    try:
        ...
        slick_creds = read_slick_oauth_credentials() or {}
        if slick_creds.get("accessToken"):
            return True
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Slick.
    """
    ...
    for row in rows:
        ...
        if slug == "anthropic" and _anthropic_oauth_credentials_present():
            # Anthropic OAuth logins (Slick device flow / Claude Code) are
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's Slick overlay declares it keyless."""
    try:
        from slick_cli.providers import SLICK_OVERLAYS
        overlay = SLICK_OVERLAYS.get(slug)
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `slick model` to configure ({auth_type})"
        )
...
def _apply_pricing(
    ...
    *,
    ...
    # Resolve Hackit free-tier once (cached in models.py for the TTL window).
    hackit_free_tier: Optional[bool] = None
    ...
    for row in rows:
        ...
        for mid in models:
            ...
            # Sale chrome is Hackit Portal-only. Other providers (OpenRouter,
        ...
        if slug == "hackit":
            try:
                if hackit_free_tier is None:
                    hackit_free_tier = check_hackit_free_tier(
                        force_fresh=force_fresh_hackit_tier
                    )
                row["free_tier"] = bool(hackit_free_tier)
                if hackit_free_tier:
                    _selectable, unavailable = partition_hackit_models_by_tier(
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`slick_cli.model_switch.list_picker_providers`) so the
