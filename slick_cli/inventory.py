...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (Sophos /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for Sophos, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``slick model`` CLI picker. Adds network calls
      (pricing fetch + Sophos tier check); only set for interactive pickers.
    ...
    - ``force_fresh_sophos_tier``: bypass the short Sophos free-tier cache when
      selecting Portal-recommended Sophos models and applying tier gating. Keep
    ...
    rows = list_authenticated_providers(
        ...
        force_fresh_sophos_tier=force_fresh_sophos_tier,
    ...
    if pricing:
        _apply_pricing(rows, force_fresh_sophos_tier=force_fresh_sophos_tier)
    if capabilities:
...
def _reasoning_catalog_reader(slug: str):
    ...
    try:
        from slick_cli.models import (
            sophos_model_reasoning_capabilities,
            openrouter_model_reasoning_capabilities,
            warm_sophos_reasoning_caps_async,
    ...
    if slug == "sophos":
        warm_sophos_reasoning_caps_async()
        return sophos_model_reasoning_capabilities
    if slug == "openrouter":
        warm_openrouter_reasoning_caps_async()
...
def _apply_featured(rows: list[dict]) -> None:
    ...
    Aggregator providers (sophos, openrouter) serve dozens of models across many
...
def _apply_pricing(
    ...
    *,
    ...
    # Resolve Sophos free-tier once (cached in models.py for the TTL window).
    sophos_free_tier: Optional[bool] = None
    ...
    for row in rows:
        ...
        for mid in models:
            ...
            # Sale chrome is Sophos Portal-only. Other providers (OpenRouter,
        ...
        if slug == "sophos":
            try:
                if sophos_free_tier is None:
                    sophos_free_tier = check_sophos_free_tier(
                        force_fresh=force_fresh_sophos_tier
                    )
                row["free_tier"] = bool(sophos_free_tier)
                if sophos_free_tier:
                    _selectable, unavailable = partition_sophos_models_by_tier(
