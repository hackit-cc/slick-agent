"""Runtime-backed validation behind ``slick plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary SLICK_HOME
    with outbound socket connects blocked.
    """
    temporary_home = tempfile.TemporaryDirectory(prefix="slick-plugin-doctor-")
    stack = ExitStack()
    ...
    stack.enter_context(
        patch.dict(
            ...
            {
                "SLICK_HOME": str(home),
                "SLICK_BUNDLED_PLUGINS": str(bundled),
                "SLICK_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    modules_before = {
        ...
        if name == "slick_plugins" or name.startswith("slick_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Slick discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "slick_plugins" or name.startswith("slick_plugins."))
            ):
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_slick_home() / "plugins"
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".slick" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from slick_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Slick supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Slick never auto-installs plugin dependencies; "
    ...
    if schema:
        from slick_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Slick' real scanner and registration path."""
    try:
