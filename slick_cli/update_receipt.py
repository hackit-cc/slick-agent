...
1. **Update receipt** — a machine-readable JSON record of what one
   ``slick update`` run discovered, did, skipped (and why), written to
   ``<SLICK_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``slick_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``slick update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``slick update`` run."""
...
def _receipt_dir() -> Path:
    ...
    return get_slick_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``slick update`` has many early-termination paths (Windows
