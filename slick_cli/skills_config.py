"""
Skills configuration for Slick Agent.
`slick skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.slick/config.yaml under:
...
from slick_cli.config import cfg_get, load_config, save_config
from slick_cli.colors import Colors, color
from slick_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    Essential skills (e.g. ``slick-agent``) are silently dropped from the
