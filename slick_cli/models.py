...
Add, remove, or reorder entries here — both `slick setup` and
`slick` provider-selection will pick up the change automatically.
"""
...
from slick_cli.urllib_security import open_credentialed_url, url_origin
...
_SLICK_USER_AGENT = f"slick-cli/{_SLICK_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from slick_cli.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `slick model`
    flow without maintaining a separate static list.
    """
    from slick_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# $SLICK_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Slick-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $SLICK_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # "opencode/latest"; we send honest Slick attribution and don't
    ...
    # endpoint expects (see slick_cli/model_setup_flows.py).
...
# ---------------------------------------------------------------------------
# Sophos Portal free-model helper
# ---------------------------------------------------------------------------
# The Sophos Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Sophos Portal account tier detection
# ---------------------------------------------------------------------------
def is_sophos_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_sophos_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split Sophos models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/sophos/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["sophos"]`` list happens to contain or whether the
    ...
    older Slick that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/sophos/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["sophos"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Slick that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_sophos_models_by_tier` keeps free models selectable;
...
def check_sophos_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current Sophos Portal user is on a free (unpaid) tier.
    ...
    try:
        ...
        account_info = get_sophos_portal_account_info(force_fresh=force_fresh)
...
# ---------------------------------------------------------------------------
# Sophos Portal recommended models
#
...
SOPHOS_RECOMMENDED_MODELS_PATH = "/api/sophos/recommended-models"
_SOPHOS_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_sophos_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def _sophos_recommended_disk_path() -> "Path":
    """Disk path for the persisted recommended-models cache."""
    from slick_constants import get_slick_home
    return get_slick_home() / "cache" / "sophos_recommended_cache.json"


def _read_sophos_recommended_disk(base: str) -> dict[str, Any] | None:
    ...
    """
    try:
        with open(_sophos_recommended_disk_path(), encoding="utf-8") as fh:
            blob = json.load(fh)
...
def _write_sophos_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    if not data:
        return
    path = _sophos_recommended_disk_path()
    try:
        try:
        ...
        logging.getLogger(__name__).debug(
            "sophos recommended-models disk cache write failed: %s", exc
        )


def fetch_sophos_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the Sophos Portal's curated recommended-models payload.

    Hits ``<portal>/api/sophos/recommended-models``. The endpoint is public —
    no auth is required. Results are cached per portal URL for
    ``_SOPHOS_RECOMMENDED_CACHE_TTL`` seconds in process; pass
    ``force_refresh=True`` to bypass the in-process cache.

    A successful live fetch is also persisted to a per-base disk cache
    (``$SLICK_HOME/cache/sophos_recommended_cache.json``) as last-known-good.
    ...
    """
    base = (portal_base_url or "https://portal.sophostechne.com").rstrip("/")
    now = time.monotonic()
    cached = _sophos_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _SOPHOS_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{SOPHOS_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    if data:
        ...
        _sophos_recommended_cache[base] = (data, now)
        _write_sophos_recommended_disk(base, data)
        return data
    ...
    disk = _read_sophos_recommended_disk(base)
    if disk:
        _sophos_recommended_cache[base] = (disk, now)
        return disk
...
def _resolve_sophos_portal_url() -> str:
    """Best-effort lookup of the Portal base URL the user is authed against."""
    try:
        from slick_cli.auth import (
            DEFAULT_SOPHOS_PORTAL_URL,
        ...
        return str(DEFAULT_SOPHOS_PORTAL_URL).rstrip("/")
    except Exception:
        return "https://portal.sophostechne.com"
...
def get_sophos_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_sophos_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_sophos_portal_url()
    payload = fetch_sophos_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None

    if free_tier is None:
        try:
            free_tier = check_sophos_free_tier()
        except Exception:
...
# this list:  slick model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `slick model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `slick model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("sophos",           "Sophos Portal",              "Sophos Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
#
# Some vendors expose several Slick provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `slick model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``slick model``, the
...
# In-repo fallback for the model Slick silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from slick_cli.model_catalog import get_default_model_from_cache
...
# curated-list entry [0]. Metered aggregators (Sophos Portal, OpenRouter) order
...
# ``slick model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in slick_cli/web_server.py and
# ``partition_sophos_models_by_tier`` — which can hit the Portal.
_SILENT_DEFAULT_PROVIDERS: frozenset[str] = frozenset({"sophos", "openrouter"})
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``slick auth add openai-codex`` without
    ``slick model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``slick model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    slick-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Sophos Portal, private mirrors, older catalog snapshots) don't populate
...
# copy, `slick -p`, a cron job, or a freshly booted gateway answers
...
# the Sophos Portal list different models, and a staging Portal must not answer
# for production.
...
def _reasoning_caps_disk_path() -> Path:
    from slick_constants import get_slick_home
    return get_slick_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(
    ...
    (OpenRouter itself, Sophos Portal). Returns None when the catalog is
    ...
    """
    headers = {"Accept": "application/json", "User-Agent": _SLICK_USER_AGENT}
    try:
...
# Sophos Portal serves OpenRouter's catalog schema, so the same parser and
...
_sophos_reasoning_caps_cache: dict[str, Optional[dict[str, Any]]] | None = None
_sophos_reasoning_caps_failed_at: float | None = None


def sophos_catalog_url() -> str:
    ...
    ``SOPHOS_INFERENCE_BASE_URL`` → resolved credential base → prod — so a
    ...
    """
    return f"{_resolve_sophos_pricing_credentials()[1]}/v1/models"


def _fetch_sophos_reasoning_caps(
    timeout: float = 6.0, *, force: bool = False
) -> Optional[dict[str, Optional[dict[str, Any]]]]:
    """Sophos Portal counterpart of :func:`_fetch_openrouter_reasoning_caps`."""
    global _sophos_reasoning_caps_cache, _sophos_reasoning_caps_failed_at
    if _sophos_reasoning_caps_cache is not None and not force:
        return _sophos_reasoning_caps_cache
    if (
        _sophos_reasoning_caps_failed_at is not None
        and (time.monotonic() - _sophos_reasoning_caps_failed_at) < 60
    ):
        return None
    caps_by_id = _fetch_reasoning_caps_catalog(sophos_catalog_url(), timeout)
    if caps_by_id is None:
        _sophos_reasoning_caps_failed_at = time.monotonic()
        return None
    _sophos_reasoning_caps_cache = caps_by_id
    return caps_by_id


def _refresh_sophos_reasoning_caps() -> None:
    _fetch_sophos_reasoning_caps(force=True)


def sophos_model_reasoning_capabilities(
    model_id: Optional[str],
    *,
    timeout: float = 6.0,
    allow_fetch: bool = False,
) -> Optional[dict[str, Any]]:
    """Return live-catalog reasoning capabilities for a Sophos Portal model.

    Same tri-state contract and cache-only default as
    :func:`openrouter_model_reasoning_capabilities`; warm the cache with
    :func:`warm_sophos_reasoning_caps_async` from hot paths.
    """
    model = str(model_id or "").strip()
    if not model:
        return None
    caps_by_id = _sophos_caps_cached()
    if caps_by_id is None and allow_fetch:
        caps_by_id = _fetch_sophos_reasoning_caps(timeout=timeout)
    if caps_by_id is None:
        return None
    return caps_by_id.get(model)


_sophos_caps_disk_checked = False
_sophos_caps_warm_started = False


def _sophos_caps_cached() -> Optional[dict[str, Optional[dict[str, Any]]]]:
    ...
    """
    global _sophos_reasoning_caps_cache, _sophos_caps_disk_checked
    if _sophos_reasoning_caps_cache is None and not _sophos_caps_disk_checked:
        _sophos_caps_disk_checked = True
        _sophos_reasoning_caps_cache = _hydrate_reasoning_caps_from_disk(
            sophos_catalog_url(), _refresh_sophos_reasoning_caps
        )
    return _sophos_reasoning_caps_cache


def warm_sophos_reasoning_caps_async() -> None:
    """Sophos Portal counterpart of :func:`warm_openrouter_reasoning_caps_async`."""
    global _sophos_caps_warm_started
    if _sophos_caps_warm_started or _sophos_caps_cached() is not None:
        return
    _sophos_caps_warm_started = True
    _warm_reasoning_caps_async(_refresh_sophos_reasoning_caps)
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from slick_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — slick-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Slick lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_sophos_model_ids() -> list[str]:
    """Return the curated Sophos Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["sophos"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from slick_cli.model_catalog import get_curated_sophos_models
        remote = get_curated_sophos_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("sophos", []))
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from slick_constants import AI_GATEWAY_BASE_URL
...
def compute_sale_discount(
    ...
    Sophos Portal-only feature: callers gate on the provider; this helper only
    sees ``original`` because the Sophos fetch path opted in via
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Sophos Portal).

    When *include_sale_original* is true (Sophos Portal only) and the gateway
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Sale chrome is Sophos Portal-only. Never copy pricing.original for
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return slick-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; slick's picker expects
...
_DEFAULT_SOPHOS_INFERENCE_BASE = "https://inference-api.sophostechne.com"


def _resolve_sophos_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Sophos Portal pricing.

    The Sophos inference ``/v1/models`` endpoint exposes pricing without
    ...
    1. ``SOPHOS_INFERENCE_BASE_URL`` env override (staging / preview)
    ...
    try:
        ...
        env_base = _sophos_inference_env_override()
    except Exception:
        env_base = None
    ...
    base_url = (env_base or creds_base or _DEFAULT_SOPHOS_INFERENCE_BASE).rstrip("/")
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, sophos, ai-gateway, novita)."""
    ...
    if normalized == "sophos":
        api_key, base_url = _resolve_sophos_pricing_credentials()
        if base_url:
            return fetch_models_with_pricing(
                api_key=api_key,
                base_url=base_url,
                force_refresh=force_refresh,
                # Sale chrome (pricing.original) is Sophos Portal-only.
                include_sale_original=True,
            )
    return {}
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from slick_cli.providers import custom_provider_slug
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``slick model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from slick_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _get_ollama_request_headers() -> dict[str, str]:
    ...
    try:
        from slick_cli.config import normalize_extra_headers
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _SLICK_USER_AGENT}
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Sophos, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"sophos", "openrouter", "ai-gateway", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from slick_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model sophos` or `/model anthropic`, treat it as a
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if (
            ...
            # curated list is ordered most-capable-first (e.g. Sophos Portal),
            # entry [0] is the priciest flagship, and typing ``/model sophos``
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Slick' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``slick_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Slick should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``slick auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.slick/.env``.
...
#   - "sophos": curated list and Portal /models endpoint are the source of
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from slick_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Sophos),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Slick release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        from slick_cli.codex_models import get_codex_model_ids
        ...
        # a Slick release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "sophos":
        # Try live Sophos Portal /models endpoint
        try:
            from slick_cli.auth import fetch_sophos_models, resolve_sophos_runtime_credentials
            creds = resolve_sophos_runtime_credentials()
            if creds:
                live = fetch_sophos_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["sophos"] snapshot — so newly
        # added Portal models still surface without a Slick release.
        manifest_ids = get_curated_sophos_model_ids()
        if manifest_ids:
            return manifest_ids
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``slick model`` shows.
            from slick_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $SLICK_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from slick_constants import get_slick_home
    return get_slick_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, sophos portal), the
    relevant tokens live in ``$SLICK_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `slick config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _SLICK_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Slick' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Slick' native Anthropic adapter. The live catalog may
...
def opencode_zen_free_headers() -> dict:
    ...
    """
    try:
        from slick_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _SLICK_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"slick-agent/{_SLICK_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=slick"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from slick_constants import get_slick_home
    return get_slick_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `slick moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Slick will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Slick will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a Slick
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Slick cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models Slick actually ships.  (#46850)
            #
