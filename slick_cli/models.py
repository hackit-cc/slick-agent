...
# ---------------------------------------------------------------------------
# Hackit Portal free-model helper
# ---------------------------------------------------------------------------
# The Hackit Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Hackit Portal account tier detection
# ---------------------------------------------------------------------------
def is_hackit_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_hackit_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split Hackit models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/hackit/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["hackit"]`` list happens to contain or whether the
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/hackit/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["hackit"]`` list happens to contain or whether
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_hackit_models_by_tier` keeps free models selectable;
...
def check_hackit_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current Hackit Portal user is on a free (unpaid) tier.
    ...
    try:
        ...
        account_info = get_hackit_portal_account_info(force_fresh=force_fresh)
...
# ---------------------------------------------------------------------------
# Hackit Portal recommended models
#
...
HACKIT_RECOMMENDED_MODELS_PATH = "/api/hackit/recommended-models"
_HACKIT_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_hackit_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def _hackit_recommended_disk_path() -> "Path":
    """Disk path for the persisted recommended-models cache."""
    from slick_constants import get_slick_home
    return get_slick_home() / "cache" / "hackit_recommended_cache.json"


def _read_hackit_recommended_disk(base: str) -> dict[str, Any] | None:
    ...
    """
    try:
        with open(_hackit_recommended_disk_path(), encoding="utf-8") as fh:
            blob = json.load(fh)
...
def _write_hackit_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    if not data:
        return
    path = _hackit_recommended_disk_path()
    try:
        try:
        ...
        logging.getLogger(__name__).debug(
            "hackit recommended-models disk cache write failed: %s", exc
        )


def fetch_hackit_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the Hackit Portal's curated recommended-models payload.

    Hits ``<portal>/api/hackit/recommended-models``. The endpoint is public —
    no auth is required. Results are cached per portal URL for
    ``_HACKIT_RECOMMENDED_CACHE_TTL`` seconds in process; pass
    ``force_refresh=True`` to bypass the in-process cache.

    A successful live fetch is also persisted to a per-base disk cache
    (``$SLICK_HOME/cache/hackit_recommended_cache.json``) as last-known-good.
    ...
    """
    base = (portal_base_url or "https://portal.hackit.cc").rstrip("/")
    now = time.monotonic()
    cached = _hackit_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _HACKIT_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{HACKIT_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    if data:
        ...
        _hackit_recommended_cache[base] = (data, now)
        _write_hackit_recommended_disk(base, data)
        return data
    ...
    disk = _read_hackit_recommended_disk(base)
    if disk:
        _hackit_recommended_cache[base] = (disk, now)
        return disk
...
def _resolve_hackit_portal_url() -> str:
    ...
    try:
        ...
        return str(DEFAULT_HACKIT_PORTAL_URL).rstrip("/")
    except Exception:
        return "https://portal.hackit.cc"
...
def get_hackit_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_hackit_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_hackit_portal_url()
    payload = fetch_hackit_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None

    if free_tier is None:
        try:
            free_tier = check_hackit_free_tier()
        except Exception:
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("hackit",           "Hackit Portal",              "Hackit Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
# curated-list entry [0]. Metered aggregators (Hackit Portal, OpenRouter) order
...
# ``partition_hackit_models_by_tier`` — which can hit the Portal.
_SILENT_DEFAULT_PROVIDERS: frozenset[str] = frozenset({"hackit", "openrouter"})
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Hackit Portal, private mirrors, older catalog snapshots) don't populate
...
# the Hackit Portal list different models, and a staging Portal must not answer
# for production.
...
def _fetch_reasoning_caps_catalog(
    ...
    (OpenRouter itself, Hackit Portal). Returns None when the catalog is
...
# Hackit Portal serves OpenRouter's catalog schema, so the same parser and
...
_hackit_reasoning_caps_cache: dict[str, Optional[dict[str, Any]]] | None = None
_hackit_reasoning_caps_failed_at: float | None = None


def hackit_catalog_url() -> str:
    ...
    ``HACKIT_INFERENCE_BASE_URL`` → resolved credential base → prod — so a
    ...
    """
    return f"{_resolve_hackit_pricing_credentials()[1]}/v1/models"


def _fetch_hackit_reasoning_caps(
    timeout: float = 6.0, *, force: bool = False
) -> Optional[dict[str, Optional[dict[str, Any]]]]:
    """Hackit Portal counterpart of :func:`_fetch_openrouter_reasoning_caps`."""
    global _hackit_reasoning_caps_cache, _hackit_reasoning_caps_failed_at
    if _hackit_reasoning_caps_cache is not None and not force:
        return _hackit_reasoning_caps_cache
    if (
        _hackit_reasoning_caps_failed_at is not None
        and (time.monotonic() - _hackit_reasoning_caps_failed_at) < 60
    ):
        return None
    caps_by_id = _fetch_reasoning_caps_catalog(hackit_catalog_url(), timeout)
    if caps_by_id is None:
        _hackit_reasoning_caps_failed_at = time.monotonic()
        return None
    _hackit_reasoning_caps_cache = caps_by_id
    return caps_by_id


def _refresh_hackit_reasoning_caps() -> None:
    _fetch_hackit_reasoning_caps(force=True)


def hackit_model_reasoning_capabilities(
    model_id: Optional[str],
    *,
    timeout: float = 6.0,
    allow_fetch: bool = False,
) -> Optional[dict[str, Any]]:
    """Return live-catalog reasoning capabilities for a Hackit Portal model.

    Same tri-state contract and cache-only default as
    :func:`openrouter_model_reasoning_capabilities`; warm the cache with
    :func:`warm_hackit_reasoning_caps_async` from hot paths.
    """
    model = str(model_id or "").strip()
    if not model:
        return None
    caps_by_id = _hackit_caps_cached()
    if caps_by_id is None and allow_fetch:
        caps_by_id = _fetch_hackit_reasoning_caps(timeout=timeout)
    if caps_by_id is None:
        return None
    return caps_by_id.get(model)


_hackit_caps_disk_checked = False
_hackit_caps_warm_started = False


def _hackit_caps_cached() -> Optional[dict[str, Optional[dict[str, Any]]]]:
    ...
    """
    global _hackit_reasoning_caps_cache, _hackit_caps_disk_checked
    if _hackit_reasoning_caps_cache is None and not _hackit_caps_disk_checked:
        _hackit_caps_disk_checked = True
        _hackit_reasoning_caps_cache = _hydrate_reasoning_caps_from_disk(
            hackit_catalog_url(), _refresh_hackit_reasoning_caps
        )
    return _hackit_reasoning_caps_cache


def warm_hackit_reasoning_caps_async() -> None:
    """Hackit Portal counterpart of :func:`warm_openrouter_reasoning_caps_async`."""
    global _hackit_caps_warm_started
    if _hackit_caps_warm_started or _hackit_caps_cached() is not None:
        return
    _hackit_caps_warm_started = True
    _warm_reasoning_caps_async(_refresh_hackit_reasoning_caps)
...
def get_curated_hackit_model_ids() -> list[str]:
    """Return the curated Hackit Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["hackit"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from slick_cli.model_catalog import get_curated_hackit_models
        remote = get_curated_hackit_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("hackit", []))
...
def compute_sale_discount(
    ...
    Hackit Portal-only feature: callers gate on the provider; this helper only
    sees ``original`` because the Hackit fetch path opted in via
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Hackit Portal).

    When *include_sale_original* is true (Hackit Portal only) and the gateway
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Sale chrome is Hackit Portal-only. Never copy pricing.original for
...
_DEFAULT_HACKIT_INFERENCE_BASE = "https://inference-api.hackit.cc"


def _resolve_hackit_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Hackit Portal pricing.

    The Hackit inference ``/v1/models`` endpoint exposes pricing without
    ...
    1. ``HACKIT_INFERENCE_BASE_URL`` env override (staging / preview)
    ...
    try:
        ...
        env_base = _hackit_inference_env_override()
    except Exception:
        env_base = None
    ...
    base_url = (env_base or creds_base or _DEFAULT_HACKIT_INFERENCE_BASE).rstrip("/")
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, hackit, ai-gateway, novita)."""
    ...
    if normalized == "hackit":
        api_key, base_url = _resolve_hackit_pricing_credentials()
        if base_url:
            return fetch_models_with_pricing(
                api_key=api_key,
                base_url=base_url,
                force_refresh=force_refresh,
                # Sale chrome (pricing.original) is Hackit Portal-only.
                include_sale_original=True,
            )
    return {}
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Hackit, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"hackit", "openrouter", "ai-gateway", "copilot", "kilocode"}
)
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model hackit` or `/model anthropic`, treat it as a
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if (
            ...
            # curated list is ordered most-capable-first (e.g. Hackit Portal),
            # entry [0] is the priciest flagship, and typing ``/model hackit``
...
#   - "hackit": curated list and Portal /models endpoint are the source of
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Hackit),
    ...
    if normalized == "hackit":
        # Try live Hackit Portal /models endpoint
        try:
            from slick_cli.auth import fetch_hackit_models, resolve_hackit_runtime_credentials
            creds = resolve_hackit_runtime_credentials()
            if creds:
                live = fetch_hackit_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["hackit"] snapshot — so newly
        # added Portal models still surface without a Slick release.
        manifest_ids = get_curated_hackit_model_ids()
        if manifest_ids:
            return manifest_ids
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, hackit portal), the
