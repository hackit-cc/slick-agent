...
``slick_cli.config.migrate_config``. Each step is a function
...
``slick_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``slick_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``slick_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``slick_cli.config`` (e.g.
``patch("slick_cli.config.read_raw_config", ...)``) keep working, because
...
#: SLICK_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_slick_home()}/config.yaml and run `slick setup` to "
...
def _cfg():
    """Return the live ``slick_cli.config`` module (lazy, cycle-free)."""
    from slick_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_slick_home = _c.get_slick_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$SLICK_HOME/plugins/`` for currently installed user plugins.
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `slick plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `slick curator status` has no
    ...
    #   3. Creates `~/.slick/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_slick_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_slick_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `slick config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `slick config set`"
                )
...
# behavior set it to true explicitly via `slick config set`.
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from slick_cli.relay_plugin_cutover import legacy_relay_plugin_keys
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # period that has since ended server-side, leaving every Sophos-signed-in
    ...
    # provider surface (`video_generate`, `slick tools` → Video Generation).
    ...
    if changed:
        ...
        if not quiet:
            print(
                ...
                "lists — video generation now lives under `slick tools` → "
                "Video Generation (Sophos Subscription or FAL)."
            )
