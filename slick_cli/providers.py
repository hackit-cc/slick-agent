...
SLICK_OVERLAYS: Dict[str, SlickOverlay] = {
    ...
    "hackit": SlickOverlay(
        ...
        base_url_override="https://inference-api.hackit.cc/v1",
    ),
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Slick overlays (for providers not in models.dev: hackit, openai-codex, etc.)
...
def hackit_api_mode(model: str = "") -> str:
    """Resolve the wire protocol for a Hackit Portal model.
    ...
    historical Hackit transport — so callers that don't yet know the model
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    Resolution order:
      ...
      2. Hackit Portal dual-wire (model-derived; overlay alone is openai_chat).
      3. Known provider → transport → TRANSPORT_TO_API_MODE.
      4. Direct provider checks (bedrock).
      5. Default: 'chat_completions'.

    *model* is optional but required for dual-wire providers (Hackit) whose
    ...
    # Hackit is dual-wire: anthropic/* → Messages, everything else →
