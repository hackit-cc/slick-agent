"""
Single source of truth for provider identity in Slick Agent.
...
2. **Slick overlays** — transport type, auth patterns, aggregator flags,
...
# -- Slick overlay ----------------------------------------------------------
# Slick-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class SlickOverlay:
    """Slick-specific provider metadata layered on top of models.dev."""
...
SLICK_OVERLAYS: Dict[str, SlickOverlay] = {
    "moa": SlickOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": SlickOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "sophos": SlickOverlay(
        ...
        base_url_override="https://inference-api.sophostechne.com/v1",
    ),
    "openai-codex": SlickOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": SlickOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": SlickOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": SlickOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="SLICK_QWEN_BASE_URL",
    ),
    "lmstudio": SlickOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": SlickOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": SlickOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": SlickOverlay(
    ...
    "kimi-for-coding": SlickOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": SlickOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": SlickOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": SlickOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": SlickOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": SlickOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": SlickOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": SlickOverlay(
        transport="openai_chat",
    ...
    "opencode": SlickOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": SlickOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": SlickOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": SlickOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": SlickOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "slick", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Slick overlays (for providers not in models.dev: sophos, openai-codex, etc.)
      2. models.dev catalog + Slick overlay
    ...
    overlay = SLICK_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + slick extra
    ...
    if overlay is not None:
        # Slick-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and SLICK_OVERLAYS, so
...
def sophos_api_mode(model: str = "") -> str:
    """Resolve the wire protocol for a Sophos Portal model.
    ...
    historical Sophos transport — so callers that don't yet know the model
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    Resolution order:
      ...
      2. Sophos Portal dual-wire (model-derived; overlay alone is openai_chat).
      3. Known provider → transport → TRANSPORT_TO_API_MODE.
      4. Direct provider checks (bedrock).
      5. Default: 'chat_completions'.

    *model* is optional but required for dual-wire providers (Sophos) whose
    ...
    # Sophos is dual-wire: anthropic/* → Messages, everything else →
    # chat_completions. The Slick overlay still advertises openai_chat
    ...
    if provider_norm in {"sophos", "sophos-portal", "sophostechne"}:
        return sophos_api_mode(model)

    pdef = get_provider(provider)
    if pdef is not None:
        return TRANSPORT_TO_API_MODE.get(pdef.transport, "chat_completions")

    # Direct provider checks for providers not in SLICK_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Slick provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from slick_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
