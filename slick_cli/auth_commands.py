...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "sophos", "openai-codex", "xai-oauth", "qwen-oauth", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if provider == "sophos":
        # Codex-style auto-import: if a shared Sophos credential lives at
        # <slick-root>/shared/sophos_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `slick --profile <name>
        # auth add sophos --type oauth` a one-tap operation for users who
        # run multiple profiles.
        shared = auth_mod._read_shared_sophos_state()
        if shared:
            try:
                path = auth_mod._sophos_shared_store_path()
            except RuntimeError:
                path = None
            print()
            if path:
                print(f"Found existing Sophos OAuth credentials at {path}")
            else:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                rehydrated = auth_mod._try_import_shared_sophos_state(
                    timeout_seconds=getattr(args, "timeout", None) or 15.0,
                )
                if rehydrated is not None:
                    custom_label = (getattr(args, "label", None) or "").strip() or None
                    entry = auth_mod.persist_sophos_credentials(rehydrated, label=custom_label)
        ...
        creds = auth_mod._sophos_device_code_login(
        ...
        # Honor `--label <name>` so sophos matches other providers' UX.  The
        # helper embeds this into providers.sophos so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("sophos").
        custom_label = (getattr(args, "label", None) or "").strip() or None
        entry = auth_mod.persist_sophos_credentials(creds, label=custom_label)
        shown_label = entry.label if entry is not None else label_from_token(
