...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "hackit", "openai-codex", "xai-oauth", "qwen-oauth", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_slick_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:slick_pkce",
    ...
    if provider == "hackit":
        # Codex-style auto-import: if a shared Hackit credential lives at
        # <slick-root>/shared/hackit_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `slick --profile <name>
        # auth add hackit --type oauth` a one-tap operation for users who
        # run multiple profiles.
        shared = auth_mod._read_shared_hackit_state()
        if shared:
            try:
                path = auth_mod._hackit_shared_store_path()
            except RuntimeError:
                path = None
            print()
            if path:
                print(f"Found existing Hackit OAuth credentials at {path}")
            else:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                rehydrated = auth_mod._try_import_shared_hackit_state(
                    timeout_seconds=getattr(args, "timeout", None) or 15.0,
                )
                if rehydrated is not None:
                    custom_label = (getattr(args, "label", None) or "").strip() or None
                    entry = auth_mod.persist_hackit_credentials(rehydrated, label=custom_label)
        ...
        creds = auth_mod._hackit_device_code_login(
        ...
        # Honor `--label <name>` so hackit matches other providers' UX.  The
        # helper embeds this into providers.hackit so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("hackit").
        custom_label = (getattr(args, "label", None) or "").strip() or None
        entry = auth_mod.persist_hackit_credentials(creds, label=custom_label)
        shown_label = entry.label if entry is not None else label_from_token(
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``slick auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``slick auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`slick auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Slick reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `slick auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `slick auth` is called bare."""
