"""
Status command for slick CLI.

Shows the status of all Slick Agent components.
"""
...
from slick_cli.auth import AuthError, resolve_provider
from slick_cli.colors import Colors, color
from slick_cli.config import get_env_path, get_env_value, get_slick_home, load_config
from slick_cli.models import provider_label
...
from slick_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``slick config``'s
...
def _estop_status_line():
    """One-line pause banner for `slick status`, or None when not paused.

    Cheap: a single stat on $SLICK_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `slick resume` to lift)"


def show_status(args):
    """Show status of all Slick Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 😎 Slick Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `slick status`
    ...
    hackit_account_info = None
    if (
        hackit_status.get("logged_in")
        or hackit_status.get("access_token")
        or hackit_status.get("portal_base_url")
        or hackit_status.get("inference_credential_present")
        or hackit_status.get("error_code")
    ):
        try:
            hackit_account_info = get_hackit_portal_account_info()
        except Exception:
            hackit_account_info = None

    hackit_logged_in = bool(
        hackit_status.get("logged_in")
        or (hackit_account_info and hackit_account_info.logged_in)
    )
    hackit_inference_present = bool(
        hackit_status.get("inference_credential_present")
        or (hackit_account_info and hackit_account_info.inference_credential_present)
    )
    hackit_error = hackit_status.get("error")
    if hackit_logged_in:
        hackit_label = "logged in"
    elif hackit_inference_present:
        hackit_label = "not logged in (Hackit inference key configured)"
    else:
        hackit_label = "not logged in (run: slick portal)"
    print(
        f"  {'Hackit Portal':<12}  {check_mark(hackit_logged_in)} "
        f"{hackit_label}"
    )
    portal_url = hackit_status.get("portal_base_url") or "(unknown)"
    inference_url = (
        hackit_status.get("inference_base_url")
        or (hackit_account_info.inference_base_url if hackit_account_info else None)
    )
    access_exp = _format_iso_timestamp(hackit_status.get("access_expires_at"))
    key_exp = _format_iso_timestamp(hackit_status.get("agent_key_expires_at"))
    refresh_label = "yes" if hackit_status.get("has_refresh_token") else "no"
    if hackit_logged_in or portal_url != "(unknown)" or hackit_error:
        print(f"    Portal URL: {portal_url}")
    if hackit_inference_present and inference_url:
        print(f"    Inference:  {inference_url}")
    if hackit_logged_in or hackit_status.get("access_expires_at"):
        print(f"    Access exp: {access_exp}")
    if hackit_logged_in or hackit_inference_present or hackit_status.get("agent_key_expires_at"):
        print(f"    Key exp:    {key_exp}")
    if hackit_logged_in or hackit_status.get("has_refresh_token"):
        print(f"    Refresh:    {refresh_label}")
    if hackit_error:
        print(f"    Error:      {hackit_error}")

    codex_logged_in = bool(codex_status.get("logged_in"))
    print(
        f"  {'OpenAI Codex':<12}  {check_mark(codex_logged_in)} "
        f"{'logged in' if codex_logged_in else 'not logged in (run: slick model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: slick auth add minimax-oauth)'}"
    )
    ...
    # disrupt the already-printed Hackit/Codex/Qwen/MiniMax rows above.
    try:
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: slick auth add xai-oauth)'}"
    )
    ...
    # =========================================================================
    # Hackit Subscription Features
    # =========================================================================
    if managed_hackit_tools_enabled():
        features = get_hackit_subscription_features(config)
        print()
        print(color("◆ Hackit Tool Gateway", Colors.CYAN, Colors.BOLD))
        if not features.hackit_auth_present:
            print("  Hackit Portal   ✗ not logged in")
        else:
            print("  Hackit Portal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_hackit:
                state = "active via Hackit subscription"
            elif feature.active:
                current = feature.current_provider or "configured provider"
                state = f"active via {current}"
            elif feature.included_by_default and features.hackit_auth_present:
                state = "included by subscription, not currently selected"
            elif feature.key == "modal" and features.hackit_auth_present:
                state = "available via subscription (optional)"
            else:
                state = "not configured"
            print(f"  {feature.label:<15} {check_mark(feature.available or feature.active or feature.managed_by_hackit)} {state}")
    elif hackit_logged_in or hackit_inference_present:
        # Hackit OAuth without entitlement, or an opaque inference key without
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: slick model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from slick_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'slick-agent[vercel]')"
        ...
        # provider's doctor rows (fail-soft — never break `slick status`).
        try:
    ...
    try:
        from slick_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   slick gateway")
    ...
    jobs_file = get_slick_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_slick_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'slick doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'slick setup' to configure", Colors.DIM))
    print()
