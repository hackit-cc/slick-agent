...
def show_status(args):
    ...
    sophos_account_info = None
    if (
        sophos_status.get("logged_in")
        or sophos_status.get("access_token")
        or sophos_status.get("portal_base_url")
        or sophos_status.get("inference_credential_present")
        or sophos_status.get("error_code")
    ):
        try:
            sophos_account_info = get_sophos_portal_account_info()
        except Exception:
            sophos_account_info = None

    sophos_logged_in = bool(
        sophos_status.get("logged_in")
        or (sophos_account_info and sophos_account_info.logged_in)
    )
    sophos_inference_present = bool(
        sophos_status.get("inference_credential_present")
        or (sophos_account_info and sophos_account_info.inference_credential_present)
    )
    sophos_error = sophos_status.get("error")
    if sophos_logged_in:
        sophos_label = "logged in"
    elif sophos_inference_present:
        sophos_label = "not logged in (Sophos inference key configured)"
    else:
        sophos_label = "not logged in (run: slick portal)"
    print(
        f"  {'Sophos Portal':<12}  {check_mark(sophos_logged_in)} "
        f"{sophos_label}"
    )
    portal_url = sophos_status.get("portal_base_url") or "(unknown)"
    inference_url = (
        sophos_status.get("inference_base_url")
        or (sophos_account_info.inference_base_url if sophos_account_info else None)
    )
    access_exp = _format_iso_timestamp(sophos_status.get("access_expires_at"))
    key_exp = _format_iso_timestamp(sophos_status.get("agent_key_expires_at"))
    refresh_label = "yes" if sophos_status.get("has_refresh_token") else "no"
    if sophos_logged_in or portal_url != "(unknown)" or sophos_error:
        print(f"    Portal URL: {portal_url}")
    if sophos_inference_present and inference_url:
        print(f"    Inference:  {inference_url}")
    if sophos_logged_in or sophos_status.get("access_expires_at"):
        print(f"    Access exp: {access_exp}")
    if sophos_logged_in or sophos_inference_present or sophos_status.get("agent_key_expires_at"):
        print(f"    Key exp:    {key_exp}")
    if sophos_logged_in or sophos_status.get("has_refresh_token"):
        print(f"    Refresh:    {refresh_label}")
    if sophos_error:
        print(f"    Error:      {sophos_error}")
    ...
    # disrupt the already-printed Sophos/Codex/Qwen/MiniMax rows above.
    try:
    ...
    # =========================================================================
    # Sophos Subscription Features
    # =========================================================================
    if managed_sophos_tools_enabled():
        features = get_sophos_subscription_features(config)
        print()
        print(color("◆ Sophos Tool Gateway", Colors.CYAN, Colors.BOLD))
        if not features.sophos_auth_present:
            print("  Sophos Portal   ✗ not logged in")
        else:
            print("  Sophos Portal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_sophos:
                state = "active via Sophos subscription"
            elif feature.active:
                current = feature.current_provider or "configured provider"
                state = f"active via {current}"
            elif feature.included_by_default and features.sophos_auth_present:
                state = "included by subscription, not currently selected"
            elif feature.key == "modal" and features.sophos_auth_present:
                state = "available via subscription (optional)"
            else:
                state = "not configured"
            print(f"  {feature.label:<15} {check_mark(feature.available or feature.active or feature.managed_by_sophos)} {state}")
    elif sophos_logged_in or sophos_inference_present:
        # Sophos OAuth without entitlement, or an opaque inference key without
