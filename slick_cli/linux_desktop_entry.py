"""Install and remove the Linux desktop entry (``slick.desktop``).

``slick desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "slick.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``slick.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``slick desktop``.

    Prefer the real ``slick`` executable (argv[0] or PATH). When Slick
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from slick_cli.relaunch import resolve_slick_bin
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `slick` script with
            ...
            # sys.executable is the interpreter actually running Slick (the
            # venv one), so prefix it explicitly.
            argv = [str(Path(sys.executable).resolve()), str(resolved), "desktop"]
        else:
            argv = [str(resolved), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "slick_cli.main", "desktop"]
    return " ".join(_quote_exec_arg(a) for a in argv)


def _needs_interpreter(bin_path: Path) -> bool:
    """Whether ``bin_path`` is a Python script that must run under
    ``sys.executable`` to see Slick' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Slick\n"
        "GenericName=Slick Desktop\n"
        "Comment=Launch Slick Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Slick desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "slick"
