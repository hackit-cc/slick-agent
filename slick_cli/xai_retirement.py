...
Pure logic: walks a Slick config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `slick doctor` and a future `slick migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Slick config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Slick config and return retirement issues.
...
def apply_migration(
    ...
    from slick_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `slick migrate xai` reports nothing to migrate
