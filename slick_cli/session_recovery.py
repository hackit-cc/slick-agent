"""Offline, non-destructive recovery for a damaged Slick session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``slick_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``slick_state._backup_db_file``.
    """
    from slick_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Slick process using this profile and retry. "
                "This includes the interactive `slick` CLI session this "
                ...
                "Slick process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `slick` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from slick_cli.session_lost_and_found import (
