"""Default configuration data for Slick Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from slick_cli/config.py. Must not import from slick_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["slick-cli"],
    # SQLite journal mode used by every Slick database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Slick server processes.
    ...
    "session": {
        # Per-terminal `slick -c`: each CLI session drops a breadcrumb file
        # under $SLICK_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by slick_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `slick chat --run-budget N`.
        ...
        # an interactive `slick gateway restart` must never block for hours
        ...
        # the Slick-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Slick
        ...
        # identity slot (SOUL.md). Empty by default. The SLICK_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Slick adds a coding operating brief
        ...
        # `slick status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``SLICK_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # Root directory for Slick' terminal session temp files (background
        # logs/pid/exit files, code-execution sandboxes, etc.). When empty,
        # Slick uses TMPDIR/TMP/TEMP if set, otherwise a managed dir on real
        # storage at SLICK_HOME/cache/terminal (auto-pruned after 72h) — NOT
        ...
        # bot_relay from a short-lived `slick -p <bot> chat -Q` recipient
        ...
        #             SLICK_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force SLICK_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Slick auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Slick sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Slick
        ...
        # bundled Slick image, which drops to the `slick` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `slick tools` picker's
    ...
    "browser": {
        ...
        # browser tools) runs on a Slick-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by Slick' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.slick/browser-profile/) so copied
        ...
        # approval does it run `slick browser close-profile` (terminates the
        ...
        "camofox": {
            # When true, Slick sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Slick should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``slick chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.slick/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: slick sweeps the checkpoint base at startup
        ...
        # ``slick checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .slick.md, or .cursorrules before Slick applies
    ...
    # Single-query (``slick -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Slick truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Slick' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); slick =
                                      # Slick' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Slick' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Slick does.
    ...
    "auxiliary": {
        ...
        "vision": {
            "provider": "auto",    # auto | openrouter | hackit | codex | custom
        ...
        "review": {
            "provider": "auto",    # auto (= inherit main model) | openrouter | hackit | anthropic | ...
        ...
        # Invoked by ``slick kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``slick kanban decompose`` and the kanban
        ...
        # ``slick profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `slick model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `slick` (and `slick chat`) launches by default:
        ...
        # REPL and `--tui` (or SLICK_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `slick --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `slick -c` muscle memory.  Default off so existing
        # users aren't surprised.  SLICK_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `slick --tui` drops a one-time hint
        ...
        # Hackit credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # model — see slick_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to SLICK_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/slick`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `slick pets`. Disabled until
        ...
        "pet": {
            ...
            # get_slick_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see slick_cli/web_server.py — an event-loop stall
        ...
        # SLICK_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (SLICK_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``--insecure`` is not). The bundled Hackit Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``SLICK_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``SLICK_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.hackit.cc).
        "oauth": {
        ...
        # (``SLICK_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # hackit-account-service at deploy time via the
        # ``SLICK_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``SLICK_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/slick``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Slick appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.slick/cache/piper-voices/
    ...
    # "Hey Slick" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey slick",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_slick" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.slick/plugins/.
    "context": {
        "engine": "compressor",
    ...
    # configured providers (OpenRouter, Hackit, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.slick/cache/delegation/
    ...
    # response. If not, Slick feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Slick auto-pauses the goal and
    ...
    "moa": {
        ...
        # <slick_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.slick/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.slick/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``slick skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``slick skills trust`` / ``slick skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${SLICK_SKILL_DIR} and ${SLICK_SESSION_ID} in SKILL.md
        ...
        # (`slick skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.slick/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.slick/.curator_backups/blobs/. Enables `slick curator ledger`
        # and single-mutation `slick curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `slick curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `slick curator run --consolidate` overrides
        ...
        # normally restored on every `slick update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `slick curator purge` deletes
        ...
        # skipped), snapshot ~/.slick/skills/ into
        # ~/.slick/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `slick curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for slick-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "😎 *Slick Agent*" header.
    ...
    # dangerous command. -q runs export SLICK_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; SLICK_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Slick' built-in platform hint for a single messaging platform
    ...
    # ``slick plugins enable|disable`` and intentionally omitted here so an
    ...
    # stored approval from ~/.slick/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SLICK_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .slick config) always require human
        ...
        # longer trigger the startup banner. Add via `slick doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``slick_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Slick to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $SLICK_HOME/plugins/<name>/) to relocate the trigger — e.g. "webcron",
        ...
        "provider": "",
        # Webcron (NAS-mediated managed cron) settings. Only consulted when
        # provider == "webcron". All non-secret (URLs + the JWT audience): the
        ...
        # agent's existing Hackit Portal token — there is no token key here.
        "webcron": {
            ...
            "portal_url": "https://portal.hackit.cc",
            # The agent's OWN publicly-reachable base URL for NAS→agent fires
            # (NAS POSTs {callback_url}/api/cron/fire). Empty → Webcron is
        ...
        # Also overridable via SLICK_CRON_MAX_PARALLEL env var.
        ...
        # SLICK_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # SLICK_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via SLICK_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `slick -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `slick` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `slick kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     slick-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Slick tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.slick/logs/.
    ...
    # curated model lists for OpenRouter and Hackit Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a slick-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://slick-agent.hackit.cc/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `slick model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Slick provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal SLICK_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.slick/sessions/sessions.json entirely.
        ...
        # the SLICK_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.slick.gateway` that defenses 1-2 don't
        ...
        # `slick update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``SLICK_GATEWAY_MAX_STARTS`` / ``SLICK_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.slick/.env,
        ...
        # behavior: files must live under the Slick cache, under
        ...
        # user. Bridged to SLICK_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Slick
        # cache (~/.slick/cache/{documents,images,audio,video,screenshots})
        ...
        # to SLICK_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # SLICK_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to SLICK_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.slick/state.db.
    ...
    "sessions": {
        ...
        # the default of ``slick sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.slick/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `slick sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `slick update` prints a one-line notice with
        ...
        # ~/.slick/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to SLICK_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # SLICK_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `slick sessions export`. Checked
    ...
    # ``slick doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `slick doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``slick update`` behaviour.
    "updates": {
        ...
        #     DBs) into <SLICK_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``slick backup``-style zip
        #     of SLICK_HOME into <SLICK_HOME>/backups/, restorable with
        #     ``slick import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `slick update` does with uncommitted local changes to the
        ...
        # When `slick update` finds the source checkout parked on a feature
        ...
        #                         merge. `slick update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<SLICK_HOME>/lsp/bin/`` on first use.
    ...
    # `slick tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.slick/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.slick/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.slick/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Slick may use this encrypted
            ...
            # ~/.slick/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.slick/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Slick disables it for our users unless they
        # explicitly opt in here. When false (default), Slick sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Slick also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Slick computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Slick passes the
    ...
    #
    # Configure with `slick egress setup`.  Disabled by default — the rest of
    # Slick works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.slick/bin/ on
        ...
        # Together, DeepSeek, Hackit).  Wildcards (`*.foo.com`) are supported.
        "extra_allowed_hosts": [],
    },

    # Slick Desktop (Electron app) launch options. These only affect
    # `slick desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the SLICK_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the SLICK_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "HACKIT_BASE_URL": {
        "description": "Hackit Portal base URL override",
        "prompt": "Hackit Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Slick mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'slick model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Hackit Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Hackit Subscribers only, used to derive vendor hosts, e.g. hackit.cc -> firecrawl-gateway.hackit.cc",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Hackit Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Hackit Subscriber access token for tool-gateway requests (optional; otherwise read from the Slick auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Slick' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Slick, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @slick:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SLICK_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #slick)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: slick-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'slick-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Slick API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Slick API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Slick API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # SLICK_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to SLICK_TOOL_PROGRESS_MODE for backward
    ...
    # SLICK_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "SLICK_PREFILL_MESSAGES_FILE": {
