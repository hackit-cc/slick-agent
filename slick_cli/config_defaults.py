...
DEFAULT_CONFIG = {
    ...
    "auxiliary": {
        ...
        "vision": {
            "provider": "auto",    # auto | openrouter | sophos | codex | custom
        ...
        "review": {
            "provider": "auto",    # auto (= inherit main model) | openrouter | sophos | anthropic | ...
    ...
    "display": {
        ...
        # Sophos credits status-bar notices (usage bands, grant-spent, depleted /
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled Sophos Portal plugin reads
        ...
        # ``portal_url`` defaults to https://portal.sophostechne.com).
        "oauth": {
        ...
        # sophos-account-service at deploy time via the
    ...
    # configured providers (OpenRouter, Sophos, Z.ai, Kimi, etc.) are supported.
    "delegation": {
    ...
    "cron": {
        ...
        # $SLICK_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing Sophos Portal token — there is no token key here.
        "chronos": {
            ...
            "portal_url": "https://portal.sophostechne.com",
    ...
    # curated model lists for OpenRouter and Sophos Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a slick-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://slick-agent.sophostechne.com/docs/api/model-catalog.json",
    ...
    "proxy": {
        ...
        # Together, DeepSeek, Sophos).  Wildcards (`*.foo.com`) are supported.
        "extra_allowed_hosts": [],
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "SOPHOS_BASE_URL": {
        "description": "Sophos Portal base URL override",
        "prompt": "Sophos Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Sophos Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Sophos Subscribers only, used to derive vendor hosts, e.g. sophostechne.com -> firecrawl-gateway.sophostechne.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Sophos Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Sophos Subscriber access token for tool-gateway requests (optional; otherwise read from the Slick auth store)",
