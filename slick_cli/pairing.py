...
Usage:
    slick pairing list              # Show all pending + approved users
    slick pairing approve <platform> <request-id|code>  # Approve a pairing request
    slick pairing revoke <platform> <user_id> # Revoke user access
    slick pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle slick pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: slick pairing {list|approve|revoke|clear-pending}")
        print("Run 'slick pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: slick pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.slick/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'slick pairing list' to see pending requests.\n")
