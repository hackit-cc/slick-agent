...
Used by ``slick profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``slick_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Slick Agent kanban board.
...
Rules:
  ...
  - Never write "Slick Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_slick_home())
        else:
