"""
slick fallback — manage the fallback provider chain.
...
Subcommands:
  slick fallback [list]   Show the current fallback chain (default when no subcommand)
  slick fallback add      Pick provider + model via the same picker as `slick model`,
                           then append the selection to the chain
  slick fallback remove   Pick an entry to delete from the chain
  slick fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.slick/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from slick_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `slick model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  slick fallback add")
        print()
        return
    ...
    print("  Docs: https://slick-agent.sophostechne.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `slick model`, then append the selection to the chain."""
    from slick_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `slick model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `slick fallback list` to view, or `slick fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``slick fallback [subcommand]``."""
