...
def check_macos_tcc_grants() -> None:
    ...
    Post-#73681 builds pin ``designated => identifier "com.hackit.slick"``
    ...
    check_info(
        ...
        "grant is stale — run `tccutil reset ScreenCapture com.hackit.slick` "
...
def _desktop_app_bundle() -> Path | None:
    ...
    launcher (``com.hackit.slick.setup``, certificate-anchored), whose
...
def run_doctor(args):
    ...
    try:
        ...
        hackit_status = get_hackit_auth_status_local()
        if hackit_status.get("logged_in"):
            check_ok("Hackit Portal auth", "(logged in)")
        else:
            check_warn("Hackit Portal auth", "(not logged in)")
    ...
    # disrupt the already-printed Hackit/Codex/Gemini/MiniMax rows above.
    try:
        from slick_cli.auth import get_xai_oauth_auth_status
