...
One read-only pass that answers, BEFORE any mutation: what Slick runtimes
...
`detect_install_method`) — so `slick update --plan` can run on a live
...
Deployment kinds (the concept most fleet-update bugs were missing):
    ...
    git      — source checkout; updatable in place via `slick update`
...
Supervisors (how a runtime is restarted after code changes):
    ...
    desktop           — Desktop app supervises `slick serve`; it respawns
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Slick runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "slick update"
...
def _detect_supervisor_for_pid(
    ...
    if pid in service_pids:
        try:
            from slick_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    if profile != "default":
        return f"slick -p {profile} gateway restart"
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    try:
        ...
        try:
            from slick_cli.image_provenance import read_image_provenance
    ...
    try:
        from slick_cli.gateway import _get_service_pids
    ...
    try:
        from slick_cli.gateway import find_windows_gateway_services
    ...
    try:
        from slick_cli.gateway import find_profile_gateway_processes
    ...
    # launched `slick serve --host <ip>` for a remote Desktop, or a
    # long-lived `slick dashboard`. Every serve/dashboard registers itself
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Slick services: none detected — code swap only.")
        return
    ...
    try:
        ...
        for runtime in plan.runtimes:
            ...
            if r.profile in relaunched or r.profile in external:
                ...
                r.profile in unit or (r.profile == "default" and "slick-gateway" in unit)
                for unit in failed_set
            ):
                outcome = "failed"
            elif any(
                r.profile in svc or (r.profile == "default" and "slick-gateway" in svc)
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    print("      slick gateway restart                # active profile")
    print("      slick -p <profile> gateway restart   # named profile")
    return True
