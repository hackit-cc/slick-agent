#!/usr/bin/env python3
"""
Slick CLI - Main entry point.

Usage:
    slick                     # Interactive chat (default)
    slick chat                # Interactive chat
    slick gateway             # Run gateway in foreground
    slick gateway start       # Start gateway as service
    slick gateway stop        # Stop gateway service
    slick gateway status      # Show gateway status
    slick gateway install     # Install gateway service
    slick gateway uninstall   # Uninstall gateway service
    slick setup               # Interactive setup wizard
    slick logout              # Clear stored authentication
    slick status              # Show status of all components
    slick cron                # Manage cron jobs
    slick cron list           # List cron jobs
    slick cron status         # Check if cron scheduler is running
    slick doctor              # Check configuration and dependencies
    slick honcho setup                    # Configure Honcho AI memory integration
    slick honcho status                   # Show Honcho config and connection status
    slick honcho sessions                 # List directory → session name mappings
    slick honcho map <name>               # Map current directory to a session name
    slick honcho peer                     # Show peer names and dialectic settings
    slick honcho peer --user NAME         # Set user peer name
    slick honcho peer --ai NAME           # Set AI peer name
    slick honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    slick honcho mode                     # Show current memory mode
    slick honcho mode [hybrid|honcho|local]  # Set memory mode
    slick honcho tokens                   # Show token budget settings
    slick honcho tokens --context N       # Set session.context() token cap
    slick honcho tokens --dialectic N     # Set dialectic result char cap
    slick honcho identity                 # Show AI peer identity representation
    slick honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    slick honcho migrate                  # Step-by-step migration guide: OpenClaw native → Slick + Honcho
    slick --version           Show version and update status
    slick update              Update to latest version
    slick uninstall           Uninstall Slick Agent
    slick acp                 Run as an ACP server for editor integration
    slick sessions browse     Interactive session picker with search

    slick claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: slick_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``slick_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``slick update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``slick_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, slick crashes on import and the user can't run
# ``slick update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import slick_bootstrap  # noqa: F401
...
from slick_cli._subprocess_compat import suppress_platform_ver_console
from slick_cli.cli_output import line_input
...
# Two lines of inline path math so ``python slick_cli/main.py`` (script
# mode — sys.path[0] is slick_cli/, not the repo root) can import the
# canonical helpers; everything else lives in slick_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from slick_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``slick update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from slick_cli.env_loader import ...`` / ``from slick_cli.config import
...
# package directory, so if IT can't import, nothing else in slick_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from slick_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from slick_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Set the process title to 'slick' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``slick tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``slick.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("slick")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"slick", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"slick")
        # Windows: the .exe name is already ``slick.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* slick_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        if home:
            ...
            cfg_path = os.path.join(os.path.expanduser("~"), ".slick", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``SLICK_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``slick … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("SLICK_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``SLICK_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("SLICK_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`slick --tui … >log`, CI capture):
...
def _active_profile_may_override_home_fast(slick_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(slick_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``slick --version`` before config/logging imports."""
...
from slick_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from slick_cli.subcommands.cron import build_cron_parser
from slick_cli.subcommands.sync import build_sync_parser
from slick_cli.subcommands.gateway import build_gateway_parser
from slick_cli.subcommands.profile import build_profile_parser
from slick_cli.subcommands.model import build_model_parser
from slick_cli.subcommands.setup import build_setup_parser

from slick_cli.subcommands.whatsapp import build_whatsapp_parser
from slick_cli.subcommands.slack import build_slack_parser
from slick_cli.subcommands.login import build_login_parser
from slick_cli.subcommands.logout import build_logout_parser
from slick_cli.subcommands.auth import build_auth_parser
from slick_cli.subcommands.status import build_status_parser
from slick_cli.subcommands.pause import build_pause_parser
from slick_cli.subcommands.webhook import build_webhook_parser
from slick_cli.subcommands.hooks import build_hooks_parser
from slick_cli.subcommands.doctor import build_doctor_parser
from slick_cli.subcommands.verify import build_verify_parser
from slick_cli.subcommands.security import build_security_parser
from slick_cli.subcommands.approvals import build_approvals_parser
from slick_cli.subcommands.dump import build_dump_parser
from slick_cli.subcommands.debug import build_debug_parser
from slick_cli.subcommands.backup import build_backup_parser
from slick_cli.subcommands.import_cmd import build_import_cmd_parser
from slick_cli.subcommands.import_agent import build_import_agent_parser
from slick_cli.subcommands.config import build_config_parser
from slick_cli.subcommands.skin import build_skin_parser
from slick_cli.subcommands.console import build_console_parser
from slick_cli.subcommands.update import build_update_parser
from slick_cli.subcommands.uninstall import build_uninstall_parser
from slick_cli.subcommands.dashboard import build_dashboard_parser
from slick_cli.subcommands.gui import build_gui_parser
from slick_cli.subcommands.logs import build_logs_parser
from slick_cli.subcommands.prompt_size import build_prompt_size_parser
from slick_cli.subcommands.memory import build_memory_parser
from slick_cli.subcommands.acp import build_acp_parser
from slick_cli.subcommands.tools import build_tools_parser
from slick_cli.subcommands.insights import build_insights_parser
from slick_cli.subcommands.monitoring import build_monitoring_parser
from slick_cli.subcommands.skills import build_skills_parser
from slick_cli.subcommands.pairing import build_pairing_parser
from slick_cli.subcommands.plugins import build_plugins_parser
from slick_cli.subcommands.mcp import build_mcp_parser
from slick_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (slick tools, slick setup, slick model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'slick {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any slick module import.
#
# Many modules cache SLICK_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("SLICK_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.slick/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set SLICK_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `slick mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Slick' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo slick -p <name>` against the invoking user's home.
        ...
        candidate = home / ".slick" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`slick chat -p coder`), so keep scanning broadly.
    # The exception is command-argv passthrough regions such as `mcp add --args`.
    from slick_cli._parser import top_level_value_flag_sets
    ...
    # Mirrors slick_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If SLICK_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.slick/profiles/coder or
    # /opt/data/profiles/coder).  If SLICK_HOME points to the slick root
    # instead (e.g. systemd hardcodes SLICK_HOME=/root/.slick), we must
    # still read active_profile — the user may have switched profiles via
    # `slick profile use` and the gateway should honour that choice.
    # See issue #22502.
    slick_home_env = os.environ.get("SLICK_HOME", "")
    if profile_name is None and slick_home_env:
        if Path(slick_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the slick root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as SLICK_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``slick gateway run`` to mean
    # "the root SLICK_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("SLICK_S6_SUPERVISED_CHILD"):
        try:
            from slick_constants import get_default_slick_root

            active_path = get_default_slick_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set SLICK_HOME
    if profile_name is not None:
        try:
            from slick_cli.profiles import resolve_profile_env

            slick_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            slick_home = _resolve_sudo_user_profile_env(profile_name)
            if not slick_home:
            ...
            # A bug in profiles.py must NEVER prevent slick from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["SLICK_HOME"] = slick_home
...
# Windows launcher self-heal — the ``slick`` command users run is a COPY of
# the venv console script, staged into the managed binary dir (the default
# Slick root's ``bin``, next to the managed uv) by install.ps1. That dir
# lives OUTSIDE the git checkout precisely because an earlier layout staged
# the copies at ``<checkout>\bin``, where ``slick update``'s autostash
# (``git stash push --include-untracked``) swept them off disk; with the
# desktop updater's ``--keep-stash`` nothing restored them and ``slick``
...
# ``python -m slick_cli.main``. Costs a few stat calls when healthy; gates
# fail toward inaction so source checkouts are untouched. Sits AFTER the
# profile override on purpose — no slick module may be imported before
# profiles resolve. The launcher dir itself is per-machine (the helper
# anchors on the DEFAULT root, not SLICK_HOME), so profile sessions heal
# the same shared dir.
if sys.platform == "win32":
    try:
        from slick_cli import _install_repair as _install_repair_mod

        _install_repair_mod.ensure_windows_bin_launchers(_bootstrap_root)
    except Exception:
        pass

# Load .env from ~/.slick/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from slick_cli.config import get_slick_home
from slick_cli.env_loader import load_slick_dotenv
...
# Bridge security.redact_secrets from config.yaml → SLICK_REDACT_SECRETS env
# var BEFORE slick_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    # yaml.load — the SAME parse then serves slick_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from slick_cli.config import read_raw_config as _read_raw_early

    _cfg_path = get_slick_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from slick_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "SLICK_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["SLICK_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `slick` subcommands
...
try:
    from slick_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from slick_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if slick_constants not importable yet
...
from slick_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to slick_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (slick_cli.main._model_flow_*) keep resolving unchanged.
from slick_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_slick_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("SLICK_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("SLICK_TERMUX_PREFETCH_UPDATES") == "1"
...
def _relative_time(ts) -> str:
    ...
    in :mod:`slick_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from slick_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    from slick_cli.config import get_env_path, get_slick_home, load_config
    from slick_cli.auth import get_auth_status

    # Determine whether Slick itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from slick_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_default, dict):
            from slick_cli.config import split_model_config_default
        ...
        _model_name = ""
    _has_slick_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from slick_cli.auth import PROVIDER_REGISTRY
    ...
    # Check for Sophos Portal OAuth credentials
    auth_file = get_slick_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Slick has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Slick to use their tokens.
    if _has_slick_config:
        try:
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        from slick_cli.config import load_config
        from slick_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_browse_picker(sessions: list, session_db=None) -> Optional[str]:
    ...
    def _delete_session(session_id: str) -> bool:
        if session_db is None:
            return False
        try:
            sessions_dir = get_slick_home() / "sessions"
        except Exception:
            sessions_dir = None
        try:
            return bool(
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``slick -c`` from repo A continues repo A's last session rather than the
    ...
    db = None
    try:
        from slick_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, slick_bin
        cli_args: the original CLI arguments (everything after 'slick')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    slick_bin = container_info["slick_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo slick {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo slick {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'slick sessions list' to see available sessions, or "
            ...
            try:
                from slick_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _print_tui_exit_summary(
    ...
    print(f"  slick --tui --resume {target}")
    if title:
        print(f'  slick --tui -c "{title}"')
    print()
    ...
    *starts* is the set of workspace keys the launch install explicitly scopes
    ...
    while stack:
        ...
        if not isinstance(entry, dict):
            continue
        # Workspace symlink (e.g. node_modules/@slick/ink → ui-tui/packages/…):
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @slick/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@slick" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/slick-ink/package.json",
    "packages/slick-ink/index.js",
    "packages/slick-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``SLICK_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("SLICK_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``SLICK_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("SLICK_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    from slick_constants import get_slick_home

    slick_home = str(get_slick_home())
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "SLICK_HOME": slick_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(slick_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(slick_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if slick_cli_dir is None:
        slick_cli_dir = Path(__file__).parent
    bundled = slick_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``slick update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("SLICK_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Slick checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `slick update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Slick checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `slick --tui`\n"
        "If the checkout is still inconsistent, run `slick update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (SLICK_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("SLICK_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $SLICK_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Slick installed,
        # and would pick a system Node over the managed one when both exist.
        from slick_constants import find_node_executable

        path = find_node_executable(bin)
        if not path and bin == "node":
            try:
                from slick_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("SLICK_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with SLICK_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset SLICK_TUI_DIR (e.g. `unset SLICK_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # slick_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            from slick_constants import with_slick_node_path
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env=_npm_lifecycle_env(with_slick_node_path()),
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Slick-managed
            ...
            from slick_cli.npm_engine import maybe_repair_npm_engine
    ...
    if tui_dev:
        # Keep the local @slick/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @slick/ink resolves through
        # packages/slick-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "slick-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from slick_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("SLICK_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["SLICK_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("SLICK_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["SLICK_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("SLICK_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["SLICK_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["SLICK_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a slick process: propagate the profile-home contract via
    # the single factory; keep secrets (the TUI/agent needs provider creds).
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    try:
        from slick_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="slick-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["SLICK_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["SLICK_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["SLICK_MODEL"] = model
        env["SLICK_INFERENCE_MODEL"] = model
    if provider:
        env["SLICK_TUI_PROVIDER"] = provider
        env["SLICK_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["SLICK_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["SLICK_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["SLICK_TUI_SKILLS"] = value
    if query:
        env["SLICK_TUI_QUERY"] = query
    if image:
        env["SLICK_TUI_IMAGE"] = image
    if checkpoints:
        env["SLICK_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["SLICK_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["SLICK_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["SLICK_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["SLICK_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["SLICK_ACCEPT_HOOKS"] = "1"
    ...
    # SLICK_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from a full os.environ snapshot (via
    # build_subprocess_env), an exported/stale value
    # in the user's shell would otherwise make a plain `slick --tui` try to
    ...
    # still set SLICK_TUI_RESUME themselves.
    env.pop("SLICK_TUI_RESUME", None)
    if resume_session_id:
        env["SLICK_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `slick update` so
    ...
    if code == 42:
        from slick_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``SLICK_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``slick kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``slick kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("SLICK_KANBAN_BOARD"):
        return
    try:
        from slick_cli.kanban_db import get_current_board

        os.environ["SLICK_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.slick/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Slick — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Slick still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``slick … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'slick sessions list' to see available sessions.")
            sys.exit(1)
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        print(f"  (later: slick --resume {_imported_id})")
    ...
    try:
        from slick_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from slick_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'slick doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Slick isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  slick setup")
        print()

        from slick_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'slick setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from slick_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    #
    # FIRST RUN is the exception: with an empty ~/.slick/skills the banner
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from slick_cli.config import get_slick_home
            skills_dir = Path(get_slick_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import slick_cli.banner as _banner_mod
    ...
    if getattr(args, "yolo", False):
        os.environ["SLICK_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.slick/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["SLICK_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["SLICK_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["SLICK_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from slick_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from slick_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from slick_cli.config import get_env_value, save_env_value
    from slick_constants import find_node_executable, with_slick_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Slick?")
        print()
    ...
    # has no creds.json.  Every subsequent `slick gateway` then paid a 30s
    ...
    session_dir = get_slick_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: slick gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `slick gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  slick gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '😎 Slick Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  slick gateway")
        ...
        print()
        print("  Or install as a service: slick gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'slick whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``slick whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``slick_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from slick_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from slick_cli.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from slick_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from slick_cli.setup import run_setup_action_with_navigation
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``slick model``) and the setup wizard
    ...
    from slick_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from slick_cli.providers import (
    ...
    effective_provider = (
        config_provider or os.getenv("SLICK_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from slick_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'slick model' for "
                "available providers, or run 'slick doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in slick_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``slick model``
    ...
    # matching against slick_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
    ...
    elif selected_provider == "sophos":
        _model_flow_sophos(config, current_model, args=args)
    ...
    # a leftover OPENAI_BASE_URL in ~/.slick/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.slick/.env if the active provider is not 'custom'.
    ...
    """
    from slick_cli.config import get_env_value, save_env_value, load_config
...
#
# Slick uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `slick model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `slick model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`slick_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from slick_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from slick_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Slick only falls back to a lightweight backend (OpenRouter,")
        print("  Sophos Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``slick model`` flow, then route aux tasks
    to them here.
    """
    from slick_cli.config import load_config
    from slick_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from slick_cli.auth import _prompt_model_selection
    from slick_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from slick_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from slick_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from slick_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Slick URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from slick_cli.curses_ui import curses_radiolist
...
# downstream call sites read `slick_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `slick` invocation — including
# fast paths like `slick --version` and slash-command dispatch that never
...
# surface so argparse wiring and test monkeypatches on slick_cli.main.<name>
# keep resolving unchanged. Importing those modules eagerly costs ~50ms on
# every `slick` invocation, including fast paths like `slick --version`
...
_LAZY_COMMAND_EXPORTS = {
    ...
    "slick_cli.dashboard_procs": (
        "_detect_concurrent_slick_instances",
...
def _self():
    ...
    keeps monkeypatches on slick_cli.main.<name> working, exactly like a
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from slick_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``slick setup`` / ``slick model``.
    ...
    recover from a malformed paste without editing ``~/.slick/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from slick_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from slick_cli.config import save_env_value
    from slick_cli.secret_prompt import masked_secret_prompt
    ...
    from slick_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `slick setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from slick_constants import display_slick_home as _dhh_fn

            print(
                f"    Slick will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Slick CLI with a provider."""
    from slick_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from slick_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from slick_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from slick_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from slick_cli.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.slick/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `slick sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            print("\nNot logged into Sophos Portal — sync is inert.", file=sys.stderr)
        elif not status.get("sophos_admin"):
            print(
            ...
            print(
                "\nSync feature is off for this instance (set SLICK_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "SLICK_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        ...
        return 1
    if not identity.get("sophos_admin"):
        print(
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="slick sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="slick sync now")
...
def cmd_webhook(args):
    ...
    from slick_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``slick slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: slick slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `slick slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from slick_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from slick_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from slick_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from slick_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from slick_cli.doctor import run_doctor
...
def cmd_verify(args):
    ...
    from slick_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `slick security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from slick_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `slick approvals <subcmd>`."""
    from slick_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from slick_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from slick_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from slick_cli.config import config_command
...
def cmd_skin(args):
    ...
    from slick_cli.skin_cmd import skin_command

    skin_command(args)


def cmd_backup(args):
    """Back up Slick home directory to a zip file."""
    if getattr(args, "quick", False):
        from slick_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from slick_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Slick backup from a zip file."""
    from slick_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # `slick --version` pre-import fast path (the `version` subcommand
...
def cmd_uninstall(args):
    """Uninstall Slick Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from slick_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from slick_cli.uninstall import run_gui_uninstall
    ...
    from slick_cli.uninstall import run_uninstall
...
# Update pipeline lives in slick_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on slick_cli.main.<name> keep resolving unchanged without
...
# validated against. Lives next to the checkout (NOT in SLICK_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``slick update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``slick update``
    ...
    ``slick`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``slick update``
    ...
    ``slick_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "slick_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $SLICK_HOME."""
    from slick_constants import get_slick_home
    return get_slick_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``slick`` launcher present but ``slick_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the slick venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — slick venv python3, no nix-shell overhead
    ...
    # a slick venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``slick update`` to stash the
    lockfile — repeatedly.
    ...
    # of a Slick-managed install, or a freshly provisioned managed npm when
    ...
    run_env["PATH"] = with_slick_node_path(run_env)["PATH"]
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "slick_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``slick web``).
    ...
    # build path usable on a stock `py -m slick_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_slick_node_path())
    ...
    # as `slick update`'s _update_node_dependencies() (ui-tui + web +
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (slick web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``slick update`` can unconditionally call ``slick desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``slick desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $SLICK_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $SLICK_HOME."""
    from slick_constants import get_slick_home
    return get_slick_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Slick.app/Contents/MacOS/Slick → …/Contents/Resources
    resources = (
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``slick desktop`` skips the rebuild that would fix
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``slick update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Slick.app/Contents/MacOS/Slick"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Slick.exe",
            release_dir / "win-ia32-unpacked" / "Slick.exe",
            release_dir / "win-arm64-unpacked" / "Slick.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "slick",
            release_dir / "linux-unpacked" / "Slick",
            release_dir / "linux-arm64-unpacked" / "slick",
            release_dir / "linux-arm64-unpacked" / "Slick",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Slick.exe
...
#
# The desktop self-update chain (Desktop → slick-setup --update →
# `slick update` → `slick desktop --build-only` → relaunch) rebuilds
# Slick.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Slick.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Slick.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 slick-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Slick.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Slick.exe from backup.")
        print("    Your existing version was kept and still works. Run `slick desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `slick desktop --force-build` to rebuild, or re-run the Slick")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Slick'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_slick_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Slick.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Slick.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Slick.exe: Access is
    ...
    an unrelated "Slick" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Slick.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Slick is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Slick.app/Contents/MacOS/Slick  ->  app bundle = .../Slick.app
    app = exe.parents[2]
...
def _desktop_macos_setup_tcc_identity(identity: str = "Slick Local Signing") -> bool:
    """Create/import a self-signed code-signing cert and configure Slick to use it.

    One-shot setup for ``slick desktop --setup-tcc-identity``. Creates a
    ...
    if not already_imported:
        ...
        tmp_dir = Path(tempfile.mkdtemp(prefix="slick-tcc-"))
        try:
            ...
            def _export_p12(extra_args: list) -> None:
                subprocess.run(
                    [
                        ...
                        "-out", str(p12), "-passout", "pass:slicklocal",
                    ],
                    capture_output=True, check=True,
                )
    ...
    # Point Slick at the identity (config.yaml, not .env — it's not a secret).
    try:
        from slick_cli.config import set_config_value
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.sophostechne.slick"
    )
    return True
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``slick desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Slick Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Slick Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `slick desktop` because the launcher environment doesn't look
...
def _desktop_launch_options() -> tuple[list[str], str, str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the SLICK_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Slick Desktop (Linux only, best-effort).
    ...
    shell. ``slick uninstall --gui`` removes it.
    """
    try:
        from slick_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from slick_logging import setup_logging as _setup_logging_gui
    ...
    # with_slick_node_path() copies os.environ when called with no arg.
    env = with_slick_node_path()
    if getattr(args, "fake_boot", False):
        env["SLICK_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["SLICK_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "slick_root", None):
        env["SLICK_DESKTOP_SLICK_ROOT"] = str(Path(args.slick_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["SLICK_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["SLICK_DESKTOP_CWD"] = os.getcwd()
    ...
    # `SLICK_DESKTOP_DISABLE_GPU=... slick desktop` and
    # `ELECTRON_OZONE_PLATFORM_HINT=... slick desktop` keep working.
    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options()
    )
    if config_disable_gpu != "auto" and "SLICK_DESKTOP_DISABLE_GPU" not in os.environ:
        env["SLICK_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    # `SLICK_DESKTOP_PASSWORD_STORE=... slick desktop` keeps working.
    if sys.platform == "linux" and "SLICK_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["SLICK_DESKTOP_PASSWORD_STORE"] = password_store
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Slick Local Signing"
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  slick gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Slick-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → slick-setup →
            # slick update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `slick update` path).
            nixos_env = with_slick_node_path(_nixos_build_env())
            ...
            if not source_mode:
                ...
                # holds Slick.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Slick.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Slick.exe, close any")
                    print("  running Slick desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> slick desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Slick is
                ...
                # success on a Slick.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Slick shows up
    ...
    if source_mode:
        print("→ Launching Slick Desktop from source build...")
    ...
    print(f"→ Launching packaged Slick Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)


# Dashboard process-hygiene helpers live in slick_cli/dashboard_procs.py
...
# slick_cli.main.<name> keep resolving unchanged.
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "slick dashboard",
            "slick_cli.main dashboard",
            "slick_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
        ...
        for pattern in (
            "slick serve",
            "slick_cli.main serve",
            "slick_cli/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
_DASHBOARD_SYSTEMD_UNIT = "slick-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Slick installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``slick-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/slick-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but slick launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``slick update``.
    ...
    log_path = get_slick_home() / "logs" / "dashboard-restart.log"
    try:
...
# =========================================================================
# Fork detection and upstream management for `slick update`
# =========================================================================
...
# Install-scoped breadcrumbs live next to the venv (not under $SLICK_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``slick update``.
    ...
    protocol stream (``slick acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `slick update` was interrupted mid-install — "
        "finishing dependency installation now..."
    )

    # Windows: a normal ``slick.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_slick_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from slick.exe; applying package-only first aid, "
    ...
    try:
        from slick_cli import _install_repair as _ir
        ...
        from slick_cli.managed_uv import ensure_uv
        ...
        if self_locked:
            print(
                "  Slick is still running from the launcher that needs "
                "replacing. Close other Slick windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\slick.exe`` is a launcher that runs the interpreter with
    ...
    ``slick ...`` command holds its own shim, and an editable install run
    ...
    ``slick.exe`` belonging to some other install never matches.
    """
    if not _is_windows():
        return None
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _slick_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_running_slick_launcher_locked() -> bool:
    """True when a venv ``slick*.exe`` shim is this process or an ancestor.
...
_UPDATE_REEXEC_ENV = "SLICK_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    The child re-runs ``slick update``, so the whole remaining flow — the
    ...
    from slick_constants import venv_python_path

    python_exe = venv_python_path(shim.parent.parent, windows=True)
    cmd = [str(python_exe), "-m", "slick_cli.main", *sys.argv[1:]]
...
def _default_venv_install_target() -> tuple[list[str], dict[str, str] | None]:
    ...
    try:
        ...
        uv_bin = None
    if uv_bin:
        from slick_constants import project_venv_dir
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``slick update`` is still progressing even if pip/uv itself is silent.
    """
...
def _venv_scripts_dir() -> Path | None:
    ...
    from slick_constants import project_venv_dir, venv_bin_dir
...
def _slick_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _quarantine_running_slick_exe(
    ...
    when ``slick update`` runs, ``slick.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``slick.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next slick invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Slick Desktop backend child process
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Slick Desktop / gateway / REPL).

    The updater's own launcher is no longer one of those culprits: an update
    started from ``slick.exe`` re-runs itself under the venv Python before
    ...
    for shim in _slick_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close Slick Desktop, exit other `slick` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `slick update`."
        )
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Slick versions queued for our shims.

    Slick used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            kept, removed = _filter_pending_shim_renames(
                entries, _slick_exe_shims(scripts_dir)
            )
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_slick_exe`` if uv didn't write replacements.

    This is the safety-critical direction. A failed *quarantine* only aborts an
    update; a failed *restore* leaves the install with no ``slick`` on PATH,
...
class ShimQuarantineError(RuntimeError):
    """A live ``slick*.exe`` shim could not be renamed aside (#87331).
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``slick.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``slick.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``slick`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_slick_exe(scripts_dir, failed_out=failed)
    ...
    try:
        ...
        # quarantined aside and `slick` missing from PATH after a green
...
def _quarantine_stamp_ms(stale: Path) -> int | None:
    ...
    :func:`_quarantine_running_slick_exe`. We neither rescue nor delete those:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``slick.exe.old.*`` from updates.

    Called early on every slick invocation. Two cases the old unconditional
    ``unlink()`` got wrong, both ending with ``slick`` gone from PATH:

    1. **Orphan rescue.** If ``slick.exe`` is missing while
       ``slick.exe.old.*`` is present, that .old file is the ONLY surviving
    ...
    2. **Concurrency.** A fresh quarantine file may belong to an update in
       ...
       ``slick update`` does exactly this). Leave anything inside the grace
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``slick.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``slick.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `slick update`:")
    print(
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    """
    from slick_constants import venv_bin_dir
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``slick.exe`` / ``slick-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_slick_exe`` for the rationale.
    ...
    if (
        ...
        # Quarantining the wrong dir means the running slick.exe stays locked
    ...
    # hours later inside ``slick update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``slick.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``slick.exe`` shim is locked during ``slick update``, or when uv/distlib
    skips a launcher write. The symptom is ``slick-agent.exe`` and
    ``slick-acp.exe`` present but ``slick.exe`` missing, so ``slick`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `slick update --force` after "
            "closing other slick processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m slick_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``slick update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``slick.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``slick`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `slick update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `slick update --force` after closing other slick processes."
        )
        return
...
class _UpdateOutputStream:
    """Stream wrapper used during ``slick update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.slick/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``slick update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``slick update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.slick/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``slick update --gateway``) the update is already
    ...
    try:
        ...
        # slick_cli.config.get_slick_home to simulate setup failure.
        from slick_cli.config import get_slick_home as _get_slick_home

        logs_dir = _get_slick_home() / "logs"
...
def cmd_update(args):
    """Update Slick Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Slick Agent")
        return
    ...
    if getattr(args, "plan", False):
        ...
        # Slick runtime across profiles, its supervisor, and its running
        # code version — without mutating anything. Safe on a live fleet.
        from slick_cli.update_inventory import (
    ...
    # exit 1 errors).
    from slick_cli.update_contract import (
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `slick update --branch=<x>` would actually pull.
    ...
    from slick_cli.update_lock import (
    ...
    try:
        ...
        try:
            from slick_cli.update_receipt import finalize_pending_update_receipt
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``slick -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from slick_cli.profiles import (
        list_profiles,
        create_profile,
        delete_profile,
        seed_profile_skills,
    ...
    from slick_constants import display_slick_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `slick profile` — show current profile status
        from slick_cli.profiles import format_profile_label

        profile_name = get_active_profile_name()
        dhh = display_slick_home()
        ...
        if current is not None:
            ...
            if p.alias_path:
                ...
                print(f"Alias:          {alias_display} → slick -p {p.name}")
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.slick)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  slick profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     slick -p {name} chat")
                else:
        ...
        from slick_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from slick_constants import get_slick_home as _hh
        ...
        from slick_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `slick profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → slick -p {name}  ({wrapper})")
        print()
        ...
        from slick_cli.profiles import profile_exists, validate_alias_name
        ...
        from slick_cli.profiles import normalize_profile_name, rename_profile
        ...
        from slick_cli.profiles import export_profile
        ...
        from slick_cli.profiles import import_profile
        ...
        import tempfile
        from slick_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="slick_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  slick -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      slick -p {plan.manifest.name} chat")
        ...
        from slick_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `slick profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"slick -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from slick_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("slick_requires"):
            print(f"Requires:     Slick {data['slick_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from slick_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.slick_requires:
        print(f"  Requires: Slick {mf.slick_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No slick dashboard or serve processes running.")
        return 0

    print(f"{len(live)} slick dashboard/serve process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    provider on the spot — or point them at ``slick dashboard register`` for
    OAuth.
    ...
    try:
        from slick_cli.web_server import should_require_dashboard_auth
    ...
    try:
        from slick_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Sophos Portal (run `slick dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    slick dashboard register\n"
            "  It provisions a Sophos Portal OAuth client and writes "
            "SLICK_DASHBOARD_OAUTH_CLIENT_ID into ~/.slick/.env for you.\n"
            "  Docs: https://slick-agent.sophostechne.com/docs/"
    ...
    try:
        from slick_cli.config import load_config, save_config
        from slick_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
    ...
    try:
        from slick_cli.plugins import discover_plugins
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from slick_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.slick/desktop-ssh: a
    # literal "~/.slick/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of SLICK_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_slick_home(): a non-default sticky profile (or any SLICK_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_slick_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".slick" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``SLICK_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``slick dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No slick dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `slick update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with slick serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with SLICK_DESKTOP=1 plus
    # SLICK_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # SLICK_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `slick dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited SLICK_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # SLICK_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (SLICK_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets SLICK_SERVE_HEADLESS below.
    if os.environ.get("SLICK_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("SLICK_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("SLICK_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("SLICK_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → SLICK_HOME points into
    ...
    try:
        from slick_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("SLICK_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "slick_cli.main",
            "-p", "default",
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: SLICK_HOME is explicitly pinned to the
        ...
        # SLICK_HOME.  We must resolve the root explicitly instead of just
        # dropping SLICK_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV SLICK_HOME=/opt/data`), so an unset
        # SLICK_HOME falls back to $HOME/.slick = /opt/data/.slick — an
        ...
        # update-button guard also misfires).  get_default_slick_root()
        # returns the root for both layouts: ~/.slick for a standard install
        ...
        try:
            from slick_constants import get_default_slick_root
            env["SLICK_HOME"] = str(get_default_slick_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop SLICK_HOME) rather than block the reroute.
            env.pop("SLICK_HOME", None)
    ...
    from slick_cli.resource_limits import apply_nofile_soft_limit
    ...
    # the same logs directory as every other Slick surface.
    try:
        from slick_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["SLICK_SERVE_HEADLESS"] = "1"
    elif "SLICK_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["SLICK_WEB_DIST"])
            if "SLICK_WEB_DIST" in os.environ
            else PROJECT_ROOT / "slick_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom SLICK_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "SLICK_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # SLICK_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["SLICK_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ SLICK_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset SLICK_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads SLICK_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["SLICK_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from SLICK_WEB_DIST: {_dist_root}")

    # Discover and load plugins so any DashboardAuthProvider plugin
    # (e.g. plugins/dashboard_auth/sophos) registers BEFORE start_server's
    ...
    try:
        from slick_cli.mcp_startup import start_background_mcp_discovery
    ...
    from slick_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    """Register a self-hosted dashboard OAuth client with Sophos Portal."""
    from slick_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from slick_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from slick_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from slick_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Slick log files."""
    from slick_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Slick command console."""
    from slick_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from slick_cli.models import CANONICAL_PROVIDERS as _cp
        return ["auto"] + [p.slug for p in _cp]
    except Exception:
        # Fallback: static list guarantees the CLI always works
        return [
            "auto", "openrouter", "sophos", "openai-codex", "xai-oauth", "copilot-acp", "copilot",
...
def _first_positional_argv() -> str | None:
    ...
    ``slick -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``slick`` or only flags → defaults to ``chat``.
        return False
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``slick <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``slick photon``
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("SLICK_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that SLICK_YOLO_MODE is set before ANY
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from slick_cli.plugins import start_background_plugin_discovery
...
def _try_fast_chat_launch() -> bool:
    ...
    ``slick`` / ``slick -w -s foo --yolo`` / ``slick chat`` don't need the
    ...
    """
    if os.environ.get("SLICK_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
    ...
    try:
        from slick_cli.config import get_container_exec_info
    ...
    from slick_cli._parser import build_top_level_parser
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("SLICK_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["SLICK_DEFER_AGENT_STARTUP"] = "1"
            os.environ["SLICK_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["SLICK_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `slick --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from slick_constants import get_slick_home, display_slick_home

        mem_dir = get_slick_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_slick_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_slick_home()}/memories/\n")
    else:
        from slick_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Slick Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from slick_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from slick_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from slick_cli.tools_config import tools_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: slick-agent[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from slick_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    elif getattr(args, "skills_action", None) in ("trust", "untrust"):
        _cmd_skills_trust(args)
    else:
        from slick_cli.skills_hub import skills_command

        skills_command(args)


def _cmd_skills_trust(args):
    """``slick skills trust [path]`` / ``slick skills untrust [path]``.
...
def cmd_pairing(args):
    from slick_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from slick_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from slick_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from slick_cli.claw import claw_command
...
def _advertise_agent_env() -> None:
    ...
    (``slick-agent`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``SLICK_AGENT`` is the Slick-specific marker. setdefault: never
    clobber an outer harness (e.g. Slick running inside another agent's
    terminal).
    """
    os.environ.setdefault("AI_AGENT", "slick-agent")
    os.environ.setdefault("SLICK_AGENT", "true")


def main():
    """Main entry point for slick CLI."""
    # Cosmetic: make the process show up as 'slick' instead of 'python3.11'
    ...
    try:
        from slick_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``slick.exe.old.*`` quarantine files left by previous
    # ``slick update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_slick_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # If the checkout changed since the last launch (slick update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``slick update``
    ...
    # ``slick skills install update``) merely defers recovery one launch;
    # under-matching (missing ``slick -p work update``) would race a recovery
    ...
    # never restarted the fleet. Do NOT restart here — that is ``slick
    ...
    # =========================================================================
    # model command  (parser built in slick_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from slick_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from slick_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://slick-agent.sophostechne.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `slick model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    worktree_parser = subparsers.add_parser(
        ...
        description=(
            "Attended reclaim for the .worktrees/ directory slick -w sessions "
            "accumulate. Never deletes uncommitted tracked changes, unique "
            "unpushed commits, or in-use trees; untracked-only scratch is "
            "archived to ~/.slick/archive/worktree-prune/ before removal. See: "
            "https://slick-agent.sophostechne.com/docs/user-guide/cli#worktree-cleanup"
        ),
    )
    ...
    def _dispatch_worktree(_args):
        from slick_cli.worktree_cmd import cmd_worktree
    ...
    browser_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "default profile so Slick can copy it — DESTRUCTIVE (unsaved tabs "
    ...
    def _dispatch_browser(_args):
        from slick_cli.browser_connect import (
            UNSUPPORTED_CHANNEL,
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.slick/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://slick-agent.sophostechne.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from slick_cli import secrets_cli as _secrets_cli
    from slick_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `slick proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://slick-agent.sophostechne.com/docs/user-guide/egress/iron-proxy"
        ),
    )

    from slick_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``slick proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from slick_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in slick_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in slick_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in slick_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `slick whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in slick_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from slick_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in slick_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in slick_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in slick_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in slick_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in slick_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in slick_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in slick_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Slick gateways)
    # =========================================================================
    from slick_cli.subcommands.peer import build_peer_parser

    build_peer_parser(subparsers)

    # =========================================================================
    # portal command — Sophos Portal status + Tool Gateway routing
    # =========================================================================
    from slick_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from slick_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from slick_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in slick_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in slick_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in slick_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in slick_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in slick_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in slick_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in slick_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in slick_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.slick/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo slick uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from slick_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in slick_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: slick_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from slick_cli.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in slick_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in slick_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in slick_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in slick_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in slick_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from slick_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in slick_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``slick --help``, ``slick logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from slick_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from slick_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Slick shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from slick_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from slick_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in slick_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in slick_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `slick computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `slick tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `slick computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Slick app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "install":
            from slick_cli.tools_config import (
        ...
        if action == "status":
            import os as _os
            import subprocess
            from slick_cli.tools_config import _cua_driver_contract_status
            ...
            override = _os.environ.get("SLICK_CUA_DRIVER_CMD", "").strip()
            if path:
                version = ""
                try:
                    from slick_cli.tools_config import _cua_driver_env
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            "    Update the binary selected by SLICK_CUA_DRIVER_CMD, or unset "
                            "the override and run: slick computer-use install --upgrade"
                        )
                    else:
                        print("    Run: slick computer-use install")
                    return 1
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: slick computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: slick computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: slick computer-use install --upgrade")
                return 0
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: slick computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: slick computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in slick_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    def _add_session_filter_args(p, default_older_help):
        ...
        p.add_argument(
            ...
            "(e.g. 'sonnet', 'gpt-5', 'slick')",
        )
        p.add_argument(
            "--provider",
            help="Only match sessions billed through this provider "
            "(e.g. openrouter, anthropic, sophos)",
        )
        p.add_argument(
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <slick home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Slick",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Slick session store "
            "so it can be resumed with 'slick --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in slick_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on slick_cli.main.cmd_sessions keep working.
    ...
    # =========================================================================
    # insights command  (parser built in slick_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in slick_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # NOTE: the `slick version` subcommand was removed — `slick --version`
    # / `-V` now carries the full output including update status.

    # =========================================================================
    # update command  (parser built in slick_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in slick_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in slick_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in slick_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in slick_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Slick-Setup.exe success screen tells users to
    # run `slick desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in slick_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in slick_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in slick_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``slick -c Pokemon Agent Dev`` → ``slick -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'slick -c model' where 'model' is consumed as the
    ...
    # --yolo: set SLICK_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (slick hooks list, cron
    ...
    # ``slick egress start`` refusing when credential_source=bitwarden
