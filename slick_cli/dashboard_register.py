...
Automates what a user otherwise does by hand: open the Hackit Portal
...
This command:
  1. Resolves a fresh Hackit Portal access token from the existing login
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      ...
      2. The ``portal_base_url`` stored on the Hackit login — this is the
    ...
    try:
        from slick_cli.auth import DEFAULT_HACKIT_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Hackit Portal rejected the access token (401). "
                "Try `slick auth add hackit` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
        ...
        raise RuntimeError(
            f"Could not reach Hackit Portal at {portal_base_url}: {exc.reason}"
        ) from exc
...
def _print_post_register_hint(
    *,
    ...
    print()
    print(
        "  Heads up — Hackit login only *engages* on a non-loopback bind. A plain\n"
    ...
    if custom_redirect_uri:
        ...
        try:
            ...
            host = "your-host"
        print("  To require Hackit login on your registered host, run the dashboard")
        print(f"  bound publicly (it must be reachable at https://{host}) and log in")
        print("  at its /login page.")
    else:
        print("  To require Hackit login (e.g. exposing on your LAN or a public host):")
...
def cmd_dashboard_register(args) -> None:
    ...
    from slick_cli.auth import AuthError, resolve_hackit_access_token
    ...
    # 1. Resolve a fresh Hackit access token (refreshes if near expiry). Fail fast
    #    with a setup hint when the user isn't logged in.
    try:
        access_token = resolve_hackit_access_token()
    except AuthError as exc:
        if getattr(exc, "relogin_required", False):
            print("✗ You're not logged into Hackit Portal.")
            print("  Run `slick setup` (or `slick auth add hackit`) first, then retry.")
        else:
            print(f"✗ Could not resolve a Hackit Portal access token: {exc}")
        sys.exit(1)
    ...
    default_portal = "https://portal.hackit.cc"
