...
This module is imported by ``slick_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no slick_cli.config,
...
``slick_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(slick_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect SLICK_HOME?"""
    active_profile = os.path.join(slick_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    slick_home = os.environ.get("SLICK_HOME", "").strip()
    if slick_home:
        return slick_home
    return os.path.join(os.path.expanduser("~"), ".slick")
...
def container_mode_may_be_active() -> bool:
    ...
    slick_home = os.environ.get("SLICK_HOME", "").strip()
    if slick_home:
        if os.path.exists(os.path.join(slick_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(slick_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(slick_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".slick")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect SLICK_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``slick --version`` output (also used by /version).
    ...
    ``slick version`` subcommand. Every lazy block degrades gracefully —
    ...
    try:
        from slick_cli.banner import format_banner_version_label

        print(format_banner_version_label())
    except Exception:
        from slick_cli import __release_date__, __version__

        print(f"Slick Agent v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from slick_cli.config import detect_install_method
    ...
    try:
        from slick_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``slick --version`` before the heavy import wall.
    ...
    container. Termux keeps the SLICK_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("SLICK_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
