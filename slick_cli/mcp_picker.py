"""MCP picker — interactive `slick mcp picker` (also the default `slick mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``slick mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `slick plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from slick_cli.cli_output import prompt_yes_no
from slick_cli.curses_ui import curses_single_select
from slick_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Slick session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from slick_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `slick mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: slick mcp install <name>    Picker: slick mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Slick is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Slick — run `slick update` "
...
def show_catalog() -> None:
    """`slick mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`slick mcp picker` (and default `slick mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`slick mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `slick mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
