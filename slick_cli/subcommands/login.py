...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``slick login`` was removed in favor of ``slick auth`` / ``slick model``
    (the runtime handler in ``slick_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``slick login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``slick --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``slick login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``slick model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `slick auth` to manage credentials, "
            "`slick model` to select a provider, or `slick setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `slick model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: slick-cli)"
    )
