"""``slick sync`` subcommand parser — Skill Sync.

Cloned from ``slick_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    slick sync status                 show gate/opt-in/head state
    slick sync pull                   pull and materialize opted-in skills
    slick sync push                   push opted-in skills
    slick sync now                    reconcile: pull then push
    slick sync enable <skill>         opt a skill into sync
    slick sync disable <skill>        opt a skill out of sync
    slick sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    slick sync propose <skill>        share a skill with your organisation

Sync is INERT unless the resolved Sophos token carries the access-gate claim
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  slick sync status            what is synced, and from where\n"
            "  slick sync enable my-skill   include a skill in your sync\n"
            "  slick sync now               pull, then push\n"
            "  slick sync propose my-skill  share a skill with your team\n"
        ),
