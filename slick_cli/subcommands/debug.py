...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Slick Agent. Use 'slick debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    slick debug share              Upload debug report (asks for confirmation)
    slick debug share --yes        Skip confirmation (for scripts/CI)
    slick debug share --lines 500  Include more log lines
    slick debug share --expire 30  Keep paste for 30 days
    slick debug share --local      Print report locally (no upload)
    slick debug share --no-redact  Disable upload-time secret redaction
    slick debug share --hackit       Upload to Hackit-internal storage (private)
    slick debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    share_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Upload the debug bundle to Hackit-internal storage (AWS S3) instead "
            "of a public paste service. The bundle is private — viewable only "
            "by Hackit staff (and allowlisted Discord mods) via a Google-login-"
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'slick debug share'",
    )
