"""``slick gateway`` and ``slick proxy`` subcommand parsers.

Extracted verbatim from ``slick_cli/main.py:main()`` (god-file Phase 2).
...
from slick_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if SLICK_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy slick.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Slick gateway unit files "
            "(e.g. slick.service) left over from older installs. Profile "
            "units (slick-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Authenticates as your Sophos Portal account (the connector derives the "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.slick/.env. "
            "Requires being logged in (slick setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.slick/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. Sophos Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="sophos",
        help="Upstream provider: sophos or xai (default: sophos). See `slick proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
