...
``slick update`` mutation (CLI apply, CLI --check, dashboard update
endpoint). The decision layers:

1. **Baked provenance marker** (``/etc/slick/image-provenance.json``,
   written by the image build — see :mod:`slick_cli.image_provenance`):
