"""Image-authored deployment provenance for immutable Slick runtimes.

The published image bakes ``/etc/slick/image-provenance.json`` outside both
``$SLICK_HOME`` and the mutable checkout.  A bind-mounted checkout (including
...
IMAGE_PROVENANCE_PATH = Path("/etc/slick/image-provenance.json")
