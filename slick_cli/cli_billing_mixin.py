...
This module hosts the Hackit billing/subscription methods lifted out of
...
class CLIBillingMixin:
    ...
    def _print_hackit_credits_block(self) -> bool:
        """Print the Hackit dollar balance block (two-bar view) when a Hackit account
        ...
        truth). Falls back to the legacy ``hackit_credits_lines`` text only when the
        model is unavailable. Agent-independent (a portal fetch gated on "a Hackit
    ...
    def _print_usage_cta(self) -> None:
        ...
        usage read with the same nudge. Only called when a Hackit account is logged
        in (the balance block printed), since both commands are Hackit-account only.
        """
    ...
    def _show_subscription(self):
        """`/subscription` (alias `/upgrade`) — view the Hackit plan + browser hand-off.
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Hackit Portal.')}")
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from slick_cli.hackit_billing import BillingError, BillingScopeRequired, post_subscription_preview
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        step_up_hackit_billing_scope directly (it opens the browser + blocks), then
        ...
        try:
            from slick_cli.auth import step_up_hackit_billing_scope

            granted = step_up_hackit_billing_scope(open_browser=True)
        ...
        try:
            from slick_cli import hackit_billing as _nb
    ...
    def _show_billing(self, command: str = "/topup"):
        """`/topup` — Remote Spending for Hackit (one interactive modal).
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow Hackit to charge your card."
        )
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        from slick_cli.hackit_billing import BillingError, post_charge
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        print()
        _ar_consent = (
            f"By confirming, you authorize Hackit to charge {card.masked} "
