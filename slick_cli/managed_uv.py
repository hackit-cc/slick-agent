"""Slick-managed uv and Python runtime repair.

Slick owns its own uv binary at ``$SLICK_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$SLICK_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every Slick
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.slick-runtime/python``. A
...
from slick_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".slick-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.sophostechne.slick.managed-python"
...
def managed_uv_path() -> Path:
    """Return the path where Slick keeps *its* uv binary.

    ``$SLICK_HOME/bin/uv`` on POSIX, ``$SLICK_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Slick-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _macos_sign_managed_python(python: Path) -> bool:
    ...
    TCC with a cdhash-only identity that changes whenever Slick provisions a
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Slick keeps databases "
            "out of WAL mode on this SQLite build. The next `slick update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``slick update`` runs
    the call site from the *old*, already-imported ``slick_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``slick update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under SLICK_HOME
    caches the last successful self-update time.
    """
    try:
        from slick_constants import get_slick_home

        stamp = get_slick_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``slick update`` so the managed copy stays current.
...
def _reload_slick_constants():
    """Re-execute ``slick_constants`` from disk and return the fresh module.

    ``slick update`` imports ``slick_constants`` from the OLD checkout,
    ...
    return importlib.reload(slick_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Slick generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `slick update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("slick_cli.main")
    ...
    if holders:
        ...
        return True, f"other Slick processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Slick-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``slick update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``slick update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$SLICK_HOME/bin/`` instead of ``~/.local/bin/``.
    """
