"""CLI handlers for the ``slick proxy`` subcommand."""
...
from slick_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "slick proxy requires aiohttp. Run `slick setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "hackit"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"slick auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Slick proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Slick proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: slick proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``slick proxy <subcommand>``."""
    ...
    print(
        "slick proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  slick proxy start [--provider hackit|xai] [--host 127.0.0.1] [--port 8645]\n"
