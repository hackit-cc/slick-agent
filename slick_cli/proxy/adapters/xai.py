...
# xAI's public API is OpenAI-compatible for the endpoints Slick commonly
# uses. The Responses endpoint is included because Slick' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Slick-managed OAuth credentials."""

    auth_hint = "slick auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`slick auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`slick auth reset xai-oauth` or re-authenticate with "
                    "`slick auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `slick auth add xai-oauth --type oauth`."
            )
