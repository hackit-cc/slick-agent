"""On-demand supply-chain audit for Slick Agent installs.

Scans three surfaces a Slick user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Slick venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.slick/plugins``
...
def _discover_plugins(slick_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.slick/plugins``.
...
def _discover_components(
    *,
    ...
    home = slick_home or Path(get_slick_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `slick security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, slick_home=home
    )
