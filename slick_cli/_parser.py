"""
Top-level argparse construction for the slick CLI.
...
# argparse runs (it sets ``SLICK_HOME`` and strips itself from ``sys.argv``),
...
# tests/slick_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``slick --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``slick_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    slick                        Start interactive chat
    slick chat -q "Hello"        Single query mode
    slick --tui                  Launch the modern TUI (or set display.interface: tui)
    slick --cli                  Force the classic REPL (overrides display.interface: tui)
    slick -c                     Resume the most recent session
    slick -c "my project"        Resume a session by name (latest in lineage)
    slick --resume <session_id>  Resume a specific session by ID
    slick --resume latest        Resume the most recent session (same as -c)
    slick --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    slick setup                  Run setup wizard
    slick logout                 Clear stored authentication
    slick auth add <provider>    Add a pooled credential
    slick auth list              List pooled credentials
    slick auth remove <p> <t>    Remove pooled credential by index, id, or label
    slick auth reset <provider>  Clear exhaustion status for a provider
    slick model                  Select default model
    slick fallback [list]        Show fallback provider chain
    slick fallback add           Add a fallback provider (same picker as `slick model`)
    slick fallback remove        Remove a fallback provider from the chain
    slick config                 View configuration
    slick config edit            Edit config in $EDITOR
    slick config set model gpt-4 Set a config value
    slick gateway                Run messaging gateway
    slick -s slick-agent-dev,github-auth
    slick -w                     Start in isolated git worktree
    slick gateway install        Install gateway background service
    slick sessions list          List past sessions
    slick sessions browse        Interactive session picker
    slick sessions rename ID T   Rename/title a session
    slick logs                   View agent.log (last 50 lines)
    slick logs -f                Follow agent.log in real time
    slick logs errors            View errors.log
    slick logs --since 1h        Lines from the last hour
    slick debug share             Upload debug report for support
    slick console                Open the safe Slick command console
    slick update                 Update to latest version
    slick dashboard              Start web UI dashboard (port 9119)
    slick dashboard --stop       Stop running dashboard processes
    slick dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="slick",
        description="Slick Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `slick chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via SLICK_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `slick setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to SLICK_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.slick/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Slick Agent",
    )
    ...
    # top-level parser: when the user writes `slick -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/slick_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also SLICK_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.slick/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Slick itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
