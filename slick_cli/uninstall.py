"""
Slick Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.slick/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Slick PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing slick-agent or slick PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Slick Agent" comment and following line
                if '# Slick Agent' in line or '# slick-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('slick' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing slick
                if 'slick' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Slick-owned file, and
...
def remove_wrapper_script():
    """Remove the slick wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "slick",
        Path.home() / ".local" / "bin" / "slick-acp",
        Path.home() / ".local" / "bin" / "slick-agent",
        Path("/usr/local/bin/slick"),
        Path("/usr/local/bin/slick-acp"),
        Path("/usr/local/bin/slick-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains slick_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'slick_cli' in content or 'slick-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(slick_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``slick`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Slick home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (slick_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``slick gateway run`` processes
    ...
    try:
        from slick_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from slick_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SLICK_HOME`` and ``SLICK_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\slick\git\cmd``, ``%LOCALAPPDATA%\slick\git\bin``,
#      ``%LOCALAPPDATA%\slick\git\usr\bin``, ``%LOCALAPPDATA%\slick\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\slick\git\`` and Node to
#      ``%LOCALAPPDATA%\slick\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``slick dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\slick\gateway-service\`` and sometimes
...
def _slick_path_markers(slick_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Path-entry substrings that identify Slick-owned User-PATH entries.

    ``include_managed_bin`` adds the managed binary dir (``<root>\\bin``,
    holding the slick launchers and the managed uv) — only wanted when
    ...
    """
    root = str(slick_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare slick-agent install dir.
    markers = [root + "\\slick-agent", root + "\\git", root + "\\node", root + "\\venv"]
    if include_managed_bin:
        markers.append(root + "\\bin")
    # Also match if SLICK_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "slick".  We don't want to catch
    # unrelated entries like "cslick-foo" or "ephermeral", so we look for
    # backslash-slick as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(slick_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Slick-owned entries from User-scope PATH in the registry.
    ...
    ``include_managed_bin`` adds ``<slick_home>\\bin`` (the managed binary
    dir holding the slick launchers and the managed uv) to the sweep. Only
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _slick_path_markers(slick_home, include_managed_bin=include_managed_bin)
            kept: list[str] = []
...
def remove_slick_env_vars_windows() -> list[str]:
    """Delete SLICK_HOME and SLICK_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SLICK_HOME", "SLICK_GIT_BASH_PATH"):
                try:
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``slick`` launchers install.ps1 staged in the managed
    binary dir (the default Slick root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``slick`` in a new terminal resolves to a launcher whose target is
    ...
    ``_install_repair._quarantine_running_slick_exe`` relies on), so
    ...
    *windows* is an injectable platform verdict for tests (same pattern as
    ...
    try:
        ...
        from slick_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from slick_constants import get_default_slick_root

        bin_dir = get_default_slick_root() / "bin"
    except Exception as e:
...
def _is_default_slick_home(slick_home: Path) -> bool:
    """Return True when ``slick_home`` points at the default (non-profile) root."""
    try:
        from slick_constants import get_default_slick_root
        return slick_home.resolve() == get_default_slick_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SLICK_HOME directory.

    We shell out to ``slick -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SLICK_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m slick_cli.main` so we don't depend on a `slick`
    #    wrapper that may be half-removed mid-uninstall.
    slick_invocation = [_sys.executable, "-m", "slick_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                slick_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's SLICK_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``slick uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$SLICK_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from slick_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(slick_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         😎 Slick Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Slick Chat GUI installation was found.")
        print(f"  Checked: {slick_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Slick agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(slick_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Slick agent at {slick_home / 'slick-agent'}")
        print(f"  • Your config, sessions, and secrets under {slick_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Slick agent is still installed. Run 'slick' to use the CLI,")
    print("or 'slick uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.slick/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.slick/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie SLICK_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_slick_home(slick_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.slick data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            😎 Slick Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {slick_home / 'config.yaml'}")
    print(f"  Secrets: {slick_home / '.env'}")
    print(f"  Data:    {slick_home / 'cron/'}, {slick_home / 'sessions/'}, {slick_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SLICK_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Slick data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Slick code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, slick_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Slick PATH entries from shell configs / Windows User PATH")
    print("  • Slick wrapper scripts and Slick-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Slick config/data: {slick_home}")
        if _is_default_slick_home(slick_home):
        ...
        print(f"  • Keep Slick config/data: {slick_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``slick`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$SLICK_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in slick_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\slick\git\cmd, not %LOCALAPPDATA%.
        # The managed binary dir (slick\bin: launchers + managed uv) leaves
        ...
        sweep_managed_bin = full_uninstall and _is_default_slick_home(slick_home)
        removed_path_entries = remove_path_from_windows_registry(
            Path(os.path.expandvars(str(slick_home))),
        ...
        if removed_path_entries:
            ...
            log_info("No Slick-owned PATH entries in User environment")

        log_info("Removing SLICK_HOME / SLICK_GIT_BASH_PATH User env vars...")
        removed_env = remove_slick_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Slick-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing slick command...")
    ...
    #     would dangle — `slick` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows slick launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows slick launchers found")

    # 3b. Remove node/npm/npx symlinks the installer left in ~/.local/bin
    #     (only when they still point into this Slick home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Slick-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(slick_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Slick-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(slick_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE SLICK_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from slick_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(slick_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.slick/, just remove the slick-agent subdir
            if slick_home in project_root.parents or project_root.parent == slick_home:
    ...
    #     under SLICK_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(slick_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(slick_home)
    ...
    # 5. Optionally remove ~/.slick/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SLICK_HOME dirs live under
        ...
        try:
            if slick_home.exists():
                shutil.rmtree(slick_home)
                log_success(f"Removed {slick_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {slick_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {slick_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://slick-agent.sophostechne.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://slick-agent.sophostechne.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Slick Agent! 😎")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m slick_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import slick_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``slick_constants`` + ``slick_cli.colors``
    (and lazily ``slick_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m slick_cli.uninstall")
    parser.add_argument(
        "--mode",
