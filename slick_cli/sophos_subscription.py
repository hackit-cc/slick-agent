"""Helpers for Sophos subscription managed-tool capabilities."""
...
# Maps a tools_config provider's ``managed_sophos_feature`` to the tool-pool
# coverage category (slick_cli.sophos_account.TOOL_COVERAGE_CATEGORIES). Lets the
...
# consistent with the per-category feature gates in get_sophos_subscription_features.
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    an in-memory section dict: ``"sophos"`` for the managed selection (stored
    ``sophos`` value or legacy ``use_gateway: true``), a vendor name for BYOK
...
class SophosFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_sophos: bool
...
@dataclass(frozen=True)
class SophosSubscriptionFeatures:
    subscribed: bool
    sophos_auth_present: bool
    provider_is_sophos: bool
    features: Dict[str, SophosFeatureState]
    account_info: Optional[SophosPortalAccountInfo] = None

    @property
    def web(self) -> SophosFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> SophosFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> SophosFeatureState:
        return self.features["tts"]

    @property
    def stt(self) -> SophosFeatureState:
        return self.features["stt"]

    @property
    def browser(self) -> SophosFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> SophosFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> SophosFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[SophosFeatureState]:
...
def _local_stt_backend_available() -> bool:
    ...
    ``apply_sophos_managed_defaults`` from flipping a working local setup
...
def get_sophos_subscription_features(
    ...
    *,
    ...
    provider_is_sophos = str(model_cfg.get("provider") or "").strip().lower() == "sophos"

    try:
        if force_fresh:
            account_info = get_sophos_portal_account_info(force_fresh=True)
        else:
            account_info = get_sophos_portal_account_info()
    except Exception:
        account_info = None
    ...
    sophos_auth_present = bool(account_info and account_info.logged_in)

    def _entitled_for(category: str) -> bool:
        return bool(account_info and account_info.tool_gateway_entitled_for(category))
    subscribed = provider_is_sophos or sophos_auth_present
    ...
    # requires `pip install faster-whisper`. For Sophos subscribers we'd
    # rather route through the managed OpenAI audio gateway — see
    # apply_sophos_managed_defaults below.
    ...
    # "sophos" (stored value or legacy use_gateway: true) = managed gateway;
    ...
    web_use_gateway = web_selected == "sophos"
    tts_use_gateway = tts_selected == "sophos"
    stt_use_gateway = stt_selected == "sophos"
    browser_use_gateway = browser_selected == "sophos"
    image_use_gateway = image_selected == "sophos"
    video_use_gateway = video_selected == "sophos"

    # The "sophos" selection is serviced by a concrete vendor implementation —
    # normalize the current-provider labels so downstream vendor checks hold.
    if web_backend == "sophos" or web_use_gateway:
        web_backend = "firecrawl"
    if tts_provider == "sophos" or tts_use_gateway:
        tts_provider = "openai"
    if stt_provider == "sophos" or stt_use_gateway:
        stt_provider = "openai"
    if browser_provider == "sophos" or browser_use_gateway:
    ...
    features = {
        "web": SophosFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_sophos=web_managed,
        ...
        "image_gen": SophosFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_sophos=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if (image_selected not in (None, "sophos") or (image_selected is None and direct_fal)) else ("Sophos Subscription" if (image_managed or image_use_gateway) else ""),
            explicit_configured=image_selected is not None or direct_fal,
        ),
        "video_gen": SophosFeatureState(
            key="video_gen",
            label="Video generation",
            included_by_default=False,
            available=video_available,
            active=video_active,
            managed_by_sophos=video_managed,
            direct_override=video_active and not video_managed,
            toolset_enabled=video_tool_enabled,
            current_provider="FAL" if (video_selected not in (None, "sophos") or (video_selected is None and direct_fal_video)) else ("Sophos Subscription" if (video_managed or video_use_gateway) else ""),
            explicit_configured=video_selected is not None or direct_fal_video,
        ),
        "tts": SophosFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_sophos=tts_managed,
        ...
        "stt": SophosFeatureState(
            key="stt",
            label="Speech-to-text",
            included_by_default=True,
            available=stt_available,
            active=stt_active,
            managed_by_sophos=stt_managed,
        ...
        "browser": SophosFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_sophos=browser_managed,
        ...
        "modal": SophosFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_sophos=modal_managed,
    ...
    return SophosSubscriptionFeatures(
        subscribed=subscribed,
        sophos_auth_present=sophos_auth_present,
        provider_is_sophos=provider_is_sophos,
        features=features,
...
def apply_sophos_managed_defaults(
    ...
    *,
    ...
    if not (
        ...
        return set()
    if not features.provider_is_sophos:
        return set()
    ...
    if "tts" in selected_toolsets and not features.tts.explicit_configured and not (
        ...
        tts_cfg["provider"] = "sophos"
        tts_cfg.pop("use_gateway", None)
        changed.add("tts")

    # STT: same pattern as TTS. The DEFAULT_CONFIG seed is "local"
    # (requires `pip install faster-whisper`); for Sophos subscribers we
    ...
    if "browser" in selected_toolsets and not features.browser.explicit_configured and not (
        ...
        browser_cfg["cloud_provider"] = "sophos"
...
# ``get_sophos_subscription_features`` reads (web uses "backend", browser
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    - unconfigured: tools with no direct credentials and no explicit
      non-sophos selection (easy switch, safe to pre-check)
    - has_direct: tools where the user has their own API keys
    - explicit_configured: tools with an explicit non-sophos selection stored
    ...
    All lists are empty when the user is not a paid Sophos subscriber or
    is not using Sophos as their provider.
    """
    ...
    # Quick provider check without the heavy get_sophos_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "sophos":
        return [], [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_sophos which fires implicitly when
    ...
    for key in _ALL_GATEWAY_KEYS:
        ...
        elif selected is not None and selected != "sophos":
            # An explicit non-sophos selection (e.g. a keyless local backend
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    # already_managed, just for a non-sophos vendor.
    ...
    try:
        account_info = get_sophos_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
    ...
    source_label = "free tool pool" if pool_only else "Sophos subscription"
    ...
    # pre-checked checklist re-fired on every Sophos model swap.
    ...
    if pool_only:
        title = "Your free Sophos tool pool — pick the tools to enable:"
    else:
        title = (
            "Your Sophos subscription includes the Tool Gateway — "
    ...
    # so the next Sophos model swap doesn't pre-check it again. Cancel paths
...
# ---------------------------------------------------------------------------
# Inline Sophos Portal login for the Tool Gateway picker (`slick tools`)
# ---------------------------------------------------------------------------


def ensure_sophos_portal_access(
    *,
    capability: str = "the Sophos Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the Sophos Tool Gateway, logging in if
    needed.

    Used by ``slick tools`` when a user selects a Sophos-managed Tool Gateway
    backend (e.g. "Firecrawl (Sophos Portal)").  Unlike ``slick model``'s Sophos
    login, this:
    ...
    It only performs the Sophos Portal device-code OAuth (when the user isn't
    ...
    try:
        info = get_sophos_portal_account_info(force_fresh=True)
    except Exception:
        info = None
    if _entitled(info):
        return True
    ...
    if info is None or not info.logged_in:
        if not _run_sophos_portal_login_only(capability=capability):
            return False
        try:
            info = get_sophos_portal_account_info(force_fresh=True)
        except Exception:
...
def _run_sophos_portal_login_only(*, capability: str) -> bool:
    """Run the Sophos Portal device-code OAuth and persist credentials only.
    ...
    try:
        from slick_cli.auth import (
            _auth_store_lock,
            _load_auth_store,
            _sophos_device_code_login,
            _read_shared_sophos_state,
            _save_auth_store,
            _save_provider_state,
            _sync_sophos_pool_from_auth_store,
            _try_import_shared_sophos_state,
            _write_shared_sophos_state,
        )
    except Exception as exc:  # pragma: no cover - defensive
        print(f"  Could not start Sophos Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a Sophos Portal login.")
    try:
        proceed = input("  Log in to Sophos Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped Sophos Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to Sophos.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing Sophos OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                auth_state = _try_import_shared_sophos_state(timeout_seconds=15.0)
        ...
        _sync_sophos_pool_from_auth_store()
        print("  Sophos Portal login successful.")
        return True
    except KeyboardInterrupt:
        print("\n  Login cancelled.")
        return False
    except SystemExit:
        # _sophos_device_code_login raises SystemExit on subscription_required;
        # it already printed billing guidance.
        return False
    except Exception as exc:
        print(f"  Sophos Portal login failed: {exc}")
        return False
