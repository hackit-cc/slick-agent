"""`slick checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.slick/checkpoints/``.  Actions:

    slick checkpoints               # same as `status`
    slick checkpoints status        # total size, project count, breakdown
    slick checkpoints list          # per-project checkpoint counts + workdir
    slick checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    slick checkpoints clear [-f]    # nuke the entire base (asks first)
    slick checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    slick checkpoints
    slick checkpoints prune --retention-days 3 --max-size-mb 200
    slick checkpoints clear -f
...
from slick_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: slick checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``slick checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `slick checkpoints` → status
