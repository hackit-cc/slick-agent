"""Shell completion script generation for slick CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_slick_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Slick Agent bash completion
# Add to ~/.bashrc:
#   eval "$(slick completion bash)"

_slick_profiles() {{
    local profiles_dir="$HOME/.slick/profiles"
...
_slick_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_slick_profiles)" -- "$cur"))
        return
    fi
...
complete -F _slick_completion slick
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef slick
# Slick Agent zsh completion
# Add to ~/.zshrc:
...
_slick_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.slick/profiles" ]]; then
        profiles+=($HOME/.slick/profiles/*(N/:t))
    fi
...
_slick() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_slick_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'slick command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Slick Agent fish completion",
        "# Add to your config:",
        "#   slick completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __slick_profiles",
        "    echo default",
        "    if test -d $HOME/.slick/profiles",
        "        for d in $HOME/.slick/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c slick -f -s p -l profile"
        " -d 'Profile name' -xa '(__slick_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__slick_profiles)' -d 'Profile name'"
                )
