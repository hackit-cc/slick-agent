...
Hackit Portal). This module fetches, validates, and caches that manifest,
...
2. ``get_curated_openrouter_models()`` / ``get_curated_hackit_models()`` —
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/hackit-cc/slick-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def get_catalog(*, force_refresh: bool = False) -> dict[str, Any]:
    ...
    # this via get_curated_hackit_model_ids on every open) never block on the
...
def get_curated_hackit_models() -> list[str] | None:
    """Return Hackit Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("hackit")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
