"""
Backup and import commands for slick CLI.

`slick backup` creates a zip archive of the entire ~/.slick/ directory
(excluding the slick-agent repo and transient files).

`slick import` restores from a backup zip, overlaying onto the current
SLICK_HOME root.
"""
...
from slick_constants import get_default_slick_root, get_slick_home, display_slick_home
from utils import (
    _preserve_file_mode,
...
from slick_cli.sizefmt import format_bytes as _format_size
...
# Where ``slick backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``slick-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/slick-agent/``
...
# SLICK_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "slick-agent",     # the codebase repo — re-clone instead
    ...
    "browser-profile",
    # Python dependency trees (plugin / MCP-server venvs under SLICK_HOME) —
...
# File names that ``slick import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting it from the Hackit portal
...
# Reserved archive subtree for provider state that lives OUTSIDE SLICK_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Slick backup slot."""
...
@contextmanager
def _backup_operation_lock(slick_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = slick_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Slick backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside SLICK_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to slick root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``slick-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/slick-agent/`` — must be preserved.
        if part == "slick-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside SLICK_HOME.
...
# to the user as a hung `slick update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # slick_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Slick home directory."""
    slick_root = get_default_slick_root()

    if not slick_root.is_dir():
        print(f"Error: Slick home directory not found at {slick_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(slick_root):
            _run_backup_locked(args, slick_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, slick_root: Path) -> None:
    ...
    try:
        if args.output:
            ...
            if out_path.is_dir():
                ...
                out_path = out_path / f"slick-backup-{stamp}.zip"
        else:
            stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
            out_path = Path.home() / f"slick-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_slick_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(slick_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(slick_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``slick-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "slick-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(slick_root)
    ...
    # outside SLICK_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_slick_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: slick import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Slick backup.
    ...
    # Look for telltale files that a slick home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Slick backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.slick/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a slick dir name
        if prefix in {".slick", "slick"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``slick import``, which is the
    ...
    so without the chown a ``sudo slick import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``slick import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Slick' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_slick_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (slick_root / "config.yaml").exists()
        has_env = (slick_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Slick configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under SLICK_HOME.
            ...
            # Hackit portal. Matched by basename so both the root profile and
            # named profiles (profiles/<name>/gateway_state.json) are covered.
            if Path(rel).name in _IMPORT_SKIP_NAMES:
                skipped_runtime.append(rel)
                continue

            target = slick_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(slick_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_slick_home()}."
            )
        ...
        profiles_dir = slick_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # slick_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: slick profile list  (after installing slick)")

        # Guidance
        print()
        if not (slick_root / "slick-agent").is_dir():
            print("Note: The slick-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: slick update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  slick -p {pname} gateway install")
        ...
        try:
            from slick_cli.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your Slick configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and slick backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to SLICK_HOME).
...
# — `slick update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $SLICK_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside SLICK_HOME and the entry is silently
...
def _quick_snapshot_root(slick_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: slick snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(slick_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside SLICK_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``slick update``.
    ...
    Args:
        ...
        slick_home: Override for the Slick home directory (tests).
...
def restore_cron_jobs_all_profiles(
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_cron_jobs_if_emptied(snap_id, slick_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(slick_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for slick backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_slick_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, slick_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(slick_root):
            return _write_full_zip_backup_locked(out_path, slick_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, slick_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``slick_root`` to ``out_path``.
...
def _pre_update_backup_dir(slick_home: Optional[Path] = None) -> Path:
    home = slick_home or get_slick_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of SLICK_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<SLICK_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``slick update``) should continue even if the backup fails.
    """
    slick_root = slick_home or get_default_slick_root()
    if not slick_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(slick_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, slick_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `slick claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of SLICK_HOME under ``backups/`` before a
    ``slick claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``slick import <archive>``.  Writes to
    ``<SLICK_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `slick import` and the
