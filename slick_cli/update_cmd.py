"""Slick update pipeline — extracted from ``slick_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``slick_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``slick_cli.main`` reference — so existing call sites and test monkeypatches
that target ``slick_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``slick_cli.main``.
...
Imports are one-way: ``slick_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``slick_cli.main`` reference.

    Lets callers keep patching ``slick_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``slick_cli.main`` -> ``slick_cli.update_cmd`` stays one-way
    at import time.
    """
    from slick_cli import main
...
#: ``_purge_stale_slick_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "slick_cli.slick_logging",
    }
)


def _purge_stale_slick_modules() -> None:
    """Evict every cached Slick module after the checkout changed in-place.

    ``slick update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from slick_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``slick_cli.gateway`` does
    ``from slick_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Slick
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Slick module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Slick modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``slick update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``slick update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``slick_cli.config_defaults``,
    ``slick_cli.config``, and ``slick_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``slick_cli._subprocess_compat`` and
    ``slick_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "slick_cli.config_defaults",
        "slick_cli.config",
        "slick_cli.config_migrations",
        "slick_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from slick_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from slick_cli.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``slick update`` historically
    ...
    caller): scope config reads/writes via the context-local SLICK_HOME
    ...
    try:
        ...
        from slick_cli.profiles import _get_profiles_root, _PROFILE_ID_RE

        active_home = get_process_slick_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_slick_home_override(entry)
            try:
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'slick config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'slick config migrate' to retry.")
        ...
        if assume_yes:
            ...
            try:
                ...
                print(
                    ...
                    "Run 'slick config migrate' manually to configure."
                )
                response = "n"
        ...
        if response in {"", "y", "yes", "auto"}:
            ...
            if (gateway_mode or assume_yes or response == "auto") and missing_env:
                print("  ℹ API keys require manual entry: slick config migrate")
        else:
            print()
            print("Skipped. Run 'slick config migrate' later to configure.")
    else:
    ...
    # context-local SLICK_HOME override (never os.environ — other
    ...
    # it and warn loudly.
    try:
        from slick_cli.backup import restore_cron_jobs_if_emptied
    ...
    try:
        from slick_cli.backup import restore_cron_jobs_all_profiles
...
# Critical files that Slick must be able to import immediately after an
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``slick.exe`` has
    ...
    Skipping is safe because Slick pins its editable finder to a *static*
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``slick`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="slick-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``slick update`` can be driven by a
    ...
    """
    from slick_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from slick_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('slick_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``slick update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `slick update`.
    ...
    print("  Preview now:  slick curator run --dry-run")
    print("  Pause it:     slick curator pause")
    print(
        "  Docs:         https://slick-agent.sophostechne.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `slick update`.
    ...
    try:
        ...
        return
    db_path = get_slick_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `slick sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    slick sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  slick sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``slick update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``slick update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: slick curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from slick_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    slick dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``slick_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.slick-update-staging"
    backup = f"{dst}.slick-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `slick update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``slick_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``slick update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && slick update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("SLICK_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== slick-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Slick launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["slick.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Slick")
    print("  Desktop, a gateway, or another slick REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `slick` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``slick.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.slick-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".slick-update-staging", ".slick-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `slick update`.")
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second Slick process the updater's drain
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from slick_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity

    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (slick gateway stop), "
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update Slick Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `slick update "
            f"--branch {branch}`, or update against main with `slick update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    zip_url = (
        f"https://github.com/SophosTechne/slick-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="slick-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"slick-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for slick-agent itself
        ...
        # GitHub ZIPs extract to slick-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"slick-agent-{branch}")
        ...
        try:
            for item in entries:
                ...
                # Slick.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `slick update` to retry; if the agent won't start, "
            "reinstall from https://slick-agent.sophostechne.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    from slick_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if uv_bin:
        ...
        from slick_cli.managed_uv import managed_python_env
    ...
    if not import_ok:
        ...
        print("  Re-run `slick update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: slick skills list-modified  "
    ...
    try:
        from slick_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from slick_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_slick_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_slick_home())
    ...
    try:
        from slick_cli.update_receipt import finalize_update_receipt
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "slick-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``slick update --keep-stash`` (the desktop updater's mode): the
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Slick behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes slick completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Slick couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Slick couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Slick behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Slick couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Slick couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/SophosTechne/slick-agent.git",
    "git@github.com:SophosTechne/slick-agent.git",
    "https://github.com/SophosTechne/slick-agent",
    "git@github.com:SophosTechne/slick-agent",
}

OFFICIAL_REPO_URL = "https://github.com/SophosTechne/slick-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_slick_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_slick_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(
    ...
    *,
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Slick repository.")
        print("  This means you may miss updates from SophosTechne/slick-agent.")
        print()
        ...
        if assume_yes or (
            ...
            print(
                "  Add it later with: git remote add upstream https://github.com/SophosTechne/slick-agent.git"
            )
            return False
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/SophosTechne/slick-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/SophosTechne/slick-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``slick update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from slick_constants import get_default_slick_root

    default_home = get_default_slick_root()
...
# ``fleet_restart_pending`` lives under SLICK_HOME (not next to the venv).
...
def _fleet_restart_pending_marker_path() -> Path:
    """SLICK_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    return get_slick_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    try:
        from slick_cli.build_info import get_code_identity
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `slick update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `slick update` or `slick gateway restart`.",
            file=stream,
        )
...
def _restart_systemd_gateway_units_best_effort(failed: list) -> None:
    """Best-effort ``systemctl restart`` of every slick-gateway/serve unit."""
...
def _run_pending_fleet_restart() -> bool:
    ...
    try:
        _m()._purge_stale_slick_modules()
    except Exception:
        pass
    try:
        from slick_cli.gateway import (
            find_gateway_pids,
            is_macos,
            is_windows,
    ...
    try:
        ...
        if is_windows():
            try:
                from slick_cli import gateway_windows
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``slick update``, finish a skipped restart.
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: slick gateway restart")
    sys.exit(1)
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "slick.exe"
    lines = ["✗ Another slick.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Slick Desktop, exit any open `slick` REPLs, and")
    lines.append("  stop the gateway (`slick gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `slick update --force` if you've already")
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from slick_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``slick update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a Slick Desktop backend child, an unrelated ``slick`` REPL)
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``slick tools``."""
    try:
        from slick_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Slick Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``slick tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Slick Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Slick Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``slick update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `slick update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `slick update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Slick' editable-install extras or
    ...
    try:
        from slick_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from slick_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$SLICK_HOME/bin/uv``, but on Termux the official
    ...
    """
    from slick_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `slick update` is exactly the step
...
def _npm_lockfile_changed(slick_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `slick
    ...
    try:
        ...
        cache_file = slick_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(slick_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    slick update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `slick update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from slick_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `slick update`.")
            failed = []
            if any(
    ...
    # Slick profile using this checkout. Keep one per-checkout cache under the
    # shared Slick root rather than rerunning npm once per named profile.
    shared_slick_root = get_default_slick_root()
    ...
    # `slick update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_slick_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `slick update`.")
        return list(labels)
    ...
    nixos_env = with_slick_node_path(_m()._nixos_build_env())
    ...
    # `slick update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_slick_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.slick/logs/update.log`` only, never the terminal.

    During ``slick update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``slick update --check``: fetch and report without installing.
    ...
    from slick_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from slick_cli.banner import _github_compare_behind
            from slick_cli.config import recommended_update_command
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``slick update`` without requiring a reinstall.  The
    ...
    ``slick`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/slick.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v slick'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/slick, code at /usr/local/lib/slick-agent).
    fhs_link = Path("/usr/local/bin/slick")
    ...
    path_comment = (
        "# Slick Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``slick-acp`` launcher next to the ``slick`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``slick update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``slick-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Slick as not installed even when it is.

    The shim simply delegates to the sibling ``slick`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``slick`` / ``slick-acp``
    launchers into the managed binary dir ``$SlickHome\bin`` and puts THAT
    ...
    ``slick_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``slick-acp`` is already present next to the
    ``slick`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        slick_cmd = bin_dir / "slick"
        acp_cmd = bin_dir / "slick-acp"
        try:
            if not (slick_cmd.is_file() or slick_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of SLICK_HOME under
      ``backups/`` (restorable via ``slick import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from slick_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from slick_constants import
        # get_slick_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from slick_cli.config import get_slick_home as _get_home
        ...
        try:
            from slick_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from slick_cli.update_receipt import record_step
    ...
    try:
        from slick_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from slick_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_slick_home so the user sees ~/.slick/...
    try:
        from slick_constants import get_slick_home, display_slick_home

        home = get_slick_home()
        try:
            display_path = f"{display_slick_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  slick import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``slick update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``slick update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run slick from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.slick-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".slick-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The slick.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m slick_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``slick
    ...
    for proc in proc_iter:
        ...
        # the `-m slick_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``slick_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``slick.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``slick`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `slick` launch will complete it")
...
def _slick_holder_subcommand(cmdline: str) -> str | None:
    """The actual Slick SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``slick_cli.main`` / ``slick(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("slick_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("slick", "slick.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``slick dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other Slick processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← Slick backend (if the Desktop app is open, close it)",
        "dashboard": "  ← slick dashboard (stop it: slick dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    for pid, name, cmdline in matches[:6]:
        sub = _slick_holder_subcommand(cmdline)
    ...
    lines.append(
        "  Close the Slick desktop app / other Slick terminals, then re-run:"
    )
    lines.append("    slick update")
    lines.append("  (or use `slick update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``slick update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from slick_cli._scan_venv_blockers import _is_pausable_gateway
...
def _ledger_manual_serve_holders(
    ...
    Slick spawner). Returns the full ledger entries so the relauncher can
    rebuild the launch command from structured host/port/profile instead of
    parsing argv.
    """
    try:
        from slick_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    if skipped or failed:
        print(
            ...
            "restart them manually (slick serve --host <ip> --port <port>)."
        )
    try:
        from slick_cli.update_receipt import record_step
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning slick-setup, and the update-in-progress marker
    ...
    the update with "Slick is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Slick backend (``slick_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.slick-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "slick_cli.main" in argv_low and (
...
def _ledger_reapable_backend_pids(
    ...
    (``slick_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Slick ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``slick.exe``) is open, NOTHING legitimate
    ...
    - Only Slick backends (``slick_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``slick.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.slick-runtime`` interpreter child alive and holding the install open
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``slick serve`` / ``slick dashboard`` argv.
    ...
    """
    if "slick_cli.main" not in (cmdline or "").lower():
        return False
    return _slick_holder_subcommand(cmdline) in ("serve", "dashboard")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    slick.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Slick...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m slick_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: slick gateway run")
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    ``slick gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from slick_cli import gateway_windows
        from slick_cli.gateway import find_gateway_pids
    except Exception as exc:
        raise RuntimeError(
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``slick-gateway*.service``/``slick-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("slick-serve")`` alone would also accept the
        # unrelated ``slick-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "slick-gateway.service"
            or unit.startswith("slick-gateway-")
            or unit == "slick-serve.service"
            or unit.startswith("slick-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``slick-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``slick-serve*`` units (#83438) don't, so sending them
    ...
    (``slick-gateway-helper`` is fine — profile units are
    ``slick-gateway-<profile>`` — but ``slick-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "slick-gateway" or svc_name.startswith("slick-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from slick_cli.gateway import is_macos
    ...
    print("    slick gateway status")
    if any(not name.startswith("ai.slick.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.slick.") for name in ordered):
...
def _restart_launchd_gateway_after_update(
    *, supervision_verify: bool = True
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: slick gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: slick gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.slick.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``slick_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    that ``slick-gateway-foobar.service`` was already restarted.  These are
    ...
    return name in {
        f"slick-gateway-{profile}",
        f"ai.slick.gateway-{profile}",
        f"gateway-{profile}",
    }
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``slick_cli.gateway`` in the failing
...
def _recover_gateway_restart_after_abort(
    ...
    ``slick update`` normally performs the fleet restart in the interpreter
    ...
    command = [
        sys.executable,
        "-m",
        "slick_cli.update_restart_recovery",
        "--stdin",
    ]
    env = os.environ.copy()
    env["SLICK_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_SLICK_GATEWAY", "SLICK_GATEWAY", "SLICK_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``slick_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``slick update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``slick-setup.exe``) executes
    ``$SLICK_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_slick_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``slick update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``slick update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `slick desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "slick_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Slick-managed Node on PATH: when
    # `slick update` runs inside the desktop updater chain (Desktop →
    # slick-setup → slick update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from slick_constants import with_slick_node_path

    build_env = with_slick_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `slick desktop` to retry)")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from slick_cli.update_receipt import begin_update_receipt
    ...
    # running Slick runtime, its supervisor, and its running code version —
    ...
    # On Windows, abort early if another slick.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        scripts_dir = _m()._venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_slick_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `slick serve` backend) will keep .pyd
    ...
    # the slick.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Slick backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Slick backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .slick-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Slick is still
        ...
        if _venv_holders:
            ...
            # `slick serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Slick backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_slick_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Slick backend process(es) "
    ...
    # process by design (a CLI `slick update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by slick.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Slick windows/gateways and re-run: slick update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Slick gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in slick_cli/config.py bricked
        # every user who ran ``slick update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``slick
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``slick update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``slick update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``slick doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && slick update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # ``slick update`` can catch up even when git is already up to date.
        ...
        # the old bytecode (e.g. get_slick_home added to slick_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``slick`` launch finishes the install
        ...
        # symbols in slick_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `slick` again "
        ...
        if not import_ok:
            ...
            print("    Run `slick update` again — if it persists, reinstall:")
            print("    https://slick-agent.sophostechne.com")
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if Slick re-prompts for permissions you already "
                "granted (toggle shows ON), the stored grant is stale — run "
                "`tccutil reset ScreenCapture com.sophostechne.slick` (repeat "
        ...
        try:
            from slick_cli.macos_tcc_anchor import ensure_tcc_anchor
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_slick_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.slick/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit SLICK_HOME so
        # it is not affected by sync_skills()'s module-level SLICK_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SLICK_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from slick_cli.profiles import backfill_profile_envs
        ...
        # check `slick curator status`. Self-stamps after printing so it
        ...
        # Self-heal the slick-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Slick on PATH without
        # a reinstall.  No-op on Windows (the launcher migration below owns
        # that) and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("slick-acp launcher self-heal failed: %s", e)

        # Migrate the Windows slick launchers to the managed binary dir
        # (the default Slick root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (slick-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `slick`
        ...
        # the managed clone under the default Slick root).
        try:
            from slick_cli._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``slick update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from slick_cli.tools_config import install_cua_driver
                ...
                # driver) keeps the installed version — `slick update`
                # must stay fast; `slick computer-use install --upgrade`
        ...
        # When running as ``slick update --gateway`` (spawned by the gateway's
        ...
        #
        # Purge stale cached Slick modules FIRST: the import below pulls
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... slick-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... slick-gateway*)
            ...
            try:
                from slick_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all slick-gateway* units (default + profiles) plus
            # slick-serve* units (the Desktop app's backend, #83438).
            if supports_systemd_services():
                try:
                    _ensure_user_systemd_env()
                except Exception:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: slick gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # slick-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `slick update` restart it automatically, allow\n"
            ...
            # Restart EVERY ai.slick.gateway* LaunchAgent, not only the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: slick gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: slick -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_slick_home() / ".update_exit_code"
                    try:
            ...
            try:
                from slick_cli.update_receipt import record_gateway_restart
        ...
        # Warn if legacy Slick gateway unit files are still installed.
        # When both slick.service (from a pre-rename install) and the
        # current slick-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `slick update` surfaces the issue until the user migrates.
        try:
            from slick_cli.gateway import (
                has_legacy_slick_units,
                _find_legacy_slick_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_slick_units():
                print()
                print("⚠ Legacy Slick gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_slick_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (slick.service) fight the current")
                print("  slick-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    slick gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes slick-serve*,
        ...
        print("  slick model              # Select provider and model")
        ...
        try:
            from slick_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import slick_cli.update_receipt as _ur
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``slick update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from slick_cli.main import main; main()" update --yes'
                    )
            try:
                from slick_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``slick_cli.gateway`` no longer imports,
