"""Per-terminal session breadcrumbs for ``slick -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$SLICK_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``slick -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_slick_home() / "terminal-sessions"
