"""``slick gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Hackit Portal access token from the existing login
     (``~/.slick/auth.json``) — the same path ``slick dashboard register``
     uses (``resolve_hackit_access_token``). This proves *which Hackit org (tenant)*
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.slick/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC or Hackit Portal).
    ...
    self-hosted-IdP path; otherwise Hackit Portal). Raises RuntimeError on failure.
    """
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your Hackit Portal "
                "token could not be verified — try `slick auth add hackit` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from slick_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `slick gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    gapped / self-hosted-IdP, NO Hackit Portal); otherwise the Hackit Portal token.
    try:
    ...
    #    save_env_value writes them to ~/.slick/.env (0600 dir) and never logs.
    to_write = {
    ...
    # process is launched under (load_slick_dotenv exports that .env), so
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    in slick_cli/gateway.py. Best-effort: any failure to determine the
    ...
    try:
        ...
        default_root = Path(get_default_slick_root()).resolve()
        home = Path(get_slick_home()).resolve()
        try:
