...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Sophos Portal access token from the existing login
     (``~/.slick/auth.json``) — the same path ``slick dashboard register``
     uses (``resolve_sophos_access_token``). This proves *which Sophos org (tenant)*
...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC or Sophos Portal).
    ...
    self-hosted-IdP path; otherwise Sophos Portal). Raises RuntimeError on failure.
    """
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your Sophos Portal "
                "token could not be verified — try `slick auth add sophos` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    #    gapped / self-hosted-IdP, NO Sophos Portal); otherwise the Sophos Portal token.
    try:
