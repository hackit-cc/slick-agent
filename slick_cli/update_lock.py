"""Cross-process mutual exclusion for in-flight Slick updates.

Three different surfaces can start an update of the same install tree:

* ``slick update`` from a terminal,
* the dashboard's Update button (``POST /api/slick/update`` →
  ``_spawn_slick_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``slick-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``slick update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <SLICK_HOME>/.slick-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``slick update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Slick is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``slick-setup`` binary under ``~/.slick`` is only refreshed by a full
  installer run (``copy_self_to_slick_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Slick is still running") on every
  GUI update forever, with no Slick process actually running.
"""
...
MARKER_NAME = ".slick-update-in-progress"

# Set by an orchestrating updater (the Tauri `slick-setup --update` flow) to
# its own pid before spawning `slick update` as a child stage. The parent
...
HANDOFF_PID_ENV = "SLICK_UPDATE_HANDOFF_PID"
...
# to show "Slick is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Slick home (never the context-local profile override):
    the Rust updater resolves ``$SLICK_HOME`` or the platform default, and the
    ...
    return get_process_slick_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``slick update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``slick-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Slick update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `slick update` as a stage): we run under ITS claim
