"""``slick logs`` — view and filter Slick log files.
...
Usage examples::
    ...
    slick logs                    # last 50 lines of agent.log
    slick logs -f                 # follow agent.log in real time
    slick logs errors             # last 50 lines of errors.log
    slick logs gateway -n 100    # last 100 lines of gateway.log
    slick logs gui -f            # follow gui.log (dashboard/pty/ws)
    slick logs desktop -f        # follow desktop.log (Electron app boot/backend)
    slick logs --level WARNING    # only WARNING+ lines
    slick logs --session abc123   # filter by session ID substring
    slick logs --component tools  # only tool-related lines
    slick logs --since 1h         # lines from the last hour
    slick logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_slick_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Slick runs — try 'slick chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_slick_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_slick_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_slick_home()}/logs/")
        return

    print(f"Log files in {display_slick_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'slick chat' to generate logs)")
