"""
Unified tool configuration for Slick Agent.

`slick tools` and `slick setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.slick/config.yaml under
...
from slick_cli.sophos_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    SophosSubscriptionFeatures,
    apply_sophos_managed_defaults,
    get_sophos_subscription_features,
)
from slick_cli.sophos_account import format_sophos_portal_entitlement_message
from tools.tool_backend_helpers import SOPHOS_MANAGED_PROVIDER, fal_key_is_configured
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``slick tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`slick_cli._subprocess_compat.windows_hide_flags`) suppresses it
    ...
    """
    from slick_cli._subprocess_compat import windows_hide_flags
...
from slick_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _SLICK_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `slick tools` → Video Generation, which walks
...
# `slick tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `slick tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from slick_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `slick tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``slick tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from slick_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from slick_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``slick tools`` checklist actually offers
    for ``platform``.
    ...
    so ``slick tools`` never claims to add or remove a toolset the user was
...
from slick_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            {
                "name": "Sophos Subscription",
                "badge": "subscription",
                "tag": "Managed OpenAI TTS billed to your subscription",
                "env_vars": [],
                "tts_provider": "openai",
                "requires_sophos_auth": True,
                "managed_sophos_feature": "tts",
    ...
    "web": {
        ...
        #   - "Sophos Subscription" — managed Firecrawl billed via Sophos
        #     subscription (requires_sophos_auth + override_env_vars).
    ...
    "image_gen": {
        ...
        #   - "Sophos Subscription" — managed FAL billed via the Sophos
        #     subscription (requires_sophos_auth + override_env_vars).
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_sophos_auth": True,
                "managed_sophos_feature": "image_gen",
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "Sophos Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the Sophos Portal.  Plugin-backed
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_sophos_auth": True,
                "managed_sophos_feature": "video_gen",
                "override_env_vars": ["FAL_KEY"],
                # The underlying plugin backend — when the user picks
                # "Sophos Subscription" we set video_gen.provider = "fal"
    ...
    "x_search": {
        ...
        "setup_note": (
            "Slick routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        # backend, never on the paid Sophos Subscription gateway row:
        #   - "Local Browser" — non-cloud option, no CloudBrowserProvider.
        #   - "Sophos Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Sophos subscription (requires_sophos_auth +
        ...
        "providers": [
            ...
            {
                "name": "Sophos Subscription (Browser Use cloud)",
                "badge": "subscription",
                "tag": "Managed Browser Use billed to your subscription",
                "env_vars": [],
                "browser_provider": "browser-use",
                "requires_sophos_auth": True,
                "managed_sophos_feature": "browser",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set SLICK_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "SLICK_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "SLICK_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "SLICK_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "SLICK_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "SLICK_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `slick model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("SLICK_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``SLICK_CUA_DRIVER_CMD`` is not obliged to answer
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Slick telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Slick' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $SLICK_HOME/bin is never on PATH, so a bare which()
    # misses the uv Slick installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``slick update`` and
      by ``slick computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``slick update`` sets
    ...
    a further ~600s wait on Windows). ``slick computer-use install
    ...
    ``slick update`` already prints a contextual line before its update
    ...
    ``slick update`` and must not inherit the explicit install command's
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `slick update` calls this
    ...
    override = os.environ.get("SLICK_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    SLICK_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails Slick' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Slick' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `slick update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Slick cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("SLICK_CUA_DRIVER_CMD", "").strip():
            _print_info(
        ...
        if is_windows and require_confirmed_update:
            ...
            _print_info(
                ...
                "slick computer-use install --upgrade"
            )
            return False
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `slick computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `slick update` (require_confirmed_update=True) keeps the
    ...
    # `slick computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: slick computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: slick computer-use doctor")
            return False
    if ok and before:
        try:
...
# Optional refreshes launched by ``slick update`` are quiet and unattended.
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    # Unattended refreshes (installer_timeout set by `slick update`) fail
    ...
    def _reap_after_timeout(proc):
        ...
        ceiling stops bounding anything and ``slick update`` hangs past its
    ...
    try:
        # When not verbose (e.g. `slick update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `slick update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.slick/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Slick process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Firecrawl, and the Sophos-managed cloud rows all execute through
    ...
    check — only the Slick-managed ``$SLICK_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Slick installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # — it resolves lazily via npx (or a global/Slick-managed install)
        ...
        # (PATH -> Homebrew/Slick-managed node -> npx) rather than a bare
        # shutil.which — Slick-managed-Node-only setups resolve agent-browser
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/sophostechne/slick-agent:latest"
            )
            return

        # browser_cmd was already resolved above (same PATH -> Homebrew ->
        # Slick-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Slick-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.slick/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.slick/config.yaml")
        ...
        # Run the full `slick auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.slick/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from slick_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: slick auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `slick auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: slick auth spotify")
        except Exception as exc:
        ...
        try:
            from slick_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: slick plugins enable observability/langfuse")
        _print_info("    Restart Slick for tracing to take effect.")
        _print_info("    Verify: slick plugins list")
        ...
        try:
            ...
            _print_info("    Run later: slick auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `slick auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: slick auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: slick auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``slick tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``slick tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: slick tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``slick-discord``, which contains those
...
#:
#: Saving ``slick tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[slick-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``slick tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"slick-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (slick-acp, slick-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``slick-discord``) is an opt-in to the platform's native default-off
    ...
    # explicit config like ``[slick-cli, a2a]`` must survive filtering just
    ...
    # "slick-cli" (which include all _SLICK_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[slick-cli, spotify]`` after enabling Spotify via ``slick
        ...
        # (e.g. "slick-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``slick tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `slick tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"slick-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("slick-")}
    ...
    # shipped as a bundled plugin but user must opt in via `slick tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `slick tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by slick tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `slick config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `slick` instead of
    # `slick-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `slick update`/`slick doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `slick tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (slick-cli, slick-telegram, etc.)
    ...
    # Opening `slick tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    features: Optional[SophosSubscriptionFeatures] = None,
) -> bool:
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "stt", "browser"}:
        if features is None:
            features = get_sophos_subscription_features(
                config, force_fresh=force_fresh
            )
        feature = features.features.get(ts_key)
        if feature and (feature.available or feature.managed_by_sophos):
            return True
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    from slick_constants import slick_home_key

    scope = slick_home_key()
...
# ("Sophos Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Sophos Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    Sophos-managed Tool Gateway rows (``managed_sophos_feature``) are always
    shown — even to logged-out / unentitled users — so the picker advertises
    that the capability exists.  Selecting one drives an inline Sophos Portal
    login + entitlement check (see ``_configure_provider``); the row only
    *activates* the gateway once paid access is confirmed.
    """
    if features is None:
        features = get_sophos_subscription_features(config, force_fresh=force_fresh)
    ...
    for provider in cat.get("providers", []):
        # Sophos-managed Tool Gateway rows stay visible regardless of auth —
        # selecting one drives an inline Portal login. A `requires_sophos_auth`
        # row that is NOT a managed gateway feature (pure pre-auth UX) is
        # still hidden until the user is logged in.
        if (
            provider.get("requires_sophos_auth")
            and not provider.get("managed_sophos_feature")
            and not features.sophos_auth_present
        ):
            continue
        ...
        if (
            pool_only
            and provider.get("managed_sophos_feature") == "video_gen"
    ...
    # later) so the picker lists them alongside FAL / Sophos Subscription.
    ...
    # remaining hardcoded rows ("Sophos Subscription", "Firecrawl
    ...
    # the hardcoded "Sophos Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_sophos_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Deprecated: Sophos Tool Gateway rows are no longer hidden.

    Previously this returned a "log in / upgrade" banner shown above a
    category when its Sophos-managed rows were filtered out for unentitled
    ...
    (``ensure_sophos_portal_access``). Kept as a no-op so call sites stay simple;
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `slick tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``slick tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``slick tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from slick_cli.sophos_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``slick tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from slick_cli.sophos_subscription import _has_agent_browser
...
def provider_readiness_status(
    ...
    *,
    ...
    - ``"needs_auth"``  — needs a sign-in: Sophos Portal login/entitlement for
    ...
    zero-env-var row, including logged-out Sophos Subscription rows).

    ``features`` (a ``SophosSubscriptionFeatures``) can be passed to avoid
    ...
    managed_feature = provider.get("managed_sophos_feature")
    if provider.get("requires_sophos_auth") or managed_feature:
        if features is None:
            features = get_sophos_subscription_features(config)
        if not features.sophos_auth_present:
            return "needs_auth"
        if managed_feature:
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_sophos_message = _hidden_sophos_gateway_message(
        cat,
        config,
        f"the Sophos Subscription provider for {name}",
        force_fresh=force_fresh,
    )
    ...
    if len(providers) == 1:
        ...
        if hidden_sophos_message:
            for line in hidden_sophos_message.splitlines():
        ...
        # When the user is logged into Sophos, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of Sophos.
        try:
            _sophos_logged_in = bool(
                get_sophos_subscription_features(
                    config,
                    force_fresh=force_fresh,
                ).sophos_auth_present
        ...
        for p in providers:
            ...
            # Mark Sophos-managed entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via Sophos Portal" hint so
            ...
            sub_marker = ""
            if p.get("managed_sophos_feature"):
                if _sophos_logged_in:
                    sub_marker = "  ★ Included with your Sophos subscription"
                else:
                    sub_marker = "  ★ via Sophos Portal (login on select)"
...
def _is_provider_active(
    ...
    *,
    ...
    if plugin_name and not provider.get("managed_sophos_feature"):
        # Managed (Sophos-subscription) entries fall through to the
    ...
    if video_plugin_name and not provider.get("managed_sophos_feature"):
    ...
    if managed_feature:
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if configured_provider not in {None, "", "fal", SOPHOS_MANAGED_PROVIDER}:
                    return False
                if (
                    configured_provider != SOPHOS_MANAGED_PROVIDER
                    and image_cfg.get("use_gateway") is not None
                    and not is_truthy_value(image_cfg.get("use_gateway"), default=False)
                ):
                    return False
            return feature.managed_by_sophos
        ...
        if provider.get("tts_provider"):
            return (
                feature.managed_by_sophos
                and cfg_get(config, "tts", "provider")
                in {provider["tts_provider"], SOPHOS_MANAGED_PROVIDER}
            )
        if provider.get("stt_provider"):
            return (
                feature.managed_by_sophos
                and cfg_get(config, "stt", "provider")
                in {provider["stt_provider"], SOPHOS_MANAGED_PROVIDER}
            )
        if "browser_provider" in provider:
            ...
            return feature.managed_by_sophos and current in {
                provider["browser_provider"],
                SOPHOS_MANAGED_PROVIDER,
            }
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return (
                feature.managed_by_sophos
                and current in {provider["web_backend"], SOPHOS_MANAGED_PROVIDER}
...
def _select_plugin_image_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    ``use_gateway=True`` marks a provider picked through the Sophos-managed
    flow: the stored selection becomes ``image_gen.provider: sophos`` (the
    ...
    img_cfg["provider"] = SOPHOS_MANAGED_PROVIDER if use_gateway else plugin_name
...
# Kept in sync with the dashboard selects (slick_cli/web_server.py
...
def _select_plugin_video_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    ``video_gen.provider: sophos``; BYOK picks store the plugin name; any
    ...
    vid_cfg["provider"] = SOPHOS_MANAGED_PROVIDER if use_gateway else plugin_name
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    post-setup hooks, gate on Sophos auth, or run interactive model pickers.
    ...
    category. Managed "Sophos Subscription" rows write ``sophos``; BYOK rows
    ...
    legacy shim (use_gateway: true ⇒ sophos) cannot override the new choice.
    """
    def _set_selection(section_key: str, name_key: str, vendor_value) -> None:
        ...
        section[name_key] = (
            SOPHOS_MANAGED_PROVIDER if managed_feature else vendor_value
        )
    ...
    # image_gen/video_gen "Sophos Subscription" rows carry only
    # managed_sophos_feature) still persist the "sophos" selection.
    if managed_feature and managed_feature not in {"web", "tts", "stt", "browser"}:
        section = config.setdefault(managed_feature, {})
        if isinstance(section, dict):
            section["provider"] = SOPHOS_MANAGED_PROVIDER
            section.pop("use_gateway", None)
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on Sophos Portal auth, or run interactive model
    ...
    # flow). Managed picks store the "sophos" selection.
    plugin_name = provider.get("image_gen_plugin_name")
    if plugin_name:
        img_cfg = config.setdefault("image_gen", {})
        if not isinstance(img_cfg, dict):
            ...
            config["image_gen"] = img_cfg
        img_cfg["provider"] = (
            SOPHOS_MANAGED_PROVIDER if managed_feature else plugin_name
        )
        img_cfg.pop("use_gateway", None)
    ...
    if video_plugin:
        ...
        if not isinstance(vid_cfg, dict):
            ...
            config["video_gen"] = vid_cfg
        vid_cfg["provider"] = (
            SOPHOS_MANAGED_PROVIDER if managed_feature else video_plugin
        )
        vid_cfg.pop("use_gateway", None)
...
def _configure_provider(
    ...
    *,
    ...
    # Sophos-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid Sophos
    ...
    # "enable all tools" prompt (that lives in `slick model`).
    if managed_feature:
        from slick_cli.sophos_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
            ensure_sophos_portal_access,
        )

        if not ensure_sophos_portal_access(
            capability=f"{provider.get('name', 'the Sophos Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — Sophos Portal access is required for this backend."
            )
            return

    # Pure pre-auth UX rows (requires_sophos_auth without a managed gateway
    # feature) keep the old gate. Managed rows are handled by the inline
    # login above, so don't double-check them here.
    if provider.get("requires_sophos_auth") and not managed_feature:
        features = get_sophos_subscription_features(config, force_fresh=force_fresh)
        entitled = bool(
            features.account_info and features.account_info.paid_service_access is True
        )
        if not features.sophos_auth_present or not entitled:
            message = format_sophos_portal_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'Sophos Subscription')}",
            )
            _print_warning(
                f"  {message or 'Sophos Subscription is only available after logging into Sophos Portal.'}"
            )
            return
    ...
    if provider.get("tts_provider"):
        ...
        tts_cfg["provider"] = (
            SOPHOS_MANAGED_PROVIDER if managed_feature else provider["tts_provider"]
        )
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Sophos subscription.")
        ...
        if backend:
            ...
            # explicit selection: "sophos" for a managed row, "fal" for BYOK.
            img_cfg = config.setdefault("image_gen", {})
            if isinstance(img_cfg, dict):
                img_cfg["provider"] = (
                    SOPHOS_MANAGED_PROVIDER if managed_feature else "fal"
                )
                img_cfg.pop("use_gateway", None)
    ...
    # Sophos-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to Sophos.
    _show_portal_hint = False
    if env_vars and not managed_feature and not provider.get("requires_sophos_auth"):
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_sophos_feature") for sib in _providers
                ):
                    _has_managed_sibling = True
                    break
            if _has_managed_sibling:
                _features = get_sophos_subscription_features(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.sophos_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through Sophos Portal subscription.")
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``slick model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``slick model`` aux-task
    ...
    """
    try:
        from slick_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `slick model`, then re-run this."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Sophos Portal login + entitlement gate as _configure_provider:
    ...
    # provider string per category — "sophos" for managed rows, the vendor name
    # for BYOK rows — and drops any legacy use_gateway key so the read-time
    # shim (use_gateway: true ⇒ sophos) cannot override the fresh pick.
    if provider.get("tts_provider"):
        tts_cfg = config.setdefault("tts", {})
        tts_cfg["provider"] = (
            SOPHOS_MANAGED_PROVIDER if managed_feature else provider["tts_provider"]
    ...
    if provider.get("stt_provider"):
        stt_cfg = config.setdefault("stt", {})
        stt_cfg["provider"] = (
            SOPHOS_MANAGED_PROVIDER if managed_feature else provider["stt_provider"]
        )
    ...
    if "browser_provider" in provider:
        ...
        browser_cfg = config.setdefault("browser", {})
        if managed_feature:
            browser_cfg["cloud_provider"] = SOPHOS_MANAGED_PROVIDER
            _print_success(f"  Browser cloud provider set to: {bp or 'sophos'}")
        elif bp == "local":
    ...
    if provider.get("web_backend"):
        ...
        web_cfg["backend"] = (
            SOPHOS_MANAGED_PROVIDER if managed_feature else provider["web_backend"]
        )
    ...
    if not env_vars:
        ...
        if backend:
            ...
            if backend == "fal":
                img_cfg = config.setdefault("image_gen", {})
                if isinstance(img_cfg, dict):
                    # A managed (Sophos Subscription) row also carries
                    # imagegen_backend="fal" — store the "sophos" selection
    ...
    if backend:
        ...
        if backend == "fal":
            ...
            if isinstance(img_cfg, dict):
                ...
                # never clobber a Sophos-managed pick back onto direct keys.
                img_cfg["provider"] = (
                    SOPHOS_MANAGED_PROVIDER if managed_feature else "fal"
                )
                img_cfg.pop("use_gateway", None)
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `slick model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `slick tools` and `slick setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("😎 Slick Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://slick-agent.sophostechne.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_sophos_managed_defaults(
                config,
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your Sophos subscription defaults", Colors.GREEN))
    ...
    print(color(f"  Tool configuration saved to {display_slick_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'slick' or gateway restart.", Colors.DIM))
    print()
