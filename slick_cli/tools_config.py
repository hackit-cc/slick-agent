...
from slick_cli.hackit_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    HackitSubscriptionFeatures,
    apply_hackit_managed_defaults,
    get_hackit_subscription_features,
)
from slick_cli.hackit_account import format_hackit_portal_entitlement_message
from tools.tool_backend_helpers import HACKIT_MANAGED_PROVIDER, fal_key_is_configured
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            {
                "name": "Hackit Subscription",
                "badge": "subscription",
                "tag": "Managed OpenAI TTS billed to your subscription",
                "env_vars": [],
                "tts_provider": "openai",
                "requires_hackit_auth": True,
                "managed_hackit_feature": "tts",
    ...
    "web": {
        ...
        #   - "Hackit Subscription" — managed Firecrawl billed via Hackit
        #     subscription (requires_hackit_auth + override_env_vars).
    ...
    "image_gen": {
        ...
        #   - "Hackit Subscription" — managed FAL billed via the Hackit
        #     subscription (requires_hackit_auth + override_env_vars).
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_hackit_auth": True,
                "managed_hackit_feature": "image_gen",
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "Hackit Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the Hackit Portal.  Plugin-backed
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_hackit_auth": True,
                "managed_hackit_feature": "video_gen",
                "override_env_vars": ["FAL_KEY"],
                # The underlying plugin backend — when the user picks
                # "Hackit Subscription" we set video_gen.provider = "fal"
    ...
    "browser": {
        ...
        # backend, never on the paid Hackit Subscription gateway row:
        #   - "Local Browser" — non-cloud option, no CloudBrowserProvider.
        #   - "Hackit Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Hackit subscription (requires_hackit_auth +
        ...
        "providers": [
            ...
            {
                "name": "Hackit Subscription (Browser Use cloud)",
                "badge": "subscription",
                "tag": "Managed Browser Use billed to your subscription",
                "env_vars": [],
                "browser_provider": "browser-use",
                "requires_hackit_auth": True,
                "managed_hackit_feature": "browser",
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Firecrawl, and the Hackit-managed cloud rows all execute through
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/hackit-cc/slick-agent:latest"
            )
            return
...
def _toolset_has_keys(
    ...
    *,
    ...
    features: Optional[HackitSubscriptionFeatures] = None,
) -> bool:
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "stt", "browser"}:
        if features is None:
            features = get_hackit_subscription_features(
                config, force_fresh=force_fresh
            )
        feature = features.features.get(ts_key)
        if feature and (feature.available or feature.managed_by_hackit):
            return True
...
# ("Hackit Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Hackit Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    Hackit-managed Tool Gateway rows (``managed_hackit_feature``) are always
    shown — even to logged-out / unentitled users — so the picker advertises
    that the capability exists.  Selecting one drives an inline Hackit Portal
    login + entitlement check (see ``_configure_provider``); the row only
    *activates* the gateway once paid access is confirmed.
    """
    if features is None:
        features = get_hackit_subscription_features(config, force_fresh=force_fresh)
    ...
    for provider in cat.get("providers", []):
        # Hackit-managed Tool Gateway rows stay visible regardless of auth —
        # selecting one drives an inline Portal login. A `requires_hackit_auth`
        # row that is NOT a managed gateway feature (pure pre-auth UX) is
        # still hidden until the user is logged in.
        if (
            provider.get("requires_hackit_auth")
            and not provider.get("managed_hackit_feature")
            and not features.hackit_auth_present
        ):
            continue
        ...
        if (
            pool_only
            and provider.get("managed_hackit_feature") == "video_gen"
    ...
    # later) so the picker lists them alongside FAL / Hackit Subscription.
    ...
    # remaining hardcoded rows ("Hackit Subscription", "Firecrawl
    ...
    # the hardcoded "Hackit Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_hackit_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Deprecated: Hackit Tool Gateway rows are no longer hidden.

    Previously this returned a "log in / upgrade" banner shown above a
    category when its Hackit-managed rows were filtered out for unentitled
    ...
    (``ensure_hackit_portal_access``). Kept as a no-op so call sites stay simple;
...
def _agent_browser_installed() -> bool:
    ...
    from slick_cli.hackit_subscription import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from slick_cli.hackit_subscription import _has_agent_browser
...
def provider_readiness_status(
    ...
    *,
    ...
    - ``"needs_auth"``  — needs a sign-in: Hackit Portal login/entitlement for
    ...
    zero-env-var row, including logged-out Hackit Subscription rows).

    ``features`` (a ``HackitSubscriptionFeatures``) can be passed to avoid
    ...
    managed_feature = provider.get("managed_hackit_feature")
    if provider.get("requires_hackit_auth") or managed_feature:
        if features is None:
            features = get_hackit_subscription_features(config)
        if not features.hackit_auth_present:
            return "needs_auth"
        if managed_feature:
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_hackit_message = _hidden_hackit_gateway_message(
        cat,
        config,
        f"the Hackit Subscription provider for {name}",
        force_fresh=force_fresh,
    )
    ...
    if len(providers) == 1:
        ...
        if hidden_hackit_message:
            for line in hidden_hackit_message.splitlines():
        ...
        # When the user is logged into Hackit, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of Hackit.
        try:
            _hackit_logged_in = bool(
                get_hackit_subscription_features(
                    config,
                    force_fresh=force_fresh,
                ).hackit_auth_present
        ...
        for p in providers:
            ...
            # Mark Hackit-managed entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via Hackit Portal" hint so
            ...
            sub_marker = ""
            if p.get("managed_hackit_feature"):
                if _hackit_logged_in:
                    sub_marker = "  ★ Included with your Hackit subscription"
                else:
                    sub_marker = "  ★ via Hackit Portal (login on select)"
...
def _is_provider_active(
    ...
    *,
    ...
    if plugin_name and not provider.get("managed_hackit_feature"):
        # Managed (Hackit-subscription) entries fall through to the
    ...
    if video_plugin_name and not provider.get("managed_hackit_feature"):
    ...
    if managed_feature:
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if configured_provider not in {None, "", "fal", HACKIT_MANAGED_PROVIDER}:
                    return False
                if (
                    configured_provider != HACKIT_MANAGED_PROVIDER
                    and image_cfg.get("use_gateway") is not None
                    and not is_truthy_value(image_cfg.get("use_gateway"), default=False)
                ):
                    return False
            return feature.managed_by_hackit
        ...
        if provider.get("tts_provider"):
            return (
                feature.managed_by_hackit
                and cfg_get(config, "tts", "provider")
                in {provider["tts_provider"], HACKIT_MANAGED_PROVIDER}
            )
        if provider.get("stt_provider"):
            return (
                feature.managed_by_hackit
                and cfg_get(config, "stt", "provider")
                in {provider["stt_provider"], HACKIT_MANAGED_PROVIDER}
            )
        if "browser_provider" in provider:
            ...
            return feature.managed_by_hackit and current in {
                provider["browser_provider"],
                HACKIT_MANAGED_PROVIDER,
            }
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return (
                feature.managed_by_hackit
                and current in {provider["web_backend"], HACKIT_MANAGED_PROVIDER}
...
def _select_plugin_image_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    ``use_gateway=True`` marks a provider picked through the Hackit-managed
    flow: the stored selection becomes ``image_gen.provider: hackit`` (the
    ...
    img_cfg["provider"] = HACKIT_MANAGED_PROVIDER if use_gateway else plugin_name
...
def _select_plugin_video_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    ``video_gen.provider: hackit``; BYOK picks store the plugin name; any
    ...
    vid_cfg["provider"] = HACKIT_MANAGED_PROVIDER if use_gateway else plugin_name
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    post-setup hooks, gate on Hackit auth, or run interactive model pickers.
    ...
    category. Managed "Hackit Subscription" rows write ``hackit``; BYOK rows
    ...
    legacy shim (use_gateway: true ⇒ hackit) cannot override the new choice.
    """
    def _set_selection(section_key: str, name_key: str, vendor_value) -> None:
        ...
        section[name_key] = (
            HACKIT_MANAGED_PROVIDER if managed_feature else vendor_value
        )
    ...
    # image_gen/video_gen "Hackit Subscription" rows carry only
    # managed_hackit_feature) still persist the "hackit" selection.
    if managed_feature and managed_feature not in {"web", "tts", "stt", "browser"}:
        section = config.setdefault(managed_feature, {})
        if isinstance(section, dict):
            section["provider"] = HACKIT_MANAGED_PROVIDER
            section.pop("use_gateway", None)
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on Hackit Portal auth, or run interactive model
    ...
    # flow). Managed picks store the "hackit" selection.
    plugin_name = provider.get("image_gen_plugin_name")
    if plugin_name:
        img_cfg = config.setdefault("image_gen", {})
        if not isinstance(img_cfg, dict):
            ...
            config["image_gen"] = img_cfg
        img_cfg["provider"] = (
            HACKIT_MANAGED_PROVIDER if managed_feature else plugin_name
        )
        img_cfg.pop("use_gateway", None)
    ...
    if video_plugin:
        ...
        if not isinstance(vid_cfg, dict):
            ...
            config["video_gen"] = vid_cfg
        vid_cfg["provider"] = (
            HACKIT_MANAGED_PROVIDER if managed_feature else video_plugin
        )
        vid_cfg.pop("use_gateway", None)
...
def _configure_provider(
    ...
    *,
    ...
    # Hackit-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid Hackit
    ...
    if managed_feature:
        from slick_cli.hackit_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
            ensure_hackit_portal_access,
        )

        if not ensure_hackit_portal_access(
            capability=f"{provider.get('name', 'the Hackit Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — Hackit Portal access is required for this backend."
            )
            return

    # Pure pre-auth UX rows (requires_hackit_auth without a managed gateway
    # feature) keep the old gate. Managed rows are handled by the inline
    # login above, so don't double-check them here.
    if provider.get("requires_hackit_auth") and not managed_feature:
        features = get_hackit_subscription_features(config, force_fresh=force_fresh)
        entitled = bool(
            features.account_info and features.account_info.paid_service_access is True
        )
        if not features.hackit_auth_present or not entitled:
            message = format_hackit_portal_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'Hackit Subscription')}",
            )
            _print_warning(
                f"  {message or 'Hackit Subscription is only available after logging into Hackit Portal.'}"
            )
            return
    ...
    if provider.get("tts_provider"):
        ...
        tts_cfg["provider"] = (
            HACKIT_MANAGED_PROVIDER if managed_feature else provider["tts_provider"]
        )
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Hackit subscription.")
        ...
        if backend:
            ...
            # explicit selection: "hackit" for a managed row, "fal" for BYOK.
            img_cfg = config.setdefault("image_gen", {})
            if isinstance(img_cfg, dict):
                img_cfg["provider"] = (
                    HACKIT_MANAGED_PROVIDER if managed_feature else "fal"
                )
                img_cfg.pop("use_gateway", None)
    ...
    # Hackit-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to Hackit.
    _show_portal_hint = False
    if env_vars and not managed_feature and not provider.get("requires_hackit_auth"):
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_hackit_feature") for sib in _providers
                ):
                    _has_managed_sibling = True
                    break
            if _has_managed_sibling:
                _features = get_hackit_subscription_features(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.hackit_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through Hackit Portal subscription.")
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Hackit Portal login + entitlement gate as _configure_provider:
    ...
    # provider string per category — "hackit" for managed rows, the vendor name
    # for BYOK rows — and drops any legacy use_gateway key so the read-time
    # shim (use_gateway: true ⇒ hackit) cannot override the fresh pick.
    if provider.get("tts_provider"):
        tts_cfg = config.setdefault("tts", {})
        tts_cfg["provider"] = (
            HACKIT_MANAGED_PROVIDER if managed_feature else provider["tts_provider"]
    ...
    if provider.get("stt_provider"):
        stt_cfg = config.setdefault("stt", {})
        stt_cfg["provider"] = (
            HACKIT_MANAGED_PROVIDER if managed_feature else provider["stt_provider"]
        )
    ...
    if "browser_provider" in provider:
        ...
        browser_cfg = config.setdefault("browser", {})
        if managed_feature:
            browser_cfg["cloud_provider"] = HACKIT_MANAGED_PROVIDER
            _print_success(f"  Browser cloud provider set to: {bp or 'hackit'}")
        elif bp == "local":
    ...
    if provider.get("web_backend"):
        ...
        web_cfg["backend"] = (
            HACKIT_MANAGED_PROVIDER if managed_feature else provider["web_backend"]
        )
    ...
    if not env_vars:
        ...
        if backend:
            ...
            if backend == "fal":
                img_cfg = config.setdefault("image_gen", {})
                if isinstance(img_cfg, dict):
                    # A managed (Hackit Subscription) row also carries
                    # imagegen_backend="fal" — store the "hackit" selection
    ...
    if backend:
        ...
        if backend == "fal":
            ...
            if isinstance(img_cfg, dict):
                ...
                # never clobber a Hackit-managed pick back onto direct keys.
                img_cfg["provider"] = (
                    HACKIT_MANAGED_PROVIDER if managed_feature else "fal"
                )
                img_cfg.pop("use_gateway", None)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://slick-agent.hackit.cc/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_hackit_managed_defaults(
                config,
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your Hackit subscription defaults", Colors.GREEN))
