"""
Configuration management for Slick Agent.

Config files are stored in ~/.slick/ for easy access:
- ~/.slick/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.slick/.env         - API keys and secrets

This module provides:
- slick config          - Show current configuration
- slick config edit     - Open config in editor
- slick config get      - Print a resolved configuration value
- slick config set      - Set a specific value
- slick config unset    - Remove a user configuration value
- slick config wizard   - Re-run setup wizard
"""
...
from slick_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``slick config set`` (which rewrites
    ...
    ``config.yaml`` in place: slick never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.slick/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``slick logs``
    ...
    if fallback == "last-known-good":
        ...
        msg = (
            ...
            f"Fix the YAML (slick config edit) and retry."
        )
    else:
        msg = (
            f"Failed to parse {config_path}: {exc}. "
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  slick config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Slick makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Slick itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``slick update``, the TUI build.
...
#   on every plugin install / ``slick update``.
...
# * ``SLICK_HOME`` / ``SLICK_PROFILE`` / ``SLICK_CONFIG`` /
#   ``SLICK_ENV`` — Slick runtime location flags. Writing these into
...
#
# IMPORTANT: ``SLICK_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (SLICK_LANGFUSE_PUBLIC_KEY,
# SLICK_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Slick runtime location — never via dashboard env writer.
    # NOT a SLICK_* blanket: integration credentials (SLICK_GEMINI_*,
    # SLICK_LANGFUSE_*, SLICK_SPOTIFY_*, ...) ARE allowed.
    "SLICK_HOME", "SLICK_PROFILE", "SLICK_CONFIG", "SLICK_ENV",
    "SLICK_CONFIG_PATH", "SLICK_ENV_PATH",
    ...
    "SLICK_COPILOT_ACP_COMMAND", "SLICK_COPILOT_ACP_ARGS",
    # Slick security policy / approval-routing context. These remain available
    ...
    "SLICK_YOLO_MODE", "SLICK_ACCEPT_HOOKS", "SLICK_REDACT_SECRETS",
    "SLICK_INTERACTIVE", "SLICK_EXEC_ASK", "SLICK_GATEWAY_SESSION",
    "SLICK_CRON_SESSION", "SLICK_SINGLE_QUERY_SESSION",
    "SLICK_SESSION_KEY", "SLICK_SESSION_PLATFORM",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Slick runtime location "
            "and security policy (SLICK_HOME, SLICK_YOLO_MODE, ...) "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # SLICK_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `slick plugins enable
    # observability/langfuse` or `slick tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "SLICK_LANGFUSE_ENV",
    "SLICK_LANGFUSE_RELEASE",
    "SLICK_LANGFUSE_SAMPLE_RATE",
    "SLICK_LANGFUSE_MAX_CHARS",
    "SLICK_LANGFUSE_CAPTURE",
    "SLICK_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "SLICK_ACP_AUTH_METHOD",
    "SLICK_ACP_AUTO_APPROVE",
    "SLICK_COPILOT_ACP_COMMAND",
    "SLICK_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from slick_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set SLICK_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SLICK_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_slick_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # SLICK_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if Slick is running in package-manager-managed mode.

    Two signals: the SLICK_MANAGED env var (set by the systemd service),
    or a .managed marker file in SLICK_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Slick through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``slick_cli/`` — i.e. the git checkout for source
    installs, ``/opt/slick`` inside the published image. It is a property of
    the running interpreter, NOT of ``$SLICK_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Slick was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    2. Legacy home-scoped stamp ``$SLICK_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. SLICK_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.slick``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$SLICK_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.slick:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$SLICK_HOME``, a
    ...
    ``docker`` and ``slick update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``sophostechne/slick-agent`` image bakes a ``docker``
        stamp into ``/opt/slick`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $SLICK_HOME.
    try:
    ...
    #    host install whose shared $SLICK_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``slick update``.
    try:
        method = (
            (get_slick_home() / ".install_method")
    ...
    # detect Nix installs that don't set SLICK_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``slick_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$SLICK_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/slick`` in the published image, which instead bakes the stamp at
...
# Long-form text for ``slick update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# slick_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Slick, it doesn't update
...
✗ ``slick update`` doesn't apply inside the Docker container.

Slick Agent runs as a published image (sophostechne/slick-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull sophostechne/slick-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate slick-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm sophostechne/slick-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/sophostechne/slick-agent/tags
  • Your config and session history live under ``$SLICK_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``slick update`` inside Docker.
...
def format_managed_message(action: str = "modify this Slick installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Slick installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Slick."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SLICK_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, slick_bin
    or None if container mode is not active, we're already inside the
    container, or SLICK_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("SLICK_DEV") == "1":
        return None
    ...
    container_mode_file = get_slick_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "slick-agent")
    exec_user = info.get("exec_user", "slick")
    slick_bin = info.get("slick_bin", "/data/current-package/bin/slick")
...
# Re-export from slick_constants — canonical definition lives there.
from slick_constants import get_slick_home, get_process_slick_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_slick_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_slick_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_slick_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the SLICK_UID / SLICK_GID env vars set by Docker deployments.

    Docker containers running Slick commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level SLICK_HOME once, but
    subdirectories created at runtime by ``ensure_slick_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("SLICK_UID", "").strip()
    gid_str = os.environ.get("SLICK_GID", "").strip()
    try:
...
def _chown_to_slick_uid(path) -> None:
    """Chown ``path`` to ``SLICK_UID:SLICK_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_slick_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_slick_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the slick group can
    share state with the gateway service.

    The mode can be overridden via the SLICK_HOME_MODE environment variable
    (e.g. SLICK_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SLICK_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``SLICK_UID``/``SLICK_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SLICK_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_slick_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Slick runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("SLICK_CONTAINER") or os.environ.get("SLICK_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SLICK_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SLICK_HOME, upgrading legacy empty templates.
...
# ensure_slick_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_SLICK_HOME_ENSURED: set = set()


def ensure_slick_home():
    """Ensure ~/.slick directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``SLICK_HOME`` we only re-run
    ...
    ``get_slick_home()`` and therefore re-run for the new path.
    """
    home = get_slick_home()
    key = str(home)

    # Named profiles must be created explicitly (e.g. ``slick profile create``).
    # Check tombstones before the memo so a stale empty shell cannot skip
    # the deleted-profile guard.
    from slick_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _SLICK_HOME_ENSURED and home.is_dir():
        return
    if is_managed():
        old_umask = os.umask(0o007)
        try:
            _ensure_slick_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _SLICK_HOME_ENSURED.add(key)


def _ensure_slick_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"SLICK_HOME {home} does not exist."
        )
...
from slick_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
# selection step (Sophos Portal / OpenRouter / Custom endpoint), so this
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `slick config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.slick.config`` entries, then checks
    ...
    try:
        ...
        # should never break `slick update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Slick' own config writer has historically emitted it
    ...
    # Slick (``_save_discovered_models_to_config``), not hand-curated.
    # Older Slick versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Slick versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``slick_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (slick_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (slick_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (slick_cli/setup.py)
    "known_plugin_toolsets", # written/read by slick_cli/tools_config.py toolset-save flow
    "known_builtin_toolsets",  # ditto — which builtin toolsets a platform's checklist has offered
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (slick_cli/sophos_subscription.py, #92647)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'slick setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Slick won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, slick send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'slick doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "slick update / slick -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from slick_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ slick config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in slick_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from slick_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from slick_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: slick config set <key> <value>")
    ...
    # `slick update`, but only the version bump is persisted.
    ...
    # metadata.slick.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.slick/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_slick_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``SlickCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``slick config set``
    ...
    backends) — while display paths (``slick status``/``dump``) read ``name``
...
#   - survives the ``str() → int()`` round-trip through the SLICK_MAX_ITERATIONS
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _slick_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in slick_cli.personality.
from slick_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`slick_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from slick_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from slick_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``SLICK_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`slick_cli.personality` (single owner).
    """
    from slick_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.slick/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_slick_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.slick/config.yaml.
    ...
    (which change ``SLICK_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Slick host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/slick/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_slick_dotenv()
...
#   openai-codex (OAuth — slick auth) — OpenAI Codex
#   sophos         (OAuth — slick auth) — Sophos Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.slick/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Slick writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.slick/.env.
    ...
    interactive menu render (`slick tools`, `slick setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `slick tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.slick/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.slick/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``SLICK_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"SLICK_CUSTOM_{slug}_API_KEY" if slug else "SLICK_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.slick/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.slick/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Slick — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Slick vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.slick/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.slick/.env`` over ``os.environ``.

    Used for Slick-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              😎 Slick Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from slick_cli.auth import get_anthropic_key
    ...
    # Warn on stale SLICK_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("SLICK_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale SLICK_MAX_ITERATIONS={_env_ghost} "
                f"(run 'slick doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    try:
        from slick_cli.personality import active_personality_name
    ...
    print(color("  slick config edit     # Edit config file", Colors.DIM))
    print(color("  slick config set <key> <value>", Colors.DIM))
    print(color("  slick setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``SLICK_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("SLICK_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `slick cron list`, then pin the "
        "intended values with `slick cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (slick_cli/plugins_cmd.py, slick_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `slick config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``slick config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``slick config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``slick config set --force``.
    """
    if is_managed():
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   slick config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   slick config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``slick config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``slick config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    slick config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    slick config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `slick config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_slick_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `slick config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `slick config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: slick config get <key> [--json]")
            print()
            print("Examples:")
            print("  slick config get model")
            print("  slick config get terminal.backend")
            print("  slick config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: slick config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  slick config set model anthropic/claude-sonnet-4")
            print("  slick config set terminal.backend docker")
            print("  slick config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: slick config unset <key>")
            print()
            print("Examples:")
            print("  slick config unset model")
            print("  slick config unset terminal.backend")
            print("  slick config unset OPENROUTER_API_KEY")
            sys.exit(1)
        try:
        ...
        if missing_config:
            ...
            print("    Run 'slick config migrate' to add them")
        ...
        print("  slick config           Show current configuration")
        print("  slick config edit      Open config in editor")
        print("  slick config get <key>          Print a resolved config value")
        print("  slick config set <key> <value>   Set a config value")
        print("  slick config unset <key>        Remove a config value")
        print("  slick config check     Check for missing/outdated config")
        print("  slick config migrate   Update config with new options")
        print("  slick config path      Show config file path")
        print("  slick config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``slick config`` UI so users
