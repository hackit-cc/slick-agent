...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``hackit/slick-agent`` image bakes a ``docker``
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if is_nix_install_method(method):
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull hackit/slick-agent:latest"
    if method == "apt":
...
Slick Agent runs as a published image (hackit/slick-agent), not a
...
Verify the new version after restart:
  docker run --rm hackit/slick-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/hackit/slick-agent/tags
...
# selection step (Hackit Portal / OpenRouter / Custom endpoint), so this
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (slick_cli/hackit_subscription.py, #92647)
...
#   hackit         (OAuth — slick auth) — Hackit Portal
