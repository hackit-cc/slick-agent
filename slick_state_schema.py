...
``slick_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import slick_state (cycle) — shared
module-level constants live in slick_state_common.
"""
...
# Moved methods logged under the "slick_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("slick_state")
...
def schema_read_probe_statements() -> tuple:
    ...
    ``slick_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `slick update` until the user's
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._slick_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._slick_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
            ...
            if escalated:
                ...
                if foreign_holders:
                    logger.error(
                        ...
                        "`slick sessions optimize-storage` with the gateway stopped. "
                        "`slick doctor` reports this degraded state.",
                        attempts,
                        foreign_holders,
                    )
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from slick_constants import get_slick_home
            cache_path = get_slick_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `slick sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `slick db optimize`) must keep its EXISTING
