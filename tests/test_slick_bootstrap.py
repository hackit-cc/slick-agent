"""Tests for slick_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Slick entry point
(slick, slick-agent, slick-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Slick entry point imports slick_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported slick_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("slick_bootstrap", None)
    import slick_bootstrap  # noqa: WPS433
    return slick_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Slick CLI
...
class TestEntryPointsImportBootstrap:
    """Every Slick entry point must import slick_bootstrap as its
    ...
    # Entry points that invoke Slick as a process.  Each one must
    # import slick_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "slick_cli/main.py",   # slick CLI (console_script)
        "run_agent.py",          # slick-agent (console_script)
        "acp_adapter/entry.py",  # slick-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import slick_bootstrap' and that
        ...
        half-finished ``slick update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``slick_bootstrap`` as a top-level module) leaves slick
        ...
        """
        # Resolve relative to the slick-agent repo root.  Tests live
        # at tests/test_slick_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # slick start even when slick_bootstrap hasn't been
        ...
        assert first_import_name == "slick_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'slick_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import slick_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Slick's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("SLICK_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["SLICK_PYTHON_SRC_ROOT"] = env
            elif "SLICK_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["SLICK_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/slick")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SLICK_PYTHON_SRC_ROOT", None)
            else:
                os.environ["SLICK_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/slick", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Slick root.  The guard must relocate Slick to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/slick"])
        assert result[0] == "/opt/slick"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Slick root.
        assert result.index("/opt/slick") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["SLICK_PYTHON_SRC_ROOT"] = "/env/slick"
            hb.harden_import_path()
            assert sys.path[0] == "/env/slick"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SLICK_PYTHON_SRC_ROOT", None)
