...
from cli import SlickCLI, _build_compact_banner, _rich_text_from_ansi
from slick_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCliSkinPromptIntegration:
    ...
    def test_handle_skin_command_refreshes_live_tui(self, capsys):
        ...
        with patch("cli.save_config_value", return_value=True):
            cli._handle_skin_command("/skin ruby")

        output = capsys.readouterr().out
        assert "Skin set to: ruby (saved)" in output
...
class TestCompactBannerSkinIntegration:
    ...
    def test_poseidon_compact_banner_uses_skin_branding_instead_of_hackit_slick(self):
        set_active_skin("mint")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "Slick Agent v0.1.0 (test)"}):
            banner = _build_compact_banner()

        assert "Mint Agent" in banner
        assert "HACKIT SLICK" not in banner
