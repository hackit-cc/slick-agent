...
def test_frontmatter_required_fields():
    ...
    slick = fm["metadata"]["slick"]
    assert slick["tags"]
    assert "related_skills" in slick
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Slick Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["slick"]["related_skills"]:
