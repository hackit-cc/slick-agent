...
def test_required_frontmatter_fields(p):
    ...
    slick = (fm.get("metadata") or {}).get("slick") or {}
    if not (slick.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.slick.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    slick = (fm.get("metadata") or {}).get("slick") or {}
    dangling = [
        rs for rs in (slick.get("related_skills") or []) if rs not in _all_names()
    ]
