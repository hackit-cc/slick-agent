"""Guard: Slick-owned subprocesses must not resolve managed runtimes by bare PATH.

Slick installs runtimes for itself — ``uv`` at ``$SLICK_HOME/bin/uv``, Node at
``$SLICK_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Slick's own
...
* when a system copy also exists, the one Slick does not own wins — which is
...
``iter_slick_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Slick provisions into SLICK_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Slick-owned subprocess code: plugins ship their own
...
def _source_files() -> list[Path]:
    ...
    for path in REPO_ROOT.rglob("*.py"):
        ...
        # slick_agent-0.20.5/, build/ and *.egg-info dirs). CI jobs that
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Slick-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$SLICK_HOME/bin (uv) and $SLICK_HOME/node are not on an "
        ...
        "  node/npm -> slick_constants.find_node_executable()\n"
        "  PATH env -> slick_constants.iter_slick_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(slick_constants, helper))


def test_managed_uv_helpers_exist():
    from slick_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
