...
The ``slick update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m slick_cli.main gateway run`` — image name
    ``pythonw``, not ``slick.exe`` — so the ``taskkill /IM slick.exe`` guard
    ...
    # The slick.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM slick.exe' in text
