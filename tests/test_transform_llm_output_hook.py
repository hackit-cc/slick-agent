...
from slick_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(slick_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <slick_home>/plugins/<name> and opt it in."""
    plugin_dir = slick_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SLICK_HOME", str(tmp_path / "slick_empty"))
