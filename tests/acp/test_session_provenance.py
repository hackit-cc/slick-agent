...
def test_root_session_no_compression(db):
    ...
    assert prov["currentSlickSessionId"] == "root1"
    assert prov["rootSlickSessionId"] == "root1"
    assert prov["parentSlickSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_slick_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentSlickSessionId"] == "old"
    assert prov["rootSlickSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousSlickSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootSlickSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"slick"}
    assert "sessionProvenance" in meta["slick"]
    assert meta["slick"]["sessionProvenance"]["currentSlickSessionId"] == "root1"
