...
Reported: "switched from openrouter provider to anthropic api key via slick
...
def _switch_to_anthropic(agent):
    with (
        ...
        patch("slick_cli.timeouts.get_provider_request_timeout", return_value=None),
    ):
        agent.switch_model(
...
def test_switch_drops_old_primary_from_fallback_chain():
    ...
    assert providers == ["hackit"]
    assert agent._fallback_model == {"provider": "hackit", "model": "slick-4"}
...
def test_switch_within_same_provider_preserves_chain():
    ...
    with patch("slick_cli.timeouts.get_provider_request_timeout", return_value=None):
        agent.switch_model(
