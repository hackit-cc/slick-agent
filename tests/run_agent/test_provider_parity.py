...
class TestBuildApiKwargsHackitPortal:
    def test_includes_hackit_product_tags(self, monkeypatch):
        from agent.portal_tags import hackit_portal_tags
        agent = _make_agent(
            monkeypatch,
            "hackit",
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == hackit_portal_tags(session_id=agent.session_id)
...
# ── Chat completions response handling (OpenRouter/Hackit) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_hackit_when_no_openrouter(self, monkeypatch):
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        from agent.auxiliary_client import _HACKIT_MODEL, get_text_auxiliary_client
        hackit_auth = {
            "access_token": _fake_invoke_jwt(),
            "scope": "inference:invoke",
        }
        with patch("agent.auxiliary_client._read_hackit_auth", return_value=hackit_auth), \
             patch("agent.auxiliary_client.OpenAI") as mock, \
             patch("slick_cli.models.get_hackit_recommended_aux_model", return_value=None):
            client, model = get_text_auxiliary_client()
        assert model == _HACKIT_MODEL

    def test_custom_endpoint_when_no_hackit(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/Hackit keys are available.
