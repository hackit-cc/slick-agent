...
class TestBuildApiKwargsSophosPortal:
    def test_includes_sophos_product_tags(self, monkeypatch):
        from agent.portal_tags import sophos_portal_tags
        agent = _make_agent(
            monkeypatch,
            "sophos",
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == sophos_portal_tags(session_id=agent.session_id)
...
# ── Chat completions response handling (OpenRouter/Sophos) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_sophos_when_no_openrouter(self, monkeypatch):
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        from agent.auxiliary_client import _SOPHOS_MODEL, get_text_auxiliary_client
        sophos_auth = {
            "access_token": _fake_invoke_jwt(),
            "scope": "inference:invoke",
        }
        with patch("agent.auxiliary_client._read_sophos_auth", return_value=sophos_auth), \
             patch("agent.auxiliary_client.OpenAI") as mock, \
             patch("slick_cli.models.get_sophos_recommended_aux_model", return_value=None):
            client, model = get_text_auxiliary_client()
        assert model == _SOPHOS_MODEL

    def test_custom_endpoint_when_no_sophos(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/Sophos keys are available.
