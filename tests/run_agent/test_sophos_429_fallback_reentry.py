"""Regression guard: a genuine Sophos 429 must re-enter the retry loop so the
top-of-loop Sophos rate-limit guard can activate the fallback chain.
...
``record_sophos_rate_limit()`` moments earlier and either activates a fallback
...
class TestGenuineSophos429ReentersLoop:
