...
class TestFallbackChainAdvancement:
    ...
    def test_hackit_anthropic_fallback_uses_the_messages_wire(self):
        """Portal Claude fallbacks must not stay on chat_completions.

        ``resolve_provider_client`` still returns an OpenAI client for Hackit;
        ...
        """
        portal = "https://inference-api.hackit.cc/v1"
        fbs = [
            {
                "provider": "hackit",
    ...
    def test_hackit_non_anthropic_fallback_stays_on_chat_completions(self):
        portal = "https://inference-api.hackit.cc/v1"
        fbs = [{"provider": "hackit", "model": "slick-4-405b"}]
        agent = _make_agent(fallback_model=fbs)
        with (
            patch(
                "agent.chat_completion_helpers._fallback_entry_unavailable_without_network",
