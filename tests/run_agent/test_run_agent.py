...
def test_malformed_memory_config_still_builds_default_store():
    ...
    with (
        patch(
            "slick_cli.config.load_config_readonly",
...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._slick_home / "logs" / "errors.log").resolve()
...
class TestSessionJsonSnapshotOptIn:
    ...
    def test_traversal_session_id_cannot_escape_logs_dir(self, agent, tmp_path):
        ...
        # originate from the untrusted X-Slick-Session-Id API header) must not
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host SLICK_REDACT_SECRETS state.
...
class TestInit:
    ...
    def test_prompt_caching_cache_ttl_defaults_without_config(self):
        ...
        with (
            ...
            patch("run_agent.check_toolset_requirements", return_value={}),
            patch("run_agent.OpenAI"),
            patch("slick_cli.config.load_config", return_value={}), patch("slick_cli.config.load_config_readonly", return_value={}),
        ):
            a = AIAgent(
                api_key="test-k...7890",
                model="anthropic/claude-sonnet-4-20250514",
...
class TestBuildApiKwargs:
    ...
    def test_core_responses_preserves_supported_xhigh(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "slick_cli.models.github_model_reasoning_efforts",
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("slick_cli.lifecycle.invoke_hook", _capture_hook)
        monkeypatch.setattr("slick_cli.lifecycle.has_hook", lambda name: True)
    ...
    def test_interrupt_skips_remaining(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("slick_cli.lifecycle.has_hook", lambda name: True)
        monkeypatch.setattr(
            "slick_cli.lifecycle.invoke_hook",
...
class TestConcurrentToolExecution:
    ...
    def test_sequential_registry_tool_forwards_request_middleware_trace(
        ...
        from slick_cli.middleware import RequestMiddlewareResult
        ...
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tool_call])
        monkeypatch.setattr(
            "slick_cli.middleware.apply_tool_request_middleware",
        ...
        monkeypatch.setattr(
            "slick_cli.middleware.run_tool_execution_middleware",
            lambda _name, args, callback, **_kwargs: callback(args),
        )
        monkeypatch.setattr(
            "slick_cli.plugins._dispatch_pre_tool_call_hooks",
    ...
    def test_managed_tool_pipeline_rejects_second_dispatch(self, agent, monkeypatch):
        ...
        assert duplicate_errors == [
            "Slick tool execution callback invoked more than once"
        ]
    ...
    def test_managed_tool_pipeline_allows_one_concurrent_dispatch(
        ...
        assert outcome.result == "ok"
        assert dispatched == [{"command": "true"}]
        assert errors == ["Slick tool execution callback invoked more than once"]
        assert outcome.blocked is False
...
class TestHandleMaxIterations:
    ...
    def test_api_sanitizer_repairs_tool_call_with_empty_function_name(self, agent):
        ...
        call, because slick' dispatch loop keeps empty-name calls paired with
...
class TestRunConversation:
    ...
    def test_task_start_failure_closes_relay_turn_and_lease(self, agent):
        ...
        with (
            ...
            patch(
                "slick_cli.observability.relay_shared_metrics.start_task_run",
                side_effect=start_error,
            ),
            patch(
                "slick_cli.observability.relay_shared_metrics.finish_task_run"
            ) as finish_task_run,
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for Slick tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "slick_cli.lifecycle.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("slick_cli.lifecycle.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("slick_cli.lifecycle.has_hook", lambda name: False)
        monkeypatch.setattr("slick_cli.lifecycle.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("slick_cli.lifecycle.has_hook", _has_hook)
        ...
        with (
            patch("slick_cli.lifecycle.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_sophos_401_refreshes_after_remint_and_retries(self, agent):
        self._setup_agent(agent)
        agent.provider = "sophos"
        ...
        with (
            ...
            patch.object(agent, "_interruptible_api_call", side_effect=_fake_api_call),
            patch.object(
                agent, "_try_refresh_sophos_client_credentials", side_effect=_fake_refresh
            ),
        ):
            result = agent.run_conversation("hello")
    ...
    def test_engine_preflight_fires_below_threshold(self, agent):
        ...
        should_compress_preflight() hook so engines like slick-lcm can
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("SLICK_KANBAN_TASK", "t_test_task_123")
        ...
        with (
            ...
            patch("slick_cli.kanban_db._record_task_failure",
                  mock_record_failure),
            patch("slick_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when SLICK_KANBAN_TASK
...
class TestRetryExhaustion:
    ...
    def test_content_filter_refusal_surfaced_not_retried(self, agent):
        ...
        portal (Sophos Portal fronting Anthropic) returns ``message.refusal``
...
class TestSophosCredentialRefresh:
    """Verify Sophos credential refresh rebuilds the runtime client."""

    def test_try_refresh_sophos_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "slick_cli.auth.resolve_sophos_runtime_credentials", _fake_resolve
        )
        ...
        with patch("run_agent.OpenAI", side_effect=_fake_openai):
            ok = agent._try_refresh_sophos_client_credentials(force=True)
        ...
        assert rebuilt["kwargs"]["api_key"] == "new-sophos-key"
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.sophostechne.com/v1"
        )
        assert "default_headers" not in rebuilt["kwargs"]
        assert isinstance(agent.client, _RebuiltClient)

    def test_try_refresh_sophos_client_credentials_rebuilds_anthropic_client(
        self, agent, monkeypatch
    ):
        ...
        agent.api_key = "stale-sophos-key"
        agent.base_url = "https://inference-api.sophostechne.com/v1"
        agent._anthropic_api_key = "stale-sophos-key"
        agent._anthropic_base_url = "https://inference-api.sophostechne.com/v1"
        ...
        assert agent.base_url == "https://inference-api.sophostechne.com/v1"
        assert agent._anthropic_api_key == "fresh-portal-jwt"
        assert agent._anthropic_base_url == (
            "https://inference-api.sophostechne.com/v1"
        )
...
class TestGpt5ApiModeRouting:
    ...
    def test_sophos_gpt5_stays_on_chat_completions(self, agent):
        """Sophos serves gpt-5.x on /chat/completions — must not upgrade to codex_responses."""
        agent.provider = "sophos"
        agent.base_url = "https://inference-api.sophostechne.com/v1"
        agent.api_mode = "chat_completions"
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Slick Agent" in agent._cached_system_prompt
