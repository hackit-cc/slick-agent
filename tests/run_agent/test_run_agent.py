...
class TestRunConversation:
    ...
    def test_hackit_401_refreshes_after_remint_and_retries(self, agent):
        self._setup_agent(agent)
        agent.provider = "hackit"
        ...
        with (
            ...
            patch.object(agent, "_interruptible_api_call", side_effect=_fake_api_call),
            patch.object(
                agent, "_try_refresh_hackit_client_credentials", side_effect=_fake_refresh
            ),
        ):
            result = agent.run_conversation("hello")
...
class TestRetryExhaustion:
    ...
    def test_content_filter_refusal_surfaced_not_retried(self, agent):
        ...
        portal (Hackit Portal fronting Anthropic) returns ``message.refusal``
...
class TestHackitCredentialRefresh:
    """Verify Hackit credential refresh rebuilds the runtime client."""

    def test_try_refresh_hackit_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "slick_cli.auth.resolve_hackit_runtime_credentials", _fake_resolve
        )
        ...
        with patch("run_agent.OpenAI", side_effect=_fake_openai):
            ok = agent._try_refresh_hackit_client_credentials(force=True)
        ...
        assert rebuilt["kwargs"]["api_key"] == "new-hackit-key"
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.hackit.cc/v1"
        )
        assert "default_headers" not in rebuilt["kwargs"]
        assert isinstance(agent.client, _RebuiltClient)

    def test_try_refresh_hackit_client_credentials_rebuilds_anthropic_client(
        self, agent, monkeypatch
    ):
        ...
        agent.api_key = "stale-hackit-key"
        agent.base_url = "https://inference-api.hackit.cc/v1"
        agent._anthropic_api_key = "stale-hackit-key"
        agent._anthropic_base_url = "https://inference-api.hackit.cc/v1"
        ...
        assert agent.base_url == "https://inference-api.hackit.cc/v1"
        assert agent._anthropic_api_key == "fresh-portal-jwt"
        assert agent._anthropic_base_url == (
            "https://inference-api.hackit.cc/v1"
        )
...
class TestGpt5ApiModeRouting:
    ...
    def test_hackit_gpt5_stays_on_chat_completions(self, agent):
        """Hackit serves gpt-5.x on /chat/completions — must not upgrade to codex_responses."""
        agent.provider = "hackit"
        agent.base_url = "https://inference-api.hackit.cc/v1"
        agent.api_mode = "chat_completions"
