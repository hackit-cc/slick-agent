"""Regression guard: a genuine Hackit 429 must re-enter the retry loop so the
top-of-loop Hackit rate-limit guard can activate the fallback chain.
...
``record_hackit_rate_limit()`` moments earlier and either activates a fallback
...
class TestGenuineHackit429ReentersLoop:
