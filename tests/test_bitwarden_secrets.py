...
binary download are exercised manually by `slick secrets bitwarden
...
def slick_home(tmp_path, monkeypatch):
    """Point Slick at an isolated home directory."""
    home = tmp_path / ".slick"
    home.mkdir()
    monkeypatch.setenv("SLICK_HOME", str(home))
    # Some modules cache get_slick_home; clear if needed.
    import slick_constants
    if hasattr(slick_constants, "_SLICK_HOME_CACHE"):
        slick_constants._SLICK_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(slick_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from slick_cli.env_loader import _apply_external_secret_sources
