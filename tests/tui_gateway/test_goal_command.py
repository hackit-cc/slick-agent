...
def slick_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves SLICK_HOME.
...
@pytest.fixture()
def server(slick_home, monkeypatch):
    ...
    # Pin config resolution to the isolated SLICK_HOME. Sibling test
    ...
    # module-level ``_slick_home = get_slick_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_slick_home", slick_home)
...
def test_moa_bare_returns_usage(server, session, slick_home):
    _write_moa_config(slick_home, """
moa:
