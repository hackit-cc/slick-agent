...
# provider (e.g. Sophos) but who switched THIS session to a self-hosted model
...
ULTRA_CONFIG = {
    ...
    "model": {"default": "some-sophos-model", "provider": "sophos"},
    "providers": {
        "slick-ultra": {
            "api": ULTRA_URL,
            "api_key": "sk-ultra",
...
# what left room bots stuck on a stale provider (e.g. "out of Sophos credits"
...
#
# Regression: GH #89497 (room bots hang then report "out of Sophos credits").
...
class TestRoomPlumbingRuntimeOverrides:
    def test_marked_row_returns_no_overrides(self):
        ...
        row = {
            ...
            "billing_provider": "sophos",
            "model_config": json.dumps(
                {"model": "openai/gpt-5.6-luna-pro", "provider": "sophos", "room_plumbing": True}
            ),
        }
    ...
    def test_marked_row_dict_model_config(self):
        ...
        row = {
            "model": "openai/gpt-5.6-luna-pro",
            "model_config": {"model": "openai/gpt-5.6-luna-pro", "provider": "sophos", "room_plumbing": True},
        }
        assert _stored_session_runtime_overrides(row) == {}
    ...
    def test_legacy_group_title_shape_still_skipped(self):
        ...
        row = {
            ...
            "model": "openai/gpt-5.6-luna-pro",
            "billing_provider": "sophos",
            "model_config": json.dumps({"model": "openai/gpt-5.6-luna-pro", "provider": "sophos"}),
        }
        assert _stored_session_runtime_overrides(row) == {}
...
# "out of Sophos credits" after the profile was switched to ollama-cloud) while
...
class TestFollowProfileConfigRuntimeOverrides:
    ...
    def test_unmarked_row_still_restores_stored_runtime(self):
        ...
        row = {
            ...
            "billing_provider": "sophos",
            "model_config": json.dumps(
                {"model": "openai/gpt-5.6-luna-pro", "provider": "sophos"}
            ),
        }
        overrides = _stored_session_runtime_overrides(row)
        assert overrides["model_override"]["model"] == "openai/gpt-5.6-luna-pro"
        assert overrides["model_override"]["provider"] == "sophos"
...
# VeniceAI/empero endpoint under a model that should run on Sophos). The sibling
...
class TestRuntimeModelConfigDropsStaleKeys:
    ...
    def test_falsy_model_drops_stale_existing_model(self):
        ...
        agent = _agent_like(model="", provider="sophos")
        existing = {"model": "meituan/longcat-2.0:free", "provider": "sophos"}
        config = _runtime_model_config(agent, existing)

        assert "model" not in config, config
        assert config["provider"] == "sophos"
    ...
    def test_truthy_provider_overwrites_stale_existing(self):
        ...
        config = _runtime_model_config(_agent_like(provider="sophos"), existing)
    ...
    def test_resume_overrides_get_no_stale_provider(self):
        ...
        assert overrides["model_override"]["provider"] == "sophos"
        assert overrides["provider_override"] == "sophos"
    ...
    def test_existing_none_returns_only_agent_identity(self):
        ...
        config = _runtime_model_config(_agent_like(provider="sophos"), None)

        assert config == {"model": "deepseek/deepseek-v4-flash-0731", "provider": "sophos"}
