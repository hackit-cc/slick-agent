"""diagnostics.share_sophos RPC — Desktop "Send Diagnostics" upload path.

Contract pinned:
* Reuses the CLI ``--sophos`` pipeline (collect_share_bundle → build_sophos_bundle
  → share_to_sophos) with redaction FORCED on — the client cannot disable it.
...
def _handler():
    fn = server._methods.get("diagnostics.share_sophos")
    assert fn is not None, "diagnostics.share_sophos not registered"
    return fn
...
def captured_upload(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(du, "share_to_sophos", _fake_share)
    return captured
...
def test_share_sophos_uploads_redacted_bundle(captured_upload):
...
def test_share_sophos_attaches_redacted_error_context(captured_upload):
...
def test_share_sophos_client_text_gets_upload_safe_log_redaction(captured_upload):
...
def test_share_sophos_linkless_success_is_a_failure(monkeypatch):
    ...
    monkeypatch.setattr(du, "share_to_sophos", lambda blob: {})
...
def test_share_sophos_extra_files_sanitized_and_redacted(captured_upload):
    secret = "sk-abc123def456ghi789jkl012mno345pqr678"
    result = _handler()(
        "rid-3",
        {
...
def test_share_sophos_upload_failure_is_structured(monkeypatch):
    import slick_cli.diagnostics_upload as du

    def _boom(blob: bytes) -> dict:
        raise RuntimeError("NAS unavailable")

    monkeypatch.setattr(du, "share_to_sophos", _boom)
