    ...
    with patch.dict(
        "sys.modules",
        {
            "slick_constants": MagicMock(
                get_slick_home=MagicMock(return_value="/tmp/slick_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'slick-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'slick-release' patched"
    }
