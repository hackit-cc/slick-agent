...
Tests cover:
- managed_sophos_tools_enabled() subscription-based gate
...
from slick_cli.sophos_account import SophosPaidServiceAccessInfo, SophosPortalAccountInfo
...
# managed_sophos_tools_enabled
# ---------------------------------------------------------------------------
class TestManagedSophosToolsEnabled:
    """Subscription-based gate: True for paid Sophos subscribers."""

    def test_disabled_when_not_logged_in(self, monkeypatch):
        monkeypatch.setattr(
            "slick_cli.sophos_account.get_sophos_portal_account_info",
            lambda: SophosPortalAccountInfo(logged_in=False, source="none", fresh=False),
        )
        assert managed_sophos_tools_enabled() is False
...
class TestSophosToolGatewayUnavailableMessage:
    def test_uses_entitlement_reason_for_logged_in_user(self, monkeypatch):
        monkeypatch.setattr(
            "slick_cli.sophos_account.get_sophos_portal_account_info",
            lambda force_fresh=False: SophosPortalAccountInfo(
                logged_in=True,
                source="account_api",
                fresh=True,
        ...
        message = sophos_tool_gateway_unavailable_message("managed image generation")
...
class TestResolveModalBackendState:
    ...
    @staticmethod
    def _resolve(monkeypatch, mode, *, has_direct, managed_ready, sophos_enabled=False):
        """Helper to call resolve_modal_backend_state with feature flag control."""
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_sophos_tools_enabled",
            lambda: sophos_enabled,
        )
    ...
    def test_auto_prefers_managed_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=True, sophos_enabled=True)
    ...
    def test_direct_selects_direct_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=True, managed_ready=True, sophos_enabled=True)
        assert result["selected_backend"] == "direct"

    def test_direct_none_when_no_credentials(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=False, managed_ready=True, sophos_enabled=True)
        assert result["selected_backend"] is None

    # --- managed mode ---


    def test_managed_blocked_when_sophos_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=True, sophos_enabled=False)
        assert result["selected_backend"] is None
