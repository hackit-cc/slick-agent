...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad SLICK_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: SLICK_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_slick_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "SLICK_HOME": "/h", "SLICK_PROFILE": "p",
        "SLICK_CONFIG": "/c.yaml", "SLICK_ENV": "/e",
        "SLICK_DELEGATED_CHILD_CONTEXT": "1",
        # other SLICK_* → dropped (broad prefix removed)
        "SLICK_BASE_URL": "https://x", "SLICK_INTERACTIVE": "1",
        "SLICK_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "SLICK_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SLICK_BASE_URL", "SLICK_INTERACTIVE", "SLICK_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped SLICK_* vars."""
    import logging
