...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("slick_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("SLICK_SESSION_KEY", session_key)
        monkeypatch.setenv("SLICK_EXEC_ASK", "1")
        monkeypatch.delenv("SLICK_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("slick-verify-", "slick-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "slick-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/slick-verify-example.py",
            "rm -f /tmp/slick-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../slick-verify-example.py",
            "rm -f /tmp/a/../../tmp/slick-verify-example.py",
            "rm -f /var/tmp/slick-verify-example.py",
            "rm -f /tmp/slick-verify-*",
            "rm -f /tmp/slick-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/slick-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/slick-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\slick-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\slick-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\slick-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"SLICK_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.slick/.env",
            "echo x | tee $SLICK_HOME/.env",
            'echo x | tee "$SLICK_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestSlickConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.slick/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.slick/config.yaml",
            "echo '  mode: off' >> ~/.slick/config.yaml",
            "echo x | tee ~/.slick/config.yaml",
            "echo x | tee $SLICK_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.slick/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Slick absolute config.yaml is
        # handled by the project patterns, not the Slick-home rule.
        for cmd in (
            "cat ~/.slick/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.slick/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Slick-home prefixes must fold to ~/ and
    ~/.slick/ in dangerous-command detection.
    ...
    SSH, and Slick config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_slick_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/SLICK_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("SLICK_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.slick/slick-agent && source venv/bin/activate && python -m slick_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m slick_cli.main gateway run --replace &",
            "nohup python -m slick_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart slick-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Slick security config via IFS.
            "sed${IFS}-i ~/.slick/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep slick) bypasses the pkill/killall name-matching
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof slick)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof slick_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof slick`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Slick service
    label achieves the same effect as `slick gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_slick_label_detected(self):
        for cmd in (
            "launchctl stop ai.slick.gateway",
            "launchctl kickstart -k system/ai.slick.gateway",
            "launchctl bootout system/ai.slick.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.slick.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Slick labels, are
    ...
    def test_spliced_detection_does_not_flag_prose_or_other_services(self):
        ...
        a slick-gateway identifier, so quoted prose and non-gateway slick
    ...
    def test_label_built_before_verb_detected(self):
        ...
        "slick"/"ai.slick" to appear AFTER the verb and missed this
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("SLICK_GATEWAY_SESSION", "SLICK_CRON_SESSION",
                      "SLICK_YOLO_MODE",
                      "SLICK_SESSION_KEY", "SLICK_INTERACTIVE")
        }
        os.environ.pop("SLICK_YOLO_MODE", None)
        os.environ.pop("SLICK_INTERACTIVE", None)
        # SLICK_CRON_SESSION takes priority over SLICK_GATEWAY_SESSION in
        ...
        os.environ.pop("SLICK_CRON_SESSION", None)
        os.environ["SLICK_GATEWAY_SESSION"] = "1"
        os.environ["SLICK_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("slick_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"SLICK_INTERACTIVE": "1"}, clear=False):
...
GATEWAY_LIFECYCLE_LAUNCHCTL = (
    ...
    "launchctl unload ~/Library/LaunchAgents/ai.slick.gateway.plist",
    "launchctl load ~/Library/LaunchAgents/ai.slick.gateway.plist",
    "launchctl stop ai.slick.gateway",
    "launchctl restart ai.slick.gateway",
    "launchctl bootout gui/501/ai.slick.gateway",
    "launchctl remove ai.slick.gateway",
    "launchctl disable gui/501/ai.slick.gateway",
)
...
class TestLifecycleGuardLaunchctlParity:
    ...
    under ``_SLICK_GATEWAY == "1"``, the ``cron.lifecycle_guard`` block is
    ...
    def test_unrelated_labels_are_not_blocked(self):
        ...
        services, including other Slick ones, stay runnable."""
        from cron.lifecycle_guard import contains_gateway_lifecycle_command

        for cmd in (
            "launchctl bootout gui/501/com.example.unrelated",
            "launchctl remove ai.slick.update-checker",
