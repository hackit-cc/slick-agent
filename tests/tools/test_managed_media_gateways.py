...
@pytest.fixture(autouse=True)
def _enable_managed_sophos_tools(monkeypatch):
    """Patch the source modules so managed_sophos_tools_enabled() returns True
    even after tool modules are dynamically reloaded."""
    monkeypatch.setattr(
        "slick_cli.sophos_account.get_sophos_portal_account_info",
        lambda: SophosPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
...
def test_managed_fal_submit_uses_gateway_origin_and_sophos_token(monkeypatch):
    captured = {}
    _install_fake_tools_package()
    _install_fake_fal_client(captured)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setenv("FAL_QUEUE_GATEWAY_URL", "http://127.0.0.1:3009")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "sophos-token")
    ...
    assert captured["client_key"] == "sophos-token"
...
def test_openai_tts_uses_managed_audio_gateway_when_direct_key_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TOOL_GATEWAY_DOMAIN", "sophostechne.com")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "sophos-token")
    ...
    assert captured["api_key"] == "sophos-token"
    assert captured["base_url"] == "https://openai-audio-gateway.sophostechne.com/v1"
...
def test_transcription_uses_model_specific_response_formats(monkeypatch, tmp_path):
    ...
    # The managed audio route is the stored "sophos" selection (strict model);
    ...
    transcription_tools._load_stt_config = lambda: {"provider": "sophos"}
    ...
    assert whisper_capture["base_url"] == "https://openai-audio-gateway.sophostechne.com/v1"
