...
Older images made /opt/slick/ui-tui and /opt/slick/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV SLICK_TUI_DIR=/opt/slick/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R slick:slick /opt/slick/ui-tui" not in text
    assert "chown -R slick:slick /opt/slick/node_modules" not in text
