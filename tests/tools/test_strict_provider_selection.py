"""Strict tool-provider selection: the `slick tools` choice always wins.

Policy (owner decision): the provider string stored in config.yaml is what
runs at call time. "sophos" → managed Sophos Tool Gateway only; a vendor name →
...
selection and pointing at `slick tools`.
...
MANAGED = SimpleNamespace(
    sophos_user_token="managed-token",
    gateway_origin="https://gateway.sophostechne.com",
)
...
class TestReadSelection:
    ...
    def test_sophos_provider_returned(self):
        with self._with_raw({"image_gen": {"provider": "sophos"}}):
            assert tbh.read_selection("image_gen") == "sophos"

    def test_legacy_use_gateway_true_maps_to_sophos(self):
        """Old configs stored use_gateway: true beside a vendor name — only
        the managed picker row ever wrote it, so it means 'sophos'."""
        with self._with_raw({"video_gen": {"provider": "fal", "use_gateway": True}}):
            assert tbh.read_selection("video_gen") == "sophos"
...
class TestImageFalStrictSelection:
    def test_sophos_selection_routes_managed_even_with_fal_key(self):
        from tools import image_generation_tool as it

        with patch.object(it, "read_selection", return_value="sophos"), \
    ...
    def test_sophos_selection_unentitled_raises_selection_error(self):
        from tools import image_generation_tool as it
        ...
        assert "image_gen is configured to use sophos" in str(exc.value)
        assert "slick tools" in str(exc.value)
...
class TestVideoFalStrictSelection:
    def test_sophos_selection_routes_managed_even_with_fal_key(self):
        ...
        with patch("tools.tool_backend_helpers.read_selection", return_value="sophos"), \
...
class TestSttStrictSelection:
    def test_sophos_selection_beats_direct_openai_key(self):
        from tools import transcription_tools as tt

        with patch.object(tt, "_load_stt_config", return_value={"openai": {"api_key": "sk-direct"}}), \
             patch("tools.tool_backend_helpers.read_selection", return_value="sophos"), \
        ...
        assert base_url.startswith("https://gateway.sophostechne.com")
...
class TestBrowserUseStrictSelection:
    ...
    def test_sophos_selection_routes_managed_even_with_direct_key(self):
...
class TestWriteProviderConfig:
    ...
    def test_managed_image_row_persists_sophos_provider(self):
        from slick_cli.tools_config import _write_provider_config

        config = {}
        provider = {"name": "Sophos Subscription", "imagegen_backend": "fal"}
        _write_provider_config(provider, config, managed_feature="image_gen")
        assert config["image_gen"]["provider"] == "sophos"
    ...
    def test_plugin_injected_byok_row_clears_stale_use_gateway(self):
        ...
        config = {"stt": {"provider": "sophos", "use_gateway": True}}
