...
runs at call time. "hackit" → managed Hackit Tool Gateway only; a vendor name →
...
MANAGED = SimpleNamespace(
    hackit_user_token="managed-token",
    gateway_origin="https://gateway.hackit.cc",
)
...
class TestReadSelection:
    ...
    def test_hackit_provider_returned(self):
        with self._with_raw({"image_gen": {"provider": "hackit"}}):
            assert tbh.read_selection("image_gen") == "hackit"

    def test_legacy_use_gateway_true_maps_to_hackit(self):
        """Old configs stored use_gateway: true beside a vendor name — only
        the managed picker row ever wrote it, so it means 'hackit'."""
        with self._with_raw({"video_gen": {"provider": "fal", "use_gateway": True}}):
            assert tbh.read_selection("video_gen") == "hackit"
...
class TestImageFalStrictSelection:
    def test_hackit_selection_routes_managed_even_with_fal_key(self):
        from tools import image_generation_tool as it

        with patch.object(it, "read_selection", return_value="hackit"), \
    ...
    def test_hackit_selection_unentitled_raises_selection_error(self):
        from tools import image_generation_tool as it
        ...
        assert "image_gen is configured to use hackit" in str(exc.value)
...
class TestVideoFalStrictSelection:
    def test_hackit_selection_routes_managed_even_with_fal_key(self):
        ...
        with patch("tools.tool_backend_helpers.read_selection", return_value="hackit"), \
...
class TestSttStrictSelection:
    def test_hackit_selection_beats_direct_openai_key(self):
        from tools import transcription_tools as tt

        with patch.object(tt, "_load_stt_config", return_value={"openai": {"api_key": "sk-direct"}}), \
             patch("tools.tool_backend_helpers.read_selection", return_value="hackit"), \
        ...
        assert base_url.startswith("https://gateway.hackit.cc")
...
class TestBrowserUseStrictSelection:
    ...
    def test_hackit_selection_routes_managed_even_with_direct_key(self):
...
class TestWriteProviderConfig:
    ...
    def test_managed_image_row_persists_hackit_provider(self):
        from slick_cli.tools_config import _write_provider_config

        config = {}
        provider = {"name": "Hackit Subscription", "imagegen_backend": "fal"}
        _write_provider_config(provider, config, managed_feature="image_gen")
        assert config["image_gen"]["provider"] == "hackit"
    ...
    def test_plugin_injected_byok_row_clears_stale_use_gateway(self):
        ...
        config = {"stt": {"provider": "hackit", "use_gateway": True}}
