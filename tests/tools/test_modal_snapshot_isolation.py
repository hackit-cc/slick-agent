...
def _restore_tool_modules():
    original_slick_home = os.environ.get("SLICK_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_slick_home is None:
            os.environ.pop("SLICK_HOME", None)
        else:
            os.environ["SLICK_HOME"] = original_slick_home
        _reset_modules(("tools", "slick_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    slick_cli = types.ModuleType("slick_cli")
    slick_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["slick_cli"] = slick_cli
    slick_home = tmp_path / "slick-home"
    os.environ["SLICK_HOME"] = str(slick_home)
    sys.modules["slick_cli.config"] = types.SimpleNamespace(
        get_slick_home=lambda: slick_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="slick-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": slick_home / "modal_snapshots.json",
