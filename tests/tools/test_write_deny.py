...
class TestWriteDenyExactPaths:
    ...
    def test_slick_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.slick/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "slick_root"
        ...
        # Sanity check: SLICK_HOME does point to the profile dir, not the root.
        from slick_constants import get_slick_home, get_default_slick_root
...
class TestWriteAllowed:
    ...
    def test_slick_control_files_requested_writable(self):
