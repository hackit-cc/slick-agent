...
def test_output_path_rejects_slick_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    slick_home = tmp_path / "slick-home"
    slick_home.mkdir()
    monkeypatch.setattr(file_safety, "_slick_home_path", lambda: slick_home)
    monkeypatch.setattr(file_safety, "_slick_root_path", lambda: slick_home)

    target = slick_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    slick_home = tmp_path / "slick-home"
    token_dir = slick_home / "mcp-tokens"
