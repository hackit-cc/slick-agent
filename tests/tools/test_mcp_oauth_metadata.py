...
Covers:
- :class:`SlickTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.SlickMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _SLICK_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# SlickTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = SlickTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SLICK_HOME", str(tmp_path))
        storage = SlickTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (SlickMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: SlickTokenStorage, **context_attrs):
    ...
    provider = _SLICK_PROVIDER_CLS.__new__(_SLICK_PROVIDER_CLS)
    provider._slick_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SLICK_HOME", str(tmp_path))
        storage = SlickTokenStorage("mgr-srv")
        ...
        with patch.object(
            _SLICK_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SLICK_HOME", str(tmp_path))
        storage = SlickTokenStorage("flow-srv")
        ...
        with patch.object(
            _SLICK_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
