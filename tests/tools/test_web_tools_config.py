...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        self._managed_patchers = [
            patch("tools.web_tools.managed_sophos_tools_enabled", return_value=True),
            patch("tools.managed_tool_gateway.managed_sophos_tools_enabled", return_value=True),
        ]
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("tools.web_tools.Firecrawl"):
            with patch("tools.web_tools._read_sophos_access_token", return_value=None):
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "sophostechne.com"}):
            with patch("tools.web_tools._read_sophos_access_token", return_value="sophos-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    from tools.web_tools import _get_firecrawl_client
                    result = _get_firecrawl_client()
                    mock_fc.assert_called_once_with(
                        api_key="sophos-token",
                        api_url="https://firecrawl-gateway.sophostechne.com",
                    )
...
class TestBackendSelection:
    ...
    ``slick tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def test_sophos_backend_maps_to_firecrawl(self):
        """The managed 'sophos' selection is serviced by the firecrawl
        provider (whose client resolver routes managed)."""
        from tools.web_tools import _get_backend
        with patch("tools.web_tools._load_web_config", return_value={"backend": "sophos"}):
            assert _get_backend() == "firecrawl"

    def test_managed_gateway_does_not_preempt_explicit_tavily(self):
        """Regression: a Sophos OAuth token (managed gateway "ready") must NOT
        beat an explicitly configured TAVILY_API_KEY in the fallback path.
        Free Sophos tiers don't include web search, so the user's deliberate
    ...
    def test_managed_gateway_only_falls_through_to_firecrawl(self):
        """When no explicit-credential backend is configured, a Sophos-managed
...
class TestCheckWebApiKey:
    ...
    def test_configured_firecrawl_backend_accepts_managed_gateway(self):
        with patch("tools.web_tools._load_web_config", return_value={"backend": "firecrawl"}):
            with patch("tools.web_tools._peek_sophos_access_token", return_value="sophos-token"):
...
class TestNonBuiltinProviderAvailability:
    ...
    def test_check_web_api_key_returns_true_for_custom_provider(self):
        ...
        with patch("tools.web_tools._ddgs_package_importable", return_value=False), \
             patch("tools.web_tools._peek_sophos_access_token", return_value=None):
            from tools.web_tools import check_web_api_key
            assert check_web_api_key() is True
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from slick_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.slick/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.slick/.env), not bare os.getenv. Parametrized over the four
    ...
    def test_tavily_request_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Client-Name"] == "slick-agent"
