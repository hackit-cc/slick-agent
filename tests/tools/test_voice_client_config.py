...
Real config files in a temp SLICK_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated SLICK_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".slick"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "SLICK_LOCAL_STT_LANGUAGE",
    ):
