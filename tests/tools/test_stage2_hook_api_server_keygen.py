...
was gated on ``[ -f "$SLICK_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'SLICK_HOME="{home}"\n'
        # In tests we run unprivileged; as_slick is a passthrough then.
        'as_slick() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Slick loads $SLICK_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/slick/.env.example -> $SLICK_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $SLICK_HOME/.env from it on first boot"
    )
