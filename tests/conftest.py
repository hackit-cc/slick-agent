"""Shared fixtures for the slick-agent test suite.
...
2. **Isolated SLICK_HOME.** SLICK_HOME points to a per-test tempdir so
   code reading ``~/.slick/*`` via ``get_slick_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".slick"`` instead of the canonical
   ``get_slick_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No SLICK_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox SLICK_HOME before ANY test module is imported ──────────────────
# `slick_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_slick_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.slick/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes SLICK_HOME, but fixtures run
...
# the REAL Slick root — capture it BEFORE the sandbox rewires SLICK_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.slick (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("SLICK_KANBAN_HOME", "").strip()
_PRE_SANDBOX_SLICK_HOME = os.environ.get("SLICK_HOME", "")


def _slick_home_points_at_production(value: str) -> bool:
    """True when a pre-set SLICK_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    SLICK_HOME=~/.slick``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``slick_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.slick`` — the escape vector that landed pytest
    ...
    SLICK_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".slick").resolve()
    except Exception:
        return True
...
if _slick_home_points_at_production(os.environ.get("SLICK_HOME", "")):
    _SESSION_SLICK_HOME = tempfile.mkdtemp(prefix="slick-test-home-")
    os.environ["SLICK_HOME"] = _SESSION_SLICK_HOME
    atexit.register(shutil.rmtree, _SESSION_SLICK_HOME, True)
...
# like a real CLI") — which used to disarm slick_state's live-DB guard in
# the child at the same moment the child lost the SLICK_HOME redirect.
# SLICK_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# slick_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export SLICK_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["SLICK_TEST_ISOLATION"] = os.environ.get("SLICK_HOME", "") or "1"

#: SLICK_HOME as it stood when conftest was imported - i.e. before any test
...
SLICK_HOME_AT_CONFTEST_IMPORT = os.environ.get("SLICK_HOME", "")
...
# SLICK_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_SLICK_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``slick_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Slick agent session inherits them and
    # slick_constants home-resolution helpers prefer them over monkeypatched
    ...
    "SLICK_MAX_ITERATIONS",
    "SLICK_SESSION_PLATFORM",
    "SLICK_SESSION_CHAT_ID",
    "SLICK_SESSION_CHAT_NAME",
    "SLICK_SESSION_CHAT_TYPE",
    "SLICK_SESSION_THREAD_ID",
    "SLICK_SESSION_SOURCE",
    "SLICK_SESSION_KEY",
    "SLICK_GATEWAY_SESSION",
    "SLICK_CRON_SESSION",
    "_SLICK_GATEWAY",
    "SLICK_PLATFORM",
    "SLICK_MODEL",
    "SLICK_INFERENCE_MODEL",
    "SLICK_INFERENCE_PROVIDER",
    "SLICK_TUI_PROVIDER",
    "SLICK_MANAGED",
    "SLICK_MANAGED_DIR",
    "SLICK_DEV",
    ...
    "SLICK_TIMEZONE",
    "SLICK_REDACT_SECRETS",
    "SLICK_BACKGROUND_NOTIFICATIONS",
    "SLICK_EXEC_ASK",
    "SLICK_HOME_MODE",
    "SLICK_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.slick/kanban.db instead of the per-test SLICK_HOME.
    "SLICK_KANBAN_DB",
    "SLICK_KANBAN_BOARD",
    "SLICK_KANBAN_HOME",
    "SLICK_KANBAN_WORKSPACES_ROOT",
    "SLICK_KANBAN_LOGS_ROOT",
    "SLICK_KANBAN_TASK",
    "SLICK_KANBAN_WORKSPACE",
    "SLICK_KANBAN_RUN_ID",
    "SLICK_KANBAN_CLAIM_LOCK",
    "SLICK_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "slick" host.
    "SLICK_HONCHO_HOST",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth `hackit` plugin auto-registers itself on plugin discovery,
    ...
    "SLICK_DASHBOARD_OAUTH_CLIENT_ID",
    "SLICK_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SLICK_HOME to per-test tempdirs so code that
    reads ``~/.slick/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral SLICK_* vars that could change test semantics.
    for name in _SLICK_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("SLICK_HONCHO_HOST", "slick")

    # 3. Redirect SLICK_HOME to a per-test tempdir. Code that reads
    #    ``~/.slick/*`` via ``get_slick_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.slick/*`` via
    #    ``Path.home() / ".slick"`` instead of ``get_slick_home()``
    #    is a bug to fix at the callsite.
    fake_slick_home = tmp_path / "slick_test"
    fake_slick_home.mkdir()
    (fake_slick_home / "sessions").mkdir()
    (fake_slick_home / "cron").mkdir()
    (fake_slick_home / "memories").mkdir()
    (fake_slick_home / "skills").mkdir()
    monkeypatch.setenv("SLICK_HOME", str(fake_slick_home))
    ...
    # slick_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("SLICK_TEST_ISOLATION", str(fake_slick_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("SLICK_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. slick_state computes ``DEFAULT_DB_PATH = get_slick_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from slick_state import ...``) that
    ...
    slick_state_mod = sys.modules.get("slick_state")
    if slick_state_mod is not None and hasattr(slick_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            slick_state_mod, "DEFAULT_DB_PATH", fake_slick_home / "state.db"
        )
    ...
    monkeypatch.setenv("SLICK_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.slick/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import slick_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path SLICK_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_slick_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from slick_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.slick. This autouse
...
# allow-list because test-level fixtures legitimately move SLICK_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session SLICK_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``SLICK_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Slick root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_SLICK_HOME and not _slick_home_points_at_production(
        _PRE_SANDBOX_SLICK_HOME
    ):
        # SLICK_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_slick_root().resolve()
    # No pre-existing SLICK_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".slick").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.slick`` captured at import time. Hermetic tests that legitimately
    move SLICK_HOME to sibling tempdirs are unaffected.

    Only patches when ``slick_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("slick_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import slick_cli.kanban_db`
...
# ``slick_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Slick root. This fixture wires the
# test-side knobs:
...
#     installs where SLICK_HOME is not ~/.slick) into the guard's
...
# so subprocess children that import slick_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("slick_state")
    ...
    if _PRE_SANDBOX_SLICK_HOME and not _slick_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_SLICK_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Slick home override redirects every later
    # ``get_slick_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from slick_constants import get_slick_home_override, set_slick_home_override

        if get_slick_home_override() is not None:
            set_slick_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal slick config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``slick-gateway`` process
...
# correlated with a ``tests/slick_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> slick-gateway``
...
def _wal_is_usable() -> bool:
    """True when Slick will actually put a database into WAL mode here.

    Slick refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Slick can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Slick managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``slick_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_slick_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_slick_home`` fixture
    redirects ``SLICK_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.slick/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``slick_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["SLICK_VOICE_TTS"] = "1"``. The
...
#      ``slick_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``slick_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``slick_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``slick_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Slick runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.slick``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Slick falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Slick uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart slick-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting slick/python patterns are also blocked.
    """
    ...
    def _matches_slick_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _SLICK_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_slick_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch slick-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m slick_cli.main".
                if (
                    "slick" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live slick-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting slick/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `slick update` (or the
        # equivalent `python -m slick_cli.main update`).  These commands
        ...
        if "update" in low and (
            # slick update / slick update --gateway / setsid bash -c ... slick update
            ("slick" in low and "update" in low.split())
            or
            # python -m slick_cli.main update --gateway
            ("slick_cli" in low and "update" in low.split())
            or
            # venv/bin/slick update  (absolute path variant used in tests)
            (".venv/bin/slick" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`slick update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_SLICK_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import slick_cli.voice as _voice
    except Exception:
