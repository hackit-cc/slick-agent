...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("slick_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated SLICK_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="slick_api_content_")
    os.makedirs(os.path.join(test_home, ".slick"))
    prev_home = os.environ.get("SLICK_HOME")
    os.environ["SLICK_HOME"] = os.path.join(test_home, ".slick")
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["SLICK_HOME"] = prev_home
