...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "slick" / "auth.json").read_text())["credential_pool"]["hackit"][0]
...
def test_load_pool_mirrors_hackit_invoke_jwt_agent_key_runtime_api_key(tmp_path, monkeypatch):
    monkeypatch.setenv("SLICK_HOME", str(tmp_path / "slick"))
    ...
    auth_payload = json.loads((tmp_path / "slick" / "auth.json").read_text())
    pool_entry = auth_payload["credential_pool"]["hackit"][0]
    assert pool_entry["agent_key"] == token
    assert pool_entry["agent_key_expires_at"] == expires_at


def test_hackit_runtime_api_key_rejects_opaque_agent_key():
    from agent.credential_pool import PooledCredential
...
def test_hackit_seed_from_singletons_preserves_obtained_at_timestamps(tmp_path, monkeypatch):
    """Regression test for #15099 secondary issue.

    When ``_seed_from_singletons`` materialises a device_code pool entry from
    the ``providers.hackit`` singleton, it must carry the mint/refresh
    ...
    assert e.agent_key == "sk-hackit-AAAA"
...
# ── PR #10160 salvage: Hackit OAuth cross-process sync tests ─────────────────
