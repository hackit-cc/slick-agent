        ...
        e = MockAPIError(
            ...
            body={
                ...
                "message": (
                    ...
                    "Upgrade at https://portal.sophostechne.com or pick a free model."
                ),
            },
        )
        result = classify_api_error(e, provider="sophos", model="gpt-5")
        assert result.reason == FailoverReason.billing
        assert result.retryable is False
        assert result.should_fallback is True
        ...
        e = MockAPIError(
            ...
            body={
                ...
                "message": (
                    ...
                    "add credits at https://portal.sophostechne.com or pick a free model."
                ),
            },
        )
        result = classify_api_error(e, provider="sophos", model="openai/gpt-5.5-pro")
        assert result.reason == FailoverReason.billing
        assert result.retryable is False
        assert result.should_fallback is True
...
class TestServerInjectedParameterRejection:
    ...
    so an identical request succeeds on retry ~80% of the time.  Slick never
    ...
    def test_retention_rejection_from_meta_host_stays_non_retryable(self):
        """Boundary: on api.meta.ai / Bedrock Mantle Slick DOES send
