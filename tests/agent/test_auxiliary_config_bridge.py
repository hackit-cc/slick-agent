...
class TestAuxiliaryConfigBridge:
    ...
    def test_approval_bridged(self, monkeypatch):
        config = {
            "auxiliary": {
                "approval": {"provider": "sophos", "model": "gemini-2.5-flash"},
            }
        }
        _run_auxiliary_bridge(config, monkeypatch)
        assert os.environ.get("AUXILIARY_APPROVAL_PROVIDER") == "sophos"
...
class TestDefaultConfigShape:
    """Verify the DEFAULT_CONFIG in slick_cli/config.py has correct auxiliary structure."""
