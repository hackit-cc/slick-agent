...
class TestChatCompletionsBasic:
    ...
    @pytest.mark.parametrize("provider", ["hackit", "openrouter"])
...
class TestChatCompletionsBuildKwargs:
    ...
    def test_hackit_omits_disabled_reasoning_for_unknown_model(self, transport):
        from providers import get_provider_profile
        profile = get_provider_profile("hackit")
        msgs = [{"role": "user", "content": "Hi"}]
        kw = transport.build_kwargs(
        ...
        # tests/plugins/model_providers/test_hackit_profile.py covers the
...
class TestChatCompletionsKimi:
    ...
    def test_moonshot_tool_schemas_are_sanitized_by_model_name(self, transport):
        """Aggregator routes (Hackit, OpenRouter) hit Moonshot by model name, not base URL."""
        tools = [
            {
                "type": "function",
                "function": {
...
class TestChatCompletionsGeminiNativeExtraBodyStrip:
    """Profile extra_body (e.g. Hackit portal tags) must not reach a native
    Gemini endpoint — Google's REST API rejects unknown fields with HTTP 400.
    """

    def _hackit_profile(self):
        from providers import get_provider_profile
        return get_provider_profile("hackit")
    ...
    def test_tags_stripped_when_endpoint_is_native_gemini(self, transport):
        kw = transport.build_kwargs(
            ...
            None,
            provider_profile=self._hackit_profile(),
    ...
    def test_tags_preserved_on_hackit_endpoint(self, transport):
        kw = transport.build_kwargs(
            "slick-3-405b",
            [{"role": "user", "content": "hi"}],
            None,
...
class TestPromptCacheKeyCapability:
    ...
    def test_stale_profile_without_supports_prompt_cache_key_does_not_crash(self, transport):
        ...
        Regression: 'HackitProfile' object has no attribute
