...
class TestChatCompletionsBasic:
    ...
    @pytest.mark.parametrize("provider", ["sophos", "openrouter"])
    ...
    def test_convert_messages_strips_internal_scaffolding_markers(self, transport):
        """Slick-internal ``_``-prefixed markers must never reach the wire.
...
class TestChatCompletionsBuildKwargs:
    ...
    def test_sophos_omits_disabled_reasoning_for_unknown_model(self, transport):
        from providers import get_provider_profile
        profile = get_provider_profile("sophos")
        msgs = [{"role": "user", "content": "Hi"}]
        kw = transport.build_kwargs(
        ...
        # tests/plugins/model_providers/test_sophos_profile.py covers the
...
class TestChatCompletionsKimi:
    ...
    def test_moonshot_tool_schemas_are_sanitized_by_model_name(self, transport):
        """Aggregator routes (Sophos, OpenRouter) hit Moonshot by model name, not base URL."""
        tools = [
            {
                "type": "function",
                "function": {
...
class TestChatCompletionsGeminiNativeExtraBodyStrip:
    """Profile extra_body (e.g. Sophos portal tags) must not reach a native
    Gemini endpoint — Google's REST API rejects unknown fields with HTTP 400.
    """

    def _sophos_profile(self):
        from providers import get_provider_profile
        return get_provider_profile("sophos")
    ...
    def test_tags_stripped_when_endpoint_is_native_gemini(self, transport):
        kw = transport.build_kwargs(
            ...
            None,
            provider_profile=self._sophos_profile(),
    ...
    def test_tags_preserved_on_sophos_endpoint(self, transport):
        kw = transport.build_kwargs(
            "slick-3-405b",
            [{"role": "user", "content": "hi"}],
            None,
...
class TestPromptCacheKeyCapability:
    ...
    def test_stale_profile_without_supports_prompt_cache_key_does_not_crash(self, transport):
        ...
        Regression: 'SophosProfile' object has no attribute
