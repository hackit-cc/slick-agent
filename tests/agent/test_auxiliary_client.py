...
from agent.auxiliary_client import (
    ...
    _refresh_hackit_recommended_model,
...
def _clean_env(monkeypatch):
    ...
    for key in (
        ...
        "OPENAI_MODEL", "LLM_MODEL", "HACKIT_INFERENCE_BASE_URL",
                    ...
                    "moa": {
                        ...
                        "presets": {
                            ...
                            "hackit-mix": {
                                "enabled": True,
                                "reference_models": [
                                    {"provider": "hackit", "model": "slick-4-70b"}
                                ],
                                "aggregator": {
                                    "provider": "hackit",
                                    "model": "slick-4-405b",
...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("hackit", "slick-4", "https://inference-api.hackit.cc/v1", "max_tokens"),
        ],
    )
...
class TestHackitTagsScoping:
    def test_tags_injected_when_provider_is_hackit(self, monkeypatch):
        import agent.auxiliary_client as aux

        monkeypatch.setattr(aux, "auxiliary_is_hackit", False)
        ...
        assert kwargs["extra_body"]["tags"] == aux._hackit_portal_tags()

    def test_tags_not_injected_for_gemini_when_main_is_hackit(self, monkeypatch):
        import agent.auxiliary_client as aux

        monkeypatch.setattr(aux, "auxiliary_is_hackit", True)
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    fallback chain (OpenRouter / Hackit / etc.), so aux tasks billed
...
class TestGetTextAuxiliaryClient:
    ...
    def test_returns_none_when_nothing_available(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_hackit_auth", return_value=None), \
    ...
    def test_custom_endpoint_uses_codex_wrapper_when_runtime_requests_responses_api(self):
        with patch("agent.auxiliary_client._resolve_custom_runtime",
             ...
             patch("agent.auxiliary_client._read_hackit_auth", return_value=None), \
             patch("agent.auxiliary_client._resolve_hackit_runtime_api", return_value=None), \
...
class TestAuxiliaryPoolAwareness:
    ...
    def test_try_hackit_refreshes_stale_pool_entry(self):
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=pool),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("slick_cli.models.get_hackit_recommended_aux_model", return_value=None),
        ):
            from agent.auxiliary_client import _try_hackit

            client, model = _try_hackit()

        assert pool.refreshed is True
        assert client is not None
        assert model == _HACKIT_MODEL
    ...
    def test_call_llm_retries_hackit_after_401(self):
        class _Auth401(Exception):
            status_code = 401

        stale_client = MagicMock()
        stale_client.base_url = "https://inference-api.hackit.cc/v1"
        stale_client.chat.completions.create.side_effect = _Auth401("stale hackit key")

        fresh_client = MagicMock()
        fresh_client.base_url = "https://inference-api.hackit.cc/v1"
        fresh_client.chat.completions.create.return_value = {"ok": True}

        with (
            patch("agent.auxiliary_client._resolve_task_provider_model", return_value=("hackit", "hackit-model", None, None, None)),
            patch("agent.auxiliary_client._get_cached_client", return_value=(stale_client, "hackit-model")),
            ...
            patch("agent.auxiliary_client._resolve_hackit_runtime_api", return_value=("fresh-agent-key", "https://inference-api.hackit.cc/v1")),
        ):
            result = call_llm(
                task="compression",
                messages=[{"role": "user", "content": "hi"}],
...
class TestIsModelNotFoundError:
    ...
    def test_hackit_openrouter_catalog_404(self):
        """The exact incident error: a Portal-recommended model dropped from
        the Hackit → OpenRouter catalog."""
        exc = Exception(
...
class TestRefreshHackitRecommendedModel:
    """_refresh_hackit_recommended_model picks a fresh model after a stale 404."""
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "slick_cli.models.get_hackit_recommended_aux_model", _boom)
        out = _refresh_hackit_recommended_model(
            vision=False, stale_model="some/dead-model")
        assert out == _HACKIT_MODEL
    ...
    def test_returns_none_when_no_distinct_alternative(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "slick_cli.models.get_hackit_recommended_aux_model",
            lambda **kw: _HACKIT_MODEL,
        )
        out = _refresh_hackit_recommended_model(
            vision=False, stale_model=_HACKIT_MODEL)
        assert out is None
...
class TestGetProviderChain:
    ...
    def test_returns_four_entries(self):
        ...
        assert labels == ["openrouter", "hackit", "local/custom", "api-key"]
...
class TestTryPaymentFallback:
    ...
    def test_skips_failed_provider(self):
        ...
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_hackit", return_value=(mock_client, "hackit-model")), \
             patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"):
            client, model, label = _try_payment_fallback("openrouter", task="compression")
        assert client is mock_client
        assert model == "hackit-model"
        assert label == "hackit"
    ...
    def test_codex_not_in_fallback_chain(self):
        ...
        When OR/Hackit/custom/api-key all fail, payment-fallback returns None —
        Codex is never tried with a guessed model.
        """
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_hackit", return_value=(None, None)), \
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/hackit-cc/slick-agent/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/hackit-cc/slick-agent/issues/18478
    """
...
def test_pool_runtime_base_url_uses_hackit_env_override(monkeypatch):
    entry = SimpleNamespace(
        provider="hackit",
        runtime_base_url="https://inference-api.hackit.cc/v1",
        inference_base_url="https://inference-api.hackit.cc/v1",
        base_url="https://inference-api.hackit.cc/v1",
    )
    monkeypatch.setenv("HACKIT_INFERENCE_BASE_URL", "https://ai.wildebeest-newton.ts.net/v1")
...
class TestAuxUnhealthyCache:
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        hackit_client = MagicMock()
        hackit_resp = MagicMock()
        hackit_resp.choices = [MagicMock(message=MagicMock(content="ok"))]
        hackit_client.chat.completions.create.return_value = hackit_resp
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(hackit_client, "n-model", "hackit")), \
...
class TestFastModelTier:
    ...
    def test_catalog_match_prefers_rolling_alias_over_pinned_id(self):
        ...
        with patch("slick_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("hackit") == "~openai/gpt-mini-latest"
    ...
    def test_catalog_match_skips_reasoning_batch_and_embedding_lookalikes(self):
        ...
        with patch("slick_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("hackit") == "google/gemini-3.6-flash"
    ...
    def test_catalog_match_skips_the_non_chat_siblings_of_a_chat_model(self):
        ...
        with patch("slick_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("hackit") == "openai/gpt-4o-mini"
    ...
    def test_catalog_match_takes_the_newest_of_a_family(self):
        ...
        with patch("slick_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("hackit") == "openai/gpt-10-mini"
    ...
    def test_fast_tier_is_opt_in(self):
        ...
        with patch.object(ac, "_fast_model_from_catalog") as spy:
            ac._get_aux_model_for_provider("hackit")
