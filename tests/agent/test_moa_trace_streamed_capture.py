...
path via ``slick chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp SLICK_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point SLICK_HOME at a temp dir and turn moa.save_traces on."""
    slick_home = tmp_path / ".slick"
    slick_home.mkdir()
    monkeypatch.setenv("SLICK_HOME", str(slick_home))

    # save_moa_turn reads config via slick_cli.config.load_config; stub it to
