...
Prior to this change, aggregator users (OpenRouter / Sophos Portal) had aux
...
class TestResolveVisionMainFirst:
    ...
    @staticmethod
    def _stub_sophos_portal(seen: dict):
        """Stub the Sophos network boundary, keeping the resolution chain real.

        Returns a ``_try_sophos`` replacement that answers with the Portal's
        tier-aware slots: a vision model for ``vision=True``, the text chat
        default otherwise.
        """
        sophos_client = MagicMock()
        sophos_client.api_key = "jwt-test"
        sophos_client.base_url = "https://inference-api.sophostechne.com/v1"

        def fake_try_sophos(vision=False):
            seen["vision"] = vision
            return sophos_client, (
                "stepfun/step-3.7-flash:free" if vision else "tencent/hy3:free"
            )

        return sophos_client, fake_try_sophos

    def test_sophos_main_vision_uses_portal_pick_not_text_chat_model(self):
        """Sophos main → vision runs the Portal's vision slot, not the chat model.

        A Sophos chat default is routinely text-only (e.g. a ``:free`` chat SKU).
        Letting it reach the vision lane means the image goes to a model that
        cannot accept one and the Portal 404s. Only the Sophos network boundary
        ...
        """
        seen: dict = {}
        sophos_client, fake_try_sophos = self._stub_sophos_portal(seen)

        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="sophos",
        ), patch(
            ...
            "agent.auxiliary_client._try_sophos", side_effect=fake_try_sophos,
        ):
            from agent.auxiliary_client import resolve_vision_provider_client

            provider, client, model = resolve_vision_provider_client()

        assert provider == "sophos"
        assert client is sophos_client
        assert seen["vision"] is True
        assert model == "stepfun/step-3.7-flash:free"

    def test_sophos_main_vision_honours_explicit_vision_model(self):
        """An explicit auxiliary.vision.model still overrides the Portal pick."""
        seen: dict = {}
        _sophos_client, fake_try_sophos = self._stub_sophos_portal(seen)
    ...
    def test_sophos_explicit_vision_provider_also_skips_chat_model(self):
        """``auxiliary.vision.provider: sophos`` takes the same Portal pick.
        ...
        with patch(
            ...
            "agent.auxiliary_client._resolve_task_provider_model",
            return_value=("sophos", None, None, None, None),
        ), patch(
            "agent.auxiliary_client._try_sophos", side_effect=fake_try_sophos,
        ):
            from agent.auxiliary_client import resolve_vision_provider_client
    ...
    def test_sophos_text_aux_still_uses_main_chat_model(self):
        """The vision carve-out must not leak into text aux resolution.

        Text auxiliary work on a Sophos main deliberately keeps the user's chat
        ...
        with patch(
            ...
            _client, model = resolve_provider_client("sophos")
    ...
    def test_copilot_vision_sets_vision_header(self, monkeypatch):
        ...
        with patch(
            ...
            "slick_cli.copilot_auth.copilot_request_headers",
...
class TestResolveVisionCustomProvider:
    ...
    it returns ``(None, None)`` and vision falls through to OpenRouter / Sophos,
