...
Prior to this change, aggregator users (OpenRouter / Hackit Portal) had aux
...
class TestResolveVisionMainFirst:
    ...
    @staticmethod
    def _stub_hackit_portal(seen: dict):
        """Stub the Hackit network boundary, keeping the resolution chain real.

        Returns a ``_try_hackit`` replacement that answers with the Portal's
        tier-aware slots: a vision model for ``vision=True``, the text chat
        default otherwise.
        """
        hackit_client = MagicMock()
        hackit_client.api_key = "jwt-test"
        hackit_client.base_url = "https://inference-api.hackit.cc/v1"

        def fake_try_hackit(vision=False):
            seen["vision"] = vision
            return hackit_client, (
                "stepfun/step-3.7-flash:free" if vision else "tencent/hy3:free"
            )

        return hackit_client, fake_try_hackit

    def test_hackit_main_vision_uses_portal_pick_not_text_chat_model(self):
        """Hackit main → vision runs the Portal's vision slot, not the chat model.

        A Hackit chat default is routinely text-only (e.g. a ``:free`` chat SKU).
        Letting it reach the vision lane means the image goes to a model that
        cannot accept one and the Portal 404s. Only the Hackit network boundary
        ...
        """
        seen: dict = {}
        hackit_client, fake_try_hackit = self._stub_hackit_portal(seen)

        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="hackit",
        ), patch(
            ...
            "agent.auxiliary_client._try_hackit", side_effect=fake_try_hackit,
        ):
            from agent.auxiliary_client import resolve_vision_provider_client

            provider, client, model = resolve_vision_provider_client()

        assert provider == "hackit"
        assert client is hackit_client
        assert seen["vision"] is True
        assert model == "stepfun/step-3.7-flash:free"

    def test_hackit_main_vision_honours_explicit_vision_model(self):
        """An explicit auxiliary.vision.model still overrides the Portal pick."""
        seen: dict = {}
        _hackit_client, fake_try_hackit = self._stub_hackit_portal(seen)
    ...
    def test_hackit_explicit_vision_provider_also_skips_chat_model(self):
        """``auxiliary.vision.provider: hackit`` takes the same Portal pick.
        ...
        with patch(
            ...
            "agent.auxiliary_client._resolve_task_provider_model",
            return_value=("hackit", None, None, None, None),
        ), patch(
            "agent.auxiliary_client._try_hackit", side_effect=fake_try_hackit,
        ):
            from agent.auxiliary_client import resolve_vision_provider_client
    ...
    def test_hackit_text_aux_still_uses_main_chat_model(self):
        """The vision carve-out must not leak into text aux resolution.

        Text auxiliary work on a Hackit main deliberately keeps the user's chat
        ...
        with patch(
            ...
            _client, model = resolve_provider_client("hackit")
...
class TestResolveVisionCustomProvider:
    ...
    it returns ``(None, None)`` and vision falls through to OpenRouter / Hackit,
