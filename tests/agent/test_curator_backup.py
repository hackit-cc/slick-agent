...
def backup_env(monkeypatch, tmp_path):
    """Isolate SLICK_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".slick"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_slick_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd slick_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under SLICK_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level SLICK_DIR picks up the tmp HOME."""
    import slick_constants
    importlib.reload(slick_constants)
