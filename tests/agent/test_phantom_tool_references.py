...
Covers:
  * SLICK_AGENT_HELP_GUIDANCE degrades to the docs-only variant when the
...
class TestSlickAgentHelpGuidance:
    def test_skill_variant_used_when_skill_view_present(self):
        from agent.prompt_builder import SLICK_AGENT_HELP_GUIDANCE
        assert "skill_view(name='slick-agent')" in SLICK_AGENT_HELP_GUIDANCE

    def test_no_skills_variant_has_no_skill_view_reference(self):
        from agent.prompt_builder import SLICK_AGENT_HELP_GUIDANCE_NO_SKILLS
        assert "skill_view" not in SLICK_AGENT_HELP_GUIDANCE_NO_SKILLS
        assert "slick-agent.sophostechne.com/docs" in SLICK_AGENT_HELP_GUIDANCE_NO_SKILLS
...
class TestEssentialSkillsUndisableable:
    def test_agent_side_reader_strips_essential(self, monkeypatch, tmp_path):
        ...
        assert "slick-agent" not in disabled
...
class TestEssentialOnlySync:
    def test_opted_out_sync_seeds_only_essential(self, monkeypatch, tmp_path):
        """A profile with .no-bundled-skills still gets the slick-agent skill."""
        ...
        for cat, name in [
            ("autonomous-ai-agents", "slick-agent"),
            ("media", "gif-search"),
        ]:
            d = bundled / cat / name
        ...
        monkeypatch.setattr(ss, "_slick_home", lambda: home)
        ...
        assert result["copied"] == ["slick-agent"]
        assert (home / "skills" / "autonomous-ai-agents" / "slick-agent" / "SKILL.md").exists()
