...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate SLICK_HOME under tmp_path."""
    monkeypatch.setenv("SLICK_HOME", str(tmp_path / ".slick"))
    (tmp_path / ".slick").mkdir()

    # Force the constants/image cache helpers to re-read SLICK_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("slick_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_slick_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under SLICK_HOME — gateway cleanup
