...
def isolated_home(monkeypatch):
    """Temp SLICK_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="slick_test_31179_")
    slick_home = os.path.join(test_home, ".slick")
    os.makedirs(slick_home)
...
def _fresh_modules():
    """Drop cached slick modules so each test reloads against current env."""
    _drop_reload_targets()
