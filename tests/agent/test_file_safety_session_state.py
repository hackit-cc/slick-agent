...
manipulation. Slick keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point SLICK_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_slick_home /
    get_default_slick_root) instead of monkeypatching private helpers —
    ...
    SLICK_HOME=<root>/profiles/<name> makes get_default_slick_root()
