...
Reports live under ``~/.slick/logs/curator/{YYYYMMDD-HHMMSS}/`` alongside
...
def curator_env(tmp_path, monkeypatch):
    """Isolated SLICK_HOME with a skills/ dir + reset curator module state."""
    home = tmp_path / ".slick"
    home.mkdir()
    (home / "skills").mkdir()
    (home / "logs").mkdir()
...
def curator_env_with_cron(curator_env, monkeypatch):
    ...
    monkeypatch.setattr(jobs_mod, "SLICK_DIR", home)
