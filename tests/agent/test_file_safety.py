...
class TestCacheFileReadBlocking:
    """Internal Slick cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        slick_home = tmp_path / ".slick"
        cache = slick_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._slick_home_path", return_value=slick_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Slick cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        slick_home = tmp_path / ".slick"
        hub = slick_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_slick_home(self, tmp_path):
        """The env basename guard does not depend on SLICK_HOME resolution."""
        slick_home = tmp_path / ".slick"
        slick_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        slick_home = tmp_path / ".slick"
        cache = slick_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
