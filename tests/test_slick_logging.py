"""Tests for slick_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import slick_logging
# Use whatever RotatingFileHandler class slick_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. slick_logging
...
from slick_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    slick_logging._logging_initialized = False
    ...
    slick_logging._reset_queued_handlers()
    ...
    slick_logging._install_session_record_factory()
    yield
    ...
    slick_logging._logging_initialized = False
    slick_logging.clear_session_context()


@pytest.fixture
def slick_home(tmp_path, monkeypatch):
    """Provide an isolated SLICK_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_slick_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["SLICK_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, slick_home):
        log_dir = slick_logging.setup_logging(slick_home=slick_home)
        assert log_dir == slick_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, slick_home):
        slick_logging.setup_logging(slick_home=slick_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in slick_logging.rotating_file_handlers()
    ...
    def test_idempotent_no_duplicate_handlers(self, slick_home):
        slick_logging.setup_logging(slick_home=slick_home)
        slick_logging.setup_logging(slick_home=slick_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, slick_home):
        slick_logging.setup_logging(slick_home=slick_home)

        test_logger = logging.getLogger("test_slick_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        slick_logging.flush_log_queue()

        agent_log = slick_home / "logs" / "agent.log"
    ...
    def test_explicit_params_override_config(self, slick_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (slick_home / "config.yaml").write_text(yaml.dump(config))

        slick_logging.setup_logging(slick_home=slick_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, slick_home):
        slick_logging.setup_logging(slick_home=slick_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, slick_home):
        slick_logging.setup_logging(slick_home=slick_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, slick_home):
        ...
        gw_log = slick_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, slick_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, slick_home):
        slick_logging.setup_logging(slick_home=slick_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, slick_home):
        slick_logging.setup_logging(slick_home=slick_home, mode="gui")

        logging.getLogger("slick_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        slick_logging.flush_log_queue()

        gui_log = slick_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, slick_home):
        """When session context is set, log lines include [session_id]."""
        slick_logging.setup_logging(slick_home=slick_home)
        slick_logging.set_session_context("abc123")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = slick_logging._ComponentFilter(("gateway",))
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, slick_home):
        slick_logging.setup_logging(slick_home=slick_home)
        slick_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_slick_verbose", False)
        ]
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        slick_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in slick_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
        ...
        slick_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("slick_cli.config.is_managed", return_value=True):
                slick_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = slick_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert slick_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not slick_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, slick_home):
        level, max_size, backup = slick_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, slick_home):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> slick_logging._ManagedRotatingFileHandler:
        handler = slick_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that slick_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        slick_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        slick_logging.setup_logging(slick_home=slick_home, mode="gateway")
        gw_path = slick_home / "logs" / "gateway.log"
        rotated = slick_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = slick_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, slick_home):
        slick_logging.setup_logging(slick_home=slick_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_slick_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in slick_logging.rotating_file_handlers()
        )
