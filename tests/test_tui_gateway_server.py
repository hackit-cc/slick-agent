...
def test_resolve_model_strips_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " sophos/slick-test "}}
    )

    assert server._resolve_model() == "sophos/slick-test"
...
def test_config_sync_switches_unpinned_session(monkeypatch):
    _patch_config_model(monkeypatch, "new/model", provider="sophos")
    session = _sync_test_session(config_model_seen=("old/model", "sophos"))
    calls = []
    monkeypatch.setattr(
        server,
        "_apply_model_switch",
    ...
    assert session["config_model_seen"] == ("new/model", "sophos")
...
def test_config_sync_switches_when_only_provider_differs(monkeypatch):
    _patch_config_model(monkeypatch, "old/model", provider="sophos")
    session = _sync_test_session(config_model_seen=("old/model", ""))
    calls = []
    monkeypatch.setattr(
        server,
    ...
    assert calls == ["old/model --provider sophos"]
...
def test_config_model_target_never_reads_env(monkeypatch):
    ...
    monkeypatch.setenv("SLICK_INFERENCE_MODEL", "seed/model")
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "sophos"}})

    assert server._config_model_target() == ("", "sophos")
...
def test_apply_model_switch_persist_override_false_never_persists(monkeypatch):
    ...
    out = server._apply_model_switch(
        "sid", session, "new/model --provider sophos", persist_override=False
    )
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("SLICK_MODEL", "sophos/slick-test")
    monkeypatch.setenv("SLICK_TUI_PROVIDER", "sophos")
    monkeypatch.delenv("SLICK_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("sophos/slick-test", "sophos")
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    ...
    monkeypatch.setenv("SLICK_INFERENCE_PROVIDER", "sophos")
    monkeypatch.setattr(
        "slick_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("sophos/slick-test", None)
...
def test_setup_runtime_check_honors_requested_provider(monkeypatch):
    ...
    scoped = server.handle_request(
        {"id": "1", "method": "setup.runtime_check", "params": {"provider": "sophos"}}
    )
    assert scoped["result"]["ok"] is True
    assert scoped["result"]["provider"] == "sophos"
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    for providers like Sophos whose /models endpoint returns image/video
    ...
    sophos = next((p for p in providers if p.get("slug") == "sophos"), None)
    assert sophos is not None
    assert sophos["models"] == [
        "moonshotai/kimi-k2.5",
        "anthropic/claude-opus-4.7",
    ]
    assert sophos["total_models"] == 30
...
def test_billing_error_serialization_preserves_server_code(
    ...
    import slick_cli.sophos_billing as nb
