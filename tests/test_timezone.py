"""
Tests for timezone support (slick_time module + integration points).
...
def _reset_slick_time_cache():
    """Reset the slick_time module cache (replacement for removed reset_cache)."""
    slick_time._cached_tz = None
    slick_time._cached_tz_name = None
    slick_time._cache_resolved = False


# =========================================================================
# slick_time.now() — core helper
# =========================================================================

class TestSlickTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_slick_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["SLICK_TIMEZONE"] = "Asia/Kolkata"
        result = slick_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["SLICK_TIMEZONE"] = "Europe/London"
        tz = slick_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When SLICK_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) SLICK_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("SLICK_TIMEZONE=" + os.environ.get("SLICK_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "SLICK_TIMEZONE=NOT_SET" in result["output"], (
            "SLICK_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When SLICK_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("SLICK_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=slick_tz) which shifted
        absolute time when system-local tz != Slick tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Slick tz
        ...
        # Use a Slick timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _slick_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["SLICK_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_slick_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Slick timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["SLICK_TIMEZONE"] = "US/Eastern"
        _reset_slick_time_cache()
