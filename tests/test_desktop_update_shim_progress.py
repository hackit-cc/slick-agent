...
def progress(tmp_path):
    ...
    status = tmp_path / "slick-update-status"
...
# Stands in for `slick update`, and reports the stage that was on screen
...
n="$(cat "$SLICK_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$SLICK_TEST_CALLS"
for f in "$TMPDIR"/slick-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$SLICK_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$SLICK_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "slick-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    slick = install_root / "venv" / "bin" / "slick"
    slick.write_text(FAKE_SLICK)
    slick.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "SLICK_TEST_CAPTURE": str(capture),
        "SLICK_TEST_CALLS": str(calls),
        "SLICK_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".slick-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`slick update` is the longest wait in the hand-off and the one the
