...
Override the image with ``SLICK_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("SLICK_TEST_IMAGE", "slick-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``SLICK_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("SLICK_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged slick user
# ---------------------------------------------------------------------------
#
# Background: every Slick runtime path inside the container drops to UID
# 10000 (the ``slick`` user) via ``s6-setuidgid slick``. ``docker exec``
...
# fails (PermissionError swallowed) for slick, so a test that ran as root
...
# helpers below run every probe under ``-u slick`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: slick).
