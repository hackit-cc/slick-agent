"""Runtime smoke test for Docker $SLICK_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the slick user after container boot.
...
registered later runs its log service as the dropped slick user and
...
def test_logs_gateways_seeded_and_slick_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by slick after boot."""
    ...
    assert "logs=slick" in r.stdout, (
        f"logs/ not owned by slick: {r.stdout}"
    )
    assert "gateways=slick" in r.stdout, (
        f"logs/gateways/ not owned by slick: {r.stdout}"
    )
