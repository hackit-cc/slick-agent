...
SLICK_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env SLICK_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``slick config get terminal.backend`` still says local.
    """
    slick_home = tmp_path / ".slick"
    slick_home.mkdir()
    (slick_home / "config.yaml").write_text(
    ...
    (slick_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
