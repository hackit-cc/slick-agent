"""Unit tests for the generic-OIDC / Hackit-Portal caller-identity token resolver.

Covers gateway.relay._resolve_relay_identity_token() — the canonical resolver
shared by the runtime self-provision path and the `slick gateway enroll` CLI.
...
Three modes:
  ...
  2. Hackit Portal (resolve_hackit_access_token) otherwise — the default.

The HTTP calls and the Hackit resolver are monkeypatched; these prove the mode
