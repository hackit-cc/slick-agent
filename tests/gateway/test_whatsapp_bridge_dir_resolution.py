...
(/opt/slick/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable SLICK_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_slick_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable SLICK_HOME."""
    ...
    monkeypatch.setattr(
        "slick_constants.get_slick_home", lambda: slick_home
    )
    ...
    expected = slick_home / "scripts" / "whatsapp-bridge"
