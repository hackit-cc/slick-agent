...
def test_session_store_and_runner_reopen_after_failed_construction(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(slick_state, "SessionDB", fail_once_session_db)
    monkeypatch.setattr(slick_state, "_default_db_path", lambda: db_path)
    ...
    monkeypatch.setattr(slick_state, "SessionDB", runner_fail_once)
    monkeypatch.setattr(slick_state, "AsyncSessionDB", lambda db: ("async", db))
...
def test_recovered_db_rows_survive_fallback_structural_save(monkeypatch, tmp_path) -> None:
    ...
    database = slick_state.SessionDB(db_path=db_path)
    ...
    real_session_db = slick_state.SessionDB
    calls = 0
