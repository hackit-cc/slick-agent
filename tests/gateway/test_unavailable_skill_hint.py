...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`slick_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + SLICK_HOME so the real user config is untouched."""
    home = tmp_path / ".slick"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `slick skills config` writes).
    monkeypatch.setattr(
    ...
    assert "slick skills config" in msg
