...
closure the PR changed, against a real temp ``SLICK_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "slick_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "slick_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_slick_home", slick_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_slick_home() / config.yaml`` — point it here.
    monkeypatch.setattr("slick_constants.get_slick_home", lambda: slick_home)
    monkeypatch.setattr("slick_cli.config.get_slick_home", lambda: slick_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_slick_home", default_home)
    _stub_picker_dependencies(monkeypatch)
