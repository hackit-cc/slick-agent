...
- Unlimited timeout (SLICK_CRON_TIMEOUT=0)
- Backward compat: SLICK_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """SLICK_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """SLICK_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("SLICK_CRON_TIMEOUT", "1200")
...
class TestSysPathOrdering:
    ...
    def test_slick_time_importable(self):
        """slick_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _slick_now
        assert callable(_slick_now)
