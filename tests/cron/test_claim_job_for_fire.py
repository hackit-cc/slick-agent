...
scheduler (Webcron) fires a job: across N gateway replicas, exactly ONE wins the
claim for a given fire. Single-machine deployments always win (unaffected).

These exercise the real store against a temp SLICK_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    ...
    # cron.jobs caches no home at import; get_slick_home() reads the env live.
    yield tmp_path
