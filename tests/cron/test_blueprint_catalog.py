...
the shared /blueprint command handler (slick_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated SLICK_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("slick://blueprint/morning-brief?")
