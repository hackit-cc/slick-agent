...
def cron_store(tmp_path, monkeypatch):
    ...
    (slick_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("SLICK_HOME", str(slick_home))
    monkeypatch.setattr(J, "SLICK_DIR", slick_home)
    monkeypatch.setattr(J, "CRON_DIR", slick_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", slick_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", slick_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_slick_now", lambda: now):
            due = J.get_due_jobs()
