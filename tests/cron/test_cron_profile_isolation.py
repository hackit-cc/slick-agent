...
profile's SLICK_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.slick/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active SLICK_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever SLICK_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    SLICK_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import slick_constants

    monkeypatch.setattr(
        slick_constants, "_get_platform_default_slick_home", lambda: root
    )
    monkeypatch.setenv("SLICK_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile SLICK_HOME (<root>/profiles/<name>), the cron store
    ...
    assert slick_constants.get_slick_home().resolve() == profile_home.resolve()
    assert slick_constants.get_default_slick_root().resolve() == root.resolve()

    # cron/jobs.py computes SLICK_DIR from get_slick_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.SLICK_DIR.resolve() == profile_home.resolve()
        assert (
