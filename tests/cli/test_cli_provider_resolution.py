...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setitem(
        cli.CLI_CONFIG,
        "model",
        {"default": "tencent/hy3:free", "provider": "hackit"},
    )
    config = {
        "model": {"default": "tencent/hy3:free", "provider": "hackit"},
        "providers": {
            "gmk-lan": {
                "name": "GMK Local",
...
def test_model_flow_hackit_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    import yaml
    ...
    monkeypatch.setattr(
        ...
        lambda provider: {
            "access_token": "hackit-token",
            "portal_base_url": "https://portal.example.com",
        },
    )
    monkeypatch.setattr(
        "slick_cli.auth.resolve_hackit_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "slick_cli.models.get_curated_hackit_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("slick_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("slick_cli.models.check_hackit_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "slick_cli.hackit_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )

    slick_main._model_flow_hackit(stale_config, current_model="glm-5.2")

    config = yaml.safe_load(config_path.read_text()) or {}
    model = config.get("model")
    assert model["provider"] == "hackit"
    assert model["default"] == selected_model
    assert model["base_url"] == "https://inference-api.hackit.cc/v1"
...
def test_cmd_model_forwards_hackit_login_tls_options(monkeypatch):
    monkeypatch.setattr(slick_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "slick_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "hackit"}},
    )
    monkeypatch.setattr("slick_cli.config.save_config", lambda cfg: None)
    ...
    monkeypatch.setattr("slick_cli.auth.resolve_provider", lambda requested, **kwargs: "hackit")
    ...
    monkeypatch.setattr("slick_cli.auth._login_hackit", _fake_login)

    slick_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.hackit.cc",
            inference_url="https://inference.hackit.cc/v1",
