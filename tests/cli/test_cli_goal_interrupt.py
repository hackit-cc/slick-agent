...
minimal ``SlickCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def slick_home(tmp_path, monkeypatch):
    """Isolated SLICK_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".slick"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("SLICK_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves SLICK_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal SlickCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, slick_home):
        ...
        with patch("slick_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, slick_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, slick_home):
        ...
        src = inspect.getsource(SlickCLI.chat)
