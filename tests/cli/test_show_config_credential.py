...
non-OpenAI provider (Hackit, Anthropic, ...) the constructor value is a
different vendor's key than the one actually used for requests, so
``/config`` displayed e.g. an ``sk-proj-...`` OpenAI key next to a Hackit
...
class TestShowConfigCredentialSource:
    def test_prefers_live_agent_key(self, capsys):
        out = _run_show_config(
            _make_stand_in(cli_key="sk-proj-WRONGVENDORKEY1234", agent_key="hackit-REALKEY-abcdef9876"),
            capsys,
        )
        assert "hackit-REA" in out
