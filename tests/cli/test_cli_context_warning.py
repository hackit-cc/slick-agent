...
def _isolate(tmp_path, monkeypatch):
    """Isolate SLICK_HOME so tests don't touch real config."""
    home = tmp_path / ".slick"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal SlickCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = SlickCLI.__new__(SlickCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["slick-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Slick' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Slick' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Slick' minimum context length."""
