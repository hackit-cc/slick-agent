...
#
# Installs a real, earlier Slick the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `slick`.
#
...
# Keep sandbox state out of the default .slick-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".slick-sandbox-e2e-$ROUTE"
export SLICK_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/slick/.slick/slick-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/slick-agent.git"
...
LOG_DIR="${SLICK_E2E_LOG_DIR:-$(mktemp -d -t slick-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED slick accept FLAG on `slick update`?
#
...
# that silently rots. `slick update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "slick update --help 2>&1" | grep -qF -- "$flag"
}
...
require_slick_works() {
  local when="$1" out
  out="$(in_sandbox "slick --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "slick --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "slick runs $when"
}

# ── install the earlier Slick ─────────────────────────────────────────────
...
require_slick_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: slick update'
    ...
    # slick which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="slick update --yes"
    else
      update_cmd="slick update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "slick update failed ($update_cmd)"
    fi
    require_landed_on_target 'slick update'
    require_slick_works 'after slick update'
    ;;
  installer)
    ...
    require_slick_works 'after installer re-run'
    ;;
esac
