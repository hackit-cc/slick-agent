"""Unit tests for the Hackit Portal profile's reasoning wiring.
...
@pytest.fixture
def hackit_profile():
    """Resolve the registered Hackit profile through the real discovery path."""
    # ``model_tools`` triggers plugin discovery on import, which is what
    # registers the Hackit profile in the global provider registry.
    import model_tools  # noqa: F401
    import providers

    profile = providers.get_provider_profile("hackit")
    assert profile is not None, "hackit provider profile must be registered"
    return profile
...
def portal_catalog(monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "_hackit_reasoning_caps_failed_at", None)
    monkeypatch.setattr(models_mod, "_hackit_reasoning_caps_cache", {
...
class TestHackitReasoningWireShape:
    """``build_api_kwargs_extras`` produces the Portal's wire format."""

    def test_disable_reaches_optional_reasoning_model(self, hackit_profile, portal_catalog):
        """The knob the user set is the knob the Portal receives."""
        extra_body, top_level = hackit_profile.build_api_kwargs_extras(
    ...
    def test_disable_dropped_for_mandatory_reasoning_model(self, hackit_profile, portal_catalog):
        """Mandatory routes 400 on a disable — send nothing instead."""
        extra_body, _ = hackit_profile.build_api_kwargs_extras(
            reasoning_config={"enabled": False},
            supports_reasoning=True,
    ...
    def test_disable_dropped_for_unknown_model(self, hackit_profile, portal_catalog):
    ...
    def test_disable_dropped_for_non_reasoning_route(self, hackit_profile, portal_catalog):
    ...
    def test_enabled_config_always_forwarded(self, hackit_profile, portal_catalog, model):
    ...
    def test_no_config_defaults_to_medium(self, hackit_profile, portal_catalog):
        extra_body, _ = hackit_profile.build_api_kwargs_extras(
    ...
    def test_nothing_emitted_without_reasoning_support(self, hackit_profile, portal_catalog):
        extra_body, top_level = hackit_profile.build_api_kwargs_extras(
            reasoning_config={"enabled": False},
    ...
    def test_caller_config_not_mutated(self, hackit_profile, portal_catalog):
        cfg = {"enabled": False}
        hackit_profile.build_api_kwargs_extras(
