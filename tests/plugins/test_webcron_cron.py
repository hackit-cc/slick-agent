"""Unit tests for the Webcron NAS-mediated cron provider (Phase 4D).
...
@pytest.fixture
def webcron(monkeypatch):
    """A ChronosCronScheduler with a fake NAS client capturing calls."""
    from plugins.cron_providers.webcron import ChronosCronScheduler
    ...
    monkeypatch.setattr("plugins.cron_providers.webcron._cfg",
...
def test_is_available_false_without_config(temp_home, monkeypatch):
    ...
    monkeypatch.setattr("plugins.cron_providers.webcron._cfg", lambda *k, default="": "")
    assert ChronosCronScheduler().is_available() is False


# -- arming -------------------------------------------------------------------

def test_arm_one_shot_sends_provision(webcron):
    prov, fake = webcron
...
def test_register_job_arms_only_the_created_job(webcron):
    prov, fake = webcron
...
def test_register_job_propagates_provision_failure(webcron):
    prov, fake = webcron
...
def test_reconcile_arms_all_enabled(temp_home, webcron, monkeypatch):
    prov, fake = webcron
    jobs = [
...
def test_fire_due_rearms_next_oneshot(webcron, monkeypatch):
    prov, fake = webcron
...
def test_fire_due_rearms_after_claimed_job_failure(webcron, monkeypatch):
...
def test_fire_due_forwards_manual_force_to_claim(webcron, monkeypatch):
...
def test_fire_due_no_rearm_when_job_gone(webcron, monkeypatch):
...
def test_fire_due_no_rearm_when_claim_lost(webcron, monkeypatch):
...
def test_chronos_is_split_fire_capable(webcron):
    """Regression: Webcron must be classified as a split-aware provider so the
    fire webhook uses durable claim admission (not the legacy fire_due path).
    Webcron deliberately has NO fire_due override — its re-arm logic lives in
...
def test_fire_claimed_no_rearm_when_run_failed(webcron, monkeypatch):
    prov, fake = webcron
    monkeypatch.setattr(
        "cron.scheduler_provider.CronScheduler.fire_claimed",
...
def test_fire_claimed_no_rearm_when_job_gone(webcron, monkeypatch):
    prov, fake = webcron
    monkeypatch.setattr(
        "cron.scheduler_provider.CronScheduler.fire_claimed",
        lambda self, job, **kw: True,
