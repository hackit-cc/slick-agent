...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure SLICK_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", slick_home=str(tmp_path / ".slick"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SLICK_HOME", str(slick_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", slick_home=str(slick_home))
        return p
