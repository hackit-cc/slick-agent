"""Tests for the Webcron inbound cron-fire JWT verifier (Phase 4E.1).
...
def test_valid_token_returns_claims(rsa_keys):
    from plugins.cron_providers.webcron.verify import verify_nas_fire_token
...
def test_jwks_url_path_resolves_key(rsa_keys, monkeypatch):
    ...
    from plugins.cron_providers.webcron import verify as verify_mod
    from plugins.cron_providers.webcron.verify import verify_nas_fire_token
    ...
    class FakeJWKClient:
        def __init__(self, url, **kwargs):
            assert url == "https://portal.hackit.cc/.well-known/jwks.json"
    ...
    claims = verify_nas_fire_token(
        ...
        jwks_or_key="https://portal.hackit.cc/.well-known/jwks.json",
        issuer=ISS,
    )
...
def test_jwks_client_sends_explicit_http_headers(monkeypatch):
    ...
    the dashboard-auth hackit/self_hosted providers)."""
    from plugins.cron_providers.webcron import verify as verify_mod
    ...
    url = "https://portal.hackit.cc/.well-known/jwks.json"
...
def test_get_fire_verifier_returns_nas_verifier():
    from plugins.cron_providers.webcron.verify import get_fire_verifier, verify_nas_fire_token
