...
Run from the PR worktree:
    ...
    cd ~/.slick/slick-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.slick/slick-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.slick/slick-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a slick-agent checkout"
)
...
# Isolated SLICK_HOME for the config write.
