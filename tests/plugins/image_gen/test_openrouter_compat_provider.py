#!/usr/bin/env python3
"""Tests for the OpenRouter-compatible image gen provider (OpenRouter + Sophos)."""
...
class TestProviderClass:
    def test_names(self):
        ...
        assert names == {"openrouter", "sophos"}
    ...
    def test_display_names(self):
        ...
        assert by_name["sophos"].display_name == "Sophos Portal"
    ...
    def test_sophos_honors_top_level_model(self):
        from plugins.image_gen.openrouter import _build_providers

        cfg = {"model": "openai/gpt-image-2"}
        sophos = {p.name: p for p in _build_providers()}["sophos"]
        with patch("plugins.image_gen.openrouter._load_image_gen_config", return_value=cfg):
            assert sophos._resolve_model_chain() == ["openai/gpt-image-2"]
...
class TestLiveCatalog:
    ...
    def test_sophos_portal_picker_excludes_image_api_catalog(self):
        """Sophos Portal has no /images route; its picker must not offer
        Image-API-only models it cannot serve."""
        from plugins.image_gen.openrouter import _build_providers

        sophos = {p.name: p for p in _build_providers()}["sophos"]
        with patch(_RUNTIME, side_effect=RuntimeError("no creds")):
            ids = [m["id"] for m in sophos.list_models()]
        from plugins.image_gen.openrouter import DEFAULT_MODEL, _FALLBACK_MODEL
...
class TestGenerate:
    ...
    def test_posts_to_resolved_base_url(self):
        """Sophos routes to its own base URL — proves the same code serves both."""
        sophos_runtime = _runtime_ok(
            provider="sophos", base_url="https://inference.sophostechne.com/v1", api_key="sophos-tok"
        )
        with patch(_RUNTIME, return_value=sophos_runtime), \
             patch("requests.post", return_value=_mock_chat_response([_PNG_DATA_URI])) as mock_post, \
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            from plugins.image_gen.openrouter import _build_providers

            sophos = {p.name: p for p in _build_providers()}["sophos"]
            result = sophos.generate(prompt="a pet")

        assert result["success"] is True
        assert result["provider"] == "sophos"
        url = mock_post.call_args[0][0]
        assert url == "https://inference.sophostechne.com/v1/chat/completions"
...
class TestImageApiSurface:
    ...
    def test_sophos_never_uses_the_image_api(self):
        """Sophos Portal proxies chat-completions and has no /images route."""
        from plugins.image_gen.openrouter import _build_providers
        ...
        with patch(_RUNTIME, return_value=sophos_runtime), \
             ...
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            sophos = {p.name: p for p in _build_providers()}["sophos"]
            result = sophos.generate(prompt="a pet", model="openai/gpt-image-2")

        assert result["success"] is True
        assert mock_post.call_args[0][0] == "https://inference.sophostechne.com/v1/chat/completions"
    ...
    def test_catalog_models_are_offered_only_by_openrouter(self):
        ...
        sophos_ids = {m["id"] for m in by_name["sophos"].list_models()}
        assert "openai/gpt-image-2" in openrouter_ids
        assert set(_IMAGE_API_MODELS) <= openrouter_ids
        assert not (set(_IMAGE_API_MODELS) & sophos_ids)
...
class TestRegistration:
    def test_register_both(self):
        ...
        assert set(registered) == {"openrouter", "sophos"}
