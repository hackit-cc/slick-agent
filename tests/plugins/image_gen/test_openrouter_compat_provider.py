#!/usr/bin/env python3
"""Tests for the OpenRouter-compatible image gen provider (OpenRouter + Hackit)."""
...
_RUNTIME = "slick_cli.runtime_provider.resolve_runtime_provider"
...
class TestProviderClass:
    def test_names(self):
        ...
        assert names == {"openrouter", "hackit"}
    ...
    def test_display_names(self):
        ...
        assert by_name["hackit"].display_name == "Hackit Portal"
    ...
    def test_hackit_honors_top_level_model(self):
        from plugins.image_gen.openrouter import _build_providers

        cfg = {"model": "openai/gpt-image-2"}
        hackit = {p.name: p for p in _build_providers()}["hackit"]
        with patch("plugins.image_gen.openrouter._load_image_gen_config", return_value=cfg):
            assert hackit._resolve_model_chain() == ["openai/gpt-image-2"]
...
class TestLiveCatalog:
    ...
    def test_hackit_portal_picker_excludes_image_api_catalog(self):
        """Hackit Portal has no /images route; its picker must not offer
        Image-API-only models it cannot serve."""
        from plugins.image_gen.openrouter import _build_providers

        hackit = {p.name: p for p in _build_providers()}["hackit"]
        with patch(_RUNTIME, side_effect=RuntimeError("no creds")):
            ids = [m["id"] for m in hackit.list_models()]
        from plugins.image_gen.openrouter import DEFAULT_MODEL, _FALLBACK_MODEL
...
class TestGenerate:
    ...
    def test_posts_to_resolved_base_url(self):
        """Hackit routes to its own base URL — proves the same code serves both."""
        hackit_runtime = _runtime_ok(
            provider="hackit", base_url="https://inference.hackit.cc/v1", api_key="hackit-tok"
        )
        with patch(_RUNTIME, return_value=hackit_runtime), \
             patch("requests.post", return_value=_mock_chat_response([_PNG_DATA_URI])) as mock_post, \
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            from plugins.image_gen.openrouter import _build_providers

            hackit = {p.name: p for p in _build_providers()}["hackit"]
            result = hackit.generate(prompt="a pet")

        assert result["success"] is True
        assert result["provider"] == "hackit"
        url = mock_post.call_args[0][0]
        assert url == "https://inference.hackit.cc/v1/chat/completions"
...
class TestImageApiSurface:
    ...
    def test_hackit_never_uses_the_image_api(self):
        """Hackit Portal proxies chat-completions and has no /images route."""
        from plugins.image_gen.openrouter import _build_providers
        ...
        with patch(_RUNTIME, return_value=hackit_runtime), \
             ...
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            hackit = {p.name: p for p in _build_providers()}["hackit"]
            result = hackit.generate(prompt="a pet", model="openai/gpt-image-2")

        assert result["success"] is True
        assert mock_post.call_args[0][0] == "https://inference.hackit.cc/v1/chat/completions"
    ...
    def test_catalog_models_are_offered_only_by_openrouter(self):
        ...
        hackit_ids = {m["id"] for m in by_name["hackit"].list_models()}
        assert "openai/gpt-image-2" in openrouter_ids
        assert set(_IMAGE_API_MODELS) <= openrouter_ids
        assert not (set(_IMAGE_API_MODELS) & hackit_ids)
...
class TestRegistration:
    def test_register_both(self):
        ...
        assert set(registered) == {"openrouter", "hackit"}
