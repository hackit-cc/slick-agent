...
# worktree), ``MAIN_DIR`` falls back to a sibling ``slick-agent-main``
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "image_generation_tool.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "slick-agent-main"
...
assert (PR_DIR / "tools" / "image_generation_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a slick-agent checkout"
)
...
# Isolated SLICK_HOME so the config write is hermetic.
home = tempfile.mkdtemp()
os.environ["SLICK_HOME"] = home
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'slick-agent-main' checkout pinned to "
