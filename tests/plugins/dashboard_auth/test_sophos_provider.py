"""Tests for the bundled Sophos dashboard-auth plugin.

Covers four shapes from Phase 4 of ``.slick/plans/2026-05-21-dashboard-oauth-auth.md``:
...
import plugins.dashboard_auth.sophos as sophos_plugin
...
def _patched_jwks(provider: sophos_plugin.SophosDashboardAuthProvider, rsa_keypair):
...
class TestConstruction:
    def test_protocol_compliance(self):
        assert_protocol_compliance(sophos_plugin.SophosDashboardAuthProvider)

    def test_name_and_display(self):
        p = sophos_plugin.SophosDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )
        assert p.name == "sophos"
        assert p.display_name == "Sophos Techne"
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            sophos_plugin.SophosDashboardAuthProvider(
                client_id="slick-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    def test_skips_when_client_id_missing(self, monkeypatch):
        monkeypatch.delenv("SLICK_DASHBOARD_OAUTH_CLIENT_ID", raising=False)
        monkeypatch.delenv("SLICK_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        sophos_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason is surfaced for the gate's fail-closed message.
        assert "SLICK_DASHBOARD_OAUTH_CLIENT_ID" in sophos_plugin.LAST_SKIP_REASON

    def test_registers_with_default_portal_url_when_only_client_id_set(
        self, monkeypatch
    ):
        """Phase 7 follow-up: SLICK_DASHBOARD_PORTAL_URL is optional —
        defaults to the production Sophos Portal. The user shouldn't have
        to set it for the common production deployment path."""
        monkeypatch.setenv("SLICK_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.delenv("SLICK_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        sophos_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert isinstance(registered, sophos_plugin.SophosDashboardAuthProvider)
        assert registered._portal_url == "https://portal.sophostechne.com"
        # Skip reason cleared on successful registration.
        assert sophos_plugin.LAST_SKIP_REASON == ""
    ...
    def test_empty_portal_url_env_uses_default(self, monkeypatch):
        ...
        monkeypatch.setenv("SLICK_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.setenv("SLICK_DASHBOARD_PORTAL_URL", "")
        ctx = MagicMock()
        sophos_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert registered._portal_url == "https://portal.sophostechne.com"
...
class TestConfigYamlSource:
    ...
    canonical surface for these settings. ``SLICK_DASHBOARD_OAUTH_CLIENT_ID``
    and ``SLICK_DASHBOARD_PORTAL_URL`` are operator overrides that win when
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``slick_cli.config.load_config``
        ...
        def _set(oauth_block: Dict[str, Any] | None) -> None:
            ...
            monkeypatch.setattr(
                "slick_cli.config.load_config", lambda: cfg
            )
    ...
    def test_env_overrides_config_client_id(self, patch_config, monkeypatch):
        ...
        SLICK_DASHBOARD_OAUTH_CLIENT_ID at deploy time and we MUST
        honour it even if a stale config.yaml ships in the image."""
        monkeypatch.setenv("SLICK_DASHBOARD_OAUTH_CLIENT_ID", "agent:from-env")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        sophos_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_neither_source_skips_with_helpful_reason(
        ...
        assert "dashboard.oauth.client_id" in sophos_plugin.LAST_SKIP_REASON, (
            f"skip reason omits the config.yaml surface — operators "
            f"won't know it exists. got: {sophos_plugin.LAST_SKIP_REASON!r}"
        )
...
class TestStartLogin:
    @pytest.fixture
    def provider(self):
        return sophos_plugin.SophosDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )

    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://slick.fly.dev/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://slick.fly.dev/auth/callback"
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "slick_session_pkce" in result.cookie_payload
        pkce = result.cookie_payload["slick_session_pkce"]
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["slick_session_pkce"] != b.cookie_payload[
            "slick_session_pkce"
        ]
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http://slick.fly.dev/auth/callback")
...
class TestCompleteLogin:
    ...
    def test_happy_path_returns_session(self, provider, rsa_keypair):
        ...
        with patch("plugins.dashboard_auth.sophos.httpx.post", return_value=mock_resp):
        ...
        assert session.provider == "sophos"
    ...
    def test_network_error_raises_provider_error(self, provider):
        with patch(
            "plugins.dashboard_auth.sophos.httpx.post",
...
class TestVerifySession:
    ...
    def test_jwks_client_sends_explicit_http_headers(self, provider):
        ...
        client_cls.assert_called_once_with(
            provider._jwks_url,
            cache_keys=True,
            lifespan=sophos_plugin._JWKS_CACHE_SECONDS,
            headers={
    ...
    def test_verification_failure_message_surfaces_token_claims(
        ...
        config drift between SLICK_DASHBOARD_PORTAL_URL/CLIENT_ID and Portal."""
    ...
    def test_contract_version_missing_warns_but_succeeds(
        ...
        with caplog.at_level(logging.WARNING, logger="plugins.dashboard_auth.sophos"):
...
class TestRefreshAndRevoke:
    ...
    def test_refresh_happy_path_returns_rotated_session(self, provider, rsa_keypair):
        ...
        with patch(
            "plugins.dashboard_auth.sophos.httpx.post", return_value=mock_resp
        ) as mock_post:
        ...
        assert kwargs["headers"]["x-sophos-refresh-token"] == "rt_old_value"
