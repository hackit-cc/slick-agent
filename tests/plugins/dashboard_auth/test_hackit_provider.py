"""Tests for the bundled Hackit dashboard-auth plugin.
...
import plugins.dashboard_auth.hackit as hackit_plugin
...
def _patched_jwks(provider: hackit_plugin.HackitDashboardAuthProvider, rsa_keypair):
...
class TestConstruction:
    def test_protocol_compliance(self):
        assert_protocol_compliance(hackit_plugin.HackitDashboardAuthProvider)

    def test_name_and_display(self):
        p = hackit_plugin.HackitDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )
        assert p.name == "hackit"
        assert p.display_name == "Hackit"
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            hackit_plugin.HackitDashboardAuthProvider(
...
class TestPluginRegister:
    def test_skips_when_client_id_missing(self, monkeypatch):
        ...
        ctx = MagicMock()
        hackit_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason is surfaced for the gate's fail-closed message.
        assert "SLICK_DASHBOARD_OAUTH_CLIENT_ID" in hackit_plugin.LAST_SKIP_REASON
    ...
    def test_registers_with_default_portal_url_when_only_client_id_set(
        ...
        defaults to the production Hackit Portal. The user shouldn't have
        ...
        assert isinstance(registered, hackit_plugin.HackitDashboardAuthProvider)
        assert registered._portal_url == "https://portal.hackit.cc"
        # Skip reason cleared on successful registration.
        assert hackit_plugin.LAST_SKIP_REASON == ""
...
class TestConfigYamlSource:
    ...
    def test_neither_source_skips_with_helpful_reason(
        ...
        assert "dashboard.oauth.client_id" in hackit_plugin.LAST_SKIP_REASON, (
            f"skip reason omits the config.yaml surface — operators "
            f"won't know it exists. got: {hackit_plugin.LAST_SKIP_REASON!r}"
        )
...
class TestStartLogin:
    @pytest.fixture
    def provider(self):
        return hackit_plugin.HackitDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )
...
class TestCompleteLogin:
    ...
    def test_happy_path_returns_session(self, provider, rsa_keypair):
        ...
        with patch("plugins.dashboard_auth.hackit.httpx.post", return_value=mock_resp):
        ...
        assert session.provider == "hackit"
    ...
    def test_network_error_raises_provider_error(self, provider):
        with patch(
            "plugins.dashboard_auth.hackit.httpx.post",
...
class TestVerifySession:
    ...
    def test_jwks_client_sends_explicit_http_headers(self, provider):
        ...
        client_cls.assert_called_once_with(
            provider._jwks_url,
            cache_keys=True,
            lifespan=hackit_plugin._JWKS_CACHE_SECONDS,
            headers={
    ...
    def test_contract_version_missing_warns_but_succeeds(
        ...
        with caplog.at_level(logging.WARNING, logger="plugins.dashboard_auth.hackit"):
...
class TestRefreshAndRevoke:
    ...
    def test_refresh_happy_path_returns_rotated_session(self, provider, rsa_keypair):
        ...
        with patch(
            "plugins.dashboard_auth.hackit.httpx.post", return_value=mock_resp
        ) as mock_post:
        ...
        assert kwargs["headers"]["x-hackit-refresh-token"] == "rt_old_value"
