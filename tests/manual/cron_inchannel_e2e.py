...
Uses a throwaway SLICK_HOME so it never touches ~/.slick. No real names.
"""
...
def _fresh_home():
    """Point SLICK_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_slick_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["SLICK_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER SLICK_HOME is set.
...
# a different get_slick_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"SLICK_HOME (throwaway): {HOME}")
