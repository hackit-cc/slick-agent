"""Tests for ``slick doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from slick_cli import doctor_live
from slick_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `slick doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("slick_cli.doctor.SLICK_HOME", tmp_path / "home")
        monkeypatch.setattr("slick_cli.doctor.PROJECT_ROOT", tmp_path / "root")
    ...
    def test_false_on_termux_local_bare_npx(self, monkeypatch, tmp_path):
        ...
        advertise as ready — must not diverge from dep_ensure/hackit_subscription's
        same carve-out."""
        self._block_path_and_node_modules_checks(monkeypatch, tmp_path)
        import tools.browser_tool as bt
