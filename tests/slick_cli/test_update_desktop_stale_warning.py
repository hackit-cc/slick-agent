...
#88251: ``slick update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "slick_constants.with_slick_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "slick_constants.display_slick_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_slick_home", lambda: tmp_path)
