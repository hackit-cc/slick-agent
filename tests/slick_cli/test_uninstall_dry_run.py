...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_slick_home", lambda: slick_home)
    monkeypatch.setattr(uninstall, "_is_default_slick_home", lambda home: False)
    ...
    assert str(slick_home) in output
    assert project_root.exists()
    assert slick_home.exists()
