"""Tests for Codex auth — tokens stored in Slick auth store (~/.slick/auth.json)."""
...
def _setup_slick_auth(slick_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Slick auth store."""
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_slick_auth(slick_home, access_token="")
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (slick_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("SLICK_HOME", str(slick_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((slick_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``slick auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``slick auth add openai-codex``.
    ...
    (slick_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `slick auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``slick auth add openai-codex`` for accounts B and C
    ...
    (slick_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``slick auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Slick auth store, not ~/.codex/."""
    slick_home = tmp_path / "slick"
    ...
    _save_codex_tokens({"access_token": "slick-at", "refresh_token": "slick-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Slick store
    assert not (codex_home / "auth.json").exists()

    # Slick auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "slick-at"


def test_resolve_returns_slick_auth_store_source(tmp_path, monkeypatch):
    slick_home = tmp_path / "slick"
    _setup_slick_auth(slick_home)
    monkeypatch.setenv("SLICK_HOME", str(slick_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "slick-auth-store"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run slick auth".
    """
    from slick_cli.auth import (
    ...
    assert "slick auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from slick_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from slick_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch slick_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("slick_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
