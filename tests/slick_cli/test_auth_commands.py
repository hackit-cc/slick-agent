...
def test_auth_add_sophos_oauth_persists_pool_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("SLICK_HOME", str(tmp_path / "slick"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("sophos@example.com")
    monkeypatch.setattr(
        "slick_cli.auth._sophos_device_code_login",
        lambda **kwargs: {
    ...
    entries = payload["credential_pool"]["sophos"]
    device_code_entries = [
    ...
    # `slick auth add sophos` must also populate providers.sophos so the
    # 401-recovery path (resolve_sophos_runtime_credentials) can refresh an
    # invoke JWT when the token expires. If this mirror is missing, recovery
    # raises "Slick is not logged into Sophos Portal" and the agent dies.
...
def test_auth_add_sophos_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`slick auth add sophos --type oauth --label <name>` must preserve the
    custom label end-to-end — it was silently dropped in the first cut of the
    persist_sophos_credentials helper because `--label` wasn't threaded through.
    """
    monkeypatch.setenv("SLICK_HOME", str(tmp_path / "slick"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("sophos@example.com")
    ...
    pool_entry = payload["credential_pool"]["sophos"][0]
    assert pool_entry["source"] == "device_code"
    assert pool_entry["label"] == "my-sophos"

    # … and survives in providers.sophos so a subsequent load_pool() re-seeds
    # it without reverting to the auto-derived fingerprint.
    assert payload["providers"]["sophos"]["label"] == "my-sophos"
