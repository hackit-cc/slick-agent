"""Tests for the `slick proxy` subcommand and its upstream adapters."""
...
from slick_cli.proxy.adapters import ADAPTERS, get_adapter
from slick_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from slick_cli.proxy.adapters.sophos_portal import SophosPortalAdapter
from slick_cli.proxy.adapters.xai import XAIGrokAdapter
...
def _write_auth_store(slick_home: Path, sophos_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given sophos state into a hermetic SLICK_HOME."""
    auth_path = slick_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {"sophos": sophos_state},
    }))
    return auth_path
...
def test_sophos_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    with patch(
        "slick_cli.proxy.adapters.sophos_portal.resolve_sophos_runtime_credentials",
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic SLICK_HOME."""
    auth_path = slick_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("slick_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from slick_cli.proxy.server import create_app  # noqa: E402
