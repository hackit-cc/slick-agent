...
from slick_cli.proxy.adapters.hackit_portal import HackitPortalAdapter
...
def _write_auth_store(slick_home: Path, hackit_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given hackit state into a hermetic SLICK_HOME."""
    auth_path = slick_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {"hackit": hackit_state},
    }))
    return auth_path
...
def test_hackit_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    with patch(
        "slick_cli.proxy.adapters.hackit_portal.resolve_hackit_runtime_credentials",
