...
class TestProfileFlagGatewayLifecycle:
    ...
    def _pin_profile_identity(self, monkeypatch):
        ...
        monkeypatch.setenv("SLICK_PROFILE", "zeus")
        monkeypatch.delenv("SLICK_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "slick -p zeus gateway stop",
        "slick -p zeus gateway restart",
        "slick --profile zeus gateway restart",
        "slick --profile zeus gateway stop",
        "slick --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "slick -v -p zeus gateway restart",
        "slick -p zeus -v gateway restart",
        "slick --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "slick -p zeus gateway start",
        "slick -p zeus gateway start --all",
    ])
    ...
    def test_slick_home_derived_profile(self, monkeypatch):
        ...
        assert _contains_gateway_lifecycle_command("slick -p zeus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("slick -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("slick gateway restart")
