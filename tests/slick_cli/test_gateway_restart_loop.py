...
Covers:
- Defense 1: gateway stop/restart refuse when _SLICK_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "slick gateway stop",
        "slick gateway uninstall",
        "slick  gateway  restart",         # double spaces
        "Slick Gateway Restart",           # mixed case handled
        "SLICK GATEWAY RESTART",           # uppercase
    ])
    def test_slick_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.slick.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.slick/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l slick-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.slick.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.slick.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.slick.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.slick/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.slick.gateway-apollo:/a.plist' "
            "'ai.slick.gateway-cronus:/c.plist' 'ai.slick.gateway-plutus:/p.plist' "
            "'ai.slick.gateway:/Users/botuser/Library/LaunchAgents/ai.slick.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated slick note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.slick.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.slick.gateway',
        "launchctl kick'start' -k gui/501/ai.slick.gateway",
        "launchctl kick\\start -k gui/501/ai.slick.gateway",
        'launchctl "kickstart" -k gui/501/ai.slick.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.slick.gateway',
        "launchctl dis\\able gui/501/ai.slick.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.slick."gateway"',
        # Same class on the systemctl and slick-CLI branches.
        'systemctl re"start" slick-gateway',
        'slick gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.slick.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.slick.update-checker',
        'systemctl re"start" slick-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "slick cron list",
        "slick update",
        "slick config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `slick gateway start` is benign — starting a gateway from inside a
        ...
        "slick gateway start",
        "slick gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway slick
        # services must not be falsely blocked (the old `.*slick` matched any
        # slick token).
        "launchctl unload ai.slick.update-checker.plist",
        "launchctl restart ai.slick.daemon",
        ...
        "systemctl restart slick-meta.service",
        "systemctl restart slick-cron-helper",
        ...
        "echo after the slick gateway restarted cleanly",
        "the slick gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "slick skill view gateway-notes && echo slick gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `slick` is a path component
        # there, not a command.
        "cat '/docs/slick gateway restart-notes.md'",
        "less /home/user/notes/slick gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "slick gateway restart",
        "slick gateway restart; echo done",
        "slick gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;slick gateway restart",
        "true && slick gateway stop",
        "echo $(slick gateway restart)",
        "echo `slick gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.slick.gateway"])',
        'subprocess.run(["slick", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.slick.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["slick", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: slick gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `slick -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry SLICK_HOME/SLICK_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("SLICK_PROFILE", "testbed")
        monkeypatch.delenv("SLICK_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "slick -p testbed gateway stop",
        "slick -p testbed gateway restart",
        "slick --profile testbed gateway restart",
        "slick --profile testbed gateway stop",
        "slick --profile=testbed gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "slick -v -p testbed gateway restart",
        "slick -p testbed -v gateway restart",
        "slick --debug --profile testbed gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "slick -p venus gateway stop",
        "slick -p venus gateway restart",
        "slick --profile venus gateway restart",
        "slick --profile=venus gateway stop",
        "slick -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "slick -p testbed gateway start",
        "slick -p testbed gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("slick gateway restart")
        assert _contains_gateway_lifecycle_command("slick gateway stop")

    def test_slick_home_derived_profile(self, monkeypatch):
        # Without SLICK_PROFILE the guard falls back to the SLICK_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("SLICK_PROFILE", raising=False)
        monkeypatch.delenv("SLICK_PROFILE_NAME", raising=False)
        import slick_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "testbed")
        assert _contains_gateway_lifecycle_command("slick -p testbed gateway restart")
        assert not _contains_gateway_lifecycle_command("slick -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("slick -p testbed gateway restart")
        assert _contains_gateway_lifecycle_command("slick gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_slick_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade slick then run slick gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_slick_gateway_once.sh). The script must live under
        # SLICK_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("SLICK_HOME", str(tmp_path / ".slick"))
        scripts_dir = tmp_path / ".slick" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_SLICK_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _SLICK_GATEWAY=1.

    Issue #37453: systemctl --user restart slick-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart slick-gateway",
        "systemctl --user restart slick-gateway",
        "systemctl stop slick-gateway.service",
        "slick gateway restart",
        "slick gateway uninstall",
        "launchctl kickstart gui/501/ai.slick.gateway",
        "launchctl bootout gui/501/ai.slick.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.slick.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.slick/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.slick.gateway.restart-once.plist",
        "pkill -f slick.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart slick-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.slick.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-slick label: label-independent detection is the point
        # (#62891 second reproduction used `ai.slick.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.slick.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _SLICK_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _SLICK_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_SLICK_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="slick gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["slick gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.slick.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.slick.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-slick systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run slick gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("slick gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeslick gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under SLICK_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("slick gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/slick\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 slick gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_slick_home()` falls back to Path.home(), which raises
        RuntimeError when neither SLICK_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart slick-gateway' .",
        "grep -rn 'slick gateway restart' ./logs",
        "rg 'slick gateway restart' ../archive",
        "grep -c 'systemctl stop slick-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM slick_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell slick gateway restart"',
        'sqlite3 ./db ".system systemctl restart slick-gateway"',
        'psql ./x -c "\\! systemctl restart slick-gateway"',
        "grep -r 'slick gateway restart' . | sh",
        "grep -r 'slick gateway restart' ./logs | bash",
        "grep -r 'slick gateway restart' . | sudo sh",
        "grep -r 'x' . ; slick gateway restart",
        "grep -r 'x' . && systemctl restart slick-gateway",
        'grep -r "$(slick gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart slick-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell slick gateway restart"',
        'sqlite3 db ".system systemctl restart slick-gateway"',
        'psql -c "\\! systemctl restart slick-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    slick_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run slick gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SLICK_HOME", str(tmp_path / ".slick"))
        (tmp_path / ".slick").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run slick gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop slick-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart slick-gateway' /var/log/syslog",
        "rg 'slick gateway restart' /home/user/.slick/logs/",
        "journalctl -u slick-gateway --grep 'systemctl restart slick-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'slick_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell slick gateway restart"',
        'psql -c "\\! systemctl restart slick-gateway"',
        "grep 'systemctl restart slick-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart slick-gateway",
        'grep "$(systemctl restart slick-gateway)" f',
        "grep 'restart' log; systemctl restart slick-gateway",
        'sqlite3 db "SELECT 1"; slick gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "slick gateway restart",
        "sudo systemctl stop slick-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart slick-gateway%'\""
        )
