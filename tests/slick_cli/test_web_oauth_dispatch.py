...
def _fake_hackit_device_data():
    return {
        "device_code": "device-code",
        "user_code": "HACKIT-1234",
        "verification_uri": "https://portal.hackit.cc/device",
        "verification_uri_complete": (
            "https://portal.hackit.cc/device?user_code=HACKIT-1234"
        ),
        "expires_in": 600,
        "interval": 5,
    }
...
def _invoke_scope_refusal():
    request = httpx.Request("POST", "https://portal.hackit.cc/oauth/device/code")
...
def test_hackit_dashboard_poller_preserves_effective_scope_when_token_omits_scope(monkeypatch):
    from slick_cli import auth as auth_mod
    from slick_cli import web_server as ws

    session_id = "hackit-effective-scope-test"
    ws._oauth_sessions[session_id] = {
        "session_id": session_id,
        "provider": "hackit",
        "flow": "device_code",
    ...
    def fake_refresh_hackit_oauth_from_state(state, **kwargs):
    ...
    monkeypatch.setattr(
        ...
        fake_refresh_hackit_oauth_from_state,
    )
    monkeypatch.setattr(auth_mod, "persist_hackit_credentials", lambda state: None)

    try:
        ws._hackit_poller(session_id)
        assert captured_state["scope"] == auth_mod.DEFAULT_HACKIT_SCOPE
