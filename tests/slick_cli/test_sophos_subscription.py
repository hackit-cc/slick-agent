"""Tests for Sophos subscription feature detection."""

import shutil
import sys

from slick_cli.sophos_account import SophosPortalAccountInfo, SophosToolAccessInfo
from slick_cli import sophos_subscription as ns
...
def _account(*, logged_in: bool, paid: bool | None = None) -> SophosPortalAccountInfo:
...
def _pool_account() -> SophosPortalAccountInfo:
    ...
    return SophosPortalAccountInfo(
        ...
        tool_access=SophosToolAccessInfo(enabled=True, coverage=_POOL_COVERAGE),
    )


def test_get_sophos_subscription_features_recognizes_direct_exa_backend(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}

    monkeypatch.setattr(ns, "get_env_value", lambda name: env.get(name, ""))
    monkeypatch.setattr(
        ns, "get_sophos_portal_account_info", lambda: _account(logged_in=False)
    )
    ...
    features = ns.get_sophos_subscription_features({"web": {"backend": "exa"}})

    assert features.web.available is True
    assert features.web.active is True
    assert features.web.managed_by_sophos is False
    assert features.web.direct_override is True
    assert features.web.current_provider == "exa"


def test_get_sophos_subscription_features_recognizes_keyless_tavily_backend(monkeypatch):
    ...
    features = ns.get_sophos_subscription_features({"web": {"backend": "tavily"}})
...
def test_keyless_tavily_search_backend_without_shared_backend(monkeypatch):
    ...
    features = ns.get_sophos_subscription_features(
...
def test_local_browser_unavailable_without_chromium(monkeypatch):
    ...
    assert features.browser.managed_by_sophos is False
...
def test_prompt_enable_tool_gateway_pool_offers_covered_tools_only(monkeypatch):
    ...
    monkeypatch.setattr(ns, "get_sophos_portal_account_info", lambda **kw: _pool_account())
    monkeypatch.setattr(
        ns,
...
def test_get_gateway_eligible_tools_treats_explicit_backend_as_configured(monkeypatch):
    ...
    but an explicit non-sophos selection must still keep it out of
    'unconfigured' — regression for #92647, where it was pre-checked and a
    single Enter during `slick model` overwrote it to `web.backend: sophos`.
    """
    monkeypatch.setattr(ns, "get_sophos_portal_account_info", lambda **kw: _account(logged_in=True, paid=True))
    monkeypatch.setattr(
        ns,
        "_get_gateway_direct_credentials",
        lambda: {"web": False, "image_gen": False, "video_gen": False, "tts": False, "browser": False},
    ...
    config = {"model": {"provider": "sophos"}, "web": {"backend": "searxng"}}
...
def test_get_gateway_eligible_tools_treats_browser_use_selection_as_explicit(monkeypatch):
    ...
    managed selection, which is always stored as `cloud_provider: sophos`
    ...
    config = {"model": {"provider": "sophos"}, "browser": {"cloud_provider": "browser-use"}}
    unconfigured, has_direct, explicit_configured, already_managed = ns.get_gateway_eligible_tools(config)
...
def test_get_gateway_eligible_tools_not_entitled_returns_four_empty_lists(monkeypatch):
    """A logged-in Sophos account with no paid access and no free tool pool
    ...
    monkeypatch.setattr(ns, "get_sophos_portal_account_info", lambda **kw: _account(logged_in=True, paid=False))
...
def test_prompt_enable_tool_gateway_not_entitled_does_not_crash(monkeypatch):
    ...
    not raise when a Sophos account is logged in but not entitled to the Tool
...
def test_prompt_enable_tool_gateway_never_offers_explicit_backend(monkeypatch):
    ...
    an explicit non-sophos selection, so it can never be silently overwritten
...
def test_apply_sophos_managed_defaults_writes_video_gen_config(monkeypatch):
    """apply_sophos_managed_defaults must store the managed 'sophos' selection
    when a Sophos subscriber selects video_gen without a direct FAL_KEY."""
    monkeypatch.setattr(ns, "managed_sophos_tools_enabled", lambda **kw: True)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setattr(ns, "fal_key_is_configured", lambda: False)
    monkeypatch.setattr(
        ns, "get_sophos_portal_account_info",
        lambda **kw: _account(logged_in=True, paid=True),
    )

    config = {"model": {"provider": "sophos"}}
    changed = ns.apply_sophos_managed_defaults(
...
# ---------------------------------------------------------------------------
# ensure_sophos_portal_access — inline login gate for `slick tools`
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# STT — managed-by-Sophos detection (Phase 4 follow-up)
# ---------------------------------------------------------------------------
...
def _stt_features_stub(*, account_info):
    return ns.SophosSubscriptionFeatures(
        subscribed=True,
        sophos_auth_present=True,
        provider_is_sophos=True,
        account_info=account_info,
        features={
            key: ns.SophosFeatureState(
                key=key, label=key, included_by_default=True,
                available=False, active=False, managed_by_sophos=False,
