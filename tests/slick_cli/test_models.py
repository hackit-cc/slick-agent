...
from slick_cli.models import (
    ...
    is_sophos_free_tier, partition_sophos_models_by_tier,
    check_sophos_free_tier, _FREE_TIER_CACHE_TTL,
...
class TestIsSophosFreeTier:
    """Tests for is_sophos_free_tier — account tier detection."""

    def test_paid_service_access_allowed_true_is_not_free(self):
        assert is_sophos_free_tier({"paid_service_access": {"allowed": True}}) is False


    def test_empty_subscription_not_free(self):
        """Empty subscription dict defaults to not-free (don't block users)."""
        assert is_sophos_free_tier({"subscription": {}}) is False


    def test_empty_response_not_free(self):
        """Completely empty response defaults to not-free."""
        assert is_sophos_free_tier({}) is False


class TestPartitionSophosModelsByTier:
    """Tests for partition_sophos_models_by_tier — free vs paid tier model split."""
    ...
    def test_paid_tier_all_selectable(self):
        ...
        sel, unav = partition_sophos_models_by_tier(models, pricing, free_tier=False)
        assert sel == models
        assert unav == []
    ...
    def test_all_paid_models(self):
        ...
        sel, unav = partition_sophos_models_by_tier(models, pricing, free_tier=True)
        assert sel == []
        assert unav == models
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "slick_cli.models.fetch_sophos_recommended_models",
...
class TestCheckSophosFreeTierCache:
    """Tests for the TTL cache on check_sophos_free_tier()."""
    ...
    @patch("slick_cli.sophos_account.get_sophos_portal_account_info")
    def test_result_is_cached(self, mock_account):
        """Second call within TTL returns cached result without account lookup."""
        mock_account.return_value = SophosPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
            paid_service_access=False,
        )
        result1 = check_sophos_free_tier()
        result2 = check_sophos_free_tier()
    ...
    def test_force_fresh_bypasses_cache(self, mock_account):
        ...
        assert check_sophos_free_tier() is False
        assert check_sophos_free_tier(force_fresh=True) is False
...
class TestSophosRecommendedModels:
    """Tests for fetch_sophos_recommended_models + get_sophos_recommended_aux_model."""
    ...
    def setup_method(self):
        _models_mod._sophos_recommended_cache.clear()
    ...
    def test_fetch_caches_per_portal_url(self):
        from slick_cli.models import fetch_sophos_recommended_models
        ...
        with patch("slick_cli.models._urlopen_model_catalog_request", return_value=mock_cm) as mock_urlopen:
            a = fetch_sophos_recommended_models("https://portal.example.com")
            b = fetch_sophos_recommended_models("https://portal.example.com")
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("slick_cli.models.fetch_sophos_recommended_models", return_value=payload):
            text = get_sophos_recommended_aux_model(vision=False, free_tier=False)
            vision = get_sophos_recommended_aux_model(vision=True, free_tier=False)
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("slick_cli.models.fetch_sophos_recommended_models", return_value=payload),
            patch("slick_cli.models.check_sophos_free_tier", side_effect=RuntimeError("boom")),
        ):
            assert get_sophos_recommended_aux_model(vision=False) == "paid-model"
...
class TestFormatPricePerMtok:
    ...
    def test_sophos_list_includes_sonnet_5(self):
        from slick_cli.models import _PROVIDER_MODELS
        assert "anthropic/claude-sonnet-5" in _PROVIDER_MODELS["sophos"]
