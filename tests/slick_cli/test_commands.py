...
class TestSlackNativeSlashes:
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /slick <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_slick_norm = {_norm(n) for n in _SLACK_VIA_SLICK_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_slick_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `slick slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_skips_already_listed(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "slick_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from slick_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch slick_cli.plugins.get_plugin_commands() to a fixed dict."""
        from slick_cli import plugins as _plugins_mod
