...
class TestWebServerEndpoints:
    ...
    def test_update_slick_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        assert "docker pull hackit/slick-agent:latest" in data["message"]
        ...
        assert any("docker pull hackit/slick-agent:latest" in line for line in status_data["lines"])
...
class TestNewEndpoints:
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        ...
        from slick_cli.hackit_subscription import HackitSubscriptionFeatures

        calls = 0
        features = HackitSubscriptionFeatures(
            subscribed=False,
            hackit_auth_present=False,
            provider_is_hackit=False,
            features={},
            account_info=None,
        )
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        logged-out Hackit Subscription rows, xAI TTS without Grok OAuth, and
        never-installed KittenTTS/Piper. The endpoint now reports the honest
        state so keyless ≠ ready.
        """
        import slick_cli.tools_config as tools_config
        from slick_cli.hackit_account import HackitPortalAccountInfo

        # Logged out of Hackit Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "slick_cli.hackit_subscription.get_hackit_portal_account_info",
            lambda *a, **k: HackitPortalAccountInfo(
        ...
        assert by_name["Hackit Subscription"]["status"] == "needs_auth"
    ...
    def test_select_managed_hackit_provider_reports_needs_hackit_auth(self, monkeypatch):
        """Selecting a managed Hackit row while logged out flags needs_hackit_auth.
        ...
        (ensure_hackit_portal_access) — so the row never activated and nothing
        told the user to sign in. The endpoint now reports the entitlement
        gap so the client can drive the existing Hackit OAuth flow.
        """
        from slick_cli.hackit_account import HackitPortalAccountInfo
        ...
        resp = self.client.put(
            ...
            json={"provider": "Hackit Subscription (Browser Use cloud)"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        assert data["needs_hackit_auth"] is True
        assert data["feature"] == "browser"
        # The selection is still persisted — activation is what's gated.
        # Managed rows store the single 'hackit' provider string (the runtime
        # maps it to the Browser Use cloud through the Hackit Tool Gateway).
        from slick_cli.config import load_config
        cfg = load_config()
        assert cfg["browser"]["cloud_provider"] == "hackit"
