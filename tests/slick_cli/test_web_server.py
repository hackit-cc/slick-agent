"""Tests for slick_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_slick_home):
    """Drop the example-dashboard fixture into the per-test SLICK_HOME
    ...
    ``SLICK_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, slick-achievements, model providers
    ...
    from slick_cli import web_server

    user_plugins_dir = get_slick_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `slick plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down SLICK_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Slick vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints SLICK_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import slick_cli.web_server as ws

        original_app = ws.app
        original_token = ws._SESSION_TOKEN
        monkeypatch.setenv("SLICK_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("SLICK_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_slick_home):
        """Create a TestClient and isolate the state DB under the test SLICK_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        import slick_state
        from slick_constants import get_slick_home
        from slick_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(slick_state, "DEFAULT_DB_PATH", get_slick_home() / "state.db")
    ...
    def test_get_status_loads_gateway_config_off_event_loop(self, monkeypatch):
        ...
        import slick_cli.web_server as web_server
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `slick update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `slick update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(slick_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = slick_state.SessionDB.__init__
        ...
        # the class on slick_state covers every open the helper makes.
        monkeypatch.setattr(slick_state.SessionDB, "__init__", counting_init)
    ...
    def test_generic_corruption_does_not_trigger_writable_heal(
        ...
        monkeypatch.setattr(slick_state, "SessionDB", corrupt_open)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        SLICK_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from slick_constants import get_slick_home
        from slick_cli.config import load_env
        ...
        config_path = get_slick_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from slick_constants import get_slick_home
        from slick_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide SLICK_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from slick_constants import get_slick_home

        path = get_slick_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_slick_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["slick"]["workspace"] == "myws"
        assert cfg["hosts"]["slick"]["peerName"] == "eri"
        assert cfg["hosts"]["slick"]["environment"] == "local"
        assert cfg["hosts"]["slick"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["slick"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from slick_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_slick_returns_docker_guidance_without_spawning(self, monkeypatch):
        import slick_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn slick update")
        ...
        # method through slick_cli.config directly.
        monkeypatch.setattr(
            "slick_cli.config.detect_install_method", lambda *_a, **_k: "docker"
        )
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_slick_action", fail_spawn)
        web_server._ACTION_PROCS.pop("slick-update", None)
        web_server._ACTION_RESULTS.pop("slick-update", None)

        resp = self.client.post("/api/slick/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "slick-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull sophostechne/slick-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/slick-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull sophostechne/slick-agent:latest" in line for line in status_data["lines"])

    def test_update_slick_returns_apt_guidance_without_spawning(self, monkeypatch):
        import slick_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn slick update")

        monkeypatch.setattr(web_server, "_dashboard_local_update_managed_externally", lambda: False)
        # The shared admission gate (#91277 Phase 3) resolves the install
        # method through slick_cli.config directly, so patch it there (the
        # web_server module alias only feeds the /update/check endpoint).
        monkeypatch.setattr(
            "slick_cli.config.detect_install_method", lambda *_a, **_k: "apt"
        )
        ...
        assert data["update_command"] == "pkg upgrade slick-agent"
        assert spawned is False

        check = self.client.get("/api/slick/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade slick-agent"
    ...
    def test_update_status_recovers_completed_result_after_dashboard_restart(self, monkeypatch, tmp_path):
        ...
        action_id = "c" * 32
        (tmp_path / "slick-update.log").write_text(
            "=== slick-update started 2026-08-17 11:19:34 ===\n"
            "pulling updates...\n",
            encoding="utf-8",
        )
        (tmp_path / "update.log").write_text(
            "=== slick update started 2026-08-17T11:19:35 ===\n"
            "✓ Update complete!\n"
            f"=== slick-update completed {action_id} ===\n",
            encoding="utf-8",
        )
        ...
        status = self.client.get("/api/actions/slick-update/status?lines=2000")
        ...
        assert f"=== slick-update completed {action_id} ===" in data["lines"]

    def test_update_slick_spawns_with_action_id(self, monkeypatch):
        import slick_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_slick_action", fake_spawn)
        web_server._ACTION_PROCS.pop("slick-update", None)
        web_server._ACTION_RESULTS.pop("slick-update", None)
        ...
        assert calls == [
            (["update"], "slick-update", {"SLICK_ACTION_ID": "a" * 32})
        ]

    def test_update_slick_reuses_running_action(self, monkeypatch):
        import slick_cli.web_server as web_server
        ...
        web_server._ACTION_PROCS["slick-update"] = Proc()
        web_server._ACTION_IDS["slick-update"] = "b" * 32

        try:
            resp = self.client.post("/api/slick/update")
        finally:
            web_server._ACTION_PROCS.pop("slick-update", None)
            web_server._ACTION_IDS.pop("slick-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/slick_cli/test_setup_hidden_env.py.
        """
        from slick_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Slick provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from slick_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from slick_cli.config import save_env_value
        from slick_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "slick_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/SlickSetupBot/slick_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/SlickSetupBot/slick_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "slick_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_slick_action", fail_spawn_action)
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from slick_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from slick_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``slick model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from slick_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_numeric_yaml_provider_key_can_be_activated_and_deleted(self):
        ...
        """
        from slick_cli.config import get_config_path, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from slick_cli import profiles as profiles_mod
        from slick_cli.config import custom_endpoint_key_env
        from slick_constants import get_slick_home

        default_home = get_slick_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from slick_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from slick_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from slick_cli.web_server import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from slick_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from slick_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_slick_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from slick_constants import get_slick_home
        import slick_cli.profiles as profiles_mod
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        import slick_cli.tools_config as tools_config
        from slick_cli.sophos_subscription import SophosSubscriptionFeatures

        calls = 0
        features = SophosSubscriptionFeatures(
            subscribed=False,
            sophos_auth_present=False,
            provider_is_sophos=False,
            features={},
            account_info=None,
        )
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        logged-out Sophos Subscription rows, xAI TTS without Grok OAuth, and
        never-installed KittenTTS/Piper. The endpoint now reports the honest
        state so keyless ≠ ready.
        """
        import slick_cli.tools_config as tools_config
        from slick_cli.sophos_account import SophosPortalAccountInfo

        # Logged out of Sophos Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "slick_cli.sophos_subscription.get_sophos_portal_account_info",
            lambda *a, **k: SophosPortalAccountInfo(
        ...
        assert by_name["Sophos Subscription"]["status"] == "needs_auth"
    ...
    def test_select_managed_sophos_provider_reports_needs_sophos_auth(self, monkeypatch):
        """Selecting a managed Sophos row while logged out flags needs_sophos_auth.
        ...
        (ensure_sophos_portal_access) — so the row never activated and nothing
        told the user to sign in. The endpoint now reports the entitlement
        gap so the client can drive the existing Sophos OAuth flow.
        """
        from slick_cli.sophos_account import SophosPortalAccountInfo
        ...
        resp = self.client.put(
            ...
            json={"provider": "Sophos Subscription (Browser Use cloud)"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        assert data["needs_sophos_auth"] is True
        assert data["feature"] == "browser"
        # The selection is still persisted — activation is what's gated.
        # Managed rows store the single 'sophos' provider string (the runtime
        # maps it to the Browser Use cloud through the Sophos Tool Gateway).
        from slick_cli.config import load_config
        cfg = load_config()
        assert cfg["browser"]["cloud_provider"] == "sophos"
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "slick"
        save_config(config)
        ...
        assert "slick@devbox.example.com" in ssh["detail"]
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from slick_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from slick_cli.web_server import _denormalize_config_from_web
        from slick_cli.config import save_config
...
class TestStatusInstallId:
    ...
    Slick home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_slick_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """SLICK_HOME=<root> and SLICK_HOME=<root>/profiles/<name> resolve to
        the same id file — profiles share one physical install identity."""
        import slick_cli.web_server as ws

        root = tmp_path / "slick-root"
        profile_home = root / "profiles" / "research"
        profile_home.mkdir(parents=True)

        monkeypatch.setenv("SLICK_HOME", str(root))
        monkeypatch.setattr(ws, "_INSTALL_ID_CACHE", {"value": None})
        root_id = ws.get_install_id()
        assert root_id

        monkeypatch.setenv("SLICK_HOME", str(profile_home))
        monkeypatch.setattr(ws, "_INSTALL_ID_CACHE", {"value": None})
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from slick_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.slick/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_slick_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(slick_home, name="ocean"):
        themes_dir = slick_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="slick-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_slick_home):
        ...
        monkeypatch.setattr(
            slick_state, "DEFAULT_DB_PATH", get_slick_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``slick_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in slick_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_slick_home):
        ...
        # Pin the SessionDB to the isolated SLICK_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_slick_home, _install_example_plugin):
        ...
        installs it into the per-test ``SLICK_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into SLICK_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, slick-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (slick-achievements is loaded by default).
        resp = self.client.get("/api/plugins/slick-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        SLICK_HOME override scopes a request to another profile."""
        from slick_constants import (
            reset_slick_home_override,
            set_slick_home_override,
        )
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets SLICK_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the slick root's plugins/ directory."""
        root = tmp_path / "slick-root"
...
# or ``sh -c 'printf …'``) instead of the real ``slick --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"slick-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "SLICK_CWD": str(tmp_path),
            "SLICK_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["SLICK_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from slick_cli.pty_bridge import PtyUnavailableError
        ...
        import slick_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from slick_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import slick_cli.main as cli_main
    import slick_cli.web_server as ws
    ...
    gateway_url = env.get("SLICK_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_slick_home, _install_example_plugin):
        ...
        the per-test ``SLICK_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_slick_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("SLICK_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under SLICK_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("SLICK_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
...
class TestHeadlessServeTokenPage:
    """Headless `slick serve` must serve the Desktop token handshake page
    at `/` when the dashboard auth gate is off (#94227).

    The Electron renderer boots by fetching `/` and extracting
    ``window.__SLICK_SESSION_TOKEN__`` for WebSocket auth. Headless serve
    ...
    def _headless_client(monkeypatch, *, gated: bool):
        ...
        monkeypatch.setenv("SLICK_SERVE_HEADLESS", "1")
        spa_app = FastAPI()
    ...
    def test_root_serves_token_page_when_not_gated(self, monkeypatch):
        ...
        match = re.search(
            r'window\.__SLICK_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")',
            resp.text,
        )
        ...
        assert "window.__SLICK_AUTH_REQUIRED__=false" in resp.text
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/slick"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/slick/ds-assets/bg.png)" in prefixed.text
        assert "url(/slick/fonts-terminal/x.woff2)" in prefixed.text
