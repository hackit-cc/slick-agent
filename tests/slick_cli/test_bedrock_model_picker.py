...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (SLICK_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via SLICK_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in SLICK_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from slick_cli.providers import SLICK_OVERLAYS
        assert "bedrock" in SLICK_OVERLAYS
