...
The bug class: on Windows, when `slick.exe`/sibling shims could not be
...
import slick_cli._install_repair as ir
import slick_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("slick", "slick-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # slick.exe cannot be renamed; slick-gateway.exe can
    real_rename = Path.rename

    def deny_slick(self, target):
        if self.name == "slick.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_slick)
    monkeypatch.setattr(cli_main, "_slick_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "slick.exe" in exc_info.value.failed_shims
    # The successful rename (slick-gateway.exe) was rolled back.
    assert (scripts / "slick-gateway.exe").exists()
    assert not list(scripts.glob("slick-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("slick",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("slick",))
