...
The portal HTTP call and the Hackit token resolution are both mocked — this
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        err = AuthError("not logged in", provider="hackit", relogin_required=True)
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "slick_cli.auth.resolve_hackit_access_token", side_effect=err
            ), patch("slick_cli.config.is_managed", return_value=False):
                with pytest.raises(SystemExit) as exc:
                    dr.cmd_dashboard_register(_ns())
        assert exc.value.code == 1
        out = capsys.readouterr().out
        assert "not logged into Hackit Portal" in out
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.hackit.cc",
        ...
        with patch(
            "slick_cli.auth.resolve_hackit_access_token", return_value=account_token
...
class TestCustomPortalPersistence:
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        with patch(
            "slick_cli.auth.resolve_hackit_access_token", return_value="tok"
...
class TestPublicUrlPersistence:
    ...
    def _run(self, *, args, existing_public=None):
        ...
        with patch(
            ...
            dr, "_resolve_portal_base_url", return_value="https://portal.hackit.cc"
        ), patch(
            "slick_cli.config.get_env_value", side_effect=fake_get_env_value
        ), patch(
            "slick_cli.config.save_env_value", side_effect=fake_save
...
class TestPortalResolution:
    ...
    def test_falls_back_to_stored_login_portal(self):
        with patch(
            ...
            return_value={"portal_base_url": "https://portal.staging-hackit.cc"},
        ):
            assert (
                dr._resolve_portal_base_url(None)
                == "https://portal.staging-hackit.cc"
            )
...
class TestPortalErrors:
    def _run_http_error(self, code, body):
        err = urllib.error.HTTPError(
            url="https://portal.hackit.cc/api/oauth/self-hosted-client",
            code=code,
            msg="err",
            hdrs=None,
