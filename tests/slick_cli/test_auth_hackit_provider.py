"""Regression tests for Hackit OAuth refresh and inference JWT interactions."""
...
from slick_cli.auth import AuthError, get_provider_auth_state, resolve_hackit_runtime_credentials
...
def test_resolve_hackit_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    tmp_path,
    monkeypatch,
):
    ...
    _setup_hackit_auth(
        slick_home,
        access_token=token,
        scope=auth_mod.DEFAULT_HACKIT_SCOPE,
        expires_at=_future_iso(3600),
        expires_in=3600,
    )
    monkeypatch.setenv("SLICK_HOME", str(slick_home))

    creds = auth_mod.resolve_hackit_runtime_credentials()

    assert creds["api_key"] == token
    assert creds["source"] == auth_mod.HACKIT_AUTH_PATH_INVOKE_JWT
    assert creds["auth_path"] == auth_mod.HACKIT_AUTH_PATH_INVOKE_JWT

    payload = json.loads((slick_home / "auth.json").read_text())
    singleton = payload["providers"]["hackit"]
    assert singleton["agent_key"] == token
    assert datetime.fromisoformat(singleton["agent_key_expires_at"]).timestamp() > time.time() + 300

    pool_entries = payload["credential_pool"]["hackit"]
    assert len(pool_entries) == 1
    assert pool_entries[0]["agent_key"] == token
    assert pool_entries[0]["source"] == auth_mod.HACKIT_DEVICE_CODE_SOURCE


def test_resolve_hackit_runtime_credentials_invoke_jwt_is_idempotent(
    tmp_path,
    monkeypatch,
):
    import slick_cli.auth as auth_mod
    ...
    auth_store = {
        ...
        "providers": {
            "hackit": {
                "portal_base_url": "https://portal.hackit.cc",
                "inference_base_url": "https://inference-api.hackit.cc/v1",
                "client_id": "slick-cli",
                "token_type": "Bearer",
                "scope": auth_mod.DEFAULT_HACKIT_SCOPE,
                "access_token": token,
    ...
    monkeypatch.setattr(auth_mod, "_write_shared_hackit_state", _unexpected_shared_write)
    monkeypatch.setattr(
        auth_mod,
    ...
    assert (
        payload["providers"]["hackit"]["agent_key_obtained_at"]
        == original_obtained_at
    )


def test_resolve_hackit_runtime_credentials_reauths_when_invoke_scope_missing(
    tmp_path,
    monkeypatch,
):
    import slick_cli.auth as auth_mod
    ...
    with pytest.raises(AuthError) as exc:
        auth_mod.resolve_hackit_runtime_credentials()

    assert exc.value.code == "missing_inference_invoke_scope"
    assert exc.value.relogin_required is True
    payload = json.loads((slick_home / "auth.json").read_text())
    assert payload["providers"]["hackit"]["agent_key"] is None
    assert "credential_pool" not in payload or not payload["credential_pool"].get("hackit")
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    assert creds["api_key"] == token
    payload = json.loads((slick_home / "auth.json").read_text())
    assert payload["providers"]["hackit"]["agent_key"] == token
    ...
    result = auth_mod._hackit_device_code_login(
        portal_base_url="https://portal.example.com",
        inference_base_url="https://inference.example.com/v1",
        open_browser=False,
        timeout_seconds=1,
    ...
    assert requested_scopes == [auth_mod.DEFAULT_HACKIT_SCOPE]
    assert result["agent_key"] == login_token


def test_hackit_inference_auth_logs_do_not_include_secret_values(
    tmp_path,
    monkeypatch,
    caplog,
):
...
def test_get_hackit_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_hackit_auth_status() should find Hackit credentials in the pool
    even when the auth store has no Hackit provider entry — this is the
    case when login happened via the dashboard device-code flow which
    saves to the pool only.
    """
    from slick_cli.auth import get_hackit_auth_status

    slick_home = tmp_path / "slick"
    slick_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no Hackit provider entry
    (slick_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("SLICK_HOME", str(slick_home))

    # Seed the credential pool with a Hackit entry
    from agent.credential_pool import PooledCredential, load_pool
    pool = load_pool("hackit")
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    entry = PooledCredential.from_dict("hackit", {
        "access_token": token,
    ...
    status = get_hackit_auth_status()
    assert status["logged_in"] is True
    assert "example.com" in str(status.get("portal_base_url", ""))


def test_get_hackit_auth_status_empty_returns_not_logged_in(tmp_path, monkeypatch):
    """get_hackit_auth_status() returns logged_in=False when both pool
...
# =============================================================================
# _login_hackit: "Skip (keep current)" must preserve prior provider + model
# =============================================================================


class TestLoginHackitSkipKeepsCurrent:
    """When a user runs `slick model` → Hackit Portal → Skip (keep current) after
    ...
    unconditionally after login, which flipped model.provider to "hackit" while
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        """Patch OAuth + model-list + prompt so _login_hackit doesn't hit network."""
        import slick_cli.auth as auth_mod
        import slick_cli.models as models_mod
        import slick_cli.hackit_subscription as ns

        fake_auth_state = {
            "access_token": "fake-hackit-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.hackit.cc",
            "portal_base_url": "https://portal.hackit.cc",
            "refresh_token": "fake-refresh",
            "token_expires_at": 9999999999,
        }
        monkeypatch.setattr(
            auth_mod, "_hackit_device_code_login",
        ...
        def _check_hackit_free_tier(**kwargs):
            free_tier_calls.append(kwargs)
            return None

        monkeypatch.setattr(models_mod, "check_hackit_free_tier", _check_hackit_free_tier)
        monkeypatch.setattr(
            models_mod, "partition_hackit_models_by_tier",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, Hackit creds still saved."""
        import argparse
        import yaml
        from slick_cli.auth import PROVIDER_REGISTRY, _login_hackit
        ...
        _login_hackit(args, PROVIDER_REGISTRY["hackit"])
        ...
        # auth.json: active_provider restored to openrouter, but Hackit creds saved
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "openrouter"
        assert "hackit" in auth_after["providers"]
        assert auth_after["providers"]["hackit"]["access_token"] == "fake-hackit-token"
        # Existing openrouter creds still intact
        assert auth_after["providers"]["openrouter"]["api_key"] == "sk-or-fake"

    def test_picking_model_switches_to_hackit(self, tmp_path, monkeypatch):
        """User picks a Hackit model → provider flips to hackit with that model."""
        import argparse
        import yaml
        from slick_cli.auth import PROVIDER_REGISTRY, _login_hackit
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        instead of leaving it as hackit."""
        import argparse
        import yaml
        from slick_cli.auth import PROVIDER_REGISTRY, _login_hackit
        ...
        auth_path = slick_home / "auth.json"
        auth_after = json.loads(auth_path.read_text())
        # active_provider should NOT be set to "hackit" after Skip
        assert auth_after.get("active_provider") in {None, ""}
        # But Hackit creds are still saved
        assert "hackit" in auth_after.get("providers", {})


# =============================================================================
# persist_hackit_credentials: shared helper for CLI + web dashboard login paths
# =============================================================================


def _full_state_fixture() -> dict:
    """Shape of the dict returned by _hackit_device_code_login /
    refresh_hackit_oauth_from_state. Used as helper input."""
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    return {
        "portal_base_url": "https://portal.example.com",
...
def test_persist_hackit_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    """Helper must populate BOTH credential_pool.hackit AND providers.hackit.

    Regression guard: before this helper existed, `slick auth add hackit`
    wrote only the pool. After the Hackit agent_key's 24h TTL expired, the
    401-recovery path in run_agent.py called resolve_hackit_runtime_credentials
    which reads providers.hackit, found it empty, raised AuthError, and the
    agent failed with "Non-retryable client error". Both stores must stay
    in sync at write time.
    """
    from slick_cli.auth import persist_hackit_credentials, HACKIT_DEVICE_CODE_SOURCE
    ...
    entry = persist_hackit_credentials(state)

    assert entry is not None
    assert entry.provider == "hackit"
    assert entry.source == HACKIT_DEVICE_CODE_SOURCE

    payload = json.loads((slick_home / "auth.json").read_text())

    # providers.hackit populated with the full state (new behaviour)
    singleton = payload["providers"]["hackit"]
    ...
    # credential_pool.hackit has exactly one canonical device_code entry
    pool_entries = payload["credential_pool"]["hackit"]
    assert len(pool_entries) == 1, pool_entries
    pool_entry = pool_entries[0]
    assert pool_entry["source"] == HACKIT_DEVICE_CODE_SOURCE
    assert pool_entry["agent_key"] == state["agent_key"]
    assert pool_entry["inference_base_url"] == "https://inference.example.com/v1"


def test_persist_hackit_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.hackit and lets seeding
    ...
    persist_hackit_credentials(first)
    ...
    persist_hackit_credentials(second)

    payload = json.loads((slick_home / "auth.json").read_text())

    # providers.hackit reflects the latest write (singleton semantics)
    assert payload["providers"]["hackit"]["access_token"] == second_token
    assert payload["providers"]["hackit"]["agent_key"] == second_token

    # credential_pool.hackit has exactly one entry, carrying the latest agent_key
    pool_entries = payload["credential_pool"]["hackit"]
    assert len(pool_entries) == 1, pool_entries
    assert pool_entries[0]["source"] == HACKIT_DEVICE_CODE_SOURCE
...
def test_persist_hackit_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    entry = persist_hackit_credentials(_full_state_fixture())
    assert entry is not None
    ...
    # No "label" key embedded in providers.hackit when the caller didn't supply one.
    payload = json.loads((slick_home / "auth.json").read_text())
    assert "label" not in payload["providers"]["hackit"]


def test_refresh_token_reuse_detection_surfaces_actionable_message():
    """Regression for #15099.

    When the Hackit Portal server returns ``invalid_grant`` with
    ...
    assert "slick auth add hackit" in message.lower()
...
def test_refresh_token_exchange_sends_refresh_token_header():
    """Hackit refresh tokens must be sent in a header so sandbox proxies can
    ...
    assert client.kwargs["headers"]["x-hackit-refresh-token"] == "refresh-1"
...
# =============================================================================
# Shared Hackit token store — cross-profile persistence (Codex-style auto-import)
# =============================================================================
...
def shared_store_env(tmp_path, monkeypatch):
    ...
    Required for every test that exercises the shared Hackit store — the
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    ...
    """
    from slick_cli.auth import _hackit_shared_store_path

    monkeypatch.delenv("SLICK_SHARED_AUTH_DIR", raising=False)

    with pytest.raises(RuntimeError, match="shared Hackit auth store"):
        _hackit_shared_store_path()
...
def test_shared_store_write_and_read_roundtrip(shared_store_env):
    ...
    loaded = _read_shared_hackit_state()
...
def test_persist_hackit_credentials_mirrors_to_shared_store(
    tmp_path, monkeypatch, shared_store_env,
):
    """persist_hackit_credentials must populate BOTH per-profile auth.json
    AND the shared store, so a future profile's `slick auth add hackit
    --type oauth` can one-tap import instead of redoing device-code.
    """
    from slick_cli.auth import (
        _hackit_shared_store_path,
        _read_shared_hackit_state,
        persist_hackit_credentials,
    )
    ...
    persist_hackit_credentials(_full_state_fixture())

    # Per-profile auth.json populated
    payload = json.loads((slick_home / "auth.json").read_text())
    assert "hackit" in payload.get("providers", {})
    ...
    assert str(_hackit_shared_store_path()).startswith(str(shared_store_env))
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    every field persist_hackit_credentials() needs.
    """
    from slick_cli import auth as auth_mod

    auth_mod._write_shared_hackit_state(_full_state_fixture())
    ...
    monkeypatch.setattr(auth_mod, "refresh_hackit_oauth_from_state", _fake_refresh)

    result = auth_mod._try_import_shared_hackit_state()
...
class TestStalePortalBaseUrlMigration:
    """_migrate_stale_hackit_portal_url auto-corrects stale portal_base_url on load."""

    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from slick_cli.auth import _load_auth_store, DEFAULT_HACKIT_PORTAL_URL
        ...
        auth_file.write_text(json.dumps({
            ...
            "providers": {
                "hackit": {
                    "portal_base_url": "https://api.hackit.cc",
        ...
        hackit = store["providers"]["hackit"]
        assert hackit["portal_base_url"] == DEFAULT_HACKIT_PORTAL_URL
    ...
    def test_runtime_credentials_rejects_http_for_production_portal(
        ...
        store["providers"]["hackit"]["portal_base_url"] = (
            "http://portal.hackit.cc"
        )
        ...
        auth_mod.resolve_hackit_runtime_credentials()
        assert refresh_calls == [auth_mod.DEFAULT_HACKIT_PORTAL_URL]
...
class TestHackitDeviceAuthTimeoutMessage:
    def test_timeout_message_mentions_captcha_login_and_retry(self):
        from slick_cli.auth import _hackit_device_auth_timeout_message

        msg = _hackit_device_auth_timeout_message("https://portal.hackit.cc")
        assert "CAPTCHA" in msg
        assert "slick portal" in msg
        assert "https://portal.hackit.cc/login" in msg
    ...
    def test_timeout_message_falls_back_to_default_portal(self):
        from slick_cli.auth import (
            ...
            _hackit_device_auth_timeout_message,
        )

        msg = _hackit_device_auth_timeout_message("")
        assert f"{DEFAULT_HACKIT_PORTAL_URL.rstrip('/')}/login" in msg
...
def test_poll_for_token_timeout_raises_actionable_message():
    ...
    so both the CLI login and the dashboard poller (web_server._hackit_poller,
...
def test_hackit_device_code_login_timeout_raises_actionable_message(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            "device_code": "device",
            "user_code": "SMCL-97YT",
            "verification_uri": "https://portal.hackit.cc/manage-subscription",
            "verification_uri_complete": (
                "https://portal.hackit.cc/manage-subscription"
                "?user_code=SMCL-97YT"
            ),
            "expires_in": 600,
            "interval": 1,
    ...
    def _timeout(**kwargs):
        raise TimeoutError(
            auth_mod._hackit_device_auth_timeout_message(
