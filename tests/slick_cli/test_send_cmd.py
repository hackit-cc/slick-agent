"""Tests for the ``slick send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``slick_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from slick_cli import send_cmd
...
def test_load_slick_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``slick send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``slick config set``) lives
    ...
    slick_home.mkdir()
    (slick_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_slick_home() to re-resolve under the patched env.
    from importlib import reload

    import slick_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_slick_env()

    assert os.environ.get("SOME_TOKEN") == "abc123"
    assert os.environ.get("TELEGRAM_HOME_CHANNEL") == "5550001111"


def test_load_slick_env_utf8_bom_preserves_first_key(tmp_path, monkeypatch):
    ...
    `slick send` failed to authenticate.
    """
    import os
...
def test_load_slick_env_bomless_utf8_still_loads(tmp_path, monkeypatch):
    """BOM-less UTF-8 .env files must keep loading after the utf-8-sig switch."""
    import os

    slick_home = tmp_path / ".slick"
    slick_home.mkdir()
    (slick_home / ".env").write_bytes(b"SEND_PLAIN_TOKEN=plain-val\n")
...
def test_load_slick_env_latin1_fallback_still_loads(tmp_path, monkeypatch):
...
def test_load_slick_env_latin1_fallback_overrides_shell(tmp_path, monkeypatch):
    ...
    (slick_home / ".env").write_bytes(b"SEND_OVR_TOKEN=caf\xe9-file\n")
...
def test_load_slick_env_fallback_read_error_is_swallowed(tmp_path, monkeypatch):
    ...
    (slick_home / ".env").write_bytes(b"SEND_ERR_TOKEN=caf\xe9\n")
...
def test_load_slick_env_bom_only_env_is_noop(tmp_path, monkeypatch):
    """A .env containing only a BOM must load zero vars without error."""
    import os

    slick_home = tmp_path / ".slick"
    slick_home.mkdir()
    (slick_home / ".env").write_bytes(b"\xef\xbb\xbf")
