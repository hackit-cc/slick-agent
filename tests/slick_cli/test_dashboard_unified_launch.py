...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "slick_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # SLICK_HOME.  For a standard install (SLICK_HOME unset) that root is
        # the platform-native default (~/.slick), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from slick_constants import get_default_slick_root
        assert env.get("SLICK_HOME") == str(get_default_slick_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (SLICK_DESKTOP=1) must NOT
