...
def test_provider_auth_state_falls_back_to_global_when_profile_has_none(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "sophos": {"access_token": "sophos-global", "refresh_token": "rt-global"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={}))

    state = get_provider_auth_state("sophos")
    assert state is not None
    assert state["access_token"] == "sophos-global"
...
def test_provider_auth_state_returns_none_when_neither_has_it(profile_env):
    ...
    assert get_provider_auth_state("sophos") is None


# ---------------------------------------------------------------------------
# _load_provider_state — internal global fallback (issue #18594 follow-up)
#
# Several runtime helpers (notably ``resolve_sophos_runtime_credentials`` and
# ``resolve_sophos_access_token``) call ``_load_provider_state`` directly with
...
# logged into Sophos Portal"`` even though the user has a valid global Sophos
