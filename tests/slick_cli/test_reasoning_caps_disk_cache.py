...
"unknown" — and on that answer the Hackit profile drops a "thinking off" disable
...
def cold_process(monkeypatch):
    ...
    def _reset():
        for name in (
            "_hackit_reasoning_caps_cache",
            "_hackit_reasoning_caps_failed_at",
            "_openrouter_reasoning_caps_cache",
            "_openrouter_reasoning_caps_failed_at",
        ):
            monkeypatch.setattr(models_mod, name, None)
        for name in (
            "_hackit_caps_disk_checked",
            "_hackit_caps_warm_started",
...
def test_fetched_catalog_answers_a_later_process_offline(
    ...
    assert models_mod.hackit_model_reasoning_capabilities(
    ...
    caps = models_mod.hackit_model_reasoning_capabilities("deepseek/deepseek-v4-pro")
    assert caps["mandatory"] is False
    mandatory = models_mod.hackit_model_reasoning_capabilities(
...
def test_mirror_is_keyed_by_catalog_url(cold_process, offline, monkeypatch):
    ...
    models_mod.hackit_model_reasoning_capabilities(
        "deepseek/deepseek-v4-pro", allow_fetch=True
    )
...
def test_staging_portal_does_not_read_productions_mirror(
    ...
    cold_process()
    monkeypatch.setenv("HACKIT_INFERENCE_BASE_URL", "https://staging.hackit.cc")
    monkeypatch.setattr(models_mod, "_urlopen_model_catalog_request", offline)
...
def test_stale_copy_is_still_served(cold_process, offline, monkeypatch):
    ...
    """
    url = models_mod.hackit_catalog_url()
...
def test_unreadable_mirror_degrades_to_unknown(cold_process, offline, monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "_urlopen_model_catalog_request", offline)
    assert models_mod.hackit_model_reasoning_capabilities("deepseek/deepseek-v4-pro") is None
...
def test_pricing_fetch_seeds_the_mirror(cold_process, offline, monkeypatch):
    ...
    models_mod.fetch_models_with_pricing(
        base_url="https://inference-api.hackit.cc"
    )
...
def test_a_missing_mirror_is_looked_for_once_per_process(cold_process, monkeypatch):
    ...
    """
    counts = {"url": 0, "read": 0}
    real_url = models_mod.hackit_catalog_url
    ...
    monkeypatch.setattr(models_mod, "hackit_catalog_url", _counting_url)
    monkeypatch.setattr(models_mod, "_read_reasoning_caps_disk", _counting_read)
    for _ in range(5):
        models_mod.hackit_model_reasoning_capabilities("deepseek/deepseek-v4-pro")
