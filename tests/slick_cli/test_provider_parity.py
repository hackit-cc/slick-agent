...
the SAME provider universe as ``slick model`` (the CLI/TUI picker).
...
def test_every_slick_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `slick model` universe."""
    ...
    assert not missing, (
        "providers shown in `slick model` but not configurable in the desktop "
