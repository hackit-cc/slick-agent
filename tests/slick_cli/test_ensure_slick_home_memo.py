"""ensure_slick_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".slick"
    second = tmp_path / "b" / ".slick"

    monkeypatch.setenv("SLICK_HOME", str(first))
    cfg.ensure_slick_home()

    # Profile switch: SLICK_HOME moves → the new path is ensured too.
    monkeypatch.setenv("SLICK_HOME", str(second))
    cfg.ensure_slick_home()
