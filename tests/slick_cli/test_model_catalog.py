...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "hackit": {
                "metadata": {"display_name": "Hackit Portal"},
                "models": [
...
class TestFallbackChain:
    ...
    PRIMARY = "https://slick-agent.hackit.cc/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/hackit-cc/slick-agent"
...
class TestCuratedAccessors:
    ...
    def test_hackit_returns_none_when_catalog_empty(self, isolated_home):
        from slick_cli import model_catalog
        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            assert model_catalog.get_curated_hackit_models() is None
...
class TestDefaultModelFromCache:
    ...
    def _manifest_with_default(self) -> dict:
        ...
        m["providers"]["hackit"]["models"][1]["default"] = True  # kimi-k2.6
        return m
    ...
    def test_reads_label_from_disk_cache(self, isolated_home):
        ...
        with patch.object(model_catalog, "_fetch_manifest") as fetch:
            ...
            assert (
                model_catalog.get_default_model_from_cache("hackit")
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        for provider in ("openrouter", "hackit"):
...
class TestIntegrationWithModelsModule:
    ...
    def test_picker_hackit_row_uses_curated_list(self, tmp_path, monkeypatch):
        """The /model picker surfaces the curated ``_PROVIDER_MODELS["hackit"]``
        ...
        import importlib
        from slick_cli import model_catalog
        from slick_cli.models import get_curated_hackit_model_ids
        importlib.reload(model_catalog)
        try:
            ...
            (active_home / "auth.json").write_text(
                json.dumps(
                    {
                        "providers": {"hackit": {"access_token": "fake"}},
                        "credential_pool": {},
                    }
                )
            )
            ...
            # (``curated["hackit"] = get_curated_hackit_model_ids()``), so the test
            # stays an invariant — it can't rot as the curated/manifest list grows.
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("slick_cli.models.check_hackit_free_tier", return_value=False), patch(
                ...
                expected = get_curated_hackit_model_ids()
                picker = list_picker_providers(
                    current_provider="hackit", max_models=99
                )
        finally:
            model_catalog.reset_cache()

        hackit_row = next((r for r in picker if r["slug"] == "hackit"), None)
        assert hackit_row is not None, "hackit row must appear when authed"
        assert hackit_row["models"] == expected
    ...
    def test_picker_max_models_cap_semantics(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            with patch.object(
                ...
                expected = get_curated_hackit_model_ids()
                full = list_picker_providers(current_provider="hackit", max_models=None)
                one = list_picker_providers(current_provider="hackit", max_models=1)
                ...
                zero = list_authenticated_providers(
                    current_provider="hackit", max_models=0
                )
        finally:
            model_catalog.reset_cache()

        def _hackit(rows):
            return next((r for r in rows if r["slug"] == "hackit"), None)

        # Only meaningful when the curated list actually exceeds 1 entry.
        assert len(expected) > 1, "test needs a multi-model curated hackit list"

        full_row = _hackit(full)
        assert full_row is not None and full_row["models"] == expected

        one_row = _hackit(one)
        assert one_row is not None and one_row["models"] == expected[:1]

        zero_row = _hackit(zero)
...
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["hackit"] in commit
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            ...
            "_PROVIDER_MODELS['hackit'] / OPENROUTER_MODELS. "
