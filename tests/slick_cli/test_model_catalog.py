"""Tests for slick_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate SLICK_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".slick"
    home.mkdir()
    ...
    import importlib
    from slick_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "sophos": {
                "metadata": {"display_name": "Sophos Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from slick_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://slick-agent.sophostechne.com/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/SophosTechne/slick-agent"
...
class TestCuratedAccessors:
    ...
    def test_sophos_returns_none_when_catalog_empty(self, isolated_home):
        from slick_cli import model_catalog
        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            assert model_catalog.get_curated_sophos_models() is None
...
class TestDefaultModelFromCache:
    ...
    def _manifest_with_default(self) -> dict:
        ...
        m["providers"]["sophos"]["models"][1]["default"] = True  # kimi-k2.6
        return m
    ...
    def test_reads_label_from_disk_cache(self, isolated_home):
        ...
        with patch.object(model_catalog, "_fetch_manifest") as fetch:
            ...
            assert (
                model_catalog.get_default_model_from_cache("sophos")
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import slick_cli.model_catalog as model_catalog
        from slick_cli.models import PREFERRED_SILENT_DEFAULT_MODEL

        repo_root = Path(model_catalog.__file__).resolve().parent.parent
        manifest = json.loads(
            (repo_root / "website" / "static" / "api" / "model-catalog.json").read_text()
        )
        for provider in ("openrouter", "sophos"):
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in slick_cli.models."""



    def test_picker_sophos_row_uses_curated_list(self, tmp_path, monkeypatch):
        """The /model picker surfaces the curated ``_PROVIDER_MODELS["sophos"]``
        list in curated order — matching the ``slick model`` CLI — not the live
        ...
        # ``SLICK_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` SLICK_HOME directly instead.
        import importlib
        from slick_cli import model_catalog
        from slick_cli.models import get_curated_sophos_model_ids
        importlib.reload(model_catalog)
        try:
            from slick_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["SLICK_HOME"])
            (active_home / "auth.json").write_text(
                json.dumps(
                    {
                        "providers": {"sophos": {"access_token": "fake"}},
                        "credential_pool": {},
                    }
                )
            )
            ...
            # (``curated["sophos"] = get_curated_sophos_model_ids()``), so the test
            # stays an invariant — it can't rot as the curated/manifest list grows.
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("slick_cli.models.check_sophos_free_tier", return_value=False), patch(
                "slick_cli.models.union_with_portal_free_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ), patch(
                "slick_cli.models.union_with_portal_paid_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ):
                expected = get_curated_sophos_model_ids()
                picker = list_picker_providers(
                    current_provider="sophos", max_models=99
                )
        finally:
            model_catalog.reset_cache()

        sophos_row = next((r for r in picker if r["slug"] == "sophos"), None)
        assert sophos_row is not None, "sophos row must appear when authed"
        assert sophos_row["models"] == expected
    ...
    def test_picker_max_models_cap_semantics(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            with patch.object(
                ...
                expected = get_curated_sophos_model_ids()
                full = list_picker_providers(current_provider="sophos", max_models=None)
                one = list_picker_providers(current_provider="sophos", max_models=1)
                ...
                zero = list_authenticated_providers(
                    current_provider="sophos", max_models=0
                )
        finally:
            model_catalog.reset_cache()

        def _sophos(rows):
            return next((r for r in rows if r["slug"] == "sophos"), None)

        # Only meaningful when the curated list actually exceeds 1 entry.
        assert len(expected) > 1, "test needs a multi-model curated sophos list"

        full_row = _sophos(full)
        assert full_row is not None and full_row["models"] == expected

        one_row = _sophos(one)
        assert one_row is not None and one_row["models"] == expected[:1]

        zero_row = _sophos(zero)
...
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["sophos"] in commit
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            ...
            "_PROVIDER_MODELS['sophos'] / OPENROUTER_MODELS. "
