...
columns + Free/Pro badges and gate paid models on free Hackit accounts, the
...
def _patch_pricing(monkeypatch, *, free_tier, pricing, unavailable=None):
    ...
    monkeypatch.setattr(models_mod, "check_hackit_free_tier", lambda *, force_fresh=False: free_tier)
    monkeypatch.setattr(
...
def test_apply_pricing_free_models_get_flat_100_percent_sale(monkeypatch):
    ...
    rows = [{"slug": "hackit", "models": ["a/free", "b/natively-free"]}]
    inv._apply_pricing(rows)
