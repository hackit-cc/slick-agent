"""Tests for the per-profile Projects store (slick_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Slick Agent", folders=["/tmp/slick"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "slick-agent"
    assert proj.name == "Slick Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/slick"
    assert [f.path for f in proj.folders] == ["/tmp/slick"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "slick-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' SLICK_HOME.
