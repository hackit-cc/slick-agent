"""Regression tests for the Sophos Portal env-override bypassing the host
allowlist, mirroring the existing SOPHOS_INFERENCE_BASE_URL /
_ALLOWED_SOPHOS_INFERENCE_HOSTS treatment.

Real incident (2026-07): a hosted agent provisioned by sophos-account-service
on the `staging` Vercel environment is stamped with
``SLICK_PORTAL_BASE_URL=https://portal.staging-sophostechne.com`` in its
...
Before this fix, ``resolve_sophos_access_token`` / ``resolve_sophos_runtime_
...
``_SOPHOS_PORTAL_ALLOWED_HOSTS``, which only recognised the production host.
...
``_quarantine_sophos_oauth_state`` and wiped the entire credential pool.

The correct fix (mirroring ``_sophos_inference_env_override()``): the env
...
from slick_cli.auth import (
    ...
    _SOPHOS_PORTAL_ALLOWED_HOSTS,
    _sophos_portal_env_override,
)
...
class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        ...
        monkeypatch.delenv("SOPHOS_PORTAL_BASE_URL", raising=False)
        assert _sophos_portal_env_override() is None


    def test_env_override_not_gated_by_allowlist(self, monkeypatch):
        """The whole point: an env-set staging host is NOT in
        _SOPHOS_PORTAL_ALLOWED_HOSTS, and the helper must return it anyway —
        gating happens only for network-provenance values."""
        monkeypatch.setenv(
            "SLICK_PORTAL_BASE_URL", "https://portal.staging-sophostechne.com"
        )
        assert "portal.staging-sophostechne.com" not in _SOPHOS_PORTAL_ALLOWED_HOSTS
        assert (
            _sophos_portal_env_override() == "https://portal.staging-sophostechne.com"
        )


class TestResolveAccessTokenEnvOverrideWins:
    """End-to-end: resolve_sophos_access_token must use the env override for
    ...
    def _run_and_capture(self, monkeypatch, auth):
        ...
        try:
            auth.resolve_sophos_access_token()
        finally:
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        staging_portal = "https://portal.staging-sophostechne.com"
    ...
    def test_no_env_no_staging_state_prod_url_used_unmodified(
        ...
        monkeypatch.delenv("SLICK_PORTAL_BASE_URL", raising=False)
        monkeypatch.delenv("SOPHOS_PORTAL_BASE_URL", raising=False)
        self._write_auth_file(tmp_path, stored_portal_url=DEFAULT_SOPHOS_PORTAL_URL)

        seen_portal_urls, records = self._run_and_capture(monkeypatch, auth)

        assert seen_portal_urls == [DEFAULT_SOPHOS_PORTAL_URL]
