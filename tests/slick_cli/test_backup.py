"""Tests for slick backup and import commands."""
...
def _no_real_gateway_service(monkeypatch):
    ...
    assert the wiring."""
    import slick_cli.gateway as gateway_mod
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew slick_cli.backup's datetime forward instead of sleeping.
    ...
    import slick_cli.backup as _backup

    shim = getattr(_backup.datetime, "_slick_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _slick_test_shim = True
...
def _make_slick_tree(root: Path) -> None:
    """Create a realistic ~/.slick directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "slick_state.db"):
    ...
    # slick-agent repo (should be EXCLUDED)
    (root / "slick-agent").mkdir(exist_ok=True)
    (root / "slick-agent" / "run_agent.py").write_text("# big file\n")
    (root / "slick-agent" / ".git").mkdir()
    (root / "slick-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_slick_agent(self):
        from slick_cli.backup import _should_exclude
        assert _should_exclude(Path("slick-agent/run_agent.py"))
        assert _should_exclude(Path("slick-agent/.git/HEAD"))
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from slick_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        slick_home = tmp_path / ".slick"
        slick_home.mkdir()
        _make_slick_tree(slick_home)

        monkeypatch.setenv("SLICK_HOME", str(slick_home))
        ...
        import slick_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = slick_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, slick_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside SLICK_HOME."""
        slick_home = tmp_path / ".slick"
        slick_home.mkdir()
        _make_slick_tree(slick_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(slick_home / "skills" / "outside-link.txt", outside)
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        slick_home.mkdir()
        _make_slick_tree(slick_home)
        with sqlite3.connect(slick_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from slick_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup

        # Real producer, so the layout under state-snapshots/ is whatever the
        # code actually writes (manifest.json + state.db copy + ...).
        snap_id = create_quick_snapshot(slick_home=slick_home)
        assert snap_id and (slick_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Slick backup."""
        from slick_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Slick configuration has been restored." in out
        assert "slick gateway install" in out
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        slick_home = tmp_path / ".slick"
        (slick_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("SLICK_HOME", str(slick_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (slick_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (slick_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            slick_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (slick_home / "gateway.pid").write_text("4242")
        (slick_home / "processes.json").write_text('{"live": true}')
        ...
        assert (slick_home / "gateway.pid").read_text() == "4242"
        assert (slick_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (slick_home / "cron.pid").exists()
        assert not (slick_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        self._make_backup_zip(zip_path, {
            ...
            "auth.json": '{"providers": {"hackit": "token"}}',
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (slick_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".slick"
        src_home.mkdir(parents=True)
        _make_slick_tree(src_home)

        monkeypatch.setenv("SLICK_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from slick_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".slick"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("SLICK_HOME", str(dst_home))
        ...
        # slick-agent should NOT be present
        assert not (dst_home / "slick-agent").exists()
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from slick_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".slick/", "")
            zf.writestr(".slick/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_slick_home(self, tmp_path, monkeypatch):
        """Backup handles empty slick home (no files to back up)."""
        slick_home = tmp_path / ".slick"
        slick_home.mkdir()
        # Only excluded dirs, no actual files
        (slick_home / "__pycache__").mkdir()
        (slick_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        slick_home = tmp_path / ".slick"
        slick_home.mkdir()
        (slick_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = slick_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        slick_home = tmp_path / ".slick"
        slick_home.mkdir()
        (slick_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("SLICK_HOME", str(slick_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (slick_home / "config.yaml").exists()
        assert (slick_home / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    """`slick import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the SLICK_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (slick_home / "config.yaml").write_text(original)
        monkeypatch.setenv("SLICK_HOME", str(slick_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (slick_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(slick_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside SLICK_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        slick_home = dst_home / ".slick"
        slick_home.mkdir()
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo slick import`` onto a user-owned (or Docker/NAS volume-owned)
        SLICK_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "slick_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``slick import`` is the
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from slick_cli.backup import _safe_copy_db
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from slick_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from slick_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def slick_home(self, tmp_path):
        """Create a fake SLICK_HOME with critical state files."""
        home = tmp_path / ".slick"
        home.mkdir()
    ...
    def test_state_db_safely_copied(self, slick_home):
        from slick_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(slick_home=slick_home)
        db_copy = slick_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, slick_home, monkeypatch, capsys):
    ...
    def test_restore_state_db_live_connection(self, slick_home):
        ...
        """
        from slick_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(slick_home=slick_home)

        # Open a live connection (simulating gateway/dashboard).
        live_conn = sqlite3.connect(str(slick_home / "state.db"))
        ...
        result = restore_quick_snapshot(snap_id, slick_home=slick_home)
        assert result is True
    ...
    def test_snapshot_includes_pairing_directories(self, slick_home):
        ...
        (slick_home / "platforms" / "pairing").mkdir(parents=True)
        (slick_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (slick_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (slick_home / "pairing").mkdir()
        (slick_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (slick_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = slick_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (slick update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, slick_home, capsys):
        ...
        """
        import json
        from slick_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", slick_home=slick_home)
        assert first_id is not None
        first_dir = slick_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", slick_home=slick_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = slick_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, slick_home=slick_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, slick_home):
        ...
        board_dir = slick_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(slick_home=slick_home)
        copy = (
            slick_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, slick_home=slick_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, slick_home, monkeypatch):
        ...
        import slick_cli.backup as bk
        from slick_cli.backup import create_quick_snapshot

        board = slick_home / "kanban" / "boards" / "work"
        ...
        copy = slick_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``slick update``
    ...
    def slick_home(self, tmp_path):
        root = tmp_path / ".slick"
        root.mkdir()
        _make_slick_tree(root)
        return root


    def test_backup_contents_match_full_backup(self, slick_home):
        """Pre-update backup should include the same user data that
        ``slick backup`` would, and should exclude the same directories."""
        from slick_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(slick_home=slick_home)
        ...
        # slick-agent repo excluded
        assert not any(n.startswith("slick-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, slick_home):
        """``slick update`` in ``full`` mode takes the quick snapshot *before*
        ...
        snap_id = create_quick_snapshot(label="pre-update", slick_home=slick_home)
        assert snap_id and (slick_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
    ...
    def test_rotation_keeps_only_n(self, slick_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(slick_home=slick_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (slick_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, slick_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside SLICK_HOME."""
        from slick_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def slick_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".slick"
        root.mkdir()
        _make_slick_tree(root)
        # Point SLICK_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("SLICK_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve SLICK_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(slick_home, value):
        import yaml
        (slick_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(slick_home):
        d = slick_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(slick_home):
        d = slick_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, slick_home, capsys):
        ...
        self._set_mode(slick_home, "off")
        from slick_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(slick_home)
        assert not self._zips(slick_home)



    def test_config_full_mode(self, slick_home, capsys):
        self._set_mode(slick_home, "full")
        from slick_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(slick_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (slick claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``slick claw migrate`` runs before mutating ~/.slick/."""
    ...
    def test_restorable_with_slick_import(self, slick_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Slick
        backup — `slick import` should accept it."""
        from slick_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(slick_home=slick_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, slick_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from slick_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(slick_home=slick_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(slick_home=slick_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`slick update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, slick_home: Path, label="pre-update"):
        from slick_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, slick_home=slick_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from slick_cli.backup import restore_cron_jobs_if_emptied
        slick_home = tmp_path / ".slick"
        jobs_path = slick_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(slick_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, slick_home=slick_home)
        assert result is not None
...
# location, NOT under SLICK_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, slick_home: Path) -> None:
        slick_home.mkdir(parents=True, exist_ok=True)
        (slick_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (slick_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (slick_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        slick_home = tmp_path / ".slick"
        self._make_min_tree(slick_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under SLICK_HOME,
        ...
        # External state did NOT leak into SLICK_HOME.
        assert not (slick_home / "_external").exists()
