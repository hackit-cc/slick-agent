...
def _summary_output(capsys, provider_ready: bool):
    ...
    with patch("slick_cli.auth.resolve_provider", resolver), \
         patch.object(setup_mod, "get_hackit_subscription_features") as feats:
...
def test_summary_warns_when_no_provider(capsys):
    ...
    assert "slick model" in out
    assert "slick setup --portal" in out
