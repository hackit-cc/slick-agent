...
from slick_cli.auth import (
    SOPHOS_BILLING_MANAGE_SCOPE,
    sophos_token_has_billing_scope,
    step_up_sophos_billing_scope,
)
...
def _stub_persist(monkeypatch):
    ...
    monkeypatch.setattr(auth, "_write_shared_sophos_state", lambda *a, **kw: None)
    monkeypatch.setattr(auth, "_sync_sophos_pool_from_auth_store", lambda: None)
...
def test_step_up_requests_billing_scope_and_reuses_prior_urls(monkeypatch, _stub_persist):
    ...
    monkeypatch.setattr(auth, "_sophos_device_code_login", _fake_login)

    granted = step_up_sophos_billing_scope()
    assert granted is True
    # Requested scope must include billing:manage, preserving prior scopes.
    assert SOPHOS_BILLING_MANAGE_SCOPE in captured["scope"].split()
    ...
    assert captured["client_id"] == "slick-cli"
...
def test_device_login_fires_on_verification_before_polling(monkeypatch):
    ...
    try:
        auth._sophos_device_code_login(open_browser=False, on_verification=_cb)
    except Exception:
        pass
