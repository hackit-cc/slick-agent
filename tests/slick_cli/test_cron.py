"""Tests for slick_cli.cron command handling."""
...
from slick_cli import cron as cron_cli
...
class TestCronCommandLifecycle:
    ...
    def test_edit_persists_user_owned_inference_pins(self, tmp_cron_dir, capsys):
        ...
        assert updated["provider"] == "hackit"
...
class TestGatewayNotRunningWarning:
    ...
    def test_list_warns_when_gateway_absent(self, tmp_cron_dir, capsys, monkeypatch):
        ...
        monkeypatch.setattr("slick_cli.gateway.find_gateway_pids", lambda: [])
...
class TestExternalCronProviderStatus:
    """With an external cron provider (e.g. Webcron), jobs fire via a
    ...
    Webcron instance (the heartbeat is intentionally never written).
    """
    ...
    def test_status_reports_provider_not_ticker_for_chronos(
        ...
        monkeypatch.setattr(
            "slick_cli.cron._active_cron_provider_name", lambda: "webcron"
        )
        # Even with NO gateway process and NO ticker heartbeat, Webcron status
        # must NOT report a stall / "not firing".
        monkeypatch.setattr("slick_cli.gateway.find_gateway_pids", lambda: [])
        cron_command(Namespace(cron_command="status"))
        out = capsys.readouterr().out
        assert "webcron" in out
...
class TestCronRunBackgroundDispatch:
    """`slick cron run` must not report 'failed' when the run was dispatched
