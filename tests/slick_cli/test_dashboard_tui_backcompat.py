"""Regression test: `slick dashboard --tui` must not hard-crash.

Older Slick desktop app shells (<= 0.15.x) spawn the backend as::

    slick dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Slick couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real slick_cli.main parser in a subprocess.
