"""Tests for the Webcron cron-fire webhook ON THE DASHBOARD APP (web_server).

Regression guard for the relocation bug: the fire webhook MUST live on the
dashboard FastAPI app (`slick_cli.web_server.app`) — the agent's public HTTP
...
def test_bad_token_401(monkeypatch):
    ...
    fired = []
    monkeypatch.setattr(
        "plugins.cron_providers.webcron.verify.get_fire_verifier",
...
def test_gateway_unreachable_503_for_nas_retry(monkeypatch):
    ...
    retries per the Webcron contract. Deliberately NO local-execution
...
def test_gateway_intentionally_stopped_drops_with_200(monkeypatch):
    ...
    502-retry storms at NAS (OOF-266). Jobs re-arm via the Webcron provider's
