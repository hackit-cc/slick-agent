"""Tests for the unified provider catalog (slick_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``slick model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from slick_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
