...
def slick_home(tmp_path, monkeypatch):
    ...
    # /etc/slick on the dev/CI box can't influence the test.
    monkeypatch.setenv("SLICK_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(slick_home, monkeypatch):
...
def test_env_expansion_in_user_config(slick_home, monkeypatch):
    from slick_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_slick_dotenv(slick_home=str(home))
