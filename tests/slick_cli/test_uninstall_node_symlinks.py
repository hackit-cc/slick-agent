"""Tests for slick_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $SLICK_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Slick-managed node dir.
"""
...
def _make_slick_node(slick_home: Path) -> Path:
    """Create a fake $SLICK_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = slick_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    slick_home = fake_home / ".slick"
    _make_slick_node(slick_home)
    ...
    removed = uninstall.remove_node_symlinks(slick_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    slick_home = fake_home / ".slick"
    node_bin = _make_slick_node(slick_home)
