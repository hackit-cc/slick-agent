"""Tests for the top-level `./slick` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./slick` should use `slick_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "slick"
    called = []

    fake_main_module = types.ModuleType("slick_cli.main")

    def fake_main():
        called.append("slick_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "slick_cli.main", fake_main_module)
