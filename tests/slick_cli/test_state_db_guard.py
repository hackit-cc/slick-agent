...
from slick_cli.backup import copy_db_and_verify, verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    SLICK_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def slick_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("slick_cli.config") or mod == "slick_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, slick_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, slick_home, capsys, monkeypatch):
