"""Tests for normalized Hackit Portal account entitlement helpers."""
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    monkeypatch.setattr("slick_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("slick_cli.auth.resolve_hackit_access_token", lambda: "fresh-token")
    monkeypatch.setattr("slick_cli.hackit_account._fetch_hackit_account_info", lambda *a, **kw: payload)

    info = get_hackit_portal_account_info(force_fresh=True)

    assert isinstance(info, HackitPortalAccountInfo)
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("slick_cli.auth.get_provider_auth_state", lambda provider: {})
...
def test_member_spend_cap_exceeded_message(monkeypatch):
    ...
    msg = format_hackit_portal_entitlement_message(info, capability="Hackit model access")
