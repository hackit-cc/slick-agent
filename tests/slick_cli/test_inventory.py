...
def _hackit_row(model: str = "openai/gpt-5.5") -> dict:
    return {
        "slug": "hackit",
        "name": "Hackit",
        "models": [model],
...
def test_list_authenticated_providers_force_fresh_is_keyword_only():
    """``force_fresh_hackit_tier`` must be keyword-only on the public listing API.
    ...
    param = sig.parameters["force_fresh_hackit_tier"]
...
def test_explicit_only_filters_ambient_credentials_but_keeps_current_and_custom_rows():
    rows = [
        ...
        {"slug": "copilot", "name": "Copilot", "models": ["gpt-5.4"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "slick"},
        {"slug": "hackit", "name": "Hackit", "models": ["anthropic/claude-sonnet-5"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "slick"},
...
def test_user_defined_rows_carry_alias_set_for_gui_current_match():
    ...
    assert "aliases" not in by_slug["hackit"]
