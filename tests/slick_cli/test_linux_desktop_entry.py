"""Tests for the Linux XDG desktop entry installed by ``slick desktop``."""
...
from slick_cli import linux_desktop_entry as lde
...
def test_install_writes_entry_with_absolute_exec_and_icon(tmp_path, xdg_home, monkeypatch):
    ...
    slick_bin = tmp_path / "bin" / "slick"
    slick_bin.parent.mkdir()
    slick_bin.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "slick_cli.relaunch.resolve_slick_bin", lambda: str(slick_bin)
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)

    assert entry == xdg_home / "applications" / "slick.desktop"
    values = _parse(entry.read_text(encoding="utf-8"))

    # Exec must be the absolute path of the resolved binary. The launcher
    # runs with a minimal PATH, so a bare `slick` would not resolve.
    assert values["Exec"] == f"{slick_bin} desktop"
    ...
    assert values["Name"] == "Slick"
    assert values["Terminal"] == "false"


def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("slick_cli.relaunch.resolve_slick_bin", lambda: "/usr/bin/slick")
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_falls_back_to_interpreter_module(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("slick_cli.relaunch.resolve_slick_bin", lambda: None)
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)
    exec_line = _parse(entry.read_text(encoding="utf-8"))["Exec"]

    assert exec_line.endswith("-m slick_cli.main desktop")
    assert Path(exec_line.split(" ")[0]).is_absolute()


# #90292: the shell installer's bash wrapper makes argv[0] the repo `slick`
...
def test_exec_prefixes_interpreter_for_env_shebang_python_script(tmp_path, xdg_home, monkeypatch):
    ...
    slick_bin = tmp_path / "bin" / "slick"
    slick_bin.parent.mkdir()
    slick_bin.write_text("#!/usr/bin/env python3\nimport slick_cli\n", encoding="utf-8")
    slick_bin.chmod(0o755)
    monkeypatch.setattr("slick_cli.relaunch.resolve_slick_bin", lambda: str(slick_bin))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
    ...
    assert str(slick_bin) in exec_line
...
def test_exec_leaves_shell_wrapper_launchers_alone(tmp_path, xdg_home, monkeypatch):
    ...
    slick_bin = tmp_path / "bin" / "slick"
    slick_bin.parent.mkdir()
    slick_bin.write_text('#!/bin/bash\nexec /opt/slick/venv/bin/python "$@"\n', encoding="utf-8")
    slick_bin.chmod(0o755)
    monkeypatch.setattr("slick_cli.relaunch.resolve_slick_bin", lambda: str(slick_bin))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
    ...
    assert exec_line == f"{slick_bin} desktop"
...
def test_exec_leaves_venv_shebang_scripts_alone(tmp_path, xdg_home, monkeypatch):
    ...
    slick_bin.parent.mkdir()
    interpreter = str(Path(sys.executable).resolve())
    slick_bin.write_text(f"#!{interpreter}\nimport slick_cli\n", encoding="utf-8")
    slick_bin.chmod(0o755)
    monkeypatch.setattr("slick_cli.relaunch.resolve_slick_bin", lambda: str(slick_bin))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_install_without_source_icon_uses_themed_name(tmp_path, xdg_home, monkeypatch):
    ...
    # when Slick is installed some other way.
    assert _parse(entry.read_text(encoding="utf-8"))["Icon"] == "slick"
...
def test_exec_arg_quoting_handles_spaces(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    spaced = tmp_path / "my apps" / "slick"
    spaced.parent.mkdir()
    spaced.write_text("", encoding="utf-8")
    monkeypatch.setattr("slick_cli.relaunch.resolve_slick_bin", lambda: str(spaced))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
