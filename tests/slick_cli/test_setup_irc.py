"""Tests for IRC gateway configuration via `slick setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Slick install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Slick install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#slick")
            monkeypatch.setenv("IRC_NICKNAME", "slick-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `slick setup gateway` experience with IRC present."""
