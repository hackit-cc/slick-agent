...
``_load_auth_store`` and the Codex/Sophos shared-store readers previously called
...
class TestAuthJsonSiblingReaders:
    ...
    def test_auxiliary_sophos_provider_reads_non_ascii_store(self, slick_home, windows_default_encoding, monkeypatch):
        """agent/auxiliary_client's Sophos-provider lookup reads the same store.

        The lookup returns None on any read failure, silently disabling Sophos as
        ...
        store = {
            ...
            "providers": {"sophos": {"agent_key": "k", "label": "工作账号"}},
        }
        _write_utf8(slick_home / "auth.json", store)
        ...
        # _read_sophos_auth consults the credential pool FIRST and returns early
        ...
        provider = aux._read_sophos_auth()
        assert provider is not None
        assert provider.get("agent_key") == "k"

    def test_read_shared_sophos_state_reads_non_ascii_store(
        self, tmp_path, monkeypatch, windows_default_encoding
    ):
        """slick_cli.auth._read_shared_sophos_state must read a non-ASCII store.

        The shared Sophos store (``sophos_auth.json``) is written as UTF-8. A
        ...
        _write_utf8(shared_dir / "sophos_auth.json", payload)

        provider = auth._read_shared_sophos_state()
        assert provider is not None
    ...
    def test_managed_tool_gateway_reads_non_ascii_sophos_state(
        self, slick_home, windows_default_encoding
    ):
        """tools.managed_tool_gateway._read_sophos_provider_state reads auth.json.

        The Sophos provider entry can carry a non-ASCII label. Under the
        ...
        from tools.managed_tool_gateway import _read_sophos_provider_state

        sophos = _read_sophos_provider_state()
        assert sophos is not None
        assert sophos.get("agent_key") == "k"
        # The non-ASCII label round-trips intact.
        assert sophos.get("label") == "工作账号"
