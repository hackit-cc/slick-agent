...
``_load_auth_store`` and the Codex/Hackit shared-store readers previously called
...
def slick_home(tmp_path, monkeypatch):
    """Point SLICK_HOME at a tmp dir so we never touch the real auth store.

    Required because ``_auth_file_path()`` has a seat belt that refuses to
    resolve to the real user's ~/.slick/auth.json under pytest.
    """
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, slick_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, slick_home, windows_default_encoding):
        ...
        assert not (slick_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, slick_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, slick_home):
        auth_path = slick_home / "auth.json"
...
# --- sibling readers of the same ~/.slick/auth.json in other modules -------
#
# _load_auth_store lives in slick_cli/auth.py, but several other modules read
# the same ~/.slick/auth.json directly. They had the same UTF-8-vs-cp1252
# asymmetry on Windows — these tests pin the sibling reads too.

class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, slick_home, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(slick_home / "auth.json", store)

        from tools.xai_http import has_xai_credentials

        assert has_xai_credentials() is True

    def test_auxiliary_hackit_provider_reads_non_ascii_store(self, slick_home, windows_default_encoding, monkeypatch):
        """agent/auxiliary_client's Hackit-provider lookup reads the same store.

        The lookup returns None on any read failure, silently disabling Hackit as
        ...
        store = {
            ...
            "providers": {"hackit": {"agent_key": "k", "label": "工作账号"}},
        }
        _write_utf8(slick_home / "auth.json", store)
        ...
        # SLICK_HOME env from the fixture doesn't reach it — point it at
        # the tmp store explicitly.
        monkeypatch.setattr(aux, "_AUTH_JSON_PATH", slick_home / "auth.json")

        # _read_hackit_auth consults the credential pool FIRST and returns early
        ...
        provider = aux._read_hackit_auth()
        assert provider is not None
        assert provider.get("agent_key") == "k"

    def test_read_shared_hackit_state_reads_non_ascii_store(
        self, tmp_path, monkeypatch, windows_default_encoding
    ):
        """slick_cli.auth._read_shared_hackit_state must read a non-ASCII store.

        The shared Hackit store (``hackit_auth.json``) is written as UTF-8. A
        ...
        _write_utf8(shared_dir / "hackit_auth.json", payload)

        provider = auth._read_shared_hackit_state()
        assert provider is not None
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, slick_home, monkeypatch, windows_default_encoding
    ):
        """slick_cli.main._has_any_provider_configured reads auth.json.
        ...
        # slick_cli.auth; patch it on the source module so the local import
        # sees the fake.
    ...
    def test_managed_tool_gateway_reads_non_ascii_hackit_state(
        self, slick_home, windows_default_encoding
    ):
        """tools.managed_tool_gateway._read_hackit_provider_state reads auth.json.

        The Hackit provider entry can carry a non-ASCII label. Under the
        ...
        from tools.managed_tool_gateway import _read_hackit_provider_state

        hackit = _read_hackit_provider_state()
        assert hackit is not None
        assert hackit.get("agent_key") == "k"
        # The non-ASCII label round-trips intact.
        assert hackit.get("label") == "工作账号"
