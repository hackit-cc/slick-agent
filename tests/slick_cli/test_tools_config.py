...
from slick_cli.sophos_account import SophosPortalAccountInfo, SophosToolAccessInfo
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/SophosTechne/slick-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_first_install_sophos_auto_configures_video_gen(monkeypatch):
    """When a Sophos subscriber checks video_gen in the toolset checklist,
    apply_sophos_managed_defaults must write video_gen.provider and
    ...
    monkeypatch.setattr("slick_cli.sophos_subscription.managed_sophos_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "sophos"},
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: SophosPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
    ...
    assert config["video_gen"]["provider"] == "sophos"
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/SophosTechne/slick-agent/issues/6901
    """
    config = {
...
class TestImagegenBackendRegistry:
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        """Both Sophos Subscription and FAL.ai providers must carry the
...
# ---------------------------------------------------------------------------
# Inline Sophos Portal login gate on managed-provider selection
# ---------------------------------------------------------------------------
...
# Ready for every zero-env-var provider row, including logged-out Sophos
...
def test_visible_providers_reuses_logged_out_feature_snapshot(monkeypatch):
    ...
    assert any(
        provider.get("managed_sophos_feature") == "image_gen"
...
def test_visible_providers_reuses_pool_video_feature_snapshot(monkeypatch):
    ...
    account = SophosPortalAccountInfo(
        ...
        tool_access=SophosToolAccessInfo(
            enabled=True,
    ...
    assert not any(
        provider.get("managed_sophos_feature") == "video_gen"
        for provider in providers
    )
