"""Tests for normalized Sophos Portal account entitlement helpers."""
...
from slick_cli.sophos_account import (
    ...
    reset_sophos_portal_account_info_cache,
)
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    monkeypatch.setattr("slick_cli.auth.resolve_sophos_access_token", lambda: "fresh-token")
    monkeypatch.setattr("slick_cli.sophos_account._fetch_sophos_account_info", lambda *a, **kw: payload)

    info = get_sophos_portal_account_info(force_fresh=True)

    assert isinstance(info, SophosPortalAccountInfo)
...
def test_member_spend_cap_exceeded_message(monkeypatch):
    ...
    msg = format_sophos_portal_entitlement_message(info, capability="Sophos model access")
