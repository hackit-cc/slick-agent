"""Tests for slick_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import slick_cli.gui_uninstall as gu


def _make_agent(slick_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = slick_home / "slick-agent"
    (agent_root / "slick_cli").mkdir(parents=True)
    (agent_root / "slick_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(slick_home: Path) -> None:
    """Create the source-built GUI artifacts a `slick desktop` run produces."""
    desktop = slick_home / "slick-agent" / "apps" / "desktop"
    ...
    (slick_home / "slick-agent" / "node_modules").mkdir(parents=True)
    (slick_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(slick_home: Path) -> None:
    (slick_home / "config.yaml").write_text("x: 1\n")
    (slick_home / ".env").write_text("KEY=secret\n")
    (slick_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(slick_home)
    _make_gui_build(slick_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(slick_home)
    ...
    assert summary["slick_home"] == str(slick_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `slick desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    slick_home = tmp_path / ".slick"
    _make_agent(slick_home)
    icon = lde.icon_path(slick_home / "slick-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(slick_home)
    ...
    assert lde.icon_path(slick_home / "slick-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (slick_home / "slick-agent" / "slick_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".slick")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import slick_cli.uninstall as uninstall
