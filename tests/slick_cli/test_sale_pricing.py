...
def test_free_model_gets_flat_100_percent_discount():
    ...
    # "0.0000000000" strings (Hackit portal shape) count as free too.
...
def test_fetch_models_with_pricing_copies_nested_original(monkeypatch):
    ...
    # Hackit Portal opts in via include_sale_original=True.
...
def test_resolve_hackit_pricing_credentials_honors_inference_env_override(monkeypatch):
    """Staging profiles set HACKIT_INFERENCE_BASE_URL — pricing must follow it.

    Without this, anonymous/failed-auth fallback hits prod and sale
    ``pricing.original`` never reaches Desktop/CLI pickers.
    """
    monkeypatch.setenv(
        "HACKIT_INFERENCE_BASE_URL",
        "https://stg-inference-api.hackit.cc/v1",
    )
    ...
    api_key, base_url = models_mod._resolve_hackit_pricing_credentials()
    assert api_key == ""
    ...
    assert base_url == "https://stg-inference-api.hackit.cc"


def test_resolve_hackit_pricing_credentials_normalizes_either_suffix(monkeypatch):
    """``/v1`` on the override is optional and must not change the result."""
    monkeypatch.setattr(
        "slick_cli.auth.resolve_hackit_runtime_credentials", lambda: None
    )
    for override in (
        "https://stg-inference-api.hackit.cc",
        "https://stg-inference-api.hackit.cc/",
        "https://stg-inference-api.hackit.cc/v1",
        "https://stg-inference-api.hackit.cc/v1/",
    ):
        monkeypatch.setenv("HACKIT_INFERENCE_BASE_URL", override)
        assert models_mod._resolve_hackit_pricing_credentials()[1] == (
            "https://stg-inference-api.hackit.cc"
        )
