...
# ---------------------------------------------------------------------------
# Shared bundle collection + Hackit-S3 path
# ---------------------------------------------------------------------------
...
class TestBuildHackitBundle:
    def test_envelope_shape_and_gzip(self, slick_home):
        import gzip
        import json as _json

        from slick_cli.debug import build_hackit_bundle

        files = {"report": "hello", "agent.log": "log line"}
        blob = build_hackit_bundle(files, redact=True)
    ...
    def test_redacted_false_recorded(self):
        ...
        blob = build_hackit_bundle({"report": "x"}, redact=False)
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["redacted"] is False


class TestRunDebugShareHackit:
    def _args(self, **over):
        class _A:
            lines = 50
            expire = 7
    ...
    def test_hackit_success_prints_view_url(self, slick_home, capsys):
        from slick_cli.debug import run_debug_share
        ...
        with patch("slick_cli.dump.run_dump"), patch(
            "slick_cli.diagnostics_upload.share_to_hackit", return_value=res
        ) as share:
            run_debug_share(self._args())

        out = capsys.readouterr().out
        assert "Hackit-INTERNAL" in out
        assert "https://support.example.com/diagnostics/id-1" in out
        assert "2026-06-20T00:00:00Z" in out
        # The blob passed to share_to_hackit must be gzip bytes.
        blob = share.call_args[0][0]
        assert isinstance(blob, (bytes, bytearray)) and blob[:2] == b"\x1f\x8b"

    def test_hackit_failure_suggests_local(self, slick_home, capsys):
        from slick_cli.debug import run_debug_share

        with patch("slick_cli.dump.run_dump"), patch(
            "slick_cli.diagnostics_upload.share_to_hackit",
        ...
        assert "Hackit upload failed" in err
        assert "--local" in err

    def test_hackit_does_not_touch_pastebin(self, slick_home):
        from slick_cli.debug import run_debug_share
...
class TestDebugSlashCommand:
    """`/debug [hackit|local]` parsing in the CLI/TUI handler.
    ...
    def test_bare_debug_defaults_to_paste(self):
        ...
        assert c["hackit"] is False and c["local"] is False
    ...
    def test_word_parsing_is_case_insensitive(self):
        c = self._captured("/debug HACKIT")
        assert c["hackit"] is True
...
class TestShareConsentGate:
    ...
    def _args(self, **over):
        ...
        base = dict(lines=50, expire=7, local=False, hackit=False,
