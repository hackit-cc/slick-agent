"""Regression tests for #34107 — Docker UID/GID handling in ensure_slick_home.

When Slick runs in Docker with ``SLICK_UID=1000`` / ``SLICK_GID=911``,
the entrypoint chowns the top-level ``SLICK_HOME`` once at startup. But
...
The fix is a ``_chown_to_slick_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_slick_uid_gid
# ---------------------------------------------------------------------------


class TestResolveSlickUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("SLICK_UID", "1000")
        monkeypatch.setenv("SLICK_GID", "911")
        from slick_cli.config import _resolve_slick_uid_gid
...
class TestChownToSlickUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_slick_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SLICK_UID", raising=False)
        monkeypatch.delenv("SLICK_GID", raising=False)
        from slick_cli import config as cfg
