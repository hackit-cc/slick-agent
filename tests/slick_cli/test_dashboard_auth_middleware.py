...
class _UnreachableProvider(StubAuthProvider):
    ...
    Models the real-world bug — a self-hosted-OIDC session hits the ``hackit``
    provider first, which tries to reach Hackit Portal's JWKS; if that's
    unreachable ``hackit`` raises ProviderError. The gate must keep trying the
