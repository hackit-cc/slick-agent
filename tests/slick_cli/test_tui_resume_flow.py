...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_slick_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import slick_cli.oneshot as oneshot
        from slick_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """slick -z bypasses SlickCLI, but recall still needs SessionDB."""
    from slick_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "slick_state", mod("slick_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "slick_cli.config",
        mod("slick_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "slick_cli.models",
        mod("slick_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "slick_cli.tools_config",
        mod("slick_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["SLICK_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="hackit/slick-test", provider="hackit", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["SLICK_MODEL"] == "hackit/slick-test"
    assert env["SLICK_INFERENCE_MODEL"] == "hackit/slick-test"
    assert env["SLICK_TUI_PROVIDER"] == "hackit"
    assert env["SLICK_INFERENCE_PROVIDER"] == "hackit"
    assert env["SLICK_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["SLICK_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("slick-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_slick_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("SLICK_TUI_DIR", raising=False)
