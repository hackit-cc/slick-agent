"""Tests for the `slick memory reset` CLI command.
...
- Profile-scoped reset (uses SLICK_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake SLICK_HOME with memory files."""
    slick_home = tmp_path / ".slick"
    memories = slick_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("SLICK_HOME", str(slick_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nSlick repo is at ~/.slick/slick-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `slick memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        slick_home = tmp_path / ".slick"
        (slick_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("SLICK_HOME", str(slick_home))
