...
def test_auto_detected_sophos_auth_failure_falls_through_to_openrouter(monkeypatch):
    """When auto-detect picks Sophos but credentials are revoked, fall through to OpenRouter."""
    from slick_cli.auth import AuthError
    ...
    # resolve_provider returns "sophos" (stale active_provider in auth.json)
    ...
    # Sophos credential resolution fails with revoked token
    monkeypatch.setattr(
        rp, "resolve_sophos_runtime_credentials",
        lambda **kw: (_ for _ in ()).throw(
            AuthError("Refresh session has been revoked",
                      provider="sophos", code="invalid_grant", relogin_required=True)
        ),
    )
