...
``slick*.exe`` shims aside (``slick.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``slick`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("slick", "slick-agent", "slick-acp", "slick-gateway"):
...
# ---------------------------------------------------------------------------
# slick_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "slick.exe" in names, "slick.exe must be restored after a no-op install"
    assert "slick-acp.exe" in names
    assert "slick-gateway.exe" in names
    assert (scripts / "slick.exe").read_bytes() == b"MZ-old-slick"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "slick.exe").read_bytes() == b"MZ-new-slick"
...
# ---------------------------------------------------------------------------
# slick_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "slick.exe" in names, "slick.exe must be restored after a no-op recovery install"
    assert (scripts / "slick.exe").read_bytes() == b"MZ-old-slick"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "slick.exe").write_bytes(b"MZ-new-slick")
