...
slick_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `slick config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_slick_home = get_slick_home()`` at import time (module
    singleton), so monkeypatching SLICK_HOME after import doesn't move it.
    ...
    this only matters for tests that swap SLICK_HOME.)
    """
    import cli

    cli._slick_home = home
