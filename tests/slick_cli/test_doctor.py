...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull hackit/slick-agent:latest" in hint
...
class TestDoctorMemoryProviderSection:
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        try:
            ...
            monkeypatch.setattr(_auth_mod, "get_hackit_auth_status_local", lambda: {})
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    try:
        from slick_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_hackit_auth_status", lambda: {})
        monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
        monkeypatch.setattr(_auth_mod, "get_xai_oauth_auth_status", lambda: {})
    except Exception:
        pass
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(_auth_mod, "get_hackit_auth_status_local", lambda: {"logged_in": True})
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the Hackit / Codex / Gemini /
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        from slick_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_hackit_auth_status_local", lambda: {"logged_in": False})
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """Hackit / Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".slick"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "Hackit Portal auth" in out
...
class TestMacOSTCCGrants:
    ...
    def test_warns_on_cdhash_pinned_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_mod,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.hackit.slick" and cdhash H"97e692f3890f781fa0ad5ad6cb9d769cfaf42628"',
        )
        doctor_mod.check_macos_tcc_grants()
        out = capsys.readouterr().out
    ...
    def test_ok_and_repair_info_on_identifier_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_mod,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.hackit.slick"',
        )
        doctor_mod.check_macos_tcc_grants()
        out = capsys.readouterr().out
        ...
        assert "tccutil reset ScreenCapture com.hackit.slick" in out
    ...
    def test_ok_on_certificate_anchored_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_mod,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.hackit.slick" and certificate root = H"aabbcc"',
        )
        doctor_mod.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC signing identity is stable" in out
