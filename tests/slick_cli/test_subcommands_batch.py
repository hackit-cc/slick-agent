...
def test_config_get_unset_subcommands_parse():
    """`slick config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="slick")
...
# ── deprecated `slick login` fails gracefully, not with argparse error ────
#
# `slick login` is a removed command; its handler (`login_command` in
# `slick_cli/auth.py`) prints a deprecation notice pointing at `slick auth` /
# `slick model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `slick --help`.
