...
These guard the contract:
  ...
  * OpenRouter and Sophos Portal are NEVER merged — they keep their curated
    (OpenRouter) or live-Portal (Sophos) semantics.
...
appear in ``/model`` without a Slick release.
"""
...
class TestProviderModelIdsPreferred:
    ...
    def test_k3_live_discovery_is_scoped_to_kimi_coding_endpoint(self):
        ...
        with patch("slick_cli.urllib_security.open_credentialed_url", side_effect=fake_open):
            with patch(
    ...
    def test_kimi_setup_flow_uses_same_coding_plan_catalog(self):
        ...
        from slick_cli.model_setup_flows import _model_flow_kimi
        ...
        with (
            patch("slick_cli.main._prompt_api_key", return_value=("sk-kimi-test", False)),
            patch("slick_cli.auth._prompt_model_selection", side_effect=fake_select),
            patch("slick_cli.config.get_env_value", return_value=""),
...
class TestOpenRouterAndSophosUnchanged:
    """Per Teknium: openrouter and sophos are NEVER merged with models.dev."""


    def test_openrouter_does_not_call_merge(self):
        """openrouter takes its own live path — merge helper must NOT run."""
        with patch(
            "slick_cli.models._merge_with_models_dev",
