...
def container_env(tmp_path, monkeypatch):
    """Set up a fake SLICK_HOME with .container-mode file."""
    slick_home = tmp_path / ".slick"
    slick_home.mkdir()
    monkeypatch.setenv("SLICK_HOME", str(slick_home))
    monkeypatch.delenv("SLICK_DEV", raising=False)

    container_mode = slick_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=slick-agent\n"
        "exec_user=slick\n"
        "slick_bin=/data/current-package/bin/slick\n"
    )
    return slick_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("slick_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "slick-agent"
    assert info["exec_user"] == "slick"
    assert info["slick_bin"] == "/data/current-package/bin/slick"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from slick_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "slick"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "slick-agent" in cmd
    assert "/data/current-package/bin/slick" in cmd
    assert "chat" in cmd
