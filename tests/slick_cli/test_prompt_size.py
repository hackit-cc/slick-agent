"""Tests for the ``slick prompt-size`` diagnostic (issue #34667)."""
...
from slick_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(slick_home, memory_text="", user_text=""):
    mem_dir = slick_home / "memories"
...
def _seed_skill(slick_home, name, description):
    skill_dir = slick_home / "skills" / "demo" / name
...
def test_runs_offline_without_credentials(isolated_home, monkeypatch):
    ...
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "HACKIT_API_KEY",
                "ANTHROPIC_API_KEY"):
