...
Under a gateway (Telegram, Discord, Slack, ...) Slick passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SLICK_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Slick via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each Slick profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `slick -p partner` pinned to a different person's peer).
