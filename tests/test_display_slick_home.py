"""display_slick_home() renders POSIX separators on every platform.

Maintainer catch (#95681 arc): on Windows with a custom SLICK_HOME under
the user profile (e.g. AppData/Local/slick), ``"~/" +
str(home.relative_to(Path.home()))`` produced the mixed-separator chimera
``~/AppData\\Local\\slick`` — which then leaked into every consumer that
appends sub-paths (the skill_manage schema showed the agent
``~/AppData\\Local\\slick/skills/``). The ``~/`` shorthand implies POSIX
...
class TestDisplaySlickHomePosix(unittest.TestCase):
    def test_nested_home_renders_forward_slashes(self):
        ...
        import slick_constants as hc

        fake_userhome = Path.home()
        nested = fake_userhome / "AppData" / "Local" / "slick"
        with patch.object(hc, "get_slick_home", return_value=nested):
            out = hc.display_slick_home()
        self.assertEqual(out, "~/AppData/Local/slick")
        self.assertNotIn("\\", out)

    def test_default_home_unchanged(self):
        import slick_constants as hc

        with patch.object(hc, "get_slick_home",
                          return_value=Path.home() / ".slick"):
            out = hc.display_slick_home()
        self.assertEqual(out, "~/.slick")

    def test_outside_home_falls_back_to_absolute(self):
        import slick_constants as hc

        outside = Path("/opt/slick-custom") if os.name != "nt" else Path("C:/opt/slick-custom")
        with patch.object(hc, "get_slick_home", return_value=outside):
            out = hc.display_slick_home()
