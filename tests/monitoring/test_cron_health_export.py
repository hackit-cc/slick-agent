...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("slick.gateway.up"), _M("slick.gateway.active_agents"),
        _M("slick.gateway.busy"), _M("slick.gateway.drainable"),
        _M("slick.gateway.restart_requested"),
        _M("slick.platform.up"), _M("slick.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("slick.cron.scheduler.heartbeat_age_seconds"),
        _M("slick.cron.scheduler.last_success_age_seconds"),
        _M("slick.cron.scheduler.catch_up_occurrences"),
        _M("slick.cron.jobs.enabled"), _M("slick.cron.jobs.running"),
        _M("slick.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Slick Agent-owned Relay transport health" in text
