...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.slick/slick-agent/utils.py)
...
2. ``slick update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``slick_cli/main.py`` (the ``slick update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``slick_constants`` after the
