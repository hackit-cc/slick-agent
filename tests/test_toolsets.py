...
class TestToolsetConsistency:
    ...
    def test_slick_platforms_share_core_tools(self):
        """All slick-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on slick-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["slick-cli", "slick-telegram", "slick-discord", "slick-whatsapp", "slick-slack", "slick-signal", "slick-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_slick_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("slick-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("slick-cli")
        second = resolve_toolset("slick-cli")
        ...
        assert (
            "slick-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("slick-cli", include_registry=False)
        second = resolve_toolset("slick-cli", include_registry=False)
        assert first == second
