...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/slick/.overview.md") == "viking://user/slick"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/slick/memories/profile.md") == "viking://user/slick/memories/profile.md"
...
class TestOpenVikingSkillQuerySafety:
    ...
    def test_skill_markers_match_slick_scaffolding(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SLICK_BUNDLES_DIR", str(bundles_dir))
...
class TestOpenVikingConfigSchema:
    ...
    def test_recall_config_reads_from_config_yaml(self, monkeypatch, tmp_path):
        ...
        # Populate config.yaml in the temp SLICK_HOME
        slick_home = tmp_path / "slick_test"
        slick_home.mkdir(exist_ok=True)
        config_yaml = slick_home / "config.yaml"
...
class TestOpenVikingRead:
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    (("uri", "viking://user/slick"),),
                ): {"result": {"content": "overview text"}},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/slick/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/slick/.overview.md"
        assert result["resolved_uri"] == "viking://user/slick"
    ...
    def test_read_accepts_uri_batch_and_caps_batch_full_content(self):
        provider = OpenVikingMemoryProvider()
        uris = [
            "viking://user/slick/memories/a.md",
            "viking://user/slick/memories/b.md",
            "viking://user/slick/memories/c.md",
            "viking://user/slick/memories/d.md",
        ]
        provider._client = FakeVikingClient(
            {
                (
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/slick/.overview.md", "level": "overview"})
            assert False, "Expected summary endpoint error to be raised"
        except RuntimeError:
            pass

        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/slick"}),
        ]
...
class TestOpenVikingAutoRecallPrefetch:
    def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch):
                    ...
                    if payload.get("context_type") == "memory":
                        self._send_json({
                            "result": {
                                "memories": [
                                    {
                                        "uri": "viking://user/peers/slick/memories/e2e-full.md",
                                        "score": 0.9,
                                        "level": 2,
                                        "category": "events",
        ...
        monkeypatch.setenv("OPENVIKING_AGENT", "slick")
        ...
        assert records["reads"] == ["viking://user/peers/slick/memories/e2e-full.md"]
        ...
        assert all(headers.get("x-openviking-actor-peer") == "slick" for headers in normalized_headers)
        assert all(
            headers.get("user-agent") == f"openviking-memory-slick/{_SLICK_VERSION}"
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/slick/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/slick/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/slick"}))

        assert result["path"] == "viking://user/slick"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/slick/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/slick/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/slick"},
...
class TestEnsureClientReloadsEnv:
    ...
    def test_ensure_client_rebuilds_when_api_key_changes(self, monkeypatch):
        ...
        class _StubClient:
            def __init__(self, endpoint, api_key, account="", user="", agent="slick"):
    ...
    def test_rebuilt_client_resolves_its_own_user_space(self, monkeypatch):
        ...
        assert alice_uri.startswith("viking://user/alice/peers/slick/")
        assert bob_uri.startswith("viking://user/bob/peers/slick/")
    ...
    def test_handle_tool_call_reconnects_after_startup_health_failure(self, monkeypatch):
        ...
        assert instances[1].posts[0][1]["uri"].startswith(
            "viking://user/default/peers/slick/memories/"
        )
...
class TestUnavailableWarningsPromiseRetry:
    ...
    reads as "it never recovers" and sends people restarting slick for
    nothing (#5721).
    """

    @staticmethod
    def _assert_promises_retry(message: str) -> None:
        assert "for this Slick run" not in message, message
