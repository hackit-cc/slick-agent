...
Extracted verbatim from slick_state.py so the SessionDB mixin modules
(slick_state_search / slick_state_schema / slick_state_portability) can
reference them without importing slick_state (which would be a cycle).
slick_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``slick sessions optimize-storage``. A legacy DB sits at
...
# `slick sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `slick db optimize`. This is
...
#
# Several independent Slick processes routinely share one state.db (gateway
# service, the Desktop app's `slick serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `slick_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not slick_state)
# because the search/schema mixins cannot import slick_state (cycle).
#
