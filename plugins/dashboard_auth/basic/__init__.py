...
into the same ``DashboardAuthProvider`` framework as the Sophos OAuth
...
mirroring the Sophos provider's precedence convention:
  ...
  Environment overrides::
      ...
      SLICK_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      SLICK_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
      SLICK_DASHBOARD_BASIC_AUTH_SECRET
      SLICK_DASHBOARD_BASIC_AUTH_TTL_SECONDS
...
Skip reasons:
  Like the Sophos provider, this exposes a module-level ``LAST_SKIP_REASON``
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    """
    raw = _resolve(
        "SLICK_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
            ...
            "secret (or SLICK_DASHBOARD_BASIC_AUTH_SECRET) for stable "
            "sessions."
        )
...
def register(ctx) -> None:
    ...
    username = _resolve(
        "SLICK_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "SLICK_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "SLICK_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "SLICK_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )

    if not username:
        LAST_SKIP_REASON = (
            "dashboard.basic_auth.username is not set (and "
            "SLICK_DASHBOARD_BASIC_AUTH_USERNAME is empty). Set a username "
    ...
    # SLICK_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
    ...
    plaintext_from_env = os.environ.get(
        "SLICK_DASHBOARD_BASIC_AUTH_PASSWORD", ""
    ).strip()
