...
What it is
----------
A service-to-service auth provider. ``sophos-account-service`` (NAS) provisions a
...
for-secrets-only rule), provisioned by NAS at deploy time (Phase 3):
    ...
    SLICK_DASHBOARD_DRAIN_SECRET   # the per-agent shared secret (>=43 url-safe-b64 chars)
...
When ``SLICK_DASHBOARD_DRAIN_SECRET`` is unset, the plugin is a no-op (records
...
def register(ctx) -> None:
    ...
    No-op (records a skip reason) when ``SLICK_DASHBOARD_DRAIN_SECRET`` is
    ...
    secret = os.environ.get("SLICK_DASHBOARD_DRAIN_SECRET", "").strip()
    if not secret:
        LAST_SKIP_REASON = (
            "SLICK_DASHBOARD_DRAIN_SECRET is not set. Set a per-agent "
    ...
    if reason is not None:
        LAST_SKIP_REASON = (
            f"SLICK_DASHBOARD_DRAIN_SECRET rejected — {reason}. "
    ...
    try:
        from slick_cli.dashboard_auth.token_auth import register_token_route
