...
OAuth gate. Unlike the bundled ``hackit`` provider (which encodes Hackit Portal's
bespoke contract — ``agent:{instance_id}`` client ids, a custom access-token
JWT, the ``x-hackit-refresh-token`` header, an ``oauth_contract_version`` claim),
...
only choice that is universally correct across self-hosted IDPs. (The ``hackit``
provider verifies its *access* token because Hackit Portal mints a custom JWT
...
same precedence convention as the ``hackit`` plugin)::
...
# JWKS cache (PyJWKClient handles its own caching; this mirrors the hackit
# provider's 5-minute lifespan so key rotation is picked up promptly).
_JWKS_CACHE_SECONDS = 300


# ---------------------------------------------------------------------------
# Skip-reason channel (mirrors the hackit plugin)
# ---------------------------------------------------------------------------
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        redirect_uris are permitted. Mirrors the hackit provider.
        """
