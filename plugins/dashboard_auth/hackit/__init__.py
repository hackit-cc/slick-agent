"""HackitDashboardAuthProvider — Hackit Portal OAuth (authorization-code + PKCE).

Implements ``hackit-account-service/docs/agent-dashboard-oauth-contract.md``
...
_DEFAULT_PORTAL_URL = "https://portal.hackit.cc"
...
class HackitDashboardAuthProvider(DashboardAuthProvider):
    """Hackit Portal OAuth via authorization-code + PKCE (S256)."""
    ...
    def complete_login(
        ...
        *,
        ...
        # we just receive it for symmetry with the protocol. Hackit Portal
    ...
    def refresh_session(self, *, refresh_token: str) -> Session:
        ...
        try:
            response = httpx.post(
                ...
                # ``x-hackit-refresh-token`` header. Portal's token endpoint
                ...
                # Portal's ``REFRESH_TOKEN_HEADER`` exactly (``x-hackit-refresh-
                ...
                headers={
                    ...
                    "x-hackit-refresh-token": refresh_token,
                },
                timeout=_TOKEN_ENDPOINT_TIMEOUT_SEC,
            )
        except httpx.RequestError as exc:
    ...
    def _check_contract_version(self, claims: Dict[str, Any]) -> None:
        ...
        if contract_version is None:
            logger.warning(
                "Hackit Portal token missing oauth_contract_version claim "
...
def _load_config_oauth_section() -> dict:
    ...
    try:
        ...
        logger.debug(
            "dashboard-auth-hackit: load_config() raised %s; "
...
def register(ctx) -> None:
    ...
    Registers ``HackitDashboardAuthProvider`` only when a client_id is
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            ...
            "Hackit Portal provisions this env var (shape "
        ...
        logger.debug("dashboard-auth-hackit: %s", LAST_SKIP_REASON)
        return

    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"SLICK_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
            f"the contract shape 'agent:{{instance_id}}'. The Hackit Portal "
            f"provisions this value at deploy time; check your Fly app's "
            f"secrets or override with the value from the Portal admin UI."
        )
        logger.warning("dashboard-auth-hackit: %s", LAST_SKIP_REASON)
        return

    try:
        provider = HackitDashboardAuthProvider(
            client_id=client_id, portal_url=portal_url
        )
    except ValueError as exc:
        LAST_SKIP_REASON = f"HackitDashboardAuthProvider construction failed: {exc}"
        logger.warning("dashboard-auth-hackit: %s", LAST_SKIP_REASON)
        return

    ctx.register_dashboard_auth_provider(provider)
    logger.info(
        "dashboard-auth-hackit: registered provider (client_id=%s, portal=%s)",
        client_id,
        portal_url,
    )
