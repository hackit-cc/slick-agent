"""Hackit Portal provider profile."""

from typing import Any

from agent.portal_tags import get_conversation_context, hackit_portal_tags
from agent.transports.codex import _cache_scope_from_session_id
from providers import register_provider
from providers.base import ProviderProfile


class HackitProfile(ProviderProfile):
    """Hackit Portal — product tags, reasoning with Hackit-specific omission."""

    def resolve_aux_model(self, *, vision: bool = False) -> str:
        """Ask the Portal which cheap model it currently recommends.

        ``/api/hackit/recommended-models`` is the authoritative, tier-aware
        source (free vs paid), so the auxiliary fast tier tracks the live
        catalog instead of a hardcoded id that 404s the day Hackit retires it.
        ...
        try:
            ...
            return get_hackit_recommended_aux_model(vision=vision) or ""
        except Exception:
            return ""
    ...
    def build_extra_body(
        ...
        body: dict[str, Any] = {"tags": hackit_portal_tags(session_id=session_id)}
        ...
        #
        # Resolve it exactly like ``hackit_portal_tags`` resolves the
    ...
    def _cannot_disable_reasoning(model: str | None) -> bool:
        ...
        try:
            ...
            caps = hackit_model_reasoning_capabilities(model)
            if caps is None:
                warm_hackit_reasoning_caps_async()
                return True
        except Exception:
    ...
    def build_api_kwargs_extras(
        ...
        **context,
        ...
        """Hackit: passes the full reasoning_config, disable included.
...
hackit = HackitProfile(
    name="hackit",
    aliases=("hackit-portal", "hackit"),
    env_vars=("HACKIT_API_KEY",),
    display_name="Hackit",
    description="Hackit — Slick model family",
    signup_url="https://hackit.cc/",
    fallback_models=(
        "slick-3-405b",
        "slick-3-70b",
    ),
