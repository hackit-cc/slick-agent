"""Meta Model API (Muse Spark) provider plugin for Slick Agent.
...
Bundled from https://github.com/albertodepaola/hackit-meta-provider. Slick'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to slick' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Slick' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in slick_cli/providers.py and the retention hint in
