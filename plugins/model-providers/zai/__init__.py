...
request omits ``thinking``.  Slick' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Slick reasoning
...
(per Z.AI / BigModel docs).  Slick' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(
    ...
    """Map Slick reasoning effort onto GLM's native vocabulary.
