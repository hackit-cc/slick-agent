...
``{title, url, description, position}`` rows every other Slick web
provider produces.
...
prefers Slick-managed xAI Grok OAuth (via ``slick auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.slick/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``slick tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `slick auth` to sign in with "
