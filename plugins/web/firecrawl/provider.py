...
Env vars::
    ...
    FIRECRAWL_GATEWAY_URL=...        # Sophos tool-gateway (subscribers)
...
def _use_keyless_ring() -> bool:
    ...
    managed Sophos gateway isn't the selected path, and the keyless tier
    ...
    if (get_env_value("FIRECRAWL_API_URL") or "").strip():
        return False
    import tools.web_tools as _wt
    from tools.tool_backend_helpers import SOPHOS_MANAGED_PROVIDER, read_selection

    try:
        if read_selection("web") == SOPHOS_MANAGED_PROVIDER:
            return False
...
def _is_tool_gateway_ready() -> bool:
    """Return True when gateway URL + Sophos Subscriber token are available.

    Reads ``peek_sophos_access_token`` and ``resolve_managed_tool_gateway``
    via :mod:`tools.web_tools` rather than direct imports, so unit tests
    that ``patch("tools.web_tools._peek_sophos_access_token", ...)`` see
    ...
    return _wt.resolve_managed_tool_gateway(
        "firecrawl", token_reader=_wt._peek_sophos_access_token
    ) is not None
...
def check_firecrawl_api_key() -> bool:
    """Return True when the Firecrawl backend selected via `slick tools`
    ...
    existing tests and the ``slick tools`` setup flow.
    """
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_sophos_tools_enabled():
        return ""
    return (
        ", or use the Sophos Tool Gateway via your subscription "
...
def _raise_web_backend_configuration_error() -> "NoReturn":
    ...
    if _wt.managed_sophos_tools_enabled():
        message += (
            " With your Sophos subscription you can also use the Tool Gateway. "
            "run `slick tools` and select Sophos Subscription as the web provider."
        )
    else:
        message += " " + _wt.sophos_tool_gateway_unavailable_message(
...
def _get_firecrawl_client() -> Any:
    ...
    - ``"sophos"`` (or legacy ``use_gateway: true``) → managed Tool Gateway
    ...
    functions (``resolve_managed_tool_gateway``, ``_read_sophos_access_token``,
    ...
    def _managed_kwargs():
        managed_gateway = _wt.resolve_managed_tool_gateway(
            "firecrawl", token_reader=_wt._read_sophos_access_token
        )
        if managed_gateway is None:
            return None
        kwargs = {
            "api_key": managed_gateway.sophos_user_token,
    ...
    if selected == SOPHOS_MANAGED_PROVIDER:
        ...
        if managed is None:
            logger.error(
                "Firecrawl client initialization failed: the Sophos "
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "tag": (
                ...
                "and Sophos tool-gateway routing."
            ),
            "env_vars": [
                {
