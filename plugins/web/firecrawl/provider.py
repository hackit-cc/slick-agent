...
Env vars::
    ...
    FIRECRAWL_GATEWAY_URL=...        # Hackit tool-gateway (subscribers)
...
def _use_keyless_ring() -> bool:
    ...
    managed Hackit gateway isn't the selected path, and the keyless tier
    ...
    if (get_env_value("FIRECRAWL_API_URL") or "").strip():
        return False
    import tools.web_tools as _wt
    from tools.tool_backend_helpers import HACKIT_MANAGED_PROVIDER, read_selection

    try:
        if read_selection("web") == HACKIT_MANAGED_PROVIDER:
            return False
...
def _is_tool_gateway_ready() -> bool:
    """Return True when gateway URL + Hackit Subscriber token are available.

    Reads ``peek_hackit_access_token`` and ``resolve_managed_tool_gateway``
    via :mod:`tools.web_tools` rather than direct imports, so unit tests
    that ``patch("tools.web_tools._peek_hackit_access_token", ...)`` see
    ...
    return _wt.resolve_managed_tool_gateway(
        "firecrawl", token_reader=_wt._peek_hackit_access_token
    ) is not None
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_hackit_tools_enabled():
        return ""
    return (
        ", or use the Hackit Tool Gateway via your subscription "
...
def _raise_web_backend_configuration_error() -> "NoReturn":
    ...
    if _wt.managed_hackit_tools_enabled():
        message += (
            " With your Hackit subscription you can also use the Tool Gateway. "
            "run `slick tools` and select Hackit Subscription as the web provider."
        )
    else:
        message += " " + _wt.hackit_tool_gateway_unavailable_message(
...
def _get_firecrawl_client() -> Any:
    ...
    - ``"hackit"`` (or legacy ``use_gateway: true``) → managed Tool Gateway
    ...
    functions (``resolve_managed_tool_gateway``, ``_read_hackit_access_token``,
    ...
    def _managed_kwargs():
        managed_gateway = _wt.resolve_managed_tool_gateway(
            "firecrawl", token_reader=_wt._read_hackit_access_token
        )
        if managed_gateway is None:
            return None
        kwargs = {
            "api_key": managed_gateway.hackit_user_token,
    ...
    if selected == HACKIT_MANAGED_PROVIDER:
        ...
        if managed is None:
            logger.error(
                "Firecrawl client initialization failed: the Hackit "
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "tag": (
                ...
                "and Hackit tool-gateway routing."
            ),
            "env_vars": [
                {
