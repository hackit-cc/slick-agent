...
Let the slick agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where slick runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `slick meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `slick meet` CLI |
...
| `cli.py` | `slick meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$SLICK_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `slick meet node {run,list,approve,remove,status,ping}` |
...
slick meet install                                      # pip + Chromium
slick meet setup                                        # preflight
slick meet auth                                         # optional
slick meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
slick meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.slick/.env
slick meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
slick meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
slick meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.slick/.env
slick meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, slick will **not** switch your system audio input automatically — the
...
slick plugins enable google_meet
slick meet node run --display-name my-mac --host 0.0.0.0 --port 18789
