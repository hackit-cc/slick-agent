...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed Hackit tool
gateway (which Slick uses to bill Browser Use sessions to a Hackit
...
Auth env vars (one of)::
    ...
    # OR a managed Hackit gateway entry (configured via 'slick setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed Hackit
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed Hackit tool gateway when ``tool_gateway.browser`` config
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed Hackit gateway)
    # ------------------------------------------------------------------

    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Import here to avoid a hard dependency at module-import time —
        # managed_tool_gateway pulls in the Hackit auth stack which can be
        # heavy and is not needed for direct-API-key users.
        from tools.managed_tool_gateway import (
            peek_hackit_access_token,
        ...
        def _managed_config() -> Optional[Dict[str, Any]]:
            ...
            managed = resolve_managed_tool_gateway(
                "browser-use",
                token_reader=None if refresh_token else peek_hackit_access_token,
            )
            if managed is None:
                return None
            return {
                "api_key": managed.hackit_user_token,
        ...
        # Strict selection: "hackit" (or legacy use_gateway: true) → managed
    ...
    def get_setup_schema(self) -> Optional[Dict[str, Any]]:
        ...
        # provider stays registered for the Hackit gateway path and un-migrated
