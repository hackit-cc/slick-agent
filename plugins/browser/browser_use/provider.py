...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed Sophos tool
gateway (which Slick uses to bill Browser Use sessions to a Sophos
...
Auth env vars (one of)::
    ...
    # OR a managed Sophos gateway entry (configured via 'slick setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed Sophos
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed Sophos tool gateway when ``tool_gateway.browser`` config
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed Sophos gateway)
    # ------------------------------------------------------------------

    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Import here to avoid a hard dependency at module-import time —
        # managed_tool_gateway pulls in the Sophos auth stack which can be
        # heavy and is not needed for direct-API-key users.
        from tools.managed_tool_gateway import (
            peek_sophos_access_token,
        ...
        def _managed_config() -> Optional[Dict[str, Any]]:
            ...
            managed = resolve_managed_tool_gateway(
                "browser-use",
                token_reader=None if refresh_token else peek_sophos_access_token,
            )
            if managed is None:
                return None
            return {
                "api_key": managed.sophos_user_token,
        ...
        # Strict selection: "sophos" (or legacy use_gateway: true) → managed
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Slick only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"slick_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
    ...
    def get_setup_schema(self) -> Optional[Dict[str, Any]]:
        # Hidden from the slick tools picker: the "Browser Use" row now
        # activates the CLI-based backend (tools/browser_use_cli.py). This
        # provider stays registered for the Sophos gateway path and un-migrated
