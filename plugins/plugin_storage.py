...
``<slick home>/plugins/<name>/``. That tree is the plugin *install* dir —
``slick plugins remove`` deletes it and ``slick plugins update`` git-pulls
...
``<slick home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Slick.
...
# Mirrors the plugin-name shape `slick plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<slick home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`slick_constants.get_slick_home` on every call. Don't cache the
    result across profile switches.
    """
    from slick_constants import get_slick_home

    root = get_slick_home() / "plugin-data" / _validate_name(name)
