...
Small Node helper that bridges Slick Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Slick is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Slick-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Slick can receive inbound messages
...
The Slick plugin's `slick photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Slick is Python, so replies go through this sidecar
