// Slick Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Slick): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Slick -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Slick-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `slick photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Slick can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "slick-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Slick'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Slick allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Slick, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-slick-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Slick supplied them so a known-good
