...
under an immutable ``/opt/slick`` (read-only for the slick user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$SLICK_HOME/photon/sidecar`` (the durable data volume,
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_slick_home() / "photon" / "sidecar"
    try:
