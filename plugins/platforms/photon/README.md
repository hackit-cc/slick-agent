...
This plugin connects Slick Agent to iMessage (and other Spectrum
...
TypeScript-only, Slick runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Slick channel,
...
Run `slick photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.slick/.env` (the same place every other
...
Management metadata lives in `~/.slick/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `slick photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Slick
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Slick coalesces
...
- **Native polls are supported.** Slick posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `slick send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<slick-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `slick photon status`, a DM and a group roundtrip,
