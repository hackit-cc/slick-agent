"""
``slick photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Slick
...
# Writable sidecar runtime dir (mirrors to SLICK_HOME on immutable
...
# wiring imports this module for `slick --help`).
...
# when called later (gateway start, slick status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `slick photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Slick Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``slick photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Slick Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.slick/.env,
    ...
    try:
        from slick_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  slick gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `slick photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`slick photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.slick/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `slick photon telemetry on` / `slick photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.slick/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  slick gateway restart")
    return 0
...
#
# `slick gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``slick photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `slick gateway setup` wizard."""
