# A2A — Agent-to-Agent protocol for Slick
...
peer (another Slick, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
slick gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Slick serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.slick/a2a_audit.jsonl`.
- Conversations persist to `~/.slick/a2a_conversations/` — they survive context
