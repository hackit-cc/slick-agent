"""
LINE Messaging API platform adapter for Slick Agent.
...
to Slick Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SLICK_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            slick_home = Path(get_slick_home()).resolve()
        except Exception:
            slick_home = Path.home().joinpath(".slick").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``slick status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``slick status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``slick setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.slick/.env`` via ``slick_cli.config``.
    """
    print()
    ...
    try:
        from slick_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("slick_cli.config not available; set LINE_* vars manually in ~/.slick/.env")
        return
