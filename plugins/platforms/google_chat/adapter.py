...
# ``model_tools`` import time, so ``slick status``, ``slick chat``, etc.
...
# ``slick_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Slick is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a slick MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "slick-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SLICK_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from slick_constants import get_slick_home as _get_slick_home
            _slick_home = _get_slick_home()
        except (ModuleNotFoundError, ImportError):
            _slick_home = _Path.home() / ".slick"
        self._thread_count_store = _ThreadCountStore(
            _slick_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Slick is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SLICK_HOME", str(_Path.home() / ".slick"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SLICK_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a slick MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Slick is thinking…' marker message.
        ...
        "Slick is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Slick is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Slick is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Slick is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``slick setup``.

    The setup wizard at ``slick_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.slick/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. slick-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.slick/.env")
    print_info("Restart the gateway: slick gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``slick cron`` running as a
    separate process from ``slick gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Slick plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `slick setup` to install Google Chat support.",
        ...
        # the "Slick is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Slick is thinking…' marker message "
