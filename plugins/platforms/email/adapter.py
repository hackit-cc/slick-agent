"""
Email platform adapter for the Slick gateway.

Allows users to interact with Slick by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "slick-agent" "version" "{_slick_version}" '
            '"vendor" "SophosTechne" '
            '"support-email" "noreply@sophostechne.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `slick config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `slick gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Slick Agent")
        ...
        msg_id = f"<slick-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# slick_cli/gateway.py, and the _send_email dispatch in
