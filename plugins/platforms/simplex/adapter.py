"""SimpleX Chat platform adapter (Slick plugin).
...
This adapter ships as a Slick platform plugin under
``plugins/platforms/simplex/``. The Slick plugin loader scans the
...
Optional environment variables:
    ...
    SLICK_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``slick setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Slick — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("SLICK_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``slick setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.slick/.env`` via
    ``slick_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "slick_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.slick/.env"
        )
        return
