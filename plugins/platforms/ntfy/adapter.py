"""ntfy platform adapter (Slick plugin).
...
``plugins/platforms/ntfy/``. The Slick plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "slick-in"              # subscribe topic (incoming)
          publish_topic: "slick-out"     # optional — defaults to topic
...
_ECHO_TAG = "slick-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``slick cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Slick dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `slick gateway status` without
