...
Chronos (the Greek god of time, alongside Slick) is the first non-default
...
NAS's ``agent-cron`` endpoints with the agent's existing Sophos token.
...
class ChronosCronScheduler(CronScheduler):
    ...
    def is_available(self) -> bool:
        ...
        callback URL (for NAS→agent fires), and a usable Sophos token (the agent
        ...
        if not (_cfg("cron", "chronos", "portal_url") and _cfg("cron", "chronos", "callback_url")):
            return False
        return self._have_sophos_token()

    def _have_sophos_token(self) -> bool:
        """True if the agent has a Sophos Portal login (no network call).

        Checks the stored auth state for a Sophos access token — does NOT refresh
