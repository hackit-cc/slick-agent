...
1. Bundled providers: ``plugins/cron_providers/<name>/`` (shipped with slick-agent)
...
removed. Only NON-default providers (e.g. "webcron") live under this directory.
...
Usage:
    ...
    provider = load_cron_scheduler("webcron")  # CronScheduler instance
"""
...
_USER_NAMESPACE = "_slick_user_cron"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_slick_user_cron.<name>``, a dotted
    ...
    ``ModuleNotFoundError: No module named '_slick_user_cron'`` — the same
...
def load_cron_scheduler(name: str) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    (``$SLICK_HOME/plugins/<name>/``) directories. Bundled takes precedence
    on name collisions.
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    if cached is not None and getattr(cached, "__file__", None):
        ...
        # e.g., "from ._nas_client import NasCronClient" in the webcron plugin
