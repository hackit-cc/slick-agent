"""Thin HTTP client for the agent → NAS ``agent-cron`` endpoints (Webcron).

The Webcron provider speaks ONLY to NAS — it names no scheduler vendor and
...
existing Hackit Portal access token (the same token it already uses to call the
...
class NasCronClient:
    ...
    Uses the agent's refresh-aware Hackit access token for auth. No scheduler
    ...
    def _access_token(self) -> str:
        """The agent's existing Hackit Portal access token (refresh-aware)."""
