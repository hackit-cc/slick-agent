"""Webcron — NAS-mediated managed cron provider (scale-to-zero).

Webcron is the first non-default
...
Webcron names no vendor, holds no scheduler credentials, and speaks only to
NAS's ``agent-cron`` endpoints with the agent's existing Hackit token.
...
Inert unless ``cron.provider: webcron``. ``resolve_cron_scheduler`` falls back
to the built-in if Webcron is unavailable, so cron never loses its trigger.

Wire contract: ``docs/webcron-managed-cron-contract.md``.
"""
...
logger = logging.getLogger("cron.webcron")


def _cfg(*keys: str, default: Any = "") -> Any:
    """Read a cron.webcron.* config value (no network)."""
    try:
...
class ChronosCronScheduler(CronScheduler):
    ...
    def is_available(self) -> bool:
        ...
        Webcron needs a portal base URL, the agent's own publicly-reachable
        callback URL (for NAS→agent fires), and a usable Hackit token (the agent
        is logged into the portal). If any is missing, resolve_cron_scheduler
        falls back to the built-in ticker.
        """
        if not (_cfg("cron", "webcron", "portal_url") and _cfg("cron", "webcron", "callback_url")):
            return False
        return self._have_hackit_token()

    def _have_hackit_token(self) -> bool:
        """True if the agent has a Hackit Portal login (no network call).

        Checks the stored auth state for a Hackit access token — does NOT refresh
    ...
    def _get_client(self):
        if self._client is None:
            ...
            self._client = NasCronClient(_cfg("cron", "webcron", "portal_url"))
        return self._client

    def _callback_url(self) -> str:
        return str(_cfg("cron", "webcron", "callback_url") or "")
    ...
    def start(self, stop_event, *, adapters=None, loop=None, interval=60):
        ...
        try:
            ...
            logger.warning("Webcron start() reconcile failed: %s", e)
    ...
    def on_jobs_changed(self) -> None:
        ...
        try:
            ...
            logger.debug("Webcron on_jobs_changed reconcile failed: %s", e)
    ...
    def _list_armed(self) -> Dict[str, str]:
        ...
        try:
            ...
            logger.debug("Webcron _list_armed failed (will re-arm idempotently): %s", e)
            return {}
            ...
            if observed.get(job_id) != fire_at:
                ...
                if job:
                    try:
                        ...
                        logger.warning("Webcron failed to arm job %s: %s", job_id, e)
        ...
        for job_id in list(observed.keys()):
            if job_id not in desired:
                try:
                    ...
                    logger.warning("Webcron failed to cancel orphan %s: %s", job_id, e)
    ...
    # overriding it here would silently opt Webcron out of claim admission,
        ...
        if ran:
            ...
            if job and job.get("enabled") and job.get("next_run_at"):
                try:
                    ...
                    logger.warning("Webcron failed to re-arm job %s after fire: %s", job_id, e)
        return ran


def register(ctx) -> None:
    """Plugin entrypoint — register the Webcron provider with the loader.
