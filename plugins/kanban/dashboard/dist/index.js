/**
 * Slick Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.slick/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__SLICK_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__SLICK_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "slick-kanban-dot-triage",
    todo: "slick-kanban-dot-todo",
    ready: "slick-kanban-dot-ready",
    running: "slick-kanban-dot-running",
    blocked: "slick-kanban-dot-blocked",
    review: "slick-kanban-dot-review",
    done: "slick-kanban-dot-done",
    archived: "slick-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-slick-task";
  ...
  const DOCS_URL = "https://slick-agent.sophostechne.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://slick-agent.sophostechne.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "slick.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="slick-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])slick-kanban-md-code\1/i.test(attrs)) {
      return ' class="slick-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "slick-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "slick-kanban-md",
  ...
  // standard `drop` event and our `slick-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("slick-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("slick-kanban-column--drop");
          if (target) target.classList.add("slick-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("slick-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("slick-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("slick-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__SLICK_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "slick-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "slick-kanban-attention",
        "slick-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "slick-kanban-attention-bar" },
        h("span", { className: "slick-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "slick-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "slick-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "slick-kanban-attention-row",
                  "slick-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "slick-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "slick-kanban-attention-row-id" }, task.id),
                h("span", { className: "slick-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "slick-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "slick-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "slick-kanban-diag-action-btn",
      action.suggested ? "slick-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " slick-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".slick-kanban-drawer-comment-row input, .slick-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "slick-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("slick-kanban-diag", sevClass) },
      h("div", { className: "slick-kanban-diag-header" },
        h("span", { className: "slick-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "slick-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "slick-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "slick-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "slick-kanban-diag-data-row" },
                  h("span", { className: "slick-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "slick-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "slick-kanban-diag-data-row" },
                h("span", { className: "slick-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "slick-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "slick-kanban-diag-reassign-row" },
            h("span", { className: "slick-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "slick-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "slick-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "slick-kanban-diag-msg",
              msg.ok ? "slick-kanban-diag-msg--ok" : "slick-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "slick-kanban-section" },
      h("div", { className: "slick-kanban-section-head-row" },
        h("span", { className: "slick-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "slick-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "slick-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "slick-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "slick-kanban-docs-link",
      title: "Open Slick Kanban docs in a new tab",
      "aria-label": "Slick Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `slick kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "slick-kanban-msg-ok" : "slick-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "slick-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "slick-kanban-boardswitcher" },
      h("div", { className: "slick-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "slick-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "slick-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "slick-kanban-dialog-title" },
        ...
        h("div", { className: "slick-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Slick profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "slick-kanban-bulk" },
      h("span", { className: "slick-kanban-bulk-count" },
      ...
      h("div", { className: "slick-kanban-bulk-priority",
      ...
      h("div", { className: "slick-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Slick profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "slick-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("slick-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("slick-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "slick-kanban-trash",
        dragOver ? "slick-kanban-trash--drop" : "",
        props.draggingTaskId ? "slick-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "slick-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "slick-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".slick-kanban-card")) return true;
      if (target.closest && target.closest(".slick-kanban-column-add")) return true;
      if (target.closest && target.closest(".slick-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("slick-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("slick-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".slick-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "slick-kanban-columns",
        isScrollable ? "slick-kanban-columns--scrollable" : "",
        isPanning ? "slick-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("slick-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("slick-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "slick-kanban-column",
        dragOver ? "slick-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "slick-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "slick-kanban-col-check",
        ...
        h("span", { className: cn("slick-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "slick-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "slick-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "slick-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "slick-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "slick-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "slick-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "slick-kanban-lane" },
                  h("div", { className: "slick-kanban-lane-head" },
                    h("span", { className: "slick-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "slick-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "slick-kanban-card--stale-red";
    if (age >= tier.amber) return "slick-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".slick-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "slick-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "slick-kanban-card",
        props.selected ? "slick-kanban-card--selected" : "",
        props.failed ? "slick-kanban-card--failed" : "",
        props.draggingSource ? "slick-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "slick-kanban-card-content" },
          h("div", { className: "slick-kanban-card-row" },
            h("label", {
              className: "slick-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "slick-kanban-card-check",
            ...
            h("span", { className: "slick-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or slick kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "slick-kanban-warning-badge",
                    "slick-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "slick-kanban-needs-assignee",
          ...
          h("div", { className: "slick-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "slick-kanban-card-row slick-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "slick-kanban-assignee",
                            title: `Assigned to Slick profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "slick-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "slick-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "slick-kanban-ago",
  ...
  // centered modal reusing the slick-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "slick-kanban-dialog slick-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Slick profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Slick profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "slick-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "slick-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "slick-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "slick-kanban-drawer-close",
        ...
        data ? h("div", { className: "slick-kanban-drawer-comment-foot" },
          h("div", {
            className: "slick-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "slick-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "slick-kanban-section" },
      h("div", { className: "slick-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "slick-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "slick-kanban-drawer-body" },
      h("div", { className: "slick-kanban-drawer-title" },
        h("span", { className: cn("slick-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "slick-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "slick-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "slick-kanban-section" },
            h("div", { className: "slick-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "slick-kanban-section" },
            h("div", { className: "slick-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "slick-kanban-done-no-result slick-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "slick-kanban-section" },
            h("div", { className: "slick-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "slick-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "slick-kanban-section" },
        h("div", { className: "slick-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "slick-kanban-comment" },
            h("div", { className: "slick-kanban-comment-head" },
              h("span", { className: "slick-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "slick-kanban-diag-action-btn",
      ...
      h("div", { className: "slick-kanban-section" },
        h("div", { className: "slick-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "slick-kanban-comment" },
                h("div", { className: "slick-kanban-comment-head" },
                  h("span", { className: "slick-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "slick-kanban-comment-ago" },
      ...
      h("div", { className: "slick-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "slick-kanban-event",
              isDiag ? "slick-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "slick-kanban-event-header" },
                  h("span", { className: "slick-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "slick-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "slick-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "slick-kanban-event-header-plain" },
                  h("span", { className: "slick-kanban-event-kind" }, e.kind),
                  h("span", { className: "slick-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "slick-kanban-event-phantom-row" },
                  h("span", { className: "slick-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "slick-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "slick-kanban-event-payload" },
    ...
    return h("div", { className: "slick-kanban-section" },
      h("div", { className: "slick-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "slick-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `slick-kanban-run--${r.outcome || r.status || "ended"}`
          : "slick-kanban-run--active";
        return h("div", { key: r.id, className: cn("slick-kanban-run", outcomeClass) },
          h("div", { className: "slick-kanban-run-head" },
            h("span", { className: "slick-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "slick-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "slick-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "slick-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "slick-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "slick-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "slick-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "slick-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "slick-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "slick-kanban-pre slick-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "slick-kanban-meta-row" },
      h("span", { className: "slick-kanban-meta-label" }, props.label),
      h("span", { className: "slick-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "slick-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "slick-kanban-meta-row" },
        h("span", { className: "slick-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "slick-kanban-meta-value slick-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "slick-kanban-meta-row" },
        h("span", { className: "slick-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "slick-kanban-meta-value slick-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "slick-kanban-meta-row" },
        h("span", { className: "slick-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "slick-kanban-meta-value slick-kanban-editable",
    ...
    return h("div", { className: "slick-kanban-meta-row" },
      h("span", { className: "slick-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "slick-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "slick-kanban-section" },
      h("div", { className: "slick-kanban-section-head-row" },
        h("span", { className: "slick-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "slick-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "slick-kanban-section" },
      h("div", { className: "slick-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "slick-kanban-deps-row" },
        h("span", { className: "slick-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "slick-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "slick-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "slick-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "slick-kanban-dep-chip-x",
      ...
      h("div", { className: "slick-kanban-deps-row" },
        h("span", { className: "slick-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "slick-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "slick-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "slick-kanban-section" },
      ...
      h("div", { className: "slick-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "slick-kanban-home-sub slick-kanban-home-sub--on"
              : "slick-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__SLICK_PLUGINS__ && typeof window.__SLICK_PLUGINS__.register === "function") {
    window.__SLICK_PLUGINS__.register("kanban", KanbanPage);
  }
})();
