...
the ``slick`` integration, and applies a chosen manifest to the user's bank
...
from slick_cli.urllib_security import open_credentialed_url
...
def fetch_slick_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``slick`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "slick" in (e.get("integrations") or [])]
