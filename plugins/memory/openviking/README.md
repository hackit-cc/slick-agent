...
- OpenViking server running and reachable from Slick

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Slick can identify older servers that expose the legacy status-only health
...
connection values into Slick, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.slick/.env`; for a named profile use
...
OpenViking's server config is separate from Slick:
...
Slick-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `slick` | Slick peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Slick lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Slick sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Slick also sends `User-Agent: openviking-memory-slick/<version>` on
OpenViking requests. This standard harness identifier contains the Slick
...
current user). Slick caches a confirmed user only for the active connection.
If the probe fails, Slick uses the configured user, or `default`, for that
...
Slick built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Slick action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Slick
...
file URIs, such as
`viking://user/default/peers/slick/memories/preferences/mem_abc123.md` (any
