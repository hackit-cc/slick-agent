"""slick-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $SLICK_HOME/config.yaml (profile-scoped):
  plugins:
    slick-memory-store:
      db_path: $SLICK_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "slick-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, slick_home):
        """Write config to config.yaml under plugins.slick-memory-store."""
        from pathlib import Path
        config_path = Path(slick_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["slick-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_slick_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _slick_home = str(get_slick_home())
        _default_db = _slick_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $SLICK_HOME in user-supplied paths so config values like
        # "$SLICK_HOME/memory_store.db" or "~/.slick/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$SLICK_HOME", _slick_home)
            db_path = db_path.replace("${SLICK_HOME}", _slick_home)
