...
```bash
slick memory setup    # select "mem0"
```
...
Behavioral settings live in `$SLICK_HOME/mem0.json` (set them via `slick memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.slick/.env`.
...
| `user_id` | `slick-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Slick session and call `mem0_search` — it connects to your server.
...
```bash
slick memory setup mem0 --mode oss \
...
```bash
slick memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$SLICK_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.slick/mem0_qdrant"}}
  }
}
```
...
```bash
slick memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$SLICK_HOME/mem0.json`).
