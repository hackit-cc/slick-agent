...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/hackit>
...
```bash
slick memory setup honcho   # configure Honcho directly (works on a fresh install)
slick memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
slick config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.slick/.env
```

> `slick honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `slick memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `slick model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$SLICK_HOME/honcho.json` | Profile-local (isolated Slick instances) |
| 2 | `~/.slick/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Slick profile: `slick` (default) or `slick_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `slick honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Slick session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`slick-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`slick`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-slick-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening slick in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Slick session ID (timestamp + hex). Every `slick` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Slick profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`slick`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Slick profile: `slick` (default) or `slick_<profile>` (e.g. `slick -p coder` -> host key `slick_coder`). Older `slick.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `SLICK_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `slick-agent`) |
...
|---------|-------------|
| `slick memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `slick honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `slick memory setup`) |
| `slick honcho status` | Show resolved config for active profile |
| `slick honcho enable` / `disable` | Toggle Honcho for active profile |
| `slick honcho mode <mode>` | Change recall or observation mode |
| `slick honcho peer --user <name>` | Update user peer name |
| `slick honcho peer --ai <name>` | Update AI peer name |
| `slick honcho tokens --context <N>` | Set context token budget |
| `slick honcho tokens --dialectic <N>` | Set dialectic max chars |
| `slick honcho map <name>` | Map current directory to a session name |
| `slick honcho sync` | Create host blocks for all Slick profiles |
