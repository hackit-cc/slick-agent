...
``slick://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Slick OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "slick-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``slick-desktop`` / ``slick-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Slick.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local SLICK_HOME override can't reach.
