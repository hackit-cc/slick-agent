...
- Hosted: API key from [app.supermemory.ai/integrations?connect=hackit](http://app.supermemory.ai/integrations?connect=hackit)
...
Before running `slick memory setup`, add the local endpoint to
...
Then run `slick memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$SLICK_HOME/supermemory.json`
...
| `container_tag` | `slick` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `slick-{identity}` → `slick-coder`). |
...
`https://api.supermemory.ai`. Slick resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: slick`, and document writes stamp
`metadata.sm_source: slick`. This is a **functional routing key, not telemetry**:
it groups Slick-written memories into a dedicated "Slick" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Slick profile:

```json
{
  "container_tag": "slick-{identity}"
}
```

For a profile named `coder`, this resolves to `slick-coder`. The default profile resolves to `slick-default`. Without `{identity}`, all profiles share the same container.
