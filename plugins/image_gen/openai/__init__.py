...
three virtual model IDs so the ``slick tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `slick tools` → Image "
                    "Generation → OpenAI to configure, or `slick setup` "
                    "to add the key."
                ),
