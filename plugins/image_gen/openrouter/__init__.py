"""OpenRouter-compatible image generation backend (OpenRouter + Hackit Portal).

Both OpenRouter and the Hackit Portal inference endpoint speak the same
...
Hackit Portal proxies OpenRouter, so one implementation services both — we only
swap the resolved ``(base_url, api_key)``. Credentials are resolved through the
agent's existing :func:`~slick_cli.runtime_provider.resolve_runtime_provider`,
which already understands OpenRouter's key pool and the Hackit OAuth device-code
...
The Image API is OpenRouter-only: Hackit Portal proxies the chat-completions
protocol and has no ``/images/generations`` route, so the second surface is
enabled per provider (see ``supports_image_api``) and stays off for Hackit.
"""
...
# image_gen.model from ``slick tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    Instantiated once per backend (OpenRouter, Hackit Portal). The two differ only
    ...
    def __init__(
        ...
        *,
        ...
        # OpenRouter only: Hackit Portal proxies the chat-completions protocol
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        after this code shipped — is selectable in ``slick tools``. Hackit
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``slick tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `slick tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
        ...
        headers = {
            ...
            # OpenRouter attribution headers (harmless against Hackit Portal).
            "HTTP-Referer": "https://github.com/hackit-cc/slick-agent",
            "X-Title": "Slick Agent",
        }
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            provider_name="hackit",
            display_name="Hackit Portal",
            runtime_name="hackit",
            config_key="hackit",
            model_env_var="HACKIT_IMAGE_MODEL",
            setup_schema={
                "name": "Hackit Portal (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via Hackit Portal (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + Hackit Portal image gen providers."""
