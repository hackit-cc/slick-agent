"""OpenRouter-compatible image generation backend (OpenRouter + Sophos Portal).

Both OpenRouter and the Sophos Portal inference endpoint speak the same
...
Sophos Portal proxies OpenRouter, so one implementation services both — we only
...
which already understands OpenRouter's key pool and the Sophos OAuth device-code
...
The Image API is OpenRouter-only: Sophos Portal proxies the chat-completions
protocol and has no ``/images/generations`` route, so the second surface is
enabled per provider (see ``supports_image_api``) and stays off for Sophos.
"""
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    Instantiated once per backend (OpenRouter, Sophos Portal). The two differ only
    ...
    def __init__(
        ...
        *,
        ...
        # OpenRouter only: Sophos Portal proxies the chat-completions protocol
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        after this code shipped — is selectable in ``slick tools``. Sophos
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        headers = {
            ...
            # OpenRouter attribution headers (harmless against Sophos Portal).
            "HTTP-Referer": "https://github.com/SophosTechne/slick-agent",
            "X-Title": "Slick Agent",
        }
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            ...
            display_name="Sophos Portal",
            runtime_name="sophos",
            config_key="sophos",
            model_env_var="SOPHOS_IMAGE_MODEL",
            setup_schema={
                "name": "Sophos Portal (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via Sophos Portal (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + Sophos Portal image gen providers."""
