...
Authentication via ``FAL_KEY`` or the managed Hackit gateway. Output is an
...
def _resolve_managed_fal_video_gateway():
    ...
    image FAL resolver: ``"hackit"`` (or legacy ``use_gateway: true``) →
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                f"Hackit Subscription gateway rejected endpoint '{endpoint}' "
        ...
        if not _check_fal_video_available():
            ...
            return error_response(
                error=(
                    ...
                    "or sign in to Hackit (`slick setup`) for managed gateway access."
                ),
                error_type="auth_required",
                provider="fal",
                prompt=prompt,
