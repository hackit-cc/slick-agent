...
Auto-tracks and cleans up ephemeral files created during Slick Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SLICK_HOME` and `/tmp/slick-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SLICK_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SLICK_HOME` or `/tmp/slick-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SLICK_HOME/disk-cleanup/` is itself excluded
- `$SLICK_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
