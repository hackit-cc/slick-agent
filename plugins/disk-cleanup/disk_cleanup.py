"""disk_cleanup — ephemeral file cleanup for Slick Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under SLICK_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly SLICK_HOME and /tmp/slick-*
Never touches: ~/.slick/logs/ or any system directory.
"""
...
try:
    ...
    def get_slick_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("SLICK_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".slick").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SLICK_HOME/logs/``."""
    return get_slick_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SLICK_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under SLICK_HOME or ``/tmp/slick-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    slick_home = get_slick_home()
    try:
        path.resolve().relative_to(slick_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/slick-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("slick-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "slick-agent", "backups", "profiles", ".worktrees",
...
# Paths under $SLICK_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so SLICK_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        slick_home = get_slick_home()
        for parent in ("cron", "cronjobs"):
            base = slick_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SLICK_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under SLICK_HOME, but never recurse into known
    # durable state trees.  Some installs place the Slick checkout, venv,
    # and desktop build under SLICK_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    slick_home = get_slick_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in slick_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    slick_home = get_slick_home()
    try:
        rel = path.resolve().relative_to(slick_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "slick-agent",
        ...
        # Path isn't under SLICK_HOME (e.g. /tmp/slick-*) — fall through.
        pass
