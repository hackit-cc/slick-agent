...
2. **Borderless elevation for floating panels.** Overlays float on
   `shadow-sophos` + a `--stroke-sophos` hairline, not thick framed boxes. In-panel
...
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-sophos`,
...
```
shadow-sophos           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-sophos) /* currentColor hairline, theme-adaptive */
```
...
| `--stroke-sophos` | the overlay hairline (pairs with `shadow-sophos`) |
...
- **`BrandMark`** (`src/components/brand-mark.tsx`) is the brand glyph — the
  `sophos-girl` mark on a white tile, softly rounded, identical in light/dark.
...
- [ ] Reuse a primitive (`Button`, `SearchField`, `SegmentedControl`,
      ...
      forking one?
- [ ] Tokens (`--ui-*`, `shadow-sophos`, `--stroke-sophos`) — zero raw colors /
      one-off shadows?
...
- [ ] Overlay uses `shadow-sophos` + `border-(--stroke-sophos)`, no hard border?
