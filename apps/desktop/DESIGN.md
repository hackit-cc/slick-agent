...
2. **Borderless elevation for floating panels.** Overlays float on
   `shadow-hackit` + a `--stroke-hackit` hairline, not thick framed boxes. In-panel
...
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-hackit`,
...
```
shadow-hackit           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-hackit) /* currentColor hairline, theme-adaptive */
```
...
| `--stroke-hackit` | the overlay hairline (pairs with `shadow-hackit`) |
...
- **`BrandMark`** (`src/components/brand-mark.tsx`) is the brand glyph — the
  `hackit-girl` mark on a white tile, softly rounded, identical in light/dark.
...
- [ ] Reuse a primitive (`Button`, `SearchField`, `SegmentedControl`,
      ...
      forking one?
- [ ] Tokens (`--ui-*`, `shadow-hackit`, `--stroke-hackit`) — zero raw colors /
      one-off shadows?
...
- [ ] Overlay uses `shadow-hackit` + `border-(--stroke-hackit)`, no hard border?
