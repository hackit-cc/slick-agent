...
//   · its own SLICK_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · SLICK_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated SLICK_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate SLICK_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = slickHome ?? mkTemp('slick-perf-home-')
  const userData = userDataDir ?? mkTemp('slick-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !slickHome) {
    seedConfigFrom(join(homedir(), '.slick'), home)
  }
  ...
  try {
    ...
    // SLICK_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set SLICK_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...
      SLICK_DESKTOP_CDP_PORT: String(port),
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.SLICK_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'slick-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'slick-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.slick'), home)
