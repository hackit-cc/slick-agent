 ...
 * product name (`Slick`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Slick'
 *
 * This is a hard failure with no obvious cause for the user — `slick desktop`
 ...
 * (the mac-unpacked Slick.app bundle) and Windows (win-unpacked), so we
 ...
 * Slick.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `slick desktop --build-only` (slick_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Slick.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Slick'}.exe`
