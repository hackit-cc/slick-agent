...
// no longer bundles the Slick Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Slick.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Slick'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Slick.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Slick'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default SLICK_HOME for non-sandboxed runs -- matches main.ts's
// resolveSlickHome(). On Windows it's %LOCALAPPDATA%\slick; elsewhere
// it's ~/.slick. The fresh-install sandbox launchFresh() sets its own
// SLICK_HOME and never touches this.
const DEFAULT_SLICK_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'slick')
  }
  return path.join(os.homedir(), '.slick')
})()
const VENV_ROOT = path.join(DEFAULT_SLICK_HOME, 'slick-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'slick-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.SLICK_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Slick-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Slick-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Slick-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Slick-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.SLICK_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.SLICK_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const slickHome = path.join(sandbox, 'slick-home')
  ...
  env.SLICK_DESKTOP_CWD = cwd
  env.SLICK_DESKTOP_IGNORE_EXISTING = '1'
  env.SLICK_DESKTOP_TEST_MODE = 'fresh-install'
  env.SLICK_DESKTOP_USER_DATA_DIR = userDataDir
  env.SLICK_HOME = slickHome
  delete env.SLICK_DESKTOP_SLICK
  delete env.SLICK_DESKTOP_SLICK_ROOT
  ...
  console.log(`  SLICK_HOME: ${slickHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(slickHome, 'slick-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Slick Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of slick_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'slick-agent', 'slick_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Slick
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + SLICK_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  SLICK_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
