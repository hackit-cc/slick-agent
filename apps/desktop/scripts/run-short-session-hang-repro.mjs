...
const UPSTREAM_URL = 'https://github.com/SophosTechne/slick-agent.git'
...
const LEGACY_STORAGE_KEY = 'slick.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'slick.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.slickDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `slick-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const slickHome = join(sandbox, 'slick-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(slickHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(slickHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        SLICK_DESKTOP_APP_NAME: `SlickShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        SLICK_DESKTOP_SLICK_ROOT: target.targetRoot,
        SLICK_DESKTOP_IGNORE_EXISTING: '1',
        SLICK_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'slick-short-session-ab-'))
  const prepared = []
