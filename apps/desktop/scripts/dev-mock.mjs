...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `slick-dev-mock-${Date.now()}`))
  ...
  return { root, slickHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(slickHome, mockUrl) {
  fs.writeFileSync(
    path.join(slickHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(slickHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.slickHome, mock.url)
  console.log(`  SLICK_HOME: ${sandbox.slickHome}`)
  ...
  const env = {
    ...
    SLICK_DESKTOP_APP_NAME: `SlickDevMock-${Date.now()}`,
  }
