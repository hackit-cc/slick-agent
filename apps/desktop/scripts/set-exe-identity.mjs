#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Slick icon + version metadata onto the
// built Slick.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Slick.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `slick desktop`, the installer's
...
// path (which rebuilds via `slick desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Slick.exe>
//
...
// Stamp the Slick icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Slick',
      FileDescription: 'Slick',
      CompanyName: 'Sophos Techne',
      LegalCopyright: 'Copyright (c) 2026 Sophos Techne'
    }
  })

  console.log('[set-exe-identity] done — Slick icon + identity stamped')
}
