...
vi.mock('@/slick', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), slickApi: vi.fn() }))
...
const { deleteProfile, slickApi } = await import('@/slick')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its SLICK_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { slickDesktop: unknown }).slickDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(slickApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(slickApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(slickApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
