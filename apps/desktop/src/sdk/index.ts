/**
 * @slick/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__SLICK_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, slickApi, type SlickGateway } from '@/slick'
import {
  $gateway,
...
import type { PaginatedSessions, UsageStats } from '@/types/slick'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Slick profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.slickDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.slickDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and SlickGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (slick-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.slickDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Slick Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.slickDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Slick Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Slick Desktop to open another Bot chat.' })
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Slick Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return slickApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return slickApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Slick gateway unavailable')
    }
...
export type { SlickGateway } from '@/slick'
...
export type { SlickOpenTarget } from '@/lib/slick-open-target'
...
/** Slick' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/slick'
