...
//
// When someone is in a hands-free "Hey Slick" voice chat, the natural way to
...
// phrase (optionally addressed to Slick), so a real turn that merely contains
...
// Optional address prefixes so "slick stop" / "ok stop" / "hey slick, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey slick', 'hey slick,', 'slick', 'slick,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("slick") is not a stop command on its own.
      continue
    }
 ...
 * to Slick). Returns false for anything that merely contains "stop" as part of
