 ...
 * `slick://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `slick://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `slick://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type SlickOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const SLICK_PROTOCOL = 'slick:'
...
export function normalizeSlickOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('slick://') || trimmed.startsWith(`${SLICK_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // slick://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // slick://index-network/intent/1 → /index-network/intent/1
...
export function resolveSlickOpenPath(target: SlickOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeSlickOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeSlickOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeSlickOpenString(target.path)
 ...
 * (`slick://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromSlickDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveSlickOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveSlickOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `slick://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromSlickDeepLink('open', name, params)
}
