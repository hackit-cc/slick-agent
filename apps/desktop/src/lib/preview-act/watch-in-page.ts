...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __slickWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__slickWatch || (win.__slickWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('slick-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('slick-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __slickWatchTag?: number }).__slickWatchTag
