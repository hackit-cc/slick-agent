...
/** Why a known Slick command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Slick profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownSlickSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Slick' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownSlickSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Slick and were never asked for. It is
