...
// Slick is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.slickMode
 ...
 * the only carriers of Slick' palette — are dropped so the paste target's
