...
const globalOptions = { model: 'slick-4', provider: 'hackit', providers: [] }
...
describe('requestModelOptions', () => {
  ...
  it('prefers an owner-routed request over the ambient gateway socket', async () => {
    const gatewayPayload = {
      model: 'chrome-model',
      provider: 'hackit',
      providers: [{ models: ['chrome-model'], name: 'Hackit', slug: 'hackit' }]
    }
...
describe('manualPickRemoved', () => {
  const providers = [
    ...
    { name: 'Hackit', slug: 'hackit', models: [] } // present but unconfigured / re-auth
  ]
  ...
  it('never clobbers when the provider has an empty model list (re-auth)', () => {
    expect(manualPickRemoved(providers, 'hackit', 'slick-4')).toBe(false)
  })
