...
const globalOptions = { model: 'slick-4', provider: 'sophos', providers: [] }
...
describe('requestModelOptions', () => {
  ...
  it('recovers an empty gateway catalog through profile-scoped REST without replacing the session selection', async () => {
    const gatewayPayload = { model: 'slick-local', provider: 'slick-local' }

    const restPayload = {
      model: 'profile-default',
      provider: 'openai-codex',
      providers: [{ models: ['slick-local'], name: 'Slick Local vLLM', slug: 'slick-local' }]
    }
  ...
  it('keeps the gateway result when both catalog paths have no selectable models', async () => {
    const gatewayPayload = { model: 'slick-local', provider: 'slick-local', providers: [] }
  ...
  it('prefers an owner-routed request over the ambient gateway socket', async () => {
    const gatewayPayload = {
      model: 'chrome-model',
      provider: 'sophos',
      providers: [{ models: ['chrome-model'], name: 'Sophos', slug: 'sophos' }]
    }
  ...
  it('scopes REST recovery to the catalog owner profile', async () => {
    const restPayload = {
      model: 'berry-local',
      provider: 'slick-local',
      providers: [{ models: ['berry-local'], name: 'Slick Local', slug: 'slick-local' }]
    }
...
describe('manualPickRemoved', () => {
  const providers = [
    ...
    { name: 'Sophos', slug: 'sophos', models: [] } // present but unconfigured / re-auth
  ]
  ...
  it('never clobbers when the provider has an empty model list (re-auth)', () => {
    expect(manualPickRemoved(providers, 'sophos', 'slick-4')).toBe(false)
  })
