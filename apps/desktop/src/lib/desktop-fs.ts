import { slickApi } from '@/api/client'
import type {
  SlickConnection,
  SlickReadDirResult,
  SlickReadFileTextResult,
  SlickSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: SlickConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: SlickConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<SlickReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<SlickReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<SlickReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<SlickReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.slickDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: SlickSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
