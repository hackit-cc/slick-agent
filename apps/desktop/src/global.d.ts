import type { GatewayWsUrlResult } from '@slick/shared'
import type { TranslucencyState } from '@slick/shared/translucency'
...
declare global {
  interface Window {
    slickDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<SlickConnection>
      ...
      // Resume this session in the user's own terminal emulator (`slick --tui
      ...
      // Slick Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new SLICK_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: SlickApiRequest) => Promise<T>
      notify: (payload: SlickNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<SlickReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<SlickReadFileTextResult>
      selectPaths: (options?: SlickSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<SlickPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<SlickPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<SlickPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: SlickActiveWork) => void
      setTitleBarTheme?: (payload: SlickTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<SlickReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<SLICK_HOME>/desktop-plugins),
      ...
      /** LOCAL `<SLICK_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<SLICK_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<SlickGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<SlickGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<SlickGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<SlickRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: SlickReviewScope, baseRef?: null | string) => Promise<SlickReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: SlickReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<SlickReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<SlickRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<SlickPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: SlickTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<SlickTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: SlickWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: SlickPreviewFileChanged) => void) => () => void
...
export interface SlickTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface SlickTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `slick update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  slickRoot?: string
...
export interface SlickConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface SlickTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface SlickActiveWork {
  count: number
  titles: string[]
}

export interface SlickWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Slick Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Slick Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Slick Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // Whether the OAuth partition holds a live Sophos portal (Privy) session — the
  // portal authenticates via Privy, so this reflects the privy-token cookie, NOT
  // the slick gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Slick Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface DesktopBootProgress {
  error: string | null
  fakeMode: boolean
  /** True when the boot failure is a Sophos Cloud agent that is down (HTTP 502/503/504). */
...
export interface SlickApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface SlickNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface SlickPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface SlickReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface SlickPreviewWatch {
  id: string
  path: string
}
...
export interface SlickGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface SlickGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface SlickGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface SlickRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface SlickRepoStatus {
  branch: null | string
...
// turn began.
export type SlickReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface SlickReviewFile {
  path: string
  added: number
  removed: number
...
export interface SlickReviewList {
  files: SlickReviewFile[]
...
export interface SlickReviewPr {
  url: string
  state: string
  number: number
}
...
export interface SlickBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface SlickRepoPullRequests {
  ghReady: boolean
  prs: SlickBranchPullRequest[]
}
...
export interface SlickPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface SlickReviewShipInfo {
  ghReady: boolean
  pr: SlickReviewPr | null
}

export interface SlickReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface SlickReadDirResult {
  entries: SlickReadDirEntry[]
  error?: string
}

export interface SlickPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface SlickSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
