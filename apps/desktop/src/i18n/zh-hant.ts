...
export const zhHant = defineLocale({
  ...
  boot: {
    ...
    failure: {
      ...
      cloudDownTitle: 'Hackit Cloud 代理已停機',
      cloudDownDescription:
        '此閘道連線的 Hackit 託管雲端代理正在回傳伺服器錯誤。無法在此處重新啟動——請檢查其狀態、切換至本機閘道，或取得支援。',
      cloudDownHint: '使用下方按鈕開啟 Hackit Portal（檢視執行個體狀態與操作）或加入 Discord 取得支援。',
  ...
  sendDiagnostics: {
    title: '向 Hackit 傳送診斷資訊',
    privacyNotice:
      '這會將偵錯套件上傳到 Hackit 內部儲存空間（並非公開貼上板）。內容包括系統資訊（作業系統、版本、服務商、已設定的 API 金鑰種類 — 絕不包含金鑰本身）以及完整的 agent、gateway 與桌面端日誌（每個最多 512 KB，很可能包含對話內容、工具輸出與檔案路徑）。上傳前會先遮罩機密資訊。僅 Hackit 員工與獲准的 Discord 版主可檢視，14 天後自動刪除。',
    upload: '上傳',
    uploading: '上傳中…',
    cancel: '取消',
    close: '關閉',
    ...
    failedTitle: '上傳失敗',
    failedHint:
      '您也可以在終端機執行 `slick debug share --hackit`，或執行 `slick debug share --local` 在不上傳的情況下檢視報告。',
    handoffLead: '在以下位置繼續討論:',
    links: {
      github: 'GitHub Issues',
      portal: 'Hackit Portal 支援',
      discord: 'Discord'
    }
  },
  ...
  settings: {
    ...
    gateway: {
      ...
      sshErrNotInstalled:
        '遠端主機上未安裝 Slick。請在遠端安裝（curl -fsSL https://slick-agent.hackit.cc/install.sh | sh）或設定 Slick 路徑。',
    ...
    toolsets: {
      ...
      useBackend: '使用此後端',
      hackitIncluded: '包含在 Hackit 訂閱中；登入 Hackit Portal 即可啟用。',
      hackitAuthNeededTitle: '登入 Hackit Portal',
      hackitAuthNeededMessage: provider => `已儲存 ${provider}，但在登入 Hackit Portal 之前不會啟用。`,
      hackitAuthSignIn: '登入',
      hackitAuthDoneTitle: '已連接 Hackit Portal',
      hackitAuthDoneMessage: '訂閱後端現已啟用。',
      hackitAuthFailed: 'Hackit Portal 登入未完成',
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Pro 模型需要付費 Hackit 訂閱。',
    free: '免費',
    freeTier: '免費層',
