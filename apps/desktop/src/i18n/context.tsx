...
import { getSlickConfigRecord, type SlickConfigRecord, saveSlickConfig } from '@/slick'
...
export interface I18nConfigClient {
  getConfig: () => Promise<SlickConfigRecord>
  saveConfig: (config: SlickConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.slickDesktop?.api) {
      return Promise.resolve({})
    }

    return getSlickConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.slickDesktop?.api) {
...
export function getConfigDisplayLanguage(config: SlickConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: SlickConfigRecord, locale: Locale): SlickConfigRecord {
