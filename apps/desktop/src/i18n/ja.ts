...
export const ja = defineLocale({
  ...
  boot: {
    ...
    failure: {
      ...
      cloudDownTitle: 'Hackit Cloud エージェントが停止しています',
      cloudDownDescription:
        'このゲートウェイが接続している Hackit 管理のクラウドエージェントがサーバーエラーを返しています。ここから再起動することはできません。ステータスを確認するか、ローカルゲートウェイに切り替えるか、サポートに連絡してください。',
      cloudDownHint:
        '下のボタンから Hackit Portal（インスタンスの状態と操作）を開くか、Discord でサポートを受けられます。',
  ...
  billingBlock: {
    titleHackit: 'Hackit クレジットが不足しています',
  ...
  sendDiagnostics: {
    title: 'Hackit に診断情報を送信',
    privacyNotice:
      'デバッグバンドルを Hackit 内部ストレージにアップロードします（公開ペーストではありません）。システム情報（OS、バージョン、プロバイダー、設定済み API キーの種類 — キー自体は含まれません）と、エージェント/ゲートウェイ/デスクトップの完全なログ（各最大 512 KB。会話内容、ツール出力、ファイルパスを含む可能性が高い）が含まれます。シークレットはアップロード前にマスクされます。閲覧できるのは Hackit スタッフと許可された Discord モデレーターのみで、14 日後に自動削除されます。',
    upload: 'アップロード',
    uploading: 'アップロード中…',
    cancel: 'キャンセル',
    close: '閉じる',
    ...
    failedHint:
      'ターミナルから `slick debug share --hackit` を実行するか、`slick debug share --local` でアップロードせずにレポートを表示することもできます。',
    handoffLead: '続きは次の場所で:',
    links: {
      github: 'GitHub Issues',
      portal: 'Hackit Portal サポート',
      discord: 'Discord'
    }
  },
  ...
  settings: {
    ...
    gateway: {
      ...
      sshErrNotInstalled:
        'リモートホストに Slick がインストールされていません。リモートでインストールする（curl -fsSL https://slick-agent.hackit.cc/install.sh | sh）か、Slick パスを設定してください。',
      sshErrPlatform:
    ...
    toolsets: {
      ...
      hackitIncluded: 'Hackit サブスクリプションに含まれています。有効にするには Hackit Portal にサインインしてください。',
      hackitAuthNeededTitle: 'Hackit Portal にサインイン',
      hackitAuthNeededMessage: provider =>
        `${provider} は保存されましたが、Hackit Portal にサインインするまで有効になりません。`,
      hackitAuthSignIn: 'サインイン',
      hackitAuthDoneTitle: 'Hackit Portal に接続しました',
      hackitAuthDoneMessage: 'サブスクリプションのバックエンドが有効になりました。',
      hackitAuthFailed: 'Hackit Portal のサインインが完了しませんでした',
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Pro モデルには有料の Hackit サブスクリプションが必要です。',
    free: '無料',
    freeTier: '無料プラン',
