...
export const zh: Translations = {
  ...
  boot: {
    ...
    failure: {
      ...
      cloudDownTitle: 'Hackit Cloud 代理已宕机',
      cloudDownDescription:
        '此网关连接的 Hackit 托管云代理正在返回服务器错误。无法在此处重启——请检查其状态、切换到本地网关或获取支持。',
      cloudDownHint: '使用下方按钮打开 Hackit Portal（查看实例状态与操作）或加入 Discord 获取支持。',
  ...
  sendDiagnostics: {
    title: '向 Hackit 发送诊断信息',
    privacyNotice:
      '这会将调试包上传到 Hackit 内部存储（并非公开粘贴板）。内容包括系统信息（操作系统、版本、服务商、已配置的 API 密钥种类 — 绝不包含密钥本身）以及完整的 agent、gateway 和桌面端日志（每个最多 512 KB，很可能包含对话内容、工具输出与文件路径）。上传前会先脱敏。仅 Hackit 员工与获准的 Discord 版主可查看，14 天后自动删除。',
    upload: '上传',
    uploading: '上传中…',
    cancel: '取消',
    close: '关闭',
    ...
    failedTitle: '上传失败',
    failedHint:
      '您也可以在终端运行 `slick debug share --hackit`，或运行 `slick debug share --local` 在不上传的情况下查看报告。',
    handoffLead: '在以下位置继续讨论:',
    links: {
      github: 'GitHub Issues',
      portal: 'Hackit Portal 支持',
      discord: 'Discord'
    }
  },
  ...
  settings: {
    ...
    gateway: {
      ...
      sshErrNotInstalled:
        '远程主机上未安装 Slick。请在远程安装（curl -fsSL https://slick-agent.hackit.cc/install.sh | sh）或设置 Slick 路径。',
    ...
    toolsets: {
      ...
      useBackend: '使用此后端',
      hackitIncluded: '包含在 Hackit 订阅中；登录 Hackit Portal 即可激活。',
      hackitAuthNeededTitle: '登录 Hackit Portal',
      hackitAuthNeededMessage: provider => `已保存 ${provider}，但在登录 Hackit Portal 之前不会激活。`,
      hackitAuthSignIn: '登录',
      hackitAuthDoneTitle: '已连接 Hackit Portal',
      hackitAuthDoneMessage: '订阅后端现已激活。',
      hackitAuthFailed: 'Hackit Portal 登录未完成',
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Pro 模型需要付费 Hackit 订阅。',
    free: '免费',
    freeTier: '免费层',
