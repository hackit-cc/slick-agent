 ...
 * app's bounds and stays visible while Slick is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'slick.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'slick.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.slickDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.slickDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.slickDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.slickDesktop?.petOverlay?.close()
}
