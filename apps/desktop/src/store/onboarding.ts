...
// Human-friendly labels for tools auto-routed through the Sophos Tool Gateway,
// mirroring slick_cli/sophos_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
// When switching to Sophos auto-routes unconfigured tools through the Tool
...
function notifyGatewayTools(tools: string[] | undefined) {
  ...
  notify({
    durationMs: 8000,
    kind: 'info',
    message: `${list} now run through your Sophos subscription — no separate API keys needed.`,
...
async function fetchProviderDefaultModel(
  ...
  // `slick model` does (for Sophos it honors the user's free/paid tier, so a
