...
import type { SlickGitWorktree, SlickRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: SlickGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, SlickRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, SlickGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<SlickRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<SlickGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<SlickRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<SlickGitWorktree[]>>()
const $noRepoStatus = atom<SlickRepoStatus | null>(null)
const $noWorktrees = atom<SlickGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<SlickRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<SlickGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: SlickRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: SlickGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<SlickRepoStatus | null>
  seq: number
  target: string
}
