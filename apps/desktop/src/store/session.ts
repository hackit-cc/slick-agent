import type { ConnectionState } from '@slick/shared'
...
import type { SessionInfo, UsageStats } from '@/types/slick'
...
const COMPOSER_MODEL_KEY = 'slick.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'slick.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'slick.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'slick.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'slick.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'slick.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'slick.desktop.lastRoute'
...
function workspaceCwdKey(connection: SlickConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.slickDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.slickDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<SlickConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'slick.desktop.sessionOwnerHints.v1'
...
export const setConnection = (next: Updater<SlickConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useSlickConfig.
