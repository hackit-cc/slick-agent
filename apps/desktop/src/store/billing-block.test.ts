import type { BillingBlock } from '@slick/shared'
...
test('runBillingRecovery routes Hackit to in-app Settings, never an external link', () => {
  runBillingRecovery(makeBlock({ is_hackit: true, provider: 'hackit', provider_label: 'Hackit Portal' }))
...
test('billingCtaLabel picks the right verb per route', () => {
  ...
  expect(billingCtaLabel(makeBlock({ is_hackit: true }), copy)).toBe('Open billing')
  expect(billingCtaLabel(makeBlock({ is_hackit: false }), copy)).toBe('Add credits')
})
