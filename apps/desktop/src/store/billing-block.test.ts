...
test('runBillingRecovery routes Sophos to in-app Settings, never an external link', () => {
  runBillingRecovery(makeBlock({ is_sophos: true, provider: 'sophos', provider_label: 'Sophos Portal' }))
...
test('billingCtaLabel picks the right verb per route', () => {
  ...
  expect(billingCtaLabel(makeBlock({ is_sophos: true }), copy)).toBe('Open billing')
  expect(billingCtaLabel(makeBlock({ is_sophos: false }), copy)).toBe('Add credits')
})
