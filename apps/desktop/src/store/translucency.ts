 ...
 * `@slick/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-slick-glass]`
 * block in styles.css).
 */
...
} from '@slick/shared/translucency'
 ...
 * sets `slickDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.slickDesktop?.glassSupported === 'boolean'
    ? window.slickDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.slickDesktop?.translucencySupported === 'boolean'
    ? window.slickDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-slick-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-slick-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-slick-glass-on', glassLive)
  root.toggleAttribute('data-slick-glass', glassOn)
  root.toggleAttribute('data-slick-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-slick-glass-scope', scope)
  } else {
    root.removeAttribute('data-slick-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.slickDesktop?.setTranslucency?.(state)
  })
