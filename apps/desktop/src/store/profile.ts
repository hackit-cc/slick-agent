import { LOCAL_CONNECTION_ID } from '@slick/shared'
import { atom, batch, computed } from 'nanostores'

import type { SlickConnection } from '@/global'
import { getProfiles, slickApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/slick'
...
import type { ProfileInfo } from '@/types/slick'
...
// /api/profiles/active `current`). "default" is the root ~/.slick. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'slick.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'slick.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await slickApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new SLICK_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.slickDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<SlickConnection | null> {
  const getConnection = window.slickDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<SlickConnection | null> {
  const getConnectionFor = window.slickDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'slick.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.slickDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.slickDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.slick) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.slickDesktop?.touchBackend?.(target).catch(() => undefined)
}
