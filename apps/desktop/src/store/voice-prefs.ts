...
import { getSlickConfigRecord, saveSlickConfig } from '@/slick'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getSlickConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveSlickConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
