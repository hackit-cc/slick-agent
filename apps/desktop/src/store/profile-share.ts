 ...
 *
 * The archive is the CLI's own `slick profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/slick'
...
import type { ProfileDesktopOverlay } from '@/types/slick'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Slick profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.slickDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.slickDesktop?.selectPaths?.({
