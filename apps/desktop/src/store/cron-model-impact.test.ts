...
import type { ModelAssignmentResponse } from '@/types/slick'
...
describe('setMainModelAssignment', () => {
  it('shows one consumer warning and routes via a read-only review action', async () => {
    ...
    await setMainModelAssignment({ provider: 'sophos', model: 'new/model' })
  ...
  it('keeps an existing warning for an older backend but clears it on explicit zero impact', async () => {
    ...
    await setMainModelAssignment({ provider: 'sophos', model: 'one' })
    expect($notifications.get()).toHaveLength(1)

    setModelAssignment.mockResolvedValueOnce(response(undefined))
    await setMainModelAssignment({ provider: 'sophos', model: 'two' })
    expect($notifications.get()).toHaveLength(1)
    ...
    await setMainModelAssignment({ provider: 'sophos', model: 'three' })
    expect($notifications.get()).toEqual([])
  })
  ...
  it('publishes only the latest same-profile assignment when responses reverse', async () => {
    ...
    const firstCall = setMainModelAssignment({ provider: 'sophos', model: 'first' })
    const secondCall = setMainModelAssignment({ provider: 'sophos', model: 'second' })
  ...
  it('invalidates pending responses and action closures on profile or connection changes', async () => {
    ...
    const call = setMainModelAssignment({ provider: 'sophos', model: 'pending' })
    ...
    await setMainModelAssignment({ provider: 'sophos', model: 'current' })
