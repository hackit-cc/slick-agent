 ...
 * composer with the reply scrolling above it, so Slick can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.slickDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.slickDesktop?.hud
...
export function resetHudLayout(): void {
  void window.slickDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.slickDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.slickDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
