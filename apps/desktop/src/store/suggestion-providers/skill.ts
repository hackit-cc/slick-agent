 ...
 *  Working in `~/www/slick-agent`, the draft says "slick-agent" constantly
 *  — a "Use skill: slick-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`slick-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/slick-agent-dev`, `plugin:skill`). */
