...
vi.mock('@/slick', () => ({
  getSlickConfigRecord: vi.fn(async () => ({})),
  saveSlickConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye slick', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye slick')
  })
