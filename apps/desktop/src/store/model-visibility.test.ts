...
import type { ModelOptionProvider } from '@/types/slick'
...
describe('model visibility', () => {
  ...
  it('preserves hidden-provider sentinel without re-adding defaults', () => {
    // User explicitly hid all models for "hackit" — sentinel marks this choice.
    const stored = new Set([emptyProviderSentinelKey('hackit')])

    const visible = effectiveVisibleKeys(stored, [
      provider('hackit', ['slick-3-llama-3.1-70b', 'slick-3-llama-3.1-8b']),
      provider('ollama', ['qwen3:latest'])
    ])

    expect(visible.has(modelVisibilityKey('hackit', 'slick-3-llama-3.1-70b'))).toBe(false)
    expect(visible.has(modelVisibilityKey('hackit', 'slick-3-llama-3.1-8b'))).toBe(false)
    // Sentinel itself is stripped from the result.
    expect(visible.has(emptyProviderSentinelKey('hackit'))).toBe(false)
  ...
  it('restores model when toggling on after hiding all', () => {
    // Simulates: user hid all "hackit" models, then toggles one back on.
    const stored = new Set([emptyProviderSentinelKey('hackit'), modelVisibilityKey('ollama', 'qwen3:latest')])

    // After toggle: sentinel removed, one model added.
    const afterToggle = new Set(stored)
    afterToggle.delete(emptyProviderSentinelKey('hackit'))
    afterToggle.add(modelVisibilityKey('hackit', 'slick-3-llama-3.1-70b'))
    ...
    expect(visible.has(modelVisibilityKey('hackit', 'slick-3-llama-3.1-70b'))).toBe(true)
    expect(visible.has(modelVisibilityKey('hackit', 'slick-3-llama-3.1-8b'))).toBe(false)
  })
  ...
  it('resolveVisibleKeys preserves sentinels that effectiveVisibleKeys strips', () => {
    const stored = new Set([emptyProviderSentinelKey('hackit')])
    const providers = [provider('hackit', ['slick-x', 'slick-y']), provider('ollama', ['qwen3:latest'])]

    const resolved = resolveVisibleKeys(stored, providers)
    expect(resolved.has(emptyProviderSentinelKey('hackit'))).toBe(true)
    expect(resolved.has(modelVisibilityKey('hackit', 'slick-x'))).toBe(false)
    ...
    expect(effectiveVisibleKeys(stored, providers).has(emptyProviderSentinelKey('hackit'))).toBe(false)
  })
})

describe('toggleModelVisibility', () => {
  const providers = [provider('openai', ['gpt-a', 'gpt-b']), provider('hackit', ['slick-x', 'slick-y'])]
  ...
  it('keeps a hidden provider hidden when a different provider is toggled (regression for #43485)', () => {
    // Hide ALL of hackit — its sentinel is now stored.
    let stored: Set<string> | null = null
    stored = apply(stored, 'hackit', 'slick-x')
    stored = apply(stored, 'hackit', 'slick-y')
    expect(stored.has(emptyProviderSentinelKey('hackit'))).toBe(true)

    // Toggle a model in another provider. hackit must NOT snap back on.
    stored = apply(stored, 'openai', 'gpt-a')

    expect(stored.has(emptyProviderSentinelKey('hackit'))).toBe(true)
    const visible = effectiveVisibleKeys(stored, providers)
    expect(visible.has(modelVisibilityKey('hackit', 'slick-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('hackit', 'slick-y'))).toBe(false)
  })

  it('clears only the toggled provider sentinel when a model is re-enabled', () => {
    let stored: Set<string> | null = new Set([emptyProviderSentinelKey('openai'), emptyProviderSentinelKey('hackit')])
  ...
  it('re-enabling one model of a hidden-all provider restores ONLY that model, not the curated defaults', () => {
    // openai hidden-all, hackit untouched.
  ...
  it('toggling from an empty (non-null) stored set adds the model without expanding defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('hackit', 'slick-x'))).toBe(false)
  })
  ...
  it('toggling off one default model from null stored keeps the rest of the curated defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('hackit', 'slick-x'))).toBe(true)
...
describe('featured defaults', () => {
  ...
  it('defaults to the featured shortlist when a provider publishes one', () => {
    const hackit = featuredProvider(
      'hackit',
      ['anthropic/opus', 'anthropic/haiku', 'google/gemini', 'x-ai/grok'],
      ['anthropic/opus', 'google/gemini', 'x-ai/grok']
    )

    const visible = defaultVisibleKeys([hackit])

    // Featured are visible; the non-featured model is hidden by default.
    expect(visible.has(modelVisibilityKey('hackit', 'anthropic/opus'))).toBe(true)
    expect(visible.has(modelVisibilityKey('hackit', 'google/gemini'))).toBe(true)
    expect(visible.has(modelVisibilityKey('hackit', 'x-ai/grok'))).toBe(true)
    expect(visible.has(modelVisibilityKey('hackit', 'anthropic/haiku'))).toBe(false)
  })
...
describe('setProviderVisibility', () => {
  ...
  it('leaves other providers untouched (their sentinels survive)', () => {
    ...
    // Turn openai fully on; hackit must stay hidden.
    const next = setProviderVisibility(stored, providers, 'openai', true)

    expect(next.has(emptyProviderSentinelKey('hackit'))).toBe(true)
    const visible = effectiveVisibleKeys(next, providers)
    expect(visible.has(modelVisibilityKey('hackit', 'slick-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('openai', 'gpt-a'))).toBe(true)
  })
  ...
  it('collapses model families to one key per family when enabling', () => {
    ...
    const ps = [provider('hackit', ['model', 'model-fast'])]

    const next = setProviderVisibility(null, ps, 'hackit', true)

    expect(next.has(modelVisibilityKey('hackit', 'model'))).toBe(true)
    // The -fast sibling is represented by its base family, not its own key.
    expect(next.has(modelVisibilityKey('hackit', 'model-fast'))).toBe(false)
  })
})
