...
/**
 * Feature store for backend (agent) plugins — the native Slick plugins plus
 ...
 * primitives `slick plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Slick. */
  portable?: boolean
}
 ...
 *  SLICK_HOME. Always refetches (it's a cheap local disk scan on the
