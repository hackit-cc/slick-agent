import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@slick/shared'
import { atom } from 'nanostores'

import type { SlickConnection } from '@/global'
import { SlickGateway, setApiRequestConnection } from '@/slick'
...
const isOpen = (gateway: SlickGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: SlickConnection) => void
  /**
...
interface Secondary {
  ...
  connection: SlickConnection | null
  gateway: SlickGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: SlickGateway | null
  ...
  $gateway: ReturnType<typeof atom<SlickGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('slick.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<SlickGateway | null>(null),
...
export function setPrimaryGateway(gateway: SlickGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<SlickConnection, 'connectionId'> | null): void {
...
export function activeGateway(): SlickGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the slick module (null
...
function publishActiveConnection(connection: SlickConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new SlickGateway()
...
async function gatewayForProfile(
  profile: string,
  leaseRequest = false
): Promise<{ gateway: SlickGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Slick gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.slickDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Slick Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<SlickGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Slick gateway is not connected" on the Sessions `+` action (#88880).
