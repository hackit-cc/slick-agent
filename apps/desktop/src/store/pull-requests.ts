...
import type { SlickBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/slick'
...
export const $pullRequestsByBranch = atom<Record<string, SlickBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'slick.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('slick.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: SlickBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
