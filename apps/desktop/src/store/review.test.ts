...
// reaches the gateway); everything else routes through window.slickDesktop.git,
...
function file(path: string, over: Partial<SlickReviewFile> = {}): SlickReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as SlickReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.slickDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as SlickReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.slickDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: SlickReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as SlickReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { slickDesktop?: unknown }).slickDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as SlickReviewShipInfo)
