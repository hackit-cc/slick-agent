...
import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@slick/shared'
...
const TILES_KEY = 'slick.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'slick.desktop.sessionTiles.v1'
 ...
 * restore re-dials the profile's backend, whose ensure_slick_home() re-creates
 * the profile directory the delete just removed (slick-agent#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (slick-agent#94235). Tiles persisted
...
export const $focusedStoredSessionId = computed(
  ...
  (groupId, tree, selected, workspaceMode) => {
    ...
    // it to the sidebar group, whose active pane ('slick-bots:pane') is not a
...
// Dev hook for automation (mirrors __SLICK_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__SLICK_SESSION_TILES__ = {
