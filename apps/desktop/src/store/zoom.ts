...
export function setZoomPercent(percent: number): void {
  window.slickDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.slickDesktop?.zoom) {
  void window.slickDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.slickDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
