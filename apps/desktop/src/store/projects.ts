...
import type { SlickGitBaseBranch, SlickGitBranch } from '@/global'
import { getSlickConfig, slickApi, type SlickGateway } from '@/slick'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/slick'
...
const PROJECT_SCOPE_KEY = 'slick.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Slick gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active Slick profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await slickApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<SlickGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<SlickGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Slick
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getSlickConfig(context.profile))
...
// repo, returning where Slick should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<SlickGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<SlickGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.slickDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.slickDesktop?.writeClipboard?.(path)
  }
}
