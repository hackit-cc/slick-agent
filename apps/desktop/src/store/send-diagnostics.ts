...
// user explicitly clicks Upload → diagnostics.share_hackit runs backend-side
// (collect + force-redact + Hackit-S3 upload) → the modal shows the private
// view link plus the support handoff (GitHub Issues · Hackit Portal Support ·
// Discord).
//
// Consent is per-upload and explicit — no "always allow", mirroring the CLI's
// `slick debug share --hackit` confirmation contract. On a remote connection
...
export async function confirmSendDiagnostics(): Promise<void> {
  ...
  try {
    ...
    const response = await gateway.request<ShareHackitResponse>(
      'diagnostics.share_hackit',
      {
