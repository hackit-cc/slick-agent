...
// user explicitly clicks Upload → diagnostics.share_sophos runs backend-side
// (collect + force-redact + Sophos-S3 upload) → the modal shows the private
// view link plus the support handoff (GitHub Issues · Sophos Portal Support ·
// Discord).
//
// Consent is per-upload and explicit — no "always allow", mirroring the CLI's
// `slick debug share --sophos` confirmation contract. On a remote connection
...
async function collectLocalExtras(): Promise<Record<string, string>> {
  try {
    const logs = await window.slickDesktop?.getRecentLogs?.()
...
export async function confirmSendDiagnostics(): Promise<void> {
  ...
  try {
    ...
    const response = await gateway.request<ShareSophosResponse>(
      'diagnostics.share_sophos',
      {
