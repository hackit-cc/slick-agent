...
const agentConn = (over: Partial<SlickConnection> = {}): SlickConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as SlickConnection

const localConn = (over: Partial<SlickConnection> = {}): SlickConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as SlickConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<SlickConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<SlickConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { slickDesktop: { getConnection, getConnectionFor } })
})
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<SlickConnection>(() => undefined))
