...
const { getProfiles } = await import('@/slick')
...
const remoteConn = (over: Partial<SlickConnection> = {}): SlickConnection =>
  ({ baseUrl: 'https://slick-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as SlickConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { slickDesktop: { getConnection } })
