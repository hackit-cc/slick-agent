...
import type { SlickReviewFile, SlickReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Slick' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'slick.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'slick.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'slick.desktop.reviewTreeMode'
const SELECTED_KEY = 'slick.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<SlickReviewFile[]>([])
...
export const $reviewShipInfo = atom<SlickReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['slickDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: SlickReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly SlickReviewFile[], path: string): SlickReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.slickDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.slickDesktop?.openExternal?.(url)
    }
