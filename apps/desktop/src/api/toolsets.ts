...
export interface SelectToolsetProviderResponse {
  ...
  capability?: string
  /** Present (true) when a managed Sophos row was selected but the Portal
   *  entitlement is missing — the row won't activate until the user signs
   *  in to Sophos Portal. */
  needs_sophos_auth?: boolean
  /** The managed feature key (e.g. "browser") when needs_sophos_auth is set. */
  feature?: string
}
