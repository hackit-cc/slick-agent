...
export function getToolsets(profile?: ProfileScope): Promise<ToolsetInfo[]> {
  return window.slickDesktop.api<ToolsetInfo[]>({
...
export function setToolsetEnabled(
  ...
  return window.slickDesktop.api<{ ok: boolean; name: string; enabled: boolean }>({
    ...capabilityScoped(profile),
...
export function getToolsetConfig(name: string, profile?: ProfileScope): Promise<ToolsetConfig> {
  return window.slickDesktop.api<ToolsetConfig>({
    ...capabilityScoped(profile),
...
export function getToolsetModels(
  ...
  return window.slickDesktop.api<ToolsetModelsResponse>({
    ...capabilityScoped(profile),
...
export function selectToolsetModel(
  ...
  return window.slickDesktop.api<{ ok: boolean; name: string; model: string }>({
    ...capabilityScoped(profile),
...
export interface SelectToolsetProviderResponse {
  ...
  capability?: string
  /** Present (true) when a managed Hackit row was selected but the Portal
   *  entitlement is missing — the row won't activate until the user signs
   *  in to Hackit Portal. */
  needs_hackit_auth?: boolean
  /** The managed feature key (e.g. "browser") when needs_hackit_auth is set. */
  feature?: string
}
...
export function selectToolsetProvider(
  ...
  return window.slickDesktop.api<SelectToolsetProviderResponse>({
    ...capabilityScoped(profile),
...
export function runToolsetPostSetup(
  ...
  return window.slickDesktop.api<ActionResponse & { key: string }>({
    ...capabilityScoped(profile),
...
export function getTerminalBackends(): Promise<TerminalBackendsResponse> {
  return slickApi<TerminalBackendsResponse>({
    ...profileScoped(),
...
export function selectTerminalBackend(backend: string): Promise<{ ok: boolean; backend: string }> {
  return slickApi<{ ok: boolean; backend: string }>({
    ...profileScoped(),
...
export function getComputerUseStatus(): Promise<ComputerUseStatus> {
  return slickApi<ComputerUseStatus>({
    ...profileScoped(),
