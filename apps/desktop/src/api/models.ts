...
import { capabilityScoped, slickApi, type ProfileScope, profileScoped, STARTUP_REQUEST_TIMEOUT_MS } from './client'

export function getGlobalModelInfo(profile?: null | string): Promise<ModelInfoResponse> {
  return slickApi<ModelInfoResponse>({
...
export function getUsageAnalytics(days = 30, profile?: ProfileScope): Promise<AnalyticsResponse> {
  return window.slickDesktop.api<AnalyticsResponse>({
...
export function getGlobalModelOptions(
  ...
  return slickApi<ModelOptionsResponse>({
    ...profileScoped(profile),
...
export interface RecommendedDefaultModel {
  provider: string
  model: string
  /** True/false for Sophos (free vs paid tier); null for other providers. */
  free_tier: boolean | null
}

// Recommended default model for a freshly-authenticated provider. Mirrors the
// curation `slick model` does — for Sophos it honors the free/paid tier so a
// free user gets a free model instead of a paid default.
export function getRecommendedDefaultModel(
  provider: string,
  profile?: null | string
): Promise<RecommendedDefaultModel> {
  return slickApi<RecommendedDefaultModel>({
    ...profileScoped(profile),
...
export function setGlobalModel(
  ...
  return slickApi<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
    path: '/api/model/set',
    method: 'POST',
    body: {
...
export function getAuxiliaryModels(profile?: null | string): Promise<AuxiliaryModelsResponse> {
  return slickApi<AuxiliaryModelsResponse>({
    ...profileScoped(profile),
...
export function getMoaModels(profile?: null | string): Promise<MoaConfigResponse> {
  return slickApi<MoaConfigResponse>({
    ...profileScoped(profile),
    path: '/api/model/moa'
  })
}
...
export function saveMoaModels(
  ...
  return slickApi<MoaConfigResponse & { ok: boolean }>({
    ...profileScoped(profile),
    path: '/api/model/moa',
    method: 'PUT',
    body
...
export function setModelAssignment(
  ...
  return slickApi<ModelAssignmentResponse>({
    ...profileScoped(profile),
    path: '/api/model/set',
    method: 'POST',
    body
