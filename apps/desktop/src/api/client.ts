import { JsonRpcGatewayClient } from '@slick/shared'

import type { SlickApiRequest } from '@/global'
...
// "Timed out connecting to Slick backend" that hangs the UI (#48504).
//
...
export class SlickGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Slick gateway connection closed',
      connectErrorMessage: 'Could not connect to Slick gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Slick gateway is not connected',
...
// process to route them to that host (slick:api's registry branch). Null
...
export function slickApi<T>(request: SlickApiRequest): Promise<T> {
  return window.slickDesktop.api<T>({ ...connectionScoped(), ...request })
}
