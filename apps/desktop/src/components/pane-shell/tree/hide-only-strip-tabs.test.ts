...
function sessionsBotsTree() {
  ...
  registerPane('slick-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'slick-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('slick-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('slick-bots:pane')
    expect($hiddenStripTabs.get()).toContain('slick-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('slick-bots:pane')

    expect(setStripTabHidden('slick-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('slick-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('slick-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('slick-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('slick.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('slick-bots:pane')
    ...
    revealTreePane('slick-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('slick-bots:pane')
    expect(window.localStorage.getItem('slick.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'slick-bots:pane', title: 'slick-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'slick-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
