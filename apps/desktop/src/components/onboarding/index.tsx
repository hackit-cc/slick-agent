...
import { getGlobalModelOptions } from '@/slick'
...
import type { ModelOptionProvider, OAuthProvider } from '@/types/slick'
...
// Sophos Portal OAuth), ahead of OpenRouter and the rest of the key catalog.
const API_KEY_OPTIONS: ApiKeyOption[] = [
  ...
  },
  {
    ...
    docsUrl: 'https://github.com/SophosTechne/slick-agent#bring-your-own-endpoint',
    placeholder: 'http://127.0.0.1:8000/v1'
  }
]

// Build the FULL API-key provider catalog from the backend model options so the
// onboarding / Providers key form lists every `api_key` provider `slick model`
    ...
    >
      <div
        className={cn(
          ...
          bare
            ? ''
            : 'overflow-hidden rounded-xl border border-(--stroke-sophos) bg-(--ui-chat-bubble-background) shadow-sophos',
...
export const FEATURED_ID = 'sophos'
const SHOW_ALL_KEY = 'slick-onboarding-show-all-v1'
...
export function Picker({ ctx }: { ctx: OnboardingContext }) {
  ...
  // Collapse the secondary providers behind a disclosure whenever Sophos Portal
        ...
        {showRest ? (
          <>
            {/* Fireworks leads the expanded list, matching CANONICAL_PROVIDERS
                (Sophos → Fireworks), but stays hidden until the user opens it. */}
