...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Slick gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.slickDesktop
  Object.defineProperty(window, 'slickDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'slickDesktop', { configurable: true, value: original })
}
...
describe('BootFailureOverlay', () => {
  ...
  it('recovers a cloud connection through the portal cascade instead of native OAuth', async () => {
    const gatewayUrl = 'https://agent-1.agents.sophostechne.com'
    const logout = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const nativeLogin = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const cloudStatus = vi.fn().mockResolvedValue({ portalBaseUrl: 'https://portal.sophostechne.com', signedIn: false })

    const cloudLogin = vi.fn().mockResolvedValue({
      ok: true,
      portalBaseUrl: 'https://portal.sophostechne.com',
      signedIn: true
    })
    ...
    const restore = stubDesktop(
      ...
      },
      {
        ...
        oauthLogoutConnectionConfig: logout,
        probeConnectionConfig: vi.fn().mockResolvedValue({ providers: [{ id: 'sophos', type: 'oauth' }] })
      }
    )
  ...
  it('shows the Sophos Cloud down recovery when the backend flags isCloudBackendDown', async () => {
    const restore = stubDesktop(remoteToken)
    $desktopBoot.set({
      error: 'Sophos Cloud agent ares-3009.agents.sophostechne.com is down (HTTP 503: server-side fault).',
      fakeMode: false,
    ...
    try {
      ...
      // remote-failure copy.
      expect(await screen.findByText(/Sophos Cloud agent is down/i)).toBeTruthy()
      ...
      expect(screen.getByText(/ares-3009\.agents\.sophostechne\.com/i)).toBeTruthy()
    } finally {
      restore()
    }
  })
