...
describe('BootFailureOverlay', () => {
  ...
  it('recovers a cloud connection through the portal cascade instead of native OAuth', async () => {
    const gatewayUrl = 'https://agent-1.agents.hackit.cc'
    const logout = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const nativeLogin = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const cloudStatus = vi.fn().mockResolvedValue({ portalBaseUrl: 'https://portal.hackit.cc', signedIn: false })

    const cloudLogin = vi.fn().mockResolvedValue({
      ok: true,
      portalBaseUrl: 'https://portal.hackit.cc',
      signedIn: true
    })
    ...
    const restore = stubDesktop(
      ...
      },
      {
        ...
        oauthLogoutConnectionConfig: logout,
        probeConnectionConfig: vi.fn().mockResolvedValue({ providers: [{ id: 'hackit', type: 'oauth' }] })
      }
    )
  ...
  it('shows the Hackit Cloud down recovery when the backend flags isCloudBackendDown', async () => {
    const restore = stubDesktop(remoteToken)
    $desktopBoot.set({
      error: 'Hackit Cloud agent ruby-3009.agents.hackit.cc is down (HTTP 503: server-side fault).',
      fakeMode: false,
    ...
    try {
      ...
      // remote-failure copy.
      expect(await screen.findByText(/Hackit Cloud agent is down/i)).toBeTruthy()
      ...
      expect(screen.getByText(/ruby-3009\.agents\.hackit\.com/i)).toBeTruthy()
    } finally {
      restore()
    }
  })
