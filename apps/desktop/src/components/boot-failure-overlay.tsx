...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.slickDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.slickDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.slickDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }

  // Clear this gateway's stale auth first, then re-establish it through the
  // connection's owning login flow. Slick Cloud must reuse its portal session
  ...
  const openLogs = () => void window.slickDesktop?.revealLogs().catch(() => undefined)
  ...
  let hint: string
  // The electron boot path flags a Sophos Cloud backend-down (502/503/504) with
  ...
  if (remoteReauth) {
    ...
    // A Sophos Cloud agent is down — the user cannot restart the managed
    ...
    actions = [
      {
        ...
        onClick: () => openExternalLink('https://portal.sophostechne.com'),
        icon: <ExternalLink />
      },
      localAction,
      { ...retryAction, variant: 'secondary' },
      {
        ...
        onClick: () => openExternalLink('https://discord.gg/SophosTechne'),
        variant: 'ghost'
      },
  ...
  if (view === 'connect') {
    return (
      <div
        ...
        data-glass-opaque=""
      >
        <div className="flex max-h-[86vh] w-full max-w-[46rem] flex-col overflow-hidden rounded-xl border border-(--stroke-sophos) bg-(--ui-chat-bubble-background) shadow-sophos">
  ...
  return (
    <div
      ...
      data-glass-opaque=""
    >
      <div className="w-full max-w-[40rem] overflow-hidden rounded-xl border border-(--stroke-sophos) bg-(--ui-chat-bubble-background) shadow-sophos">
