...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's SLICK_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.slickDesktop?.logsRoot?.()
      ...
      const result = await window.slickDesktop?.openDir?.(root)
