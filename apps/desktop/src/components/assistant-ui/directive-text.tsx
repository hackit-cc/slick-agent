...
const SLICK_REF_TYPES = WIRE_REFERENCE_KINDS
type SlickRefType = (typeof SLICK_REF_TYPES)[number]
...
const SLICK_DIRECTIVE_RE = referenceRe()
...
export const slickDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(SLICK_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...slickDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Slick directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.slickDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.slickDesktop?.readFileDataUrl(id)
