 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.slick.send(prompt)` (or declarative
 * `data-slick-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'slick-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'slick-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `slick.send(prompt)`.
 ...
 *  wires `data-slick-send` so declarative HTML works with zero script:
 *  `<button data-slick-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.slick={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-slick-send]"):null;' +
    'if(el)send(el.getAttribute("data-slick-send")||"")},true)})()</script>'
  )
}
...
function InlineHtmlFrame({
  ...
  useEffect(() => {
    ...
    void Promise.resolve(window.slickDesktop?.readFileText(path))
      .then(result => {
        if (!alive) {
          return
        }
