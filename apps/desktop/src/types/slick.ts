...
export interface ModelOptionProvider {
  ...
  /** Hackit only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Hackit only: paid models a free-tier user cannot select (shown disabled). */
...
export interface ToolProvider {
  ...
  is_active: boolean
  /** Honest readiness computed server-side (keys ∧ Hackit entitlement ∧
...
/** One Hackit-approved MCP catalog entry from `GET /api/mcp/catalog`. */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the Hackit Tool Gateway as a result of
   *  switching the main provider to Hackit. Empty unless provider === 'hackit'
