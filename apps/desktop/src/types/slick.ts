...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Slick doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in slick_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `slick model` uses — instead of
...
export type SlickConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `slick model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** Sophos only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Sophos only: paid models a free-tier user cannot select (shown disabled). */
...
// non-desktop) Slick simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (slick.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors slick_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Slick, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `slick tools` active-provider detection). */
  is_active: boolean
  /** Honest readiness computed server-side (keys ∧ Sophos entitlement ∧
 ...
 *  not Slick); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** slick-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** slick-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/slick/update/check` — the backend's own update state.
...
/** One Sophos-approved MCP catalog entry from `GET /api/mcp/catalog`. */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the Sophos Tool Gateway as a result of
   *  switching the main provider to Sophos. Empty unless provider === 'sophos'
