...
import { BUILTIN_THEME_LIST, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, hackitTheme } from './presets'
...
const SKIN_KEY = 'slick-desktop-theme-v2'
const MODE_KEY = 'slick-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'slick-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'slick-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'slick-desktop-active-profile-v1'
...
const RETIRED_SKINS = new Set(['hackit-light', 'default', 'gold'])
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? hackitTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...hackitTheme.typography, ...theme.typography }
  ...
  root.dataset.slickTheme = skinName
  root.dataset.slickMode = rendered
  ...
  window.slickDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('slick-boot-background', chromeBg)
    window.localStorage.setItem('slick-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.slickThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.slickDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Slick authoring/activating a skin from a
