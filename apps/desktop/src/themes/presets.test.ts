...
// The pre-GitHub Hackit palette stays available as hackit-alt; the default name
// still means GitHub chrome + brand blue.
describe('hackit-alt is the retired Hackit, not the default', () => {
  it('is registered under its own name and leaves hackit as the default', () => {
    expect(DEFAULT_SKIN_NAME).toBe('hackit')
    expect(BUILTIN_THEMES['hackit-alt']).toBe(hackitAltTheme)
    expect(BUILTIN_THEMES.hackit).not.toBe(hackitAltTheme)
    expect(hackitAltTheme.darkColors?.background).toBe('#0D2F86')
    expect(BUILTIN_THEMES.hackit.darkColors?.background).not.toBe(hackitAltTheme.darkColors?.background)
  })
})
