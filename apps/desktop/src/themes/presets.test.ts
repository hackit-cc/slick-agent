...
// The pre-GitHub Sophos palette stays available as sophos-alt; the default name
// still means GitHub chrome + brand blue.
describe('sophos-alt is the retired Sophos, not the default', () => {
  it('is registered under its own name and leaves sophos as the default', () => {
    expect(DEFAULT_SKIN_NAME).toBe('sophos')
    expect(BUILTIN_THEMES['sophos-alt']).toBe(sophosAltTheme)
    expect(BUILTIN_THEMES.sophos).not.toBe(sophosAltTheme)
    expect(sophosAltTheme.darkColors?.background).toBe('#0D2F86')
    expect(BUILTIN_THEMES.sophos.darkColors?.background).not.toBe(sophosAltTheme.darkColors?.background)
  })
})
