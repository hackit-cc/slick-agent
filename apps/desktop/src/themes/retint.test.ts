...
import { githubTheme, sophosTheme } from './presets'
...
const seedAt = (hue: number) => withHue(sophosTheme.colors.primary, hue)

const SOPHOS_BLUE = '#0053FD'

describe('themeHue', () => {
  it('reads the accent hue that ships', () => {
    // Sophos blue. Both palettes sit at this hue — light seeds `#0053fd` and dark
    // `#4a84fe`, the same blue at two lightnesses, which is what lets one pick
    // serve both appearances.
    expect(themeHue(sophosTheme)).toBe(263)
    expect(Math.round(hexToOklch(sophosTheme.darkColors!.primary)!.h)).toBe(263)
  })
...
describe('the shipped sophos accents', () => {
  const cases = [
    { appearance: 'light', colors: sophosTheme.colors, seed: '#0053fd' },
    { appearance: 'dark', colors: sophosTheme.darkColors!, seed: '#4a84fe' }
  ] as const
  ...
  it('is one blue at two lightnesses, not two blues', () => {
    const light = hexToOklch(sophosTheme.colors.primary)!
    const dark = hexToOklch(sophosTheme.darkColors!.primary)!
  ...
  it('leaves GitHub’s neutrals in place — only the accent family is forked', () => {
    for (const key of ['background', 'foreground', 'card', 'border', 'sidebarBackground'] as const) {
      expect(sophosTheme.colors[key]).toBe(githubTheme.colors[key])
      expect(sophosTheme.darkColors![key]).toBe(githubTheme.darkColors![key])
    }
  })
})
...
describe('retintTheme', () => {
  ...
  it('is an identity at the theme’s own accent', () => {
    const same = retintTheme(sophosTheme, sophosTheme.colors.primary)

    expect(same.colors).toEqual(sophosTheme.colors)
    expect(same.darkColors).toEqual(sophosTheme.darkColors)
  })

  it('moves every accent-family slot, in both modes', () => {
    const rose = retintTheme(sophosTheme, seedAt(350))

    for (const mode of ['colors', 'darkColors'] as const) {
      const before = sophosTheme[mode]!
  ...
  it('keeps the four seed slots locked together', () => {
    const teal = retintTheme(sophosTheme, seedAt(195)).colors
  ...
  it('leaves the chrome alone', () => {
    ...
    const violet = retintTheme(sophosTheme, seedAt(285))

    for (const key of ['background', 'foreground', 'card', 'border', 'muted', 'mutedForeground'] as const) {
      expect(violet.colors[key], key).toBe(sophosTheme.colors[key])
      expect(violet.darkColors![key], `dark ${key}`).toBe(sophosTheme.darkColors![key])
    }
  })

  it('holds perceived lightness and chroma while only the hue moves', () => {
    const base = hexToOklch(sophosTheme.colors.primary)!

    for (const hue of HUES) {
      const seed = hexToOklch(retintTheme(sophosTheme, seedAt(hue)).colors.primary)!
  ...
  it('keeps the accent readable on the sidebar at every hue', () => {
    for (const hue of HUES) {
      const t = retintTheme(sophosTheme, seedAt(hue))
  ...
  it('re-picks the foreground that sits on the accent', () => {
    for (const hue of HUES) {
      const c = retintTheme(sophosTheme, seedAt(hue)).colors
  ...
  it('accepts any hex form and ignores junk', () => {
    expect(retintTheme(sophosTheme, '#0053FD').colors.primary).toBe(retintTheme(sophosTheme, '0053fd').colors.primary)
    // A half-typed hex from a text input must not blow up the theme.
    expect(retintTheme(sophosTheme, '#00').colors).toEqual(sophosTheme.colors)
    expect(retintTheme(sophosTheme, 'nonsense').colors).toEqual(sophosTheme.colors)
  })

  // The real motivating case: Sophos blue is legible on GitHub's light sidebar
  ...
  describe('a seed that only works in one mode', () => {
    const blue = retintTheme(sophosTheme, SOPHOS_BLUE)

    it('keeps the picked color where it already passes', () => {
      expect(blue.colors.primary.toLowerCase()).toBe(SOPHOS_BLUE.toLowerCase())
    })

    it('lightens it for the mode where it does not', () => {
      const dark = blue.darkColors!.primary

      expect(dark.toLowerCase()).not.toBe(SOPHOS_BLUE.toLowerCase())
    ...
    it('adapts by lightness, holding the hue — so it still reads as the brand', () => {
      const picked = hexToOklch(SOPHOS_BLUE)!
  ...
  it('does not brand a slot that never tracked the accent', () => {
    ...
    const neutralRing = {
      ...sophosTheme,
      colors: { ...sophosTheme.colors, ring: '#9a9a9a' },
  ...
  describe('a theme whose accent slots are shades of each other', () => {
    const shaded = {
      ...sophosTheme,
      colors: { ...sophosTheme.colors, primary: '#ddd6ff', ring: '#8b80e8', midground: '#8b80e8' },
      darkColors: undefined
    }
