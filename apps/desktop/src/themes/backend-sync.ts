/**
 * Live skin sync from the Slick backend.
 *
 * The backend resolves the active skin (built-in or `$SLICK_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Slick
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { SlickSkin } from '@slick/shared/skin'
 ...
 */
export function ingestBackendSkin(skin: SlickSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  // (hackit), so we never register a converted theme under `default`. It is still a
  // valid apply TARGET though: a runtime switch back to `default` must repaint the
  // desktop to its own default (setTheme normalizes `default` → hackit). So we only
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as SlickSkin)
