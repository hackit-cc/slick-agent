...
describe('requestTheme', () => {
  ...
  it('activates a theme contributed through the registry', () => {
    const testbed: DesktopTheme = { ...midnightTheme, description: 'Testbed', label: 'Testbed', name: 'testbed' }
    const dispose = registry.register({ area: THEMES_AREA, data: testbed, id: 'testbed' })

    renderProbe()

    let accepted = false
    act(() => {
      accepted = requestTheme('testbed')
    })

    expect(accepted).toBe(true)
    expect(ctx.themeName).toBe('testbed')
