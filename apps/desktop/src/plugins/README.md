...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `SlickPlugin` and
...
`$SLICK_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`slick-desktop-plugins` skill.
