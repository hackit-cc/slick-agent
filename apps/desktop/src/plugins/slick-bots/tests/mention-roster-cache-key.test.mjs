...
function load({ cacheKeyConnection = 'local' } = {}) {
  ...
  queryClient.setQueryData(['slick-bots', 'roster', cacheKeyConnection], ROSTER)
    ...
    host: {
      ...
      requestProfile: async (route, method, params) => {
        ...
        if (method === 'profiles.list') {
          return {
            profiles: [{ name: 'default', ui_meta: { 'slick-bots': { chat: 'remote-pin' } } }]
          }
        }
...
test('the roster cache entry lives under the connection-suffixed key, invisible to the bare-key read', () => {
  ...
  assert.equal(queryClient.getQueryData(['slick-bots', 'roster']), undefined)
  assert.equal(queryClient.getQueriesData({ queryKey: ['slick-bots', 'roster'] }).length, 1)
})
...
test('the middleware identifies a remote @name-device mention without delivering (identification-only)', async () => {
  ...
  assert.doesNotMatch(result.text, /slick -p '?default/)
