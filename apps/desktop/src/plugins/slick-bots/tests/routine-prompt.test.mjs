...
test('integration: a different active profile retains the delegated routine wrapper', () => {
  ...
  assert.match(prompt, /slick -p 'research' chat/)
...
test('security: delegated routine arguments remain literal shell values', () => {
  ...
  const command = prompt.slice(prompt.indexOf('slick '), prompt.lastIndexOf('\n\nIf the command'))
  const script = `slick() { printf '%s\\037' "$@"; }\n${command}`
...
test('security: upgrade pauses persisted delegated routines before they can execute', async () => {
  const titleSentinel = `/tmp/slick-bot-mode-title-${process.pid}`
  const taskSentinel = `/tmp/slick-bot-mode-task-${process.pid}`
  ...
  const oldPrompt =
    ...
    `slick -p research chat -c "Routine: ${title}" -q ${JSON.stringify(`[Scheduled routine] ${instruction}`)}\n\n` +
  ...
  const command = recreated.slice(recreated.indexOf('slick '), recreated.lastIndexOf('\n\nIf the command'))
  const script = `slick() { printf '%s\\037' "$@"; }\n${command}`
...
test('regression: Create Cronjob passes the active profile to routinePrompt', () => {
  ...
  assert.doesNotMatch(__routines.routinePrompt('ops', 'Check', instruction, 'ops'), /slick -p/)
})
