...
test('render: BotRow previews the pinned canonical chat, not an unrelated latest session', () => {
  // slick-agent#88200: the row opens the pinned chat on click, so the
...
test('previewKind: the primary profile surfaces as @slick, never @default', () => {
  ...
  assert.equal(fromBotOf("Message from agent 'default': deploy is green"), 'slick')
})
