...
function load(turnScript, { busyUntilResumeCall, clarifyUntilResumeCall, approvalUntilResumeCall, conflictOnce = false, deferredTimers = false } = {}) {
      ...
      request: async (method, params) => {
        ...
        if (method === 'profiles.configure') {
          if (conflictOnce && !injectedConflict) {
            ...
            sharedUiMeta['slick-bots-groups'] = {
              version: 2,
              rooms: {
                Shared: {
                  revision: 1,
            ...
            uiMetaRevisions['slick-bots-groups'] = 1
            return {
              applied: {
                ui_meta: false,
                ui_meta_conflicts: { 'slick-bots-groups': { expected: 0, actual: 1 } },
                ui_meta_revisions: { 'slick-bots-groups': 1 }
              }
            }
          }
  ...
  const source = pluginSource
    .replace(/^import\s+\*\s+as\s+sdk\s+from '@slick\/plugin-sdk'\r?\n/m, '')
    .replace(/^import\s+\{[\s\S]*?\}\s+from '@slick\/plugin-sdk'\r?\n/m, '')
...
test('room plumbing sessions carry the room_plumbing contract', async () => {
  ...
  // (GH #89497 — room bots stuck on Sophos after a provider switch).
...
test('bot sessions carry the follow-profile-config contract (canonical DM + room)', () => {
  ...
  // (the "out of Sophos credits" DM bug after a profile switch).
...
test('group room messages and members mirror through bounded gateway profile metadata', async () => {
  ...
  const envelope = configure.params.ui_meta['slick-bots-groups']
...
test('an explicit final-room disband may clear the gateway room mirror', async () => {
  ...
  const configure = gc.requests.filter(call => call.method === 'profiles.configure').at(-1)
  assert.deepEqual(Object.keys(configure.params.ui_meta['slick-bots-groups'].rooms), [])
  assert.ok(configure.params.ui_meta['slick-bots-groups'].deleted['name:Research'] > 0)
})
...
test('sync worker retries a gateway CAS conflict and publishes the merged room', async () => {
  ...
  for (let i = 0; i < 20 && (gc.uiMetaRevisions['slick-bots-groups'] || 0) < 2; i++) {
    await new Promise(resolve => setImmediate(resolve))
  }

  const stored = gc.sharedUiMeta['slick-bots-groups']
  assert.equal(gc.uiMetaRevisions['slick-bots-groups'], 2)
  assert.equal(
...
test('fan-out class: a room write is mirrored to every reachable default-profile gateway', async () => {
  ...
  gc.host.requestProfile = async (route, method) => {
    ...
    if (method === 'profiles.configure') {
      return { applied: { ui_meta: true, ui_meta_revisions: { 'slick-bots-groups': 1 } } }
    }
    return {}
  }
...
test('default profile speaks as Slick in room transcripts, not @default', () => {
  const gc = load(() => '(pass)')
  const line = gc.formatGroupChatLine({ from: { kind: 'member', name: 'default' }, text: 'hello room' }, 'builder')
  assert.equal(line, 'Slick: hello room')
  ...
  assert.equal(you, 'Slick (you): hi')
  const plain = gc.formatGroupChatLine({ from: { kind: 'member', name: 'builder' }, text: 'yo' }, 'research')
  assert.equal(plain, 'builder: yo')
})

test('speaker labels honor friendly identity: Bot Mode title, then display_name, never a stale Slick', () => {
  const gc = load(() => '(pass)')

  // A renamed default (core display_name via `slick profile rename`) must
  // read as its new name — the community report was "Lucy" still showing
  // "Slick is thinking…" in group rooms.
  ...
  // Untitled rows keep today's behavior: default → Slick, others verbatim.
  gc.$botMeta.set({})
  assert.equal(gc.groupSpeakerLabel('default'), 'Slick')
...
test('turn prompt addresses the default profile as @slick', () => {
  const gc = load(() => '(pass)')
  ...
  assert.match(prompt, /You are @slick,/)
  ...
  assert.match(peerView, /group chat with @slick/)
})

test('mention routing: @slick resolves to the default member', () => {
  const gc = load(() => '(pass)')
  const members = [{ name: 'default', title: '' }, { name: 'builder', title: '' }]
  const parsed = gc.parseGroupChatMentions('@slick take a look', members)
...
test('source contract: workspace speaker labels use displayName with a click-to-reveal handle', () => {
  // Speaker labels come from the roster displayName (default → Slick)…
...
test('group room preview renders the bot HANDLE, not the raw profile name', () => {
  ...
  // @slick, so users concluded mention routing was broken.
