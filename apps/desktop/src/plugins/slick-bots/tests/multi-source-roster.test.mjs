...
function runtime() {
  ...
  const code = source
    .replace(/^import \* as sdk from '@slick\/plugin-sdk'\r?\n/m, '')
...
test('default rows use source identity without borrowing another source title', () => {
  ...
  // label. Titled: the title wins. Untitled: it stays "Slick". Regression:
  ...
  assert.equal(name(active, undefined), 'Slick')
})
...
test('botHandle: precomputed multi-source handle wins; default stays slick', () => {
  ...
  assert.equal(botHandle('default'), 'slick')
})
...
test('displayName: local default stays Slick; remote default uses the device label', () => {
...
test('resolveRosterMentions: @slick in this chat is not a handoff to yourself', () => {
  const { __resolveRosterMentions: resolve } = runtime()
  const roster = [
    { name: 'default', connectionId: 'local', handle: 'slick' },
    {
      name: 'default',
      connectionId: 'mac-mini',
      connectionLabel: 'Mac Mini',
  ...
  const hits = resolve('ask @slick and @default-mac-mini', roster, {
    name: 'default',
    connectionId: 'local'
  })
