...
// Bot Mode title or `slick profile rename` display_name — must update what
...
test('mentionNameForms: slugged + collapsed, reserved tokens dropped', () => {
  ...
  // A bot renamed "Slick" or "everyone" cannot hijack reserved tags.
  assert.equal(mentionNameForms('Slick').length, 0)
...
test('botMentionTag: renamed slug wins, profile handle is the fallback', () => {
  ...
  assert.equal(botMentionTag({ name: 'default' }), 'slick')
  // display_name (slick profile rename) drives the tag too.
...
test('parseGroupChatMentions: display_name and ui_meta title forms address a member', () => {
  ...
  const members = [
    ...
    { name: 'ops', title: '', ui_meta: { 'slick-bots': { title: 'Night Watch' } } }
  ]
