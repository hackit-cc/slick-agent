...
function createCanonicalChat(owner, { kickoff = false, openingStillCurrent = null } = {}) {
  ...
  const run = (async () => {
    ...
    const res = await requestForBot(bot, 'session.create', {
      ...
      // stale provider — e.g. "out of Hackit credits" — after a profile switch).
    ...
    return jsxs('div', {
      ...
      children: [
        labeled(
          'Provider',
          jsx(Input, {
            placeholder: 'omnirouter / 9router / hackit \u2026',
...
// https://slick-agent.hackit.cc/docs/skills?embed=picker hides the
...
const HUB_ORIGIN = 'https://slick-agent.hackit.cc'
