/**
 * Slick Bot Mode — a "one chat per agent" roster for the Slick desktop.
 *
 * Left pane "Bots": one row per Slick profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Slick cron jobs) scoped to the
 ...
 * bot-initiated sends use `slick -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import * as sdk from '@slick/plugin-sdk'
import {
  atom,
  Button,
  Checkbox,
...
const BOT_ATTENTION_HINTS = {
  ...
  missing_config: 'Provider not configured — run slick model',
...
/** Mirrors host.paneVisibility('slick-bots:pane') — wired in register(). */
...
const GROUP_CHAT_SYNC_META_KEY = 'slick-bots-groups'
 ...
 *  clients (Slick Go, headless backends) see rooms regardless of which
...
// to (local, remote URL, SSH, Slick Cloud, docker) must be able to find
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['slick-bots']?.title || profile?.display_name || ''),
...
async function saveBotMeta(owner, patch) {
  ...
  try {
    ...
    const request = route ? requestForBot(bot, 'profiles.configure', { name, ui_meta: { 'slick-bots': rest } }) :
      host.request('profiles.configure', { name, ui_meta: { 'slick-bots': rest } })
...
// handoffs (`slick -p <bot> chat --in ~ -c "Bot Chat" --create-if-missing`)
  ...
  for (const bot of roster) {
    ...
    if (image && typeof image === 'string' && image.startsWith('data:')) {
      ...
      Promise.resolve(request)
        .then(() => queryClient.invalidateQueries({ queryKey: ['slick-bots', 'roster'] }))
...
function mergeServerMeta(roster, fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['slick-bots']
...
/** Permanently delete a bot's Slick profile, then remove plugin-local state
 ...
 * directory (slick-agent#52279). That is the "can't delete a bot" error. */
...
if (typeof document !== 'undefined' && !document.getElementById('slick-bots-roster-css')) {
  const style = document.createElement('style')
  style.id = 'slick-bots-roster-css'
  style.textContent =
    '.slick-bots-roster [data-radix-scroll-area-viewport] > div {' +
    ' display: block !important; width: 100%; min-width: 0; }' +
    '.slick-scroll-cap > [data-radix-scroll-area-viewport] { max-height: inherit; }' +
    '@keyframes slick-bots-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.35; } }' +
    '.slick-bots-pulse { animation: slick-bots-pulse 1.2s ease-in-out infinite; }'
...
// and the row falls back to the "run slick mcp / Settings" hint. profile is
...
function McpSetupButton({ profile, entry, onDone, ensureProfile }) {
  ...
  const beginOAuth = async () => {
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.slickDesktop && window.slickDesktop.openExternal) {
        window.slickDesktop.openExternal(authUrl)
      } else {
      ...
      tab === 'generate'
        ? imagen
          ...
          : jsx('div', {
              ...
              children:
                imagen === false
                  ? 'No image model available. If you just enabled one (or updated Slick), restart the gateway: Ctrl+K → "Restart gateway".'
...
function PetTab({ image, onImage }) {
  ...
  if (!pets.length) {
    return jsx('div', {
      ...
      children: 'No pets in the petdex gallery. Run `slick pets` to explore.'
    })
  }
 ...
 *  protocol into the system prompt itself (slick-agent bot_mode_probe).
...
function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (slick-agent #86875) also expose the union
 ...
 *  callable alias is 'slick' — the mention middleware resolves it back to
...
function botHandle(name, bot) {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'slick' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`slick profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Slick"
 *  can never hijack the primary profile's @slick alias. */
function mentionNameForms(value) {
  ...
  return [...new Set([slug, collapsed])].filter(
    form => /^[a-z0-9][a-z0-9_-]*$/.test(form) && !['all', 'everyone', 'user', 'default', 'slick'].includes(form)
  )
}
...
function botFriendlyNames(bot) {
  ...
  return [bot?.ui_meta?.['slick-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
function newBotChat(bot) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({ kind: 'error', message: 'Update Slick Desktop to open another Bot chat.' })
...
function resolveRosterMentions(text, roster, active = {}) {
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    ...
    if (token === 'slick') {
      token = byForm.has('slick') ? 'slick' : token
    }
...
// raw Cloud hostname, or to generic 'Slick' in Cloud-only mode.
//
...
// grandfather, or recover. Stored-id pins (ui_meta['slick-bots'].chat) were
...
async function openStoredBotChat(owner, storedId, summary) {
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This Slick Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (slick-agent#89617) — clicking Retry
  ...
  // until an app restart (slick-agent#93604). A resume is cheap and
...
function createCanonicalChat(owner, { kickoff = false, openingStillCurrent = null } = {}) {
  ...
  const run = (async () => {
    ...
    const res = await requestForBot(bot, 'session.create', {
      ...
      // stale provider — e.g. "out of Sophos credits" — after a profile switch).
...
async function prepareBotSource(bot) {
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error('Update Slick Desktop to chat with agents on other connections.')
  }
...
async function ensureBotMetadata(bot) {
  ...
  const server = row?.ui_meta?.['slick-bots']
...
function displayName(bot, meta) {
  ...
  // "Slick". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `slick profile rename
  ...
  // never generic "Slick" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Slick (the agent it is)
...
function durableGroupChatMembers(bots) {
  return (bots || []).map(bot => {
    ...
    const title = String(botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['slick-bots']?.title || bot.title || '').trim()
...
// turn in its OWN persistent per-group Slick session and is fed only the
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Slick"
 *  (community report, Aug 21 2026: renamed default still read "Slick is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Slick (matching displayName and the
 *  @slick handle) so the main agent never loses its name in rooms. */
function groupSpeakerLabel(name) {
  ...
  // Core profile display_name (`slick profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Slick' : trimmed
}
...
function messagingProtocolSection(name, roster) {
  ...
  return [
    ...
    '',
    '```',
    'slick -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from \uD83E\uDD16 ' + handle + ' (@' + handle + '): your message"',
    '',
    ...
    '',
    'The roster grows over time — run `slick profile list` for the LIVE',
...
function previewKind(preview) {
  ...
  if (match) {
    ...
    // profile reads @slick, never @default (#89484).
 ...
 *  (slick-agent#90268). Older gateways omit worker_session — always false. */
    ...
    return jsxs('div', {
      ...
      children: [
        labeled(
          'Provider',
          jsx(Input, {
            placeholder: 'omnirouter / 9router / sophos \u2026',
...
function AdvancedProfileConfig({ bot, state, setState }) {
  ...
  if (unsupported) {
    return jsx('div', {
      className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
      children: 'Full configuration needs a newer gateway (restart it after updating Slick).'
    })
  }
  ...
  // (slick-agent#87317): Skills (installed list + one-click hub installs +
...
// https://slick-agent.sophostechne.com/docs/skills?embed=picker hides the
// docs chrome and adds "+ Add to this Agent" per card, posting
// {type: 'slick-skill-pick', ...} to us (slick-agent#86243). We validate
...
const HUB_ORIGIN = 'https://slick-agent.sophostechne.com'
...
function HubSkillsSection({ forProfile, onInstalled }) {
  ...
  useEffect(() => {
    ...
    const onMessage = event => {
      ...
      if (!data || data.type !== 'slick-skill-pick' || !data.name) {
        return
      }
  ...
  const ensureAgentCreated = () => {
    ...
    const flight = singleFlight(flightRef, async () => {
      ...
      if (remoteTarget) {
        ...
        try {
          void requestForTarget('profiles.configure', { name: slug, ui_meta: { 'slick-bots': look } }).catch(() => undefined)
                    ...
                    advTab === 'general'
                      ...
                      : capsFailed
                        ? jsx('div', {
                            className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
                            children:
                              'Capability catalog needs a newer gateway (restart it after updating Slick).'
                          })
                        : !caps
                          ? jsx('div', {
                              className: 'flex justify-center py-4',
...
// slick -p <bot> chat delegation wrapper so the run reaches that bot's
...
function routinePrompt(bot, title, instruction, activeProfile) {
  ...
  return (
    ...
    `slick -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Slick-native schedule string; Advanced exposes it raw.
const FREQUENCIES = [
...
/** Compose the Slick schedule string from picker state. */
...
function GroupChatWorkspace({ group, members, onBack, visible = true }) {
  ...
  const renderEntry = (entry, index) => {
                  ...
                  // primary "default" profile renders as Slick, remote dupes
...
function BotsHomeView() {
  ...
  const gateway = bot.connectionLabel || (bot.connectionId === 'local' ? 'This device' : 'Slick gateway')
...
function GroupRow({ active, group, members, needsYou, onOpen, onDisband }) {
  ...
  // and the group prompt: the primary profile is @slick, not @default.
...
function BotsPane() {
  ...
  const activityOf = bot => {
    const created = botRosterMeta(bot, allMeta)?.created || bot.ui_meta?.['slick-bots']?.created || 0
        ...
        : error && !roster.length
          ? jsxs('div', {
              ...
              children: [
                jsx('div', {
                  children: gatewayUp
                    ? `Roster unavailable: ${error instanceof Error ? error.message : 'gateway error'}. If your gateway predates profiles.list, update Slick and restart the gateway.`
          ...
          : roster.length === 0
            ...
            : allBotsHidden && !hiddenExpanded
              ...
              : rosterRows.length === 0 && matchingHiddenBots.length === 0
                ...
                : jsx(ScrollArea, {
                    className: 'slick-bots-roster min-h-0 flex-1',
                    children: jsx('div', {
        ...
        description: deleting
          ? jsxs('span', {
              children: [
                ...
                ' and its associated Slick profile at ',
...
export default {
  ...
  register(ctx) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @slick.
    ctx.register({
    ...
    if (!document.getElementById('slick-bots-keyframes')) {
      const style = document.createElement('style')
      style.id = 'slick-bots-keyframes'
      style.textContent = '@keyframes slick-bots-bob { from { transform: translateY(0); } to { transform: translateY(-3px); } }'
      document.head.appendChild(style)
    }
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.slickDesktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.slickDesktop?.connections?.onChanged?.(payload => {
