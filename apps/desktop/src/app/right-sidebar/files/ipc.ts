...
import type { SlickReadDirEntry, SlickReadDirResult } from '@/global'
...
export type ProjectTreeEntry = SlickReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: SlickReadDirEntry) {
...
async function filterIgnored(entries: SlickReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<SlickReadDirResult> {
  if (!window.slickDesktop) {
