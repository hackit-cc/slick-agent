...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { slickDesktop: { readDir: typeof readDir } }).slickDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): SlickReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { slickDesktop: unknown }).slickDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: SlickReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SlickReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: SlickReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: SlickReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: SlickReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SlickReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.slickDesktop is missing', async () => {
    delete (window as unknown as { slickDesktop?: unknown }).slickDesktop
