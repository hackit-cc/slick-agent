...
const SLICK_PATHS_MIME = 'application/x-slick-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(SLICK_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(SLICK_PATHS_MIME)
  ...
  const getPath = window.slickDesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.slickDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (slick --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.slickDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[slick-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.slickDesktop?.terminal?.write(sessionId, `${command}\r`)
