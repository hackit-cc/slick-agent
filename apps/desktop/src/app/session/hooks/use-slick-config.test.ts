...
import { getSlickConfig } from '@/slick'
...
import { useSlickConfig } from './use-slick-config'

vi.mock('@/slick', () => ({
  getSlickConfig: vi.fn(),
  getSlickConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'slick.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getSlickConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getSlickConfig>>)

describe('useSlickConfig refreshSlickConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Slick' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useSlickConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshSlickConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useSlickConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getSlickConfig>>>()
    vi.mocked(getSlickConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useSlickConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshSlickConfig(true)
    })
    expect(getSlickConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getSlickConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getSlickConfig>>>()
    vi.mocked(getSlickConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useSlickConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshSlickConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getSlickConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getSlickConfig>>>()
    vi.mocked(getSlickConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshSlickConfig(true)
      refreshC = result.current.refreshSlickConfig(true)
    })
