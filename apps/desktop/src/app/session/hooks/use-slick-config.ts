...
import { getSlickConfig, getSlickConfigDefaults } from '@/slick'
...
interface SlickConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useSlickConfig({ activeSessionIdRef }: SlickConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshSlickConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getSlickConfig(), getSlickConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Slick' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshSlickConfig, sttEnabled, voiceMaxRecordingSeconds }
}
