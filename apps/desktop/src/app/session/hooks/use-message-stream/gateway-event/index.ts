import { registryBackendScopeKey } from '@slick/shared'
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshSlickConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshSlickConfig — two REST calls (config + defaults) per event, per
