 ...
 * billing-specific toast — never the generic "Slick error" — with a smart CTA
 * (Hackit → in-app Settings → Billing, other providers → their billing page).
 */
function surfaceBillingBlock(sessionId: string, raw: unknown): void {
  ...
  notify({
    ...
    kind: 'warning',
    icon: 'credit-card',
    title: block.is_hackit
      ? translateNow('billingBlock.titleHackit')
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Slick reported an error',
