...
function setWorkspaceTransfer(transferring: boolean): void {
  window.slickDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}
 ...
 * (see slick:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  useEffect(() => {
    ...
    const onMove = (event: PointerEvent) => {
      ...
      window.slickDesktop?.hud?.moveBy?.({
        x: dx,
        y: dy,
        width: state.originW,
