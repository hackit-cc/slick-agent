...
import { getLatestSessionMessages } from '@/slick'
...
import { useSlickConfig } from '../session/hooks/use-slick-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshSlickConfig, sttEnabled, voiceMaxRecordingSeconds } = useSlickConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('slick:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('slick:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshSlickConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshSlickConfig])
