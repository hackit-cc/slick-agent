...
import { pathFromSlickDeepLink, resolveSlickOpenPath } from '@/lib/slick-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.slickDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.slickDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.slickDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.slickDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // slick://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.slickDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future slickDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveSlickOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.slickDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (slick://index-network/intent/1, slick://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromSlickDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.slickDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.slickDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.slickDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.slickDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
