...
import { getSlickConfigRecord, saveMcpServers } from '@/slick'
...
import { setSlickConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `slick://mcp/install` deep links. The payload is
 ...
 * confirming lets Slick spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getSlickConfigRecord())
      ...
      setSlickConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
