...
import { getSlickConfigRecord, type ProfileScope, profileScopeKey } from '@/slick'
...
//
// Distinct from session/hooks/use-slick-config.ts, which is side-effecting —
// it pushes personality/cwd/voice/… into the session stores for live chat.
export const SLICK_CONFIG_KEY = ['slick-config-record'] as const
...
// share a cache row.
export const slickConfigKey = (profile?: ProfileScope) =>
  profile == null ? SLICK_CONFIG_KEY : ([...SLICK_CONFIG_KEY, profileScopeKey(profile)] as const)
...
export const useSlickConfigRecord = (profile?: ProfileScope) =>
  useQuery({
    queryKey: slickConfigKey(profile),
    ...
    queryFn: () => getSlickConfigRecord(profile ?? undefined),
    staleTime: 0
  })

// setSlickConfigCache writes the app-wide (base-key) record. Pass a profile to
...
export const setSlickConfigCache = writeCache<SlickConfigRecord>(SLICK_CONFIG_KEY)
export const slickConfigCacheWriter = (profile?: ProfileScope) =>
  writeCache<SlickConfigRecord>(slickConfigKey(profile))

export const invalidateSlickConfig = (profile?: ProfileScope) =>
  queryClient.invalidateQueries({ queryKey: slickConfigKey(profile) })
