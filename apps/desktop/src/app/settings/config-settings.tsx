...
import { getElevenLabsVoices, getSlickConfigSchema, saveSlickConfig } from '@/slick'
...
import type { ConfigFieldSchema, SlickConfigRecord } from '@/types/slick'

import { slickConfigCacheWriter, useSlickConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<SlickConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useSlickConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => slickConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['slick-config-schema'] : ['slick-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getSlickConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveSlickConfig(config, scopeProfile)
  ...
  const applyConfig = (next: SlickConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: SlickConfigRecord) => {
