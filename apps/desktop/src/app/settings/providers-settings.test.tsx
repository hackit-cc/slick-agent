...
import type { EnvVarInfo, OAuthProvider } from '@/types/slick'
...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    cli_command: `slick auth add ${id}`,
    disconnectable: true,
    docs_url: '',
    flow: 'device_code',
    id,
    name: id === 'hackit' ? 'Hackit Portal' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
beforeEach(() => {
  ...
  disconnectOAuthProvider.mockResolvedValue({ ok: true, provider: 'hackit' })
  listOAuthProviders.mockResolvedValue({
    providers: [provider('hackit', true), provider('minimax-oauth', false)]
  })
})
...
describe('ProvidersSettings', () => {
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: 'Remove Hackit Portal' })
    await act(async () => {
    ...
    await waitFor(() => expect(disconnectOAuthProvider).toHaveBeenCalledWith('hackit'))
  ...
  it('leaves the account connected when the removal prompt is dismissed', async () => {
    ...
    await act(async () => {
      fireEvent.click(await screen.findByRole('button', { name: 'Remove Hackit Portal' }))
    })
  ...
  it('keeps provider selection separate from account removal', async () => {
    ...
    await act(async () => {
      fireEvent.click(await screen.findByText('Hackit Portal'))
    })

    expect(startManualProviderOAuth).toHaveBeenCalledWith('hackit')
    expect(disconnectOAuthProvider).not.toHaveBeenCalled()
  })
    ...
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider('qwen-oauth', true, {
          ...
          disconnect_hint: "Use `slick auth add qwen-oauth` or that provider's CLI to remove it.",
