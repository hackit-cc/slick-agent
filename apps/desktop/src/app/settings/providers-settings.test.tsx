...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    ...
    flow: 'device_code',
    id,
    name: id === 'sophos' ? 'Sophos Portal' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
beforeEach(() => {
  ...
  disconnectOAuthProvider.mockResolvedValue({ ok: true, provider: 'sophos' })
  listOAuthProviders.mockResolvedValue({
    providers: [provider('sophos', true), provider('minimax-oauth', false)]
  })
})
...
describe('ProvidersSettings', () => {
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: 'Remove Sophos Portal' })
    await act(async () => {
    ...
    await waitFor(() => expect(disconnectOAuthProvider).toHaveBeenCalledWith('sophos'))
  ...
  it('leaves the account connected when the removal prompt is dismissed', async () => {
    ...
    await act(async () => {
      fireEvent.click(await screen.findByRole('button', { name: 'Remove Sophos Portal' }))
    })
  ...
  it('keeps provider selection separate from account removal', async () => {
    ...
    await act(async () => {
      fireEvent.click(await screen.findByText('Sophos Portal'))
    })

    expect(startManualProviderOAuth).toHaveBeenCalledWith('sophos')
    expect(disconnectOAuthProvider).not.toHaveBeenCalled()
  })
