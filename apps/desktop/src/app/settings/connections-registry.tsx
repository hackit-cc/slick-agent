 ...
 * agent sources (local runtime + any number of remote gateways / Slick Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.slickDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.slickDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `slick update` to every eligible source; per-connection results
