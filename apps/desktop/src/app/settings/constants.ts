...
// lib/personalities (mirrors slick_cli/personality.py BUILTIN_PERSONALITIES).
...
export const EMPTY_SELECT_VALUE = '__slick_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'SOPHOS_',
    name: 'Sophos Portal',
    description: 'Hosted Slick & Sophos-trained models',
    docsUrl: 'https://portal.sophostechne.com',
    priority: 0
  },
  {
    prefix: 'FIREWORKS_',
    ...
    // Slot #2 — mirrors CANONICAL_PROVIDERS (after Sophos, ahead of OpenRouter).
  ...
  { prefix: 'SLICK_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // slick_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  browser: {
    useRealProfile:
      "Local browsing uses your real logins. Slick copies your default browser's profile (cookies, logins, preferences) into a managed snapshot and drives it with its packaged Chromium — your live profile is never opened directly, and the copy is refreshed from it on each run. Also lets the agent open a local real-profile session on request even when a cloud browser backend is configured. Only Chromium browsers (Chrome, Edge, Brave, Chromium) are supported; a non-Chromium default fails with a clear message. Off by default."
  },
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Slick stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Slick can read from one file request.',
  approvals: {
    mode: 'How Slick handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Slick updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
