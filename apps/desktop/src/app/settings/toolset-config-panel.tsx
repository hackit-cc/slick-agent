 ...
 * server-computed `status` (keys ∧ Sophos entitlement ∧ post-setup install
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange, profile }: ToolsetConfigPanelProps) {
  ...
  // Guard the Sophos Portal sign-in poll loop against unmount/state updates.
  ...
  // panel highlighted the first keyless provider (e.g. Sophos Portal) even when
  ...
  async function handleSelect(provider: ToolProvider) {
    ...
    try {
      ...
      if (result.needs_sophos_auth) {
        // Managed Sophos row selected without Portal entitlement: the config
        ...
        // sign-in action). Reuses the existing Sophos Portal device-code flow.
        notify({
          kind: 'warning',
          title: copy.sophosAuthNeededTitle,
          message: copy.sophosAuthNeededMessage(provider.name),
          action: { label: copy.sophosAuthSignIn, onClick: () => void signInToSophosPortal() }
        })
  ...
  // Drive the existing Sophos Portal OAuth device-code flow (the same session
  ...
  async function signInToSophosPortal() {
    try {
      const start = await startOAuthLogin('sophos', profile)

      if (start.flow !== 'device_code') {
        notifyError(new Error(`unexpected flow: ${start.flow}`), copy.sophosAuthFailed)
      ...
      for (let attempt = 0; attempt < 120 && mountedRef.current; attempt += 1) {
        ...
        const polled = await pollOAuthSession('sophos', start.session_id, profile)

        if (polled.status === 'approved') {
          notify({ kind: 'success', title: copy.sophosAuthDoneTitle, message: copy.sophosAuthDoneMessage })
          await refresh()
          onConfiguredChange?.()
        ...
        if (polled.status !== 'pending') {
          notifyError(new Error(polled.error_message || `Sign-in ${polled.status}`), copy.sophosAuthFailed)
      ...
      if (mountedRef.current) {
        notifyError(err, copy.sophosAuthFailed)
      }
    }
  }
          ...
          <div className="overflow-hidden rounded-xl bg-background/60" key={provider.name}>
            ...
            {isExpanded && (
              <div className="grid gap-2 bg-muted/20 p-3">
                ...
                {provider.requires_sophos_auth && (
                  <p className="text-[0.72rem] text-muted-foreground">{copy.sophosIncluded}</p>
                )}
