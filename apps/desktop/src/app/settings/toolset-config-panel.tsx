 ...
 * server-computed `status` (keys ∧ Hackit entitlement ∧ post-setup install
 ...
 * log inline — the GUI equivalent of the install step `slick tools` runs
...
/**
 * Backend model catalog — the GUI counterpart of the model picker `slick
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange, profile }: ToolsetConfigPanelProps) {
  ...
  // Guard the Hackit Portal sign-in poll loop against unmount/state updates.
  ...
  // panel highlighted the first keyless provider (e.g. Hackit Portal) even when
  ...
  async function handleSelect(provider: ToolProvider) {
    ...
    try {
      ...
      if (result.needs_hackit_auth) {
        // Managed Hackit row selected without Portal entitlement: the config
        ...
        // sign-in action). Reuses the existing Hackit Portal device-code flow.
        notify({
          kind: 'warning',
          title: copy.hackitAuthNeededTitle,
          message: copy.hackitAuthNeededMessage(provider.name),
          action: { label: copy.hackitAuthSignIn, onClick: () => void signInToHackitPortal() }
        })
  ...
  // Drive the existing Hackit Portal OAuth device-code flow (the same session
  ...
  async function signInToHackitPortal() {
    try {
      const start = await startOAuthLogin('hackit', profile)

      if (start.flow !== 'device_code') {
        notifyError(new Error(`unexpected flow: ${start.flow}`), copy.hackitAuthFailed)
      ...
      if (window.slickDesktop?.openExternal) {
        try {
          await window.slickDesktop.openExternal(url)
        } catch {
      ...
      for (let attempt = 0; attempt < 120 && mountedRef.current; attempt += 1) {
        ...
        const polled = await pollOAuthSession('hackit', start.session_id, profile)

        if (polled.status === 'approved') {
          notify({ kind: 'success', title: copy.hackitAuthDoneTitle, message: copy.hackitAuthDoneMessage })
          await refresh()
          onConfiguredChange?.()
        ...
        if (polled.status !== 'pending') {
          notifyError(new Error(polled.error_message || `Sign-in ${polled.status}`), copy.hackitAuthFailed)
      ...
      if (mountedRef.current) {
        notifyError(err, copy.hackitAuthFailed)
      }
    }
  }
          ...
          <div className="overflow-hidden rounded-xl bg-background/60" key={provider.name}>
            ...
            {isExpanded && (
              <div className="grid gap-2 bg-muted/20 p-3">
                ...
                {provider.requires_hackit_auth && (
                  <p className="text-[0.72rem] text-muted-foreground">{copy.hackitIncluded}</p>
                )}
