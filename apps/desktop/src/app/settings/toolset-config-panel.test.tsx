...
import type { ToolsetConfig } from '@/types/slick'
...
const getSlickConfigRecord = vi.fn()
const getSlickConfigSchema = vi.fn()
const saveSlickConfig = vi.fn()
...
vi.mock('@/slick', () => ({
  ...
  getSlickConfigRecord: () => getSlickConfigRecord(),
  getSlickConfigSchema: () => getSlickConfigSchema(),
  saveSlickConfig: (config: unknown) => saveSlickConfig(config),
...
beforeEach(() => {
  ...
  getSlickConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getSlickConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
  saveSlickConfig.mockResolvedValue({ ok: true })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveSlickConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveSlickConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
  ...
  describe('readiness pills', () => {
    it('renders the server status instead of assuming keyless rows are Ready', async () => {
      // The false-Ready bug: a logged-out Sophos Subscription row and a
  ...
  describe('managed Sophos provider activation', () => {
    const sophosBrowserConfig = () =>
      config({
        name: 'browser',
        active_provider: null,
        providers: [
    ...
    it('surfaces a sign-in notice when the PUT reports needs_sophos_auth', async () => {
      ...
      // reports needs_sophos_auth and the panel must surface a sign-in action
      // instead of the misleading "provider selected" success toast.
      const { notify } = await import('@/store/notifications')

      getToolsetConfig.mockResolvedValue(sophosBrowserConfig())
      selectToolsetProvider.mockResolvedValue({
        ok: true,
        name: 'browser',
        provider: 'Sophos Subscription (Browser Use cloud)',
        needs_sophos_auth: true,
        feature: 'browser'
      })
      ...
      // The single Sophos row auto-expands; activate via the explicit button.
      await screen.findByRole('button', { name: /Sophos Subscription/ })
      fireEvent.click(await screen.findByRole('button', { name: /Use this backend/ }))

      await waitFor(() =>
        expect(selectToolsetProvider).toHaveBeenCalledWith('browser', 'Sophos Subscription (Browser Use cloud)')
      )
      await waitFor(() =>
    ...
    it('drives the existing Sophos OAuth device-code flow from the sign-in action and refetches', async () => {
      const { notify } = await import('@/store/notifications')
      ...
      startOAuthLogin.mockResolvedValue({
        ...
        verification_url: 'https://portal.sophostechne.com/device?user_code=SOPHOS-1234',
        poll_interval: 5,
        expires_in: 600
      })
      ...
      try {
        ...
        await waitFor(() => expect(startOAuthLogin).toHaveBeenCalledWith('sophos'))
        ...
        await waitFor(() => expect(pollOAuthSession).toHaveBeenCalledWith('sophos', 'sess-1'), { timeout: 8000 })
