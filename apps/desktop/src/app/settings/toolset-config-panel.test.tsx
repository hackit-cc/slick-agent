...
describe('ToolsetConfigPanel', () => {
  ...
  describe('readiness pills', () => {
    it('renders the server status instead of assuming keyless rows are Ready', async () => {
      // The false-Ready bug: a logged-out Hackit Subscription row and a
  ...
  describe('managed Hackit provider activation', () => {
    const hackitBrowserConfig = () =>
      config({
        name: 'browser',
        active_provider: null,
        providers: [
    ...
    it('surfaces a sign-in notice when the PUT reports needs_hackit_auth', async () => {
      ...
      // reports needs_hackit_auth and the panel must surface a sign-in action
      // instead of the misleading "provider selected" success toast.
      const { notify } = await import('@/store/notifications')

      getToolsetConfig.mockResolvedValue(hackitBrowserConfig())
      selectToolsetProvider.mockResolvedValue({
        ok: true,
        name: 'browser',
        provider: 'Hackit Subscription (Browser Use cloud)',
        needs_hackit_auth: true,
        feature: 'browser'
      })
      ...
      // The single Hackit row auto-expands; activate via the explicit button.
      await screen.findByRole('button', { name: /Hackit Subscription/ })
      fireEvent.click(await screen.findByRole('button', { name: /Use this backend/ }))

      await waitFor(() =>
        expect(selectToolsetProvider).toHaveBeenCalledWith('browser', 'Hackit Subscription (Browser Use cloud)')
      )
      await waitFor(() =>
    ...
    it('drives the existing Hackit OAuth device-code flow from the sign-in action and refetches', async () => {
      const { notify } = await import('@/store/notifications')
      ...
      startOAuthLogin.mockResolvedValue({
        ...
        verification_url: 'https://portal.hackit.cc/device?user_code=HACKIT-1234',
        poll_interval: 5,
        expires_in: 600
      })
      ...
      try {
        ...
        await waitFor(() => expect(startOAuthLogin).toHaveBeenCalledWith('hackit'))
        ...
        await waitFor(() => expect(pollOAuthSession).toHaveBeenCalledWith('hackit', 'sess-1'), { timeout: 8000 })
