...
import { disconnectOAuthProvider, listOAuthProviders } from '@/slick'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.slickDesktop?.terminal)
...
//      in slick_cli/provider_catalog.py) — the SAME provider identity
//      `slick model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Slick can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Slick can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Slick never deletes creds another tool owns behind a silent API call.
