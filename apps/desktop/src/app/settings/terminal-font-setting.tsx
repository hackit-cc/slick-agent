...
import { saveSlickConfig } from '@/slick'
import { useI18n } from '@/i18n'
import { notifyError } from '@/store/notifications'
import type { SlickConfigRecord } from '@/types/slick'

import { setSlickConfigCache, useSlickConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: SlickConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = useSlickConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<SlickConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setSlickConfigCache, so
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="slick-terminal-font-families"
          ...
          <datalist id="slick-terminal-font-families">
