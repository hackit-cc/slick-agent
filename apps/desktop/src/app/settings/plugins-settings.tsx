...
// User-installed plugins first — mirrors `slick plugins list --user`.
...
function reveal(file: string) {
  void window.slickDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // slick_home breaks against a remote backend (#66899).
    const dir = await window.slickDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.slickDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's slick home (profile-aware), so the
// path comes from the gateway — not from the renderer's local SLICK_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.slickDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Slick (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
