...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'sophos', model: 'slick-4' })
  ...
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'sophos', model: 'slick-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ ok: true, provider: 'sophos', model: 'slick-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'sophos', model: 'slick-4', free_tier: null })
...
describe('ModelSettings', () => {
  it('loads the current main model and lists configured providers only', async () => {
    ...
    // "Sophos" shows in both the trigger and the open list.
    expect((await screen.findAllByText('Sophos')).length).toBeGreaterThan(0)
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'sophos', model: 'slick-4' })
    ...
    await waitFor(() => expect(screen.getAllByRole('combobox')[0].textContent).toContain('Sophos'))
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'sophos', model: 'slick-4' }]
    })
    ...
    expect(screen.getByText('sophos')).toBeTruthy()
  })
        ...
        expect.objectContaining({
          presets: expect.objectContaining({
            default: expect.objectContaining({
              reference_models: [
                expect.objectContaining({ provider: 'sophos', model: 'slick-4', enabled: false }),
