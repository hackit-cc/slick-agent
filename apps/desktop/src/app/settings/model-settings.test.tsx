...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'hackit', model: 'slick-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Hackit',
        slug: 'hackit',
        models: ['slick-4', 'slick-4-mini'],
        authenticated: true,
        capabilities: { 'slick-4': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'hackit', model: 'slick-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ ok: true, provider: 'hackit', model: 'slick-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'hackit', model: 'slick-4', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getSlickConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
...
describe('ModelSettings', () => {
  it('loads the current main model and lists configured providers only', async () => {
    ...
    // "Hackit" shows in both the trigger and the open list.
    expect((await screen.findAllByText('Hackit')).length).toBeGreaterThan(0)
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'hackit', model: 'slick-4' })
    ...
    await waitFor(() => expect(screen.getAllByRole('combobox')[0].textContent).toContain('Hackit'))
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getSlickConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveSlickConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['slick-4'],
          authenticated: true,
          capabilities: { 'slick-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'hackit', model: 'slick-4' }]
    })
    ...
    expect(screen.getByText('hackit')).toBeTruthy()
  })
        ...
        expect.objectContaining({
          presets: expect.objectContaining({
            default: expect.objectContaining({
              reference_models: [
                expect.objectContaining({ provider: 'hackit', model: 'slick-4', enabled: false }),
