...
import { slickDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
function sentLabel(draft: string) {
  return slickDirectiveFormatter
    .parse(draft)
...
describe('one label per reference, on every surface', () => {
  ...
  it('an agent mention inserts as plain @name text, never an @simple: chip', () => {
    ...
    expect(slickDirectiveFormatter.serialize(item)).toBe('@mr-tester')
  ...
  it('a url still reads host + path on every surface', () => {
    const item = backendRow('@url:https://github.com/SophosTechne/slick-agent/pull/74533', '', '')
    const { editor, result } = typed('@gith')

    act(() => result.current.replaceTriggerWithChip(item))

    const expected = 'github.com/SophosTechne/slick-agent/pull/74533'
