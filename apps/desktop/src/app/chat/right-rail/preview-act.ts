...
const preamble = () => `  var w = window;
  ...
  w.__slickActHolder = w.__slickActHolder || {};
  w.__slickAct = ${actEngineSource()};
  w.__slickWatch_fn = (${watchInPage.toString()});
  w.__slickWatchTag = ${WATCH_TAG};
  var holder = w.__slickActHolder;
  var act = function (a) { return w.__slickAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__slickWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__slickHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'SLICK-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
