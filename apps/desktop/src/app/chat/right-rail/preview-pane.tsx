        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.slickDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Slick to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.slickDesktop?.watchPreviewFile ||
      !window.slickDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.slickDesktop.onPreviewFileChanged(payload => {
    ...
    void window.slickDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.slickDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.slickDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:slick-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.slickDesktop?.zoom?.factor?.() || 1
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.slickDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.slickDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.slickDesktop?.openExternal(currentUrl)
                : undefined
            }
