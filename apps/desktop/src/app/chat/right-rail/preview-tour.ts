...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__slickTourEngine) {
    ${driverIife}
    w.__slickTourHolder = {};
    w.__slickTourCollect = (${collectTourTargets.toString()});
    w.__slickTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__slick-tour-style')) {
    var style = document.createElement('style');
    style.id = '__slick-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__slickTourEngine(
    w.driver.js.driver,
    w.__slickTourHolder,
    ${JSON.stringify(action)},
    w.__slickTourCollect,
    document
  ));
})()`
}
