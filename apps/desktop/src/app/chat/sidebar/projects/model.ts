...
import type { SessionInfo } from '@/slick'
...
// zero Slick sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, SlickGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, SlickGitWorktree[]>>({})
