import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@slick/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as SlickGateway | null
  const gatewayRef = useRef<SlickGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.slickDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<SlickGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as SlickGateway | null
      }),
    []
  )
