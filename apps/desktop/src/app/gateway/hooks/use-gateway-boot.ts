import { isGatewayReauthRequired, JsonRpcGatewayError, resolveGatewayWsUrl } from '@slick/shared'
import { useEffect, useRef } from 'react'

import { shouldApplyPostBootProgressError } from '@/components/boot-failure-reauth'
import type { SlickConnection } from '@/global'
import { SlickGateway } from '@/slick'
...
import type { RpcEvent } from '@/types/slick'
...
export function primaryRuntimeConnectionId(connection: Pick<SlickConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.slickDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: SlickGateway | null) => void
  refreshSlickConfig: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: SlickConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Slick...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Slick…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Slick backend'
        )
        ...
        // as an opaque "Could not connect to Slick gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshSlickConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshSlickConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startSlick re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new SlickGateway()
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting Slick…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshSlickConfig(),
          ...
          // retry it), not brick boot behind the "Slick couldn't start"
