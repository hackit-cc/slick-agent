...
vi.mock('@/slick', async importOriginal => {
  const actual = await importOriginal<typeof SlickModule>()

  class FakeSlickGateway {
  ...
  return { ...actual, SlickGateway: FakeSlickGateway }
})

import type * as SlickModule from '@/slick'
import type { SlickGateway } from '@/slick'
import {
  $gateway,
...
const fakeGateway = { connectionState: 'open' } as unknown as SlickGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as SlickGateway, 'default')
  $gateway.set(primary as unknown as SlickGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.slickDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as SlickGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { slickDesktop: unknown }).slickDesktop = { getConnection }
    $gateway.set(dropped)
