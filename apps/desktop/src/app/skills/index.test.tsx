...
import type * as SlickApi from '@/slick'
...
vi.mock('@/slick', async importOriginal => ({
  ...(await importOriginal<typeof SlickApi>()),
...
describe('SkillsView toolset management', { timeout: 60_000 }, () => {
    ...
    await act(async () => {
      window.dispatchEvent(
        new MessageEvent('message', {
          data: { type: 'slick-skill-pick', name: 'web-research', identifier: 'web-research' },
          origin: 'https://slick-agent.sophostechne.com'
        })
      )
    })
  ...
  it('offers (connection, profile) scope rows on multi-connection desktops', async () => {
    ...
    ;(window as { slickDesktop?: unknown }).slickDesktop = { connections, getAgentRoster }
