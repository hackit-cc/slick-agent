...
import type { SkillInfo, ToolsetInfo } from '@/types/slick'
...
export function SkillsView({
  ...
  const registryBridge = window.slickDesktop?.connections
  const rosterBridge = window.slickDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Slick (default)' : p.name,
      value: p.name
    }))
