...
How to build Slick Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Slick surface can also
...
- A **runtime home change** (switching the underlying `SLICK_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Slick — do not assume
