...
async function seedBot(slickHome: string, mockUrl: string, name: string): Promise<void> {
  const dir = path.join(slickHome, 'profiles', name)
...
test.beforeAll(async () => {
  ...
  await seedBot(sandbox.slickHome, mock.url, 'alpha')
  await seedBot(sandbox.slickHome, mock.url, 'beta')
