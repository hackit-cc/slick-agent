 ...
 * packaged binary) with a real `slick serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → slick serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Slick title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Slick')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { slickDesktop?: Record<string, unknown> }).slickDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → slick serve is spawned → WS connects → config
