...
/**
 * E2E smoke tests for the packaged Slick desktop app.
 *
 ...
 * (credential stripping, isolated SLICK_HOME + userData, unique app name).
 *
...
test('window opens with the Slick title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      slickDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).slickDesktop?.hud?.open({ sessionId: null })
  )
