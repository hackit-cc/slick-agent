...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Slick E2E'], { cwd: repo })
...
function configureRepoCwd(slickHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(slickHome, mockUrl)
  fs.appendFileSync(path.join(slickHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(slickHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.slickHome, mock.url, repo)
