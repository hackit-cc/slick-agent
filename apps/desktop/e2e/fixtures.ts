/**
 * Shared E2E fixtures for the Slick desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → slick serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `slick desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `slick-e2e-${prefix}-${Math.random()}`))
  const slickHome = path.join(root, 'slick-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(slickHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(slickHome, 'config.yaml')
 ...
 */
export function writeEnvFile(slickHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(slickHome, '.env')
 ...
 */
function writeEmptyConfig(slickHome: string): void {
  const configPath = path.join(slickHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - SLICK_HOME → sandbox slick-home (isolated config/sessions)
 *  - SLICK_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - SLICK_DESKTOP_IGNORE_EXISTING=1 → don't pick up `slick` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - SLICK_DESKTOP_SLICK_ROOT → REPO_ROOT (dev checkout resolution)
 *  - SLICK_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    SLICK_HOME: sandbox.slickHome,
    SLICK_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    SLICK_DESKTOP_IGNORE_EXISTING: '1',
    SLICK_DESKTOP_SLICK_ROOT: REPO_ROOT,
    SLICK_DESKTOP_APP_NAME: `SlickE2E-${Date.now()}`,
    ...
    SLICK_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated SLICK_HOME + userData
 * @param env      - the process environment (already has SLICK_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.slickHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.slickHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via SLICK_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`slick serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.slickHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { SLICK_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Slick backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Slick.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Slick.app', 'Contents', 'MacOS', 'Slick')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'slick')
}
 ...
 * progress without spawning a real Slick backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * SLICK_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    SLICK_DESKTOP_BOOT_FAKE: '1',
    SLICK_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + slick-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).SLICK_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).SLICK_DESKTOP_SLICK
  delete (env as Record<string, string | undefined>).SLICK_DESKTOP_SLICK_ROOT
