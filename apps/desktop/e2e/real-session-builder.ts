 ...
 * AIAgent loop, using the E2E mock provider configured in `slickHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(slickHome: string) {
  ...
  static async start(slickHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(slickHome)
