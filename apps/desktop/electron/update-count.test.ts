...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'slick-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'Slick Test')
    git('config', 'user.email', 'slick@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/SophosTechne/slick-agent.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/SophosTechne/slick-agent/compare/${SHA_A}...${SHA_B}`
  )
})
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/SophosTechne/slick-agent.git',
