...
export const ZOOM_STORAGE_KEY = 'slick:desktop:zoomLevel'
 ...
 * but forgot to emit 'slick:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('slick:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `slick:zoom:changed`, which never fired —
