...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'SLICK_DESKTOP_REMOTE_URL is set but SLICK_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Slick backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `slick serve` the desktop never
