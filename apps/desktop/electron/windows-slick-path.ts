/**
 * windows-slick-path.ts
 *
 * Pure, dependency-injected pieces of Windows `slick` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvSlickCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `slick` shim shadowed the real
 *      slick.cmd/slick.exe; the shim then failed the --version probe and
 ...
 *      and Slick launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvSlickCommand() — unwrapWindowsVenvSlickCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `slick`) must
 * not shadow `slick.cmd`/`slick.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Slick launcher that drives `slick update`.
...
export interface ResolveVenvSlickCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportSlickCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `slick`/`slick.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/slick(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m slick_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportSlickCli(). A
 * venv whose update died mid-`pip install` still has python.exe + slick.exe
 ...
 *
 * Returns null when `command` is not a venv slick shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvSlickCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvSlickCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^slick(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Slick at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Slick Python at ${python}`,
    command: python,
