 ...
 * the three user-facing uninstall modes to the `slick uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m slick_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `slick_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'slick_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Slick.app/Contents/MacOS/Slick  → …/Slick.app
 *   Windows: …\Slick\Slick.exe                 → …\Slick  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Slick.app/Contents/MacOS/Slick → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Slick.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Slick.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Slick$/i.test(dir) || /[\\/]slick-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/slick
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import slick_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, slickHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import slick_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Slick install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "SLICK_HOME=${String(slickHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
