// Unit tests for the pure Windows `slick` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvSlickCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `slick` shim shadows
//      the real slick.cmd/slick.exe.
...
//   3. resolveVenvSlickCommand() — must probe the venv python via
//      canImportSlickCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSlick: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSlick: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSlick: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSlick: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvSlick: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvSlick: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvSlickCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportSlickCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    slickHome: '/fake/slick-home',
...
test('resolveVenvSlickCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvSlickCommand('/root/venv/Scripts/slick.exe', [], deps), null)
})

test('resolveVenvSlickCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvSlickCommand('/root/venv/Scripts/slick.cmd', [], deps), null)
})

test('resolveVenvSlickCommand: returns null when the basename is not slick/slick.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvSlickCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvSlickCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvSlickCommand('/root/venv/bin/slick.exe', [], deps), null)
})

test('resolveVenvSlickCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvSlickCommand('/root/venv/Scripts/slick.exe', [], deps), null)
})

test('resolveVenvSlickCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportSlickCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvSlickCommand('/root/venv/Scripts/slick.exe', ['serve'], deps)
...
test('resolveVenvSlickCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvSlickCommand('/root/venv/Scripts/slick.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'slick_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvSlickCommand: is case-insensitive on slick.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvSlickCommand('/root/venv/Scripts/SLICK.EXE', [], deps))
  assert.ok(resolveVenvSlickCommand('/root/venv/SCRIPTS/slick.exe', [], deps))
})
