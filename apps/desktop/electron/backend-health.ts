...
export interface SlickReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForSlickReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Slick gateway uses OAuth, but you are not signed in. ' +
...
/**
 * The one factory for the actionable Sophos Cloud agent-is-down error, shared by
 ...
 *  - readiness-probe exhaustion in waitForSlickReady().
 *
 * Returns null unless the backend is a *.agents.sophostechne.com host AND the
 ...
 */
export function makeSophosCloudBackendDownError(baseUrl: string, error: unknown): Error | null {
  if (!isSophosCloudAgentUrl(baseUrl)) {
    return null
  }
  ...
  try {
    ...
    // baseUrl is known to parse (isSophosCloudAgentUrl already did); keep the raw
  ...
  const err = new Error(
    `Sophos Cloud agent ${hostname} is down ` +
      `(HTTP ${serverError.statusCode}: server-side fault). ` +
      'Check https://portal.sophostechne.com for backend status, ' +
      'or switch to Local mode in Settings → Gateway. ' +
      'You can also reach out on Discord at discord.gg/SophosTechne ' +
...
/**
 * True when the backend URL points at a Sophos-managed Slick Cloud instance
 * (e.g. ares-3009.agents.sophostechne.com). These are Fly.io-hosted machines
 * the user cannot restart themselves — a 503 from one means the server is down
 * and the recovery path is Portal/Discord/wait.
 */
export function isSophosCloudAgentUrl(baseUrl: string): boolean {
  try {
    const host = new URL(baseUrl).hostname

    return host.endsWith('.agents.sophostechne.com')
  } catch {
    return false
  }
}
 ...
 * `isReauthRequired` so startSlick latches; `needsOauthLogin` alone only
...
export async function waitForSlickReady(baseUrl: string, options: SlickReadyOptions): Promise<void> {
  ...
  // When a Sophos-managed cloud agent returns a server-side HTTP error
  ...
  const cloudError = makeSophosCloudBackendDownError(baseUrl, lastError)

  if (cloudError !== null) {
    throw cloudError
  }

  throw new Error(`Slick backend did not become ready: ${detail}`)
}
