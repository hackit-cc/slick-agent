...
/**
 * The one factory for the actionable Hackit Cloud agent-is-down error, shared by
 ...
 *
 * Returns null unless the backend is a *.agents.hackit.cc host AND the
 ...
 */
export function makeHackitCloudBackendDownError(baseUrl: string, error: unknown): Error | null {
  if (!isHackitCloudAgentUrl(baseUrl)) {
    return null
  }
  ...
  try {
    ...
    // baseUrl is known to parse (isHackitCloudAgentUrl already did); keep the raw
  ...
  const err = new Error(
    `Hackit Cloud agent ${hostname} is down ` +
      `(HTTP ${serverError.statusCode}: server-side fault). ` +
      'Check https://portal.hackit.cc for backend status, ' +
      'or switch to Local mode in Settings → Gateway. ' +
      'You can also reach out on Discord at discord.gg/Hackit ' +
...
/**
 * True when the backend URL points at a Hackit-managed Slick Cloud instance
 * (e.g. ruby-3009.agents.hackit.cc). These are Fly.io-hosted machines
 * the user cannot restart themselves — a 503 from one means the server is down
 * and the recovery path is Portal/Discord/wait.
 */
export function isHackitCloudAgentUrl(baseUrl: string): boolean {
  try {
    const host = new URL(baseUrl).hostname

    return host.endsWith('.agents.hackit.cc')
  } catch {
    return false
  }
}
...
export async function waitForSlickReady(baseUrl: string, options: SlickReadyOptions): Promise<void> {
  ...
  // When a Hackit-managed cloud agent returns a server-side HTTP error
  ...
  const cloudError = makeHackitCloudBackendDownError(baseUrl, lastError)
