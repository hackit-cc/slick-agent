 ...
 * console. It writes SLICK_HOME/.slick-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Slick within 30 minutes.
...
export function handoffResultPath(slickHome: string): string {
  return path.join(slickHome, '.slick-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(slickHome)
  let raw: string
