 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `slick update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `slick desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Slick couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>slick desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>slick desktop --force-build</code>, then restart the app.</p>
