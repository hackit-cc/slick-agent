...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'slick-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'slick-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Slick Test'], { cwd: dir })
