...
export function registerHudIpc({
  ...
  ipcMain.on('slick:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('slick:hud:windowing', event => {
  ...
  ipcMain.on('slick:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('slick:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('slick:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('slick:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('slick:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('slick:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('slick:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('slick:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('slick:hud:close', async () => {
    closeHudWindow()
