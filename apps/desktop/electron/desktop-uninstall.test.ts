...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'slick_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'slick_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'slick_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Slick.app/Contents/MacOS/Slick', 'darwin'),
    '/Applications/Slick.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Slick.app/Contents/MacOS/Slick', 'darwin'),
    '/Users/x/Applications/Slick.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Slick\\Slick.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Slick'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\slick-desktop\\Slick.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\slick-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Slick.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_SlickXXXX/slick', 'linux', { APPIMAGE: '/home/x/Apps/Slick.AppImage' }),
    '/home/x/Apps/Slick.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/slick/linux-unpacked/slick', 'linux', {}), '/opt/slick/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/slick', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Slick.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Slick.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.slick/slick-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.slick/slick-agent',
    uninstallArgs: ['-m', 'slick_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/slick/linux-unpacked',
    slickHome: '/home/x/.slick'
  })
  ...
  assert.match(script, /'-m' 'slick_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/slick\/linux-unpacked'/)
  assert.match(script, /export SLICK_HOME='\/home\/x\/\.slick'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.slick/slick-agent',
    agentRoot: '/home/x/.slick/slick-agent',
    uninstallArgs: ['-m', 'slick_cli.uninstall', '--mode', 'full'],
    appPath: null,
    slickHome: '/home/x/.slick'
  })

  // System python + source on PYTHONPATH so import slick_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.slick\/slick-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'slick_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'slick_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    slickHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'slick_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\slick',
    uninstallArgs: ['-m', 'slick_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Slick',
    slickHome: 'C:\\Users\\x\\AppData\\Local\\slick'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import slick_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\slick;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "slick_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Slick" >nul 2>&1/)
