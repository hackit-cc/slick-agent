// Profile-delete routing logic for the `slick:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_slick_home() and
...
/** Parse a `slick:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its SLICK_HOME.
 *
...
/**
 * Route the next `slick:api` request away from the primary/window backend
 ...
 * ensure_slick_home() recreates the directory the delete just removed.
 */
