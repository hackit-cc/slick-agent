...
import { buildDesktopBackendEnv, slickManagedNodePathEntries, normalizeSlickHomeRoot } from './backend-env'
import {
  isReauthRequiredError,
  makeSophosCloudBackendDownError,
...
import {
  ...
  resolveVenvSlickCommand
} from './windows-slick-path'
import {
...
const USER_DATA_OVERRIDE = process.env.SLICK_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.SLICK_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.SLICK_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with SLICK_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[slick] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[slick] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. SLICK_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[slick] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[slick] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via SLICK_DESKTOP_PASSWORD_STORE, bridged by the
// `slick desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[slick] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[slick] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `slick desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Slick sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[slick] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[slick] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[slick] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[slick] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[slick] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('slick:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Slick burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[slick] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[slick] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[slick] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[slick] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// SLICK_HOME — the user-facing root for everything Slick-related. Mirrors
// scripts/install.ps1's $SlickHome and scripts/install.sh's $SLICK_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\slick (matches install.ps1)
//   macOS / Linux: ~/.slick (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.slick directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\slick yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// SLICK_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// SLICK_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.slick / %LOCALAPPDATA%\slick.
function resolveSlickHome() {
  if (process.env.SLICK_HOME) {
    return normalizeSlickHomeRoot(process.env.SLICK_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'slick-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a SLICK_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\slick and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('SLICK_HOME')

    if (fromRegistry) {
      return normalizeSlickHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'slick')
    const legacy = path.join(app.getPath('home'), '.slick')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.slick setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.slick')
}

const SLICK_HOME = resolveSlickHome()

function pathWithSlickManagedNode(...entries) {
  const managed = slickManagedNodePathEntries(SLICK_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_SLICK_ROOT — the canonical mutable Slick install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_SLICK_ROOT = path.join(SLICK_HOME, 'slick-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_SLICK_ROOT, 'venv')
...
// boots, going straight to `resolveSlickBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_SLICK_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_SLICK_ROOT, '.slick-bootstrap-complete')
...
// active-profile.json records which Slick profile the desktop launches its
// local backend as. When set, startSlick() passes `slick --profile <name>
// dashboard …`, which deterministically pins SLICK_HOME (see
// _apply_profile_override in slick_cli/main.py) and bypasses the sticky
// ~/.slick/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors slick_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// slickDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under SLICK_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by slick_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(SLICK_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (slick_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.SLICK_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.SLICK_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.SLICK_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.SLICK_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.SLICK_DESKTOP_APP_NAME || 'Slick'
...
// 'slick:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.sophostechne.slick) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.sophostechne.slick')
}

// Seed the native About panel with the live Slick version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `slick update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveSlickVersion(),
  copyright: 'Copyright © 2026 Sophos Techne'
})
...
// startSlick(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.SLICK_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.SLICK_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
//   * WSL2 IPC stall  = the renderer's `slick:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.SLICK_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startSlick() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning slick serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Slick backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  if (slickLog.length > 300) {
    slickLog.splice(0, slickLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-slick-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('slick:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the slick:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('slick:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_SLICK_ROOT
  })
}
...
// The Tauri updater writes SLICK_HOME/.slick-update-in-progress for the whole
...
// (`force_kill_other_slick`, taskkill /IM slick.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(SLICK_HOME)),
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Slick will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(SLICK_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Slick update did not finish',
        `${result.message}\n\nDetails: ${path.join(SLICK_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `slick` — must not shadow `slick.cmd`/`slick.exe`.
...
function unwrapWindowsVenvSlickCommand(command, backendArgs) {
  return resolveVenvSlickCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `slick serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `slick` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'slick_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `slick --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, SLICK_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveSlickBackend step 4); execFileSync of a .cmd without
...
function isSlickSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'slick_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.SLICK_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Slick deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the slick-import
...
// findGitBash — locate bash.exe on Windows. Resolves SLICK_GIT_BASH_PATH
...
// normal SLICK_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\slick\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'slick', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'slick', 'git', 'bin', 'git.exe'))
  }
...
function recentSlickLog() {
  return slickLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's slick root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_SLICK_ROOT.
// SLICK_DESKTOP_SLICK_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.SLICK_DESKTOP_SLICK_ROOT && path.resolve(process.env.SLICK_DESKTOP_SLICK_ROOT),
    !IS_PACKAGED && isSlickSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isSlickSourceRoot(ACTIVE_SLICK_ROOT) ? ACTIVE_SLICK_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_SLICK_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('slick:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'slick-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `slick update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(SLICK_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `slick update` can write
// fresh entry points. On Windows this is the file the running backend
// `slick.exe` holds open; on POSIX it's never mandatory-locked.
function venvSlickShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'slick.exe')
    : path.join(updateRoot, 'venv', 'bin', 'slick')
}
...
// only signals the direct child, so on Windows a backend `slick.exe` that
// spawned its own grandchildren (a `slick` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Slick backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Slick backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.slickBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Slick backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.slickBackendIdentity
...
// `slickProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `slick.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a slick
// REPL, a pty terminal, the gateway) keep `slick.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const slickProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\slick.exe, but the
  // backend is `python.exe -m slick_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (slickProcess && Number.isInteger(slickProcess.pid)) {
    initialPids.push(slickProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(slickProcess, {
    forceKillProcessTree,
    stopAllPoolBackends
  })

  const shim = venvSlickShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentSlickProcess = backendConnectionState.getProcess()

        if (currentSlickProcess && Number.isInteger(currentSlickProcess.pid)) {
          stragglers.push(currentSlickProcess.pid)
        }
...
// `slick update`). Instead we spawn the staged Slick-Setup binary with
// --update and quit, so it can run `slick update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `slick update` → relaunch),
      ...
      // in: the SLICK_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `slick update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `slick desktop`, never the Tauri installer that self-copies
      // slick-setup.exe into SLICK_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `slick` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `slick update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `slick update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, slickRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(SLICK_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Slick — this window will close and the updater will open. Don’t reopen Slick yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(SLICK_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // slick.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running slick, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Slick install open ' +
        '(a second Slick window or a terminal running slick?). Close it and retry.'
    ...
    // Slick processes running from this venv.  The updater normally refuses
    ...
    // `slick update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `slick update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          SLICK_HOME,
          SLICK_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithSlickManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `slick update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(SLICK_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Slick update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(SLICK_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Slick will keep running — try again, or run \`slick update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvSlick = path.join(venvBin, IS_WINDOWS ? 'slick.exe' : 'slick')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Slick launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the slick
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.slick-bootstrap-complete')),
      hasVenvSlick: fileExists(venvSlick),
...
// Python-level pre-update snapshot inside `slick update`.
function preflightStateDb(slickHome, rememberLog) {
  const stateDbPath = path.join(slickHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(slickHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(slickHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(slickHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `slick update`, swaps/relaunches the app bundle, and writes
// .slick-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'slick update', percent: null })

    return { ok: true, manual: true, command: 'slick update', slickRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      SLICK_HOME,
      SLICK_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithSlickManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Slick — this window will close. Don’t reopen Slick yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_SLICK_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isSlickSourceRoot(ACTIVE_SLICK_ROOT) &&
    fileExists(venvPython) &&
    canImportSlickCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_SLICK_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `slick
...
function resolveWebDist() {
  const override = process.env.SLICK_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set SLICK_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: slick desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: slick desktop --force-build`
  )
...
function resolveSlickCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Slick.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveSlickCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'slick_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_SLICK_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Slick at ${ACTIVE_SLICK_ROOT}`,
    command,
    args: ['-m', 'slick_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      slickHome: SLICK_HOME,
      pythonPathEntries: [ACTIVE_SLICK_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_SLICK_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveSlickBackend(backendArgs) {
  // 1. Explicit override -- SLICK_DESKTOP_SLICK_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.SLICK_DESKTOP_SLICK_ROOT && path.resolve(process.env.SLICK_DESKTOP_SLICK_ROOT)

  if (overrideRoot && isSlickSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Slick source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `slick` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isSlickSourceRoot.)
  if (!IS_PACKAGED && isSlickSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Slick source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_SLICK_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\slick\\slick-agent (Windows) or ~/.slick/slick-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Slick runtime at ${ACTIVE_SLICK_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `slick` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    SLICK_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.SLICK_DESKTOP_IGNORE_EXISTING !== '1') {
    let slickCommand = null
    const slickOverride = process.env.SLICK_DESKTOP_SLICK

    if (slickOverride) {
      const resolvedOverride = findOnPath(slickOverride)

      if (resolvedOverride) {
        slickCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(slickOverride, { isWsl: IS_WSL })) {
        slickCommand = slickOverride
      } else {
        rememberLog(`Ignoring Windows Slick override under WSL: ${slickOverride}`)
      }
    } else {
      slickCommand = findOnPath('slick')
    }

    if (slickCommand) {
      if (looksLikeDesktopAppBinary(slickCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Slick CLI: ${slickCommand}`)
        slickCommand = null
      }
    }

    if (slickCommand) {
      const unwrapped = unwrapWindowsVenvSlickCommand(slickCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `slick` shim
      ...
      const shellForProbe = isCommandScript(slickCommand)

      // SLICK_DESKTOP_SLICK is an explicit deployment override (used by
      ...
      if (shouldTrustSlickOverride(slickOverride) || verifySlickCli(slickCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvSlickCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Slick CLI at ${slickCommand}`,
          command: slickCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Slick CLI at ${slickCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed slick_cli module via system Python.
  ...
  if (python) {
    ...
    // having slick_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\slick\slick-agent\venv.
    if (canImportSlickCli(python)) {
      return {
        kind: 'python',
        label: `installed slick_cli module via ${python}`,
        command: python,
        args: ['-m', 'slick_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: slick_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveSlickBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Slick Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveSlickBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Slick install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Slick recovery was handed off to Slick Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      slickHome: SLICK_HOME,
      logRoot: path.join(SLICK_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Slick install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Slick bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(SLICK_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startSlick() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // slick:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveSlickBackend(backend.args))
  }
  ...
  if (!isSlickSourceRoot(ACTIVE_SLICK_ROOT)) {
    throw new Error(
      `Slick install at ${ACTIVE_SLICK_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Slick' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\slick\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `slick` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Slick on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isSlickSourceRoot()
    // plus an importable slick_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Slick venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Slick at ${ACTIVE_SLICK_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Slick runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----slick-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Slick backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Slick backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Slick backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Slick-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Slick-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('slick:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveSlickCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('slick:preview-file-changed', payload)
}
...
// waitForSlickReady how to read a 401 — rejected session vs gated route.
...
async function waitForSlick(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForSlickReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('slick:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('slick:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('slick:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('slick:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Slick...".
function sendPowerResume() {
  ...
  webContents.send('slick:power-resume')
}
...
// 'slick:power-battery' push below.
ipcMain.handle('slick:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('slick:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('slick:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('slick:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('slick:context-menu-spellcheck', {
...
//
// Hosted Slick gateways gate the dashboard behind an OAuth provider (e.g.
// Sophos Techne) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``slick_session_at``),
...
//   * Sophos Portal now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / slick #37247).
//     Both are set as HttpOnly cookies (``slick_session_at`` ~15 min,
//     ``slick_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startSlick → the renderer's
// single-shot boot() with no retry) surfaced as the "Slick couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Slick
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Slick Cloud agent…' : 'Sign in to Slick gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Slick backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in slick_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Slick Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
// The "cloud" connection mode lets a user sign in to the Sophos portal ONCE in
...
// portal session cookie living in `persist:slick-remote-oauth`:
...
// Canonical Sophos portal base URL, overridable for staging/dev. Mirrors the CLI
// convention (slick_cli/auth.py DEFAULT_SOPHOS_PORTAL_URL + the same env names)
// so a single override flips every Slick surface to the same portal.
const DEFAULT_SOPHOS_PORTAL_URL = 'https://portal.sophostechne.com'

function resolvePortalBaseUrl() {
  const raw = process.env.SLICK_PORTAL_BASE_URL || process.env.SOPHOS_PORTAL_BASE_URL || DEFAULT_SOPHOS_PORTAL_URL

  return String(raw).trim().replace(/\/+$/, '')
}

// Whether the OAuth partition currently holds a live Sophos portal session — the
// credential that powers both discovery and the silent cascade. The portal
// authenticates via PRIVY, not the Slick gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for slick_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Slick Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Slick Cloud sign-in.'))
...
// Discover the hosted (Slick Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Slick Cloud. Open Settings → Gateway, choose Slick Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Slick Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Slick Cloud session has expired. Sign in to Slick Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Slick Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every slick:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteSlickPath`/`remoteProfile` —
...
// a valid stored value (pins the root SLICK_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.SLICK_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.SLICK_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Slick Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteSlickPath: (ssh || savedSsh)?.remoteSlickPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by SLICK_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Slick Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Slick Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteSlickPath: input.sshRemoteSlickPath ?? existingBlock.remoteSlickPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (slick #37247), and the gateway middleware transparently
    ...
    try {
      ...
      // For a Sophos-managed Cloud agent, a 502/503/504 from the WS-ticket mint
      ...
      // else (#85335).
      const cloudError = makeSophosCloudBackendDownError(baseUrl, error)
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Slick gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Slick gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.SLICK_DESKTOP_REMOTE_TOKEN,
      url: process.env.SLICK_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      slickPath: result.slickPath,
      slickHome: result.slickHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { slickPath: result.slickPath, slickHome: result.slickHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteSlickPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteSlickPath: sshConfig.remoteSlickPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForSlick: (baseUrl, token) => waitForSlick(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        slickVersion: result.slickVersion || '',
        ...
        // per-profile pools also use bare keys. Only startSlick' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.slickVersion || 'slick (version unknown)'} at ${result.slickPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteSlickVersion: result.slickVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (SLICK_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.SLICK_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startSlick() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Slick
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Sophos Techne" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteSlickPath: input.sshRemoteSlickPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteSlickPath || '')
          let slickPath
          let slickVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            slickPath = runtime.slickPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.slickPath])
            slickVersion = inspection.version
            supported = inspection.supported
          } else {
            slickPath = await remoteLifecycle.locateSlick(ssh, sshConfig.remoteSlickPath || '')
            slickVersion = await remoteLifecycle.probeSlickVersion(ssh, slickPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, slickPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Slick on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteSlickPath: slickPath,
            remoteSlickVersion: slickVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startSlick())
  ...
  // connect to Slick gateway". Mirror the renderer's connect here so the test
...
function resetSlickConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const slickProcess = backendConnectionState.invalidate()
  stopBackendChild(slickProcess)
...
// startSlick() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetSlickConnection() invalidates it.
  const slickProcess = backendConnectionState.getProcess()
  const dying = slickProcess && !slickProcess.killed ? slickProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('slick:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('slick:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startSlick()
  ...
  // spawning, so two children never share one profile's SLICK_HOME.
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Slick backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteSlickPath: source.remoteSlickPath,
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Slick backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. slick:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: slickLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForSlick(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.slickPath || !state?.slickHome) {
  ...
  return {
    ...
    slickPath: state.slickPath,
    slickHome: state.slickHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteSlickPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.slickPath ? platform : await probeWindowsRemote(ssh, config.remoteSlickPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          slickPath: runtime.slickPath,
          slickHome: runtime.slickHome,
          pythonPath: runtime.python
        }
      }
    }

    const slickPath = await remoteLifecycle.locateSlick(ssh, config.remoteSlickPath || '')
    const slickHome = await remoteLifecycle.probeRemoteSlickHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, slickPath, slickHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { slickPath: state.slickPath, slickHome: state.slickHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForSlick(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startSlick() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForSlick(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited SLICK_HOME env (see _apply_profile_override
  // step 3 in slick_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveSlickBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const slickCwd = resolveSlickCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(SLICK_HOME, 'profiles', key))
  )

  rememberLog(`Starting Slick backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: slickCwd,
        SLICK_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        SLICK_WEB_DIST: webDist,
        ...(readyFile ? { SLICK_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Slick backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Slick backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Slick backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForSlick(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Slick backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Slick backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_slick_home() recreates the deleted profile directory.
//
...
async function startSlick() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Slick Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startSlick() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Slick backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Slick backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Slick backend at ${remote.baseUrl}`, 24)
      await waitForSlick(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Slick backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, slickLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.slick/active_profile file) and
    // resolves SLICK_HOME the same way `slick -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Slick runtime', 28)

        return resolveSlickBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Slick backend via ${backend.label}`, 84)
    rememberLog(`Starting Slick backend via ${backend.label}`)
    ...
    const slickProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin SLICK_HOME for the child so Python's get_slick_home()
          // resolves to the SAME location our resolveSlickHome() picked. Without
          // this pin, Python falls back to ~/.slick on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\slick, which differs from C:\Users\<u>\.slick.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets SLICK_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(slickProcess)
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, slickProcess)

    if (!processOwner) {
      stopBackendChild(slickProcess)
      await waitForBackendExit(slickProcess)
      releaseBackendChild(slickProcess)
      throw new Error('Slick backend start was superseded by a newer connection attempt.')
    }

    slickProcess.stdout.on('data', rememberLog)
    slickProcess.stderr.on('data', rememberLog)
    ...
    slickProcess.once('error', error => {
      releaseBackendChild(slickProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Slick backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Slick backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Slick backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Slick backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    slickProcess.once('exit', (code, signal) => {
      releaseBackendChild(slickProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Slick backend exit (${signal || code})`)
      ...
      rememberLog(`Slick backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Slick backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Slick backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentSlickLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Slick backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(slickProcess, {
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Slick backend to become ready', 90)
    await Promise.race([waitForSlick(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => slickProcess.exitCode === null && !slickProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Slick backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Slick backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('slick:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Slick is
...
// pushes pet state over IPC (slick:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via slick:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Slick app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Slick icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see slick:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Slick in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('slick:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Slick can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('slick:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('slick:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[slick] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('slick:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `slick:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, slick:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('slick:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('slick:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('slick:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'slick desktop --force-build',
  ...
  startSlick().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('slick:connection', async (_event, profile) => {
...
ipcMain.handle('slick:connection:for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
  const registry = readDesktopConnectionsRegistry()
  const id = String(connectionId || '').trim() || registry.primary
  // Same single-owner claim as 'slick:connection', keyed by the composite
...
ipcMain.on('slick:connection:active-route', (event, route) => {
...
// "Starting Slick…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('slick:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetSlickConnection({ soft: true }),
...
ipcMain.handle('slick:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('slick:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('slick:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('slick:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('slick:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('slick:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveSlickBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Slick is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `slick-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, SLICK_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('slick:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('slick:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('slick:zoom:get', event => {
...
ipcMain.on('slick:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('slick:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startSlick() call restarts the
...
ipcMain.handle('slick:bootstrap:repair', async () => {
  // Forceful repair: force the next startSlick() through the full installer
  ...
  // "bypass active runtime" path inside startSlick() and fall through
...
ipcMain.handle('slick:boot-progress:get', async () => bootProgressState)
ipcMain.handle('slick:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('slick:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('slick:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('slick:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('slick:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('slick:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('slick:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('slick:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('slick:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('slick:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('slick:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('slick:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('slick:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('slick:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('slick:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteSlickPath: entry.remoteSlickPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteSlickPath: connection.remoteSlickPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteSlickProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root SLICK_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('slick:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// slick:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('slick:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('slick:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `slick update` to every eligible registered connection at once.
...
ipcMain.handle('slick:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/slick/update', {}, { timeoutMs: 15_000 })
...
// analogue of the slick:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('slick:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('slick:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startSlick() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('slick:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Slick Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('slick:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('slick:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('slick:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('slick:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of slick:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('slick:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('slick:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new SLICK_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('slick:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('slick:requestMicrophoneAccess', async () => {
...
ipcMain.handle('slick:window:readBelow', async event => {
...
async function handleSlickApiRequest(request) {
  ...
  // backend calls ensure_slick_home() which recreates the profile directory,
...
ipcMain.handle('slick:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its SLICK_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleSlickApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleSlickApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('slick:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('slick:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('slick:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as slick://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('slick:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('slick:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('slick:data-url-read-max:get', () => ({
...
ipcMain.handle('slick:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('slick:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('slick:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('slick:readFileText', async (_event, filePath) => {
...
// `slick:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('slick:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('slick:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('slick:writeClipboard', (_event, text) => {
...
ipcMain.handle('slick:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('slick:readClipboard', () => clipboard.readText())

ipcMain.handle('slick:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('slick:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('slick:context-menu:edit', (event, command) => {
...
ipcMain.handle('slick:context-menu:copy-image', event => {
...
ipcMain.handle('slick:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('slick:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('slick:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('slick:saveClipboardImage', async () => {
...
ipcMain.handle('slick:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('slick:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('slick:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('slick:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('slick:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('slick:titlebar-theme', (_event, payload) => {
...
ipcMain.on('slick:native-theme', (_event, mode) => {
...
ipcMain.on('slick:translucency:support', event => {
  event.returnValue = { glass: GLASS_SUPPORTED, translucency: TRANSLUCENCY_SUPPORTED }
})

ipcMain.on('slick:translucency', (_event, payload) => {
...
ipcMain.on('slick:keep-awake', (_event, on) => {
...
ipcMain.handle('slick:quick-entry:settings:get', async () => {
...
ipcMain.handle('slick:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('slick:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('slick:quick-entry:submit', {
...
ipcMain.on('slick:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('slick:quick-entry:state', payload)
  }
})

ipcMain.on('slick:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('slick:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('slick:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `slick:window:openSession`).
...
ipcMain.handle('slick:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('slick:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('slick:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('slick:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveSlickCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('slick:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveSlickCwd()
}))

ipcMain.handle('slick:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('slick:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('slick:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('slick:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('slick:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('slick:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('slick:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: slickLog.slice(-200) }))
...
ipcMain.on('slick:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (slick:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('slick:updates:check', async () =>
...
ipcMain.handle('slick:updates:apply', async (_event, payload) =>
...
ipcMain.handle('slick:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('slick:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Slick version (the one `release.py` bumps in
// slick_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Slick version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveSlickVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'slick_cli', '__init__.py')
...
// Renderer-bundle skew: `slick update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Slick version and push it into the native About panel
// just before showing it, so an in-place `slick update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveSlickVersion()} — app build out of date, update the desktop app`
        : resolveSlickVersion(),
      copyright: 'Copyright © 2026 Sophos Techne'
    })
    app.showAboutPanel()
...
ipcMain.handle('slick:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveSlickVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    slickRoot: resolveUpdateRoot(),
...
// the actual removal via `slick uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (slick_cli/uninstall.py + slick_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_SLICK_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isSlickSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'slick_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, SLICK_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Slick agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import slick_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_SLICK_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `slick-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `slick-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('slick:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('slick:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('slick:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('slick:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// slick:// deep links (e.g. slick://blueprint/morning-brief?time=08:00,
// slick://mcp/install?name=NAME&config=B64 — the vendor "Add to Slick"
// button, or slick://plugin/install?repo=owner/repo). Dev
// (`SLICK_DESKTOP_DEV_SERVER`) registers slick-dev:// instead — bare
// Electron or a stale OS handler often owns slick:// on dev machines.
...
// ---------------------------------------------------------------------------
const SLICK_PROTOCOL = DEV_SERVER ? 'slick-dev' : 'slick'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['slick-dev', 'slick'] : ['slick']
...
function handleDeepLink(url) {
  ...
  // slick://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('slick:deep-link', payload)
...
ipcMain.handle('slick:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${SLICK_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(SLICK_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(SLICK_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${SLICK_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `slick://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startSlick), whose
...
app.whenReady().then(() => {
  ...
  // startSlick() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching slick:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Slick never keeps another app's chord hostage.
