...
import {
  ...
  makeHackitCloudBackendDownError,
...
// electron-builder derives from the build `appId` (com.hackit.slick) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.hackit.slick')
}
...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 Hackit'
})
...
// Hackit) instead of a static session token. The auth model is
...
//   * Hackit Portal now issues a 24h ROTATING, reuse-detected refresh token
...
// Phase 3) ---------------------------------------------------------------
//
// The "cloud" connection mode lets a user sign in to the Hackit portal ONCE in
...
// Canonical Hackit portal base URL, overridable for staging/dev. Mirrors the CLI
// convention (slick_cli/auth.py DEFAULT_HACKIT_PORTAL_URL + the same env names)
// so a single override flips every Slick surface to the same portal.
const DEFAULT_HACKIT_PORTAL_URL = 'https://portal.hackit.cc'

function resolvePortalBaseUrl() {
  const raw = process.env.SLICK_PORTAL_BASE_URL || process.env.HACKIT_PORTAL_BASE_URL || DEFAULT_HACKIT_PORTAL_URL

  return String(raw).trim().replace(/\/+$/, '')
}

// Whether the OAuth partition currently holds a live Hackit portal session — the
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    try {
      ...
      // For a Hackit-managed Cloud agent, a 502/503/504 from the WS-ticket mint
      ...
      // else (#85335).
      const cloudError = makeHackitCloudBackendDownError(baseUrl, error)
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Hackit" instead of a generic
