// Launching the Slick TUI in the user's OWN terminal emulator.
//
...
// `slick --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Slick runtime is often a venv Python invoked
//     as `python -m slick_cli.main`, not a `slick` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Slick
