 ...
 *
 * The staged Tauri `slick-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `slick update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `slick update`, swaps/relaunches
 * the app, and writes .slick-update-result.json. With the app gone before
 * the update starts, the SLICK_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `slick update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into SLICK_HOME on *every* platform
 * (`slick-setup.exe` on Windows, `slick-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_slick_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `slick update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(slickHome, 'slick-setup.exe')
 ...
 *
 * `copy_self_to_slick_home` deliberately no-ops during `--update`
 ...
 * "Another Slick update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `slick update`, which
 ...
 * the installer is written to SLICK_HOME at install/repair time, making mtime
