 ...
 *   1. SLICK_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\slick\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect SLICK_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.SLICK_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'slick', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'slick', 'git', 'usr', 'bin', 'bash.exe'))
  }
