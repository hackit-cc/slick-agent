 ...
 *     `X-Slick-Session-Token` header; WS uses `?token=`.
...
// slick_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - slick_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - slick_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / slick #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-slick_session_at', '__Secure-slick_session_at', 'slick_session_at']
const RT_COOKIE_VARIANTS = ['__Host-slick_session_rt', '__Secure-slick_session_rt', 'slick_session_rt']

// The Sophos portal (NAS) does NOT use Slick gateway session cookies — it is a
...
// Keep this aligned with slick_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['slick', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  // classification (isServerSideHttpError / makeSophosCloudBackendDownError) and
...
/** Which Slick profile the remote SSH dashboard should actually run as.
...
// Slick Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteSlickPath = String(entry.remoteSlickPath || '').trim()

  if (remoteSlickPath) {
    out.remoteSlickPath = remoteSlickPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Slick installation. Preserve an explicit mapping
  // when it is a valid Slick profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_slick_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `SLICK_HOME` scopes it.
 *
 ...
 * `slick:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://slick.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `slick:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a slick session ACCESS-token cookie
...
/**
 * True if the cookie jar holds a live Sophos PORTAL (Privy) session — a non-empty
 ...
 * the Slick gateway session cookies, so cloud sign-in / discovery liveness
