 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `slick update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.slickHome, 'Slick home')
  const slickPath = validateRemoteValue(target.slickPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(slickPath)

  const updateCommand =
    `env SLICK_HOME=${homeWord} ` +
    `SLICK_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'SLICK_UPDATE_ORIGIN_PROFILE=default ' +
    `SLICK_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `SLICK_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:SLICK_HOME=${psLiteral(home)}`,
    `$env:SLICK_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:SLICK_UPDATE_ORIGIN_PROFILE="default"',
    `$env:SLICK_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:SLICK_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:SLICK_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:SLICK_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:SLICK_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(slickPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.slick-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Slick updated and every managed SSH profile is ready.'
        : restoreOk
