...
test('oauthGuardMayHardFail keeps the strict guard for oauth and mixed deployments', () => {
  assert.equal(oauthGuardMayHardFail([{ name: 'sophos', supportsPassword: false }]), true)
  assert.equal(
    oauthGuardMayHardFail([
...
test('oauthGuardMayHardFail keeps the strict guard for string oauth providers', () => {
  assert.equal(oauthGuardMayHardFail(['sophos'] as any), true)
  assert.equal(oauthGuardMayHardFail(['sophos', 'basic'] as any), true)
})
