...
test('oauthGuardMayHardFail keeps the strict guard for oauth and mixed deployments', () => {
  assert.equal(oauthGuardMayHardFail([{ name: 'hackit', supportsPassword: false }]), true)
  assert.equal(
    oauthGuardMayHardFail([
...
test('oauthGuardMayHardFail keeps the strict guard for string oauth providers', () => {
  assert.equal(oauthGuardMayHardFail(['hackit'] as any), true)
  assert.equal(oauthGuardMayHardFail(['hackit', 'basic'] as any), true)
})
