...
// hardening, SLICK_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('slick:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('slick:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('slick:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('slick:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<SLICK_HOME>/desktop-plugins`,
  // resolved from the main-process SLICK_HOME (see resolveSlickHome) — NOT from
  // the connected backend. A remote backend reports its own `slick_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped slick_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(slickHome, 'profiles', profile) : slickHome
  ...
  ipcMain.handle('slick:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<SLICK_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where SLICK_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('slick:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<SLICK_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('slick:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('slick:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('slick:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('slick:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('slick:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('slick:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
