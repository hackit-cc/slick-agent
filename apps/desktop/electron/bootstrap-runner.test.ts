...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'slick-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/slick-runner-test',
    sourceRepoRoot: null,
    slickHome: '/tmp/slick-runner-test',
    logRoot: '/tmp/slick-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'slick-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'slick-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/slick-agent',
      slickHome: '/tmp/slick'
    }),
    ['--dir', '/tmp/slick-agent', '--slick-home', '/tmp/slick', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/slick-agent', '--slick-home', '/tmp/slick', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/slick',
      slickHome: '/tmp/home'
    }),
    ['--dir', '/tmp/slick', '--slick-home', '/tmp/home', '--branch', 'main']
  )
})
