 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Slick Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.slick-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(slickHome) {
  return path.join(slickHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.slick/slick-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(slickHome) {
  if (!slickHome) {
    return null
  }

  const candidate = path.join(slickHome, 'slick-agent', 'scripts', installScriptName())
...
function cachedScriptPath(slickHome, commit) {
  return path.join(bootstrapCacheDir(slickHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/SophosTechne/slick-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(slickHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(slickHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, slickHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass SLICK_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          SLICK_HOME: slickHome || process.env.SLICK_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, slickHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, slickHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--slick-home', slickHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, slickHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, slickHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, slickHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(slickHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, slickHome, emit })
