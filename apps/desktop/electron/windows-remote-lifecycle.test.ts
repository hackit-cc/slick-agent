...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command slick.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          slickHome: 'C:\\Users\\alice\\.slick',
          slickPath: 'C:\\Slick\\slick.exe',
          python: 'C:\\Slick\\python.exe'
        })
      }

      if (script.includes('.slick-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('slick_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.slick\\profiles\\research')
  assert.match(script, /\.slick-update-in-progress/)
...
test('Windows probe validates Slick and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $slickHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Slick is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Slick is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Slick's\\python.exe" }, 'inspect', [
    'C:\\x y\\slick.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'slick_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Slick''s/)
  assert.match(script, /C:\\x y\\slick\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { slickPath: lock.slickPath, slickHome: lock.slickHome }
