...
const READY_RE = /^SLICK_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitSlickPath = '') {
  const explicit = psLiteral(explicitSlickPath)
  ...
  const script = [
    ...
    '$slickHome=$env:SLICK_HOME',
    'if(-not $slickHome){$slickHome=Join-Path $env:LOCALAPPDATA "slick"}',
    'Assert-NoReparse $slickHome $true',
    '$candidate=[IO.Path]::Combine($slickHome, "slick-agent\\venv\\Scripts\\slick.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "slick-agent\\.venv\\Scripts\\slick.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $slickHome "slick-agent\\venv\\Scripts\\slick.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "slick-agent\\.venv\\Scripts\\slick.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command slick.exe -ErrorAction SilentlyContinue',
    ...
    '$slick=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$slick=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $slick){throw "Slick is not installed on the remote Windows host."}',
    'Assert-NoReparse $slick $false',
    'if($explicit -and $slick -ne $explicit){throw "The configured Slick path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($slick), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;slickHome=$slickHome;slickPath=$slick;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(slickHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class SlickMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(slickHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".slick-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[SlickMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, slickHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(slickHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitSlickPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitSlickPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'slick_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'slick_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'slick_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.slickHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".slick-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};slickPath=${psLiteral(reservation.slickPath)};slickHome=${psLiteral(reservation.slickHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.slickPath === 'string' &&
    typeof lock.slickHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.slickPath === runtime.slickPath &&
    lock.slickHome === runtime.slickHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteSlickPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.slickHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.slickPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Slick on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.slickPath = inspection.path
  const slickVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located slick at ${runtime.slickPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, slickPath: runtime.slickPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForSlick(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Slick SSH"', 'powershell.exe -NoLogo')
