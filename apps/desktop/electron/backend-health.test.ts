...
test('uses lightweight /api/health for current backends', async () => {
  ...
  await waitForSlickReady('http://127.0.0.1:9000/', {
    token: 'secret-token',
...
test('does not fall back to heavyweight /api/status for transient health failures', async () => {
  ...
  await assert.rejects(
    waitForSlickReady('http://127.0.0.1:9000', {
      fetchPublicJson: async url => {
        calls.push(['public', url])
        throw new Error('Timed out connecting to Slick backend after 15000ms')
      },
      fetchJson: async url => {
        calls.push(['token', url])
      },
...
test('recognizes missing-route shapes only', () => {
  ...
  assert.equal(
    isMissingHealthEndpointError(
      new Error('Expected JSON from /api/health but got HTML. The endpoint is likely missing on the Slick backend.')
    ),
    true
  )
  assert.equal(isMissingHealthEndpointError(new Error('Timed out connecting to Slick backend after 15000ms')), false)
...
test('anonymous gate-shaped 401 falls back to /api/status (backend predates /api/health)', async () => {
  ...
  await waitForSlickReady('http://192.168.1.132:9119', {
    token: null,
    fetchPublicJson: async url => {
      calls.push(['public', url])
...
test('a credentialed 401 fails fast for reauth instead of reporting a dead session ready', async () => {
  ...
  await assert.rejects(
    waitForSlickReady('https://gateway.example', {
      token: 'session-token',
      fetchPublicJson: async () => {
...
test('unsigned OAuth is a terminal reauth failure; needsOauthLogin alone is not', () => {
  // The unsigned-in throw must set isReauthRequired so startSlick latches.
  ...
  assert.equal(isReauthRequiredError(new Error('Could not reach the remote Slick gateway')), false)
})
...
test('a credentialed probe still uses the 404 fallback for a genuinely missing route', async () => {
  ...
  await waitForSlickReady('https://gateway.example', {
    token: 'session-token',
    fetchPublicJson: async () => {
      throw new Error('public probe must not be used when credentialed')
    },
...
test('isSophosCloudAgentUrl detects cloud agent hosts', () => {
  // Positive cases
  assert.equal(isSophosCloudAgentUrl('https://ares-3009.agents.sophostechne.com'), true)
  assert.equal(isSophosCloudAgentUrl('https://ares-3009.agents.sophostechne.com/api/health'), true)
  assert.equal(isSophosCloudAgentUrl('http://test.agents.sophostechne.com'), true)

  // Negative cases
  assert.equal(isSophosCloudAgentUrl('http://127.0.0.1:9000'), false)
  assert.equal(isSophosCloudAgentUrl('https://gateway.example.com'), false)
  assert.equal(isSophosCloudAgentUrl('https://sophostechne.com'), false)
  assert.equal(isSophosCloudAgentUrl('not-a-url'), false)
})

test('waitForSlickReady surfaces actionable error for cloud agent 503', async () => {
  let attempts = 0
  const currentTime = { value: 0 }

  try {
    await waitForSlickReady('https://ares-3009.agents.sophostechne.com', {
      fetchPublicJson: async () => {
        attempts++
        // Always return 503
    ...
    assert.ok(error.message.includes('Sophos Cloud agent'), `unexpected message: ${error.message}`)
    assert.ok(error.message.includes('503'), `should mention status code: ${error.message}`)
    assert.ok(error.message.includes('portal.sophostechne.com'), `should mention portal: ${error.message}`)
    assert.ok(error.message.includes('discord.gg/SophosTechne'), `should mention Discord: ${error.message}`)
...
test('waitForSlickReady does not cloud-wrap non-cloud 503 errors', async () => {
  const currentTime = { value: 0 }
...
test('makeSophosCloudBackendDownError produces the Cloud shape and preserves cause', () => {
  const err = new Error('upstream unavailable') as any
  err.statusCode = 503
  const result = makeSophosCloudBackendDownError('https://ares-3009.agents.sophostechne.com', err)
  assert.ok(result)
  ...
  assert.ok(result?.message.includes('Sophos Cloud agent ares-3009.agents.sophostechne.com is down'))
})

test('makeSophosCloudBackendDownError returns null for a Cloud 401 (routes to reauth)', () => {
  const err = new Error('Unauthorized') as any
  err.statusCode = 401
  assert.equal(makeSophosCloudBackendDownError('https://ares-3009.agents.sophostechne.com', err), null)
})

test('makeSophosCloudBackendDownError returns null for a non-Cloud 503 (generic remote failure)', () => {
  const err = new Error('Service Unavailable') as any
  err.statusCode = 503
  assert.equal(makeSophosCloudBackendDownError('https://gateway.example.com', err), null)
  assert.equal(makeSophosCloudBackendDownError('http://127.0.0.1:9000', err), null)
})

test('makeSophosCloudBackendDownError preserves legacy string-prefix compatibility', () => {
  const result = makeSophosCloudBackendDownError(
    'https://ares-3009.agents.sophostechne.com',
