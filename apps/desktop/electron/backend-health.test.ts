...
test('isHackitCloudAgentUrl detects cloud agent hosts', () => {
  // Positive cases
  assert.equal(isHackitCloudAgentUrl('https://ruby-3009.agents.hackit.cc'), true)
  assert.equal(isHackitCloudAgentUrl('https://ruby-3009.agents.hackit.cc/api/health'), true)
  assert.equal(isHackitCloudAgentUrl('http://test.agents.hackit.cc'), true)

  // Negative cases
  assert.equal(isHackitCloudAgentUrl('http://127.0.0.1:9000'), false)
  assert.equal(isHackitCloudAgentUrl('https://gateway.example.com'), false)
  assert.equal(isHackitCloudAgentUrl('https://hackit.cc'), false)
  assert.equal(isHackitCloudAgentUrl('not-a-url'), false)
})
...
test('waitForSlickReady surfaces actionable error for cloud agent 503', async () => {
  ...
  try {
    await waitForSlickReady('https://ruby-3009.agents.hackit.cc', {
      fetchPublicJson: async () => {
        attempts++
        // Always return 503
    ...
    assert.ok(error.message.includes('Hackit Cloud agent'), `unexpected message: ${error.message}`)
    assert.ok(error.message.includes('503'), `should mention status code: ${error.message}`)
    assert.ok(error.message.includes('portal.hackit.cc'), `should mention portal: ${error.message}`)
    assert.ok(error.message.includes('discord.gg/Hackit'), `should mention Discord: ${error.message}`)
...
test('makeHackitCloudBackendDownError produces the Cloud shape and preserves cause', () => {
  const err = new Error('upstream unavailable') as any
  err.statusCode = 503
  const result = makeHackitCloudBackendDownError('https://ruby-3009.agents.hackit.cc', err)
  assert.ok(result)
  ...
  assert.ok(result?.message.includes('Hackit Cloud agent ruby-3009.agents.hackit.cc is down'))
})

test('makeHackitCloudBackendDownError returns null for a Cloud 401 (routes to reauth)', () => {
  const err = new Error('Unauthorized') as any
  err.statusCode = 401
  assert.equal(makeHackitCloudBackendDownError('https://ruby-3009.agents.hackit.cc', err), null)
})

test('makeHackitCloudBackendDownError returns null for a non-Cloud 503 (generic remote failure)', () => {
  const err = new Error('Service Unavailable') as any
  err.statusCode = 503
  assert.equal(makeHackitCloudBackendDownError('https://gateway.example.com', err), null)
  assert.equal(makeHackitCloudBackendDownError('http://127.0.0.1:9000', err), null)
})

test('makeHackitCloudBackendDownError preserves legacy string-prefix compatibility', () => {
  const result = makeHackitCloudBackendDownError(
    'https://ruby-3009.agents.hackit.cc',
