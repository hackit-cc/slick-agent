...
// evict each other's `slick_session*` cookies when they ride the single
// shared `persist:slick-remote-oauth` partition — and, worse, gateway A's
...
describe('resolveOauthPartition (#92183 per-connection cookie jars)', () => {
  ...
  it('keeps cloud connections on the legacy partition (silent portal cascade needs the shared jar)', () => {
    const reg = registry('local', [
      { id: 'cloud-1', kind: 'cloud', url: 'https://agent.sophostechne.com', authMode: 'oauth' }
    ])

    expect(resolveOauthPartition('https://agent.sophostechne.com/api/status', { registry: reg })).toBe(
      LEGACY_OAUTH_PARTITION
    )
  })
  ...
  it('falls back to the legacy partition for unmatched, portal, and malformed inputs', () => {
    ...
    expect(resolveOauthPartition('https://portal.sophostechne.com/api/agents', { registry: reg })).toBe(
      LEGACY_OAUTH_PARTITION
    )
