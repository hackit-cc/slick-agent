...
import { makeSophosCloudBackendDownError } from './backend-health'
import {
...
test('normalizeRemoteHeaders keeps safe proxy headers and drops transport/auth headers', () => {
  assert.deepEqual(
    normalizeRemoteHeaders({
      ...
      'X-Slick-Session-Token': { encoding: 'plain', value: 'token' },
...
test('remoteRequestMatchesBaseUrl treats HTTPS and WSS as the same gateway origin', () => {
  assert.equal(
    remoteRequestMatchesBaseUrl(
      'wss://slick.example.com/gateway/api/ws?ticket=abc',
      'https://slick.example.com/gateway'
    ),
    true
  )
  assert.equal(remoteRequestMatchesBaseUrl('ws://slick.example.com/api/ws', 'http://slick.example.com'), true)
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://slick.example.com/other/api/ws', 'https://slick.example.com/gateway'),
    false
  )
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://other.example.com/gateway/api/ws', 'https://slick.example.com/gateway'),
    false
  )
})
...
test('profileRemoteOverride returns the per-profile remote with defaulted auth mode', () => {
  const config = {
    profiles: {
      coder: { mode: 'remote', url: '  https://coder.example.com/slick  ', token: { value: 'sek' } }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://coder.example.com/slick',
    authMode: 'token',
    token: { value: 'sek' }
  })
})
...
test('profileRemoteOverride treats a cloud entry as a remote override', () => {
  ...
  const config = {
    profiles: {
      coder: { mode: 'cloud', url: 'https://agent-1.agents.sophostechne.com', authMode: 'oauth' }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://agent-1.agents.sophostechne.com',
    authMode: 'oauth',
    token: undefined
  })
})
...
test('localProfileEntry preserves inactive SSH drafts but drops Cloud state', () => {
  const ssh = { mode: 'ssh', host: 'box', user: 'alice', remoteSlickPath: '/slick' }
...
// --- translateSelfProfileQuery (registry SSH-scoped slick:api contract) ---
...
test('resolveProfileApiRequest uses exact method and path eligibility for mixed families', () => {
  ...
  assert.deepEqual(
    resolveProfileApiRequest('iris', '/api/model/recommended-default?provider=sophos', {
      requestMethod: 'GET'
    }),
    {
      backendProfile: 'iris',
      requestPath: '/api/model/recommended-default?provider=sophos'
    }
  )
})
...
test('resolveProfileApiRequest routes action-status polls with the action-spawning routes', () => {
  ...
  // install): _spawn_slick_action registers the dynamic action name only in
...
test('normalizeRemoteBaseUrl strips trailing slashes, hash, and query', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/slick/'), 'https://gw.example.com/slick')
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/slick?x=1#frag'), 'https://gw.example.com/slick')
})

test('normalizeRemoteBaseUrl preserves a path prefix', () => {
  assert.equal(normalizeRemoteBaseUrl('https://host/slick'), 'https://host/slick')
})
...
test('normalizeRemoteBaseUrl auto-prepends http:// for scheme-less host:port input', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('gw.example.com/slick/'), 'http://gw.example.com/slick')
})
...
test('buildGatewayWsUrl honors a path prefix', () => {
  assert.equal(buildGatewayWsUrl('https://host/slick', 't'), 'wss://host/slick/api/ws?token=t')
})
...
test('buildGatewayWsUrlWithTicket uses ?ticket= not ?token=', () => {
  const url = buildGatewayWsUrlWithTicket('https://gw.example.com/slick', 'tkt-9')
  assert.equal(url, 'wss://gw.example.com/slick/api/ws?ticket=tkt-9')
...
test('authModeFromStatus returns oauth when auth_required is true', () => {
  assert.equal(authModeFromStatus({ auth_required: true, auth_providers: ['sophos'] }), 'oauth')
})
...
test('cookiesHaveSession detects the bare access-token cookie', () => {
  assert.equal(cookiesHaveSession([{ name: 'slick_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession detects the __Host- and __Secure- prefixed variants', () => {
  assert.equal(cookiesHaveSession([{ name: '__Host-slick_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveSession([{ name: '__Secure-slick_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession is false for an empty value', () => {
  assert.equal(cookiesHaveSession([{ name: 'slick_session_at', value: '' }]), false)
})
...
test('cookiesHaveSession ignores unrelated cookies (AT-only by design)', () => {
  ...
  assert.equal(cookiesHaveSession([{ name: 'slick_session_rt', value: 'x' }]), false)
...
test('AT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(AT_COOKIE_VARIANTS, ['__Host-slick_session_at', '__Secure-slick_session_at', 'slick_session_at'])
})

test('RT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(RT_COOKIE_VARIANTS, ['__Host-slick_session_rt', '__Secure-slick_session_rt', 'slick_session_rt'])
})

// --- cookiesHaveLiveSession (AT or RT — the connectivity check) ---

test('cookiesHaveLiveSession is true for a live access-token cookie', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'slick_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-slick_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-slick_session_at', value: 'x' }]), true)
})
...
test('cookiesHaveLiveSession is true for an RT cookie even with NO access-token cookie', () => {
  ...
  assert.equal(cookiesHaveLiveSession([{ name: 'slick_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-slick_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-slick_session_rt', value: 'x' }]), true)
})

test('cookiesHaveLiveSession is true when both AT and RT are present', () => {
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'slick_session_at', value: 'a' },
      { name: 'slick_session_rt', value: 'r' }
    ]),
    true
  )
})
...
test('cookiesHaveLiveSession is false for empty values', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'slick_session_at', value: '' }]), false)
  assert.equal(cookiesHaveLiveSession([{ name: 'slick_session_rt', value: '' }]), false)
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'slick_session_at', value: '' },
      { name: 'slick_session_rt', value: '' }
    ]),
    false
  )
})
...
// --- cookiesHavePrivySession (Sophos portal / Privy auth, NOT gateway cookies) ---
...
test('cookiesHavePrivySession does NOT treat slick gateway cookies as a portal session', () => {
  // The whole point of Q7: a gateway session cookie is NOT a portal sign-in.
  assert.equal(cookiesHavePrivySession([{ name: 'slick_session_at', value: 'x' }]), false)
  assert.equal(cookiesHavePrivySession([{ name: '__Host-slick_session_rt', value: 'x' }]), false)
})
...
test('cookiesHavePrivyAccessToken is false for empty values, gateway cookies, and non-arrays', () => {
  ...
  assert.equal(cookiesHavePrivyAccessToken([{ name: 'slick_session_at', value: 'x' }]), false)
...
test('gateway WS URL IPC result serializes success and the auth-vs-transport matrix', async () => {
  ...
  for (const error of [
    ...
    new Error('Timed out connecting to Slick backend after 8000ms'),
...
test('gatewayTicketFailure only copies an integer statusCode, not a message prefix', () => {
  ...
  // classifier (makeSophosCloudBackendDownError) handles the prefix at the mint
...
// BEFORE waitForSlickReady. This mirrors main.ts buildRemoteConnection's
// catch — classify a Sophos Cloud server fault via the shared factory, else
