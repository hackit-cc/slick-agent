...
import { makeHackitCloudBackendDownError } from './backend-health'
import {
...
test('profileRemoteOverride treats a cloud entry as a remote override', () => {
  ...
  const config = {
    profiles: {
      coder: { mode: 'cloud', url: 'https://agent-1.agents.hackit.cc', authMode: 'oauth' }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://agent-1.agents.hackit.cc',
    authMode: 'oauth',
    token: undefined
  })
})
...
test('resolveProfileApiRequest uses exact method and path eligibility for mixed families', () => {
  ...
  assert.deepEqual(
    resolveProfileApiRequest('iris', '/api/model/recommended-default?provider=hackit', {
      requestMethod: 'GET'
    }),
    {
      backendProfile: 'iris',
      requestPath: '/api/model/recommended-default?provider=hackit'
    }
  )
})
...
test('authModeFromStatus returns oauth when auth_required is true', () => {
  assert.equal(authModeFromStatus({ auth_required: true, auth_providers: ['hackit'] }), 'oauth')
})
...
// --- cookiesHavePrivySession (Hackit portal / Privy auth, NOT gateway cookies) ---
...
test('gatewayTicketFailure only copies an integer statusCode, not a message prefix', () => {
  ...
  // classifier (makeHackitCloudBackendDownError) handles the prefix at the mint
...
// catch — classify a Hackit Cloud server fault via the shared factory, else
