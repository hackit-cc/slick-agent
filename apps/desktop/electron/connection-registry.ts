 ...
 * runtime, remote gateways, Slick Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@slick/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteSlickPath || '').trim() !== String(activeSsh.remoteSlickPath || '').trim() ||
...
/** Turn `ls ~/.slick/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Slick Cloud
 * instances are platform-managed — we never run `slick update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteSlickPath: input.remoteSlickPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Slick-managed names; an empty result stores nothing.
 ...
 * `remoteSlickPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Slick Cloud' : 'Remote gateway'),
