 ...
 * `iter_slick_node_dirs()` in slick_constants.py, which the Electron main
 * process cannot import.
 */
function slickManagedNodePathEntries(
  slickHome,
  ...
  const root = pathModule.join(slickHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const slickNodeDirs = slickManagedNodePathEntries(slickHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([slickNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeSlickHomeRoot(slickHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!slickHome) {
    return slickHome
  }

  const resolved = pathModule.resolve(String(slickHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). slick_bootstrap sets this inside the child too, but only
