...
const SCAN_MODULE = 'slick_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Slick processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'slick_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Slick process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Slick installation is free.\n' +
    '\n' +
    'Close other Slick windows and terminals, then retry.  If the problem\n' +
    'persists, run `slick update` in a terminal for detailed diagnostics.'
  )
}
