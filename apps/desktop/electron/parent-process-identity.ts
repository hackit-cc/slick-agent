export type ParentWatchdogEnv = {
  SLICK_PARENT_PID: string
  SLICK_PARENT_START_MARKER?: string
  SLICK_PARENT_NONCE?: string
  /** Spawn tag consumed by slick_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { SLICK_PARENT_PID: String(pid) }
  env.SLICK_SPAWN = spawnTag(pid, startMarker)
  ...
  env.SLICK_PARENT_START_MARKER = startMarker
  env.SLICK_PARENT_NONCE = nonce

  return env
}

/** Build the `SLICK_SPAWN` tag mirrored by slick_cli/process_identity.py.
