...
//   SLICK_WIN_SSH_HOST   ssh alias/host of the Windows box
//   SLICK_WIN_SSH_USER   remote user
//   SLICK_WIN_SSH_SLICK absolute path to the remote slick.exe under test
const liveHost = process.env.SLICK_WIN_SSH_HOST || ''
const liveUser = process.env.SLICK_WIN_SSH_USER || ''
const configuredSlick = process.env.SLICK_WIN_SSH_SLICK || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Slick-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredSlick)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredSlick.replace('slick.exe', 'python.exe')}' -m slick_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredSlick.replace('slick.exe', 'python.exe')
        const terminate = `& '${python}' -m slick_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.slickPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m slick_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m slick_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
