...
export function markerPath(slickHome) {
  return path.join(slickHome, '.slick-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(slickHome)
  let raw
 ...
 *
 * The Tauri-based slick-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(slickHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(slickHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Slick to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(slickHome, opts)
