...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('slick:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('slick:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('slick:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('slick:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('slick:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('slick:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('slick:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('slick:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('slick:pet-overlay:control', payload)
  })
}
