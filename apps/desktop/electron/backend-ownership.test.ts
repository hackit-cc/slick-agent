...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'slick serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/slick serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m slick_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/slick --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Slick Runtime\\slick.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('slick chat --query serve'), false)
