...
// registry, and the slick:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // SLICK_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.SLICK_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Slick' TUI into
    ...
    env.TERM_PROGRAM = 'Slick'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a slick/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from SLICK_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.SLICK_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `slick:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('slick:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('slick:terminal:write', (_event, id, data) => {
  ...
  ipcMain.handle('slick:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('slick:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('slick:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
