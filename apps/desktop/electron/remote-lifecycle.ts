 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Slick dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Slick install on the remote (login-shell probe),
 ...
 *     `SLICK_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForSlick in.
 *
 * The minted SLICK_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^SLICK_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.slick/desktop-ssh'
...
// Resolve the remote slick executable. An EXPLICIT path is honored strictly
...
async function locateSlick(ssh, remoteSlickPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The slick binary or wrapper script
    ...
    //     the Slick version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/SophosTechne/slick-agent/issues/74411
    return candidate
  }
  ...
  if (remoteSlickPath) {
    if (await isExecutable(remoteSlickPath)) {
      return resolveLauncher(remoteSlickPath)
    }

    const err: any = new Error(
      `The Slick path you set is not an executable on the remote host: "${remoteSlickPath}". ` +
        'Check the path (it must be the full path to the `slick` binary on the remote, e.g. ' +
        '~/slick-agent/.venv/bin/slick), or clear it to auto-detect.'
    )

    err.kind = 'slick-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v slick')}`)).trim()
  ...
  candidates.push('~/.local/bin/slick')
  candidates.push('/usr/local/bin/slick')
  candidates.push('~/.slick/slick-agent/venv/bin/slick')
  ...
  const err: any = new Error(
    'Slick is not installed on the remote host (could not find a `slick` executable). ' +
      'Install it on the remote with:  curl -fsSL https://slick-agent.sophostechne.com/install.sh | sh  ' +
      '— or set the Slick path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<slick> --version`,
// e.g. "Slick Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH slick a
// connection uses, so a stale/unexpected install is visible.
async function probeSlickVersion(ssh, slickPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(slickPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Slick Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The SLICK_HOME the remote dashboard will use (explicit env wins, else
// ~/.slick). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteSlickHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${SLICK_HOME:-$HOME/.slick}"')).trim().split('\n').pop()

    return out || '~/.slick'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Slick home.')
...
marker=home/'.slick-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Slick checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, slickHome) {
  const home = assertSafeRemoteHome(slickHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Slick install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Slick update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Slick update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteSlickProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteSlickHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Slick profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Slick home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'slickPath', 'slickHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !slickPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(slickPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/slick`, leaving neither
      // launcher nor SLICK_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `slick_home=os.path.expanduser(${shq(slickHome)}) if ${shq(slickHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if slick_home:\n' +
      ' expected_entries.add(os.path.join(slick_home,"slick-agent","venv","bin","slick"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.slickPath)
  const expectedHome = lock.slickHome ? expandRemotePath(lock.slickHome) : "''"
  ...
  const executableMatch = lock.slickHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.slickPath === expected.slickPath &&
    lock.slickHome === expected.slickHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.slickPath)})
slick_home=os.path.expanduser(${py(lock.slickHome)})
expected_entries={expected_path,os.path.join(slick_home,"slick-agent","venv","bin","slick")}
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"slick-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(slickPath, profile, opts: any = {}) {
  const slick = expandRemotePath(slickPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.slickHome || '~/.slick')}/.slick-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.slickHome || '~/.slick')}/.slick-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env SLICK_DESKTOP=1 ${slick} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} slick-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, slickPath) {
  const slick = expandRemotePath(slickPath)

  const out = await ssh.exec(
    `help="$(${slick} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ssh,
  { slickPath, profile, token, ownershipId, slickHome = '~/.slick', assertInstallClear = async () => {} }
) {
  if (!(await remoteSupportsSshOwnership(ssh, slickPath))) {
    const err: any = new Error(
      'The remote Slick install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Slick on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(slickPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        slickHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForSlick, a token-gated
...
async function connect(deps) {
  ...
  const slickHome = await probeRemoteSlickHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, slickHome)
  const slickPath = await locateSlick(ssh, remoteSlickPath)
  log(`located slick at ${slickPath}`)
  const slickVersion = await probeSlickVersion(ssh, slickPath)

  if (slickVersion) {
    log(`remote slick version: ${slickVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Slick Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.slickPath === slickPath &&
      lock.slickHome === slickHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    slickHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, slickHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForSlick(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
