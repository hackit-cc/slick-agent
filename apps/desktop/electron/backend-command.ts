// Backend subcommand routing for the desktop-managed Slick process.
//
// The desktop app launches its own headless backend via `slick serve` — it
...
// app hasn't updated yet, or an older `slick` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Slick profile to pin via `--profile`.
 */
 ...
 * `-m slick_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `slick_cli/subcommands/dashboard.py` source registers
