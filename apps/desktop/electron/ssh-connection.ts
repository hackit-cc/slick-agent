...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(SLICK_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Slick-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.slick/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'slick-desktop-ssh')
  }

  return path.join(os.homedir(), '.slick', 'desktop-ssh')
}
