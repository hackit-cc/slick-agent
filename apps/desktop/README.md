...
<p align="center">
  <a href="https://github.com/SophosTechne/slick-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [Slick Agent](../../README.md) — the self-improving AI agent from [Sophos Techne](https://sophostechne.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Slick surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Slick and hear it back.</td></tr>
...
### Install with Slick (recommended)

Already have the Slick CLI? Just run:

```bash
slick desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Slick gateway or install Slick locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Slick Desktop website.](https://slick-agent.sophostechne.com/).
...
```bash
# throwaway SLICK_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Slick Agent runtime into `SLICK_HOME`
(`~/.slick`, or `%LOCALAPPDATA%\slick` on Windows), using the same layout as a
CLI install.
...
- **Slick Agent** runs as a headless `slick serve` process and exposes the
...
1. `SLICK_DESKTOP_SLICK_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `SLICK_DESKTOP_SLICK`, or `slick` on `PATH`
5. a system Python that can import the Slick runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Slick
...
screen offers **Connect to existing Slick** before starting the local installer.
...
terminal commands, and file operations run against the remote Slick host, not
...
transport- or Slick-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Slick-Session-Token`.
...
Boot logs land in `SLICK_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\slick\slick-agent\.slick-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\slick\slick-agent\venv"
```

> The default Slick home on Windows is `%LOCALAPPDATA%\slick`. Set the `SLICK_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://slick-agent.sophostechne.com/docs/)
