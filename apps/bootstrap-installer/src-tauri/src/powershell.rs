...
/// read side does not see EOF until the last of them closes it. `slick update`
...
/// stranded with it.
///
/// Same bound `Invoke-SlickStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `slick_home_override` propagates to the child as $SLICK_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, slick_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = slick_home_override {
        cmd.env("SLICK_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, slick_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = slick_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_slick_home() {
    ...
    /// child. `slick update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
