...
//!
//! Mirrors `slick_constants.get_slick_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\slick
//!   macOS:   ~/.slick
//!   Linux:   ~/.slick  (override via $SLICK_HOME)
//!
//! NOTE (macOS): Python's get_slick_home(), scripts/install.sh, and the
//! Electron desktop's resolveSlickHome() ALL use ~/.slick on macOS — there
...
/// Returns the canonical Slick home directory, respecting $SLICK_HOME if set.
pub fn slick_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("SLICK_HOME") {
    ...
    {
        // %LOCALAPPDATA%\slick — matches scripts/install.ps1's $SlickHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("slick");
        }
    }

    // macOS + Linux + fallback: ~/.slick (matches Python get_slick_home(),
    // install.sh, and the Electron desktop's resolveSlickHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".slick");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    slick_home().join("bootstrap-cache")
}
...
/// SLICK_HOME so it survives repo checkout deletion (unlike anything under
/// slick-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\slick\slick-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_slick`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under SLICK_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves SLICK_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    slick_home().join(".slick-update-in-progress")
}
...
pub fn copy_self_to_slick_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to SLICK_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_SLICK_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".slick-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under SLICK_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[slick-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("SLICK_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_slick_home() -> String {
    slick_home().to_string_lossy().into_owned()
}
