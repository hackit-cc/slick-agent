...
//!
//! Driven when the installer is launched as `Slick-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Slick desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `slick update`
//!      or repair bootstrap can race locked files),
//!   2. run `slick update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in slick_cli/main.py),
//!   3. run `slick desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `slick update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_slick) and
...
/// `slick update` exit code meaning "another slick process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// slick_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `slick update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `slick update` claims
/// the same file (see `slick_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `slick update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in slick_cli/update_lock.py — all three read
...
/// a dashboard-spawned `slick update`) still blocks.
...
/// the marker name our PID — because a `slick update` child that
...
async fn run_update(app: AppHandle) -> Result<()> {
    let slick_home = crate::paths::slick_home();
    let install_root = slick_home.join("slick-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_slick`
    ...
    //
    // The same marker is the cross-process update lock (slick_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `slick update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Slick update is already running (PID {}, started {} ago). \
    ...
    let slick = resolve_slick(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the slick CLI under {}. Is Slick installed? \
    ...
    // async on Windows. If it still holds the venv shim, `slick update`
    ...
    // ---- stage 2: slick update -----------------------------------------
    // Pass --branch so `slick update` targets the branch this installer was
    ...
    // without --branch, `slick update` switches the checkout to `main` (a
    ...
    // --force skips `slick update`'s Windows running-exe guard (which would
    ...
    // time `slick update` runs there is no legitimate slick.exe to protect,
    // and the guard would only produce a false "Slick is still running" stop.
    //
    ...
    // probe only checks the slick.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Slick windows" message.
    ...
    // Retry-once for the update-boundary crash. `slick update` lazily imports
    ...
    // second `slick update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Slick" state
    // a retry can't fix.
    ...
    // checkout predating the SLICK_UPDATE_HANDOFF_PID env fix (8c76fe19f)
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Slick is still running. Close all Slick windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "slick update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::slick_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: slick desktop --build-only ----------------------------
    // `slick update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `slick update` already does above, and `slick update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `slick desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Slick manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_slick_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Slick to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // Slick.exe, so an image-name kill would tear down the app itself.
            emit_log(
                app,
                Some(stage),
                LogStream::Stdout,
                &format!(
                    "[handoff] Slick still holding install files ({}); locating backend shims…",
                    format_locked_paths(&locked)
                ),
            );
            let shim = venv_slick(install_root);
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_slick(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Slick.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Slick.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Find processes running the exact `venv\Scripts\slick.exe` shim for this
/// installation. Windows image names are case-insensitive and the desktop is
/// also Slick.exe, so matching by image name alone is unsafe.
#[cfg(windows)]
...
/// Spawn `slick <args>` from `cwd`, stream stdout/stderr as Log events on the
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `slick update` is precisely the shape that
...
/// Path to the venv slick shim under an install root, regardless of existence.
fn venv_slick(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("slick.exe")
    } else {
        install_root.join("venv").join("bin").join("slick")
    }
}

/// Resolve the slick CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `slick` on PATH.
fn resolve_slick(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_slick(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "slick.exe" } else { "slick" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let slick_home = crate::paths::slick_home();
    let mut envs = vec![(
        "SLICK_HOME".to_string(),
        slick_home.as_os_str().to_os_string(),
    )];
    // `slick update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `slick update` child claims that SAME lock (slick_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Slick is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in slick_cli/update_lock.py.
    envs.push((
        "SLICK_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        slick_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_slick_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Slick.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.slick-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.slick-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::slick_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_slick_is_under_install_root() {
        let root = Path::new("/x/slick-agent");
        let shim = venv_slick(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/slick-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/slick-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "SLICK_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the slick update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_slick(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/slick-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn same_windows_path_accepts_case_only_difference() {
        assert!(same_windows_path(
            Path::new(r"C:\Users\tester\.slick\slick-agent\venv\scripts\SLICK.EXE"),
            Path::new(r"c:\users\tester\.slick\slick-agent\venv\Scripts\slick.exe"),
        ));
    }
    ...
    fn same_windows_path_rejects_desktop_binary() {
        assert!(!same_windows_path(
            Path::new(r"C:\Users\tester\.slick\slick-agent\apps\desktop\Slick.exe"),
            Path::new(r"C:\Users\tester\.slick\slick-agent\venv\Scripts\slick.exe"),
        ));
    }
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".slick-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `slick update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(slickHome, child.pid) races ahead
    ...
    // checkout's `slick update` reads it as a live foreign update and exits
    ...
    fn missing_or_garbage_marker_never_heals() {
        ...
        let garbage = dir.join(".slick-update-in-progress");
    ...
    fn heal_end_to_end_marker_lifecycle() {
        ...
        // own exit 2, e.g. a genuinely still-running Slick, stays terminal).
        assert!(!should_heal_self_marker_refusal(
            Some(UPDATE_EXIT_CONCURRENT),
            &marker
        ));
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Slick.app"]),
            Some(PathBuf::from("/Applications/Slick.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Slick.app");
        let tmp = base.join("Slick.app.slick-update-new");
        let old = base.join("Slick.app.slick-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Slick.app");
        let tmp = base.join("Slick.app.slick-update-new"); // intentionally absent
        let old = base.join("Slick.app.slick-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Slick.app");
        let tmp = base.join("Slick.app.slick-update-new"); // absent
        let old = base.join("Slick.app.slick-update-old");
        write_marker(&target, "OLD");
