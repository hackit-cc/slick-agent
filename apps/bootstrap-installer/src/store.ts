...
export const $slickHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.slick/logs/bootstrap-installer.log')
    $slickHome.set('~/.slick')
  ...
  try {
    const [logPath, slickHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_slick_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $slickHome.set(slickHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Slick" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Slick-Setup.exe --update);
...
export async function launchSlickDesktop(): Promise<void> {
  ...
  await invoke('launch_slick_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Slick repository' },
