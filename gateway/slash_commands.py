...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        async def _finish_switch() -> str:
            ...
            # Copilot, and Hackit-enforced caps win over the raw models.dev entry.
            mi = result.model_info
            from slick_cli.model_switch import resolve_display_context_length_async
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        """Handle /topup -- show the Hackit balance and hand off to the portal.
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # ── Hackit credits magnitudes + monthly-grant % gauge ─────────────
        # Shared with the CLI / TUI /usage block via hackit_credits_lines(): a single
        # auth-gate + portal-fetch + render path (which also honors the dev fixture).
        # Run off the event loop. The helper gates on "a Hackit account is logged in"
        # — NOT the inference provider and NOT nested under `if provider:` — so a
        # Hackit-credentialled user running inference elsewhere (or with none resident)
        ...
        try:
            from agent.account_usage import hackit_credits_lines

            credits_lines = await asyncio.to_thread(hackit_credits_lines, markdown=True)
        except Exception:
