...
(the updater, `slick serve`/dashboard, the Desktop app) currently discovers
...
- ``identify`` → pid, profile label, slick_home, code_sha/code_version
...
- POSIX: Unix domain socket at ``$SLICK_HOME/gateway.sock``. When the home
  path is too long for ``sun_path`` (~104 bytes on macOS/BSD), the socket is
  bound in the system temp dir and a pointer file
  ``$SLICK_HOME/gateway.sock.path`` records the real location; clients
  follow the pointer transparently.
- Windows: named pipe ``\\\\.\\pipe\\slick-gateway-<home-hash>`` served via
...
Consumers (``slick update --plan`` inventory, the post-update fleet version
...
def windows_pipe_name(home: Path) -> str:
    """Per-SLICK_HOME named pipe path (Windows transport)."""
    return rf"\\.\pipe\slick-gateway-{_home_hash(home)}"
...
def _fallback_socket_path(home: Path) -> Path:
    ...
    """
    name = f"slick-gw-{_home_hash(home)}.sock"
...
def _detect_supervisor() -> str:
    ...
    if sys.platform == "darwin" and (
        env.get("XPC_SERVICE_NAME", "").startswith("ai.slick")
        or env.get("LAUNCHD_SOCKET")
    ):
        return "launchd"
    if env.get("SLICK_DESKTOP_MANAGED"):
        return "desktop"
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "slick_home": record.get("slick_home"),
        "profile": _profile_label_for_home(record.get("slick_home") or ""),
...
class GatewayControlServer:
    ...
    its SLICK_HOME), ``stop()`` on shutdown. All failures are non-fatal —
    ...
    def __init__(
        ...
        *,
        ...
        if home is None:
            from gateway.status import _get_process_slick_home
