...
Defenses 1 and 2 (the ``_SLICK_GATEWAY`` guard on ``slick gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.slick.gateway")``,
...
State lives in ``<SLICK_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_slick_home() / "gateway" / "restart_loop.json"
