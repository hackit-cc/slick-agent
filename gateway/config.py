...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from slick_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (slick_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Slick is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_slick_home() / "sessions")
    ...
    # gateway behaves exactly as before — single SLICK_HOME, no profile stamping.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        # set to a recognized value. Hosted deployments (Hackit Portal / Fly) stamp
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.slick/config.yaml (primary user-facing config)
    3. ~/.slick/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_slick_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through slick_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `slick config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `slick config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``slick config set gateway.streaming.*``
