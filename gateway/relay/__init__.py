...
def _resolve_relay_identity_token() -> str:
    ...
    ``slick gateway enroll`` CLI. Three modes, in precedence order:
      ...
      1. **Generic OIDC client-credentials** (air-gapped / self-hosted-IdP, NO
         Hackit Portal): when ``gateway.idp.token_url`` (or
      ...
      2. **Hackit Portal** (default): ``resolve_hackit_access_token()`` — existing
    ...
    if not token_url:
        # Mode 2 — Hackit Portal (default, unchanged behaviour).
        from slick_cli.auth import resolve_hackit_access_token

        return resolve_hackit_access_token()
...
def self_provision_relay() -> bool:
    ...
    is already present, AND the agent can resolve its own Hackit access token. In
    that case the runtime resolves the agent's own Hackit access token (the same
    ``resolve_hackit_access_token()`` the enroll CLI / dashboard register use),
    ...
    self-guarding:
      ...
      - A self-hosted box with a relay URL but no NAS identity:
        ``resolve_hackit_access_token()`` fails -> graceful no-op.
