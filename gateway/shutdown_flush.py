...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<slick_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active SLICK_HOME."""
    from slick_constants import get_slick_home

    flush_dir = get_slick_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<slick_home>/pending_messages/``), so a runtime cap rotation no
