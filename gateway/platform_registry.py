...
class PlatformEntry:
    ...
    # status displays (``slick setup``, ``slick status``, the dashboard
    ...
    # Env vars this platform needs (for ``slick setup`` display).
    ...
    # built-ins).  Used by ``slick gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved SLICK_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `slick`
        # invocation -- including plain `slick chat`, which never touches any
        ...
        # `slick setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``slick setup``/``gateway status``, channel
