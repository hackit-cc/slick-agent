...
There are two ways to add a platform to the Slick gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.slick/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Slick code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `slick gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `slick_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Slick core.
...
And add it to the `slick-gateway` composite:

```python
"slick-gateway": {
    "includes": [..., "slick-your-platform"]
}
```
...
## 12. Status Display (`slick_cli/status.py`)
...
## 13. Gateway Setup Wizard (`slick_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ slick_cli/ toolsets.py \
  --include="*.py" -l | sort -u
