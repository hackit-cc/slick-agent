...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.slick to the active profile home."""
    ...
    if raw == "~/.slick":
        return slick_home
    if raw.startswith("~/.slick/"):
        return slick_home / raw.removeprefix("~/.slick/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return slick_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under SLICK_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from slick_constants import get_slick_home

    scripts_root = (get_slick_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.slick" or raw_text.startswith("~/.slick/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__slick_ignore__") is True
        ):
            return False, None
