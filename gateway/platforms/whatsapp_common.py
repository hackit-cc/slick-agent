...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "😎 *Slick Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to SLICK_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/slick), this function
    mirrors the bridge source to a writable SLICK_HOME location and returns that
    ...
    # Try SLICK_HOME location first
    slick_home = get_slick_home()
    slick_home_bridge = slick_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to SLICK_HOME if needed
    if slick_home_bridge.exists():
        return slick_home_bridge

    # Mirror the bridge source to SLICK_HOME
    try:
        slick_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            slick_home_bridge,
            dirs_exist_ok=False,
