...
#
# Wired into the image as /etc/cont-init.d/01-slick-setup by the
...
SLICK_HOME="${SLICK_HOME:-/opt/data}"
INSTALL_DIR="/opt/slick"

# Drop to slick via s6-setuidgid, but skip it when already non-root.
as_slick() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid slick "$@"; }
...
# starts non-root. The baked install tree under /opt/slick is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $SLICK_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass SLICK_UID/SLICK_GID — or the PUID/PGID aliases — which the
...
# `id -u slick` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# slick UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u slick)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-slick UID).
...
and the baked /opt/slick install tree is intentionally root-owned and
...
# --- Bootstrap SLICK_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid slick mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom SLICK_HOME paths whose parents only
# root can create (e.g. `SLICK_HOME=/home/slick/.slick` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for SLICK_UID/SLICK_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  SLICK_UID/
# SLICK_GID still win when both are set.  See #15290, salvages #25872.
SLICK_UID="${SLICK_UID:-${PUID:-}}"
SLICK_GID="${SLICK_GID:-${PGID:-}}"

if [ -n "${SLICK_UID:-}" ] && validate_uid_gid "$SLICK_UID" && [ "$SLICK_UID" != "$(id -u slick)" ]; then
    echo "[stage2] Changing slick UID to $SLICK_UID"
    usermod -u "$SLICK_UID" slick
fi
if [ -n "${SLICK_GID:-}" ] && validate_uid_gid "$SLICK_GID" && [ "$SLICK_GID" != "$(id -g slick)" ]; then
    echo "[stage2] Changing slick GID to $SLICK_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$SLICK_GID" slick 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised slick user (UID 10000)
...
# slick process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes slick. Idempotent across container
...
#   - slick is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G slick 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] slick already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" slick 2>/dev/null; then
        echo "[stage2] Added slick to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group slick failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When SLICK_UID is remapped or the top-level $SLICK_HOME isn't owned by
# the runtime slick UID, restore ownership to slick — but ONLY for the
# directories slick actually writes to. The full $SLICK_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of slick-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_slick_uid=$(id -u slick)
...
chown_slick_tree() {
    ...
    chown -R slick:slick "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_slick_owner() {
    target="$1"
    find "$target" \( ! -user slick -o ! -group slick \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$SLICK_HOME" 2>/dev/null)" != "$actual_slick_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $SLICK_HOME (targeted) to slick ($actual_slick_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $SLICK_HOME: chown the directory itself (not its contents)
    # so slick can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$SLICK_HOME"; then
        :
    else
        chown slick:slick "$SLICK_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $SLICK_HOME failed (rootless container?) — continuing"
    fi
    # Slick-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by slick (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$SLICK_HOME/$sub" ] && tree_has_non_slick_owner "$SLICK_HOME/$sub"; then
            chown_slick_tree "$SLICK_HOME/$sub"
        fi
    done
fi
...
# slick user. Hosted/container instances keep mutable state under
# $SLICK_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# SLICK_DISABLE_LAZY_INSTALLS=1. Keeping /opt/slick root-owned and
...
# $SLICK_HOME/lazy-packages dir on the data volume (Dockerfile sets
# SLICK_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to slick
...
# Always reset ownership of $SLICK_HOME/profiles to slick on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> slick …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as slick and walks
...
if [ -d "$SLICK_HOME/profiles" ] && tree_has_non_slick_owner "$SLICK_HOME/profiles"; then
    chown_slick_tree "$SLICK_HOME/profiles"
fi

# Always reset ownership of $SLICK_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged slick runtime even
...
# gate as profiles/).
if [ -d "$SLICK_HOME/cron" ] && tree_has_non_slick_owner "$SLICK_HOME/cron"; then
    chown_slick_tree "$SLICK_HOME/cron"
fi

# Always ensure logs/gateways is slick-owned (#45258). Formerly healed by
...
# top-level $SLICK_HOME is mis-owned, so a warm volume with slick-owned
# SLICK_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid slick mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as slick.
if [ -d "$SLICK_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$SLICK_HOME/logs/gateways"; then
        :
    else
        chown slick:slick "$SLICK_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# slick pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged slick gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $SLICK_HOME is
...
# and cron/.
if [ -d "$SLICK_HOME/platforms/pairing" ] && tree_has_non_slick_owner "$SLICK_HOME/platforms/pairing"; then
    chown_slick_tree "$SLICK_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$SLICK_HOME/pairing" ] && tree_has_non_slick_owner "$SLICK_HOME/pairing"; then
    chown_slick_tree "$SLICK_HOME/pairing"
fi

# Reset ownership of slick-owned top-level state files on every boot.
# The targeted data-volume chown above only covers slick-owned
# *subdirectories*; loose state files living directly under $SLICK_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> slick …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged slick runtime then hits
...
# sweep so host-owned files in a bind-mounted $SLICK_HOME are never
...
# entries of slick_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$SLICK_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$SLICK_HOME/$f"; then
            :
        else
            chown slick:slick "$SLICK_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the slick runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$SLICK_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$SLICK_HOME/config.yaml"; then
        :
    else
        chown slick:slick "$SLICK_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$SLICK_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as slick user ---
# Run as slick via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $SLICK_HOME values
...
as_slick mkdir -p \
    ...
    "$SLICK_HOME/sessions" \
    "$SLICK_HOME/logs" \
    "$SLICK_HOME/logs/gateways" \
    "$SLICK_HOME/hooks" \
    "$SLICK_HOME/memories" \
    "$SLICK_HOME/skills" \
    "$SLICK_HOME/skins" \
    "$SLICK_HOME/plans" \
    "$SLICK_HOME/workspace" \
    "$SLICK_HOME/home" \
    "$SLICK_HOME/pairing" \
    "$SLICK_HOME/platforms/pairing" \
    "$SLICK_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/slick/.install_method (see Dockerfile), NOT written here into
# $SLICK_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $SLICK_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.slick:/opt/data) and sometimes shared with a
...
# 'slick update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $SLICK_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/slick stamp, so deleting the data-dir copy is safe).
if [ -f "$SLICK_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$SLICK_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$SLICK_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$SLICK_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$SLICK_HOME/$dest"; then
            :
        else
            as_slick cp "$INSTALL_DIR/$src" "$SLICK_HOME/$dest"
        fi
    fi
}
...
# container's only public HTTP door — can forward Webcron cron fires into the
...
# the first-boot seed above depends on /opt/slick/.env.example being present
...
# not generate one. Slick loads $SLICK_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$SLICK_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$SLICK_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $SLICK_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$SLICK_HOME/.env" ] && ! refuse_symlinked_path "clean" "$SLICK_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$SLICK_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$SLICK_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$SLICK_HOME/.env"; then
        :
    else
        if [ ! -f "$SLICK_HOME/.env" ]; then
            ...
            (umask 077 && as_slick touch "$SLICK_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$SLICK_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$SLICK_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$SLICK_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $SLICK_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $SLICK_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$SLICK_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$SLICK_HOME/.env"; then
        :
    else
        chown slick:slick "$SLICK_HOME/.env" 2>/dev/null || true
        chmod 600 "$SLICK_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `slick` user — without this it gets EACCES on every
...
# slick group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp slick /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted slick group access to the Fly Machines API socket"
    else
...
# $SLICK_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `slick update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set SLICK_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$SLICK_HOME/config.yaml" ]; then
    s6-setuidgid slick "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$SLICK_HOME/auth.json" ] && [ -n "${SLICK_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$SLICK_HOME/auth.json"; then
        :
    else
        printf '%s' "$SLICK_AUTH_JSON_BOOTSTRAP" > "$SLICK_HOME/auth.json"
        chown slick:slick "$SLICK_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$SLICK_HOME/auth.json"
    fi
fi

# auth.json: re-seed a TERMINALLY-DEAD Hackit bootstrap session (self-heal).
#
# The [ ! -f ] guard above deliberately refuses to clobber an existing
# auth.json, so a container whose Hackit bootstrap session took a terminal
# invalid_grant (tokens cleared, providers.hackit.last_auth_error.relogin_required
...
# supply a freshly-issued session via SLICK_AUTH_JSON_REBOOTSTRAP (distinct
# from the create-only *_BOOTSTRAP var); this helper swaps ONLY the
# providers.hackit entry when the on-disk entry is provably terminal OR the
...
if [ -f "$SLICK_HOME/auth.json" ] && [ -n "${SLICK_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$SLICK_HOME/auth.json"; then
        :
    else
        s6-setuidgid slick "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_hackit_session.py" \
            "$SLICK_HOME/auth.json" \
            || echo "[stage2] Warning: docker_rebootstrap_hackit_session.py failed; continuing"
    fi
fi
...
# can set SLICK_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# SLICK_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$SLICK_HOME/gateway_state.json" ] && \
        [ "${SLICK_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$SLICK_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$SLICK_HOME/gateway_state.json"
        chown slick:slick "$SLICK_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$SLICK_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_slick "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/slick/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Slick spawns for the
...
# main-wrapper.sh propagates it into the supervised ``slick`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-slick, dashboard, etc.).
