...
# auth.json: re-seed a TERMINALLY-DEAD Sophos bootstrap session (self-heal).
#
# The [ ! -f ] guard above deliberately refuses to clobber an existing
# auth.json, so a container whose Sophos bootstrap session took a terminal
# invalid_grant (tokens cleared, providers.sophos.last_auth_error.relogin_required
...
# providers.sophos entry when the on-disk entry is provably terminal OR the
...
if [ -f "$SLICK_HOME/auth.json" ] && [ -n "${SLICK_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$SLICK_HOME/auth.json"; then
        ...
        s6-setuidgid slick "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_sophos_session.py" \
            "$SLICK_HOME/auth.json" \
            || echo "[stage2] Warning: docker_rebootstrap_sophos_session.py failed; continuing"
    fi
fi
