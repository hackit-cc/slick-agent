...
# Slick WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: SLICK_TINI_SHIM_TARGET overrides the `/init` path (and
# SLICK_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${SLICK_TINI_SHIM_TARGET:-/init}"
WRAPPER="${SLICK_TINI_SHIM_WRAPPER:-/opt/slick/docker/main-wrapper.sh}"
