#!/bin/sh
# shellcheck shell=sh
# /opt/slick/bin/slick — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `slick` user (UID 10000). When an operator runs `docker exec <c> slick ...`
# the default UID is root (0), and any file the command writes under
# $SLICK_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> slick login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Slick is not logged into Hackit
# Portal" on every incoming message — even though `docker exec <c> slick
...
# Fix
# ---
# This shim sits at /opt/slick/bin/slick and is placed earliest on PATH.
# When invoked as root, it drops to the slick user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $SLICK_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user slick`, kanban subagents, etc. — it short-circuits
...
# (/opt/slick/.venv/bin/slick), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set SLICK_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the slick CLI. Default is to drop.

set -e

REAL=/opt/slick/.venv/bin/slick

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "slick-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user slick`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${SLICK_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the slick user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "slick-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "slick-shim: re-run with --user slick or set SLICK_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the slick user's home before dropping privileges. Without
