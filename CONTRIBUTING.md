...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo Hackit/slick-agent "<your terms>"
  gh search prs --repo Hackit/slick-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/hackit-cc/slick-agent/issues?q=) · [PRs (all states)](https://github.com/hackit-cc/slick-agent/pulls?q=is%3Apr).
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Hackit Discord](https://discord.gg/Hackit). Users can install it with `slick skills install`.
...
- Implement the relevant ABC and use the existing plugin discovery path (`~/.slick/plugins/`, project `.slick/plugins/`, or a pip entry point) — see [Build a Slick Plugin](https://slick-agent.hackit.cc/docs/guides/build-a-slick-plugin)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Promote it in the [Hackit Discord](https://discord.gg/Hackit) `#plugins-skills-and-skins` channel so users can find and install it
...
│   ├── auth.py                   # Provider resolution, OAuth, Hackit Portal
...
├── tests/                    # Test suite
├── website/                  # Documentation site (slick-agent.hackit.cc)
│
...
| `~/.slick/auth.json` | OAuth credentials (Hackit Portal) |
...
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (Hackit Portal OAuth, OpenRouter API key, or custom endpoint).
...
- Use [GitHub Issues](https://github.com/hackit-cc/slick-agent/issues)
...
- **Discord**: [discord.gg/Hackit](https://discord.gg/Hackit) — for questions, showcasing projects, and sharing skills
