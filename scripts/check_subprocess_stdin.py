...
When Slick runs in TUI mode, the gateway child process communicates with
...
# ``get_slick_home() / "plugins"`` (user) and ``./.slick/plugins/`` (project,
# gated behind ``SLICK_ENABLE_PROJECT_PLUGINS``) — see
# ``slick_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``slick_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import slick_constants (this script
    ...
    # plugins in get_slick_home()/plugins/ and project plugins in
    # ./.slick/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_slick_home() / "plugins"]
    if os.environ.get("SLICK_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".slick" / "plugins")
