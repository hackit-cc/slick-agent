# ============================================================================
# Slick Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$SlickHome = $(if ($env:SLICK_HOME) { $env:SLICK_HOME } else { "$env:LOCALAPPDATA\slick" }),
    [string]$InstallDir = $(if ($env:SLICK_HOME) { "$env:SLICK_HOME\slick-agent" } else { "$env:LOCALAPPDATA\slick\slick-agent" }),
    ...
    # builds apps/desktop into a launchable Slick.exe.
    #
    # Why opt-in:
    #   * Slick-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Slick.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Slick.exe
    ...
    #     `slick desktop` already builds on demand.
...
# SLICK_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[slick] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'SlickInstall.LongPath').Type) {
            Add-Type -Namespace 'SlickInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [SlickInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -SlickHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('SlickHome')) {
    $SlickHome = ConvertTo-LongPath $SlickHome
} else {
    $SlickHome = ConvertTo-LongPath $(
        if ($env:SLICK_HOME) { $env:SLICK_HOME } else { "$env:LOCALAPPDATA\slick" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:SLICK_HOME) { "$env:SLICK_HOME\slick-agent" } else { "$env:LOCALAPPDATA\slick\slick-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: SlickHome=$SlickHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:SophosTechne/slick-agent.git"
$RepoUrlHttps = "https://github.com/SophosTechne/slick-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Sophos Techne.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $SlickHome)) {
        New-Item -ItemType Directory -Force -Path $SlickHome | Out-Null
    }
    $envFile = Join-Path $SlickHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `slick update`'s npx
    ...
    $prefixDir = Join-Path $SlickHome "node"
...
function Install-Uv {
    # Slick owns its own uv at $SlickHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (slick_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `slick update` stay in sync.
    $managedUv = Join-Path $SlickHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $SlickHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $SlickHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $SlickHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $SlickHome "bin"
        ...
        # (slick_cli/managed_uv.py looks only at $SlickHome\bin\uv.exe).
...
# Put the Slick-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone slick-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Slick-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Slick-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("slick-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and SLICK_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Slick-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Slick setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Slick uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\slick\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Slick needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Slick copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\slick\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``SLICK_GIT_BASH_PATH`` (User scope) so Slick can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Slick-managed PortableGit install instead..."
    }

    # Download PortableGit into $SlickHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $SlickHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Slick features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Slick needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``SLICK_GIT_BASH_PATH`` (User env scope) so Slick can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $SlickHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $SlickHome\git\usr\bin\bash.exe
    $candidates += "$SlickHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$SlickHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("SLICK_GIT_BASH_PATH", $candidate, "User")
            $env:SLICK_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set SLICK_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Slick may not find Git Bash."
...
# range used to provision the Slick-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Slick requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Slick-managed Node.js installation instead..."

    # Prefer a Slick-managed Node from a previous run over a too-old system one.
    $managedNode = "$SlickHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$SlickHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$SlickHome\node"
        Write-Success "Node.js $version found (Slick-managed)"
        ...
        Update-ManagedNpm "$SlickHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Slick-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $SlickHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $SlickHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\slick-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$SlickHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$SlickHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$SlickHome\node") {
                    try {
                        Rename-Item "$SlickHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Slick-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$SlickHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $SlickHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\slick-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `slick
                    # update` path (#4735).
                    ...
                    $stashName = "slick-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. slick-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``slick update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Slick behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running slick processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/SophosTechne/slick-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/SophosTechne/slick-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/SophosTechne/slick-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\slick-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\slick-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `slick update` (see the notes at the shared clone-path
    ...
    # next `slick update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Slick-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running slick process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running slick processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Slick_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (slick.exe) plus its child tree.
            & taskkill /F /T /IM slick.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM slick.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m slick_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not slick.exe.
            ...
            # Slick children re-exec through .slick-runtime, so killing only
            ...
            # user (see slick_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Slick " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `slick update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Slick (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `slick.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install slick-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and slick.exe broken with
    ...
    # here, not 30 seconds later when the user runs `slick`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Slick processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Slick processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Slick and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register slick.exe in dist-info/RECORD but fail to
        ...
        # `slick` command while slick-agent.exe / slick-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m slick_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `slick dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\slick_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `slick dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `slick dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: slick_cli/web_server.py"
        }
    }
...
function Install-SlickCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the slick launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring slick.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "slick.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the slick command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # slick_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("slick", "slick-acp")) {
    ...
    $requiredExe = Join-Path $Destination "slick.exe"
    $requiredCmd = Join-Path $Destination "slick.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the slick command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up slick command..."
    
    if ($NoVenv) {
        $slickBin = "$InstallDir"
    } else {
        # $SlickHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `slick update`'s autostash
        ...
        # installer staged under slick-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-SlickCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $slickBin = "$SlickHome\bin"
        Install-SlickCommandLaunchers -Root $InstallDir -Destination $slickBin | Out-Null
    }
    ...
    #   slick-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The slick-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept slick via $slickBin)"
        }
    }
    
    if ($currentPath -notlike "*$slickBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$slickBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $slickBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set SLICK_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\slick instead
    # of the Unix default ~/.slick
    $currentSlickHome = [Environment]::GetEnvironmentVariable("SLICK_HOME", "User")
    if (-not $currentSlickHome -or $currentSlickHome -ne $SlickHome) {
        [Environment]::SetEnvironmentVariable("SLICK_HOME", $SlickHome, "User")
        Write-Success "Set SLICK_HOME=$SlickHome"
    }
    $env:SLICK_HOME = $SlickHome
    
    # Update current session
    $env:Path = "$slickBin;$env:Path"
    
    Write-Success "slick command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.slick-bootstrap-complete which tells the Slick
    ...
    $markerPath = Join-Path $InstallDir ".slick-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the SLICK_HOME directory structure ($SlickHome, default %LOCALAPPDATA%\slick)
    New-Item -ItemType Directory -Force -Path "$SlickHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SlickHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SlickHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SlickHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SlickHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SlickHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SlickHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SlickHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SlickHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$SlickHome\.env"
    ...
    # Create config.yaml
    $configPath = "$SlickHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Slick's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$SlickHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in slick_cli/default_soul.py. The runtime
        ...
        $soulContent = @"
You are Slick Agent, an intelligent AI assistant created by Sophos Techne. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
"@
        $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    ...
    Write-Success "Configuration directory ready: $SlickHome"
    
    # Seed bundled skills into $SlickHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $SlickHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $SlickHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$SlickHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $SlickHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Slick-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'slick setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\slick-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\slick-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\slick-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `slick tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $SlickHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Slick' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Slick' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'slick tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `slick
...
# paths still lazy-install via install_cua_driver() (slick_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `slick computer-use install` runs. Bounded
        ...
        # Slick' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'slick tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: slick computer-use install"
            }
        } else {
...
# the final `electron` -> `Slick` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Slick.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_slick_desktop command
    # resolves apps/desktop/release/win-unpacked/Slick.exe directly,
    ...
    # enforces the supported Node lines and prepends the Slick-managed Node to
    ...
    # The --dir mode produces an unpacked Slick.exe in
    ...
    # are NOT trying to work around). The Slick icon + product name are
    # stamped onto Slick.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\slick-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Slick.exe",
        "$desktopDir\release\win-arm64-unpacked\Slick.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Slick.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Slick icon + identity are stamped onto Slick.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, slick-agent#38216).
    ...
    #    Slick.exe. We deliberately do NOT point them at `slick desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Slick.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Slick.lnk')
        )
        ...
        # with the Slick icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.slick/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'slick setup'."
        return
    }
    ...
    # Run slick setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m slick_cli.main setup
    } else {
        python -m slick_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $slickCmd = "$InstallDir\venv\Scripts\slick.exe"
    if (-not (Test-Path $slickCmd)) {
        $slickCmd = "slick"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$SlickHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'slick whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: slick gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$SlickHome\logs\gateway.log"
            Start-Process -FilePath $slickCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$SlickHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: slick gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: slick gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$SlickHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$SlickHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SlickHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SlickHome\slick-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   slick              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   slick setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   slick config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   slick config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   slick gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   slick update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Slick repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Slick-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Slick to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://slick-agent.sophostechne.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
