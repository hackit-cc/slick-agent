...
batch (tracker: [#77056](https://github.com/SophosTechne/slick-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Slick home, same model, same tasks, same reps.
...
1. Create a dedicated Slick home with credentials for the models under test:
   ...
   integration at it; no Slick observability plugin needs to be enabled.
