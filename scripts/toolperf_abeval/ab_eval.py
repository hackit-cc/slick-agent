...
measured in the production session DB) through `slick chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    SLICK_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Slick home with credentials for the
...
(tracker: SophosTechne/slick-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "SLICK_HOME": str(HOME),
                "SLICK_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "slick_cli.main", "chat", "--query", q,
