...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[slick] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[slick`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[slick`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        SlickHome = $(if ($paths) { $paths.slick_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\slick\slick-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-slick'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-slick') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
