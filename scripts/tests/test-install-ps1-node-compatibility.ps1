...
# changing PATH, installing software, or touching the user's Slick home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("slick-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$SlickHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -SlickHome $SlickHome -InstallDir $InstallDir
