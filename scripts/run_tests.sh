#!/usr/bin/env bash
# Canonical test runner for slick-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.slick/...)
#
# Usage:
...
# (SLICK_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.slick/slick-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.slick/slick-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${SLICK_PYTHON:-}" ] && [ -x "$SLICK_PYTHON" ] \
    && "$SLICK_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: SLICK_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `slick` binary rather
  # than the devShell hook.
  PYTHON="$SLICK_PYTHON"
  echo "▶ no local venv — using Nix dev venv via SLICK_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and SLICK_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.slick/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.slick"
...
#
#   * SLICK_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * SLICK_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     SLICK_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# SLICK_RUN_SLOW_PET_TESTS / SLICK_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no SLICK_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in SLICK_TEST_IMAGE SLICK_TEST_WORKERS SLICK_TEST_PATHS \
  SLICK_TEST_FILE_TIMEOUT SLICK_TEST_FILE_RETRIES SLICK_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${SLICK_RUN_SLOW_PET_TESTS:+SLICK_RUN_SLOW_PET_TESTS="$SLICK_RUN_SLOW_PET_TESTS"} \
  ${SLICK_E2E_BROWSER:+SLICK_E2E_BROWSER="$SLICK_E2E_BROWSER"} \
