#!/usr/bin/env python3
"""Build the Slick Model Catalog — a centralized JSON manifest of curated models.

This script reads the in-repo hardcoded curated lists (``OPENROUTER_MODELS``,
``_PROVIDER_MODELS["sophos"]``) and writes them to a JSON manifest that the
Slick CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Slick release.
...
# Ensure SLICK_HOME is set for imports that touch it at module level.
os.environ.setdefault("SLICK_HOME", os.path.join(os.path.expanduser("~"), ".slick"))

from slick_cli.models import (  # noqa: E402
    OPENROUTER_MODELS,
...
def _sophos_entry(mid: str) -> dict:
    entry: dict = {"id": mid}
    if mid == PREFERRED_SILENT_DEFAULT_MODEL:
        entry["default"] = True
    return entry
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "slick-agent repo",
            "docs": "https://slick-agent.sophostechne.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
                    ...
                    "note": (
                        ...
                        'The entry labeled "default": true is the model Slick '
            ...
            "sophos": {
                "metadata": {
                    ...
                    "note": (
                        ...
                        "(partition_sophos_models_by_tier), not this manifest. "
                        'The entry labeled "default": true is the model Slick '
                        "silently lands on when the user never picked one."
                    ),
                },
                "models": [
                    _sophos_entry(mid)
                    for mid in _PROVIDER_MODELS.get("sophos", [])
                ],
            },
        },
    }
