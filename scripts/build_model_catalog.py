...
``_PROVIDER_MODELS["hackit"]``) and writes them to a JSON manifest that the
...
def _hackit_entry(mid: str) -> dict:
    entry: dict = {"id": mid}
    if mid == PREFERRED_SILENT_DEFAULT_MODEL:
        entry["default"] = True
    return entry
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            ...
            "docs": "https://slick-agent.hackit.cc/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
            ...
            "hackit": {
                "metadata": {
                    ...
                    "note": (
                        ...
                        "(partition_hackit_models_by_tier), not this manifest. "
                        'The entry labeled "default": true is the model Slick '
                        "silently lands on when the user never picked one."
                    ),
                },
                "models": [
                    _hackit_entry(mid)
                    for mid in _PROVIDER_MODELS.get("hackit", [])
                ],
            },
        },
    }
