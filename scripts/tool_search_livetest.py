#!/usr/bin/env python3
"""Live test harness for Slick Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any slick imports.
ORIGINAL_HOME = os.environ.get("SLICK_HOME")
ORIGINAL_AUTH = Path.home() / ".slick" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.slick/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.slick/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="slick_ts_live_"))
    slick_home = home_dir / ".slick"
    slick_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, slick_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".slick" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, slick_home / ".env")
        ...
        load_slick_dotenv(slick_home=str(Path.home() / ".slick"))
    ...
    (slick_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return slick_home
...
def reset_module_state():
    """Drop cached modules so the new SLICK_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "slick_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["SLICK_HOME"] = str(home)
...
def main():
    ...
    # Restore original SLICK_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SLICK_HOME"] = ORIGINAL_HOME
    else:
