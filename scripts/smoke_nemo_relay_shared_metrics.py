"""Run a real Slick CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_slick_executable(slick_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "slick",
        Path(".venv") / "Scripts" / "slick.exe",
    ):
        candidate = slick_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("slick")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Slick executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Slick source checkout containing .venv/bin/slick",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated SLICK_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["slick.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['slick.client.active']}"
        )
    [model] = by_name["slick.model_route.count"]
    expected_model = {
        "name": "slick.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['slick.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["slick.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['slick.task_run.started']}"
        )
    [terminal] = by_name["slick.task_run.finished"]
    ...
    [tool] = by_name["slick.tool_call.count"]
    ...
    lifecycle = by_name["slick.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["slick.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Slick development environment requires jsonschema"
        ) from exc
    ...
    if metrics["slick.client.active"] != [
        {
            "name": "slick.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['slick.client.active']}"
        )
    [model] = metrics["slick.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['slick.model_route.count']}"
        )
    [terminal] = metrics["slick.task_run.finished"]
    ...
    [tool] = metrics["slick.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["slick.skill.lifecycle.count"]
    ...
    loads = metrics["slick.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    slick_repo = args.slick_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    slick = _resolve_slick_executable(slick_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="slick-relay-shared-metrics-"))
    home = root / "slick-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["SLICK_HOME"] = str(home)
        python_paths = [str(slick_repo)]
    ...
    (root / "slick.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "slick.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Slick exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Slick model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Slick did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Slick did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from slick_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "slick.shared_metrics.v2.schema.json",
    )

    print("Slick -> NeMo Relay shared-metrics smoke test passed")
