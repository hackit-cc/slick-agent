# Behavioral test for install.ps1's slick launcher staging (PR #92092,
...
#
# The test lifts the real Install-SlickCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $SlickHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-SlickCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-SlickCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'slick-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'slick-agent'
    ...
    $missingThrew = $false
    try {
        Install-SlickCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing slick.exe fails the launcher stage'
    ...
    $slickV1 = [byte[]](77, 90, 1)
    $slickV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'slick.exe'), $slickV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-SlickCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $slickV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'slick.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'slick-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'slick.exe'), $slickV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'slick-acp.exe'), $acp)
    Install-SlickCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $slickV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'slick.exe'))) `
        'installer refreshes an existing Slick launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'slick-acp.exe'))) `
    ...
    Install-SlickCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'slick.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'slick.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'slick.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'slick.exe')) -and $cmdBody.Contains('%*')) `
