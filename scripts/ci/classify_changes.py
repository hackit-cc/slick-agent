...
# tree generates llms.txt — the index every LLM (Slick included, via the
# slick-agent skill) reads to learn what Slick can do — and
...
_MCP_CATALOG_FILES = {"slick_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Slick-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``slick serve`` backend, the Docker image)
