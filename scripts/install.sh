...
# Configuration
REPO_URL_SSH="git@github.com:hackit-cc/slick-agent.git"
REPO_URL_HTTPS="https://github.com/hackit-cc/slick-agent.git"
...
print_banner() {
    ...
    echo "│             😎 Slick Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Hackit.                     │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://slick-agent.hackit.cc/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        ...
        log_warn "Node.js $(node --version) is unsupported (Slick requires Node 22.22+, 24.11+, or 26+) — installing Slick-managed Node $NODE_VERSION..."
