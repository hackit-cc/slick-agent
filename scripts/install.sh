...
# Python-driven tool session (e.g. Slick terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:SophosTechne/slick-agent.git"
REPO_URL_HTTPS="https://github.com/SophosTechne/slick-agent.git"
SLICK_HOME="${SLICK_HOME:-$HOME/.slick}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${SLICK_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$SLICK_INSTALL_DIR"
...
#   code at /usr/local/lib/slick-agent, command at /usr/local/bin/slick,
#   data still at /root/.slick (SLICK_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$SLICK_HOME/.no-bundled-skills so future"
            echo "                   'slick update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Slick.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.slick/slick-agent"
            echo "                   default (root, Linux): /usr/local/lib/slick-agent"
            echo "  --slick-home PATH  Data directory (default: ~/.slick, or \$SLICK_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Slick installs the code under"
            echo "  /usr/local/lib/slick-agent and links the command into"
            echo "  /usr/local/bin/slick (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$SLICK_HOME"
            echo "  (default /root/.slick).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$SLICK_HOME/slick-agent are preserved in-place."
...
print_banner() {
    ...
    echo "│             😎 Slick Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Sophos Techne.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `slick update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Slick-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Slick Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install slick command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `slick` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SLICK_HOME/slick-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SLICK_HOME/slick-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/slick-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SLICK_HOME/slick-agent — then preserve it)
#
# Always no-op when the user set --dir or $SLICK_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SLICK_HOME.
    if is_termux; then
        INSTALL_DIR="$SLICK_HOME/slick-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SLICK_HOME/slick-agent/.git" ]; then
            INSTALL_DIR="$SLICK_HOME/slick-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/slick-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/slick-agent"
        ...
        # /usr/local/bin/slick wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/slick"
        log_info "  Data:    $SLICK_HOME (unchanged)"
...
# Point a Slick-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $SLICK_HOME/node/bin — which is NOT on
...
# Slick's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Slick-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$SLICK_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$SLICK_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$SLICK_HOME/node/etc/npmrc"
}

get_slick_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/slick" ]; then
        echo "$link_dir/slick"
    else
        echo "slick"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://slick-agent.sophostechne.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Slick owns its own uv at $SLICK_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (slick_cli/managed_uv.py) looks in the same
    # place, so install.sh and `slick update` stay in sync.
    local _managed_uv="$SLICK_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $SLICK_HOME/bin ..."
    mkdir -p "$SLICK_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/slick-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/slick-uv-installer.$$.sh")"
    ...
    # directly into $SLICK_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$SLICK_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# replaced with the Slick-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Slick-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Slick-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Slick-managed Node from a previous run over a too-old system one.
    if [ -x "$SLICK_HOME/node/bin/node" ] && [ -x "$SLICK_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$SLICK_HOME/node/bin/node" --version)"; then
        export PATH="$SLICK_HOME/node/bin:$PATH"
        log_success "Node.js $("$SLICK_HOME/node/bin/node" --version) found (Slick-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Slick-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Slick requires Node 22.22+, 24, or 26+) — installing Slick-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.slick/node/..."
    ...
    # Place into ~/.slick/node/ and symlink binaries into the same bin dir
    # the slick command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$SLICK_HOME/node"
    mkdir -p "$SLICK_HOME"
    mv "$extracted_dir" "$SLICK_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$SLICK_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$SLICK_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$SLICK_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$SLICK_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.slick/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Slick install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Slick Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `slick update` path
                # (#4735).
                ...
                local stash_name
                stash_name="slick-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``slick update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Slick behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up slick command..."

    if [ "$USE_VENV" = true ]; then
        SLICK_BIN="$INSTALL_DIR/venv/bin/python"
        SLICK_ENTRYPOINT="$INSTALL_DIR/slick"
    else
        SLICK_BIN="$(which slick 2>/dev/null || echo "")"
        if [ -z "$SLICK_BIN" ]; then
            log_warn "slick not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$SLICK_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$SLICK_ENTRYPOINT" ]; }; then
        log_warn "Slick launcher prerequisites not found"
    ...
    # Create a user-facing shim for the slick command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $SLICK_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$SLICK_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/slick"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/slick" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SLICK_BIN" "$SLICK_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/slick" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/slick"
    log_success "Installed slick launcher → $command_link_display_dir/slick"

    # Also expose `slick-agent`. The `slick-agent` console script declared in
    ...
    rm -f "$command_link_dir/slick-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/slick-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SLICK_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/slick-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SLICK_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/slick-agent"
    log_success "Installed slick-agent launcher → $command_link_display_dir/slick-agent"

    # Also expose `slick-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `slick-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Slick as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/slick-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/slick-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SLICK_BIN" "$SLICK_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/slick-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/slick-acp"
    log_success "Installed slick-acp launcher → $command_link_display_dir/slick-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "slick command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v slick' \
                >/dev/null 2>&1; then
        ...
        log_info "slick not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Slick Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Slick Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Slick Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so slick works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.slick directory structure (config at top level, code in subdir)
    mkdir -p "$SLICK_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.slick/.env (top level, easy to find)
    if [ ! -f "$SLICK_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SLICK_HOME/.env"
            log_success "Created ~/.slick/.env from template"
        else
            touch "$SLICK_HOME/.env"
            log_success "Created ~/.slick/.env"
        fi
    else
        log_info "~/.slick/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$SLICK_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.slick/config.yaml (top level, easy to find)
    if [ ! -f "$SLICK_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SLICK_HOME/config.yaml"
            log_success "Created ~/.slick/config.yaml from template"
        fi
    else
        log_info "~/.slick/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in slick_cli/default_soul.py — the
    ...
    if [ ! -f "$SLICK_HOME/SOUL.md" ]; then
        cat > "$SLICK_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.slick/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.slick/"

    # Seed bundled skills into ~/.slick/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `slick update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'slick update'." \
            > "$SLICK_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $SLICK_HOME/.no-bundled-skills"
        log_info "  Future 'slick update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.slick/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.slick/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SLICK_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$SLICK_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.slick/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Slick Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Slick will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Slick Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `slick update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `slick tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$SLICK_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Slick' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$SLICK_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'slick tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `slick tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `slick computer-use install` on their own).
    ...
    # Same upstream installer `slick computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Slick install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Slick'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'slick tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: slick computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'slick setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m slick_cli.main setup < /dev/tty
    else
        python -m slick_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$SLICK_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Slick to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SLICK_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'slick whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                SLICK_CMD="$(get_slick_command_path)"
                $SLICK_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'slick whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'slick gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SLICK_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SLICK_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: slick gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: slick gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $SLICK_CMD gateway > "$SLICK_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.slick/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: slick gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: slick gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.slick-bootstrap-complete, which tells the Slick
    ...
    local marker_path="$INSTALL_DIR/.slick-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SLICK_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SLICK_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SLICK_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}slick${NC}              Start chatting"
    echo -e "   ${GREEN}slick setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}slick config${NC}       View/edit configuration"
    echo -e "   ${GREEN}slick config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}slick gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}slick update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'slick' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'slick' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'slick' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$SLICK_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$SLICK_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$SLICK_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$SLICK_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Slick` rename
...
# _purge_electron_build_cache() used by `slick desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the Slick-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `slick desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Slick" ]; then
            app="$desktop_dir/release/linux-unpacked/Slick"
        elif [ -x "$desktop_dir/release/linux-unpacked/slick" ]; then
            app="$desktop_dir/release/linux-unpacked/slick"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Slick.app" \
            "$desktop_dir/release/mac/Slick.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `slick desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading SLICK_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if SLICK_HOME="$SLICK_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from slick_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `slick update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Slick-managed Node
            # provisioned during prerequisites/node-deps (at $SLICK_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $SLICK_HOME.
    # $SLICK_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'slick update' on this host install.
