...
INSTALL_REF=""
UPSTREAM_URL="${SLICK_DEV_SANDBOX_UPSTREAM:-https://github.com/hackit-cc/slick-agent.git}"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/slick-agent.hackit.cc"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/slick-agent.hackit.cc/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/slick-agent.hackit.cc/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://slick-agent.hackit.cc/install.sh | bash -s -- "$@"
    install_status=$?
