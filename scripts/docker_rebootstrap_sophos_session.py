#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead Sophos bootstrap session.

Background
----------
A Sophos bootstrap session (client_id ``slick-cli-vps``) can take a terminal
``invalid_grant`` and be quarantined locally — the refresh path clears the dead
tokens from ``auth.json`` and stamps
``providers.sophos.last_auth_error.relogin_required = true``. From then on every
...
``SLICK_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Sophos
...
- Surgical: replaces ONLY ``providers.sophos`` in the existing auth.json, leaving
...
# overwrites a terminally-dead Sophos entry on an existing volume.
REBOOTSTRAP_ENV = "SLICK_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "slick-cli-vps"


def _sophos_entry_is_terminal(sophos_state: Any) -> bool:
    """True iff the on-disk Sophos provider entry is in the terminal/quarantined
    state AND holds no usable credential.

    Mirrors the ``terminal`` predicate in ``slick_cli.auth.get_sophos_session_validity``:
    ...
    """
    if not isinstance(sophos_state, dict):
        return False
    last_err = sophos_state.get("last_auth_error")
    ...
    if sophos_state.get("access_token") or sophos_state.get("refresh_token"):
        return False
    return True


def _extract_sophos_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.sophos`` block out of a SLICK_AUTH_JSON_REBOOTSTRAP
    ...
    if not isinstance(sophos, dict) or not sophos:
        return None
    if sophos.get("client_id") != BOOTSTRAP_CLIENT_ID:
        return None
    if not (
        isinstance(sophos.get("access_token"), str)
        and sophos["access_token"].strip()
        and isinstance(sophos.get("refresh_token"), str)
        and sophos["refresh_token"].strip()
    ):
        return None
    return sophos
...
def _seed_is_newer(local_sophos: Any, seed_sophos: dict) -> bool:
    ...
    """
    if not isinstance(local_sophos, dict):
        return False
    local_obtained = _parse_timestamp(local_sophos.get("obtained_at"))
    seed_obtained = _parse_timestamp(seed_sophos.get("obtained_at"))
    return bool(
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "bad_seed"         — seed present but unparseable / no sophos entry
    ...
    seed_sophos = _extract_sophos_from_seed(seed_raw)
    if seed_sophos is None:
        return "bad_seed"
    ...
    local_sophos = providers.get("sophos")
    terminal = _sophos_entry_is_terminal(local_sophos)
    newer_seed = _seed_is_newer(local_sophos, seed_sophos)
    ...
    # Surgical replacement: swap ONLY providers.sophos, preserve everything else.
    providers["sophos"] = seed_sophos
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] Sophos bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    elif result == "reseeded_newer":
        print("[rebootstrap] Applied newer orchestrator-issued Sophos bootstrap session from "
              f"{REBOOTSTRAP_ENV}")
    else:
