...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `slick backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - slick_home (str): The active SLICK_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.slick``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "slick").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'slick memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], slick_home: str) -> None:
        ...
        Called by 'slick memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``slick_home`` is the active SLICK_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE SLICK_HOME.

        ``slick backup`` only walks SLICK_HOME, so any provider state kept
        ...
        of the archive, and ``slick import`` restores them to their original
