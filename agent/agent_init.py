...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.slick/.env``. See Hackit/slick-agent#2765.
...
def init_agent(
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server"}:
    ...
    elif agent.provider in {"hackit", "hackit-portal", "hackit"}:
        ...
        agent.api_mode = hackit_api_mode(agent.model)
    else:
    ...
    # x-hackit-credits-* response headers after each API call.  Session-start
    ...
    # Hackit Slick 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            ...
            if _slick_warn:
                _user_msg = (
                    "⚠ Hackit Slick 3 & 4 models are NOT agentic — they "
