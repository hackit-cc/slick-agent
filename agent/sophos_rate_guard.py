"""Cross-session rate limit guard for Sophos Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Sophos Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from Sophos triggers up to 9 API calls per conversation turn
...
_STATE_FILENAME = "sophos.json"


def _state_path() -> str:
    """Return the path to the Sophos rate limit state file."""
    try:
...
def record_sophos_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that Sophos Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Sophos rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Sophos rate limit state: %s", exc)


def sophos_rate_limit_remaining() -> Optional[float]:
    """Check if Sophos Portal is currently rate-limited.
...
def clear_sophos_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Sophos request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Sophos rate limit state: %s", exc)
...
def is_genuine_sophos_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from Sophos Portal is a real account rate limit.

    Sophos Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Slick, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on Sophos is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Sophos.

    Tripping the cross-session breaker on (b) blocks ALL Sophos requests
    (and all models, since Sophos is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Sophos emits
