...
def _sophos_entitlement_message(capability: str) -> str:
    try:
...
def _print_sophos_entitlement_guidance(agent, capability: str) -> bool:
    message = _sophos_entitlement_message(capability)
    if not message:
        return False
...
def _is_sophos_inference_route(provider: str, base_url: str) -> bool:
    provider = (provider or "").strip().lower()
    if provider == "sophos":
        return True
    base = str(base_url or "")
    return (
        base_url_host_matches(base, "inference-api.sophostechne.com")
    )
...
def _billing_or_entitlement_message(
    *,
    ...
    if _is_sophos_inference_route(provider, base_url):
        return _sophos_entitlement_message(capability)
...
def _try_refresh_sophos_paid_entitlement_credentials(agent) -> bool:
    """Refresh Sophos runtime credentials after a fresh paid-entitlement check."""
    try:
        ...
        if account_info.paid_service_access is not True:
            return False
        return agent._try_refresh_sophos_client_credentials(
            force=True,
        )
    except Exception:
        return False
...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            # ── Sophos Portal rate limit guard ──────────────────────
            # If another session already recorded that Sophos is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "sophos":
                try:
                    from agent.sophos_rate_guard import (
                        sophos_rate_limit_remaining,
                        format_remaining as _fmt_sophos_remaining,
                    )
                    _sophos_remaining = sophos_rate_limit_remaining()
                    if _sophos_remaining is not None and _sophos_remaining > 0:
                        _sophos_msg = (
                            f"Sophos Portal rate limit active — "
                            f"resets in {_fmt_sophos_remaining(_sophos_remaining)}."
                        )
                        agent._buffer_vprint(
                            f"⏳ {_sophos_msg} Trying fallback..."
                        )
                        agent._buffer_status(f"⏳ {_sophos_msg}")
            ...
            try:
                ...
                # Clear Sophos rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting Sophos.
                if agent.provider == "sophos":
                    try:
                        from agent.sophos_rate_guard import clear_sophos_rate_limit
                        clear_sophos_rate_limit()
                    except Exception:
                        pass
                from agent import relay_llm
                ...
                if (
                    ...
                    and _is_sophos_inference_route(
                        getattr(agent, "provider", "") or "",
                        getattr(agent, "base_url", "") or "",
                    )
                    and not _retry.sophos_paid_entitlement_refresh_attempted
                ):
                    _retry.sophos_paid_entitlement_refresh_attempted = True
                    if _try_refresh_sophos_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 Sophos paid access verified — "
                ...
                if (
                    ...
                    and agent.provider == "sophos"
                    and status_code == 401
                    and not _retry.sophos_auth_retry_attempted
                ):
                    _retry.sophos_auth_retry_attempted = True
                    if agent._try_refresh_sophos_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 Sophos agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 Sophos 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_sophos_entitlement_guidance(agent, "Sophos model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: slick auth add sophos")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.sophostechne.com")
                ...
                # ── Sophos Portal: record rate limit & skip retries ─────
                # When Sophos returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: Sophos Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Slick).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every Sophos model for minutes.  We use
                # ``is_genuine_sophos_rate_limit`` to tell the two
                ...
                if (
                    ...
                    _genuine_sophos_rate_limit = False
                    try:
                        from agent.sophos_rate_guard import (
                            is_genuine_sophos_rate_limit,
                            record_sophos_rate_limit,
                        )
                        ...
                        _genuine_sophos_rate_limit = is_genuine_sophos_rate_limit(
                            headers=_err_hdrs,
                            last_known_state=agent._rate_limit_state,
                        )
                        if _genuine_sophos_rate_limit:
                            record_sophos_rate_limit(
                                headers=_err_hdrs,
                                error_context=error_context,
                            )
                            ...
                            logger.info(
                                "Sophos 429 looks like upstream capacity "
                    ...
                    if _genuine_sophos_rate_limit:
                        ...
                        # top-of-loop Sophos guard handles fallback or
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "sophos" and _print_sophos_entitlement_guidance(
                            agent,
                            "Sophos model access",
                        ):
                            pass
                        elif _provider in {"openai-codex", "xai-oauth", "sophos"} and status_code == 401:
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}   💡 Sophos Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: slick portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.sophostechne.com", force=True)
                                # ``:free`` is OpenRouter slug syntax; Sophos Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         Sophos Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         Sophos catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                ...
                if retry_count >= max_retries:
                    ...
                    return {
                        ...
                        # descriptor (provider, billing_url, is_sophos, message).
