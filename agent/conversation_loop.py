...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in slick_state.py — kept local
# to avoid importing slick_state at module load time (its module-level
# DEFAULT_DB_PATH = get_slick_home() / "state.db" breaks tests that
# monkeypatch get_slick_home to return a str).
...
from slick_constants import PARTIAL_STREAM_STUB_ID
from slick_logging import set_session_context
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.slick/prefill.json incident, 20/20 blocked with
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Slick tool use: "
    ...
    return (
        ...
        f"context, but Slick needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Slick config, set `model.ollama_num_ctx: 65536` "
...
def _hackit_entitlement_message(capability: str) -> str:
    try:
...
def _print_hackit_entitlement_guidance(agent, capability: str) -> bool:
    message = _hackit_entitlement_message(capability)
    if not message:
        return False
...
def _is_hackit_inference_route(provider: str, base_url: str) -> bool:
    provider = (provider or "").strip().lower()
    if provider == "hackit":
        return True
    base = str(base_url or "")
    return (
        base_url_host_matches(base, "inference-api.hackit.cc")
    )
...
def _billing_or_entitlement_message(
    *,
    ...
    if _is_hackit_inference_route(provider, base_url):
        return _hackit_entitlement_message(capability)
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            lines = [
                ...
                "Retry with a fresh credential state: `slick auth reset anthropic`. Until "
...
def _try_refresh_hackit_paid_entitlement_credentials(agent) -> bool:
    """Refresh Hackit runtime credentials after a fresh paid-entitlement check."""
    try:
        ...
        if account_info.paid_service_access is not True:
            return False
        return agent._try_refresh_hackit_client_credentials(
            force=True,
        )
    except Exception:
        return False
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    try:
        from slick_cli.lifecycle import invoke_hook as _invoke_hook
        _invoke_hook(
            "on_session_start",
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only Slick' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `slick fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from slick_cli.moa_config import decode_moa_turn
    ...
    # Adopt any ~/.slick/.env credential/base-url edits made since the last
    ...
    # 'disk', or 'unknown' — see slick_state.classify_persistence_error).
    ...
    # all run inside Codex). Default Slick path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Slick internals.
        #
        # Slick invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Slick tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            # ── Hackit Portal rate limit guard ──────────────────────
            # If another session already recorded that Hackit is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "hackit":
                try:
                    from agent.hackit_rate_guard import (
                        hackit_rate_limit_remaining,
                        format_remaining as _fmt_hackit_remaining,
                    )
                    _hackit_remaining = hackit_rate_limit_remaining()
                    if _hackit_remaining is not None and _hackit_remaining > 0:
                        _hackit_msg = (
                            f"Hackit Portal rate limit active — "
                            f"resets in {_fmt_hackit_remaining(_hackit_remaining)}."
                        )
                        agent._buffer_vprint(
                            f"⏳ {_hackit_msg} Trying fallback..."
                        )
                        agent._buffer_status(f"⏳ {_hackit_msg}")
            ...
            try:
                ...
                try:
                    from slick_cli.middleware import apply_llm_request_middleware
                ...
                try:
                    from slick_cli.lifecycle import (
                        has_hook,
                ...
                if env_var_enabled("SLICK_DUMP_REQUESTS"):
                ...
                from slick_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Slick/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                # Clear Hackit rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting Hackit.
                if agent.provider == "hackit":
                    try:
                        from agent.hackit_rate_guard import clear_hackit_rate_limit
                        clear_hackit_rate_limit()
                    except Exception:
                        pass
                from agent import relay_llm
                ...
                if (
                    ...
                    and _is_hackit_inference_route(
                        getattr(agent, "provider", "") or "",
                        getattr(agent, "base_url", "") or "",
                    )
                    and not _retry.hackit_paid_entitlement_refresh_attempted
                ):
                    _retry.hackit_paid_entitlement_refresh_attempted = True
                    if _try_refresh_hackit_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 Hackit paid access verified — "
                ...
                if (
                    ...
                    and agent.provider == "hackit"
                    and status_code == 401
                    and not _retry.hackit_auth_retry_attempted
                ):
                    _retry.hackit_auth_retry_attempted = True
                    if agent._try_refresh_hackit_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 Hackit agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 Hackit 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_hackit_entitlement_guidance(agent, "Hackit model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: slick auth add hackit")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.hackit.cc")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `slick doctor` for credential-chain diagnostics, or")
                    ...
                    from slick_constants import display_slick_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Slick-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: slick config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: slick config set ANTHROPIC_API_KEY \"\"")
                ...
                # and omits the field, and Slick' local compression takes
                ...
                if getattr(api_error, "status_code", None) == 404:
                    try:
                        from slick_cli.model_normalize import suggest_prefixed_model_id
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `slick model`."
                        )
                ...
                # ── Hackit Portal: record rate limit & skip retries ─────
                # When Hackit returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: Hackit Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Slick).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every Hackit model for minutes.  We use
                # ``is_genuine_hackit_rate_limit`` to tell the two
                ...
                if (
                    ...
                    _genuine_hackit_rate_limit = False
                    try:
                        from agent.hackit_rate_guard import (
                            is_genuine_hackit_rate_limit,
                            record_hackit_rate_limit,
                        )
                        ...
                        _genuine_hackit_rate_limit = is_genuine_hackit_rate_limit(
                            headers=_err_hdrs,
                            last_known_state=agent._rate_limit_state,
                        )
                        if _genuine_hackit_rate_limit:
                            record_hackit_rate_limit(
                                headers=_err_hdrs,
                                error_context=error_context,
                            )
                            ...
                            logger.info(
                                "Hackit 429 looks like upstream capacity "
                    ...
                    if _genuine_hackit_rate_limit:
                        ...
                        # top-of-loop Hackit guard handles fallback or
                ...
                # which Slick' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Slick' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`slick",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Slick may add API-only content not present in persisted
                    ...
                    # recovery arms on the TRUE request that overflowed. See slick-lcm engine
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "hackit" and _print_hackit_entitlement_guidance(
                            agent,
                            "Hackit model access",
                        ):
                            pass
                        elif _provider in {"openai-codex", "xai-oauth", "hackit"} and status_code == 401:
                            if _provider == "openai-codex":
                                agent._vprint(f"{agent.log_prefix}   💡 Codex OAuth token was rejected (HTTP 401). Your token may have been", force=True)
                                agent._vprint(f"{agent.log_prefix}      refreshed by another client (Codex CLI, VS Code). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Run `codex` in your terminal to generate fresh tokens.", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Then run `slick auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `slick model`.", force=True)
                            else:  # hackit
                                agent._vprint(f"{agent.log_prefix}   💡 Hackit Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: slick portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.hackit.cc", force=True)
                                # ``:free`` is OpenRouter slug syntax; Hackit Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         Hackit Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         Hackit catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: slick setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        slick fallback add   (interactive picker — same as `slick model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.slick/config.yaml` to extend the per-call "
                            f"timeout. (Slick's built-in floor is 600s for "
                    ...
                    return {
                        ...
                        # descriptor (provider, billing_url, is_hackit, message).
        ...
        try:
            ...
            # with the iterations Slick never saw. Appended before this turn's
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Slick mid-turn, resume logic still sees the
                    ...
                    from slick_state import classify_persistence_error
                ...
                # timeout (SLICK_AGENT_TIMEOUT, default 1800s) and the
                ...
                try:
                    ...
                    from slick_cli.plugins import get_pre_verify_continue_message
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("SLICK_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
            ...
            if sys.is_finalizing() or _is_interpreter_shutdown_error(e):
                ...
                final_response = (
                    ...
                    "resumed with: slick --resume <session-id>"
                )
