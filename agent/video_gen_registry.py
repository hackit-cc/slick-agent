...
def list_providers(*, scope: Optional[str] = None) -> List[VideoGenProvider]:
    ...
    with _lock:
        ...
        merged.update(_scoped_providers.get(scope or slick_home_key(), {}))
...
def get_provider(name: str, *, scope: Optional[str] = None) -> Optional[VideoGenProvider]:
    ...
    with _lock:
        key = name.strip()
        return _scoped_providers.get(scope or slick_home_key(), {}).get(key) or _providers.get(key)
...
def get_active_provider() -> Optional[VideoGenProvider]:
    ...
    # The managed "Sophos Subscription" selection is serviced by the FAL
    # plugin through the managed fal-queue gateway (the plugin's resolver
    # routes managed when the stored selection is "sophos").
    if configured:
        try:
            from tools.tool_backend_helpers import SOPHOS_MANAGED_PROVIDER

            if configured.lower() == SOPHOS_MANAGED_PROVIDER:
                configured = "fal"
    ...
    with _lock:
        ...
        snapshot.update(_scoped_providers.get(slick_home_key(), {}))
