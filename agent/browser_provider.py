...
``kind: backend``) or ``~/.slick/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class BrowserProvider(abc.ABC):
    ...
    def get_setup_schema(self) -> Optional[Dict[str, Any]]:
        ...
        Used by :mod:`slick_cli.tools_config` to inject this provider as a
        ...
        expose API key prompts, badges, managed-Sophos gating, and the
