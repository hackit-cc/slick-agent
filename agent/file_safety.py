...
def _slick_home_path() -> Path:
    """Resolve the active SLICK_HOME (profile-aware) without circular imports."""
    try:
        from slick_constants import get_slick_home  # local import to avoid cycles
        return get_slick_home()
    except Exception:
        return Path(os.path.expanduser("~/.slick"))


def _slick_root_path() -> Path:
    """Resolve the Slick root dir (always the parent of any profile, never per-profile)."""
    try:
        from slick_constants import get_default_slick_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    slick_home = _slick_home_path()
    slick_root = _slick_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(slick_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(slick_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(slick_home / "cache" / "bws_cache.enc.json"),
            str(slick_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved SLICK_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("SLICK_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    slick_dirs = []
    for base in (_slick_home_path(), _slick_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in slick_dirs:
                slick_dirs.append(real)
        except Exception:
            continue

    for base_real in slick_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside SLICK_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Slick path.

    Three categories are blocked:

      * Internal Slick cache files under ``SLICK_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under SLICK_HOME and the global Slick
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.slick/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active SLICK_HOME (profile-aware) AND the global
    # Slick root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (SLICK_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    slick_dirs: list[Path] = []
    for base in (_slick_home_path(), _slick_root_path()):
        try:
            real = base.resolve()
            if real not in slick_dirs:
    ...
    for hd in slick_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Slick cache file "
    ...
    for hd in slick_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Slick credential store "
    ...
    for hd in slick_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Slick MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Slick MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
    ...
    for hd in slick_dirs:
        ...
        if resolved == browser_profile:
            return (
                f"Access denied: {path} is the Slick real-profile browser "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is inside the Slick real-profile browser "
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Slick read (see
...
#
# Slick profiles are separate SLICK_HOME dirs under
...
#
# Reference: May 2026 incident where a slick-security profile session
# edited skills under both ``~/.slick/profiles/slick-security/skills/``
# AND ``~/.slick/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under SLICK_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from SLICK_HOME.
    ...
    """
    try:
        home_real = _slick_home_path().resolve()
        root_real = _slick_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Slick scope, or is inside
...
#
#   <SLICK_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.slick/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.slick/…`` segment and warns
# regardless of which Slick profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.slick/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.slick`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.slick/…`` and returns the
    index where the inner Slick-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .slick / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".slick":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Slick state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.slick``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.slick`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Slick resolver to
    succeed, so it works correctly even when called from contexts where
    SLICK_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Slick process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real SLICK_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.slick/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.slick/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host SLICK_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Slick state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Slick "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real SLICK_HOME. Use the "
