...
Plugins built on Slick Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Slick
...
:class:`~slick_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Slick' main config
...
handles every provider, fallback chain, and per-task override Slick
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`slick_cli.plugins.PluginContext`
