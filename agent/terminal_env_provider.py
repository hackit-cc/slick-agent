...
Providers live in ``~/.slick/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.slick/cache`` files land
  inside the backend (e.g. ``"~/.slick"`` for home-synced backends,
  ``"/root/.slick"`` for root-homed containers), or ``None`` when host
...
The factory stamps ``_slick_backend_name`` on the returned object so
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced Slick cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``slick setup`` after this backend is selected.
