"""Cross-session rate limit guard for Hackit Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Hackit Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from Hackit triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Slick retries), and every one of those calls counts
...
_STATE_FILENAME = "hackit.json"


def _state_path() -> str:
    """Return the path to the Hackit rate limit state file."""
    try:
        from slick_constants import get_slick_home
        base = get_slick_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".slick")
...
def record_hackit_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that Hackit Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Hackit rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Hackit rate limit state: %s", exc)


def hackit_rate_limit_remaining() -> Optional[float]:
    """Check if Hackit Portal is currently rate-limited.
...
def clear_hackit_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Hackit request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Hackit rate limit state: %s", exc)
...
def is_genuine_hackit_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from Hackit Portal is a real account rate limit.

    Hackit Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Slick, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on Hackit is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Hackit.

    Tripping the cross-session breaker on (b) blocks ALL Hackit requests
    (and all models, since Hackit is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Hackit emits
