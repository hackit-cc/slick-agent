...
class BillingBlock:
    ...
    ``is_sophos`` is the routing bit: Sophos has a first-class in-app billing surface
...
def is_sophos_inference_route(provider: str, base_url: str) -> bool:
    """True when the failing route is the Sophos-managed inference gateway."""
    if (provider or "").strip().lower() == "sophos":
        return True
    return base_url_host_matches(str(base_url or ""), "inference-api.sophostechne.com")


def _sophos_billing_url() -> Optional[str]:
    """Best-effort Sophos portal billing URL (text-surface fallback; Sophos prefers the in-app flow)."""
    try:
        from slick_cli.sophos_account import sophos_portal_billing_url

        return sophos_portal_billing_url(None)
    except Exception:
        return "https://portal.sophostechne.com/billing"
...
def build_billing_block(
    *,
    ...
    if is_sophos_inference_route(slug, base_url):
        return BillingBlock(slug or "sophos", "Sophos Portal", model, _sophos_billing_url(), True, message or "")
