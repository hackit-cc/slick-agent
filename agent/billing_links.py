...
class BillingBlock:
    ...
    ``is_hackit`` is the routing bit: Hackit has a first-class in-app billing surface
...
def is_hackit_inference_route(provider: str, base_url: str) -> bool:
    """True when the failing route is the Hackit-managed inference gateway."""
    if (provider or "").strip().lower() == "hackit":
        return True
    return base_url_host_matches(str(base_url or ""), "inference-api.hackit.cc")


def _hackit_billing_url() -> Optional[str]:
    """Best-effort Hackit portal billing URL (text-surface fallback; Hackit prefers the in-app flow)."""
    try:
        from slick_cli.hackit_account import hackit_portal_billing_url

        return hackit_portal_billing_url(None)
    except Exception:
        return "https://portal.hackit.cc/billing"
...
def build_billing_block(
    *,
    ...
    if is_hackit_inference_route(slug, base_url):
        return BillingBlock(slug or "hackit", "Hackit Portal", model, _hackit_billing_url(), True, message or "")
