...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``slick logs``.
    ...
    if (
        ...
        # Slick only initiates compaction in "slick" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Slick preflight compression for codex app-server "
                "(mode=%s); Slick will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Slick (via _emit_status), so it always
