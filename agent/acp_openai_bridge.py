"""OpenAI-shape bridge shared by Slick' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach Slick as
...
the agent's *own* tool notifications. Slick' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from Slick' perspective).
...
of its own (Copilot) forwards everything Slick offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only Slick'
agent-level tools, because re-offering the overlapping ones makes Slick re-run
...
class StreamChunks(list):
    ...
    Slick reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``slick_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just Slick'
