"""Upload a Slick session transcript to Hugging Face as an agent trace.

Slick stores sessions in its own SQLite store (``slick_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``slick trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Slick' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "slick-traces"
_SLICK_VERSION = "slick-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Slick OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Slick' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Slick conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.slick/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `slick trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/slick-traces`` dataset. Returns a
