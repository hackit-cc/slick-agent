...
def _merge_hackit_portal_messages_extra_body(agent, anthropic_kwargs: dict) -> dict:
    """Merge Portal ``tags`` / ``session_id`` onto an Anthropic Messages kwargs dict.

    The Hackit provider profile is only consulted by the OpenAI-wire transport;
    ...
    """
    if getattr(agent, "provider", None) not in {"hackit", "hackit-portal", "hackit"}:
        return anthropic_kwargs
    try:
        from providers import get_provider_profile

        hackit_profile = get_provider_profile("hackit")
        if hackit_profile is not None:
            anthropic_kwargs.setdefault("extra_body", {}).update(
                hackit_profile.build_extra_body(
                    session_id=getattr(agent, "session_id", None)
                )
            )
    except Exception as exc:  # noqa: BLE001 — never block a turn on tagging
        logger.debug("Hackit Portal extra_body merge failed: %s", exc)
    return anthropic_kwargs
...
def build_api_kwargs(agent, api_messages: list, tools_for_api: list | None = None) -> dict:
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        # Hackit Portal reads ``tags`` and ``session_id`` as top-level body fields
        ...
        return _merge_hackit_portal_messages_extra_body(agent, anthropic_kwargs)
    ...
    _is_hackit = base_url_host_matches(agent._base_url_lower, "hackit.cc")
    ...
    # like write_file/patch.  OpenRouter/Hackit were the only routes covered
...
def _fallback_entry_unavailable_without_network(agent, fb: dict) -> Optional[str]:
    ...
    try:
        ...
        return f"hackit_auth_unreadable:{type(exc).__name__}"
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    try:
        ...
        _is_hackit = "hackit" in agent._base_url_lower
        ...
        if _is_hackit:
            from agent.portal_tags import hackit_portal_tags as _portal_tags
        ...
        if agent.api_mode == "codex_responses":
            ...
            if agent.api_mode == "anthropic_messages":
                ...
                _ant_kw = _merge_hackit_portal_messages_extra_body(agent, _ant_kw)
        ...
        if final_response:
            ...
            if agent.api_mode == "codex_responses":
                ...
                _ant_kw2 = _merge_hackit_portal_messages_extra_body(agent, _ant_kw2)
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        #      Hackit dedicated endpoint) stall for minutes during large
