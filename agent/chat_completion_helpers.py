...
from slick_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from slick_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def _merge_sophos_portal_messages_extra_body(agent, anthropic_kwargs: dict) -> dict:
    """Merge Portal ``tags`` / ``session_id`` onto an Anthropic Messages kwargs dict.

    The Sophos provider profile is only consulted by the OpenAI-wire transport;
    ...
    """
    if getattr(agent, "provider", None) not in {"sophos", "sophos-portal", "sophostechne"}:
        return anthropic_kwargs
    try:
        from providers import get_provider_profile

        sophos_profile = get_provider_profile("sophos")
        if sophos_profile is not None:
            anthropic_kwargs.setdefault("extra_body", {}).update(
                sophos_profile.build_extra_body(
                    session_id=getattr(agent, "session_id", None)
                )
            )
    except Exception as exc:  # noqa: BLE001 — never block a turn on tagging
        logger.debug("Sophos Portal extra_body merge failed: %s", exc)
    return anthropic_kwargs
...
def _check_stale_giveup(agent) -> None:
    ...
    _giveup = env_int("SLICK_STREAM_STALE_GIVEUP", 5)
...
def _derive_stream_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    if _cfg_stale is not None:
        ...
        _base = env_float("SLICK_STREAM_STALE_TIMEOUT", 180.0)
...
def _resolve_direct_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    ``SLICK_API_CALL_STALE_TIMEOUT`` → reasoning-model floor → context-size
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # activity. Operators can tune via SLICK_CODEX_TTFB_TIMEOUT_SECONDS and
    # SLICK_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each).
    ...
    # limit. Tunable via SLICK_CODEX_HARD_TIMEOUT_SECONDS (set to 0 to
    # disable the ceiling entirely; that restores the pre-fix behavior).
    _codex_hard_timeout = _env_float("SLICK_CODEX_HARD_TIMEOUT_SECONDS", 1500.0)
    if (
    ...
    # SLICK_CODEX_TTFB_TIMEOUT_SECONDS=0 to disable this watchdog entirely.
    _ttfb_enabled = _codex_watchdog_enabled
    _ttfb_timeout = _env_float("SLICK_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if _ttfb_timeout <= 0:
        _ttfb_enabled = False
    elif _openai_codex_backend:
        _ttfb_disable_above = _env_float("SLICK_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0)
        _ttfb_strict = os.environ.get("SLICK_CODEX_TTFB_STRICT", "").strip().lower() in {
            "1", "true", "yes", "on"
        }
        if (
            not _ttfb_strict
            ...
            if _ttfb_timeout < _large_request_ttfb_timeout:
                logger.info(
                    ...
                    "Set SLICK_CODEX_TTFB_STRICT=1 to keep the smaller cutoff.",
                    _ttfb_timeout,
        ...
        _ttfb_cap = _env_float("SLICK_CODEX_TTFB_MAX_SECONDS", 120.0)
        if _ttfb_cap > 0 and _ttfb_timeout > _ttfb_cap:
            logger.info(
                "Capping openai-codex no-byte TTFB timeout from %.0fs to %.0fs "
                "(context=~%s tokens). Set SLICK_CODEX_TTFB_MAX_SECONDS to tune.",
                _ttfb_timeout,
                _ttfb_cap,
                f"{_est_tokens_for_codex_watchdog:,}",
            )
    ...
    _codex_idle_timeout = _env_float(
        "SLICK_CODEX_EVENT_STALE_TIMEOUT_SECONDS",
...
def build_api_kwargs(agent, api_messages: list, tools_for_api: list | None = None) -> dict:
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        # Sophos Portal reads ``tags`` and ``session_id`` as top-level body fields
        ...
        return _merge_sophos_portal_messages_extra_body(agent, anthropic_kwargs)
    ...
    _is_sophos = base_url_host_matches(agent._base_url_lower, "sophostechne.com")
    ...
    # like write_file/patch.  OpenRouter/Sophos were the only routes covered
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "slick",
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    # Respects SLICK_REDACT_SECRETS via redact_sensitive_text — no-op
    # when disabled. (#19798)
    if isinstance(_san_content, str) and _san_content:
        from agent.redact import redact_sensitive_text
...
def _fallback_entry_unavailable_without_network(agent, fb: dict) -> Optional[str]:
    ...
    try:
        ...
        return f"sophos_auth_unreadable:{type(exc).__name__}"
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        if not fb_api_mode_explicit and fb_api_mode == "chat_completions":
            ...
            elif fb_provider in {"sophos", "sophos-portal", "sophostechne"}:
                # Portal is dual-wire: anthropic/* must land on /v1/messages.
                # resolve_provider_client still returns an OpenAI client for
                # Sophos; the anthropic_messages branch below rebuilds the native
                # client from that credential + base_url.
                from slick_cli.providers import sophos_api_mode

                fb_api_mode = sophos_api_mode(fb_model)
            elif (
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    if getattr(agent, "suppress_status_output", False):
        # Strict machine-readable mode (slick chat -Q, oneshot, background
    ...
    try:
        ...
        for msg in messages:
            ...
            # and every Slick-internal underscore-prefixed scaffolding key.
            ...
            # substitute it before dropping the key (Slick bookkeeping,
        ...
        _is_sophos = "sophostechne" in agent._base_url_lower
        ...
        if _is_sophos:
            from agent.portal_tags import sophos_portal_tags as _portal_tags
        ...
        if agent.api_mode == "codex_responses":
            ...
            if agent.api_mode == "anthropic_messages":
                ...
                _ant_kw = _merge_sophos_portal_messages_extra_body(agent, _ant_kw)
        ...
        if final_response:
            ...
            if agent.api_mode == "codex_responses":
                ...
                _ant_kw2 = _merge_sophos_portal_messages_extra_body(agent, _ant_kw2)
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
        ...
        try:
            ...
            while t.is_alive():
                ...
                if _stale_elapsed > _bedrock_stale_timeout:
                    ...
                    # crosses SLICK_STREAM_STALE_GIVEUP, so a persistently wedged
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        # wins over the SLICK_API_TIMEOUT env default if the user set it.
        _provider_timeout_cfg = get_provider_request_timeout(agent.provider, agent.model)
        _base_timeout = (
            _provider_timeout_cfg
            if _provider_timeout_cfg is not None
            else env_float("SLICK_API_TIMEOUT", 1800.0)
        )
        # Read timeout: config wins here too.  Otherwise use
        # SLICK_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = env_float("SLICK_STREAM_READ_TIMEOUT", 120.0)
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode SLICK_STREAM_READ_TIMEOUT.
        ...
        def _accept_stream_chunk(_chunk: Any) -> bool:
            ...
            # already-received tool-call chunk back to Slick. Preserve only
        ...
        if agent.provider == "moa":
            # Slick interrupts the managed stream; Relay retains sole
        ...
        # completed response. Relay records that attempt while Slick preserves
        ...
        #      Sophos dedicated endpoint) stall for minutes during large
    ...
    def _call():
        ...
        _max_stream_retries = env_int("SLICK_STREAM_RETRIES", 2)
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = env_float("SLICK_STREAM_STALE_TIMEOUT", 180.0)
    ...
    # unless the user explicitly set SLICK_STREAM_STALE_TIMEOUT; override the
    # local ceiling with SLICK_LOCAL_STREAM_STALE_TIMEOUT (documented in
    # website/docs/reference/environment-variables.md).
    if _stream_stale_timeout_base == 180.0 and agent.base_url and is_local_endpoint(agent.base_url):
        # Read config.yaml ``agent.local_stream_stale_timeout`` (default 900),
        # env var ``SLICK_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch.
        ...
        try:
            ...
            pass
        _stream_stale_timeout = env_float("SLICK_LOCAL_STREAM_STALE_TIMEOUT", _local_default)
        logger.debug(
