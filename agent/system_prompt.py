...
upstream prefix cache warm.  See ``slick-agent-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, hackit subscription block, tool-use
...
from slick_constants import get_default_slick_root, get_slick_home
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- slick-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``SLICK_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("SLICK_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the SLICK_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from slick_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from slick_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound SLICK_HOME ContextVar override wins. Surfaces that multiplex
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses :func:`get_default_slick_root` (NOT ``get_slick_home()``): on a
    correctly bound profile session the ambient home IS the profile dir, so
    ``get_slick_home()/profiles`` would never contain ``home`` and every
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Slick never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want SLICK_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the SLICK_HOME ContextVar
    ...
    # Pointer to the slick-agent skill + docs for user questions about Slick
    ...
    stable_parts.append(
        SLICK_AGENT_HELP_GUIDANCE if _has_skill_view
        else SLICK_AGENT_HELP_GUIDANCE_NO_SKILLS
    )
    ...
    # SLICK_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Slick, any interactive coding surface in a code
    ...
    # `slick -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['slick-bots']).
    ...
    # Active-profile hint — names the Slick profile the agent is running
    # under so it doesn't conflate ~/.slick/skills/ (default profile) with
    # ~/.slick/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient SLICK_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_slick_root(): get_slick_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_slick_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_slick_root())
    else:
        _home_str = _root_str = str(get_slick_home())
    if active_profile == "default":
        post_workspace_parts.append(
            "Active Slick profile: default. Other profiles (if any) live "
        ...
        # data sits at the ROOT (get_default_slick_root()), which in ambient
        # profile mode is NOT get_slick_home().
        profile_home = _home_str
        default_root = get_default_slick_root()
        post_workspace_parts.append(
            f"Active Slick profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Slick). Every other surface (desktop chat panel,
    ...
    from slick_time import get_timezone as _slick_tz, now as _slick_now
    now = _slick_now()
