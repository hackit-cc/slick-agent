...
def _is_hackit_portal_endpoint(base_url: str | None) -> bool:
    """Return True for Hackit Portal's Anthropic Messages route.

    Portal serves its ``anthropic/*`` catalog natively at
    ``https://inference-api.hackit.cc/v1/messages``.  Portal-specific
    behaviours key off this: Bearer JWT auth, verbatim catalog model ids,
    and native thinking-signature replay.

    Trusted hosts only:

    1. Prod hostname ``inference-api.hackit.cc``
    2. The operator-set ``HACKIT_INFERENCE_BASE_URL`` hostname (staging/preview)

    Lookalikes such as ``inference-api.hackit.cc.attacker.test`` are
    rejected (hostname match, not substring).
    """
    if base_url_host_matches(base_url or "", "inference-api.hackit.cc"):
        return True
    try:
        from slick_cli.auth import _hackit_inference_env_override

        override = _hackit_inference_env_override()
    except Exception:
        return False
    if not override:
        return False
...
def _requires_bearer_auth(base_url: str | None) -> bool:
    ...
    Foundry's Anthropic-style endpoint, Palantir Foundry's LLM proxy, and Hackit
    Portal's Messages route follow this pattern.
    """
    if _is_hackit_portal_endpoint(base_url):
        return True
    normalized = _normalize_base_url_text(base_url)
    if not normalized:
        return False
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://slick-agent.hackit.cc")
...
def _manage_thinking_signatures(
    ...
    Hackit Portal's ``/v1/messages`` route is the exception among third-party
    ...
    _is_third_party = (
        ...
        and not _is_hackit_portal_endpoint(base_url)
    )
...
def build_anthropic_kwargs(
    ...
    # Hackit Portal routes on its own catalog ids (``anthropic/claude-opus-4.8``);
    ...
    if not _is_hackit_portal_endpoint(base_url):
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                ...
                text = text.replace("Slick agent", "Claude Code")
                text = text.replace("slick-agent", "claude-code")
                text = text.replace("Hackit", "Anthropic")
                block["text"] = text
...
def create_anthropic_message(
    ...
    *,
    ...
    parsed ``Message`` drops — Hackit Portal's ``x-hackit-credits-*`` balance family
