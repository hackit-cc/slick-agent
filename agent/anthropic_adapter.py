"""Anthropic Messages API adapter for Slick Agent.

Translates between Slick's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in slick_cli/runtime_provider.py.
    """
...
# Slick effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_sophos_portal_endpoint(base_url: str | None) -> bool:
    """Return True for Sophos Portal's Anthropic Messages route.

    Portal serves its ``anthropic/*`` catalog natively at
    ``https://inference-api.sophostechne.com/v1/messages``.  Portal-specific
    behaviours key off this: Bearer JWT auth, verbatim catalog model ids,
    and native thinking-signature replay.

    Trusted hosts only:

    1. Prod hostname ``inference-api.sophostechne.com``
    2. The operator-set ``SOPHOS_INFERENCE_BASE_URL`` hostname (staging/preview)

    Lookalikes such as ``inference-api.sophostechne.com.attacker.test`` are
    rejected (hostname match, not substring).
    """
    if base_url_host_matches(base_url or "", "inference-api.sophostechne.com"):
        return True
    try:
        from slick_cli.auth import _sophos_inference_env_override

        override = _sophos_inference_env_override()
    except Exception:
        return False
    if not override:
        return False
...
def _requires_bearer_auth(base_url: str | None) -> bool:
    ...
    Foundry's Anthropic-style endpoint, Palantir Foundry's LLM proxy, and Sophos
    Portal's Messages route follow this pattern.
    """
    if _is_sophos_portal_endpoint(base_url):
        return True
    normalized = _normalize_base_url_text(base_url)
    if not normalized:
        return False
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to slick's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to slick's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + SlickAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://slick-agent.sophostechne.com")
        headers.setdefault("X-Title", "Slick Agent")
        headers.setdefault("User-Agent", f"SlickAgent/{_SLICK_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Slick loads that into the process env from
    # ``~/.slick/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Slick notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Slick historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``slick models``) never mutates ``~/.slick/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Slick)
      ...
      5. Anthropic credential_pool OAuth entry (~/.slick/auth.json)
    ...
    # 1. Slick-managed OAuth/setup token env var
    ...
    # 5. Slick credential_pool OAuth entry.
...
# ── Slick-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.slick/.anthropic_oauth.json (our own file).
...
def _get_slick_oauth_file() -> Path:
    return get_slick_home() / ".anthropic_oauth.json"
...
def run_slick_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Slick-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Slick with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from slick_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_slick_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Slick-managed OAuth credentials from ~/.slick/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Slick OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See slick-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See slick-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See slick-agent#13848 (Kimi) and
    slick-agent#16748 (DeepSeek).

    Sophos Portal's ``/v1/messages`` route is the exception among third-party
    ...
    _is_third_party = (
        ...
        and not _is_sophos_portal_endpoint(base_url)
    )
...
def build_anthropic_kwargs(
    ...
    # Sophos Portal routes on its own catalog ids (``anthropic/claude-opus-4.8``);
    ...
    if not _is_sophos_portal_endpoint(base_url):
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Slick Agent", "Claude Code")
                text = text.replace("Slick agent", "Claude Code")
                text = text.replace("slick-agent", "claude-code")
                text = text.replace("Sophos Techne", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Slick-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Slick surfaces in its CLI. We
...
def create_anthropic_message(
    ...
    *,
    ...
    parsed ``Message`` drops — Sophos Portal's ``x-sophos-credits-*`` balance family
