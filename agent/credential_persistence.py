...
``auth.json``.  They intentionally have no dependency on ``slick_cli.auth`` so
...
# Sources Slick owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Slick-owned OAuth/device-code state)
