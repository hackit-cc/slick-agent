...
Storage
-------
Bundles live in ``~/.slick/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``SLICK_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``slick bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under SLICK_HOME.

    Honors ``SLICK_BUNDLES_DIR`` for tests; falls back to
    ``<SLICK_HOME>/skill-bundles``.
    """
    override = os.environ.get("SLICK_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_slick_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `slick bundles` CLI subcommand.
# ---------------------------------------------------------------------------
