...
projection into Slick' display, approval bridging, transcript projection into
...
"codex_app_server"`. Slick' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Slick secret — gateway
        ...
        spawn_env = slick_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("SLICK_KANBAN_TASK"):
            kanban_db = spawn_env.get("SLICK_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "SLICK_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("SLICK_HOME", os.path.expanduser("~/.slick")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "slick",
        client_title: str = "Slick Agent",
