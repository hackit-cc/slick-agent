...
# Wire-name used when Slick keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Slick's configured provider
# (Firecrawl / Tavily / …). Mapped back to ``web_search`` in normalize_response.
_XAI_CLIENT_WEB_SEARCH_ALIAS = "slick_web_search"
...
# rename on the wire (slick_<name>), map back in normalize_response so
# Slick dispatch is unaffected.
_OPENCODE_RESERVED_TOOL_NAMES = ("web_search", "search_files")
_RESERVED_TOOL_ALIAS_PREFIX = "slick_"
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # xAI server-side web search vs Slick web providers.
        #
        ...
        #    keep Slick dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``slick_web_search`` so Grok cannot hijack the name. The alias
        ...
        kwargs = {
            # ``-900k`` large-context picker variants are Slick-side aliases
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so Slick dispatches
                ...
                # (slick_web_search / slick_search_files, #85589).
