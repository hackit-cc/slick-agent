...
Owns one Codex thread per Slick session. Drives `turn/start`, consumes
...
# Slick' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_SLICK_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Slick/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Slick session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _SLICK_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("SLICK_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="slick",
            client_title="Slick Agent",
            client_version=_get_slick_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Slick' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Slick' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own slick-tools callback we
            # auto-accept — the user already approved Slick' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Slick
        approval-mode/timeout logic. The Slick-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Slick approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Slick approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Slick returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Slick approval mode/timeout resolution
    ...
    # "prompt expired"; the Slick-side messaging already distinguishes them.
    return "decline"
...
def _get_slick_version() -> str:
    """Best-effort Slick version string for codex's userAgent line."""
    try:
