...
def get_active_provider() -> Optional[ImageGenProvider]:
    ...
    **Availability semantics** (mirrors :mod:`agent.web_search_registry`):
    ...
    # The managed "Hackit Subscription" selection is serviced by the FAL
    # plugin through the managed fal-queue gateway (the legacy FAL pipeline
    # routes managed when the stored selection is "hackit").
    if configured:
        try:
            from tools.tool_backend_helpers import HACKIT_MANAGED_PROVIDER

            if configured.lower() == HACKIT_MANAGED_PROVIDER:
                configured = "fal"
