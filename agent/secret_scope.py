...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the SLICK_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Slick runtime / deployment
    "SLICK_HOME", "SLICK_PROFILE", "SLICK_GATEWAY_LOCK_DIR",
    "SLICK_MAX_ITERATIONS", "SLICK_MAX_TOKENS", "SLICK_API_TIMEOUT",
    "SLICK_REDACT_SECRETS", "SLICK_HACKIT_TIMEOUT_SECONDS",
    "_SLICK_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "SLICK_KANBAN_DB", "SLICK_KANBAN_WORKSPACES_ROOT", "SLICK_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "SLICK_KANBAN_",
    "SLICK_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Slick writes
    ...
    ``slick_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Slick parser: save_env_value
    ...
    from slick_cli.config import _parse_env_value
...
def build_profile_secret_scope(slick_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(slick_home)
    secrets = load_env_file(home / ".env")

    try:
        from slick_cli.env_loader import get_secret_source_values
