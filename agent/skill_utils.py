...
from slick_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.slick/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    SLICK_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``SLICK_KANBAN_TASK`` / ``SLICK_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("SLICK_KANBAN_TASK") or os.getenv("SLICK_KANBAN_BOARD"):
        ...
        # The Slick Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``slick_cli.config`` on the
...
# `slick-agent` skill is the agent's own operating manual — it drives
# configuring, extending, and troubleshooting Slick itself, and the system
# prompt unconditionally points at it. Disabling it leaves the agent unable
# to help with Slick, so disable requests for these names are ignored
# everywhere the disabled list is consulted.
ESSENTIAL_SKILLS: frozenset = frozenset({"slick-agent"})
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``SLICK_PLATFORM`` or
            ``SLICK_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("SLICK_PLATFORM")
        or get_session_env("SLICK_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``slick config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``slick`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.slick/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``slick`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against SLICK_HOME, not cwd
        if not p.is_absolute():
            p = (slick_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.slick/skills/`` first, then external.

    The local dir is always first (and always included even if it doesn't exist
    yet — callers handle that).  External dirs follow in config order.

    NOTE: trusted project-local dirs (``./.slick/skills`` at the git root) are
...
#   <root>/.slick/skills/   — Slick-native location
...
# a one-line "run `slick skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when SLICK_HOME itself lives inside a git checkout:
    ``<root>/.slick/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``slick skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`slick skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under SLICK_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_slick_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.slick/skills/`` (including via symlinks) or configured
    ...
    # ~/.slick/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Slick can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    slick = metadata.get("slick") or {}
    if not isinstance(slick, dict):
        slick = {}
    return {
        "fallback_for_toolsets": slick.get("fallback_for_toolsets", []),
        "requires_toolsets": slick.get("requires_toolsets", []),
        "fallback_for_tools": slick.get("fallback_for_tools", []),
        "requires_tools": slick.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    slick = metadata.get("slick")
    if not isinstance(slick, dict):
        return []
    raw = slick.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Slick metadata, VCS, virtualenv/dependency, cache, and skill
