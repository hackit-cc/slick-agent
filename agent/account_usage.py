...
from slick_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
def build_hackit_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    """Map a HackitPortalAccountInfo into an AccountUsageSnapshot for /usage.
    ...
    """
    try:
        from slick_cli.hackit_account import hackit_portal_topup_url
        ...
        details.append(f"Top up: {hackit_portal_topup_url(account_info)}")
...
def hackit_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    """Return rendered Hackit-credits /usage lines, or [] when there's nothing to show.

    Account-independent of any live agent: gated on "a Hackit account is logged in"
    ...
    Dev override: when SLICK_DEV_CREDITS_FIXTURE selects a fixture state, /usage
    ...
    try:
        ...
        tok = (get_provider_auth_state("hackit") or {}).get("access_token")
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(
                get_hackit_portal_account_info, force_fresh=True
            ).result(timeout=timeout)
        snapshot = build_hackit_credits_snapshot(account)
        return render_account_usage_lines(snapshot, markdown=markdown)
    except Exception:
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    so SLICK_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
    *_usd strings are mock display values here (not server balance to compute on);
    ...
    try:
        ...
        details.append("(dev fixture — SLICK_DEV_CREDITS_FIXTURE)")
        return AccountUsageSnapshot(
            provider="hackit",
            source="dev-fixture",
            fetched_at=_utc_now(),
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    try:
        ...
        from slick_cli.hackit_account import (
            ...
            hackit_portal_topup_url,
        )

        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(get_hackit_portal_account_info, force_fresh=True).result(
                timeout=timeout
            )
    except Exception:
    ...
    # ("Top up: <url>" + "(or run /topup)") that build_hackit_credits_snapshot
    ...
    return CreditsView(
        ...
        topup_url=hackit_portal_topup_url(account),
...
def _resolve_codex_usage_credentials(
    ...
    state, then the credential pool.  Slick's native OAuth setup now stores
    ...
    *,
    ...
    try:
        ...
        return CodexResetRedeemResult(
            status="unavailable",
            message="No Codex credentials available. Run `slick auth` to sign in with your ChatGPT account.",
        )
        ...
        if code in (401, 403):
            return CodexResetRedeemResult(
                ...
                message=(
                    ...
                    "run `slick auth` and sign in with your ChatGPT account."
                ),
            )
        return CodexResetRedeemResult(
            status="unavailable",
    ...
    if code == "reset":
        ...
        # persisted pool cooldowns so Slick doesn't keep the credential
        # frozen behind the now-stale ``last_error_reset_at`` (issue #43747).
        try:
            from slick_cli.auth import clear_codex_pool_quota_cooldowns
