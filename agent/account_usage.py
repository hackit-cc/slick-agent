...
def build_sophos_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    """Map a SophosPortalAccountInfo into an AccountUsageSnapshot for /usage.
    ...
    """
    try:
        from slick_cli.sophos_account import sophos_portal_topup_url
        ...
        details.append(f"Top up: {sophos_portal_topup_url(account_info)}")
...
def sophos_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    """Return rendered Sophos-credits /usage lines, or [] when there's nothing to show.

    Account-independent of any live agent: gated on "a Sophos account is logged in"
    ...
    try:
        ...
        tok = (get_provider_auth_state("sophos") or {}).get("access_token")
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(
                get_sophos_portal_account_info, force_fresh=True
            ).result(timeout=timeout)
        snapshot = build_sophos_credits_snapshot(account)
        return render_account_usage_lines(snapshot, markdown=markdown)
    except Exception:
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    try:
        ...
        from slick_cli.sophos_account import (
            ...
            sophos_portal_topup_url,
        )

        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(get_sophos_portal_account_info, force_fresh=True).result(
                timeout=timeout
            )
    except Exception:
    ...
    # ("Top up: <url>" + "(or run /topup)") that build_sophos_credits_snapshot
    ...
    return CreditsView(
        ...
        topup_url=sophos_portal_topup_url(account),
