...
See the ``slick-agent-dev`` skill (``references/self-improvement-loop.md``)
...
def _parent_can_emit_tool_calls(agent: Any) -> bool:
    ...
    calls. A provider that IS an autonomous agent reaches Slick through a client
    shim, and a shim that cannot carry Slick tool calls back turns the fork into
    ...
    exists so a shim that can't declares ``SUPPORTS_SLICK_TOOL_CALLS = False``
    ...
    for candidate in (client, type(client) if client is not None else None):
        if candidate is None:
            continue
        supported = getattr(candidate, "SUPPORTS_SLICK_TOOL_CALLS", None)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with Slick, e.g. 'slick-agent').\n"
    "  • Hub-installed skills (installed via 'slick skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'slick curator pin'). You are an "
    ...
    "'slick curator adopt <name>' — do not try to patch it.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "  • Hub-installed skills (installed via 'slick skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'slick curator pin'). Pin blocks "
    ...
    "reply and recommend 'slick curator adopt <name>' instead.\n"
    "If the only skills that need updating are protected, say\n"
    "'Nothing to save.' and stop.\n\n"
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("SLICK_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
...
def _run_review_in_thread(
    ...
    # An agent-as-provider whose client can't carry Slick tool calls back would
    ...
    if not _parent_can_emit_tool_calls(agent) and not bool(
        ...
        logger.warning(
            "Background review skipped: provider %r cannot emit Slick tool calls, "
    ...
    try:
        ...
        with thread_scoped_silence():
            ...
            # scratch (fresh _slick_now() timestamp, fresh
