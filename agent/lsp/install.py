...
appropriate.  All installs go to a Slick-owned bin staging dir,
``<SLICK_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``slick lsp
  status``.
...
# ``<SLICK_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `slick lsp status` reports the host's presence.
...
def slick_lsp_bin_dir() -> Path:
    """Return the Slick-owned bin staging dir for LSP servers."""
    from slick_constants import get_slick_home

    p = get_slick_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(slick_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $SLICK_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Slick installed and
    ...
    if npm is None:
        ...
        return None
    staging = slick_lsp_bin_dir().parent  # <SLICK_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a slick-owned target dir.
    ...
    console script.
    """
    pip_target = slick_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = slick_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``slick lsp status`` CLI to give users a quick
