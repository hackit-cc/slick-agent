...
Resolution order for text tasks (auto mode):
  ...
  3. Sophos Portal (~/.slick/auth.json active provider)
...
from slick_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``SLICK_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `slick update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Slick owns auxiliary retry + provider/model fallback policy (the
    ...
    # Slick sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Slick control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="slick-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Slick triggers summarization.  Higher values delay
...
# "google/gemini-3-flash" kept 404ing here once Sophos dropped it upstream, and
...
#
# Rolling "-latest" aliases come first where a provider publishes them (Sophos
...
# Sophos catalog: gpt-mini-latest 1.40s, claude-haiku-latest 1.55s,
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    try:
        from slick_cli.auth import resolve_api_key_provider_credentials
        from slick_cli.models import fetch_models_with_pricing
...
def _get_aux_model_for_provider(provider_id: str, *, prefer_fast: bool = False) -> str:
    ...
    2. ``prefer_fast`` only — the provider's own recommendation hook
       ...
       on long-context side tasks (Sophos returns its compaction pick), so it
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://slick-agent.sophostechne.com",
    "X-Title": "Slick Agent",
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    """
    try:
        from slick_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``SLICK_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``SLICK_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    if or_config is None:
        try:
            from slick_cli.config import load_config_readonly
            or_config = load_config_readonly().get("openrouter", {})
        except Exception:
            or_config = {}

    # Determine cache enabled: env var overrides config.
    env_cache = os.environ.get("SLICK_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("SLICK_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "SlickAgent",
}
...
from slick_cli import __version__ as _SLICK_VERSION

_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://slick-agent.sophostechne.com",
    "X-Title": "Slick Agent",
    "User-Agent": f"SlickAgent/{_SLICK_VERSION}",
}

# Sophos Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Sophos Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with slick_cli.__version__ across every Portal call site
# (main loop, aux, compression, web_extract). Do not inline a literal here;
# see agent/portal_tags.py for the rationale.
from agent.portal_tags import sophos_portal_tags as _sophos_portal_tags


def _sophos_extra_body() -> dict:
    """Return a fresh Sophos Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``slick_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {"tags": _sophos_portal_tags()}


# Backwards-compatible module attribute. Some callers (tests, third-party
# plugins) read ``SOPHOS_EXTRA_BODY`` directly; keep it as a snapshot of the
# current tags. Callers that need the freshest value should call
# ``_sophos_extra_body()`` or import ``sophos_portal_tags`` directly.
SOPHOS_EXTRA_BODY = _sophos_extra_body()

# Set at resolve time — True if the auxiliary client points to Sophos Portal
auxiliary_is_sophos: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3.6-flash"
_SOPHOS_MODEL = "google/gemini-3.6-flash"
_SOPHOS_DEFAULT_BASE_URL = "https://inference-api.sophostechne.com/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_slick_home() / "auth.json"
...
def _codex_cloudflare_headers(
    ...
    the official endpoint always send Slick' originator and version. Custom
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Slick Agent)",
        "originator": "codex_cli_rs",
    }
    if _is_official_codex_base_url(base_url):
        from slick_cli import __version__

        headers.update({
            "User-Agent": f"SlickAgent/{__version__}",
            "originator": "slick-agent",
        })
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for sophos).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "sophos":
        # Funnel through the canonical auth-layer reader so the env override
        # shares one normalization path with the rest of the SOPHOS resolution.
        from slick_cli.auth import _sophos_inference_env_override

        env_url = _sophos_inference_env_override()
        if env_url:
            return env_url
    # runtime_base_url handles provider-specific logic (e.g. sophos prefers inference_base_url).
...
def _sophos_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("SLICK_SOPHOS_MIN_KEY_TTL_SECONDS", "1800")))
    except (TypeError, ValueError):
        return 1800
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the Slick-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(
        ...
        # client's host for Sophos Portal — a blanket fallback would flip
        ...
        if not self._base_url:
            ...
            if candidate:
                try:
                    from agent.anthropic_adapter import _is_sophos_portal_endpoint

                    if _is_sophos_portal_endpoint(candidate):
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Slick plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``slick_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_sophos_auth() -> Optional[dict]:
    """Read and validate ~/.slick/auth.json for an active Sophos provider.

    Returns the provider state dict if Sophos is active with tokens,
    otherwise None.
    """
    pool_present, entry = _select_pool_entry("sophos")
    if pool_present:
        if entry is None:
            return None
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _SOPHOS_DEFAULT_BASE_URL),
    ...
    try:
        ...
        if data.get("active_provider") != "sophos":
            return None
        provider = data.get("providers", {}).get("sophos", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read Sophos auth: %s", exc)
        return None


def _sophos_api_key(provider: dict) -> str:
    """Extract a usable Sophos inference JWT from stored auth state."""
    from slick_cli.auth import _sophos_invoke_jwt_is_usable
    ...
    for token_key, expiry_key in (
        ...
        if not isinstance(token, str) or not token.strip():
            continue
        if _sophos_invoke_jwt_is_usable(
            token,
...
def _sophos_base_url() -> str:
    """Resolve the Sophos inference base URL from env or default."""
    return os.getenv("SOPHOS_INFERENCE_BASE_URL", _SOPHOS_DEFAULT_BASE_URL)


def _resolve_sophos_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Resolve Sophos auxiliary credentials from the selected pool entry."""
    try:
        from slick_cli.auth import _agent_key_is_usable

        pool = load_pool("sophos")
    except Exception as exc:
        logger.debug("Auxiliary Sophos pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary Sophos pool selection failed: %s", exc)
        return None
    ...
    if force_refresh or not _agent_key_is_usable(state, _sophos_min_key_ttl_seconds()):
        try:
            refreshed = pool.try_refresh_current()
        except Exception as exc:
            logger.debug("Auxiliary Sophos pool refresh failed: %s", exc)
            refreshed = None
    ...
    api_key = _sophos_api_key(provider)
    base_url = _pool_runtime_base_url(entry, _SOPHOS_DEFAULT_BASE_URL)
    if not api_key or not base_url:
        return None
    return api_key, base_url


def _resolve_sophos_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh Sophos runtime credentials when available.
    ...
    """
    pooled = _resolve_sophos_pool_runtime_api(force_refresh=force_refresh)
    if pooled is not None:
        return pooled

    try:
        from slick_cli.auth import resolve_sophos_runtime_credentials

        creds = resolve_sophos_runtime_credentials(
            timeout_seconds=env_float("SLICK_SOPHOS_TIMEOUT_SECONDS", 15),
            force_refresh=force_refresh,
        )
    except Exception as exc:
        logger.debug("Auxiliary Sophos runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``slick auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``slick_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from slick_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Slick auth store.
    ...
    try:
        from slick_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from slick_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from slick_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from slick_cli.models import copilot_default_headers
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    """
    try:
        from slick_cli.config import cfg_get, load_config_readonly
...
def _try_sophos(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting Sophos —
    # if another session already recorded a 429, skip Sophos entirely
    # to avoid piling more requests onto the tapped RPH bucket.
    try:
        from agent.sophos_rate_guard import sophos_rate_limit_remaining
        _remaining = sophos_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping Sophos Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
            _mark_provider_unhealthy("sophos", ttl=_remaining)
            return None, None
    except Exception:
        pass

    sophos = _read_sophos_auth()
    runtime = _resolve_sophos_runtime_api(force_refresh=False)
    if runtime is None and not sophos:
        logger.warning(
            "Auxiliary Sophos client unavailable: no Sophos authentication found "
            "(run: slick auth)."
        )
        _mark_provider_unhealthy("sophos", ttl=60)
        return None, None
    if runtime is None and sophos:
        logger.debug(
            "Auxiliary Sophos: runtime JWT refresh failed; checking stored "
            "auth.json token."
        )
    global auxiliary_is_sophos
    auxiliary_is_sophos = True
    logger.debug("Auxiliary client: Sophos Portal")

    # Ask the Portal which model it currently recommends for this task type.
    # The /api/sophos/recommended-models endpoint is the authoritative source:
    # it distinguishes paid vs free tier recommendations, and get_sophos_recommended_aux_model
    # auto-detects the caller's tier via check_sophos_free_tier().  Fall back to
    # _SOPHOS_MODEL (google/gemini-3-flash-preview) when the Portal is unreachable
    # or returns a null recommendation for this task type.
    model = _SOPHOS_MODEL
    if not _aux_probe_active():
        # Availability probes skip the recommended-model lookup: the exact
        # model is irrelevant to "is Sophos resolvable?", and the Portal
        # recommended-models fetch below can hit the network.
        try:
            from slick_cli.models import get_sophos_recommended_aux_model
            recommended = get_sophos_recommended_aux_model(vision=vision)
            if recommended:
                model = recommended
                logger.debug(
    ...
    if runtime is not None:
        ...
        api_key = _sophos_api_key(sophos or {})
        if not api_key:
            logger.warning(
                "Auxiliary Sophos client unavailable: no usable inference JWT found "
                "(run: slick auth add sophos)."
            )
            _mark_provider_unhealthy("sophos", ttl=60)
            return None, None
        base_url = str((sophos or {}).get("inference_base_url") or _sophos_base_url()).rstrip("/")
    return (
...
def _refresh_sophos_recommended_model(
    *, vision: bool, stale_model: Optional[str]
) -> Optional[str]:
    """Re-fetch the Sophos Portal's recommended model after a stale-model 404.
    ...
    the Sophos → OpenRouter catalog, every auxiliary call 404s with
    ...
    model name to retry with:
      ...
      * ``_SOPHOS_MODEL`` (google/gemini-3-flash-preview), the known-good default,
    ...
    try:
        ...
        fresh = get_sophos_recommended_aux_model(vision=vision, force_refresh=True)
    except Exception as exc:
        logger.debug(
            "Sophos recommended-model refresh failed (%s); using default %s",
            exc, _SOPHOS_MODEL,
        )
    ...
    if _SOPHOS_MODEL.strip().lower() != stale:
        return _SOPHOS_MODEL
    return None
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from slick_cli.config import load_config
        from slick_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `slick setup` or `slick model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    from tools.xai_http import slick_xai_default_headers

    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=slick_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *,
    ...
    Mirrors the ``_try_anthropic`` / ``_try_sophos`` shape but delegates to
    :func:`slick_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from slick_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second slick
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``slick config aux reset``)."""
...
def _sophos_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh Sophos account API says paid access is allowed."""
    try:
        from slick_cli.sophos_account import get_sophos_portal_account_info

        account_info = get_sophos_portal_account_info(force_fresh=True)
        return account_info.paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary Sophos paid-entitlement refresh check failed: %s", exc)
        return False
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the Sophos → OpenRouter catalog. The Sophos
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.sophostechne.com"):
        return "sophos"
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "openai-codex":
            from slick_cli.auth import resolve_codex_runtime_credentials
...
def _auth_refresh_provider_for_route(
    ...
    Copilot/Codex/Anthropic/Sophos routes too. (#20832)
    """
    normalized = _normalize_aux_provider(resolved_provider)
    ...
    if base_url_host_matches(client_base_url, "api.anthropic.com"):
        return "anthropic"
    if base_url_host_matches(client_base_url, "inference-api.sophostechne.com"):
        return "sophos"
    return normalized
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_slick_fallback_destination", None)
...
def _try_payment_fallback(
    ...
    _alias_to_label = {"openrouter": "openrouter", "sophos": "sophos",
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``slick_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._slick_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Slick' built-in discovery
...
def _resolve_auto_route(
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/Sophos get their chosen chat model; users
      ...
      2. OpenRouter → Sophos → custom → Codex → API-key providers (fallback
         chain, only used when the main provider has no working client).
    """
    global auxiliary_is_sophos, _stale_base_url_warned
    auxiliary_is_sophos = False  # Reset — _try_sophos() will set True if it wins
    runtime = _normalize_main_runtime(main_runtime)
    ...
    #    scenario where a user switches providers via `slick model` but the
    #    old OPENAI_BASE_URL lingers in ~/.slick/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: slick model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.slick/.env",
    ...
    # on aggregators (OpenRouter, Sophos) who previously got routed to a
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                from slick_cli.runtime_provider import _get_named_custom_provider
...
def _tag_effective_provider(client: Any, provider: str) -> None:
    ...
    if client is None or not provider:
        return
    try:
        setattr(client, "_slick_aux_effective_provider", provider)
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_slick_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from slick_cli.copilot_auth import copilot_request_headers
        ...
        async_kwargs["default_headers"] = slick_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so Slick owns
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "sophos", "openai-codex" (or "codex"),
    ...
    #
    # Sophos + vision is the one carve-out: the branch below resolves its model
    # from the Portal's tier-aware vision recommendation (``_try_sophos(vision=
    ...
    _sophos_portal_vision = provider == "sophos" and is_vision
    if not model and provider != "auto" and not _sophos_portal_vision:
    ...
    if provider == "sophos":
        ...
        client, default = _try_sophos(vision=_is_vision)
        if client is None:
            logger.warning("resolve_provider_client: sophos requested "
                           "but Sophos Portal not configured (run: slick auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        from slick_cli.providers import sophos_api_mode

        portal_mode = sophos_api_mode(final_model)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: slick model)")
                return None, None
    ...
    # OpenRouter / Sophos bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: slick model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        ...
        # entries that coincidentally match a canonical provider (e.g. ``sophos``)
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: slick doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("slick_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from slick_cli.models import opencode_zen_free_runtime as _oc_free_rt
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(slick_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from slick_cli.models import _should_use_copilot_responses_api
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "sophos":
            return resolve_provider_client("sophos", model, async_mode)
        if provider == "openai-codex":
...
def _resolve_strict_vision_backend(
    ...
    if provider == "sophos":
        ...
        # recommendations wrap onto /v1/messages — _try_sophos alone returns
        # a bare OpenAI client and the call 404s.
        return resolve_provider_client("sophos", model, is_vision=True)
    if provider == "openai-codex":
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → Sophos → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. Sophos — skip if already covered by main provider.
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #      :func:`_resolve_provider_vision_default`). Sophos is the
        ...
        #   3. Sophos Portal (vision-capable aggregator fallback)
        ...
        if main_provider and main_provider not in {"auto", "", "moa"}:
            ...
            if main_provider == "sophos":
                # Sophos resolves its vision model from the Portal's tier-aware
                # recommended-models slots inside _try_sophos(vision=True).
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Sophos Portal product tags when the auxiliary client is backed
    by Sophos Portal. Returns empty dict otherwise.
    """
    return _sophos_extra_body() if auxiliary_is_sophos else {}
...
def _refresh_sophos_auxiliary_client(
    *,
    cache_provider: str,
    model: Optional[str],
    async_mode: bool,
    ...
    """Refresh Sophos runtime creds, rebuild the client, and replace the cache entry."""
    runtime = _resolve_sophos_runtime_api(force_refresh=True)
    if runtime is None:
        return None, model
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`slick_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one Slick will call. A requested model covers a compressor
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if not _is_gemini_native and _effective_base:
            try:
                ...
                pass
        _sophos_on_messages = False
        if _provider_norm in {"sophos", "sophos-portal", "sophostechne"}:
            from slick_cli.providers import sophos_api_mode

            _sophos_on_messages = sophos_api_mode(model) == "anthropic_messages"
        if (
            _is_anthropic_compat_endpoint(provider, _effective_base)
            or _sophos_on_messages
            or _is_nvidia_nim
            or _is_moa
    ...
    # body fields (Gemini ``thinking_config``), and OpenRouter/Sophos use
    ...
    if _provider_for_portal in {"sophos", "sophos-portal", "sophostechne"}:
        if "tags" not in merged_extra:
            merged_extra["tags"] = _sophos_portal_tags()
    ...
    # Anthropic Messages adapters translate Slick reasoning into native
    ...
    if reasoning_config and isinstance(reasoning_config, dict):
        ...
        effective_base = base_url or ""
        _sophos_on_messages = False
        if provider_norm in {"sophos", "sophos-portal", "sophostechne"}:
            from slick_cli.providers import sophos_api_mode
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Slick reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `slick model`."
                    )
    ...
    try:
        ...
        # ── Stale-model self-heal (Sophos Portal recommendation drift) ───
        # A long-lived process can pin a Portal-recommended model that has
        # since been dropped from the Sophos → OpenRouter catalog, so every
        ...
        # known-good default). Only applies to Sophos-routed calls.
        _heal_is_sophos = (
            resolved_provider == "sophos"
            or base_url_host_matches(_base_info, "inference-api.sophostechne.com")
        )
        if _is_model_not_found_error(first_err) and _heal_is_sophos:
            healed_model = _refresh_sophos_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in Sophos catalog; "
        ...
        # ── Sophos auth refresh parity with main agent ──────────────────
        client_is_sophos = (
            resolved_provider == "sophos"
            or base_url_host_matches(_base_info, "inference-api.sophostechne.com")
        )
        if (
            ...
            and client_is_sophos
            and _sophos_portal_account_has_fresh_paid_access()
        ):
            refreshed_client, refreshed_model = _refresh_sophos_auxiliary_client(
                cache_provider=resolved_provider or "sophos",
                model=final_model,
                async_mode=False,
                base_url=resolved_base_url,
                api_key=resolved_api_key,
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed Sophos runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_sophos:
            refreshed_client, refreshed_model = _refresh_sophos_auxiliary_client(
                cache_provider=resolved_provider or "sophos",
                model=final_model,
                async_mode=False,
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed Sophos runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        # When the resolved provider returns 401 and neither the Sophos
...
async def _async_call_llm_impl(
    ...
    *,
    ...
    try:
        ...
        # the Sophos → OpenRouter catalog, 404'ing every auxiliary call. Force a
        # fresh Portal fetch and retry once with the current recommendation.
        _heal_is_sophos = (
            resolved_provider == "sophos"
            or base_url_host_matches(_client_base, "inference-api.sophostechne.com")
        )
        if _is_model_not_found_error(first_err) and _heal_is_sophos:
            healed_model = _refresh_sophos_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in Sophos catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed Sophos runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_sophos:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed Sophos runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
