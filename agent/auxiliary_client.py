...
Resolution order for text tasks (auto mode):
  ...
  3. Hackit Portal (~/.slick/auth.json active provider)
...
# "google/gemini-3-flash" kept 404ing here once Hackit dropped it upstream, and
...
#
# Rolling "-latest" aliases come first where a provider publishes them (Hackit
...
# Hackit catalog: gpt-mini-latest 1.40s, claude-haiku-latest 1.55s,
...
def _get_aux_model_for_provider(provider_id: str, *, prefer_fast: bool = False) -> str:
    ...
    2. ``prefer_fast`` only — the provider's own recommendation hook
       ...
       on long-context side tasks (Hackit returns its compaction pick), so it
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://slick-agent.hackit.cc",
    "X-Title": "Slick Agent",
...
# Hackit Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Hackit Portal.
#
...
from agent.portal_tags import hackit_portal_tags as _hackit_portal_tags


def _hackit_extra_body() -> dict:
    """Return a fresh Hackit Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``slick_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {"tags": _hackit_portal_tags()}


# Backwards-compatible module attribute. Some callers (tests, third-party
# plugins) read ``HACKIT_EXTRA_BODY`` directly; keep it as a snapshot of the
# current tags. Callers that need the freshest value should call
# ``_hackit_extra_body()`` or import ``hackit_portal_tags`` directly.
HACKIT_EXTRA_BODY = _hackit_extra_body()

# Set at resolve time — True if the auxiliary client points to Hackit Portal
auxiliary_is_hackit: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3.6-flash"
_HACKIT_MODEL = "google/gemini-3.6-flash"
_HACKIT_DEFAULT_BASE_URL = "https://inference-api.hackit.cc/v1"
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for hackit).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "hackit":
        # Funnel through the canonical auth-layer reader so the env override
        # shares one normalization path with the rest of the HACKIT resolution.
        from slick_cli.auth import _hackit_inference_env_override

        env_url = _hackit_inference_env_override()
        if env_url:
            return env_url
    # runtime_base_url handles provider-specific logic (e.g. hackit prefers inference_base_url).
...
def _hackit_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("SLICK_HACKIT_MIN_KEY_TTL_SECONDS", "1800")))
    except (TypeError, ValueError):
        return 1800
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(
        ...
        # client's host for Hackit Portal — a blanket fallback would flip
        ...
        if not self._base_url:
            ...
            if candidate:
                try:
                    from agent.anthropic_adapter import _is_hackit_portal_endpoint

                    if _is_hackit_portal_endpoint(candidate):
...
def _read_hackit_auth() -> Optional[dict]:
    """Read and validate ~/.slick/auth.json for an active Hackit provider.

    Returns the provider state dict if Hackit is active with tokens,
    otherwise None.
    """
    pool_present, entry = _select_pool_entry("hackit")
    if pool_present:
        if entry is None:
            return None
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _HACKIT_DEFAULT_BASE_URL),
    ...
    try:
        ...
        if data.get("active_provider") != "hackit":
            return None
        provider = data.get("providers", {}).get("hackit", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read Hackit auth: %s", exc)
        return None


def _hackit_api_key(provider: dict) -> str:
    """Extract a usable Hackit inference JWT from stored auth state."""
    from slick_cli.auth import _hackit_invoke_jwt_is_usable
    ...
    for token_key, expiry_key in (
        ...
        if not isinstance(token, str) or not token.strip():
            continue
        if _hackit_invoke_jwt_is_usable(
            token,
...
def _hackit_base_url() -> str:
    """Resolve the Hackit inference base URL from env or default."""
    return os.getenv("HACKIT_INFERENCE_BASE_URL", _HACKIT_DEFAULT_BASE_URL)


def _resolve_hackit_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Resolve Hackit auxiliary credentials from the selected pool entry."""
    try:
        from slick_cli.auth import _agent_key_is_usable

        pool = load_pool("hackit")
    except Exception as exc:
        logger.debug("Auxiliary Hackit pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary Hackit pool selection failed: %s", exc)
        return None
    ...
    if force_refresh or not _agent_key_is_usable(state, _hackit_min_key_ttl_seconds()):
        try:
            refreshed = pool.try_refresh_current()
        except Exception as exc:
            logger.debug("Auxiliary Hackit pool refresh failed: %s", exc)
            refreshed = None
    ...
    api_key = _hackit_api_key(provider)
    base_url = _pool_runtime_base_url(entry, _HACKIT_DEFAULT_BASE_URL)
    if not api_key or not base_url:
        return None
    return api_key, base_url


def _resolve_hackit_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh Hackit runtime credentials when available.
    ...
    """
    pooled = _resolve_hackit_pool_runtime_api(force_refresh=force_refresh)
    if pooled is not None:
        return pooled

    try:
        from slick_cli.auth import resolve_hackit_runtime_credentials

        creds = resolve_hackit_runtime_credentials(
            timeout_seconds=env_float("SLICK_HACKIT_TIMEOUT_SECONDS", 15),
            force_refresh=force_refresh,
        )
    except Exception as exc:
        logger.debug("Auxiliary Hackit runtime credential resolution failed: %s", exc)
        return None
...
def _try_hackit(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting Hackit —
    # if another session already recorded a 429, skip Hackit entirely
    # to avoid piling more requests onto the tapped RPH bucket.
    try:
        from agent.hackit_rate_guard import hackit_rate_limit_remaining
        _remaining = hackit_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping Hackit Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
            _mark_provider_unhealthy("hackit", ttl=_remaining)
            return None, None
    except Exception:
        pass

    hackit = _read_hackit_auth()
    runtime = _resolve_hackit_runtime_api(force_refresh=False)
    if runtime is None and not hackit:
        logger.warning(
            "Auxiliary Hackit client unavailable: no Hackit authentication found "
            "(run: slick auth)."
        )
        _mark_provider_unhealthy("hackit", ttl=60)
        return None, None
    if runtime is None and hackit:
        logger.debug(
            "Auxiliary Hackit: runtime JWT refresh failed; checking stored "
            "auth.json token."
        )
    global auxiliary_is_hackit
    auxiliary_is_hackit = True
    logger.debug("Auxiliary client: Hackit Portal")

    # Ask the Portal which model it currently recommends for this task type.
    # The /api/hackit/recommended-models endpoint is the authoritative source:
    # it distinguishes paid vs free tier recommendations, and get_hackit_recommended_aux_model
    # auto-detects the caller's tier via check_hackit_free_tier().  Fall back to
    # _HACKIT_MODEL (google/gemini-3-flash-preview) when the Portal is unreachable
    # or returns a null recommendation for this task type.
    model = _HACKIT_MODEL
    if not _aux_probe_active():
        # Availability probes skip the recommended-model lookup: the exact
        # model is irrelevant to "is Hackit resolvable?", and the Portal
        # recommended-models fetch below can hit the network.
        try:
            from slick_cli.models import get_hackit_recommended_aux_model
            recommended = get_hackit_recommended_aux_model(vision=vision)
            if recommended:
                model = recommended
                logger.debug(
    ...
    if runtime is not None:
        ...
        api_key = _hackit_api_key(hackit or {})
        if not api_key:
            logger.warning(
                "Auxiliary Hackit client unavailable: no usable inference JWT found "
                "(run: slick auth add hackit)."
            )
            _mark_provider_unhealthy("hackit", ttl=60)
            return None, None
        base_url = str((hackit or {}).get("inference_base_url") or _hackit_base_url()).rstrip("/")
    return (
...
def _refresh_hackit_recommended_model(
    *, vision: bool, stale_model: Optional[str]
) -> Optional[str]:
    """Re-fetch the Hackit Portal's recommended model after a stale-model 404.
    ...
    the Hackit → OpenRouter catalog, every auxiliary call 404s with
    ...
    model name to retry with:
      ...
      * ``_HACKIT_MODEL`` (google/gemini-3-flash-preview), the known-good default,
    ...
    try:
        ...
        fresh = get_hackit_recommended_aux_model(vision=vision, force_refresh=True)
    except Exception as exc:
        logger.debug(
            "Hackit recommended-model refresh failed (%s); using default %s",
            exc, _HACKIT_MODEL,
        )
    ...
    if _HACKIT_MODEL.strip().lower() != stale:
        return _HACKIT_MODEL
    return None
...
def _try_azure_foundry(
    *,
    ...
    Mirrors the ``_try_anthropic`` / ``_try_hackit`` shape but delegates to
...
def _hackit_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh Hackit account API says paid access is allowed."""
    try:
        from slick_cli.hackit_account import get_hackit_portal_account_info

        account_info = get_hackit_portal_account_info(force_fresh=True)
        return account_info.paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary Hackit paid-entitlement refresh check failed: %s", exc)
        return False
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the Hackit → OpenRouter catalog. The Hackit
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.hackit.cc"):
        return "hackit"
...
def _auth_refresh_provider_for_route(
    ...
    Copilot/Codex/Anthropic/Hackit routes too. (#20832)
    """
    normalized = _normalize_aux_provider(resolved_provider)
    ...
    if base_url_host_matches(client_base_url, "api.anthropic.com"):
        return "anthropic"
    if base_url_host_matches(client_base_url, "inference-api.hackit.cc"):
        return "hackit"
    return normalized
...
def _try_payment_fallback(
    ...
    _alias_to_label = {"openrouter": "openrouter", "hackit": "hackit",
...
def _resolve_auto_route(
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/Hackit get their chosen chat model; users
      ...
      2. OpenRouter → Hackit → custom → Codex → API-key providers (fallback
         chain, only used when the main provider has no working client).
    """
    global auxiliary_is_hackit, _stale_base_url_warned
    auxiliary_is_hackit = False  # Reset — _try_hackit() will set True if it wins
    runtime = _normalize_main_runtime(main_runtime)
    ...
    # on aggregators (OpenRouter, Hackit) who previously got routed to a
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "hackit", "openai-codex" (or "codex"),
    ...
    #
    # Hackit + vision is the one carve-out: the branch below resolves its model
    # from the Portal's tier-aware vision recommendation (``_try_hackit(vision=
    ...
    _hackit_portal_vision = provider == "hackit" and is_vision
    if not model and provider != "auto" and not _hackit_portal_vision:
    ...
    if provider == "hackit":
        ...
        client, default = _try_hackit(vision=_is_vision)
        if client is None:
            logger.warning("resolve_provider_client: hackit requested "
                           "but Hackit Portal not configured (run: slick auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        from slick_cli.providers import hackit_api_mode

        portal_mode = hackit_api_mode(final_model)
    ...
    # OpenRouter / Hackit bills for side tasks they thought were running on
    ...
    try:
        ...
        # entries that coincidentally match a canonical provider (e.g. ``hackit``)
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "hackit":
            return resolve_provider_client("hackit", model, async_mode)
        if provider == "openai-codex":
...
def _resolve_strict_vision_backend(
    ...
    if provider == "hackit":
        ...
        # recommendations wrap onto /v1/messages — _try_hackit alone returns
        # a bare OpenAI client and the call 404s.
        return resolve_provider_client("hackit", model, is_vision=True)
    if provider == "openai-codex":
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → Hackit → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. Hackit — skip if already covered by main provider.
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #      :func:`_resolve_provider_vision_default`). Hackit is the
        ...
        #   3. Hackit Portal (vision-capable aggregator fallback)
        ...
        if main_provider and main_provider not in {"auto", "", "moa"}:
            ...
            if main_provider == "hackit":
                # Hackit resolves its vision model from the Portal's tier-aware
                # recommended-models slots inside _try_hackit(vision=True).
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Hackit Portal product tags when the auxiliary client is backed
    by Hackit Portal. Returns empty dict otherwise.
    """
    return _hackit_extra_body() if auxiliary_is_hackit else {}
...
def _refresh_hackit_auxiliary_client(
    *,
    cache_provider: str,
    model: Optional[str],
    async_mode: bool,
    ...
    """Refresh Hackit runtime creds, rebuild the client, and replace the cache entry."""
    runtime = _resolve_hackit_runtime_api(force_refresh=True)
    if runtime is None:
        return None, model
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if not _is_gemini_native and _effective_base:
            try:
                ...
                pass
        _hackit_on_messages = False
        if _provider_norm in {"hackit", "hackit-portal", "hackit"}:
            from slick_cli.providers import hackit_api_mode

            _hackit_on_messages = hackit_api_mode(model) == "anthropic_messages"
        if (
            _is_anthropic_compat_endpoint(provider, _effective_base)
            or _hackit_on_messages
            or _is_nvidia_nim
            or _is_moa
    ...
    # body fields (Gemini ``thinking_config``), and OpenRouter/Hackit use
    ...
    if _provider_for_portal in {"hackit", "hackit-portal", "hackit"}:
        if "tags" not in merged_extra:
            merged_extra["tags"] = _hackit_portal_tags()
    ...
    if reasoning_config and isinstance(reasoning_config, dict):
        ...
        effective_base = base_url or ""
        _hackit_on_messages = False
        if provider_norm in {"hackit", "hackit-portal", "hackit"}:
            from slick_cli.providers import hackit_api_mode
...
def _call_llm_impl(
    ...
    *,
    ...
    try:
        ...
        # ── Stale-model self-heal (Hackit Portal recommendation drift) ───
        # A long-lived process can pin a Portal-recommended model that has
        # since been dropped from the Hackit → OpenRouter catalog, so every
        ...
        # known-good default). Only applies to Hackit-routed calls.
        _heal_is_hackit = (
            resolved_provider == "hackit"
            or base_url_host_matches(_base_info, "inference-api.hackit.cc")
        )
        if _is_model_not_found_error(first_err) and _heal_is_hackit:
            healed_model = _refresh_hackit_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in Hackit catalog; "
        ...
        # ── Hackit auth refresh parity with main agent ──────────────────
        client_is_hackit = (
            resolved_provider == "hackit"
            or base_url_host_matches(_base_info, "inference-api.hackit.cc")
        )
        if (
            ...
            and client_is_hackit
            and _hackit_portal_account_has_fresh_paid_access()
        ):
            refreshed_client, refreshed_model = _refresh_hackit_auxiliary_client(
                cache_provider=resolved_provider or "hackit",
                model=final_model,
                async_mode=False,
                base_url=resolved_base_url,
                api_key=resolved_api_key,
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed Hackit runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_hackit:
            refreshed_client, refreshed_model = _refresh_hackit_auxiliary_client(
                cache_provider=resolved_provider or "hackit",
                model=final_model,
                async_mode=False,
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed Hackit runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        # When the resolved provider returns 401 and neither the Hackit
...
async def _async_call_llm_impl(
    ...
    *,
    ...
    try:
        ...
        # the Hackit → OpenRouter catalog, 404'ing every auxiliary call. Force a
        # fresh Portal fetch and retry once with the current recommendation.
        _heal_is_hackit = (
            resolved_provider == "hackit"
            or base_url_host_matches(_client_base, "inference-api.hackit.cc")
        )
        if _is_model_not_found_error(first_err) and _heal_is_hackit:
            healed_model = _refresh_hackit_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in Hackit catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed Hackit runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_hackit:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed Hackit runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
