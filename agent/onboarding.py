...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Slick starts and finds ``~/.openclaw/``.

    Points users at ``slick claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``slick claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Slick, run "
        "`slick claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `slick claw cleanup` (renames it to ~/.openclaw.pre-migration — "
