...
(``gateway/platforms/webhook.py``): inbound wakes Slick when the world
changes; outbound tells the world when Slick does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Slick-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``SLICK_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.slick/config.yaml``)::
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/slick-events
          ...
          secret_env: SLICK_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `slick hooks list`
...
Headers::
    ...
    User-Agent:              Slick-Agent-Outbound-Webhook
    X-Slick-Event:          <hook event name>
    X-Slick-Delivery:       <delivery_id>
    X-Slick-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("SLICK_SAFE_MODE"):
        logger.info("SLICK_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Slick-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Slick-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
