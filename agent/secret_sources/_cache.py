...
cache problem must never block Slick startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Slick home used for cache paths.

    ``home_path`` is whatever ``load_slick_dotenv()`` already resolved;
    falling back to ``$SLICK_HOME`` / ``~/.slick`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from slick_constants import get_slick_home

        home_path = get_slick_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<slick_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if SLICK_HOME is ever made traversable.
            try:
