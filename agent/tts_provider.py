...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Slick with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.slick/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Slick
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``slick tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``slick setup`` for
