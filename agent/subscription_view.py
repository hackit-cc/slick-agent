...
def build_subscription_state(*, timeout: float = 15.0) -> SubscriptionState:
    ...
    Dev override: when ``SLICK_DEV_SUBSCRIPTION_FIXTURE`` names a fixture state,
...
_DEV_FIXTURE_PORTAL = "https://portal.sophostechne.com/billing"
...
def dev_fixture_subscription_state() -> Optional[SubscriptionState]:
    """Return a fixture :class:`SubscriptionState` for ``SLICK_DEV_SUBSCRIPTION_FIXTURE``.
    ...
    Throwaway scaffolding — mirrors ``SLICK_DEV_CREDITS_FIXTURE``.
    """
    name = (os.getenv("SLICK_DEV_SUBSCRIPTION_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    return SubscriptionState(logged_in=False, error=f"unknown SLICK_DEV_SUBSCRIPTION_FIXTURE: {name}")
