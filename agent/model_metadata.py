...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    3. Env vars ``SLICK_CA_BUNDLE`` / ``REQUESTS_CA_BUNDLE`` / ``SSL_CERT_FILE``
    ...
    for env_var in ("SLICK_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_slick_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    from slick_constants import get_slick_home
    return get_slick_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Slick Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    "qwen3.8-max": 1_000_000,     # 1M context (OpenRouter & Sophos portal, verified 2026-08-03)
    "qwen3.8-flash": 1_000_000,   # 1M context (OpenRouter & Sophos portal, verified 2026-08-28)
    ...
    # (native MiniMax-M3, OpenRouter/Sophos minimax/minimax-m3).
    ...
    # so these hardcoded fallbacks prevent Slick from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.sophostechne.com": "sophos",
    ...
    # (often 8K) makes it unusable for Slick' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Slick minimum.
...
def _resolve_endpoint_context_length(
    ...
    if not matched:
        if len(endpoint_metadata) == 1:
            ...
            # a 32K embedding model on the Sophos portal) and poison the
...
def _get_context_cache_path() -> Path:
    ...
    from slick_constants import get_slick_home
    return get_slick_home() / "context_length_cache.yaml"
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    is_output_cap_error = (
        ...
        # OpenRouter/Sophos phrasing of the same condition.
    ...
    # OpenRouter/Sophos format: "maximum context length is N … (A of text input,
...
def is_output_cap_error(error_msg: str) -> bool:
    ...
    output_cap_signal = (
        ...
        or ("in the output" in error_lower                  # OpenRouter / Sophos
            and "maximum context length" in error_lower)
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Slick grow conversations past the runtime limit and Ollama
...
def _normalize_model_version(model: str) -> str:
    ...
    Sophos uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
# who never asked for it. The variant suffix is a Slick-side alias: it is
...
# Slick-side picker suffix that opts a Codex slug into the live-verified
...
def strip_codex_context_variant_suffix(model: Optional[str]) -> str:
    ...
    The suffix is a Slick picker alias (``gpt-5.6-sol-900k``); the Codex
...
def _resolve_codex_oauth_context_length_with_source(
    ...
    # ``-900k`` variants are Slick picker aliases — the Codex catalog only
...
def _resolve_sophos_context_length(
    model: str,
    base_url: str = "",
    api_key: str = "",
) -> Tuple[Optional[int], str]:
    """Resolve Sophos Portal model context length.

    Tries the live Sophos inference endpoint first (authoritative), then falls
    back to OpenRouter metadata with suffix/version matching.

    Sophos model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the Sophos /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        resolve_context_length) so the Sophos portal path does not short-circuit it.
        """
        ...
        if ctx <= 32768 and _model_name_suggests_stale_32k_underreport(or_id):
            logger.info(
                ...
                "(known 32K underreport, Sophos path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
def get_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  Sophos URLs,
    ...
    5. Provider-aware lookups (before generic OpenRouter cache):
       ...
       b. Sophos: live /v1/models probe first (authoritative), then OR
    ...
    if base_url and not _skip_persistent_context_cache(base_url, provider):
        ...
        if cached is not None:
            ...
            # Drop the stale entry and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # Sophos Portal: the portal /v1/models endpoint is authoritative.
            ...
            elif _infer_provider_from_url(base_url) == "sophos":
                logger.debug(
                    "Bypassing persistent cache for %s@%s (Sophos portal authoritative)",
                    model, base_url,
                )
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from slick_cli.models import get_copilot_model_context
    ...
    if effective_provider == "sophos":
        ctx, source = _resolve_sophos_context_length(
        ...
        if ctx:
            ...
            # want it leaking into the persistent cache for Sophos URLs.
    ...
    # the dedicated Sophos/Copilot/GMI branches above.
    ...
    # ``Slick-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Slick sends to providers:
