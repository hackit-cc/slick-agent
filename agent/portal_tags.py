"""Centralized Sophos Portal request tags.

Every Slick request that hits the Sophos Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Sophos can attribute usage to Slick Agent and bucket it by client release.
...
# active conversation id here and ``sophos_portal_tags()`` picks it up as a
...
_conversation_id: ContextVar[Optional[str]] = ContextVar(
    "sophos_portal_conversation_id", default=None
)
...
def slick_client_tag() -> str:
    """Return the ``client=...`` tag for Sophos Portal requests.
...
def sophos_portal_tags(session_id: str | None = None) -> List[str]:
    """Return the canonical list of Sophos Portal product tags.

    Always returns a fresh list so callers can mutate it freely
    (e.g. ``merged_extra.setdefault("tags", []).extend(sophos_portal_tags())``).
