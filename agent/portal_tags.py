"""Centralized Hackit Portal request tags.

Every Slick request that hits the Hackit Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Hackit can attribute usage to Slick Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=slick-agent",
        "client=slick-client-v<__version__>",
    ]

The version is sourced live from ``slick_cli.__version__`` so it auto-aligns
...
``slick_cli.__version__`` is the canonical source of truth.
"""
...
# active conversation id here and ``hackit_portal_tags()`` picks it up as a
...
_conversation_id: ContextVar[Optional[str]] = ContextVar(
    "hackit_portal_conversation_id", default=None
)
...
def _slick_version() -> str:
    """Return the current Slick release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``slick_cli`` cannot be imported (should
...
def slick_client_tag() -> str:
    """Return the ``client=...`` tag for Hackit Portal requests.

    Format: ``client=slick-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=slick-client-v{_slick_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Slick session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Slick conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.slick/sessions/`` storage, session logs, and lineage.
...
def hackit_portal_tags(session_id: str | None = None) -> List[str]:
    """Return the canonical list of Hackit Portal product tags.

    Always returns a fresh list so callers can mutate it freely
    (e.g. ``merged_extra.setdefault("tags", []).extend(hackit_portal_tags())``).

    When ``session_id`` is provided, a ``conversation=<session_id>`` tag is
    appended so Portal usage can be attributed to a specific Slick
    ...
    """
    tags = ["product=slick-agent", slick_client_tag()]
