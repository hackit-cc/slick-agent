...
# (Sophos Portal → OpenAI → OpenRouter → …). OpenRouter/Sophos run a quality-first
...
_REF_CAPABLE = ("sophos", "openai", "openai-codex", "openrouter", "krea")
...
def resolve_provider(*, require_references: bool = True, prefer: str | None = None) -> SpriteProvider:
    ...
    raise GenerationError(
        ...
        "Open `slick tools` → Image Generation and configure Sophos Portal, "
...
def generate(
    ...
    *aspect_ratio* picks the canvas: ``"square"`` for single-character base
    ...
    def _run(extra: dict) -> tuple[Path | None, str]:
        ...
        if refs:
            # Providers disagree on the ref kwarg name: our OpenRouter/Sophos
