...
row taxonomy is inferred from the atlas shape so Slick can render both legacy
...
# rows (1536x1872), while older Slick/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Slick' activity state names. They are not always identical to the
...
# Legacy Slick/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Slick activity names -> accepted row-name aliases in descending
