...
Slick seeds its credential pool from many places:
    ...
    device_code   — auth.json providers.<provider> (sophos, openai-codex, ...)
...
qwen-cli, sophos device_code (partial), slick_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"sophos"``, ...).
...
def _remove_sophos_device_code(provider: str, removed) -> RemovalResult:
    """Sophos OAuth lives in auth.json providers.sophos — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `slick auth add sophos` (or any other path that writes providers.sophos)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `slick auth add sophos` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as Sophos: single-source OAuth state with refresh tokens.
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="sophos", source_id="device_code",
        remove_fn=_remove_sophos_device_code,
        description="auth.json providers.sophos",
    ))
    register(RemovalStep(
