...
as ``kind: backend``) or ``~/.slick/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        backend can't honor are not advertised), and by ``slick tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$SLICK_HOME/cache/videos/``, creating parents as needed."""
    from slick_constants import get_slick_home

    path = get_slick_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$SLICK_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$SLICK_HOME/cache/videos/``.
