...
def try_recover_primary_transport(
    ...
    Skipped for proxy/aggregator providers (OpenRouter, Sophos) which
    ...
    if (
        provider_lower in {"sophos", "sophos-portal", "sophostechne"}
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Slick-Session-Id header), and an
        ...
        if env_var_enabled("SLICK_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from slick_cli.config import load_config as _load_moa_cfg
    ...
    # Sophos Portal proxies to OpenRouter behind the scenes — identical
    # OpenAI-wire envelope cache_control semantics. Treat it as an
    # OpenRouter-equivalent endpoint for caching layout purposes.
    is_sophos_portal = base_url_host_matches(eff_base_url, "sophostechne.com")
    ...
    # that are canonicalized only after Slick sends the request. Honor its
    ...
    if _supports_anthropic_cache_markers and (
        ...
        try:
            from slick_cli.config import get_custom_provider_model_capability
    ...
    if (
        (is_openrouter or is_sophos_portal)
        and (is_claude or is_kimi)
        and not is_anthropic_wire
    ):
        return True, False
    # Sophos Portal Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
    ...
    if is_sophos_portal and "qwen" in model_lower:
        return True, False
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to slick's outer conversation loop,
    ...
    if agent.provider == "opencode-free":
        from slick_cli.models import opencode_zen_free_headers
    ...
    # All primary construction and recovery paths must identify Slick to the
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    # Pass model so dual-wire providers (Sophos Portal anthropic/* → Messages)
    ...
    try:
        ...
        from slick_cli.config import load_config as _sm_load_config
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from slick_cli.middleware import apply_tool_request_middleware
    ...
    if not pre_tool_block_checked:
        try:
            from slick_cli.plugins import _dispatch_pre_tool_call_hooks
    ...
    from slick_cli.middleware import run_tool_execution_middleware
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: slick' own dispatch loop intentionally keeps an
...
def _iter_httpx_pool_objects(http_client: Any):
    ...
    Slick' keepalive client (#10324 / ``_build_keepalive_http_client``) and
