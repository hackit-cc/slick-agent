...
def try_recover_primary_transport(
    ...
    Skipped for proxy/aggregator providers (OpenRouter, Hackit) which
    ...
    if (
        provider_lower in {"hackit", "hackit-portal", "hackit"}
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    # Hackit Portal proxies to OpenRouter behind the scenes — identical
    # OpenAI-wire envelope cache_control semantics. Treat it as an
    # OpenRouter-equivalent endpoint for caching layout purposes.
    is_hackit_portal = base_url_host_matches(eff_base_url, "hackit.cc")
    ...
    if (
        (is_openrouter or is_hackit_portal)
        and (is_claude or is_kimi)
        and not is_anthropic_wire
    ):
        return True, False
    # Hackit Portal Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
    ...
    if is_hackit_portal and "qwen" in model_lower:
        return True, False
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    # Pass model so dual-wire providers (Hackit Portal anthropic/* → Messages)
