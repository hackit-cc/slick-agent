...
``slick pause`` writes a sentinel file at ``$SLICK_HOME/ESTOP``;
``slick resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Slick is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.slick/ESTOP``.
...
def _slick_home() -> Path:
    """Resolve the active SLICK_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active SLICK_HOME."""
    return _slick_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on SLICK_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Slick is paused ({reason}). New work is on hold; "
            "run `slick resume` to pick things back up."
        )
    return (
        "⏸️ Slick is paused. New work is on hold; "
        "run `slick resume` to pick things back up."
    )
