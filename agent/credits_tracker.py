"""Credits tracking for Sophos inference API responses.

Parses x-sophos-credits-* (and optional x-sophos-tool-pool-*) headers from
...
Header schema (x-sophos-credits-* family):
    x-sophos-credits-version                    contract/schema version
    x-sophos-credits-remaining-micros           total remaining balance (micros)
    x-sophos-credits-remaining-usd              same, formatted USD string
    x-sophos-credits-subscription-micros        subscription balance (SIGNED; may be negative/debt)
    x-sophos-credits-subscription-usd           same, formatted USD string
    x-sophos-credits-subscription-limit-micros  subscription cap (PAIRED/optional)
    x-sophos-credits-subscription-limit-usd     same, formatted USD string (PAIRED/optional)
    x-sophos-credits-rollover-micros            rolled-over balance (micros)
    x-sophos-credits-purchased-micros           purchased balance (micros)
    x-sophos-credits-purchased-usd              same, formatted USD string
    x-sophos-credits-denominator-kind           "subscription_cap" | "none"
    x-sophos-credits-paid-access                "true" | "false" (STRING!)
    x-sophos-credits-disabled-reason            reason string (header omitted when null)
    x-sophos-credits-as-of-ms                   server-side timestamp (ms epoch)

Tool-pool headers use a SEPARATE prefix:
    x-sophos-tool-pool-micros                   tool-pool balance (micros)
    x-sophos-tool-pool-gated-off                "true" | "false" (STRING!)
...
@dataclass
class CreditsState:
    """Full credits state parsed from x-sophos-credits-* response headers."""
...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    """Return True when *model* is a Sophos free-tier model, using ONLY local data.

    Two signals, both zero-network:

    1. The ``:free`` suffix — the canonical Sophos free SKU marker (e.g.
       ``nvidia/nemotron-3-ultra:free``). Free by construction on the API side
       (spend is forced to 0 for ``:free`` ids).
    2. The ``stealth/`` prefix — Sophos stealth-preview SKUs (e.g.
    ...
    3. A peek into the in-process pricing cache in ``slick_cli.models``
    ...
    if not base_url:
        return False
    try:
        from slick_cli.models import _is_model_free, _pricing_cache

        # Mirror get_pricing_for_provider's key normalization: the agent's
        # Sophos base_url is /v1-suffixed (https://inference-api.sophostechne.com/v1)
...
def evaluate_credits_notices(
    ...
    *,
    ...
    ``model_is_free``: True when the session's active model is a Sophos free-tier
    ...
    if target_band != shown_band:
        ...
        if target_band is not None:
            ...
            # only meaningful against a Sophos subscription cap (no cap → never fires),
...
def parse_credits_headers(
    ...
    """Parse x-sophos-credits-* (and x-sophos-tool-pool-*) headers into a CreditsState.

    Returns None (miss) on ANY of:
    - No ``x-sophos-credits-version`` header present.
    ...
    try:
        ...
        # sentinel header is absent (the common case for non-Sophos providers, on
        ...
        if not any(k.lower() == "x-sophos-credits-version" for k in headers):
            return None
        ...
        version_raw = lowered.get("x-sophos-credits-version")
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update Slick",
                    version_val,
                )
            return None
        ...
        # ── Parse micros fields ──────────────────────────────────────────────
        remaining_micros = _req_nonneg("x-sophos-credits-remaining-micros")
        if remaining_micros is _SENTINEL:
            return None

        subscription_micros = _req_int("x-sophos-credits-subscription-micros")
        if subscription_micros is _SENTINEL:
            return None

        rollover_micros = _req_nonneg("x-sophos-credits-rollover-micros")
        if rollover_micros is _SENTINEL:
            return None

        purchased_micros = _req_nonneg("x-sophos-credits-purchased-micros")
        if purchased_micros is _SENTINEL:
            return None

        # tool_pool_micros is OPTIONAL: absent → 0 (default); present-but-invalid → None (miss).
        _tp_raw = lowered.get("x-sophos-tool-pool-micros")
        if _tp_raw is None:
            tool_pool_micros = 0
        else:
        ...
        as_of_ms = _req_nonneg("x-sophos-credits-as-of-ms")
        if as_of_ms is _SENTINEL:
            return None

        # ── Validate USD strings ─────────────────────────────────────────────
        remaining_usd = lowered.get("x-sophos-credits-remaining-usd", "")
        if not _validate_usd(remaining_usd):
            return None

        subscription_usd = lowered.get("x-sophos-credits-subscription-usd", "")
        if not _validate_usd(subscription_usd):
            return None

        purchased_usd = lowered.get("x-sophos-credits-purchased-usd", "")
        ...
        sub_limit_micros_raw = lowered.get("x-sophos-credits-subscription-limit-micros")
        sub_limit_usd_raw = lowered.get("x-sophos-credits-subscription-limit-usd")
        ...
        # ── denominator_kind ─────────────────────────────────────────────────
        denominator_kind = lowered.get("x-sophos-credits-denominator-kind", "none")
        ...
        if "x-sophos-credits-paid-access" in lowered:
            pa_raw = lowered["x-sophos-credits-paid-access"].strip().lower()
        ...
        if "x-sophos-tool-pool-gated-off" in lowered:
            tpgo_raw = lowered["x-sophos-tool-pool-gated-off"].strip().lower()
        ...
        disabled_reason = lowered.get("x-sophos-credits-disabled-reason")  # None if absent
...
# ── Dev test fixtures (SLICK_DEV_CREDITS_FIXTURE) ───────────────────────────
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set SLICK_DEV_CREDITS_FIXTURE to either a
...
# immediately), and (3) the /usage view (sophos_credits_lines renders the fixture).
# `clear` / `none` / unset → real behaviour. Delete with the rest of the
# SLICK_DEV_CREDITS scaffolding.
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for SLICK_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag SLICK_DEV_CREDITS
    is also on, so a stray SLICK_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, …) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("SLICK_DEV_CREDITS")):
        return None
    raw = os.environ.get("SLICK_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
...
def _credits_state_from_account(info) -> Optional[CreditsState]:
    """Map a SophosPortalAccountInfo into a header-shaped CreditsState for the seed.
...
def seed_credits_at_session_start(agent) -> bool:
    ...
    Returns True if it seeded this call, False otherwise (not sophos / already seeded /
    fail-open error). Never raises — credits must never block session startup.
    """
    try:
        if getattr(agent, "provider", "") != "sophos":
            return False
