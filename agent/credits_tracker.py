"""Credits tracking for Hackit inference API responses.

Parses x-hackit-credits-* (and optional x-hackit-tool-pool-*) headers from
...
Header schema (x-hackit-credits-* family):
    x-hackit-credits-version                    contract/schema version
    x-hackit-credits-remaining-micros           total remaining balance (micros)
    x-hackit-credits-remaining-usd              same, formatted USD string
    x-hackit-credits-subscription-micros        subscription balance (SIGNED; may be negative/debt)
    x-hackit-credits-subscription-usd           same, formatted USD string
    x-hackit-credits-subscription-limit-micros  subscription cap (PAIRED/optional)
    x-hackit-credits-subscription-limit-usd     same, formatted USD string (PAIRED/optional)
    x-hackit-credits-rollover-micros            rolled-over balance (micros)
    x-hackit-credits-purchased-micros           purchased balance (micros)
    x-hackit-credits-purchased-usd              same, formatted USD string
    x-hackit-credits-denominator-kind           "subscription_cap" | "none"
    x-hackit-credits-paid-access                "true" | "false" (STRING!)
    x-hackit-credits-disabled-reason            reason string (header omitted when null)
    x-hackit-credits-as-of-ms                   server-side timestamp (ms epoch)

Tool-pool headers use a SEPARATE prefix:
    x-hackit-tool-pool-micros                   tool-pool balance (micros)
    x-hackit-tool-pool-gated-off                "true" | "false" (STRING!)
...
@dataclass
class CreditsState:
    """Full credits state parsed from x-hackit-credits-* response headers."""
...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    """Return True when *model* is a Hackit free-tier model, using ONLY local data.

    Two signals, both zero-network:

    1. The ``:free`` suffix — the canonical Hackit free SKU marker (e.g.
       ``nvidia/nemotron-3-ultra:free``). Free by construction on the API side
       (spend is forced to 0 for ``:free`` ids).
    2. The ``stealth/`` prefix — Hackit stealth-preview SKUs (e.g.
    ...
    try:
        ...
        # Hackit base_url is /v1-suffixed (https://inference-api.hackit.cc/v1)
...
def evaluate_credits_notices(
    ...
    *,
    ...
    ``model_is_free``: True when the session's active model is a Hackit free-tier
    ...
    if target_band != shown_band:
        ...
        if target_band is not None:
            ...
            # only meaningful against a Hackit subscription cap (no cap → never fires),
...
def parse_credits_headers(
    ...
    """Parse x-hackit-credits-* (and x-hackit-tool-pool-*) headers into a CreditsState.

    Returns None (miss) on ANY of:
    - No ``x-hackit-credits-version`` header present.
    ...
    try:
        ...
        # sentinel header is absent (the common case for non-Hackit providers, on
        ...
        if not any(k.lower() == "x-hackit-credits-version" for k in headers):
            return None
        ...
        version_raw = lowered.get("x-hackit-credits-version")
        ...
        # ── Parse micros fields ──────────────────────────────────────────────
        remaining_micros = _req_nonneg("x-hackit-credits-remaining-micros")
        if remaining_micros is _SENTINEL:
            return None

        subscription_micros = _req_int("x-hackit-credits-subscription-micros")
        if subscription_micros is _SENTINEL:
            return None

        rollover_micros = _req_nonneg("x-hackit-credits-rollover-micros")
        if rollover_micros is _SENTINEL:
            return None

        purchased_micros = _req_nonneg("x-hackit-credits-purchased-micros")
        if purchased_micros is _SENTINEL:
            return None

        # tool_pool_micros is OPTIONAL: absent → 0 (default); present-but-invalid → None (miss).
        _tp_raw = lowered.get("x-hackit-tool-pool-micros")
        if _tp_raw is None:
            tool_pool_micros = 0
        else:
        ...
        as_of_ms = _req_nonneg("x-hackit-credits-as-of-ms")
        if as_of_ms is _SENTINEL:
            return None

        # ── Validate USD strings ─────────────────────────────────────────────
        remaining_usd = lowered.get("x-hackit-credits-remaining-usd", "")
        if not _validate_usd(remaining_usd):
            return None

        subscription_usd = lowered.get("x-hackit-credits-subscription-usd", "")
        if not _validate_usd(subscription_usd):
            return None

        purchased_usd = lowered.get("x-hackit-credits-purchased-usd", "")
        ...
        sub_limit_micros_raw = lowered.get("x-hackit-credits-subscription-limit-micros")
        sub_limit_usd_raw = lowered.get("x-hackit-credits-subscription-limit-usd")
        ...
        # ── denominator_kind ─────────────────────────────────────────────────
        denominator_kind = lowered.get("x-hackit-credits-denominator-kind", "none")
        ...
        if "x-hackit-credits-paid-access" in lowered:
            pa_raw = lowered["x-hackit-credits-paid-access"].strip().lower()
        ...
        if "x-hackit-tool-pool-gated-off" in lowered:
            tpgo_raw = lowered["x-hackit-tool-pool-gated-off"].strip().lower()
        ...
        disabled_reason = lowered.get("x-hackit-credits-disabled-reason")  # None if absent
...
# immediately), and (3) the /usage view (hackit_credits_lines renders the fixture).
...
def _credits_state_from_account(info) -> Optional[CreditsState]:
    """Map a HackitPortalAccountInfo into a header-shaped CreditsState for the seed.
...
def seed_credits_at_session_start(agent) -> bool:
    ...
    Returns True if it seeded this call, False otherwise (not hackit / already seeded /
    fail-open error). Never raises — credits must never block session startup.
    """
    try:
        if getattr(agent, "provider", "") != "hackit":
            return False
