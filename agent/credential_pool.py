...
# un-throttled log fires on *every* selection. On Windows several Slick
...
# readiness handshake ("Timed out connecting to Slick backend after
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "hackit":
            # Hackit stores the runtime inference credential in agent_key for
            ...
            for token, expires_at in (
                ...
                if (
                    ...
                    and token.strip()
                    and auth_mod._hackit_invoke_jwt_is_usable(
                        token,
...
def _write_through_provider_state_to_global_root(
    ...
    (#48415 / #43589): hackit, openai-codex, and xai-oauth rotate the
    ...
    ``slick_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``slick model`` / ``slick auth`` which
        ...
        Mirrors the Hackit/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Slick process
    ...
    def _sync_hackit_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Hackit pool entry from auth.json if tokens differ.

        Hackit OAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_hackit_runtime_credentials``, it writes fresh tokens to
        auth.json under ``_auth_store_lock``.  The pool entry's tokens
        become stale.  This method detects that and adopts the newer pair,
        avoiding a "refresh token reuse" revocation on the Hackit Portal.
        """
        if self.provider != "hackit" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
                state = _load_provider_state(auth_store, "hackit")
            if not state:
                return entry
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing Hackit state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync Hackit entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Hackit, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Hackit/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``slick`` invocation that defaults to the active provider
        (e.g. setup wizard, ``slick auth status``) would land on
        ...
        # device-code sources (hackit, openai-codex, xAI) use ``device_code``.
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Slick
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``SLICK_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "SLICK_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "SLICK_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("slick_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Slick
            ...
            elif self.provider == "hackit":
                synced = self._sync_hackit_entry_from_auth_store(entry)
                if synced is not entry:
                    entry = synced
                auth_mod.resolve_hackit_runtime_credentials(
                    force_refresh=force,
                )
                updated = self._sync_hackit_entry_from_auth_store(entry)
            else:
                return entry
        except Exception as exc:
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("slick_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For xai-oauth: same race as hackit — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # pool. Mirrors the Hackit quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/hackit — another Slick process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and Hackit quarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    with self._lock:
                        ...
                        self._persist(removed_ids=removed_ids)
                    return None
            # For hackit: another process may have consumed the refresh token
            # between our proactive sync and the HTTP call.  Re-sync from
            # auth.json and adopt the fresh tokens if available.
            if self.provider == "hackit":
                synced = self._sync_hackit_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("Hackit refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                    ...
                    return updated
                if auth_mod._is_terminal_hackit_refresh_error(exc):
                    logger.debug("Hackit refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                            state = _load_provider_state(auth_store, "hackit") or {
                            ...
                            entry_refresh = str(entry.refresh_token or "").strip()
                            if not store_refresh or store_refresh == entry_refresh:
                                auth_mod._quarantine_hackit_oauth_state(
                                    state,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                                auth_mod._quarantine_hackit_pool_entries(
                                    auth_store,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                        ...
                        logger.debug("Failed to clear terminal Hackit OAuth state: %s", clear_exc)

                    singleton_sources = {
                        auth_mod.HACKIT_DEVICE_CODE_SOURCE,
                        f"manual:{auth_mod.HACKIT_DEVICE_CODE_SOURCE}",
                    }
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "hackit":
            # Hackit refresh can require network access and should happen when
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Slick profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For hackit entries, sync from auth.json before status checks.
            # Another process may have successfully refreshed via
            # resolve_hackit_runtime_credentials(), making this entry's
            ...
            # re-authed via `slick model` / `slick auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `slick model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``slick auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `slick auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `slick auth remove <provider> <N>` is stable across all source types.
    try:
        from slick_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Slick PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `slick setup` ("Claude
        ...
        # Claude Code CLI, slick_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.slick/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `slick setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"slick_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_slick_oauth_credentials

        for source_name, creds in (
            ("slick_pkce", read_slick_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # (set by persist_hackit_credentials(label=...) when the user ran
            # `slick auth add hackit --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (slick auth remove copilot gh_cli) must not pay either on
        ...
        # Slick auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from slick_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.slick/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `slick auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `slick auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Slick auth store.  Without this gate the removal is instantly
        ...
        # Slick owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``slick model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``slick auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            # always surfaced as ``device_code`` (consistent with hackit/codex).
            source = "device_code"
            if _is_suppressed(provider, source):
                return changed, active_sources
            active_sources.add(source)
            from slick_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# Prefer ~/.slick/.env over os.environ — the user's config file is the
# authoritative source for Slick credentials. Stale env vars from parent
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_slick_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `slick auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.slick/.env.
    ...
    try:
        from slick_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from slick_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.slick/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `slick auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Slick
