...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "sophos":
            # Sophos stores the runtime inference credential in agent_key for
            ...
            for token, expires_at in (
                ...
                if (
                    ...
                    and token.strip()
                    and auth_mod._sophos_invoke_jwt_is_usable(
                        token,
...
def _write_through_provider_state_to_global_root(
    ...
    (#48415 / #43589): sophos, openai-codex, and xai-oauth rotate the
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Mirrors the Sophos/Anthropic resync paths above.  Only applies to
    ...
    def _sync_sophos_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Sophos pool entry from auth.json if tokens differ.

        Sophos OAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_sophos_runtime_credentials``, it writes fresh tokens to
        auth.json under ``_auth_store_lock``.  The pool entry's tokens
        become stale.  This method detects that and adopts the newer pair,
        avoiding a "refresh token reuse" revocation on the Sophos Portal.
        """
        if self.provider != "sophos" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
                state = _load_provider_state(auth_store, "sophos")
            if not state:
                return entry
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing Sophos state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync Sophos entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Sophos, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Sophos/Codex/xAI refresh in a multi-provider
        ...
        # device-code sources (sophos, openai-codex, xAI) use ``device_code``.
    ...
    def _refresh_entry_impl(
        ...
        try:
            ...
            elif self.provider == "sophos":
                synced = self._sync_sophos_entry_from_auth_store(entry)
                if synced is not entry:
                    entry = synced
                auth_mod.resolve_sophos_runtime_credentials(
                    force_refresh=force,
                )
                updated = self._sync_sophos_entry_from_auth_store(entry)
            else:
                return entry
        except Exception as exc:
            ...
            if self.provider == "anthropic" and entry.source == "claude_code":
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    return synced
            # For xai-oauth: same race as sophos — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # pool. Mirrors the Sophos quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/sophos — another Slick process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and Sophos quarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    with self._lock:
                        ...
                        self._persist(removed_ids=removed_ids)
                    return None
            # For sophos: another process may have consumed the refresh token
            # between our proactive sync and the HTTP call.  Re-sync from
            # auth.json and adopt the fresh tokens if available.
            if self.provider == "sophos":
                synced = self._sync_sophos_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("Sophos refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                    ...
                    return updated
                if auth_mod._is_terminal_sophos_refresh_error(exc):
                    logger.debug("Sophos refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                            state = _load_provider_state(auth_store, "sophos") or {
                            ...
                            entry_refresh = str(entry.refresh_token or "").strip()
                            if not store_refresh or store_refresh == entry_refresh:
                                auth_mod._quarantine_sophos_oauth_state(
                                    state,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                                auth_mod._quarantine_sophos_pool_entries(
                                    auth_store,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                        ...
                        logger.debug("Failed to clear terminal Sophos OAuth state: %s", clear_exc)

                    singleton_sources = {
                        auth_mod.SOPHOS_DEVICE_CODE_SOURCE,
                        f"manual:{auth_mod.SOPHOS_DEVICE_CODE_SOURCE}",
                    }
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "sophos":
            # Sophos refresh can require network access and should happen when
        ...
        for entry in self._entries:
            ...
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For sophos entries, sync from auth.json before status checks.
            # Another process may have successfully refreshed via
            # resolve_sophos_runtime_credentials(), making this entry's
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # (set by persist_sophos_credentials(label=...) when the user ran
            # `slick auth add sophos --label <name>`).  Fall back to the
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            # always surfaced as ``device_code`` (consistent with sophos/codex).
            source = "device_code"
