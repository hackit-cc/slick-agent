# nix/desktop.nix — Slick Desktop (Electron) app build + wrapper
#
# `slickAgent` is the fully-built `.#default` package — it ships the
# `slick` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `SLICK_DESKTOP_SLICK` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Slick CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # SLICK_HOME and SLICK_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "slick-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "slick-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = slickNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "slick-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe slickNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/slick-desktop $out/bin
    cp -r ${renderer}/* $out/share/slick-desktop/
    ...
    substituteInPlace $out/share/slick-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/slick-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # SLICK_DESKTOP_SLICK to the absolute path of the nix-built `slick`
    # binary so the desktop's resolver step 4 ("existing Slick CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/slick-desktop \
      --add-flags "$out/share/slick-desktop" \
      --set SLICK_DESKTOP_SLICK "${lib.getExe slickAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/slick.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../slick_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/slick-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/slick.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/slick.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Slick Agent";
    homepage = "https://github.com/SophosTechne/slick-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "slick-desktop";
  };
}
