...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Slick derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            slick-agent = prev.slick-agent.overrideAttrs (_old: {
              SLICK_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $SLICK_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$SLICK_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        slick-agent = prev.slick-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "slick-agent-env" {
    slick-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "slick-agent-editable-env" {
    slick-agent = dependency-groups;
  };
}
