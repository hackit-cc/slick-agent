# nix/nixosModules.nix — the NixOS module for slick-agent
#
...
#
# Container mode: slick runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $SLICK_HOME/.env
# and read by slick at startup — no container recreation needed for env changes.
#
# Tool resolution: the slick wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.slick-agent;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      slick-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      slickHome = "${cfg.stateDir}/.slick";
      ...
      # SLICK_HOME via addToSystemPackages, so they can save settings through the
      ...
      # slick user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "slick-container-entrypoint" ''
        set -eu

        SLICK_UID="''${SLICK_UID:?SLICK_UID must be set}"
        SLICK_GID="''${SLICK_GID:?SLICK_GID must be set}"

        # ── Group: ensure a group with GID=$SLICK_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$SLICK_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="slick"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$SLICK_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$SLICK_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$SLICK_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$SLICK_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="slick"
          TARGET_HOME="/home/slick"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$SLICK_UID" -g "$SLICK_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$SLICK_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$SLICK_UID:$SLICK_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure SLICK_HOME is owned by the target user.
        ...
        if [ -n "''${SLICK_HOME:-}" ] && [ -d "$SLICK_HOME" ]; then
          find "$SLICK_HOME" \! -user "$SLICK_UID" -exec chown "$SLICK_UID:$SLICK_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/slick-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/slick-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/slick ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/slick
          chmod 0440 /etc/sudoers.d/slick
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$SLICK_UID" --regid="$SLICK_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $SLICK_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "slick-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        slick_bin=${containerDataDir}/current-package/bin/slick
      '';

      # Default: /var/lib/slick/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the slick group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit slickHome; };

      unitPath = common.processPath { inherit pkgs cfg; };

    in
    {
      options.services.slick-agent =
        common.sharedOptions {
          defaultPackage = slick-agent;
          defaultPackageText = lib.literalExpression "slick-agent.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/slick";
              description = "State directory. Contains .slick/ subdir (SLICK_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the slick CLI to environment.systemPackages and export
                SLICK_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.slick symlink to the service
                  stateDir. These users are automatically added to the slick group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.slick-agent.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the slick CLI to system PATH and export SLICK_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.slick/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.SLICK_HOME = slickHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.slick-agent";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.slick-agent.workingDirectory;
                optionPath = "services.slick-agent";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.slick-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the slick user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.slick-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed slick
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure slick is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${slickHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${slickHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."slick-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${slickHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${slickHome} ${cfg.workingDirectory}
                  ...
                  find ${slickHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${slickHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${slickHome}/$_subdir"
                    chmod 2770 "${slickHome}/$_subdir"
                    find "${slickHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg slickHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${slickHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${slickHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${slickHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.slick";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${slickHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "slick-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.slick -> stateDir/.slick for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.slick-agent = {
              description = "Slick Agent Gateway";
              wantedBy = [ "multi-user.target" ];
              after = [ "network-online.target" ];
              wants = [ "network-online.target" ];

              # cfg.environment and cfg.environmentFiles are written to
              # $SLICK_HOME/.env by the activation script. load_slick_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.slick-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.slick-agent = {
              description = "Slick Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  SLICK_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  SLICK_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env SLICK_UID="$SLICK_UID" \
                    --env SLICK_GID="$SLICK_GID" \
                    --env SLICK_HOME=${containerDataDir}/.slick \
                    --env SLICK_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/slick gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
