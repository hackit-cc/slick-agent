...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      slick-agent = self'.packages.default;
      slickVenv = slick-agent.slickVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "slick-check";
                homeDirectory = "/home/slick-check";
                stateVersion = "24.11";
              };
            }
            { services.slick-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.slick-agent = programs;
              services.slick-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.slick-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.slick-agent);
      ...
      configKeys = pkgs.runCommand "slick-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${slickVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "slick-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "slick-build-package" { } ''
          echo "PASS: package built at ${slick-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "slick-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.SLICK_TEST = "1";
                environmentFiles = [ "/run/secrets/slick-env" ];
                slickHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "slick" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "slick" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.slickAgentSetup.data;

            failures =
              lib.optional (names != [
                "slick-agent"
                "slick-backend"
              ]) "expected slick-agent + slick-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/slick gateway" (argvOf "slick-agent")
              ) "gateway process does not run `slick gateway`: ${argvOf "slick-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/slick serve" (argvOf "slick-backend")
              ) "backend process does not run `slick serve`: ${argvOf "slick-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "slick-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/slick-check/.slick" (envOf n)) names
              ) "gateway and backend must share one SLICK_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.SLICK_HOME or null != "/home/slick-check/.slick"
              ) "programs.slick-agent.enable must export SLICK_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "slick-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/slick-check/.slick/SOUL.md" activation
              ) "slickHomeFiles must install into SLICK_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads SLICK_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "SLICK_MANAGED=home-manager" (envOf n)) names
              ) "processes must report SLICK_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "slick-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "slick-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/slick-check";
            ...
            cases = [
              ...
              {
                name = "slickHomeFiles needs no directory";
                ok = accepts { slickHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "slick-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one SLICK_HOME ───────────────
        # `programs.enable` exports SLICK_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.slick
        # while the services use the SLICK_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/slick-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                slickHome = "/home/slick-check/.slick-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "slick-desktop") cfg.home.packages;
            ...
            # second Slick runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "slick-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.slick-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.slick-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one slick-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "SLICK_HOME" != "/home/slick-check/.slick-work"
              ) "the launcher must carry SLICK_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "SLICK_HOME")})"
              ++ lib.optional (
                setValue "SLICK_MANAGED" != "home-manager"
              ) "the launcher must report SLICK_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "SLICK_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "SLICK_DESKTOP_SLICK" != "${lib.head agentPackages}/bin/slick"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "SLICK_DESKTOP_SLICK")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "SLICK_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "SLICK_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "SLICK_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "SLICK_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "SLICK_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "slick-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares SLICK_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set SLICK_HOME" wrapper
              ) "the launcher must carry SLICK_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "SLICK_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "SLICK_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? slick-backend || cfg.launchd.agents ? slick-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "slick-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `slick`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.slick-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.slick-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "slick-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "slick-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "slick" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."slick-agent-setup".text;

            failures =
              lib.optional (names != [
                "slick-agent"
                "slick-backend"
              ]) "expected slick-agent + slick-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/slick gateway" (execOf "slick-agent")
              ) "gateway unit does not run `slick gateway`: ${execOf "slick-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/slick dashboard" (execOf "slick-backend")
              ) "backend unit does not run `slick dashboard`: ${execOf "slick-backend"}"
              ++ lib.optional (
                units.slick-agent.environment.SLICK_HOME != units.slick-backend.environment.SLICK_HOME
              ) "gateway and backend must share one SLICK_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/slick/.slick/SOUL.md" activation
              ) "slickHomeFiles must install into SLICK_HOME";
          ...
          in
          pkgs.runCommand "slick-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs slick, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.slick-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/slick serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec slick directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "slick-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "slick-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec slick, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/slick dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "slick-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $SLICK_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                SLICK_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "slick-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'SLICK_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--slick-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "slick-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "slick-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${slick-agent}/bin/slick || (echo "FAIL: slick binary missing"; exit 1)
          test -x ${slick-agent}/bin/slick-agent || (echo "FAIL: slick-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${slick-agent}/bin/slick --version 2>&1 | grep -qi "slick" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "slick-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in slick slick-agent slick-acp; do
            test -x ${slick-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "slick-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking slick --help ==="
          ${slick-agent}/bin/slick --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${slick-agent}/bin/slick --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "slick-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${slick-agent}/share/slick-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${slick-agent}/share/slick-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "SLICK_BUNDLED_SKILLS" ${slick-agent}/bin/slick || \
            (echo "FAIL: SLICK_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: SLICK_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${slick-agent}/share/slick-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${slick-agent}/share/slick-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "SLICK_OPTIONAL_SKILLS" ${slick-agent}/bin/slick || \
            (echo "FAIL: SLICK_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, SLICK_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "slick-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${slick-agent}/share/slick-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${slick-agent}/share/slick-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "SLICK_BUNDLED_PLUGINS" ${slick-agent}/bin/slick || \
            (echo "FAIL: SLICK_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: SLICK_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "slick-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${slick-agent}/share/slick-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${slick-agent}/share/slick-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${slick-agent}/share/slick-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "SLICK_BUNDLED_LOCALES" ${slick-agent}/bin/slick || \
            (echo "FAIL: SLICK_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: SLICK_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + SLICK_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && SLICK_BUNDLED_LOCALES=${slick-agent}/share/slick-agent/locales \
            ${slickVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with SLICK_BUNDLED_LOCALES set"; exit 1)
        ...
        # SLICK_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "slick-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${slick-agent}/share/slick-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${slick-agent}/share/slick-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "SLICK_OPTIONAL_MCPS" ${slick-agent}/bin/slick || \
            (echo "FAIL: SLICK_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: SLICK_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${slick-agent}/bin/slick mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: slick mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "slick-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${slick-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${slick-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "SLICK_TUI_DIR" ${slick-agent}/bin/slick || \
            (echo "FAIL: SLICK_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: SLICK_TUI_DIR set in wrapper"
        ...
        # Verify SLICK_NODE is set in wrapper and points to Node 26+
        # (Slick pins its toolchain to Node 26 everywhere)
        slick-node = pkgs.runCommand "slick-node-version" { } ''
          set -e
          echo "=== Checking SLICK_NODE in wrapper ==="
          grep -q "SLICK_NODE" ${slick-agent}/bin/slick || \
            (echo "FAIL: SLICK_NODE not set in wrapper"; exit 1)
          echo "PASS: SLICK_NODE present in wrapper"

          SLICK_NODE=$(sed -n "s/^export SLICK_NODE='\(.*\)'/\1/p" ${slick-agent}/bin/slick)
          test -x "$SLICK_NODE" || (echo "FAIL: SLICK_NODE=$SLICK_NODE not executable"; exit 1)
          echo "PASS: SLICK_NODE executable at $SLICK_NODE"

          NODE_MAJOR=$("$SLICK_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Slick requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All SLICK_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify SLICK_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "slick-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(SLICK_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking SLICK_MANAGED guards ==="
          check_blocked "config set" ${slick-agent}/bin/slick config set model foo
          check_blocked "config edit" ${slick-agent}/bin/slick config edit
        ...
        extra-python-packages = let
          ...
          slickWithExtra = slick-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "slick-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${slickWithExtra}/bin/slick || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${slickWithExtra}/bin/slick || \
          ...
          if grep -q "PYTHONPATH" ${slick-agent}/bin/slick; then
        ...
        extra-dependency-groups = let
          slickWithGroups = slick-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "slick-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${slickWithGroups}"
          echo "venv: ${slickWithGroups.slickVenv}"
        ...
        messaging-variant = pkgs.runCommand "slick-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.slickVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "slick-config-roundtrip" {
          ...
          merge_and_load() {
            local slick_home="$1"
            export SLICK_HOME="$slick_home"
            ${configMergeScript} ${nixSettings} "$slick_home/config.yaml"
            ${slickVenv}/bin/python3 -c '
import json, sys
