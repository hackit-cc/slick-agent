...
#
# `services.slick-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  slickHome, programs.slick-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "slick-config-attrs";
    description = "Slick YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $SLICK_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Slick Agent";
      ...
      package = mkOption {
        ...
        description = "The slick-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Slick configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `slick config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $SLICK_HOME/.env. Slick reads that file at each start, with
          load_slick_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."slick/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Slick refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Slick reads SOUL.md and memories/ from SLICK_HOME, so put those
          files in `slickHomeFiles`.
      ...
      slickHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into SLICK_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Slick reads SOUL.md and the memory files from SLICK_HOME and not
          from the working directory. Declare those files here, or Slick
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the slick plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Slick discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          slick_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as slick (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in slick-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `slick gateway`.";
      };
      ...
      # ── The backend: `slick serve` or `slick dashboard` ──────────────
      # `slick serve` and `slick dashboard` are the same entry point,
      # slick_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Slick Desktop
      ...
      # controls an external gateway, with `slick gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Slick
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            SLICK_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Slick Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."slick/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `slick` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.slick-agent.installPackage was removed. Slick now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.slick-agent.enable = ${lib.boolToString (value != false)};  # the slick CLI, and SLICK_HOME for your shells
        programs.slick-agent.desktop.enable = true;  # the desktop application

      `services.slick-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "slick-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "slick-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "slick-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "slick-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "slick-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # SLICK_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Slick make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${slickHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Slick
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${slickHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${slickHome}/config.yaml
            ${run}chmod ${modes.config} ${slickHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "slick-managed" managedSystem} ${slickHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${slickHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${slickHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${slickHome}/auth.json"
        else
          ''
            if [ ! -e ${slickHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${slickHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree slickHome cfg.slickHomeFiles}
      ...
      ${run}find ${slickHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "slick-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${slickHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/slick"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps slick as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "slick-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "slick-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "slick-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          SLICK_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export SLICK_DASHBOARD_SESSION_TOKEN

          if [ -z "$SLICK_DASHBOARD_SESSION_TOKEN" ]; then
            echo "slick-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "slick-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "slick-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "slick-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "slick-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "slick-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "slick-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Slick Agent web dashboard and desktop backend"
    else
      "Slick Agent backend for Slick Desktop";

  # The environment that each Slick process needs, from either module.
  #
  # managedSystem gives the value of SLICK_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Slick an identity and a memory, use
          ${optionPath}.slickHomeFiles instead. Those files go to
          SLICK_HOME. Slick reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of SLICK_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
