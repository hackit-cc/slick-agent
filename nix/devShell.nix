...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      slickNpmLib = self'.packages.default.passthru.slickNpmLib;
      ...
      slickAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "slick" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../slick} $out/bin/slick
          '')
        ...
        shellHook = ''
          ${slickAgentDevShellHook}
          ${slickNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export SLICK_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Slick Agent dev shell in $SLICK_PYTHON_SRC_ROOT"
          echo "Ready. Run 'slick' or 'sandbox slick' to start."
        '';
      };
    };
}
