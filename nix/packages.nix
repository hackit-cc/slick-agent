# nix/packages.nix — Slick Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./slick-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.slickTui;
        web = full.slickWeb;
        desktop = full.slickDesktop;

        update-npm-lockfile = full.slickNpmLib.updateNpmLockfile;
      };
    };
}
