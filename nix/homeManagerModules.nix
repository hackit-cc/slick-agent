# nix/homeManagerModules.nix — the Home Manager module for slick-agent
#
# This module is the user-level equivalent of nixosModules.default. Slick is
...
#
# `services.slick-agent` is the same option set on both modules. All of the
...
#   changed   addToSystemPackages      -> programs.slick-agent.enable and
#                                        home.sessionVariables
#   added     programs.slick-agent    the CLI and the desktop application,
#                                      because Home Manager separates an
#                                      installation from a daemon
#   changed   stateDir (+ "/.slick")  -> slickHome, set directly
#
# To use the module:
#   imports = [ slick-agent.homeManagerModules.default ];
#   programs.slick-agent = {
#     enable = true;          # the slick CLI on your PATH
...
{ inputs, ... }:
{
  flake.homeManagerModules.default =
    ...
    let
      ...
      cfgPrograms = config.programs.slick-agent;
      ...
      # not reach it, and the application would open ~/.slick while the
      # services use slickHome. Thus the launcher carries the value itself.
      #
      # SLICK_MANAGED rides along only when the services are enabled. That
      ...
      desktopEnvironment = {
        SLICK_HOME = cfg.slickHome;
      }
      // lib.optionalAttrs cfg.enable {
        inherit (processEnvironment) SLICK_MANAGED;
      }
      // lib.optionalAttrs desktopUsesService {
        SLICK_DESKTOP_REMOTE_URL = "http://${cfg.backend.host}:${toString cfg.backend.port}";
      };
      ...
      # token the desktop resolver throws ("SLICK_DESKTOP_REMOTE_URL is set
      # but SLICK_DESKTOP_REMOTE_TOKEN is not"), so the two variables travel
      ...
      desktopRun = lib.optional desktopUsesService ''
        if [ -r ${lib.escapeShellArg cfg.backend.sessionTokenFile} ]; then
          SLICK_DESKTOP_REMOTE_TOKEN="$(tr -d '\r\n' < ${lib.escapeShellArg cfg.backend.sessionTokenFile})"
          export SLICK_DESKTOP_REMOTE_TOKEN
        else
          echo "slick-desktop: cannot read the session token at ${cfg.backend.sessionTokenFile}." >&2
          echo "slick-desktop: the application starts its own backend instead of the one of the service." >&2
        fi
      '';
    ...
    in
    {
      # ── programs.slick-agent — the installation ───────────────────────
      # Home Manager separates "install this application for me" from "run
      # this daemon". Slick needs both, and a person can want one without
      # the other: an application with no gateway, or a headless gateway on
      # a machine with no display.
      #
      # `services.slick-agent` stays the authority for the state and the
      # configuration. This module reads slickHome and the backend address
      # from it, and never the reverse.
      options.programs.slick-agent = {
        enable = lib.mkEnableOption ''
          the Slick Agent command line application.

          This adds `slick` to home.packages, and exports SLICK_HOME with
          home.sessionVariables. An interactive shell then uses the same
          state as `services.slick-agent`
        '';

        package = lib.mkOption {
          type = lib.types.package;
          default = effectivePackage;
          defaultText = lib.literalExpression "config.services.slick-agent.package";
          description = ''
            The slick-agent package to install.

            The default follows `services.slick-agent.package`, and applies
        ...
        desktop = {
          enable = lib.mkEnableOption ''
            the Slick Desktop application (Electron).

            This adds `slick-desktop` to home.packages, with an XDG
            launcher entry on Linux. The launcher starts the same Slick
            runtime that `package` gives, and reads the SLICK_HOME of
            `services.slick-agent`. Thus the application, the interactive
            shell and the services share one state directory.

            The Electron application carries its own Slick runtime with
            the usual distribution. This module gives it the Nix package
            instead, with SLICK_DESKTOP_SLICK. It installs no second copy
            of Slick, and it downloads nothing on the first start
          '';

          package = lib.mkOption {
            type = lib.types.package;
            default = cfgPrograms.package.slickDesktop;
            defaultText = lib.literalExpression "config.programs.slick-agent.package.slickDesktop";
            description = ''
              The slick-desktop package to use.

              The default follows `package`, and thus also
              `services.slick-agent.extraPythonPackages` and
              `extraDependencyGroups`, because the desktop application is a
              passthru of the agent package. A package that you set here
              carries its own Slick runtime, and this module cannot make
      ...
      options.services.slick-agent =
        ...
        // {
          slickHome = lib.mkOption {
            type = lib.types.str;
            default = "${config.home.homeDirectory}/.slick";
            defaultText = lib.literalExpression ''"''${config.home.homeDirectory}/.slick"'';
            description = ''
              The value of SLICK_HOME. This state directory holds
              config.yaml, .env, auth.json, the sessions, the skills, the
              memory and the cron jobs.

              The NixOS module takes a `stateDir` and adds `/.slick` to it.
              This module sets SLICK_HOME directly. Thus an existing
              ~/.slick continues to work, and you can give the directory any
              name.
            '';
            example = "/home/alice/.slick-work";
          };

          # `installPackage` moved to `programs.slick-agent.enable`. The
          ...
          # quiet removal gives them a machine with no `slick` and no
          ...
          installPackage = lib.mkOption {
            ...
            visible = false;
            description = ''
              Removed. Use `programs.slick-agent.enable` instead.
            '';
          };
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf cfgPrograms.enable {
          ...
          home.sessionVariables.SLICK_HOME = cfg.slickHome;
        })

        # A launcher from the desktop menu reads no shell profile, so the
        # SLICK_HOME that `programs.enable` exports does not reach it. Home
        # Manager writes only systemd.user.sessionVariables into
        # environment.d, and this module does not put SLICK_HOME there,
          ...
          assertions = [
            {
              ...
              # with no `slick` on the PATH and no message.
        ...
        (lib.mkIf cfg.enable (
          lib.mkMerge [
                ...
                ++ [
                  {
                    ...
                    message = "services.slick-agent.backend.waitFor = \"interface\" works on Linux only. Use \"hostname\" on Darwin.";
                  }
                ];
            }
            ...
            # ── Activation: directories, config, secrets, documents ────────
            {
              ...
              home.activation.slickAgentSetup =
                lib.hm.dag.entryAfter
                  ...
                  (
                    common.mkStateScript {
                      inherit pkgs cfg;
                      inherit (cfg) slickHome workingDirectory;
                      run = "$DRY_RUN_CMD ";
            ...
            (lib.mkIf (isLinux && cfg.gateway.enable) {
              systemd.user.services.slick-agent = mkUnit {
            ...
            (lib.mkIf (isLinux && cfg.backend.mode != "none") {
              systemd.user.services.slick-backend = mkUnit {
            ...
            (lib.mkIf (isDarwin && cfg.gateway.enable) {
              launchd.agents.slick-agent = mkAgent {
                argv = common.gatewayArgv cfg;
                logName = "slick-agent";
              };
            })
            ...
            (lib.mkIf (isDarwin && cfg.backend.mode != "none") {
              launchd.agents.slick-backend = mkAgent {
                argv = common.backendArgv { inherit pkgs cfg; };
                logName = "slick-backend";
              };
            })
          ]
        ))
