# nix/web.nix — Slick Web Dashboard (Vite/React) frontend build
{ slickNpmLib, ... }:
slickNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @slick/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../slick_cli/web_dist for the
