...
# 5. Symlinks the 'slick' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}😎 Slick Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/slick-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink slick into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up slick command..."

SLICK_BIN="$SCRIPT_DIR/venv/bin/slick"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SLICK_BIN" "$COMMAND_LINK_DIR/slick"
echo -e "${GREEN}✓${NC} Symlinked slick → $COMMAND_LINK_DISPLAY_DIR/slick"
...
# ============================================================================
# Seed bundled skills into ~/.slick/skills/
# ============================================================================

SLICK_SKILLS_DIR="${SLICK_HOME:-$HOME/.slick}/skills"
mkdir -p "$SLICK_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.slick/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SLICK_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  slick status        # Check configuration"
if is_termux; then
    echo "  slick gateway       # Run gateway in foreground"
else
    echo "  slick gateway install # Install gateway service (messaging + cron)"
fi
echo "  slick cron list     # View scheduled jobs"
echo "  slick doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m slick_cli.main setup
fi
