...
class AIAgent:
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        # Hackit serves GPT-5.x models via its OpenAI-compatible chat
        # completions endpoint; its /v1/responses endpoint returns 404.
        if normalized_provider == "hackit":
            return False
    ...
    def _capture_credits(self, http_response: Any) -> None:
        """Parse x-hackit-credits-* headers, cache CreditsState, fire threshold notices.
        ...
        if state is None:
            if _dev:
                logger.info(
                    "credits ▸ response had no valid x-hackit-credits-* headers "
                    "(miss — producer off / non-Hackit path / >TTL stale)"
                )
            return
    ...
    def _try_refresh_hackit_client_credentials(
        self,
        *,
        force: bool = True,
    ) -> bool:
        if self.provider != "hackit":
            return False
        ...
        try:
            ...
            logger.debug("Hackit credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # Hackit requests should not inherit OpenRouter-only attribution headers.
        self._client_kwargs.pop("default_headers", None)

        if not self._replace_primary_openai_client(reason="hackit_credential_refresh"):
            return False
    ...
    def _anthropic_messages_create(self, api_kwargs: dict, *, client: Any = None):
        ...
        return create_anthropic_message(
            ...
            # matching header families (x-ratelimit-* / x-hackit-credits-*).
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct Hackit Portal.
        """
        if base_url_host_matches(self._base_url_lower, "hackit.cc"):
            return True
    ...
    def run_conversation(
        ...
        try:
            ...
            if getattr(relay_turn, "relay_enabled", True):
                ...
                task_started = True
            # Publish the conversation id for ambient Hackit Portal tagging. Every
