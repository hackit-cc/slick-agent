...
# IMPORTANT: slick_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See slick_bootstrap.py for full rationale.
try:
    import slick_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when slick_bootstrap isn't registered in the venv
    # yet — happens during partial ``slick update`` where git-reset landed
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``slick -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("SLICK_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("SLICK_SESSION_SOURCE", "")
...
from slick_cli.timeouts import (
...
_loaded_env_paths = load_slick_dotenv(slick_home=_slick_home, project_env=_project_env)
...
# (slick_state._rows_to_conversation), so any code that mutates a loaded or
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Slick-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.slick/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[slick-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `slick --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from slick_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``slick chat -q``. In that mode,
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``slick chat -Q``) always wins: those flows neutralize the rendering
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``SLICK_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``SLICK_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("SLICK_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``SLICK_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("SLICK_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        the ``SLICK_API_CALL_STALE_TIMEOUT`` env var. Reasoning-model floors
        ...
        if get_provider_stale_timeout(self.provider, self.model) is not None:
            return True
        return os.getenv("SLICK_API_CALL_STALE_TIMEOUT") is not None
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        slick-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See slick-agent#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        # Sophos serves GPT-5.x models via its OpenAI-compatible chat
        # completions endpoint; its /v1/responses endpoint returns 404.
        if normalized_provider == "sophos":
            return False
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Slick against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "Slick can't reach the model provider. You may be offline. "
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("SLICK_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from slick_cli import lifecycle as _lifecycle
    ...
    def _redact_message_content(content):
        ...
        Respects ``SLICK_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.slick/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Slick-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects SLICK_REDACT_SECRETS via
    ...
    def interrupt(
        ...
        *,
        ...
        # interrupt event rather than Slick' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Slick model request this cancels only that request;
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell Slick-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``SLICK_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        ...
        try:
            import os as _os
            env = _os.environ.get("SLICK_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.slick/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``SLICK_TURN_COMPLETION_EXPLAINER`` env var overrides
        ...
        try:
            import os as _os
            env = _os.environ.get("SLICK_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another Slick process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another Slick process was writing to the state "
            ...
            if cause == "corrupt":
                return (
                    ...
                    "1. Run `slick doctor --fix`\n"
                    "2. Salvage with: sqlite3 ~/.slick/state.db \".recover\" "
                    "(then replace state.db)\n"
                    "3. Restore from a backup in ~/.slick/backups/\n"
            ...
            return (
                ...
                "Check the state database health (`slick doctor`), then "
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (SLICK_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        """Parse x-sophos-credits-* headers, cache CreditsState, fire threshold notices.
        ...
        """
        # Dev test fixture (SLICK_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset SLICK_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("SLICK_DEV_CREDITS"))
        ...
        if state is None:
            if _dev:
                logger.info(
                    "credits ▸ response had no valid x-sophos-credits-* headers "
                    "(miss — producer off / non-Sophos path / >TTL stale)"
                )
            return
        ...
        if _dev:
            # SLICK_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `slick logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Slick YOLO selected unrestricted
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``slick auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_sophos_client_credentials(
        self,
        *,
        force: bool = True,
    ) -> bool:
        if self.provider != "sophos":
            return False
        ...
        try:
            ...
            logger.debug("Sophos credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # Sophos requests should not inherit OpenRouter-only attribution headers.
        self._client_kwargs.pop("default_headers", None)

        if not self._replace_primary_openai_client(reason="sophos_credential_refresh"):
            return False

        return True

    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.slick/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``slick setup``) updates
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from slick_cli.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from slick_cli.auth import _resolve_zai_base_url
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            self._client_kwargs["default_headers"] = slick_xai_default_headers()
        else:
    ...
    def _anthropic_messages_create(self, api_kwargs: dict, *, client: Any = None):
        ...
        return create_anthropic_message(
            ...
            # matching header families (x-ratelimit-* / x-sophos-credits-*).
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct Sophos Portal.
        """
        if base_url_host_matches(self._base_url_lower, "sophostechne.com"):
            return True
        ...
        if (
            ...
            try:
                from slick_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from slick_cli.models import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from slick_cli.models import ollama_model_supports_thinking
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # id rotation updated slick_logging._session_context (a
            ...
            # SLICK_SESSION_ID to the child id after an out-of-place
    ...
    def run_conversation(
        ...
        from slick_cli.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
        ...
        try:
            # Serialize the full load -> run -> flush region across Slick
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another Slick process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other Slick process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another Slick process on "
                    ...
                    timeout_msg = (
                        "⏳ Another Slick process kept this session busy too "
            ...
            if getattr(relay_turn, "relay_enabled", True):
                ...
                task_started = True
            # Publish the conversation id for ambient Sophos Portal tagging. Every
