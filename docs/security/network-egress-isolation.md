...
When running Slick inside Docker, the default `network_mode: host` gives the
...
The Slick [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   SLICK_UID=$(id -u) SLICK_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=slick,slick-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://slick-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Slick trust model and vulnerability reporting
