...
Slick gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Slick's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `slick.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `slick.platform.up/degraded` with bounded `error_code` attributes |
...
`slick.gateway.active_agents`, `slick.gateway.background_work`, and
`slick.gateway.background_delegations` are complementary. `active_agents`
...
```bash
slick monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'slick-agent[otlp]'`),
...
a plain `slick gateway run`: the exporter lives in the gateway process, so
...
`slick.gateway.up`, `slick.platform.up`, and `slick.platform.degraded`.
...
absent_over_time(slick_gateway_up[5m])

# Locally owned bridge is explicitly down.
slick_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
slick_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
slick_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
slick_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(slick_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `slick.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
slick.status = failed|unknown
slick.delivery_outcome = failed|not_configured
slick.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `slick.job_key`;
...
Slick Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/slick_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/slick_otel_capture.jsonl --wait 8
...
`slick.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
slick monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `slick monitoring` CLI intentionally exposes `status` only. This first
release covers only Slick Agent-owned service-health and operational-diagnostic
signals, including Slick Agent-owned Relay transport health. Team Gateway's
