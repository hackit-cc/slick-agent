...
Slick includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Slick and does not require a Slick observability plugin. Slick remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Slick execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `slick-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Slick `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `SLICK_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `SLICK_NEMO_RELAY_ATOF_*` and `SLICK_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Slick does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Slick requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
Slick installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Slick process so configured interceptors can
...
Collection remains off unless Slick policy enables it:
...
opt-in. Set `SLICK_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Slick does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, Slick
...
retain the same `session_id` and add `slick.session.segment` plus
`slick.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Slick
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Slick process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Slick logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this Slick process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Slick
process and share the same policy boundary.

Run profiles in separate Slick processes when they require different trust
...
Slick core owns one Relay host and one isolated Relay session scope per Slick
session. Core lifecycle producers use
`slick_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Slick plugin registration. Shared-metrics marks must
...
```text
Slick turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Slick shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Slick sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Slick used for the logical call, such as
...
New calls use `slick.model_route.count`. The previous
`slick.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `slick.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Slick
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Slick processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `slick.task_run`, parented to
the owning Slick session. The start counter contains only bounded execution
...
provider attempts for the same Slick API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Slick tool-call
...
cancellations. Active task ownership follows the task ID if Slick rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`slick.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Slick
...
tool or plugin names. Slick does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `slick.tool_approval` marks and
...
Successful skill mutations emit `slick.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `slick.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Slick derives reuse and patch-generation
...
```text
$SLICK_HOME/telemetry/shared_metrics/metrics.sqlite3
$SLICK_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Slick version,
...
schema field name, its current scope is one `SLICK_HOME`, so it is more
...
Deleting `$SLICK_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `SLICK_HOME`. To reset it, stop Slick
processes and remove `$SLICK_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Slick CLI turn against the deterministic local model server:
