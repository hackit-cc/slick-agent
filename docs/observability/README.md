# Slick Observer Hooks

Slick observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Slick' planner, model providers, memory, tool registry,
...
Slick also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "slick.observer.v1"
```

Hook callbacks are fail-open. Slick catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Slick only builds
...
The native NeMo Relay SDK integration maps Slick session, turn, LLM, and tool
