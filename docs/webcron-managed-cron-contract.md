# Webcron managed-cron — agent ↔ NAS wire contract

**Status:** authoritative wire spec for the Webcron cron provider.
**Audience:** the NAS-side implementer of the `agent-cron` endpoints
(`hackit-account-service`) and anyone debugging the managed-cron path.

Webcron lets a hosted Slick gateway **scale to zero** while idle and still
...
ChronosCronScheduler.reconcile()        ── agent computes next_run_at
  │  POST {portal}/api/agent-cron/provision   (auth: agent's Hackit access token)
  ▼
...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **Hackit Portal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `slick-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
> agent uses the **bootstrap-session access token** (`resolve_hackit_access_token`),
> minted under the bootstrap-only client `slick-cli-vps` and seeded into the
...
- **Auth:** `Authorization: Bearer <agent Hackit access token>`. NAS validates via
...
1. **Dashboard app** (`slick_cli/web_server.py`) — the agent's only public
...
`plugins/cron/webcron/verify.py`.

- **Auth:** `Authorization: Bearer <NAS-minted JWT>`. The agent verifies:
  - signature against the NAS JWKS (`cron.webcron.nas_jwks_url`),
  - `aud` == `cron.webcron.expected_audience` (this agent's
    `agent:{instance_id}`),
  - `iss` == `cron.webcron.portal_url`,
  - `exp` / `nbf` (30s leeway),
...
- **Multi-replica agents:** the store CAS makes the fire at-most-once across N
  gateway replicas sharing one `SLICK_HOME` — exactly one replica runs each
  fire.
...
All non-secret (`cron.webcron.*` in `config.yaml`); the agent holds no scheduler
credentials. For hosted agents NAS sets these at provision time:

| key | meaning |
|---|---|
| `cron.provider` | `"webcron"` to activate (empty = built-in ticker) |
| `cron.webcron.portal_url` | NAS base URL (also the expected JWT `iss`) |
| `cron.webcron.callback_url` | the agent's own public base URL for NAS→agent fires |
| `cron.webcron.expected_audience` | this agent's JWT `aud` (`agent:{instance_id}`) |
| `cron.webcron.nas_jwks_url` | NAS JWKS for verifying the fire JWT |

If `callback_url` / `portal_url` is blank or the agent has no Hackit login,
