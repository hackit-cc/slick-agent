...
This document is the formal interface between the **Slick gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`SophosTechne/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `slick gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`slick gateway enroll` → connector `/relay/enroll`, or
...
(`SophosTechne/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `slick` CLI.
