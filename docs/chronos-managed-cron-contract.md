...
(`sophos-account-service`) and anyone debugging the managed-cron path.

Chronos lets a hosted Slick gateway **scale to zero** while idle and still
...
ChronosCronScheduler.reconcile()        ── agent computes next_run_at
  │  POST {portal}/api/agent-cron/provision   (auth: agent's Sophos access token)
  ▼
...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **Sophos Portal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `slick-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
> agent uses the **bootstrap-session access token** (`resolve_sophos_access_token`),
...
- **Auth:** `Authorization: Bearer <agent Sophos access token>`. NAS validates via
...
If `callback_url` / `portal_url` is blank or the agent has no Sophos login,
