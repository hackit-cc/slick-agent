...
Slick supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.slick/config.yaml`:
...
Or set the env var: `SLICK_KANBAN_DISPATCH_IN_GATEWAY=false`
