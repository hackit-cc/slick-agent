...
The canonical transcript store is SQLite via `SessionDB` (from `slick_state`). The
...
Counts consecutive restarts via a JSON file (`{SLICK_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `slick update`, `slick gateway restart`,
or `/restart`.
