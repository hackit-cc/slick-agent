...
has to be thrown away or summarized. Slick has always done this in one batch:
...
Slick folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Slick's normal session flush is
...
With `micro_compact` unset or `false` Slick behaves exactly as it always has:
...
Defaults to `$SLICK_HOME/logs/agent.log`. It reports passes, outcome mix, net
