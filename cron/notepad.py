...
Write path is the CLI (``slick cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_slick_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`slick cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `slick cron notepad {job_id} delete "
