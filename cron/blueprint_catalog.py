...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``slick://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``slick://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"slick://blueprint/{quote(blueprint.key)}{qs}"
