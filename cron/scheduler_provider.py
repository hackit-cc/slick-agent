...
built-in) until an external provider (Webcron, Phase 4) shakes it out. Until
...
ticker. Alternative providers (e.g. Webcron, a NAS-mediated managed-cron
...
def _existing_profile_homes(profile_homes: list) -> list:
    ...
    the ticker runs — via ``slick profile delete``, the desktop's DELETE
    ...
    and in ``slick profile list`` (#47368). Filtering on directory existence
...
class CronScheduler(ABC):
    ...
    @abstractmethod
    def name(self) -> str:
        """Short identifier, e.g. 'builtin', 'webcron'."""
    ...
    def on_jobs_changed(self) -> None:
        ...
        Webcron re-provisions/cancels the affected one-shot via NAS).
...
def provider_supports_split_fire(provider: Any) -> bool:
    ...
    # provider composes with the inherited claim path) — e.g. Webcron keeps
...
def fire_overdue_jobs(
    ...
    *,
    ...
    (Webcron) deliver scheduled fires over HTTP to this process's api_server
    ...
    - **Routes through the provider's own two-phase fire path** — a
      ...
      of an agent run. Provider-specific re-arm logic (Webcron NAS
    ...
    from cron.jobs import _ensure_aware, _slick_now, is_job_runnable, load_jobs
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        # only the process-global SLICK_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `slick cron
        ...
        # Heartbeat once before the first sleep so `slick cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `slick cron status`/`list` (separate processes) can show
    ...
    def _start_multiplex(
        ...
        *,
        ...
        Each profile uses ``set_slick_home_override()`` + ``use_cron_store()``
        ...
        for entry in _existing_profile_homes(profile_homes):
            home = entry[1] if isinstance(entry, tuple) else entry
            home_token = set_slick_home_override(str(home))
            try:
            ...
            for entry in _existing_profile_homes(profile_homes):
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `slick cron status` can show WHY ticks fail
                        # (#68483).
                        if ok:
                            clear_ticker_error()
                        elif _tick_error:
