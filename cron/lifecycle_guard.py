...
``slick gateway restart`` (or ``launchctl kickstart ai.slick.gateway``
or ``systemctl restart slick-gateway``).  When the cron fires, the
...
``slick cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``slick gateway``, ``launchctl ... slick-gateway``,
``systemctl ... slick-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_SLICK_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``slick gateway
...
The profile-flag form (``slick -p <profile> gateway restart|stop``, #78028)
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `slick gateway` operations.
    # The destructive operations are restart, stop, and uninstall.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    # The lookbehind (#77173): `slick` must not be a path component or a
    # word tail. Excluding `/`, word chars, `.` and `-` keeps file paths
    # with embedded spaces (`/docs/slick gateway restart-notes.md`) from
    # matching via the `/slick` tail, while every real command position
    # (start of text, whitespace, `;`/`&`/`|`, `$(`, backtick, even a
    # U+FFFD from binary-content decoding) still matches.
    r"(?:(?<![/\w.\-])slick\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops on a slick-gateway label. macOS launchd
    # labels look like `ai.slick.gateway` / `slick-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated slick services (e.g.
    # `launchctl unload ai.slick.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.slick.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a slick-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bslick[.\-]?gateway)"
    # Branch D: pkill / kill targeting the slick gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.slick.gateway-... \` / `  -- ...`,
...
# selector — `slick -p <profile> gateway restart|stop` / `--profile <name>`
# / `--profile=<name>`. The selector token between `slick` and `gateway`
# breaks Branch A's literal adjacency. Unlike Branch A this form is NOT
# unconditionally self-targeting: issued from inside gateway `testbed`,
# `slick -p venus gateway restart` operates on a sibling profile's gateway
...
def _current_profile_name() -> Optional[str]:
    ...
    Prefers the explicit ``SLICK_PROFILE_NAME`` / ``SLICK_PROFILE`` env
    (set by the profile launcher and kanban worker spawns), falling back to
    ``slick_cli.profiles.get_active_profile_name`` (derived from
    ``SLICK_HOME``, which the gateway process inherits from its launch
    profile). Returns ``None`` when neither source yields a name.
    """
    for env_name in ("SLICK_PROFILE_NAME", "SLICK_PROFILE"):
...
# Branch B only catches `launchctl <verb> ... slick[.-]?gateway` when the
...
# command — `for item in 'ai.slick.gateway-apollo:...' 'ai.slick.gateway:...';
...
# is the unexpanded variable `$label`, not the string "slick.gateway". This
...
# EXISTING job, so anchoring to the slick-gateway label is still correct —
# `test_safe_commands` requires unrelated-label ops (e.g. `launchctl unload
# ai.slick.update-checker.plist`) to stay unblocked. The fix is checking
...
_SLICK_GATEWAY_LABEL_RE = re.compile(r"(?i)\bslick[.\-]?gateway\b")
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _SLICK_GATEWAY_LABEL_RE.search(normalized_text)
    )
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    a ``slick``/``gateway`` token) — this function is the single choke
    ...
    # a runbook line "a human can run: slick gateway restart" inside such a
    ...
    if _GATEWAY_LIFECYCLE_PATTERN.search(normalized):
        return True
    # Profile-flag form (#78028): `slick -p <profile> gateway restart|stop`
    # bypasses Branch A because the selector sits between `slick` and
    ...
    if profile_match:
        ...
        if named:
            # Profile ids cannot contain quotes (slick_cli.profiles
            # enforces `^[a-z0-9][a-z0-9_-]{0,63}$`), so a shell-quoted
            # `-p 'testbed'` compares equal to the bare name.
...
# for `systemctl restart slick-gateway` in syslog, a SQL LIKE literal over a
...
# blocking `grep -r 'systemctl restart slick-gateway' .` outright. Require a
...
# to execution (`grep 'systemctl restart slick-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.slick.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart slick-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart slick-gateway%'"
...
def _references_at(
    ...
    # (e.g. `Path.home() / ".slick"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<SLICK_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.slick/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_slick_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_slick_home() falls back to Path.home(), which raises when
        # neither SLICK_HOME nor HOME is resolvable (launchd/systemd
...
def check_gateway_lifecycle(
        ...
        if resolved_script is not None:
            ...
            if _is_cloud_placeholder_path(resolved_script) or _is_cloud_placeholder_path(
                ...
                raise GatewayLifecycleBlocked(
                    ...
                    "(e.g. ~/.slick/scripts/) and recreate the job."
                )
    ...
    if python_script:
        ...
        # `slick gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `slick gateway restart` from a shell outside "
