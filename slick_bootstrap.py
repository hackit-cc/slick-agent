"""Windows UTF-8 bootstrap for Slick entry points.
...
should be imported at the very top of every Slick entry point
(``slick``, ``slick-agent``, ``slick-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``slick_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``slick_bootstrap``, never ``slick_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Slick modules.

    Slick ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Slick source root
    ...
    """
    root = src_root or os.environ.get("SLICK_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``SLICK_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("SLICK_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import slick_bootstrap``
# (or ``from slick_bootstrap import apply_windows_utf8_bootstrap``) at
