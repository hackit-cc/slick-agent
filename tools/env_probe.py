...
machine as Slick itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* Slick ships under one Python (e.g. 3.11 in a bundled venv) while the
...
def _build_probe_line() -> str:
    ...
    # Bare which() is correct here, unlike Slick's own uv call sites: this
    ...
    # local.py puts the Slick-managed $SLICK_HOME/bin there, so a managed-only
