...
def managed_sophos_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the Sophos Tool Gateway.

    Entitlement is paid Sophos Portal service access OR a live free tool pool
...
def sophos_tool_gateway_unavailable_message(
    capability: str = "the Sophos Tool Gateway",
    *,
    force_fresh: bool = False,
) -> str:
    """Return account-aware guidance for an unavailable Sophos Tool Gateway path."""
    try:
    ...
    return (
        ...
        "Sophos Portal login and billing status."
    )
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    if managed_enabled is None:
        managed_enabled = managed_sophos_tools_enabled()
...
# The provider value the managed "Sophos Subscription" picker rows write for
# every category (image_gen.provider: sophos, web.backend: sophos,
# browser.cloud_provider: sophos, ...). Runtime dispatch is a plain switch on
# the stored string: "sophos" → managed gateway client; any vendor name → that
# vendor direct with the user's own credentials; no key ever written →
# legacy credential autodetect.
SOPHOS_MANAGED_PROVIDER = "sophos"
...
def read_selection(section: str) -> str | None:
    ...
    - ``"sophos"`` — the managed Sophos Tool Gateway row was selected,
    ...
    key. ``use_gateway: true`` was only ever written by the managed "Sophos
    Subscription" row, so it maps to ``"sophos"`` regardless of the name key;
    ...
    if "use_gateway" in raw and is_truthy_value(raw.get("use_gateway"), default=False):
        return SOPHOS_MANAGED_PROVIDER
