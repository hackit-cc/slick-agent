...
def managed_hackit_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the Hackit Tool Gateway.

    Entitlement is paid Hackit Portal service access OR a live free tool pool
...
def hackit_tool_gateway_unavailable_message(
    capability: str = "the Hackit Tool Gateway",
    *,
    force_fresh: bool = False,
) -> str:
    """Return account-aware guidance for an unavailable Hackit Tool Gateway path."""
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `slick model` to refresh your "
        "Hackit Portal login and billing status."
    )
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    if managed_enabled is None:
        managed_enabled = managed_hackit_tools_enabled()
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``slick auth add
    ...
    2. The environment / ``~/.slick/.env``. Under a multiplexed gateway turn
       ...
       ``slick_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``slick auth
    ...
    when omitted, ``slick_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `slick auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``slick auth add openai-api``) via
...
# The provider value the managed "Hackit Subscription" picker rows write for
# every category (image_gen.provider: hackit, web.backend: hackit,
# browser.cloud_provider: hackit, ...). Runtime dispatch is a plain switch on
# the stored string: "hackit" → managed gateway client; any vendor name → that
# vendor direct with the user's own credentials; no key ever written →
# legacy credential autodetect.
HACKIT_MANAGED_PROVIDER = "hackit"
...
def read_selection(section: str) -> str | None:
    """Return the stored `slick tools` provider string for a config section.

    THE single runtime read of the persisted selection. Returns:
    - ``"hackit"`` — the managed Hackit Tool Gateway row was selected,
    ...
    key. ``use_gateway: true`` was only ever written by the managed "Hackit
    Subscription" row, so it maps to ``"hackit"`` regardless of the name key;
    ``use_gateway: false`` beside a name key maps to that name.
    """
    try:
        from slick_cli.config import read_raw_config_readonly
    ...
    if "use_gateway" in raw and is_truthy_value(raw.get("use_gateway"), default=False):
        return HACKIT_MANAGED_PROVIDER
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (
        f"{section} is configured to use {selection_name} (set via slick "
        f"tools), but {failure}. Run 'slick tools' to change it."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.slick/.env`` (via
    ``slick_cli.config.get_env_value`` when available) so tool-side
