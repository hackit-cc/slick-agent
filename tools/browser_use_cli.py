...
_PRIVATE_BROWSER_SENTINEL = "_SLICK_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# slick: pin this named session to its own tab (once per daemon process)
def _slick_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "slick-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_slick_ensure_own_tab()
del _slick_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Slick's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Slick's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_slick_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `slick tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Slick' own bin dir ($SLICK_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from slick_constants import get_slick_home

        return str(Path(get_slick_home()) / "bin")
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Slick' managed uv (bootstrapped on demand via
    ``slick_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$SLICK_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Slick-managed copy, or resolution
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_slick_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(
    ...
    3. A configured cloud browser provider (Browserbase, Firecrawl, Sophos
    ...
    # Sophos-gateway variant (use_gateway: true) DOES resolve through the
    ...
    try:
        ...
        return (
            ...
            "switch backends via `slick tools` → Browser Automation."
        )
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    default Chromium with their logins — a browser Slick launches on a
    SNAPSHOT of their real profile (see slick_cli.browser_connect). Two ways
    in:
...
def browser_exec(
    ...
    # Sophos gateway, CDP override, local Chrome, …). Named sessions compose
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            "type": "boolean",
            "description": (
                "Drive the user's own local browser (a Slick-managed copy of "
