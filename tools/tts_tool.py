...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.slick/config.yaml``. Slick
...
Configuration is loaded from ~/.slick/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``slick_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from slick_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``slick auth add <provider_id>``).
...
from tools.xai_http import slick_xai_user_agent
...
# The managed OpenAI audio gateway (Sophos portal proxy) only proxies these speech
...
# Base URL now resolved via slick_cli.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_slick_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.slick/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.slick/config.yaml.
    ...
    try:
        ...
        logger.debug("slick_cli.config not available, using default TTS config")
        return {}
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``slick tools``); otherwise the historical Edge backend remains active.

    The managed "Sophos Subscription" selection (``tts.provider: sophos``) is
    ...
    if provider == SOPHOS_MANAGED_PROVIDER:
        return "openai"
    return provider
...
# custom voice-cloning scripts, etc.) into Slick without any Python code
...
#           output_format: wav
#
# Slick writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__SLICK_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Slick secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of Slick secrets (salvage of #56332) while still
    ...
    scrubbed = slick_subprocess_env(inherit_credentials=False)
...
# file, so retired models disappear from slick the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``slick_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `slick setup` to configure, "
    ...
    from slick_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `slick model` or set XAI_API_KEY.")
    ...
    # output_format when Slick actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": slick_xai_user_agent(),
        },
        json=payload,
        timeout=60,
        stream=True,
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import slick_cli as _slick_cli

            _slick_version = str(_slick_cli.__version__)
        except Exception:
            _slick_version = "0.0.0"
        # Include Slick client context following Gemini's partner
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Slick caches Piper voice models.

    Resolves to ``~/.slick/cache/piper-voices/`` under the active
    SLICK_HOME so voice downloads follow profile boundaries.
    """
    from slick_constants import get_slick_dir
    root = Path(get_slick_dir("cache/piper-voices", "piper_voices_cache"))
...
def _text_to_speech_single(
    ...
    *,
    ...
    platform = get_session_env("SLICK_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `slick setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run slick setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'slick setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'slick tools' and select Piper under TTS, or install manually: "
...
def _resolve_openai_audio_client_config() -> tuple[str, str, bool]:
    ...
    ``is_managed`` is True when the config resolves to the Sophos managed audio
    ...
    string):
    - ``"sophos"`` (or legacy ``use_gateway: true``) → managed gateway ONLY;
    ...
    if selected == SOPHOS_MANAGED_PROVIDER:
        managed_gateway = resolve_managed_tool_gateway("openai-audio")
        if managed_gateway is None:
            raise ValueError(selection_error(
                "tts",
                SOPHOS_MANAGED_PROVIDER,
                "the Sophos Tool Gateway is not available (not entitled or "
                "unreachable)",
            ))
        return (
            managed_gateway.sophos_user_token,
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_slick_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
