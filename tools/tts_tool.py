...
# The managed OpenAI audio gateway (Hackit portal proxy) only proxies these speech
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    The managed "Hackit Subscription" selection (``tts.provider: hackit``) is
    ...
    if provider == HACKIT_MANAGED_PROVIDER:
        return "openai"
    return provider
...
def _resolve_openai_audio_client_config() -> tuple[str, str, bool]:
    ...
    ``is_managed`` is True when the config resolves to the Hackit managed audio
    ...
    string):
    - ``"hackit"`` (or legacy ``use_gateway: true``) → managed gateway ONLY;
    ...
    if selected == HACKIT_MANAGED_PROVIDER:
        managed_gateway = resolve_managed_tool_gateway("openai-audio")
        if managed_gateway is None:
            raise ValueError(selection_error(
                "tts",
                HACKIT_MANAGED_PROVIDER,
                "the Hackit Tool Gateway is not available (not entitled or "
                "unreachable)",
            ))
        return (
            managed_gateway.hackit_user_token,
