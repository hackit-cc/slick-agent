...
def _resolve_stt_client_config() -> Dict[str, Any]:
    ...
    if provider == "openai":
        # Handles the Hackit-managed selection too: the resolver returns the
...
def _resolve_tts_client_config() -> Dict[str, Any]:
    ...
    if provider == "openai":
        # Covers the direct-key, custom-base_url, and Hackit-managed selections.
        try:
...
def resolve_client_voice_config() -> Dict[str, Any]:
    ...
    Callers scope the profile via ``slick_constants.set_slick_home_override``
