...
teammate agent (another Slick profile on this install, or an agent on a
...
DM each other via a prompt-injected ``slick -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``slick -p`` / ``slick peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``slick -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``slick peer dm <peer>[/<name>] < <tmp>``
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'slick' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('slick' → 'default')."""
    want = target.strip()
    if not want:
        return None
    if want.lower() == "slick":
...
def cleanup_bot_dm_cache(
    ...
    locations: list[tuple[Path, str]] = [
        (temp_root, "slick-dm-*.txt"),
        (temp_root, "slick-relay-dm-*.txt"),
    ]
    try:
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    Local deliveries (``slick -p <profile> chat …``) collide with relay
    ...
    # resolves the venv-relative slick next to this gateway's interpreter
    ...
    if (
        stdin_file
        or len(argv) < 3
        or cli not in ("slick", "slick.exe")
        or argv[1] != "-p"
    ):
        return contextlib.nullcontext()
    from tools.bot_relay import acquire_turn_lock

    home = Path(os.getenv("SLICK_HOME") or os.path.expanduser("~/.slick"))
    return acquire_turn_lock(_slick_root(home), argv[2])
...
def _agent_home(agent: Any) -> str:
    ...
    try:
        ...
        pass
    return os.getenv("SLICK_HOME") or os.path.expanduser("~/.slick")
