...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("SLICK_YOLO_MODE", ""))
...
# Slick session id (observability identity, distinct from the gateway
...
# os.environ["SLICK_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export SLICK_INTERACTIVE.
_slick_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "slick_interactive",
    default=None,
)


def set_slick_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["SLICK_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``SLICK_INTERACTIVE`` env var for legacy callers.
    """
    return _slick_interactive_ctx.set("1" if interactive else "")


def reset_slick_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_slick_interactive_context`."""
    _slick_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``SLICK_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _slick_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("SLICK_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Slick session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("SLICK_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("SLICK_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("SLICK_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("SLICK_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("SLICK_CRON_SESSION")


def _is_single_query_approval_context() -> bool:
    """True when the current approval decision is from a single-query (-q) session.

    ``slick chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``SLICK_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("SLICK_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("SLICK_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set SLICK_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind SLICK_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds SLICK_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("SLICK_GATEWAY_SESSION"):
        return True
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``SLICK_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $SLICK_HOME, or by the resolved absolute
# active profile home path such as /home/slick/.slick/config.yaml. The
# resolved-absolute form is folded into the ~/.slick/ patterns at detection
...
# go stale when SLICK_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_SLICK_ENV_PATH = (
    r'(?:~\/\.slick/|'
    r'(?:\$home|\$\{home\})/\.slick/|'
    r'(?:\$slick_home|\$\{slick_home\})/)'
    r'\.env\b'
)
# ~/.slick/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _SLICK_ENV_PATH; matches the SLICK_HOME override form as
# well as ~/.slick/.
_SLICK_CONFIG_PATH = (
    r'(?:~\/\.slick/|'
    r'(?:\$home|\$\{home\})/\.slick/|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_slick_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Slick: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Slick
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]slick[^\n]*\.env\b', "access to Slick secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `slick` and `gateway` (e.g. `slick -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bslick\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart slick gateway (kills running agents)"),
    (r'\bslick\s+update\b', "slick update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `slick gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `slick ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart slick-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart slick-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(slick|gateway|cli\.py)\b', "kill slick/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill slick` but not
    # `kill -9 $(pgrep -f slick)` because the substitution is opaque
    ...
    # the `slick gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.slick.gateway`).
    # Catch the operations that stop, restart, or unload it.
    #
    # Order-independent (2026-08-02 incident): the previous version required
    # "slick"/"ai.slick" to appear AFTER the launchctl verb in the same
    ...
    # 'ai.slick.gateway-apollo:...' ...; do label=${item%%:*}; launchctl
    # bootout "$label"; done` — never has the literal text "slick" appear
    ...
    # an unrelated "slick" mention now also matches) — for an approval gate
    # that's the correct direction to err: an extra approval prompt is
    # cheap, a missed one took down the whole gateway fleet.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:slick|ai\.slick)\b)', "stop/restart slick launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Slick file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.slick/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Slick-managed security file (~/.slick/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_SLICK_CONFIG_PATH}|{_SLICK_ENV_PATH})', "in-place edit of Slick config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_SLICK_CONFIG_PATH}|{_SLICK_ENV_PATH})', "in-place edit of Slick config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_SLICK_CONFIG_PATH}|{_SLICK_ENV_PATH})', "in-place edit of Slick config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.slick/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / SLICK_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Slick home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\slick), so folding the user home
    # first would eat the prefix the Slick-home fold needs.
    command = _rewrite_resolved_slick_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / SLICK_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.slick``); the matched
...
def _rewrite_resolved_slick_home(command: str) -> str:
    """Rewrite the resolved absolute Slick home prefix to ``~/.slick/``.

    Resolves the active ``SLICK_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.slick/`` so the static ``_SLICK_CONFIG_PATH`` / ``_SLICK_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/slick/.slick/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$SLICK_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from slick_constants import get_slick_home
        home = get_slick_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.slick")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``slick send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Slick ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"slick-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None


_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = (
    "stop/restart slick gateway via shell-spliced verb (kills running agents)"
)
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    ``launchctl kick"start" -k gui/501/ai.slick.gateway`` auto-approved
    ...
    anchored on a slick-gateway identifier — reusing its prose
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Slick YOLO.
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["SLICK_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "SLICK_SPINNER_PAUSE" in os.environ:
            del os.environ["SLICK_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Slick approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``SLICK_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            SLICK_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export SLICK_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set SLICK_INTERACTIVE or "
                "SLICK_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set SLICK_INTERACTIVE or "
            "SLICK_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("SLICK_EXEC_ASK"):
        ...
        # can see (SLICK_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``SLICK_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from slick_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from slick_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("SLICK_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # SLICK_EXEC_ASK routes through the gateway decision loop (no human
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from slick_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (SLICK_EXEC_ASK) still takes this path even when INTERACTIVE
    ...
    if notify_cb is None:
        # SLICK_EXEC_ASK (and sometimes a session platform marker) can leak
