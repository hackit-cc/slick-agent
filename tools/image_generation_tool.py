...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/nano-banana-pro": {
        ...
        "defaults": {
            ...
            # Users on Sophos Subscription should stay at 1K for predictable billing.
            "resolution": "1K",
        },
        "supports": {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps Sophos
...
# ---------------------------------------------------------------------------
# Managed FAL gateway (Sophos Subscription)
# ---------------------------------------------------------------------------
def _resolve_managed_fal_gateway():
    ...
    - ``"sophos"`` (or legacy ``use_gateway: true``) → managed fal-queue
...
def _get_managed_fal_client(managed_gateway):
    ...
    with _managed_fal_client_lock:
        ...
        _managed_fal_client = _ManagedFalSyncClient(
            fal_client,
            key=managed_gateway.sophos_user_token,
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                f"Sophos Subscription gateway rejected model '{model}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the Sophos Portal's FAL proxy. Either:\n"
...
def _build_no_backend_setup_message() -> str:
    ...
    Used by the in-tree FAL path. Mentions:
      - FAL_KEY signup link
      - managed-gateway status (if Sophos tools are enabled)
    ...
    if managed_sophos_tools_enabled():
        ...
        gateway_message = sophos_tool_gateway_unavailable_message(
            "managed FAL image generation",
        )
        if gateway_message:
    ...
    if managed_sophos_tools_enabled():
        lines.append(
            "  2. Sign in to a Sophos account that has the managed FAL "
...
def check_image_generation_requirements() -> bool:
    ...
    if not configured or configured in ("fal", SOPHOS_MANAGED_PROVIDER):
        return False
...
def _dispatch_to_plugin_provider(
    ...
    configured = _read_configured_image_provider()
    if not configured or configured in ("fal", SOPHOS_MANAGED_PROVIDER):
        # Unset/explicit FAL keeps the legacy FAL path; "sophos" (managed
        # Sophos Subscription selection) also runs the legacy pipeline, which
...
def _maybe_route_managed_krea(
    ...
    # (other than the managed "sophos" selection, which IS a managed-mode
    ...
    if configured_provider is not None and configured_provider != SOPHOS_MANAGED_PROVIDER:
        return None
