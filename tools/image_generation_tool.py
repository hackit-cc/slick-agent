...
selectable via ``slick tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/nano-banana-pro": {
        ...
        "defaults": {
            ...
            # Users on Hackit Subscription should stay at 1K for predictable billing.
            "resolution": "1K",
        },
        "supports": {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps Hackit
...
# ---------------------------------------------------------------------------
# Managed FAL gateway (Hackit Subscription)
# ---------------------------------------------------------------------------
def _resolve_managed_fal_gateway():
    """Resolve the FAL route from the stored `slick tools` selection.

    Dispatch is a plain switch on the stored ``image_gen`` provider string:
    - ``"hackit"`` (or legacy ``use_gateway: true``) → managed fal-queue
...
def _get_managed_fal_client(managed_gateway):
    ...
    with _managed_fal_client_lock:
        ...
        _managed_fal_client = _ManagedFalSyncClient(
            fal_client,
            key=managed_gateway.hackit_user_token,
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                f"Hackit Subscription gateway rejected model '{model}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the Hackit Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `slick tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.slick"
    ...
    # Slick cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `slick tools` → Image Generation."
            )
...
def check_fal_api_key() -> bool:
    """True if the FAL backend selected via `slick tools` (or, on a
...
def _build_no_backend_setup_message() -> str:
    ...
    Used by the in-tree FAL path. Mentions:
      - FAL_KEY signup link
      - managed-gateway status (if Hackit tools are enabled)
    ...
    if managed_hackit_tools_enabled():
        ...
        gateway_message = hackit_tool_gateway_unavailable_message(
            "managed FAL image generation",
        )
        if gateway_message:
    ...
    if managed_hackit_tools_enabled():
        lines.append(
            "  2. Sign in to a Hackit account that has the managed FAL "
            "gateway enabled (`slick setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `slick tools` "
        "→ Image Generation (run `slick plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    if not configured or configured in ("fal", HACKIT_MANAGED_PROVIDER):
        return False
...
def _dispatch_to_plugin_provider(
    ...
    configured = _read_configured_image_provider()
    if not configured or configured in ("fal", HACKIT_MANAGED_PROVIDER):
        # Unset/explicit FAL keeps the legacy FAL path; "hackit" (managed
        # Hackit Subscription selection) also runs the legacy pipeline, which
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `slick plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `slick tools` → Image Generation."
                ),
...
def _maybe_route_managed_krea(
    ...
    # (other than the managed "hackit" selection, which IS a managed-mode
    ...
    if configured_provider is not None and configured_provider != HACKIT_MANAGED_PROVIDER:
        return None
...
# model/provider via `slick tools` or `/skills`.
