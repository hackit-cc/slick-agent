...
`slick skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.slick.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.slick.blueprint`` is a mapping containing
    ...
    slick = meta.get("slick") if isinstance(meta, dict) else None
    blueprint = slick.get("blueprint") if isinstance(slick, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.slick.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.slick.blueprint`` block) they can hand
    to ``slick skills publish`` to share. ``body`` is the plain-language
