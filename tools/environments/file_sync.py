...
_FORCE_SYNC_ENV = "SLICK_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.slick") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.slick to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``SLICK_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, slick_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.slick/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(slick_home=slick_home)

    def _sync_back_transaction(self, slick_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="slick-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.slick/skills/a.md`` →
        ``~/.slick/skills/a.md``, a new remote file at
        ``/root/.slick/skills/b.md`` maps to ``~/.slick/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
