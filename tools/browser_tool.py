...
supports multiple backends — **Browser Use** (cloud, default for Sophos
...
from slick_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Slick secret
...
def _build_browser_env() -> dict:
    ...
    Strips Slick-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``slick_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = slick_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    slick_node_bin = str(slick_home / "node" / "bin")
    slick_node_root = str(slick_home / "node")
    slick_nm_bin = str(slick_home / "node_modules" / ".bin")
    return [slick_node_bin, slick_node_root, slick_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``slick doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.slick/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Slick profile."""
...
def _resolve_cloud_provider_uncached() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed Sophos gateway or
    ...
    browser plugins (``~/.slick/plugins/browser/<vendor>/``) participate
    ...
    try:
        ...
        if isinstance(browser_cfg, dict) and "cloud_provider" in browser_cfg:
            ...
            if provider_key in ("local", "camofox"):
                ...
                _cloud_provider_resolved = True
                return None
            if provider_key == "sophos":
                # Managed "Sophos Subscription" selection is serviced by the
    ...
    if resolved is None and provider_key is None:
        ...
        # was ever written: Browser Use first (managed Sophos gateway or
...
from slick_constants import is_termux as _is_termux_environment
...
# comparison sites (four here, plus slick_cli/tools_config.py and
# slick_cli/doctor.py) from drifting if the sentinel's exact spelling ever
# changes.
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Slick silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Slick-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Slick CLI where sys.stdout and sys.stderr both report
...
_REAL_PROFILE_SESSION = "slick-real-profile"
...
def _cdp_http_ready(http_cdp: str) -> bool:
    ...
    try:
        from slick_cli.browser_connect import is_browser_debug_ready
...
def _real_profile_cdp() -> tuple:
    ...
    Snapshots the user's default-Chromium profile into a slick-owned copy
    ...
    if not _use_real_profile():
        ...
        try:
            from slick_cli.browser_connect import cleanup_real_profile_snapshots
    ...
    with _real_profile_cdp_lock:
        ...
        # from a previous slick process; if it is driving OUR copy dir, hand it
        ...
        if err or not snap_dir:
            from slick_cli.browser_connect import _PROFILE_LOCKED_PREFIX
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-slick_…`` the resulting socket path exceeds the
...
# run exactly once, before the cleanup loop started, which meant a slick
...
# Hard ceiling for a daemon whose owning slick process is still alive but
...
def _emergency_cleanup_all_sessions():
    ...
    crashed slick processes — this way every clean slick exit sweeps
    ...
    # Sweep orphans from other crashed slick processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live slick processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current slick PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live slick process
...
def _socket_dir_idle_seconds(socket_dir: str) -> Optional[float]:
    ...
    slick restarts and does not depend on in-memory bookkeeping surviving an
    exception path.
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning slick process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced slick PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent slick instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-slick_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # live slick process and must not be touched — but "owner alive"
...
def _create_local_session(task_id: str, allow_real_profile: bool = True) -> Dict[str, str]:
    ...
    # browser running on a slick-owned SNAPSHOT of their real profile —
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Slick-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Slick-managed one with no recovery — every candidate
    is therefore validated with ``node_tool_runnable`` (the same check
    ``find_slick_node_executable`` uses to self-heal a managed Node tree)
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Slick-managed
    ...
    # node_modules binary, which disappears on the next ``slick update`` and
    ...
    # Build an extended search PATH including Slick-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_slick_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_slick_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_slick_home() / "node")),
            ]
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``slick update`` (or ``slick doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential Slick holds, while running registry-fetched
    npm code on every ``slick update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `slick update`/`doctor
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this slick PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_slick_dir("cache/web", "web_cache")
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = slick_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Slick
    ...
    from slick_constants import get_slick_dir
    screenshots_dir = get_slick_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/sophostechne/slick-agent:latest")
                else:
