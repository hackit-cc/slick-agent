...
Trust levels:
  - builtin:   Ships with Slick. Never scanned, always trusted.
...
_SLICK_CONFIG_FILES = r'\.slick/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.slick/\.env|\~/\.slick/\.env',
     "slick_env_access", "critical", "exfiltration",
     "directly references Slick secrets file"),
    ...
    #     For Slick/other config files it is high (caution) — legitimate
    ...
    (_prose_modify_re(_SLICK_CONFIG_FILES),
     "slick_config_mod", "high", "persistence",
     "modification language aimed at Slick configuration files (verify intent)"),
    (_shell_write_re(_SLICK_CONFIG_FILES),
     "slick_config_mod_shell", "critical", "persistence",
     "shell write (redirect/sed -i/tee/cp/mv) targeting Slick configuration files"),
    (r'\.slick/config\.yaml|\.slick/SOUL\.md',
     "slick_config_ref", "low", "persistence",
     "references Slick configuration files (informational; only modification intent is scored)"),
...
# scan. `.skillignore` is the Slick-native name; `.clawhubignore` is honored
