...
provider** — turn on a backend by enabling a plugin (``slick plugins
enable video_gen/<name>``) and selecting it in ``slick tools`` → Video
Generation.
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `slick tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `slick tools` → "
...
# `slick tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`slick tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `slick tools` → Video Generation."
        )
        return {
            "description": "\n".join(parts),
            "parameters": {
