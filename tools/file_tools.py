...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``slick_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from slick_constants import get_subprocess_home
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where slick previously hard-rejected an oversized read
...
def _terminal_env_type_for_task(task_id: str = "default") -> str:
    ...
    try:
        ...
        if env is not None:
            ...
            if "daytona" in name:
                return "daytona"
            stamped = getattr(env, "_slick_backend_name", None)
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Slick.
...
_slick_config_resolved: str | None = None
_slick_config_resolved_loaded = False


def _get_slick_config_resolved() -> str | None:
    """Return the resolved absolute path of the Slick config file (cached)."""
    global _slick_config_resolved, _slick_config_resolved_loaded
    if _slick_config_resolved_loaded:
        return _slick_config_resolved
    _slick_config_resolved_loaded = True
    try:
        from slick_cli.config import get_config_path
        _slick_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _slick_config_resolved = str(Path(_expand_tilde("~/.slick/config.yaml")).resolve())
        except Exception:
            _slick_config_resolved = None
    return _slick_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Slick config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    slick_config = _get_slick_config_resolved()
    if slick_config and (resolved == slick_config or normalized == slick_config):
        return (
            f"Refusing to write to Slick config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.slick/config.yaml directly or use 'slick config' instead."
        )
    return None
...
# .cursorrules (or a project-local .slick config tree) outlives the current
...
_real_slick_home_cached: str | None = None
_real_slick_home_loaded = False


def _get_real_slick_home() -> str | None:
    """Return the realpath of the authoritative Slick home (cached)."""
    global _real_slick_home_cached, _real_slick_home_loaded
    if _real_slick_home_loaded:
        return _real_slick_home_cached
    _real_slick_home_loaded = True
    try:
        from slick_constants import get_slick_home
        _real_slick_home_cached = os.path.realpath(str(get_slick_home()))
    except Exception:
        try:
            _real_slick_home_cached = os.path.realpath(_expand_tilde("~/.slick"))
        except Exception:
            _real_slick_home_cached = None
    return _real_slick_home_cached
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    """
    try:
        from slick_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.slick home is governed by its own guards
    ...
    # ``.slick`` component rule below, which would otherwise match the
    # home directory itself.
    real_home = _get_real_slick_home()
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .slick config dirs (e.g. <repo>/.slick/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.slick`` — matching
        # any ancestor named .slick would gate every write inside a
        # checkout that happens to live under ~/.slick (e.g. the
        # slick-agent repo itself at ~/.slick/slick-agent).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".slick":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Slick mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    container's sandbox mirror of Slick state.

    Two detectors (both #32049): these catch writes that would be
    SILENTLY LOST — the host Slick process never reads the mirror, so
    ...
    Returns ``None`` when the write is in-scope or outside Slick scope.
    """
    try:
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Slick internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under SLICK_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == SLICK_HOME (e.g. "auth.json") still hits the
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with slick_tools.write_file() for very "
            "large files."
        )
