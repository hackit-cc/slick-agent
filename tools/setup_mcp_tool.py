...
``slick mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Slick desktop app. Use the "
            "terminal instead: `slick mcp install <name>` for catalog entries, "
            "`slick mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Slick desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `slick mcp catalog` "
