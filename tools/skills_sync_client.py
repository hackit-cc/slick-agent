...
**Hackit admin**. We read that off the access token, which rides on the same
bearer ``resolve_hackit_runtime_credentials()`` returns; we decode the JWT
...
status conflates "may administer Hackit" with "has Skill Sync enabled", and has
...
#
# We reuse resolve_hackit_runtime_credentials() for the bearer (it honors the
...
def _decode_jwt_payload_unverified(token: str) -> Dict[str, Any]:
    ...
    plugins/dashboard_auth/hackit/__init__.py:463.
    """
    try:
...
def resolve_identity() -> Dict[str, Any]:
    """Resolve the Hackit bearer + owner + dev-gate flag.

    Returns a dict: ``{api_key, base_url, owner, hackit_admin, claims}``.
    ...
    try:
        ...
        raise SyncInertError(f"no Hackit credentials: {e}") from e
    ...
    hackit_admin = claims.get(HACKIT_ADMIN_CLAIM) is True
    return {
        "api_key": api_key,
        "base_url": (creds or {}).get("base_url"),
        "owner": str(owner),
        "hackit_admin": hackit_admin,
        "claims": claims,
    }
...
def dev_gate_open() -> bool:
    ...
    try:
        return bool(resolve_identity().get("hackit_admin"))
...
DEFAULT_SYNC_BASE_URL = "https://gateway-gateway.hackit.cc"
...
def sync_feature_enabled() -> bool:
    ...
    the Hackit-admin token gate and a configured base URL — all three must hold for
...
# the caller's three-way merge. Auth is the Hackit bearer resolved above.
# ---------------------------------------------------------------------------
...
# (no push, no pull, no-op) unless the signed-in user is a Hackit admin.
# ---------------------------------------------------------------------------
...
def maybe_push_skills(*, message: str = "slick skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not identity.get("hackit_admin"):
            return None  # access gate: inert unless the user is a Hackit admin
...
def sync_status() -> Dict[str, Any]:
    ...
    try:
        ...
        status["hackit_admin"] = bool(identity.get("hackit_admin"))
    except SyncInertError:
        pass
    except Exception as e:
