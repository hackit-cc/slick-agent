...
409), and is driven by:
  ...
  * the ``slick sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``slick_cli/``) so the
...
**Sophos admin**. We read that off the access token, which rides on the same
bearer ``resolve_sophos_runtime_credentials()`` returns; we decode the JWT
...
status conflates "may administer Sophos" with "has Skill Sync enabled", and has
...
is toggled via ``slick sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.slick/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``slick sync enable`` — it is
...
# namespace from slick-agent's local ``content_hash`` (skills_guard.py:846),
...
#
# We reuse resolve_sophos_runtime_credentials() for the bearer (it honors the
...
def _decode_jwt_payload_unverified(token: str) -> Dict[str, Any]:
    ...
    plugins/dashboard_auth/sophos/__init__.py:463.
    """
    try:
...
def resolve_identity() -> Dict[str, Any]:
    """Resolve the Sophos bearer + owner + dev-gate flag.

    Returns a dict: ``{api_key, base_url, owner, sophos_admin, claims}``.
    ...
    try:
        ...
        raise SyncInertError(f"no Sophos credentials: {e}") from e
    ...
    sophos_admin = claims.get(SOPHOS_ADMIN_CLAIM) is True
    return {
        "api_key": api_key,
        "base_url": (creds or {}).get("base_url"),
        "owner": str(owner),
        "sophos_admin": sophos_admin,
        "claims": claims,
    }
...
def dev_gate_open() -> bool:
    ...
    try:
        return bool(resolve_identity().get("sophos_admin"))
...
# sync.base_url (or the SLICK_SYNC_BASE_URL bridge env) overrides it to point
...
DEFAULT_SYNC_BASE_URL = "https://gateway-gateway.sophostechne.com"

def resolve_sync_base_url() -> Optional[str]:
    """Resolve the sync-plane base URL.

    Order: SLICK_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("SLICK_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Slick Cloud instance can be set
...
# precedence as base_url: the SLICK_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   SLICK_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   SLICK_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   SLICK_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``SLICK_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a Slick Cloud deployment sets to opt its instances into sync by
    default. It is checked by the gate-and-swallow entrypoints IN ADDITION to
    the Sophos-admin token gate and a configured base URL — all three must hold for
    background sync to run.
    """
    return _sync_config_bool("SLICK_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``SLICK_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``slick sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "SLICK_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``SLICK_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``slick sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Slick Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("SLICK_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.slick/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.slick/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Slick Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``SLICK_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.slick/skills/ containing
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``slick sync device --name`` to set an explicit
    label."""
    ...
    # Slick Cloud (and any templated deployment) can seed the label
    # declaratively via SLICK_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `slick sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("SLICK_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.slick/skills/.sync_device_id, overwriting
...
# the caller's three-way merge. Auth is the Sophos bearer resolved above.
# ---------------------------------------------------------------------------
...
# ~/.slick/skills/.sync_state as JSON.
#
...
# path under ~/.slick/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.slick/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Slick speaks "
            f"{WIRE_VERSION} — update Slick to sync with it"
        )
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (slick sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.slick/skills/. Only paths the user has opted into (``sync: true``) are
...
# (no push, no pull, no-op) unless the signed-in user is a Sophos admin.
# ---------------------------------------------------------------------------

def maybe_push_skills(*, message: str = "slick skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not identity.get("sophos_admin"):
            return None  # access gate: inert unless the user is a Sophos admin
        if not sync_feature_enabled():
            return None  # feature off for this instance (SLICK_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``slick sync status``. Never raises."""
    status: Dict[str, Any] = {
        "sophos_admin": False,
        "logged_in": False,
    ...
    try:
        ...
        status["sophos_admin"] = bool(identity.get("sophos_admin"))
    except SyncInertError:
        pass
    except Exception as e:
...
#
# Org skills live under a DISTINCT local namespace, ~/.slick/skills/_org/
...
#
# `slick sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.slick/skills/_org/<org_id>/``.
