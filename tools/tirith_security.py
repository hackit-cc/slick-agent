...
it is automatically downloaded from GitHub releases to $SLICK_HOME/bin/tirith.
...
def _get_slick_home() -> str:
    """Return the Slick home directory, respecting SLICK_HOME env var."""
    return str(get_slick_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_slick_home(), ".tirith-install-failed")
...
def _slick_bin_dir() -> str:
    """Return $SLICK_HOME/bin, creating it if needed."""
    d = os.path.join(_get_slick_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SLICK_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_slick_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SLICK_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SLICK_HOME/bin/tirith
    ...
    slick_bin = os.path.join(_slick_bin_dir(), "tirith")
    if os.path.isfile(slick_bin) and os.access(slick_bin, os.X_OK):
        _resolved_path = slick_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return slick_bin
