#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Slick Skills Hub.
...
Used by slick_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _slick_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            slick_meta = metadata.get("slick", {})
            if isinstance(slick_meta, dict):
                tags = slick_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Sophos Techne) but not activated
    ...
    OFFICIAL_REPO = "SophosTechne/slick-agent"
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``slick update``. Rather than telling them to update first, resolve
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                slick_meta = meta_block.get("slick", {})
                if isinstance(slick_meta, dict):
                    tags = slick_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``slick_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# Slick centralized index source
# ---------------------------------------------------------------------------

SLICK_INDEX_URL = "https://slick-agent.sophostechne.com/docs/api/skills-index.json"
SLICK_INDEX_TTL = 6 * 3600  # 6 hours


def _slick_index_cache_file() -> Path:
    return _index_cache_dir() / "slick-index.json"


def _load_slick_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for SLICK_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    slick_index_cache_file = _slick_index_cache_file()
    if slick_index_cache_file.exists():
        try:
            age = time.time() - slick_index_cache_file.stat().st_mtime
            if age < SLICK_INDEX_TTL:
                return json.loads(slick_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Slick index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Slick index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Slick index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        slick_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        slick_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class SlickIndexSource(SkillSource):
    """Skill source backed by the centralized Slick Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "slick-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "slick-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        SlickIndexSource(auth=auth), # Centralized index (search + resolved install paths)
