...
One ``CDPSupervisor`` runs per Slick ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Slick logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "slick-dialog-bridge.invalid"
...
(() => {
  if (window.__slickDialogBridgeInstalled) return;
  window.__slickDialogBridgeInstalled = true;
  const ENDPOINT = "http://slick-dialog-bridge.invalid/";
