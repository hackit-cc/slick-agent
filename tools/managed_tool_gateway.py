"""Generic managed-tool gateway helpers for Hackit-hosted vendor passthroughs."""
...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "hackit.cc"
_DEFAULT_TOOL_GATEWAY_SCHEME = "https"
_HACKIT_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120
...
def _read_hackit_provider_state() -> Optional[dict]:
    try:
        path = auth_json_path()
        if not path.is_file():
            return None
        ...
        if not isinstance(providers, dict):
            return None
        hackit_provider = providers.get("hackit", {})
        if isinstance(hackit_provider, dict):
            return hackit_provider
    except Exception:
        pass
    return None
...
def peek_hackit_access_token() -> Optional[str]:
    """Cheap probe for a Hackit gateway token without triggering refresh.
    ...
    that call :func:`read_hackit_access_token`.
    """
    explicit = _read_user_token_override()
    if explicit:
        return explicit

    hackit_provider = _read_hackit_provider_state() or {}
    access_token = hackit_provider.get("access_token")
    if isinstance(access_token, str) and access_token.strip():
        return access_token.strip()
    return None


def read_hackit_access_token() -> Optional[str]:
    """Read a Hackit Subscriber OAuth access token from auth store or env override."""
    explicit = _read_user_token_override()
    if explicit:
        return explicit
    hackit_provider = _read_hackit_provider_state() or {}
    cached_token = peek_hackit_access_token()

    if cached_token and not _access_token_is_expiring(
        hackit_provider.get("expires_at"),
        _HACKIT_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
    ):
        return cached_token

    try:
        from slick_cli.auth import resolve_hackit_access_token

        refreshed_token = resolve_hackit_access_token(
            refresh_skew_seconds=_HACKIT_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
        )
        if isinstance(refreshed_token, str) and refreshed_token.strip():
            return refreshed_token.strip()
    except Exception as exc:
        logger.debug("Hackit access token refresh failed: %s", exc)
...
def resolve_managed_tool_gateway(
    ...
    if not managed_hackit_tools_enabled():
        return None

    resolved_gateway_builder = gateway_builder or build_vendor_gateway_url
    resolved_token_reader = token_reader or read_hackit_access_token

    gateway_origin = resolved_gateway_builder(vendor)
    hackit_user_token = resolved_token_reader()
    if not gateway_origin or not hackit_user_token:
        return None

    return ManagedToolGatewayConfig(
        vendor=vendor,
        gateway_origin=gateway_origin,
        hackit_user_token=hackit_user_token,
        managed_mode=True,
    )
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and a likely-usable Hackit token are present.

    Defaults to :func:`peek_hackit_access_token` so read-only availability scans
    ...
    still defaults to the refresh-aware :func:`read_hackit_access_token`).
    """
    return resolve_managed_tool_gateway(
        vendor,
        gateway_builder=gateway_builder,
        token_reader=token_reader or peek_hackit_access_token,
    ) is not None
...
#
# The gateway exposes a Hackit-owned REST contract per vendor; it names the
...
def is_managed_hackit_gateway_url(
    url: object,
    gateway_builder: Optional[Callable[[str], str]] = None,
) -> bool:
    """True when ``url`` is on the Hackit tool-gateway origin this client builds.
...
def managed_gateway_auth_headers(
    ...
    Read fresh on every call rather than cached: a Hackit access token expires
    ...
    """
    if not is_managed_hackit_gateway_url(url, gateway_builder):
        return {}
...
# the tool argument carries an opaque `hackit-upload:<token>` reference instead.
#
# The protocol lives HERE rather than in a vendor tool module: the presign
# request shape, the response contract, and the `hackit-upload:` scheme are Hackit
...
def build_managed_media_uploader(
    ...
    Hackit URL, or no ``upload_path``); callers then refuse local paths with a
    ...
    3. Return ``hackit-upload:<token>`` for the tool argument. The token is
       bound to this Hackit principal and is redeemable only through the
       gateway, so it is inert anywhere else it might end up.
    """
    if not is_managed_hackit_gateway_url(server_url, gateway_builder):
        return None
    ...
    async def upload(data: bytes, mime: str) -> str:
        ...
        if not headers:
            raise RuntimeError("no Hackit credential is available for the upload")
        ...
        # is_managed_hackit_gateway_url) plus the pinned upload_path — the same
