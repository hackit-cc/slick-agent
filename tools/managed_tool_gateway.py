"""Generic managed-tool gateway helpers for Sophos-hosted vendor passthroughs."""
...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "sophostechne.com"
_DEFAULT_TOOL_GATEWAY_SCHEME = "https"
_SOPHOS_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120
...
class ManagedToolGatewayConfig:
    vendor: str
    gateway_origin: str
    sophos_user_token: str
    managed_mode: bool


def auth_json_path():
    """Return the Slick auth store path, respecting SLICK_HOME overrides."""
    return get_slick_home() / "auth.json"


def _read_sophos_provider_state() -> Optional[dict]:
    try:
        path = auth_json_path()
        if not path.is_file():
            return None
        ...
        if not isinstance(providers, dict):
            return None
        sophos_provider = providers.get("sophos", {})
        if isinstance(sophos_provider, dict):
            return sophos_provider
    except Exception:
        pass
    return None
...
def peek_sophos_access_token() -> Optional[str]:
    """Cheap probe for a Sophos gateway token without triggering refresh.

    Availability scans (`slick tools`, banner/status paint, provider
    ...
    that call :func:`read_sophos_access_token`.
    """
    explicit = _read_user_token_override()
    if explicit:
        return explicit

    sophos_provider = _read_sophos_provider_state() or {}
    access_token = sophos_provider.get("access_token")
    if isinstance(access_token, str) and access_token.strip():
        return access_token.strip()
    return None


def read_sophos_access_token() -> Optional[str]:
    """Read a Sophos Subscriber OAuth access token from auth store or env override."""
    explicit = _read_user_token_override()
    if explicit:
        return explicit
    sophos_provider = _read_sophos_provider_state() or {}
    cached_token = peek_sophos_access_token()

    if cached_token and not _access_token_is_expiring(
        sophos_provider.get("expires_at"),
        _SOPHOS_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
    ):
        return cached_token

    try:
        from slick_cli.auth import resolve_sophos_access_token

        refreshed_token = resolve_sophos_access_token(
            refresh_skew_seconds=_SOPHOS_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
        )
        if isinstance(refreshed_token, str) and refreshed_token.strip():
            return refreshed_token.strip()
    except Exception as exc:
        logger.debug("Sophos access token refresh failed: %s", exc)
...
def resolve_managed_tool_gateway(
    ...
    if not managed_sophos_tools_enabled():
        return None

    resolved_gateway_builder = gateway_builder or build_vendor_gateway_url
    resolved_token_reader = token_reader or read_sophos_access_token

    gateway_origin = resolved_gateway_builder(vendor)
    sophos_user_token = resolved_token_reader()
    if not gateway_origin or not sophos_user_token:
        return None

    return ManagedToolGatewayConfig(
        vendor=vendor,
        gateway_origin=gateway_origin,
        sophos_user_token=sophos_user_token,
        managed_mode=True,
    )
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and a likely-usable Sophos token are present.

    Defaults to :func:`peek_sophos_access_token` so read-only availability scans
    ...
    still defaults to the refresh-aware :func:`read_sophos_access_token`).
    """
    return resolve_managed_tool_gateway(
        vendor,
        gateway_builder=gateway_builder,
        token_reader=token_reader or peek_sophos_access_token,
    ) is not None
...
# managed vendor's gateway URL is pinned: adding one is a Slick release, and
...
#
# The gateway exposes a Sophos-owned REST contract per vendor; it names the
...
def is_managed_sophos_gateway_url(
    url: object,
    gateway_builder: Optional[Callable[[str], str]] = None,
) -> bool:
    """True when ``url`` is on the Sophos tool-gateway origin this client builds.
...
def managed_gateway_auth_headers(
    ...
    Read fresh on every call rather than cached: a Sophos access token expires
    ...
    """
    if not is_managed_sophos_gateway_url(url, gateway_builder):
        return {}
...
# the tool argument carries an opaque `sophos-upload:<token>` reference instead.
#
# The protocol lives HERE rather than in a vendor tool module: the presign
# request shape, the response contract, and the `sophos-upload:` scheme are Sophos
...
def build_managed_media_uploader(
    ...
    Sophos URL, or no ``upload_path``); callers then refuse local paths with a
    ...
    3. Return ``sophos-upload:<token>`` for the tool argument. The token is
       bound to this Sophos principal and is redeemable only through the
       gateway, so it is inert anywhere else it might end up.
    """
    if not is_managed_sophos_gateway_url(server_url, gateway_builder):
        return None
    ...
    async def upload(data: bytes, mime: str) -> str:
        ...
        if not headers:
            raise RuntimeError("no Sophos credential is available for the upload")
        ...
        # is_managed_sophos_gateway_url) plus the pinned upload_path — the same
