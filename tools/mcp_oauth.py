...
URL of Slick' published Client ID Metadata Document (CIMD) as the
...
This module provides the glue:
    - ``SlickTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Slick Agent"
...
from slick_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__slick_user_skipped__"
...
def _get_token_dir(slick_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses SLICK_HOME so each profile gets its own OAuth tokens.
    Layout: ``SLICK_HOME/mcp-tokens/``
    """
    from slick_constants import get_slick_home

    base = Path(slick_home) if slick_home is not None else Path(get_slick_home())
...
def _cached_redirect_port(storage: "SlickTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Slick restarts and chooses a
...
def _cached_redirect_uri(storage: "SlickTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``slick mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `slick mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# SlickTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class SlickTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        SLICK_HOME/mcp-tokens/<server_name>.json         -- tokens
        SLICK_HOME/mcp-tokens/<server_name>.client.json   -- client info
        SLICK_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
        SLICK_HOME/mcp-tokens/<server_name>.cimd-off      -- CIMD refused here
    """

    def __init__(self, server_name: str, *, slick_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._slick_home = Path(slick_home) if slick_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._slick_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._slick_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._slick_home) / f"{self._server_name}.meta.json"

    def _cimd_rejected_path(self) -> Path:
        return _get_token_dir(self._slick_home) / f"{self._server_name}.cimd-off"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Slick records an absolute wall-clock ``expires_at`` alongside the
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``slick mcp login`` /
        ``slick mcp remove``, so a fixed document gets another chance.
        """
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._slick_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Slick.</p></body></html>"
            ) if code else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://slick-agent.sophostechne.com/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"Slick' Client ID Metadata Document ({cimd_url}); set "
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `slick mcp login <server>` later to "
...
SlickOAuthClientProvider: Any = None


def _get_slick_oauth_provider_class() -> type | None:
    global SlickOAuthClientProvider
    if SlickOAuthClientProvider is not None:
        return SlickOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _SlickOAuthClientProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, token_user_agent: "str | None" = None, **kwargs: Any):
            ...
            self._slick_token_user_agent = token_user_agent

        def _stamp_token_user_agent(self, request):
            ua = getattr(self, "_slick_token_user_agent", None)
            if ua:
    ...
    _SlickOAuthClientProvider.__name__ = "SlickOAuthClientProvider"
    _SlickOAuthClientProvider.__qualname__ = "SlickOAuthClientProvider"
    SlickOAuthClientProvider = _SlickOAuthClientProvider
    return SlickOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = SlickTokenStorage(server_name, slick_home=slick_home)
    storage.remove()
...
# 2026-07-28. The SDK does the protocol work; Slick only decides whether a
...
# slick-agent.sophostechne.com/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = (
    "https://sophostechne.github.io/slick-agent/docs/oauth/client-metadata.json"
)
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port Slick picks
...
def _has_cached_client_info(storage: "SlickTokenStorage | None") -> bool:
...
def _server_declined_cimd(storage: "SlickTokenStorage | None") -> bool:
    ...
    Slick has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    cfg: dict,
    storage: "SlickTokenStorage | None" = None,
) -> "tuple[str, int] | None":
    """Return ``(client_id URL, pinned callback port)``, or None to use DCR.

    Every early return below is a case where the redirect URI Slick would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Slick Agent" / "Slick" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Slick Agent")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. Slick is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: slick mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Slick auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Slick is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = SlickTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`slick mcp login {server_name}` interactively first to complete "
    ...
    provider_class = _get_slick_oauth_provider_class()
