...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Slick's live source checkout "
        ...
        "checkout, stop Slick, run the command externally, then restart "
        "Slick."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(slick_home).expanduser() if slick_home else Path.home() / ".slick"
