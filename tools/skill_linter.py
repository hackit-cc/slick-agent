...
Design contract (matches the Slick "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    slick_meta = meta.get("slick") if isinstance(meta, dict) else None
    if not isinstance(slick_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.slick.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in slick_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.slick.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("slick", "agent", "slick agent") and (
        author != "Slick Agent"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Slick Agent' (proper caps) "
