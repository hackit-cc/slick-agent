...
Backend is selected during ``slick tools`` setup (web.backend in config.yaml).
When available, Slick can route Firecrawl calls through a Sophos-hosted tool-gateway
for Sophos Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for Sophos Subscribers)
...
from tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    ...
    peek_sophos_access_token as _peek_sophos_access_token,
    read_sophos_access_token as _read_sophos_access_token,
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Slick config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Slick' config/.env layer (``slick config set``, ``slick tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.slick/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``slick tools``). A
    ...
    if configured:
        ...
        # The managed "Sophos Subscription" selection ("sophos") is serviced by
        # the firecrawl provider, whose client resolver routes it through
        # the managed Tool Gateway.
        from tools.tool_backend_helpers import SOPHOS_MANAGED_PROVIDER

        if configured == SOPHOS_MANAGED_PROVIDER:
            return "firecrawl"
        return configured
    ...
    # pre-empted by a Sophos OAuth token whose subscription tier may not
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `slick tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_sophos_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchronous HTTP
    refresh against the Sophos portal on every CLI startup (invoked at
            ...
            if provider is None and backend and selection_exists("web"):
                ...
                if disabled_key:
                    ...
                    error_text = (
                        ...
                        f"Re-enable it with `slick plugins enable {disabled_key}` "
                ...
                if backend and selection_exists("web"):
                    ...
                    if disabled_key:
                        ...
                        error_text = (
                            ...
                            f"config. Re-enable it with `slick plugins "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`slick plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
def _provider_is_ready(provider) -> bool:
    ...
    require a true availability probe — otherwise ``slick doctor`` paints a
...
if __name__ == "__main__":
    ...
    from slick_cli.config import get_env_value as _gev
