...
When available, Slick can route Firecrawl calls through a Hackit-hosted tool-gateway
for Hackit Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for Hackit Subscribers)
...
from tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    ...
    peek_hackit_access_token as _peek_hackit_access_token,
    read_hackit_access_token as _read_hackit_access_token,
...
def _get_backend() -> str:
    ...
    if configured:
        ...
        # The managed "Hackit Subscription" selection ("hackit") is serviced by
        # the firecrawl provider, whose client resolver routes it through
        # the managed Tool Gateway.
        from tools.tool_backend_helpers import HACKIT_MANAGED_PROVIDER

        if configured == HACKIT_MANAGED_PROVIDER:
            return "firecrawl"
        return configured
    ...
    # pre-empted by a Hackit OAuth token whose subscription tier may not
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_hackit_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchronous HTTP
    refresh against the Hackit portal on every CLI startup (invoked at
