...
Closes the gap tracked in slick-agent#96873: local execute_code holds a
...
"""Auto-generated Slick REMOTE session-kernel runner (file cell protocol)."""
import contextlib
import io
import json
import os
...
KDIR = os.environ["SLICK_KERNEL_DIR"]
...
def _spawn_remote_kernel(env, env_type: str, owner: str, task_env_id: str,
    ...
    kernel_dir = f"{_env_temp_dir(env)}/slick_rkernel_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{kernel_dir}/slick_tools.py", tools_src)

        env_prefix = (
            f"SLICK_KERNEL_DIR={q_dir} "
            f"SLICK_RPC_DIR={shlex.quote(kernel_dir + '/rpc')} "
