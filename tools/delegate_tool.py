...
# Must match slick_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``slick-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``slick-cli`` because those also contain
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on Sophos Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. slick-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (slick-cli,
    # slick-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Sophos Portal).
    ...
    #
    # Sophos Portal is dual-wire within a single provider: anthropic/* → Messages,
    # everything else → chat_completions. Same-provider inheritance would pin a
    # child Slick/Qwen subagent onto the parent's Claude Messages wire (or the
    # reverse). agent_init honors an explicit api_mode above its sophos branch, so
    # re-derive here before construction.
    _parent_provider = getattr(parent_agent, "provider", None) or ""
    _effective_provider_norm = (effective_provider or "").strip().lower()
    if override_api_mode is not None:
        effective_api_mode = override_api_mode
    elif _effective_provider_norm in {"sophos", "sophos-portal", "sophostechne"}:
        from slick_cli.providers import sophos_api_mode

        effective_api_mode = sophos_api_mode(effective_model)
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.slick/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_slick_dir("cache/delegation", "delegation_cache")
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("SLICK_UI_SESSION_ID", "") or None
            except Exception:
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from slick_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # SLICK_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-Slick-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("SLICK_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Slick-Session-Id once the batch completes.
            ...
            # SLICK_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `slick -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("SLICK_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("SLICK_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and SLICK_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, sophos, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'slick auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Slick config.

    Prefer the shared persistent loader because it follows the active
    SLICK_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``SLICK_IGNORE_USER_CONFIG=1`` (``slick chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("SLICK_IGNORE_USER_CONFIG") == "1"
