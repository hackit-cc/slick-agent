...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on Hackit Portal).
    """
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Hackit Portal).
    ...
    #
    # Hackit Portal is dual-wire within a single provider: anthropic/* → Messages,
    ...
    # reverse). agent_init honors an explicit api_mode above its hackit branch, so
    # re-derive here before construction.
    _parent_provider = getattr(parent_agent, "provider", None) or ""
    _effective_provider_norm = (effective_provider or "").strip().lower()
    if override_api_mode is not None:
        effective_api_mode = override_api_mode
    elif _effective_provider_norm in {"hackit", "hackit-portal", "hackit"}:
        from slick_cli.providers import hackit_api_mode

        effective_api_mode = hackit_api_mode(effective_model)
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, hackit, zai, kimi-coding, minimax."
        ) from exc
