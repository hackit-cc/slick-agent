...
Slick Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own SLICK root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in slick_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "slick" if profile == "default" else profile
    if (
...
def _slick_cli() -> str:
    """Resolve the slick CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``slick``
    entrypoint. A bare ``"slick"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("slick.exe" if sys.platform == "win32" else "slick")
    if sibling.is_file():
        return str(sibling)
    found = shutil.which("slick")
    if found:
        return found
    return "slick"
...
# turns concurrently: deliveries spawn separate ``slick`` subprocesses, so
