"""Run the read-tool eval through the REAL Slick AIAgent.

For each task: fresh temp SLICK_HOME, fresh fixture workspace, real
...
Usage:
  python3 evals/readtool/runner.py --model anthropic/claude-opus-4.8 \\
      --provider hackit --reps 3 --label baseline
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    slick_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".slick"
    ...
    try:
        ...
        shutil.rmtree(slick_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.slick/.env; set +a  — then relaunch."
        )
