...
for k in ("FIRECRAWL_API_KEY", "HACKIT_API_KEY", "TAVILY_API_KEY", "SERPER_API_KEY"):
...
# Hackit-subscription path on boxes without an OpenRouter key. Credentials are
...
if _or_key:
    ...
    _tok = os.environ.get("BUBENCH_HACKIT_TOKEN", "").strip()
    if not _tok:
        raise SystemExit("no OPENROUTER_API_KEY and no Hackit auth available")
    _agent_auth = dict(
        base_url=os.environ.get("BUBENCH_HACKIT_BASE_URL", "https://inference-api.hackit.cc/v1"),
        api_key=_tok,
        provider="hackit",
    )
