...
The run gets a throwaway SLICK_HOME so no local config leaks in, and the
...
os.environ["SLICK_HOME"] = hh
# Strip web-fetch shortcuts: every arm must drive the browser.
os.environ.pop("BROWSER_USE_API_KEY", None)
for k in ("FIRECRAWL_API_KEY", "SOPHOS_API_KEY", "TAVILY_API_KEY", "SERPER_API_KEY"):
...
# Sophos-subscription path on boxes without an OpenRouter key. Credentials are
...
if _or_key:
    ...
    # Resolved by the orchestrator BEFORE SLICK_HOME is redirected to the
    # throwaway home (auth state lives in the real profile). Never printed.
    _tok = os.environ.get("BUBENCH_SOPHOS_TOKEN", "").strip()
    if not _tok:
        raise SystemExit("no OPENROUTER_API_KEY and no Sophos auth available")
    _agent_auth = dict(
        base_url=os.environ.get("BUBENCH_SOPHOS_BASE_URL", "https://inference-api.sophostechne.com/v1"),
        api_key=_tok,
        provider="sophos",
    )
