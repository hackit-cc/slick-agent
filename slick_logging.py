"""Centralized logging setup for Slick Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.slick/logs/`` (profile-aware via ``get_slick_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``slick_cli.web_server``,
    ``slick_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Slick (TUI, gateway, ``hy_memory`` server, MCP
...
from slick_constants import get_config_path, get_slick_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_slick_session_injector", False):
    ...
    _session_record_factory._slick_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``slick logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``slick logs --component gateway``.
    "gateway": ("gateway", "slick_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("slick_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "slick_cli.web_server",
        "slick_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Slick logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    slick_home
        Override for the Slick home directory.  Falls back to
        ``get_slick_home()`` (profile-aware).
    log_level
    ...
    home = slick_home or get_slick_home()
    from slick_constants import mkdir_under_slick_home
    log_dir = mkdir_under_slick_home(home / "logs")
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_slick_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._slick_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the slick group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from slick_cli.config import is_managed
...
# the module header): when several Slick processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._slick_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="slick-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_slick_queue", False):
...
def _add_rotating_handler(
    ...
    *,
    ...
    from slick_constants import mkdir_under_slick_home
    mkdir_under_slick_home(path.parent)
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse slick_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when slick_cli.config isn't importable (bare
        # slick_logging consumers).
        try:
            from slick_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
